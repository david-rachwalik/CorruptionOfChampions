...
ConsumableEffects.fishFillet = function () {
    ...
    //Turned off factory
    else if (gameFlags[FACTORY_SHUTDOWN] == 1)
...
ConsumableEffects.scholarsTea = function () {
    ...
    //Now NERFED!
    if (player.inte < 40)
        player.modStats("int", 1.5 + rand(4));
    else if (player.inte < 60)
        player.modStats("int", 1 + rand(3));
    else if (player.inte < 80)
        player.modStats("int", 0.5 + rand(2));
    else
        player.modStats("int", 0.2 + rand(2));
    player.refillHunger(10);
};
//Hair Dyes
...
ConsumableEffects.minotaurCum = function (purified) {
    ...
    if (gameFlags[MINOTAUR_CUM_ADDICTION_STATE] == 0 && player.findPerk(PerkLib.MinotaurCumResistance) < 0)
        outputText("potent musk washes over you.", false);
    else
        outputText("heavenly scent fills your nostrils.", false);
    if (!purified) {
        if (gameFlags[MINOTAUR_CUM_ADDICTION_TRACKER] < 50)
    ...
    if (player.hasVagina()) {
        ...
        if (player.vaginas[0].vaginalWetness <= VAGINA_WETNESS_NORMAL)
            outputText("  Wetness builds inside you as your " + player.vaginaDescript(0) + " tingles and aches to be filled.", false);
        else if (player.vaginas[0].vaginalWetness <= VAGINA_WETNESS_SLICK)
            outputText("  A trickle of wetness escapes your " + player.vaginaDescript(0) + " as your body reacts to the desire burning inside you.", false);
