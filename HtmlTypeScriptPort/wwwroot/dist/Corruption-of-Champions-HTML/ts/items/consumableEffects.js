...
import { ReductoMenu } from "./specialConsumables/reducto.js";
...
class ConsumableEffects {
    ...
    static fishFillet() {
        GUI.clearOutput();
        if (!COMBAT.inCombat)
            GUI.outputText("You sit down and unwrap your fish fillet. It's perfectly flaky, allowing you to break it off in bite-sized chunks.  The salty meal disappears quickly, and your stomach gives an appreciative gurgle.");
        //(In combat?)
        else
            GUI.outputText("You produce the fish fillet from your bag.  Rather than unwrap it and savor the taste as you normally would, you take a large bite out of it, leaf wrapping and all.  In no time your salty meal is gone, your stomach giving an appreciative gurgle.  ");
        //Blew up factory
    ...
    static lactaid() {
        ...
        if (liveData.player.biggestLactation() < 1) {
            GUI.outputText("<br><br>");
            GUI.outputText("You feel your " + liveData.player.nippleDescript(0) + "s become tight and engorged.  A single droplet of milk escapes each, rolling down the curves of your breasts.  <b>You are now lactating!</b>");
    ...
    static lustDraft(fuck) {
        ...
        if (liveData.player.lust >= liveData.player.maxLust() && !COMBAT.inCombat) {
            GUI.outputText("<br><br>The arousal from the potion overwhelms your senses and causes you to spontaneously orgasm.  You rip off your " + liveData.player.armor.equipmentName + " and look down as your ");
            ...
            if (liveData.player.gender == 0)
                GUI.outputText("body begins to quiver with orgasmic bliss.  ");
            GUI.outputText("Once you've had a chance to calm down, you notice that the explosion of pleasure you just experienced has rocked you to your core.  You are a little hornier than you were before.");
    ...
    static vitalityTincture() {
        liveData.player.slimeFeed();
        GUI.outputText("You down the contents of the bottle. The liquid is thick and tastes remarkably like cherries. Within moments, you feel much more fit and healthy.");
        //Strength changes
    ...
    static hairDye(newColor) {
        if (liveData.player.hairLength == 0) {
            ...
            GUI.outputText("You massage the dye into your " + liveData.player.hairDescript() + " but the dye cannot penetrate the impermeable material your hair is composed of.");
        }
        else {
            GUI.outputText("You rub the dye into your " + liveData.player.hairDescript() + ", then use a bucket of cool lakewater to rinse clean a few minutes later. ");
        ...
        if (liveData.player.skinTone == newColor) {
            ...
            switch (liveData.player.skinType) {
                ...
                case 2: //Scales
                    ...
                    break;
                case 3: //Goo
                    GUI.outputText("You take the oil and pour the contents into your skin. The clear liquid dissolves, leaving your gooey skin unchanged. You do feel a little less thirsty though.");
                    liveData.player.slimeFeed();
                    break;
                default:
                    GUI.outputText("You " +
        ...
        if (liveData.player.skinTone == newAdj) {
            ...
            switch (liveData.player.skinType) {
                ...
                case 2: //Scales
                    ...
                    break;
                case 3: //Goo
                    GUI.outputText("You take the lotion and pour the " +
                        this.liquidDesc(newAdj) +
    ...
    static tatteredScroll() {
        GUI.outputText("Your wobbly " + liveData.player.legs() + " give out underneath you as your body's willpower seems to evaporate, your mouth reading the words on the scroll with a backwards sounding sing-song voice.<br><br>");
        if (liveData.player.hairColor == "sandy blonde") {
            ...
            if (liveData.player.biggestLactation() > 0) {
                ...
                if (liveData.player.biggestLactation() < 2)
                    GUI.outputText("powerful jets of milk spray from your nipples, spraying thick streams over the ground.  You moan at the sensation and squeeze your tits, hosing down the tainted earth with an offering of your milk.  You blush as the milk ends, quite embarassed with your increased milk production.  ");
                ...
                if (liveData.player.biggestLactation() > 2.6 && liveData.player.biggestLactation() < 3)
                    GUI.outputText("thick hoses of milk erupt from your aching nipples, forming puddles on the ground.  You smile at how well you're feeding the earth, your milk coating the ground faster than it can be absorbed.  The constant lactation is pleasurable... in a highly erotic way, and you find yourself moaning and pulling on your nipples, your hands completely out of control.  In time you realize the milk has stopped, and even had time to soak into the dirt.  You wonder at your strange thoughts and pull your hands from your sensitive nipples.  ");
            ...
            if (liveData.player.biggestLactation() == 0) {
                GUI.outputText("A pleasurable release suddenly erupts from your nipples!  Twin streams of milk are spraying from your breasts, soaking into the ground immediately.  It stops all too soon, though a voice in your head assures you that you can lactate quite often now.  ");
            ...
            GUI.outputText('Your mouth forms a smile of its own volition, reading, "<i>nuf erutuf rof riah ydnas, nus tresed eht sa ydnas.</i>"<br><br>You feel a tingling in your scalp, and realize your hair has become a sandy blonde!');
    ...
    static blackSpellbook() {
        GUI.outputText("You open the small black book, and discover it to be an instructional book on the use of black magic.  Most of it is filled with generic information about black magic - how it is drawn from emotions (typically lust), and how it has the power to affect bodies and emotions.  It also warns against using it on oneself, as it is difficult to draw on your emotions while meddling with your own body.  In no time at all you've read the whole thing, but it disappears into thin air before you can put it away.");
    ...
    static whiteSpellBook() {
        GUI.outputText("You open the white tome, and discover it to be an instructional book on the use of white magic.  Most of it is filled with generic information about white magic - how it is drawn for mental focus, is difficult to use when tired or aroused, and can be used to create and control energy.  In no time at all you've read the whole thing, but it disappears into thin air before you can put it away.");
        if (liveData.player.inte < 30) {
            GUI.outputText("<br><br>You feel greatly enlightened by your time spent reading.");
            liveData.player.dynStats(["int", 4]);
        }
    ...
    static minotaurCum(purified) {
        ...
        if (liveData.gameFlags[FLAG.MINOTAUR_CUM_ADDICTION_STATE] == 0 && liveData.player.findPerk(PerkLib.MinotaurCumResistance) < 0)
            ...
            GUI.outputText("heavenly scent fills your nostrils.");
        if (!purified) {
            if (liveData.gameFlags[FLAG.MINOTAUR_CUM_ADDICTION_TRACKER] < 50)
                GUI.outputText("  It makes you feel dizzy, ditzy, and placid.");
            else
                GUI.outputText("  It makes you feel euphoric, happy, and willing to do ANYTHING to keep feeling this way.");
        }
        else
            GUI.outputText("  You know that the bottle is purified and you're positive you won't get any addiction from this bottle.");
        ...
        GUI.outputText("<br><br>Intermittent waves of numbness wash through your body, turning into a warm tingling that makes you feel sensitive all over.  The warmth flows through you, converging in your loins and bubbling up into lust.");
        if (liveData.player.cocks.length > 0) {
            GUI.outputText("  ");
        ...
        if (liveData.player.hasVagina()) {
            ...
            if (liveData.player.clitLength < 3)
                GUI.outputText("parting your lips.");
            else
                GUI.outputText("bursting free of your lips and bobbing under its own weight.");
        ...
        if (liveData.player.findPerk(PerkLib.MinotaurCumAddict) >= 0 && liveData.gameFlags[FLAG.MINOTAUR_CUM_REALLY_ADDICTED_STATE] <= 0 && !purified) {
            ...
            GUI.outputText("<br><br><b>Your body feels so amazing and sensitive.  Experimentally you pinch yourself and discover that even pain is turning you on!</b>");
        }
        //Clear mind a bit
    ...
    static oviElixir() {
        ...
        */
        ...
        if (liveData.player.pregnancyType == ENUM.PregnancyType.PREGNANCY_GOO_STUFFED) {
            GUI.outputText("<br><br>For a moment you feel even more bloated than you already are.  That feeling is soon replaced by a dull throbbing pain.  It seems that with Valeria's goo filling your womb the ovielixir is unable to work its magic on you.");
            return false;
        }
        if (liveData.player.pregnancyType == ENUM.PregnancyType.PREGNANCY_WORM_STUFFED) {
            GUI.outputText("<br><br>For a moment you feel even more bloated than you already are.  That feeling is soon replaced by a dull throbbing pain.  It seems that with the worms filling your womb the ovielixir is unable to work its magic on you.");
            return false;
        }
        if (liveData.player.pregnancyIncubation == 0) {
            //If the player is not pregnant, get preggers with eggs!
            GUI.outputText("<br><br>The elixir has an immediate effect on your belly, causing it to swell out slightly as if pregnant.  You guess you'll be laying eggs sometime soon!");
        ...
        if (!changeOccurred && liveData.player.pregnancyIncubation > 20 && liveData.player.pregnancyType != ENUM.PregnancyType.PREGNANCY_BUNNY) {
            ...
            GUI.outputText("<br><br>You gasp as your pregnancy suddenly leaps forwards, your belly bulging outward a few inches as it gets closer to time for birthing.");
            var newIncubation = liveData.player.pregnancyIncubation - (liveData.player.pregnancyIncubation * 0.3 + 10);
            if (newIncubation < 2)
                newIncubation = 2;
