...
let fishFillet = function () {
    ...
    //Turned off factory
    else if (liveData.gameFlags[FACTORY_SHUTDOWN] == 1)
...
let lactaid = function () {
    ...
    if (liveData.player.averageBreastSize() < 8) {
        ...
        if (liveData.player.breastRows.length == 1)
            liveData.player.growTits(1 + rand(5), 1, true, 1);
        else
            liveData.player.growTits(1 + rand(2), liveData.player.breastRows.length, true, 1);
    }
    //Player doesn't lactate
    if (liveData.player.biggestLactation() < 1) {
        outputText("<br><br>", false);
        outputText("You feel your " + liveData.player.nippleDescript(0) + "s become tight and engorged.  A single droplet of milk escapes each, rolling down the curves of your breasts.  <b>You are now lactating!</b>", false);
...
let lustDraft = function (fuck) {
    ...
    //ORGAZMO
    if (liveData.player.lust >= liveData.player.maxLust() && !inCombat) {
        outputText("<br><br>The arousal from the potion overwhelms your senses and causes you to spontaneously orgasm.  You rip off your " + liveData.player.armor.equipmentName + " and look down as your ", false);
        if (liveData.player.cocks.length > 0) {
            outputText(liveData.player.multiCockDescriptLight() + " erupts in front of you, liberally spraying the ground around you.  ", false);
        }
        ...
        if (liveData.player.vaginas.length > 0) {
            outputText(liveData.player.vaginaDescript(0) + " soaks your thighs.  ", false);
        }
...
let vitalityTincture = function () {
    ...
    if (rand(3) == 0)
        outputText(liveData.player.modTone(95, 3), false);
    liveData.player.refillHunger(10);
};
//Scholar's Tea
let scholarsTea = function () {
    liveData.player.slimeFeed();
    ...
    if (rand(3) == 0)
        outputText(liveData.player.modTone(15, 1), false);
    //Now NERFED!
    if (liveData.player.inte < 40)
        liveData.player.modStats("int", 1.5 + rand(4));
    else if (liveData.player.inte < 60)
        liveData.player.modStats("int", 1 + rand(3));
    else if (liveData.player.inte < 80)
        liveData.player.modStats("int", 0.5 + rand(2));
    else
        liveData.player.modStats("int", 0.2 + rand(2));
    liveData.player.refillHunger(10);
};
//Hair Dyes
let hairDye = function (newColor) {
...
//Skin Oils
let skinOil = function (newColor) {
    if (liveData.player.skinTone == newColor) {
        outputText("You " + liveData.player.clothedOrNaked("take a second to disrobe before uncorking the bottle of oil and rubbing", "uncork the bottle of oil and rub") + " the smooth liquid across your body. Once you’ve finished you feel rejuvenated. ");
...
//Body Lotions
let bodyLotion = function (newAdj) {
...
let tatteredScroll = function () {
    ...
    if (liveData.player.hairColor == "sandy blonde") {
        ...
        if (liveData.player.averageNipplesPerBreast() < 1) {
            ...
            var temp = liveData.player.breastRows.length;
            while (temp > 0) {
                temp--;
...
let blackSpellbook = function () {
    ...
    if (liveData.player.inte >= 25 && liveData.player.spells.arouse == 0) {
    ...
    //Smart enough for arouse and doesnt have it
    if (liveData.player.inte >= 30 && liveData.player.spells.heal == 0) {
    ...
    //Smart enough for arouse and doesnt have it
    if (liveData.player.inte >= 40 && liveData.player.spells.might == 0) {
...
let whiteSpellBook = function () {
    ...
    //Smart enough for arouse and doesnt have it
    if (liveData.player.inte >= 25 && liveData.player.spells.chargeWeapon == 0) {
    ...
    //Smart enough for arouse and doesnt have it
    if (liveData.player.inte >= 30 && liveData.player.spells.chargeWeapon == 0) {
    ...
    //Smart enough for arouse and doesnt have it
    if (liveData.player.inte >= 40 && liveData.player.spells.whitefire == 0) {
...
// Minotaur Cum
let minotaurCum = function (purified) {
    liveData.player.slimeFeed();
    clearOutput();
    ...
    if (liveData.gameFlags[MINOTAUR_CUM_ADDICTION_STATE] == 0 && liveData.player.findPerk(PerkLib.MinotaurCumResistance) < 0)
        outputText("potent musk washes over you.", false);
    else
        outputText("heavenly scent fills your nostrils.", false);
    if (!purified) {
        if (liveData.gameFlags[MINOTAUR_CUM_ADDICTION_TRACKER] < 50)
    ...
    if (liveData.player.cocks.length > 0) {
        ...
        outputText("our " + liveData.player.multiCockDescriptLight() + " aches, flooding with blood until it's bloating and trembling.", false);
    }
    if (liveData.player.hasVagina()) {
        outputText("  Your " + liveData.player.clitDescript() + " engorges, ", false);
        ...
        if (liveData.player.vaginas[0].vaginalWetness <= VAGINA_WETNESS_NORMAL)
            outputText("  Wetness builds inside you as your " + liveData.player.vaginaDescript(0) + " tingles and aches to be filled.", false);
        else if (liveData.player.vaginas[0].vaginalWetness <= VAGINA_WETNESS_SLICK)
            outputText("  A trickle of wetness escapes your " + liveData.player.vaginaDescript(0) + " as your body reacts to the desire burning inside you.", false);
        else if (liveData.player.vaginas[0].vaginalWetness <= VAGINA_WETNESS_DROOLING)
    ...
    //Clear mind a bit
    if (purified && (liveData.player.findPerk(PerkLib.MinotaurCumAddict) >= 0 || liveData.gameFlags[MINOTAUR_CUM_ADDICTION_TRACKER] >= 40)) {
        ...
        if (liveData.player.findPerk(PerkLib.MinotaurCumAddict) >= 0 && liveData.gameFlags[MINOTAUR_CUM_ADDICTION_TRACKER] <= 50) {
...
export { fishFillet, lactaid, lustDraft, vitalityTincture, scholarsTea, hairDye, skinOil, bodyLotion, liquidDesc, tatteredScroll, blackSpellbook, whiteSpellBook, minotaurCum };
