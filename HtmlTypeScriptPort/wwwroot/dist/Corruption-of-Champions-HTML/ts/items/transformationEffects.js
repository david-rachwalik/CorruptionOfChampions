...
class TransformationEffects {
    static beeTFs(type) {
        ...
        //Drink text
        if (special) {
            GUI.outputText("You uncork the bottle and pour the incredibly strong smelling concentrated honey down your throat. Its taste is also mighty intense. All at once you feel the effects of the substance start to course through your body.");
        }
        else {
            //Text for normal or pure
            GUI.outputText("Opening the crystal vial, you are greeted by a super-concentrated wave of sweet honey-scent. It makes you feel lightheaded. You giggle and lick the honey from your lips, having drank down the syrupy elixir without a thought.");
        }
        ...
        if ((pure || special) && liveData.player.pregnancyType == ENUM.PregnancyType.PREGNANCY_FAERIE) {
            ...
            if (liveData.gameFlags[FLAG.PREGNANCY_CORRUPTION] > 1) {
                ...
                GUI.outputText("<br><br>You feel queasy and want to throw up.  There's a pain in your belly and you realize the baby you're carrying didn't like that at all.  Then again, maybe pure honey is good for it.");
            }
            else if (liveData.gameFlags[FLAG.PREGNANCY_CORRUPTION] < 1) {
                //Child is faerie, loves pure honey
                GUI.outputText("<br><br>A warm sensation starts in your belly and runs all through your body.  It's almost as if you're feeling music and you guess your passenger enjoyed the meal.");
            }
            else {
                //Child is on the line, will become a faerie with this drink
                GUI.outputText("<br><br>At first you feel your baby struggle against the honey, then it seems to grow content and enjoy it.");
            }
        ...
        if (changes < changeLimit && pure) {
            ...
            if (liveData.player.cor > 80)
                GUI.outputText("Your head aches, as if thunder was echoing around your skull. ");
            else if (liveData.player.cor > 60)
                GUI.outputText("You feel a headache forming just behind your eyes. In no time flat it reaches full strength. ");
            else if (liveData.player.cor > 40)
                GUI.outputText("A wave of stinging pain slices through your skull. ");
            else if (liveData.player.cor > 20)
                GUI.outputText("A prickling pain spreads throughout your skull. ");
            else
                GUI.outputText("You feel a mildly unpleasant tingling inside your skull. ");
            if (liveData.player.cor > 0)
                GUI.outputText("It quickly passes, leaving you more clearheaded");
            ...
            if (liveData.player.cor > 0 && changes < changeLimit && UTIL.rand(1.5) == 0 && liveData.player.lib > 40) {
                GUI.outputText(" and settling your overcharged sex-drive a bit.");
        ...
        if (changes < changeLimit && liveData.player.hairType == 4 && UTIL.rand(2) == 0) {
            ...
            GUI.outputText("<br><br>As you down the sticky-sweet honey, your head begins to feel heavier. Reaching up, you notice your tentacles becoming soft and somewhat fibrous. Pulling one down reveals that it feels and smells like the honey you just ate; you watch as it dissolves into many thin strands coated in the sugary syrup. <b>Your hair is back to normal (well, once you wash the honey out)!</b>");
        ...
        if (changes < changeLimit && UTIL.rand(2) == 0 && liveData.player.inte < 80) {
            ...
            GUI.outputText("<br><br>You spend a few moments analyzing the taste and texture of the honey's residue, feeling awfully smart.");
            changes++;
        }
        //Sexual Stuff
        //No idears
        ...
        if (changes < changeLimit && liveData.player.hairColor != "shiny black" && liveData.player.hairColor != "black and yellow" && liveData.player.hairLength > 10 && UTIL.rand(5) == 0) {
            GUI.outputText("<br><br>You feel your scalp tingling, and you grab your hair in a panic, pulling a strand forward. ");
            if (UTIL.rand(9) == 0)
                liveData.player.hairColor = "black and yellow";
            else
                liveData.player.hairColor = "shiny black";
            GUI.outputText("Your hair is now " + liveData.player.hairColor + ", just like a bee-girl's!");
            changes++;
        }
        //Hair Length
        if (changes < changeLimit && liveData.player.hairLength < 25 && UTIL.rand(3) == 0) {
            GUI.outputText("<br><br>Feeling a bit off-balance, you discover your hair has lengthened, ");
            liveData.player.hairLength += UTIL.rand(4) + 1;
            GUI.outputText("becoming " + liveData.player.hairDescript() + ".");
            changes++;
        }
        //-Remove extra breast rows
        if (changes < changeLimit && liveData.player.bRows() > 2 && UTIL.rand(3) == 0 && !liveData.hyperHappy) {
            changes++;
            GUI.outputText("<br><br>You stumble back when your center of balance shifts, and though you adjust before you can fall over, you're left to watch in awe as your bottom-most " +
            ...
            if (liveData.player.bRows() >= 3)
                GUI.outputText("abdomen");
            else
                GUI.outputText("chest");
            ...
            if (liveData.player.skinType == ENUM.SkinType.SKIN_TYPE_FUR)
                GUI.outputText(liveData.player.hairColor + " " + liveData.player.skinDesc);
            else
                GUI.outputText(liveData.player.skinTone + " " + liveData.player.skinDesc);
        ...
        if (changes < changeLimit && liveData.player.lowerBody != ENUM.LowerBodyType.LOWER_BODY_TYPE_BEE && UTIL.rand(4) == 0) {
            GUI.outputText("<br><br>Your legs tremble with sudden unbearable pain, as if they're being ripped apart from the inside out and being stitched together again all at once. You scream in agony as you hear bones snapping and cracking. A moment later the pain fades and you are able to turn your gaze down to your beautiful new legs, covered in shining black chitin from the thigh down, and downy yellow fuzz along your upper thighs.");
        ...
        if (liveData.player.averageNipplesPerBreast() > 1 && changes < changeLimit && UTIL.rand(4) == 0) {
            GUI.outputText("<br><br>A chill runs over your " +
        ...
        if (changes < changeLimit && liveData.player.tailType != ENUM.TailType.TAIL_TYPE_BEE_ABDOMEN && (liveData.player.tailType == ENUM.TailType.TAIL_TYPE_NONE || UTIL.rand(1.5) == 0) && UTIL.rand(4) == 0) {
            if (liveData.player.tailType > ENUM.TailType.TAIL_TYPE_NONE)
                GUI.outputText("<br><br>Painful swelling just above your " +
        ...
        if (changes < changeLimit && liveData.player.tailType == ENUM.TailType.TAIL_TYPE_BEE_ABDOMEN && liveData.player.tailRecharge < 15 && UTIL.rand(2)) {
            ...
            GUI.outputText("<br><br>Your abdomen swells with vitality and a drop of venom escapes your stinger as it begins producing it in slightly larger quantities.");
            changes++;
        }
        //Wings
        ...
        if (changes < changeLimit && liveData.player.wingType == ENUM.WingType.WING_TYPE_BEE_LIKE_SMALL && UTIL.rand(4)) {
            ...
            GUI.outputText("<br><br>Your wings tingle as they grow, filling out until they are large enough to lift you from the ground and allow you to fly! <b>You now have large bee wings!</b> You give a few experimental flaps and begin hovering in place, a giddy smile plastered on your face by the thrill of flight.");
        }
        ...
        if (special) {
            ...
            if (!liveData.player.hasCock()) {
                ...
                GUI.outputText("<br><br>The effects of the honey move towards your groin, and into your " +
                ...
                if (liveData.player.cocks[0].cArea() <= 20)
                    GUI.outputText("It now swings as low as your knees!");
                else if (liveData.player.cocks[0].cArea() <= 50)
                    GUI.outputText("While erect, your massive member fills the lower half of your vision.");
                else
                    GUI.outputText("Your member is now simply huge, you wonder what in the world could actually take your massive size now?");
                GUI.outputText("</b>");
                ...
                GUI.outputText("<br><br>Your huge member suddenly starts to hurt, especially the tip of the thing. At the same time, you feel your length start to get incredibly sensitive and the base of your shaft starts to itch. You tear off your " +
                    liveData.player.armor.equipmentName +
                ...
                if (mult > 0) {
                    GUI.outputText("<br><br>The effects of the honey don't seem to focus on your groin this time, but you still feel your " + liveData.player.cockDescript(0) + " grow slightly under your " + liveData.player.armor.equipmentName + ".");
                    liveData.player.cocks[0].cockLength += (0.1 * UTIL.rand(10) + 1) * mult;
                    liveData.player.cocks[0].cockThickness += (0.1 * UTIL.rand(2) + 0.1) * mult;
                }
                else {
                    GUI.outputText("<br><br>The effects of the honey don't seem to focus on your groin this time and you have a feeling that your " +
                        liveData.player.cockDescript(0) +
            ...
            if (liveData.player.cor >= 5) {
                GUI.outputText("<br><br>Your mind feels surprisingly clear of the twisted thoughts that have plagued it as of late, but you find yourself feeling more and more aroused than usual.");
                ...
                GUI.outputText("<br><br>You find your mind is drifting to the thought of using your member to fertilize hundreds and hundreds of eggs every day. You shake your head, the bizarre fantasy catching you completely off guard.");
                liveData.player.modStats(["cor=", 0], ["lib", 5]);
            }
            if (liveData.player.femininity >= 60 || liveData.player.femininity <= 40) {
                GUI.outputText("<br><br>Your face shifts in shape, becoming more androgynous.");
    ...
    static canineTFs(type) {
        ...
        switch (type) {
            case 0: //Normal
                if (crit)
                    GUI.outputText("The pepper tastes particularly potent, searingly hot and spicy.");
                else
                    GUI.outputText("The pepper is strangely spicy but very tasty.");
                break;
            case 1: //Oversized
                GUI.outputText("The pepper is so large and thick that you have to eat it in several large bites. It is not as spicy as the normal ones, but is delicious and flavorful.");
                break;
            case 2: //Double
                GUI.outputText("The double-pepper is strange, looking like it was formed when two peppers grew together near their bases.");
                break;
            case 3: //Black (Corruptive)
                GUI.outputText("This black pepper tastes sweet, but has a bit of a tangy aftertaste.");
                break;
            case 4: //Knotty
                GUI.outputText("The pepper is a bit tough to eat due to the swollen bulge near the base, but you manage to cram it down and munch on it. It's extra spicy!");
                break;
            case 5: //Bulbous
                GUI.outputText("You eat the pepper, even the two orb-like growths that have grown out from the base. It's delicious!");
                break;
        }
        liveData.player.refillHunger(15);
        //------------
        ...
        if (type == 3) {
            ...
            GUI.outputText("<br><br>You feel yourself relaxing as gentle warmth spreads through your body. Honestly you don't think you'd mind running into a demon or monster right now, they'd make for good entertainment.");
            if (liveData.player.cor < 50)
                GUI.outputText(" You shake your head, blushing hotly. Where did that thought come from?");
        }
        if (liveData.player.str < 50 && UTIL.rand(3) == 0) {
            liveData.player.modStats(["str", crit]);
            if (crit > 1)
                GUI.outputText("<br><br>Your muscles ripple and grow, bulging outwards.");
            else
                GUI.outputText("<br><br>Your muscles feel more toned.");
            changes++;
        }
        if (liveData.player.spe < 30 && UTIL.rand(3) == 0 && changes < changeLimit) {
            liveData.player.modStats(["spe", crit]);
            if (crit > 1)
                GUI.outputText("<br><br>You find your muscles responding quicker, faster, and you feel an odd desire to go for a walk.");
            else
                GUI.outputText("<br><br>You feel quicker.");
            changes++;
        }
        if (liveData.player.inte > 30 && UTIL.rand(3) == 0 && changes < changeLimit && type != 3) {
            liveData.player.modStats(["int", -1 * crit]);
            GUI.outputText("<br><br>You feel ");
            if (crit > 1)
                GUI.outputText("MUCH ");
            GUI.outputText("dumber.");
            changes++;
        }
        //------------
        // NORMALIZATION
        ...
        if (changes < changeLimit && liveData.player.armType == ENUM.ArmType.ARM_TYPE_HARPY && UTIL.rand(4) == 0) {
            GUI.outputText("<br><br>You scratch at your biceps absentmindedly, but no matter how much you scratch, it isn't getting rid of the itch. Glancing down in irritation, you discover that your feathery arms are shedding their feathery coating. The wing-like shape your arms once had is gone in a matter of moments, leaving " +
        ...
        if (changes < changeLimit && liveData.player.armType == ENUM.ArmType.ARM_TYPE_SPIDER && UTIL.rand(4) == 0) {
            GUI.outputText("<br><br>You scratch at your biceps absentmindedly, but no matter how much you scratch, it isn't getting rid of the itch. Glancing down in irritation, you discover that your arms' chitinous covering is flaking away. The glossy black coating is soon gone, leaving " +
                liveData.player.skinDesc +
                " behind.");
            liveData.player.armType = ENUM.ArmType.ARM_TYPE_HUMAN;
            changes++;
        ...
        if (changes < changeLimit && liveData.player.hairType == 1 && UTIL.rand(4) == 0) {
            ...
            if (liveData.player.hairLength >= 6)
                GUI.outputText("<br><br>A lock of your downy-soft feather-hair droops over your eye. Before you can blow the offending down away, you realize the feather is collapsing in on itself. It continues to curl inward until all that remains is a normal strand of hair. <b>Your hair is no longer feathery!</b>");
            //(short)
            else
                GUI.outputText("<br><br>You run your fingers through your downy-soft feather-hair while you await the effects of the item you just ingested. While your hand is up there, it detects a change in the texture of your feathers. They're completely disappearing, merging down into strands of regular hair. <b>Your hair is no longer feathery!</b>");
            changes++;
        ...
        if (changes < changeLimit && UTIL.rand(5) == 0 && liveData.player.eyeType > ENUM.EyeType.EYES_HUMAN) {
            if (liveData.player.eyeType == ENUM.EyeType.EYES_BLACK_EYES_SAND_TRAP) {
                GUI.outputText("<br><br>You feel a twinge in your eyes and you blink. It feels like black cataracts have just fallen away from you, and you know without needing to see your reflection that your eyes have gone back to looking human.");
            }
            else {
                GUI.outputText("<br><br>You blink and stumble, a wave of vertigo threatening to pull your " + liveData.player.feet() + " from under you. As you steady and open your eyes, you realize something seems different. Your vision is changed somehow.");
                if (liveData.player.eyeType == ENUM.EyeType.EYES_FOUR_SPIDER_EYES)
                    GUI.outputText(" Your multiple, arachnid eyes are gone!</b>");
            ...
            //If player doesnt have 2 dogdicks
                ...
                if (liveData.player.countCocksOfType(ENUM.CockType.DOG) == 0) {
                    ...
                    //2 dicks+ - convert first 2 to doggie-dom
                        ...
                        GUI.outputText("<br><br>Your crotch twitches, and you pull open your " +
                            liveData.player.armor.equipmentName +
        ...
        if (type == 4) {
            ...
            if (liveData.player.cockTotal() > 0) {
                ...
                if (liveData.player.countCocksOfType(ENUM.CockType.DOG) > 0) {
                    ...
                    GUI.outputText("<br><br>");
                    if (temp3 < 0.06)
                        GUI.outputText("Your " + Appearance.cockNoun(ENUM.CockType.DOG) + " feels unusually tight in your sheath as your knot grows.");
                    if (temp3 >= 0.06 && temp3 <= 0.12)
                        GUI.outputText("Your " + Appearance.cockNoun(ENUM.CockType.DOG) + " pops free of your sheath, thickening nicely into a bigger knot.");
                    if (temp3 > 0.12)
                        GUI.outputText("Your " + Appearance.cockNoun(ENUM.CockType.DOG) + " surges free of your sheath, swelling thicker with each passing second. Your knot bulges out at the base, growing far beyond normal.");
            ...
            //You wasted knot pepper!
                ...
                GUI.outputText("<br><br>A slight wave of nausea passes through you. It seems this pepper does not quite agree with your body.");
        }
        //GROW BALLS
        if (type == 5) {
            if (liveData.player.balls <= 1) {
                GUI.outputText("<br><br>A spike of pain doubles you up, nearly making you vomit. You stay like that, nearly crying, as a palpable sense of relief suddenly washes over you. You look down and realize you now have a small sack, complete with two relatively small balls.");
                ...
                if (liveData.player.ballSize <= 2)
                    GUI.outputText("<br><br>A flash of warmth passes through you and a sudden weight develops in your groin. You pause to examine the changes and your roving fingers discover your " +
                        liveData.player.ballsDescriptLight() +
                        " have grown larger than a human's.");
                if (liveData.player.ballSize > 2)
                    GUI.outputText("<br><br>A sudden onset of heat envelops your groin, focusing on your " + liveData.player.sackDescript() + ". Walking becomes difficult as you discover your " + liveData.player.ballsDescriptLight() + " have enlarged again.");
        ...
        if (liveData.player.cocks.length > 0) {
            ...
            if (type != 4 && liveData.player.countCocksOfType(ENUM.CockType.DOG) > 0 && ((changes < changeLimit && UTIL.rand(1.4) == 0) || type == 1)) {
                ...
                liveData.player.cocks[temp2].knotMultiplier += temp3;
                if (temp3 < 0.06)
                    GUI.outputText("<br><br>Your " + liveData.player.cockDescript(temp2) + " feels unusually tight in your sheath as your knot grows.");
                if (temp3 >= 0.06 && temp3 <= 0.12)
                    GUI.outputText("<br><br>Your " + liveData.player.cockDescript(temp2) + " pops free of your sheath, thickening nicely into a bigger knot.");
                if (temp3 > 0.12)
                    GUI.outputText("<br><br>Your " + liveData.player.cockDescript(temp2) + " surges free of your sheath, swelling thicker with each passing second. Your knot bulges out at the base, growing far beyond normal.");
                liveData.player.modStats(["sen", 0.5], ["lus", 5 * crit]);
                changes++;
            }
            ...
            if (liveData.player.countCocksOfType(ENUM.CockType.DOG) < liveData.player.cocks.length && ((changes < changeLimit && UTIL.rand(1.6)) || type == 1) == 0) {
                ...
                if (liveData.player.cocks[temp3].cockType > 4) {
                    ...
                    GUI.outputText("<br><br>Your " + liveData.player.cockDescript(temp3) + " trembles, reshaping itself into a shiny red doggie-dick with a fat knot at the base. <b>You now have a dog-cock.</b>");
                    liveData.player.modStats(["sen", 4], ["lus", 5 * crit]);
                }
                temp = 0;
                //Demon
                if (liveData.player.cocks[temp3].cockType == ENUM.CockType.DEMON) {
                    GUI.outputText("<br><br>Your " + liveData.player.cockDescript(temp3) + " color shifts red for a moment and begins to swell at the base, but within moments it smooths out, retaining its distinctive demonic shape, only perhaps a bit thicker.");
            ...
            if (liveData.player.cumMultiplier < 2 && UTIL.rand(2) == 0 && changes < changeLimit) {
                ...
                if (temp < liveData.player.cumMultiplier + 0.05 * crit) {
                    ...
                    if (liveData.player.balls == 0)
                        GUI.outputText("<br><br>You feel a churning inside your gut as something inside you changes.");
                    if (liveData.player.balls > 0)
                        GUI.outputText("<br><br>You feel a churning in your " + liveData.player.ballsDescriptLight() + ". It quickly settles, leaving them feeling somewhat more dense.");
                    if (crit > 1)
                        GUI.outputText(" A bit of milky pre dribbles from your " + liveData.player.multiCockDescriptLight() + ", pushed out by the change.");
                }
                changes++;
            }
            //Oversized pepper
            if (type == 1) {
                ...
                if (temp2 > 1 && temp2 <= 2)
                    GUI.outputText("<br><br>Aching pressure builds within your crotch, suddenly releasing as an inch or more of extra dick-flesh spills out. A dollop of pre beads on the head of your enlarged " +
                        liveData.player.cockDescript(temp) +
                        " from the pleasure of the growth.");
                if (temp2 <= 1)
                    GUI.outputText("<br><br>A slight pressure builds and releases as your " + liveData.player.cockDescript(temp) + " pushes a bit further out of your crotch.");
            }
        }
        //Female Stuff
        //Multiboobages
            ...
            if (liveData.player.breastRows[0].breastRating > 0 && liveData.player.vaginas.length > 0) {
                ...
                if (liveData.player.breastRows.length < 3 && UTIL.rand(2) == 0 && changes < changeLimit) {
                    ...
                    if (liveData.player.vaginas.length > 0 && liveData.player.breastRows[0].breastRating <= liveData.player.breastRows.length) {
                        GUI.outputText("<br><br>Your " + liveData.player.breastDescript(0) + " feel constrained and painful against your top as they grow larger by the moment, finally stopping as they reach ");
                        liveData.player.breastRows[0].breastRating += 2;
                        GUI.outputText(liveData.player.breastCup(0) + " size. But it doesn't stop there, you feel a tightness beginning lower on your torso...");
                        changes++;
                    }
                    //Had 1 row to start
                    if (liveData.player.breastRows.length == 2) {
                        ...
                        if (liveData.player.breastRows[0].breastRating - 1 == 0)
                            GUI.outputText("<br><br>A second set of breasts forms under your current pair, stopping while they are still fairly flat and masculine looking.");
                        else
                            GUI.outputText("<br><br>A second set of breasts bulges forth under your current pair, stopping as they reach " + liveData.player.breastCup(temp) + "s.");
                        GUI.outputText(" A sensitive nub grows on the summit of each new tit, becoming a new nipple.");
                    ...
                    if (liveData.player.breastRows.length > 2 && liveData.player.breastRows[0].breastRating > liveData.player.breastRows.length) {
                        ...
                        if (liveData.player.breastRows[temp].breastRating == 0)
                            GUI.outputText("<br><br>Your abdomen tingles and twitches as a new row of breasts sprouts below the others. Your new breasts stay flat and masculine, not growing any larger.");
                        else
                            GUI.outputText("<br><br>Your abdomen tingles and twitches as a new row of " + liveData.player.breastCup(temp) + " " + liveData.player.breastDescript(temp) + " sprouts below your others.");
                        GUI.outputText(" A sensitive nub grows on the summit of each new tit, becoming a new nipple.");
                        changes++;
                    }
                    ...
                    if (crit > 1) {
                        if (crit > 2) {
                            GUI.outputText(" You heft your new chest experimentally, exploring the new flesh with tender touches. Your eyes nearly roll back in your head from the intense feelings.");
                            liveData.player.modStats(["sen", 6], ["lus", 15], ["cor", 0]);
                        }
                        else {
                            GUI.outputText(" You touch your new nipples with a mixture of awe and desire, the experience arousing beyond measure. You squeal in delight, nearly orgasming, but in time finding the willpower to stop yourself.");
                ...
                //If already has max doggie breasts!
                    ...
                    while (temp > 1 && temp2 == 0) {
                        ...
                        if (liveData.player.breastRows[temp].breastRating + 1 < liveData.player.breastRows[temp - 1].breastRating) {
                            ...
                            GUI.outputText("<br>Your ");
                            if (temp == 0)
                                GUI.outputText("first ");
                            if (temp == 1)
                                GUI.outputText("second ");
                            if (temp == 2)
                                GUI.outputText("third ");
                            if (temp == 3)
                                GUI.outputText("fourth ");
                            if (temp == 4)
                                GUI.outputText("fifth ");
                            if (temp > 4)
                                GUI.outputText("");
                            GUI.outputText("row of " + liveData.player.breastDescript(temp) + " grows larger, as if jealous of the jiggling flesh above.");
        ...
        //Grow tits if have NO breasts/nipples AT ALL
            ...
            GUI.outputText("<br><br>Your chest tingles uncomfortably as your center of balance shifts. <b>You now have a pair of B-cup breasts.</b>");
            GUI.outputText(" A sensitive nub grows on the summit of each tit, becoming a new nipple.");
            liveData.player.createBreastRow();
            liveData.player.breastRows[0].breastRating = 2;
        ...
        if (changes < changeLimit && liveData.player.dogScore() >= 3 && UTIL.rand(4) == 0) {
            changes++;
            GUI.outputText("<br><br>Images and thoughts come unbidden to your mind, overwhelming your control as you rapidly lose yourself in them, daydreaming of... ");
            //cawk fantasies
            if (liveData.player.gender <= 1 || (liveData.player.gender == 3 && UTIL.rand(2) == 0)) {
                GUI.outputText("bounding through the woods, hunting with your master. Feeling the wind in your fur and the thrill of the hunt coursing through your veins intoxicates you. You have your nose to the ground, tracking your quarry as you run, until a heavenly scent stops you in your tracks.");
                liveData.player.modStats(["lus", 5 + liveData.player.lib / 20]);
                //break1
                if (liveData.player.cor < 33 || !liveData.player.hasCock())
                    GUI.outputText("<br>You shake your head to clear the unwanted fantasy from your mind, repulsed by it.");
                else {
                    GUI.outputText(" Heart pounding, your shaft pops free of its sheath on instinct, as you take off after the new scent. Caught firmly in the grip of a female's heat, you ignore your master's cry as you disappear into the wild, " +
                        Appearance.cockNoun(ENUM.CockType.DOG) +
                    ...
                    if (liveData.player.cor < 66)
                        GUI.outputText("<br>You blink a few times, the fantasy fading as you master yourself. That daydream was so strange, yet so hot.");
                    else {
                        GUI.outputText(" Unable to wait any longer, you mount her, pressing your bulging knot against her vulva as she yips in pleasure. The heat of her sex is unreal, the tight passage gripping you like a vice as you jackhammer against her, biting her neck gently in spite of the violent pounding.");
                        liveData.player.modStats(["lus", 5 + liveData.player.lib / 20]);
                        //break3
                        if (liveData.player.cor < 80) {
                            if (liveData.player.vaginas.length > 0)
                                GUI.outputText("<br>You reluctantly pry your hand from your aching " + liveData.player.vaginaDescript(0) + " as you drag yourself out of your fantasy.");
                            else
                                GUI.outputText("<br>You reluctantly pry your hand from your aching " + liveData.player.cockDescript(0) + " as you drag yourself out of your fantasy.");
                        }
                        else {
                            GUI.outputText(" At last your knot pops into her juicy snatch, splattering her groin with a smattering of her arousal. The scents of your mating reach a peak as the velvet vice around your " +
                                Appearance.cockNoun(ENUM.CockType.DOG) +
            ...
            //Pure female fantasies
                ...
                GUI.outputText("wagging your dripping " + liveData.player.vaginaDescript(0) + " before a pack of horny wolves, watching their shiny red doggie-pricks practically jump out of their sheaths at your fertile scent.");
                liveData.player.modStats(["lus", 5 + liveData.player.lib / 20]);
                //BREAK 1
                if (liveData.player.cor < 33) {
                    ...
                    GUI.outputText(" In moments they begin their advance, plunging their pointed beast-dicks into you, one after another. You yip and howl with pleasure as each one takes his turn knotting you.");
                    liveData.player.modStats(["lus", 5 + liveData.player.lib / 20]);
                    //BREAK 2
                    if (liveData.player.cor <= 66) {
                        GUI.outputText("<br>You blink a few times, the fantasy fading as you master yourself. That daydream was so strange, yet so hot.");
                        ...
                        GUI.outputText(" The feeling of all that hot wolf-spooge spilling from your overfilled snatch and running down your thighs is heavenly, nearly making you orgasm on the spot. You see the alpha of the pack is hard again, and his impressive member is throbbing with the need to breed you.");
                        liveData.player.modStats(["lus", 5 + liveData.player.lib / 20]);
                        //break3
                        if (liveData.player.cor < 80) {
                            GUI.outputText("<br>You reluctantly pry your hand from your aching " + liveData.player.vaginaDescript(0) + " as you drag yourself out of your fantasy.");
                ...
                GUI.outputText("wagging your [asshole] before a pack of horny wolves, watching their shiny red doggie-pricks practically jump out of their sheaths at you after going so long without a female in the pack.");
                liveData.player.modStats(["lus", 5 + liveData.player.lib / 20]);
                //BREAK 1
                if (liveData.player.cor < 33) {
                    GUI.outputText("<br>You shake your head to clear the unwanted fantasy from your mind, repulsed by it.");
                    ...
                    if (liveData.player.cor <= 66) {
                        ...
                        GUI.outputText(" The feeling of all that hot wolf-spooge spilling from your overfilled ass and running down your thighs is heavenly, nearly making you orgasm on the spot. You see the alpha of the pack is hard again, and his impressive member is throbbing with the need to spend his lust on you.");
                        liveData.player.modStats(["lus", 5 + liveData.player.lib / 20]);
                        //break3
                        if (liveData.player.cor < 80) {
                            GUI.outputText("<br>You reluctantly pry your hand from your aching asshole as you drag yourself out of your fantasy.");
                        }
                        else {
        ...
        if (UTIL.rand(5) == 0 && changes < changeLimit && liveData.player.faceType != ENUM.FaceType.FACE_DOG && liveData.player.skinType == ENUM.SkinType.SKIN_TYPE_FUR && liveData.player.lowerBody == ENUM.LowerBodyType.LOWER_BODY_TYPE_DOG) {
            if (liveData.player.faceType == ENUM.FaceType.FACE_HORSE)
                GUI.outputText("<br><br>Your face is wracked with pain. You throw back your head and scream in agony as you feel your cheekbones breaking and shifting, reforming into something else. <b>Your horse-like features rearrange to take on many canine aspects.</b>");
            else
                GUI.outputText("<br><br>Your face is wracked with pain. You throw back your head and scream in agony as you feel your cheekbones breaking and shifting, reforming into something... different. You find a puddle to view your reflection...<b>your face is now a cross between human and canine features.</b>");
            liveData.player.faceType = ENUM.FaceType.FACE_DOG;
            changes++;
        }
        if (type == 3 && liveData.player.hairColor != "midnight black") {
            if (liveData.player.skinType == ENUM.SkinType.SKIN_TYPE_FUR)
                GUI.outputText("<b><br><br>Your fur and hair tingles, growing in thicker than ever as darkness begins to spread from the roots, turning it midnight black.</b>");
            else
                GUI.outputText("<b><br><br>Your " + liveData.player.skinDesc + " itches like crazy as fur grows out from it, coating your body. It's incredibly dense and black as the middle of a moonless night.</b>");
        ...
        if (UTIL.rand(4) == 0 && changes < changeLimit && liveData.player.lowerBody == ENUM.LowerBodyType.LOWER_BODY_TYPE_DOG && liveData.player.tailType == ENUM.TailType.TAIL_TYPE_DOG && liveData.player.skinType != ENUM.SkinType.SKIN_TYPE_FUR) {
            if (liveData.player.skinType == ENUM.SkinType.SKIN_TYPE_PLAIN)
                GUI.outputText("<br><br>Your skin itches intensely. You gaze down as more and more hairs break forth from your skin, quickly transforming into a soft coat of fur. <b>You are now covered in " +
                    liveData.player.furColor +
                    " fur from head to toe.</b>");
            if (liveData.player.skinType == ENUM.SkinType.SKIN_TYPE_SCALES)
                GUI.outputText("<br><br>Your scales itch incessantly. You scratch, feeling them flake off to reveal a coat of " +
                    liveData.player.furColor +
        ...
        if (UTIL.rand(3) == 0 && liveData.player.lowerBody != ENUM.LowerBodyType.LOWER_BODY_TYPE_DOG && liveData.player.tailType == ENUM.TailType.TAIL_TYPE_DOG && liveData.player.earType == ENUM.EarType.EARS_DOG && changes < changeLimit) {
            ...
            //Hooves -> Paws
                ...
                GUI.outputText("<br><br>You feel your hooves suddenly splinter, growing into five unique digits. Their flesh softens as your hooves reshape into furred paws.");
            else
                GUI.outputText("<br><br>Your lower body is wracked by pain! Once it passes, you discover that you're standing on fur-covered paws! <b>You now have paws</b>.");
        ...
        if (UTIL.rand(2) == 0 && liveData.player.earType != ENUM.EarType.EARS_DOG && liveData.player.tailType == ENUM.TailType.TAIL_TYPE_DOG && changes < changeLimit) {
            if (liveData.player.earType == -1)
                GUI.outputText("<br><br>Two painful nubs begin sprouting from your head, growing and opening into canine ears. ");
            if (liveData.player.earType == ENUM.EarType.EARS_HUMAN)
                GUI.outputText("<br><br>The skin on the sides of your face stretches painfully as your ears migrate upwards, towards the top of your head. They shift and elongate, becoming canine in nature. ");
            if (liveData.player.earType == ENUM.EarType.EARS_HORSE)
                GUI.outputText("<br><br>Your equine ears twist as they transform into canine versions. ");
            if (liveData.player.earType > ENUM.EarType.EARS_DOG)
                GUI.outputText("<br><br>Your ears transform, becoming more canine in appearance. ");
        ...
        if (UTIL.rand(3) == 0 && changes < changeLimit && liveData.player.tailType != ENUM.TailType.TAIL_TYPE_DOG) {
            if (liveData.player.tailType == ENUM.TailType.TAIL_TYPE_NONE)
                GUI.outputText("<br><br>A pressure builds on your backside. You feel under your clothes and discover an odd bump that seems to be growing larger by the moment. In seconds it passes between your fingers, bursts out the back of your clothes, and grows most of the way to the ground. A thick coat of fur springs up to cover your new tail. ");
            if (liveData.player.tailType == ENUM.TailType.TAIL_TYPE_HORSE)
                GUI.outputText("<br><br>You feel a tightness in your rump, matched by the tightness with which the strands of your tail clump together. In seconds they fuse into a single tail, rapidly sprouting thick fur. ");
            if (liveData.player.tailType == ENUM.TailType.TAIL_TYPE_DEMONIC)
                GUI.outputText("<br><br>The tip of your tail feels strange. As you pull it around to check on it, the spaded tip disappears, quickly replaced by a thick coat of fur over the entire surface of your tail. ");
        ...
        if (liveData.player.skinType == ENUM.SkinType.SKIN_TYPE_FUR && changes < changeLimit && UTIL.rand(3) == 0) {
            GUI.outputText("<br><br>You become more... solid. Sinewy. A memory comes unbidden from your youth of a grizzled wolf you encountered while hunting, covered in scars, yet still moving with an easy grace. You imagine that must have felt something like this.");
            liveData.player.modStats(["tou", 4], ["sen", -3]);
            changes++;
        }
        //If no changes yay
        if (changes == 0) {
            GUI.outputText("<br><br>Inhuman vitality spreads through your body, invigorating you!<br>");
    ...
    static cowTFs(tainted, enhanced) {
        ...
        if (enhanced)
            GUI.outputText("Pro Bova");
        else
            GUI.outputText("La Bova");
        GUI.outputText(".  The drink has an odd texture, but is very sweet.  It has a slight aftertaste of milk.");
        ...
        if (changes < changeLimit && UTIL.rand(3) == 0) {
            ...
            if (temp <= 0)
                ...
                if (UTIL.rand(2) == 0)
                    GUI.outputText("<br><br>There is a slight pain as you feel your muscles shift somewhat.  Their appearance does not change much, but you feel much stronger.");
                else
                    GUI.outputText("<br><br>You feel your muscles tighten and clench as they become slightly more pronounced.");
        ...
        if (changes < changeLimit && UTIL.rand(3) == 0) {
            ...
            if (temp <= 0)
                ...
                if (UTIL.rand(2) == 0)
                    GUI.outputText("<br><br>You feel your insides toughening up; it feels like you could stand up to almost any blow.");
                else
                    GUI.outputText("<br><br>Your bones and joints feel sore for a moment, and before long you realize they've gotten more durable.");
        ...
        if (changes < changeLimit && UTIL.rand(3) == 0) {
            if (liveData.player.spe > 30) {
                GUI.outputText("<br><br>The body mass you've gained is making your movements more sluggish.");
                changes++;
        ...
        if (liveData.player.cocks.length > 0 && UTIL.rand(2) == 0 && !liveData.gameFlags[FLAG.HYPER_HAPPY]) {
            ...
            if (liveData.player.cocks[temp].cockLength < 2) {
                ...
                if (liveData.player.cockTotal() == 1 && !liveData.player.hasVagina()) {
                    ...
                    if (liveData.player.balls > 0)
                        GUI.outputText("  At the same time, your " + liveData.player.ballsDescriptLight() + " fall victim to the same sensation; eagerly swallowed whole by your crotch.");
                    GUI.outputText("  Curious, you touch around down there, to find you don't have any exterior organs left.  All of it got swallowed into the gash you now have running between two fleshy folds, like sensitive lips.  It suddenly occurs to you; <b>you now have a vagina!</b>");
            ...
            if (liveData.player.cocks.length == 0 && !liveData.player.hasVagina()) {
                ...
                liveData.player.clitLength = 0.25;
                GUI.outputText("<br><br>An itching starts in your crotch and spreads vertically.  You reach down and discover an opening.  You have grown a <b>new " + liveData.player.vaginaDescript(0) + "</b>!");
                changes++;
                liveData.player.genderCheck();
        ...
        if (((tainted && liveData.player.biggestTitSize() <= 11) || (!tainted && liveData.player.biggestTitSize() <= 5)) && changes < changeLimit && (UTIL.rand(3) == 0 || enhanced)) {
            if (UTIL.rand(2) == 0)
                GUI.outputText("<br><br>Your " + liveData.player.breastDescript(0) + " tingle for a moment before becoming larger.");
            else
                GUI.outputText("<br><br>You feel a little weight added to your chest as your " + liveData.player.breastDescript(0) + " seem to inflate and settle in a larger size.");
        ...
        if (liveData.player.biggestTitSize() >= 4 && liveData.player.breastRows[0].lactationMultiplier < 1 && changes < changeLimit && (UTIL.rand(3) == 0 || boobsGrew || enhanced)) {
            GUI.outputText("<br><br>You gasp as your " +
        ...
        if (enhanced) {
            ...
            if (liveData.player.breastRows[0].nipplesPerBreast == 1) {
                ...
                if (liveData.player.breastRows.length >= 2 && liveData.player.breastRows[1].nipplesPerBreast == 1) {
                    GUI.outputText("A moment later your second row of " + liveData.player.breastDescript(1) + " does the same.  <b>You have sixteen nipples now!</b>");
                ...
                if (liveData.player.breastRows.length >= 3 && liveData.player.breastRows[2].nipplesPerBreast == 1) {
                    GUI.outputText("Finally, your ");
                    if (liveData.player.bRows() == 3)
                        GUI.outputText("third row of " + liveData.player.breastDescript(2) + " mutates along with its sisters, sprouting into a wonderland of nipples.");
                    else if (liveData.player.bRows() >= 4) {
                        GUI.outputText("everything from the third row down mutates, sprouting into a wonderland of nipples.");
                    ...
                    GUI.outputText("  <b>You have a total of " + UTIL.num2Text(liveData.player.totalNipples()) + " nipples.</b>");
                }
            }
            //QUAD DAMAGE IF WEIRD SHIT BROKE BEFORE
                ...
                if (liveData.player.breastRows[1].nipplesPerBreast == 1) {
                    GUI.outputText("<br><br>Your second row of " +
                ...
                if (liveData.player.breastRows[2].nipplesPerBreast == 1) {
                    GUI.outputText("<br><br>Your third row of " +
                ...
                if (liveData.player.breastRows[3].nipplesPerBreast == 1) {
                    GUI.outputText("<br><br>Your fourth row of " +
                ...
                if (UTIL.rand(2) == 0)
                    GUI.outputText("<br><br>A wave of pleasure passes through your chest as your " + liveData.player.breastDescript(0) + " start leaking milk from a massive jump in production.");
                else
                    GUI.outputText("<br><br>Something shifts inside your " + liveData.player.breastDescript(0) + " and they feel MUCH fuller and riper.  You know that you've started producing much more milk.");
                liveData.player.boostLactation(2.5);
                if ((liveData.player.nippleLength < 1.5 && tainted) || (!tainted && liveData.player.nippleLength < 1)) {
                    GUI.outputText("  Your " + liveData.player.nippleDescript(0) + "s swell up, growing larger to accommodate your increased milk flow.");
        ...
        //If breasts are already lactating and the player is not lactating beyond a reasonable level, they start lactating more:
            ...
            if (tainted && liveData.player.breastRows[0].lactationMultiplier > 1 && liveData.player.breastRows[0].lactationMultiplier < 5 && changes < changeLimit && (UTIL.rand(3) == 0 || enhanced)) {
                if (UTIL.rand(2) == 0)
                    GUI.outputText("<br><br>A wave of pleasure passes through your chest as your " + liveData.player.breastDescript(0) + " start producing more milk.");
                else
                    GUI.outputText("<br><br>Something shifts inside your " + liveData.player.breastDescript(0) + " and they feel fuller and riper.  You know that you've started producing more milk.");
            ...
            if (!tainted) {
                ...
                if ((liveData.player.breastRows[0].lactationMultiplier > 2 && liveData.player.findStatusEffect(StatusEffects.Feeder) >= 0) || liveData.player.breastRows[0].lactationMultiplier > 5) {
                    if (UTIL.rand(2) == 0)
                        GUI.outputText("<br><br>Your breasts suddenly feel less full, it seems you aren't lactating at quite the level you were.");
                    else
                        GUI.outputText("<br><br>The insides of your breasts suddenly feel bloated.  There is a spray of milk from them, and they settle closer to a more natural level of lactation.");
                    changes++;
                    liveData.player.dynStats(["sen", 0.5]);
        ...
        if (liveData.player.findStatusEffect(StatusEffects.Feeder) < 0 && liveData.player.biggestLactation() >= 3 && UTIL.rand(2) == 0 && liveData.player.biggestTitSize() >= 5 && liveData.player.cor >= 35) {
            GUI.outputText("<br><br>You start to feel a strange desire to give your milk to other creatures.  For some reason, you know it will be very satisfying.<br><br><b>(You have gained the 'Feeder' perk!)</b>");
        ...
        if (liveData.player.hasVagina()) {
            if (liveData.player.vaginas[0].vaginalLooseness < ENUM.VaginalLoosenessType.VAGINA_LOOSENESS_LOOSE && changes < changeLimit && UTIL.rand(2) == 0) {
                GUI.outputText("<br><br>You feel a relaxing sensation in your groin.  On further inspection you discover your " + liveData.player.vaginaDescript(0) + " has somehow relaxed, permanently loosening.");
        ...
        if (tainted && UTIL.rand(5) == 0) {
            if (liveData.player.findPerk(PerkLib.Oviposition) == 1 && liveData.player.lizardScore() < 8) {
                GUI.outputText("<br><br>Another change in your uterus ripples through your reproductive systems." + "  Somehow you know you've lost a little bit of reptilian reproductive ability.<br>");
        ...
        if (tainted && liveData.player.tailType != ENUM.TailType.TAIL_TYPE_COW && changes < changeLimit && UTIL.rand(3) == 0) {
            if (liveData.player.tailType == ENUM.TailType.TAIL_TYPE_NONE)
                GUI.outputText("<br><br>You feel the flesh above your " +
                    liveData.player.buttDescript() +
                ...
                if (liveData.player.tailType < ENUM.TailType.TAIL_TYPE_SPIDER_ADBOMEN || liveData.player.tailType > ENUM.TailType.TAIL_TYPE_BEE_ABDOMEN) {
                    GUI.outputText("<br><br>Your tail bunches uncomfortably, twisting and writhing around itself before flopping straight down, now shaped into a distinctly bovine form.  You have a <b>cow tail</b>.");
                }
                //insect
                if (liveData.player.tailType == ENUM.TailType.TAIL_TYPE_SPIDER_ADBOMEN || liveData.player.tailType == ENUM.TailType.TAIL_TYPE_BEE_ABDOMEN) {
                    GUI.outputText("<br><br>Your insect-like abdomen tingles pleasantly as it begins shrinking and softening, chitin morphing and reshaping until it looks exactly like a <b>cow tail</b>.");
                }
            }
        ...
        if (((enhanced && liveData.player.tallness < 96) || liveData.player.tallness < 84) && changes < changeLimit && UTIL.rand(2) == 0) {
            ...
            if (temp < 5)
                GUI.outputText("<br><br>You shift uncomfortably as you realize you feel off balance.  Gazing down, you realize you have grown SLIGHTLY taller.");
            if (temp >= 5 && temp < 7)
                GUI.outputText("<br><br>You feel dizzy and slightly off, but quickly realize it's due to a sudden increase in height.");
            if (temp == 7)
                GUI.outputText("<br><br>Staggering forwards, you clutch at your head dizzily.  You spend a moment getting your balance, and stand up, feeling noticeably taller.");
        ...
        if (tainted && liveData.player.lowerBody != ENUM.LowerBodyType.LOWER_BODY_TYPE_HOOFED && liveData.player.earType == ENUM.EarType.EARS_COW) {
            if (changes < changeLimit && UTIL.rand(3) == 0) {
                changes++;
                if (liveData.player.lowerBody == ENUM.LowerBodyType.LOWER_BODY_TYPE_HUMAN)
                    GUI.outputText("<br><br>You stagger as your feet change, curling up into painful angry lumps of flesh.  They get tighter and tighter, harder and harder, until at last they solidify into hooves!");
                if (liveData.player.lowerBody == ENUM.LowerBodyType.LOWER_BODY_TYPE_DOG)
                    GUI.outputText("<br><br>You stagger as your paws change, curling up into painful angry lumps of flesh.  They get tighter and tighter, harder and harder, until at last they solidify into hooves!");
                if (liveData.player.lowerBody == ENUM.LowerBodyType.LOWER_BODY_TYPE_NAGA)
                    GUI.outputText("<br><br>You collapse as your sinuous snake-tail tears in half, shifting into legs.  The pain is immense, particularly in your new feet as they curl inward and transform into hooves!");
                //Catch-all
                if (liveData.player.lowerBody > ENUM.LowerBodyType.LOWER_BODY_TYPE_NAGA)
                    GUI.outputText("<br><br>You stagger as your " + liveData.player.feet() + " change, curling up into painful angry lumps of flesh.  They get tighter and tighter, harder and harder, until at last they solidify into hooves!");
        ...
        if (enhanced && (liveData.player.skinDesc != "fur" || liveData.player.furColor != "black and white spotted")) {
            if (liveData.player.skinDesc != "fur")
                ...
                GUI.outputText("<br><br>A ripple spreads through your fur as some patches darken and others lighten.  After a few moments you're left with a black and white spotted pattern that goes the whole way up to the hair on your head!  <b>You've got cow fur!</b>");
            liveData.player.skinDesc = "fur";
        ...
        //if enhanced to probova give a shitty cow face
            ...
            GUI.outputText("<br><br>Your visage twists painfully, warping and crackling as your bones are molded into a new shape.  Once it finishes, you reach up to touch it, and you discover that <b>your face is like that of a cow!</b>");
        ...
        if (tainted && changes < changeLimit && UTIL.rand(3) == 0 && liveData.player.faceType == ENUM.FaceType.FACE_HUMAN) {
            ...
            if (liveData.player.hornType == ENUM.HornType.HORNS_COW_MINOTAUR || liveData.player.hornType == ENUM.HornType.HORNS_NONE) {
                ...
                if (liveData.player.hornType == ENUM.HornType.HORNS_NONE || liveData.player.horns == 0) {
                    GUI.outputText("<br><br>With painful pressure, the skin on your forehead splits around two tiny nub-like horns, similar to those you would see on the cattle back in your homeland.");
        ...
        if (UTIL.rand(2) == 0 && liveData.player.hipRating < 15 && changes < changeLimit) {
            if ((!tainted && liveData.player.hipRating < 8) || tainted) {
                GUI.outputText("<br><br>You stumble as you feel the bones in your hips grinding, expanding your hips noticeably.");
        ...
        if (UTIL.rand(2) == 0 && liveData.player.buttRating < 13 && changes < changeLimit) {
            if ((!tainted && liveData.player.buttRating < 8) || tainted) {
                GUI.outputText("<br><br>A sensation of being unbalanced makes it difficult to walk.  You pause, paying careful attention to your new center of gravity before understanding dawns on you - your ass has grown!");
        ...
        if (liveData.gameFlags[FLAG.HAS_BLACK_NIPPLES] == 1 && changes < changeLimit && UTIL.rand(3) == 0) {
            GUI.outputText("<br><br>Something invisible brushes against your " +
                liveData.player.nippleDescript(0) +
        ...
        if (changes < changeLimit && UTIL.rand(3) == 0 && liveData.player.vaginaType() == 5 && liveData.player.hasVagina()) {
            GUI.outputText("<br><br>Something invisible brushes against your sex, making you twinge.  Undoing your clothes, you take a look at your vagina and find that it has turned back to its natural flesh colour.");
            liveData.player.vaginaType(0);
            changes++;
        }
        if (UTIL.rand(3) == 0)
            GUI.outputText(liveData.player.modFem(79, 3));
        if (UTIL.rand(3) == 0)
            GUI.outputText(liveData.player.modThickness(70, 4));
        if (UTIL.rand(5) == 0)
            GUI.outputText(liveData.player.modTone(10, 5));
    ...
    static demonTFs(type, purified) {
        ...
        GUI.outputText("(Placeholder) You drink the draft.");
    }
    static succubiDelight(tainted) {
        ...
        if (liveData.player.cor < 33)
            GUI.outputText(" This stuff is gross, why are you drinking it?");
        //high corruption
        if (liveData.player.cor >= 66)
            GUI.outputText(" You lick your lips, marvelling at how thick and sticky it is.");
        //Corruption increase
        if ((liveData.player.cor < 50 || UTIL.rand(2)) && tainted) {
            GUI.outputText("<br><br>The drink makes you feel... dirty.");
            let temp = 1;
        ...
        if (liveData.player.balls < 2 && changes < changeLimit && UTIL.rand(4) == 0) {
            if (liveData.player.balls == 0) {
                liveData.player.balls = 2;
                GUI.outputText("<br><br>Incredible pain scythes through your crotch, doubling you over. You stagger around, struggling to pull open your " +
                    liveData.player.armor.equipmentName +
        ...
        if (changes < changeLimit && UTIL.rand(2) == 0 && liveData.player.cocks.length > 0) {
            if (liveData.player.cumMultiplier < 6 && UTIL.rand(2) == 0 && changes < changeLimit) {
                ...
                if (temp < liveData.player.cumMultiplier + 0.4 * crit) {
                    ...
                    //Flavor text
                    if (liveData.player.balls == 0)
                        GUI.outputText("<br><br>You feel a churning inside your body as something inside you changes.");
                    if (liveData.player.balls > 0)
                        GUI.outputText("<br><br>You feel a churning in your " + liveData.player.ballsDescriptLight() + ". It quickly settles, leaving them feeling somewhat more dense.");
                    if (crit > 1)
                        GUI.outputText(" A bit of milky pre dribbles from your " + liveData.player.multiCockDescriptLight() + ", pushed out by the change.");
        ...
        //Fail-safe
        if (changes == 0) {
            GUI.outputText("<br><br>Your groin tingles, making it feel as if you haven't cum in a long time.");
            liveData.player.hoursSinceCum += 100;
            changes++;
        }
        if (liveData.player.balls > 0 && UTIL.rand(3) == 0) {
            GUI.outputText(liveData.player.modFem(12, 3));
        }
    ...
    static equineTFs() {
            ...
            if (UTIL.rand(4) == 0 && liveData.gameFlags[FLAG.HORSE_WARNING] == 1 && liveData.player.findPerk(PerkLib.TransformationResistance) < 0) {
                ...
                if (liveData.player.cockTotal() > 0) {
                    ...
                    if (liveData.player.countCocksOfType(ENUM.CockType.HORSE) > 0) {
                        ...
                        GUI.outputText("<b>One year later...</b><br><br>As you graze upon the small plants that coat the open plains of your home, you hear a noise on your right side. As you raise your head to check where the noise comes from, preparing to run from a potential predator, you see a strange creature. It stands on its two feet, its furless pink skin appearing beneath its clothes. With a start, you realize you can identify the strange creatures gender. ");
                        if (liveData.player.gender == 0 || liveData.player.gender == 1)
                            GUI.outputText("He is clearly a male, but you are somewhat confused as you can see not one but three bulges where his manhood would be.<br><br>");
                        if (liveData.player.gender == 2)
                            GUI.outputText("She is clearly a female, as you can see her six breasts jiggle as she walks towards you, small stains appearing on her shirt where her nipples are.<br><br>");
                        if (liveData.player.gender == 3)
                            GUI.outputText("You are somewhat confused as you can see a bulge near her thighs but also huge boobs jiggling as she walks, and you can't say if she's a male or female.<br><br>");
                ...
                //If player has no cocks
                    ...
                    GUI.outputText("<b>One year after...</b><br><br>As you graze small plants in the open plains that became your home, you hear a noise on your right side. As you raise your head to check where the noise comes from, preparing to run from a potential predator, you see a strange creature. It stands on two feet, its furless pink skin appearing beneath its clothes. ");
                    if (liveData.player.gender == 0 || liveData.player.gender == 1)
                        GUI.outputText("He is clearly a male, but you are somewhat confused as you can see not one but three bulges where his manhood would be.<br><br>");
                    if (liveData.player.gender == 2)
                        GUI.outputText("She is clearly a female, as you can see her six breasts jiggle as she walks towards you, small stains appearing on her shirt where her nipples are.<br><br>");
        ...
        if (UTIL.rand(2) == 0) {
            ...
            if (liveData.player.str >= 60) {
                GUI.outputText("<br><br>You feel strong enough to single-handedly pull a fully-loaded wagon.");
            }
            //NOT MAXXED
            else {
                liveData.player.modStats(["str", 1]);
                GUI.outputText("<br><br>Your muscles clench and surge, making you feel as strong as a horse.");
                changes++;
            }
        }
        //TOUGHNESS
        if (UTIL.rand(2) == 0) {
            ...
            if (liveData.player.tou >= 75) {
                GUI.outputText("<br><br>Your body is as tough and solid as a " + liveData.player.mf("stallion's", "mare's") + ".");
            }
            //NOT MAXXED
            else {
                liveData.player.modStats(["tou", 1.25]);
                GUI.outputText("<br><br>Your body suddenly feels tougher and more resilient.");
                changes++;
            }
        }
        //INTELLECT
        if (UTIL.rand(3) == 0) {
            if (liveData.player.inte <= 5) {
                GUI.outputText('<br><br>You let out a throaty "Neiiiigh" as your animalistic instincts take over.');
            }
            if (liveData.player.inte < 10 && liveData.player.inte > 5) {
                liveData.player.modStats(["int", -1]);
                GUI.outputText("<br><br>You smile vacantly as you drink the potion, knowing you're just a big dumb animal who loves to fuck.");
                changes++;
            }
            if (liveData.player.inte <= 20 && liveData.player.inte >= 10) {
                liveData.player.modStats(["int", -2]);
                GUI.outputText("<br><br>You find yourself looking down at the empty bottle in your hand and realize you haven't thought ANYTHING since your first sip.");
                changes++;
            }
            if (liveData.player.inte <= 30 && liveData.player.inte > 20) {
                liveData.player.modStats(["int", -3]);
                GUI.outputText("<br><br>You smile broadly as your cares seem to melt away. A small part of you worries that you're getting dumber.");
                changes++;
            }
            if (liveData.player.inte <= 50 && liveData.player.inte > 30) {
                liveData.player.modStats(["int", -4]);
                GUI.outputText("<br><br>It becomes harder to keep your mind focused as your intellect diminishes.");
                changes++;
            }
            if (liveData.player.inte > 50) {
                liveData.player.modStats(["int", -5]);
                GUI.outputText("<br><br>Your usually intelligent mind feels much more sluggish.");
                changes++;
            }
        }
        //------------
        ...
        if ((liveData.player.gender == 1 || liveData.player.gender == 3) && UTIL.rand(1.5) == 0 && changes < changeLimit) {
            ...
            if (liveData.player.countCocksOfType(ENUM.CockType.HORSE) + liveData.player.countCocksOfType(ENUM.CockType.DEMON) < liveData.player.cocks.length) {
                ...
                if (liveData.player.cocks.length == 1) {
                    ...
                    liveData.player.orgasm();
                    if (temp3 == 1)
                        GUI.outputText(" <b>Your penis has transformed into a horse's!</b>");
                }
                //MULTICOCK
                else {
                    GUI.outputText("<br><br>One of your penises begins to feel strange. You pull down your clothes to take a look and see the skin of your " + liveData.player.cockDescript(temp) + " darkening to a mottled brown and black pattern.");
                    temp = liveData.player.changeCockType(ENUM.CockType.HORSE);
                    liveData.player.modStats(["lib", 5], ["sen", 4], ["lus", 35]);
                    liveData.player.orgasm();
                    //Already have a sheath
                    if (liveData.player.hasSheath())
                        GUI.outputText(" Your sheath tingles and begins growing larger as the cock's base shifts to lie inside it.");
                    else
                        GUI.outputText(" You feel a tightness near the base where your skin seems to be bunching up. A sheath begins forming around your " +
                            liveData.player.cockDescript(temp) +
                            "'s root, tightening and pulling your " +
                            liveData.player.cockDescript(temp) +
                            " inside its depths.");
                    temp2 = liveData.player.increaseCock(temp, UTIL.rand(4) + 4);
                    GUI.outputText(" The shaft suddenly explodes with movement, growing longer and developing a thick flared head leaking steady stream of animal-cum.");
                    GUI.outputText(" <b>You now have a horse-cock.</b>");
                }
            ...
            //Players cocks are all horse-type - increase size!
                ...
                if (temp2 > 1 && temp2 <= 2)
                    GUI.outputText("Aching pressure builds within your sheath, suddenly releasing as an inch or more of extra dick flesh spills out. A dollop of pre beads on the head of your enlarged " +
                        liveData.player.cockDescript(temp) +
                        " from the pleasure of the growth.");
                if (temp2 <= 1)
                    GUI.outputText("A slight pressure builds and releases as your " + liveData.player.cockDescript(temp) + " pushes a bit further out of your sheath.");
                changes++;
            }
            ...
            if (UTIL.rand(2) == 0 && changes < changeLimit && liveData.player.countCocksOfType(ENUM.CockType.HORSE) > 0) {
                ...
                GUI.outputText("<br><br>Your " + Appearance.cockNoun(ENUM.CockType.HORSE) + " thickens inside its sheath, growing larger and fatter as your veins thicken, becoming more noticeable. It feels right");
                if (liveData.player.cor + liveData.player.lib < 50)
                    GUI.outputText(" to have such a splendid tool. You idly daydream about cunts and pussies, your " + Appearance.cockNoun(ENUM.CockType.HORSE) + " plowing them relentlessly, stuffing them pregnant with cum");
                if (liveData.player.cor + liveData.player.lib >= 50 && liveData.player.cor + liveData.player.lib < 80)
                    GUI.outputText(" to be this way... You breath the powerful animalistic scent and fantasize about fucking centaurs night and day until their bellies slosh with your cum");
                if (liveData.player.cor + liveData.player.lib >= 75 && liveData.player.cor + liveData.player.lib <= 125)
                    GUI.outputText(" to be a rutting stud. You ache to find a mare or centaur to breed with. Longing to spend your evenings plunging a " +
                        Appearance.cockNoun(ENUM.CockType.HORSE) +
                ...
                if (liveData.player.cor + liveData.player.lib > 125)
                    GUI.outputText(" to whinny loudly like a rutting stallion. Your " +
                        Appearance.cockNoun(ENUM.CockType.HORSE) +
                ...
                GUI.outputText(".");
                if (liveData.player.cor < 30)
                    GUI.outputText(" You shudder in revulsion at the strange thoughts and vow to control yourself better.");
                if (liveData.player.cor >= 30 && liveData.player.cor < 60)
                    GUI.outputText(" You wonder why you thought such odd things, but they have a certain appeal.");
                if (liveData.player.cor >= 60 && liveData.player.cor < 90)
                    GUI.outputText(" You relish your twisted fantasies, hoping to dream of them again.");
                if (liveData.player.cor >= 90)
                    GUI.outputText(" You flush hotly and give a twisted smile, resolving to find a fitting subject to rape and relive your fantasies.");
            ...
            if (UTIL.rand(2) == 0 && changes < changeLimit && liveData.player.ballSize <= 3 && liveData.player.countCocksOfType(ENUM.CockType.HORSE) > 0) {
                if (liveData.player.balls == 0) {
                    liveData.player.balls = 2;
                    liveData.player.ballSize = 1;
                    GUI.outputText("<br><br>A nauseating pressure forms just under the base of your maleness. With agonizing pain the flesh bulges and distends, pushing out a rounded lump of flesh that you recognize as a testicle! A moment later relief overwhelms you as the second drops into your newly formed sack.");
        ...
        if (liveData.player.gender == 2 || liveData.player.gender == 3) {
            ...
            if (liveData.player.vaginas.length == 1) {
                if (liveData.player.vaginas[0].vaginalLooseness <= ENUM.VaginalLoosenessType.VAGINA_LOOSENESS_GAPING && changes < changeLimit && UTIL.rand(2) == 0) {
                    GUI.outputText("<br><br>You grip your gut in pain as you feel your organs shift slightly. When the pressure passes, you realize your " + liveData.player.vaginaDescript(0) + " has grown larger, in depth AND size.");
                    liveData.player.vaginas[0].vaginalLooseness++;
                    changes++;
                }
            ...
            //Multicooch
                ...
                if (liveData.player.vaginas[0].vaginalLooseness <= ENUM.VaginalLoosenessType.VAGINA_LOOSENESS_GAPING && changes < changeLimit && UTIL.rand(2) == 0) {
                    GUI.outputText("<br><br>You grip your gut in pain as you feel your organs shift slightly. When the pressure passes, you realize one of your " + liveData.player.vaginaDescript(temp) + " has grown larger, in depth AND size.");
                ...
                if (UTIL.rand(2) == 0 && changes < changeLimit) {
                    ...
                    if (liveData.player.breastRows.length == 1) {
                        ...
                        if (liveData.player.breastRows[0].breastRating > 3) {
                            ...
                            if (temp == 1)
                                GUI.outputText("<br><br>You feel a weight lifted from you, and realize your " + liveData.player.breastDescript(0) + " have shrunk to a " + liveData.player.breastCup(0) + ".");
                            if (temp == 2)
                                GUI.outputText("<br><br>You feel significantly lighter. Looking down, you realize your breasts are MUCH smaller, down to " + liveData.player.breastCup(0) + "s.");
                            changes++;
                        }
                    }
                    //multiple
                        ...
                        while (temp3 > 0) {
                            ...
                            if (liveData.player.breastRows[temp3].breastRating > 3) {
                                ...
                                if (temp3 < liveData.player.breastRows.length - 1)
                                    GUI.outputText("...and y");
                                else
                                    GUI.outputText("Y");
                                GUI.outputText("our " + liveData.player.breastDescript(temp3) + " shrink, dropping to " + liveData.player.breastCup(temp3) + "s.");
                            }
                        }
                        if (temp2 == 2)
                            GUI.outputText("<br>You feel so much lighter after the change.");
                        if (temp2 == 3)
                            GUI.outputText("<br>Without the extra weight you feel particularly limber.");
                        if (temp2 >= 4)
                            GUI.outputText("<br>It feels as if the weight of the world has been lifted from your shoulders, or in this case, your chest.");
                        if (temp2 > 0)
                            changes++;
                    }
                }
        ...
        if (liveData.player.faceType != ENUM.FaceType.FACE_HORSE && liveData.player.skinType == ENUM.SkinType.SKIN_TYPE_FUR && changes < changeLimit && UTIL.rand(5) == 0 && liveData.player.earType == ENUM.EarType.EARS_HORSE) {
            if (liveData.player.faceType == ENUM.FaceType.FACE_DOG)
                GUI.outputText("<br><br>Mind-numbing pain shatters through you as you feel your facial bones rearranging. You clutch at your face in agony as your skin crawls and shifts, your visage reshaping to replace your dog-like characteristics with those of a horse. <b>You now have a horse's face.</b>");
            else
                GUI.outputText("<br><br>Mind-numbing pain shatters through you as you feel your facial bones breaking and shifting. You clutch at yourself in agony as you feel your skin crawl and elongate under your fingers. Eventually the pain subsides, leaving you with a face that seamlessly blends human and equine features. <b>You have a very equine-looking face.</b>");
            liveData.player.faceType = ENUM.FaceType.FACE_HORSE;
            changes++;
        }
        ...
        if (liveData.player.skinType != ENUM.SkinType.SKIN_TYPE_FUR && changes < changeLimit && UTIL.rand(4) == 0 && liveData.player.tailType == ENUM.TailType.TAIL_TYPE_HORSE) {
            if (liveData.player.skinType == ENUM.SkinType.SKIN_TYPE_PLAIN)
                GUI.outputText("<br><br>An itchy feeling springs up over every inch of your skin. As you scratch yourself madly, you feel fur grow out of your skin until <b>you have a fine coat of " + liveData.player.hairColor + "-colored fur.</b>");
            if (liveData.player.skinType == ENUM.SkinType.SKIN_TYPE_SCALES) {
                liveData.player.skinDesc = "fur";
                GUI.outputText("<br><br>Your " +
        ...
        if (liveData.player.earType != ENUM.EarType.EARS_HORSE && liveData.player.tailType == ENUM.TailType.TAIL_TYPE_HORSE && changes < changeLimit && UTIL.rand(3) == 0) {
            ...
            if (liveData.player.earType == ENUM.EarType.EARS_HUMAN)
                GUI.outputText("<br><br>Your ears tug painfully on your face as they begin shifting, moving upwards to the top of your head and transforming into a upright animalistic ears. ");
            if (liveData.player.earType == ENUM.EarType.EARS_DOG)
                GUI.outputText("<br><br>Your ears change shape, morphing into from their doglike shape into equine-like ears! ");
            if (liveData.player.earType > ENUM.EarType.EARS_DOG)
                GUI.outputText("<br><br>Your ears change shape, morphing into teardrop-shaped horse ears! ");
        ...
        if (liveData.player.tailType != ENUM.TailType.TAIL_TYPE_HORSE && UTIL.rand(2) == 0 && changes < changeLimit) {
            ...
            if (liveData.player.tailType == 0) {
                GUI.outputText("<br><br>There is a sudden tickling on your ass, and you notice you have sprouted a long shiny horsetail of the same " + liveData.player.hairColor + " color as your hair.");
            }
            ...
            if (liveData.player.tailType > ENUM.TailType.TAIL_TYPE_HORSE && liveData.player.tailType <= ENUM.TailType.TAIL_TYPE_COW) {
                GUI.outputText("<br><br>Pain lances up your " + liveData.player.assholeDescript() + " as your tail shifts and morphs disgustingly. With one last wave of pain, it splits into hundreds of tiny filaments, transforming into a horsetail.");
            }
            //if bee/spider-butt.
            if (liveData.player.tailType > ENUM.TailType.TAIL_TYPE_COW && liveData.player.tailType < ENUM.TailType.TAIL_TYPE_SHARK) {
                GUI.outputText("<br><br>Your insect-like abdomen bunches up as it begins shrinking, exoskeleton flaking off like a snake sheds its skin. It bunches up until it is as small as a tennis ball, then explodes outwards, growing into an animalistic tail shape. Moments later, it explodes into filaments of pain, dividing into hundreds of strands and turning into a shiny horsetail.");
            }
        ...
        if (UTIL.rand(3) == 0)
            GUI.outputText(liveData.player.modTone(60, 1));
        //FAILSAFE CHANGE
        if (changes == 0) {
            GUI.outputText("<br><br>Inhuman vitality spreads through your body, invigorating you!<br>");
            liveData.player.changeHP(20, true);
    ...
    static felineTFs() {
        ...
        //Text go!
        GUI.clearOutput();
        GUI.outputText("You take a bite of the fruit and gulp it down. It's thick and juicy and has an almost overpowering sweetness. Nevertheless, it is delicious and you certainly could use a meal. You devour the fruit, stopping only when the hard, nubby pit is left; which you toss aside.");
        //------------
        // STATS CHANGES
        //------------
        //STRENGTH
        if (liveData.player.str < 40 && UTIL.rand(3) == 0 && changes < changeLimit) {
            ...
            if (UTIL.rand(2) == 0)
                GUI.outputText("<br><br>Your muscles feel taut, like a coiled spring, and a bit more on edge.");
            else
                GUI.outputText("<br><br>You arch your back as your muscles clench painfully. The cramp passes swiftly, leaving you feeling like you've gotten a bit stronger.");
            liveData.player.modStats(["str", 1]);
            changes++;
        }
        else if (liveData.player.str > 60 && UTIL.rand(2) == 0) {
            //Strength drops if over 60, does not add to change total
            GUI.outputText("<br><br>Shivers run from your head to your toes, leaving you feeling weak. Looking yourself over, your muscles seemed to have lost some bulk.");
        ...
        if (liveData.player.tou > 50 && UTIL.rand(2) == 0) {
            ...
            GUI.outputText("<br><br>Your body seems to compress momentarily, becoming leaner and noticeably less tough.");
        ...
        if (liveData.player.spe < 75 && UTIL.rand(3) == 0 && changes < changeLimit) {
            //low speed
            if (liveData.player.spe <= 30) {
                GUI.outputText("<br><br>You feel... more balanced, sure of step. You're certain that you've become just a little bit faster.");
                liveData.player.modStats(["spe", 2]);
            }
            //medium speed
            else if (liveData.player.spe <= 60) {
                GUI.outputText("<br><br>You stumble as you shift position, surprised by how quickly you move. After a moment or two of disorientation, you adjust. You're certain that you can run faster now.");
                liveData.player.modStats(["spe", 1]);
            }
            //high speed
            else {
                GUI.outputText("<br><br>You pause mid-step and crouch. Your leg muscles have cramped up like crazy. After a few moments, the pain passes and you feel like you could chase anything down.");
        ...
        if (UTIL.rand(4) == 0 && changes < changeLimit) {
            ...
            if (liveData.player.inte < 15)
                //low intelligence
                GUI.outputText("<br><br>You feel like something is slipping away from you but can't figure out exactly what's happening. You scrunch up your " +
                ...
                //medium intelligence
                GUI.outputText("<br><br>Your mind feels somewhat sluggish, and you wonder if you should just lie down ");
                if (UTIL.rand(2) == 0) {
                    GUI.outputText("somewhere and ");
                    temp = UTIL.rand(3);
                    if (temp == 0)
                        GUI.outputText("toss a ball around or something");
                    else if (temp == 1)
                        GUI.outputText("play with some yarn");
                    else if (temp == 2)
                        GUI.outputText("take a nap and stop worrying");
                }
                else
                    GUI.outputText("in the sun and let your troubles slip away");
                GUI.outputText(".");
            } //High intelligence
            else
                GUI.outputText("<br><br>You start to feel a bit dizzy, but the sensation quickly passes. Thinking hard on it, you mentally brush away the fuzziness that seems to permeate your brain and determine that this fruit may have actually made you dumber. It would be best not to eat too much of it.");
            liveData.player.modStats(["int", -1]);
            changes++;
        }
        //LIBIDO
        if (liveData.player.lib < 80 && changes < changeLimit && UTIL.rand(4) == 0) {
            ...
            if (liveData.player.countCocksOfType(ENUM.CockType.CAT) > 0) {
                ...
                GUI.outputText("<br><br>You feel your " +
                    liveData.player.cockDescript(temp) +
                ...
                if (liveData.player.cor < 33)
                    GUI.outputText("You need to control yourself better.");
                else if (liveData.player.cor < 66)
                    GUI.outputText("You're not sure how you feel about the fantasy.");
                else
                    GUI.outputText("You hope to find a willing partner to make this a reality.");
            }
            //Else –
            else {
                GUI.outputText("<br><br>A rush of tingling warmth spreads through your body as it digests the fruit. You can feel your blood pumping through your extremities, making them feel sensitive and surprisingly sensual. It's going to be hard to resist getting ");
                if (liveData.player.lust > 60)
                    GUI.outputText("even more ");
                GUI.outputText("turned on.");
            }
        ...
        if (UTIL.rand(4) == 0 && changes < changeLimit) {
            ...
            if (liveData.player.goIntoHeat(false)) {
                if (intensified) {
                    if (UTIL.rand(2) == 0)
                        GUI.outputText("<br><br>The itch inside your " + liveData.player.vaginaDescript(0) + " is growing stronger, and you desperately want to find a nice cock to massage the inside.");
                    else
                        GUI.outputText("<br><br>The need inside your " +
                    ...
                    GUI.outputText("<br><br>The interior of your " + liveData.player.vaginaDescript(0) + " clenches tightly, squeezing with reflexive, aching need. Your skin flushes hot ");
                    if (liveData.player.skinType == ENUM.SkinType.SKIN_TYPE_FUR)
                        GUI.outputText("underneath your fur ");
                    GUI.outputText("as images and fantasies ");
                    if (liveData.player.cor < 50)
                        GUI.outputText("assault ");
                    else
                        GUI.outputText("fill ");
                    GUI.outputText(" your mind. Lithe cat-boys with their perfect, spine-covered cocks line up behind you, and you bend over to present your needy pussy to them. You tremble with the desire to feel the exotic texture of their soft barbs rubbing your inner walls, smearing your " +
                        liveData.player.vaginaDescript(0) +
                ...
                for (let k = 0; k < liveData.player.breastRows.length; k++) {
                    ...
                    if (liveData.player.breastRows[k].breastRating > temp2) {
                        ...
                        if (liveData.player.breastRows[k].breastRating > 10) {
                            ...
                            if (temp3 == 0)
                                GUI.outputText("<br><br>The " + liveData.player.breastDescript(0) + " on your chest wobble for a second, then tighten up, losing several cup-sizes in the process!");
                            else
                                GUI.outputText(" The change moves down to your " + UTIL.num2Text2(k + 1) + " row of " + liveData.player.breastDescript(0) + ". They shrink greatly, losing a couple cup-sizes.");
                        }
                        //Small change
                        else {
                            liveData.player.breastRows[k].breastRating -= 1;
                            if (temp3 == 0)
                                GUI.outputText("<br><br>All at once, your sense of gravity shifts. Your back feels a sense of relief, and it takes you a moment to realize your " + liveData.player.breastDescript(k) + " have shrunk!");
                            else
                                GUI.outputText(" Your " + UTIL.num2Text2(k + 1) + " row of " + liveData.player.breastDescript(k) + " gives a tiny jiggle as it shrinks, losing some off its mass.");
                        }
        ...
        if (liveData.player.cockTotal() > 0 &&
            ...
            if (!liveData.player.hasSheath()) {
                GUI.outputText("Then, it begins to shrink and sucks itself inside your body. Within a few moments, a fleshy sheath is formed.");
                if (liveData.player.balls > 0)
                    GUI.outputText(" Thankfully, your balls appear untouched.");
            }
            else
                GUI.outputText("Then, it disappears back into your sheath.");
        ...
        if (liveData.player.countCocksOfType(ENUM.CockType.CAT) > 0 && UTIL.rand(3) == 0 && changes < changeLimit && !liveData.hyperHappy) {
            ...
            if (temp == 1) {
                ...
                if (liveData.player.cocks[j].cockLength > 16) {
                    GUI.outputText("<br><br>Your " + liveData.player.cockDescript(j) + " tingles, making your sheath feel a little less tight. It dwindles in size, losing a full third of its length and a bit of girth before the change finally stops.");
                    liveData.player.cocks[j].cockLength *= 0.66;
                }
                else if (liveData.player.cocks[j].cockLength > 6) {
                    GUI.outputText("<br><br>Your " + liveData.player.cockDescript(j) + " tingles and withdraws further into your sheath. If you had to guess, you'd say you've lost about two inches of total length and perhaps some girth.");
                ...
                GUI.outputText(" Although the package is smaller, it feels even more sensitive - as if it retained all sensation of its larger size in its smaller form.");
                liveData.player.modStats(["sen", 5]);
                //Make note of other dicks changing
                if (temp2 == 1)
                    GUI.outputText(" Upon further inspection, all your " + Appearance.cockNoun(ENUM.CockType.CAT) + "s have shrunk!");
                changes++;
            }
        }
        //------------
        ...
        if (liveData.player.earType != ENUM.EarType.EARS_CAT && UTIL.rand(5) == 0 && changes < changeLimit) {
            ...
            if (liveData.player.earType == ENUM.EarType.EARS_HUMAN) {
                if (UTIL.rand(2) == 0)
                    GUI.outputText("<br><br>The skin on the sides of your face stretches painfully as your ears migrate upwards, towards the top of your head. They shift and elongate a little, fur growing on them as they become feline in nature. <b>You now have cat ears.</b>");
                else
                    GUI.outputText("<br><br>Your ears begin to tingle. You reach up with one hand and gently rub them. They appear to be growing fur. Within a few moments, they've migrated up to the top of your head and increased in size. The tingling stops and you find yourself hearing noises in a whole new way. <b>You now have cat ears.</b>");
            }
            //non human to cat:
            else {
                if (UTIL.rand(2) == 0)
                    GUI.outputText("<br><br>Your ears change shape, morphing into pointed, feline ears! They swivel about reflexively as you adjust to them. <b>You now have cat ears.</b>");
                else
                    GUI.outputText("<br><br>Your ears tingle and begin to change shape. Within a few moments, they've become long and feline. Thanks to the new fuzzy organs, you find yourself able to hear things that eluded your notice up until now. <b>You now have cat ears.</b>");
            }
        ...
        if (liveData.player.tailType != ENUM.TailType.TAIL_TYPE_CAT && liveData.player.earType == ENUM.EarType.EARS_CAT && UTIL.rand(5) == 0 && changes < changeLimit) {
            if (liveData.player.tailType == ENUM.TailType.TAIL_TYPE_NONE) {
                temp = UTIL.rand(3);
                if (temp == 0)
                    GUI.outputText("<br><br>A pressure builds in your backside. You feel under your " +
                        liveData.player.armor.equipmentName +
                ...
                if (temp == 1)
                    GUI.outputText("<br><br>You feel your backside shift and change, flesh molding and displacing into a long, flexible tail! <b>You now have a cat tail.</b>");
                if (temp == 2)
                    GUI.outputText("<br><br>You feel an odd tingling in your spine and your tail bone starts to throb and then swell. Within a few moments it begins to grow, adding new bones to your spine. Before you know it, you have a tail. Just before you think it's over, the tail begins to sprout soft, glossy " +
                        liveData.player.furColor +
                        " fur. <b>You now have a cat tail.</b>");
            }
            else
                GUI.outputText("<br><br>You pause and tilt your head... something feels different. Ah, that's what it is; you turn around and look down at your tail as it starts to change shape, narrowing and sprouting glossy fur. <b>You now have a cat tail.</b>");
        ...
        if (liveData.player.tailType == ENUM.TailType.TAIL_TYPE_CAT && liveData.player.earType == ENUM.EarType.EARS_CAT && UTIL.rand(5) == 0 && changes < changeLimit && liveData.player.lowerBody != ENUM.LowerBodyType.LOWER_BODY_TYPE_CAT) {
            ...
            if (liveData.player.lowerBody == ENUM.LowerBodyType.LOWER_BODY_TYPE_HOOFED) {
                GUI.outputText("<br><br>You feel your hooves suddenly splinter, growing into five unique digits. Their flesh softens as your hooves reshape into furred cat paws. <b>You now have cat paws.</b>");
            ...
            //Goo to cat
                ...
                GUI.outputText("<br><br>Your lower body rushes inward, molding into two leg-like shapes that gradually stiffen up. In moments they solidify into digitigrade legs, complete with soft, padded cat-paws. <b>You now have cat-paws!</b>");
            }
            //non hoof to cat:
            else
                GUI.outputText("<br><br>You scream in agony as you feel the bones in your " + liveData.player.feet() + " break and begin to rearrange. When the pain fades, you feel surprisingly well-balanced. <b>You now have cat paws.</b>");
        ...
        if (liveData.player.tailType == ENUM.TailType.TAIL_TYPE_CAT &&
            ...
            GUI.outputText("You reach down to scratch your arm absent-mindedly and pull your fingers away to find strands of " +
                liveData.player.furColor +
        ...
        if (liveData.player.tailType == ENUM.TailType.TAIL_TYPE_CAT &&
            ...
            temp = UTIL.rand(3);
            if (temp == 0)
                GUI.outputText("<br><br>Your face is wracked with pain. You throw back your head and scream in agony as you feel your cheekbones breaking and shifting, reforming into something... different. You find a puddle to view your reflection and discover <b>your face is now a cross between human and feline features.</b>");
            else if (temp == 1)
                GUI.outputText("<br><br>Mind-numbing pain courses through you as you feel your facial bones rearranging. You clutch at your face in agony as your skin crawls and shifts, your visage reshaping to replace your facial characteristics with those of a feline. <b>You now have an anthropomorphic cat-face.</b>");
            else
                GUI.outputText("<br><br>Your face is wracked with pain. You throw back your head and scream in agony as you feel your cheekbones breaking and shifting, reforming into something else. <b>Your facial features rearrange to take on many feline aspects.</b>");
            liveData.player.faceType = ENUM.FaceType.FACE_CAT;
            changes++;
        }
        if (UTIL.rand(4) == 0 && liveData.player.gills && changes < changeLimit) {
        ...
        if (changes < changeLimit) {
            if (UTIL.rand(2) == 0)
                GUI.outputText(liveData.player.modThickness(5, 2));
            if (UTIL.rand(2) == 0)
                GUI.outputText(liveData.player.modTone(76, 2));
            if (liveData.player.gender < 2) {
                if (UTIL.rand(2) == 0)
                    GUI.outputText(liveData.player.modFem(65, 1));
                else
                    GUI.outputText(liveData.player.modFem(85, 2));
            }
        }
        liveData.gameFlags[FLAG.TIMES_TRANSFORMED] += changes;
    }
    static goblinTFs() {
        ...
        if (liveData.player.str > 50) {
            ...
            if (liveData.player.str > 90)
                liveData.player.modStats(["str", -2]);
            GUI.outputText("<br><br>You feel a little weaker, but maybe it's just the alcohol.");
        }
        //Less tough
        if (liveData.player.tou > 50) {
            GUI.outputText("<br><br>Giggling, you poke yourself, which only makes you giggle harder when you realize how much softer you feel.");
        ...
        if (UTIL.rand(3) == 0 && liveData.player.spe < 50 && changes < changeLimit) {
            ...
            GUI.outputText("<br><br>You feel like dancing, and stumble as your legs react more quickly than you'd think. Is the alcohol slowing you down or are you really faster? You take a step and nearly faceplant as you go off balance. It's definitely both.");
            changes++;
        }
        //------------
        // SEXUAL TFs
        ...
        if (changes < changeLimit && UTIL.rand(3) == 0 && liveData.player.hasVagina() && liveData.player.statusEffectValue(StatusEffects.BonusVCapacity, 1) < 40) {
            ...
            GUI.outputText("<br><br>There is a sudden... emptiness within your " + liveData.player.vaginaDescript(0) + ". Somehow you know you could accommodate even larger... insertions.");
            changes++;
        }
        //Boost fertility
        if (changes < changeLimit && UTIL.rand(4) == 0 && liveData.player.fertility < 40 && liveData.player.hasVagina()) {
            ...
            changes++;
            GUI.outputText("<br><br>You feel strange. Fertile... somehow. You don't know how else to think of it, but you're ready to be a mother.");
        }
        ...
        if (UTIL.rand(2) == 0 && liveData.player.tallness > 48) {
            changes++;
            GUI.outputText("<br><br>The world spins, and not just from the strength of the drink! Your viewpoint is closer to the ground. How fun!");
        ...
        if (changes < changeLimit && UTIL.rand(4) == 0 && (liveData.player.wingType == ENUM.WingType.WING_TYPE_BEE_LIKE_SMALL || liveData.player.wingType == ENUM.WingType.WING_TYPE_BEE_LIKE_LARGE || player.wingType >= WING_TYPE_HARPY)) {
            if (liveData.player.wingType == ENUM.WingType.WING_TYPE_SHARK_FIN)
                ...
                GUI.outputText("<br><br>Your shoulders tingle, feeling lighter. Something lands behind you with a 'thump', and when you turn to look you see your wings have fallen off. This might be the best (and worst) booze you've ever had! <b>You no longer have wings!</b>");
            liveData.player.wingType = ENUM.WingType.WING_TYPE_NONE;
            changes++;
        }
        //Removes wings and antennaes!
        if (changes < changeLimit && UTIL.rand(3) == 0 && liveData.player.antennae > ENUM.AntennaeType.ANTENNAE_NONE) {
            GUI.outputText("<br><br>Your " + liveData.player.hairDescript() + " itches so you give it a scratch, only to have your antennae fall to the ground. What a relief. <b>You've lost your antennae!</b>");
            changes++;
        ...
        if (liveData.player.skinType != ENUM.SkinType.SKIN_TYPE_PLAIN && changes < changeLimit && UTIL.rand(4) == 0 && liveData.player.faceType == ENUM.FaceType.FACE_HUMAN) {
            if (liveData.player.skinType == ENUM.SkinType.SKIN_TYPE_FUR)
                GUI.outputText("<br><br>Your fur itches incessantly, so you start scratching it. It starts coming off in big clumps before the whole mess begins sloughing off your body. In seconds, your skin is nude. <b>You've lost your fur!</b>");
            if (liveData.player.skinType == ENUM.SkinType.SKIN_TYPE_SCALES)
                GUI.outputText("<br><br>Your scales itch incessantly, so you scratch at them. They start falling off wholesale, leaving you standing in a pile of scales after only a few moments. <b>You've lost your scales!</b>");
            if (liveData.player.skinType > ENUM.SkinType.SKIN_TYPE_SCALES)
                GUI.outputText("<br><br>Your " + liveData.player.skinDesc + " itches incessantly, and as you scratch it shifts and changes, becoming normal human-like skin. <b>Your skin is once again normal!</b>");
            liveData.player.skinAdj = "";
        ...
        if (liveData.player.skinTone != "green" && liveData.player.skinTone != "grayish-blue" && liveData.player.skinTone != "dark green" && liveData.player.skinTone != "pale yellow" && changes < changeLimit && UTIL.rand(2) == 0) {
            ...
            if (liveData.player.skinType == ENUM.SkinType.SKIN_TYPE_FUR)
                ...
                GUI.outputText(liveData.player.skinDesc);
        ...
        if (liveData.player.faceType != ENUM.FaceType.FACE_HUMAN && changes < changeLimit && UTIL.rand(4) == 0 && liveData.player.earType == ENUM.EarType.EARS_ELFIN) {
            changes++;
            liveData.player.faceType = ENUM.FaceType.FACE_HUMAN;
            GUI.outputText("<br><br>Another violent sneeze escapes you. It hurt! You feel your nose and discover your face has changed back into a more normal look. <b>You have a human looking face again!</b>");
        }
        //Ears!
        if (liveData.player.earType != ENUM.EarType.EARS_ELFIN && changes < changeLimit && UTIL.rand(3) == 0) {
            GUI.outputText("<br><br>A weird tingling runs through your scalp as your " + liveData.player.hairDescript() + " shifts slightly. You reach up to touch and bump <b>your new pointed elfin ears</b>. You bet they look cute!");
            changes++;
        ...
        if (changes < changeLimit && UTIL.rand(4) == 0 && ((liveData.player.ass.analWetness > 0 && liveData.player.findPerk(PerkLib.MaraesGiftButtslut) < 0) || liveData.player.ass.analWetness > 1)) {
            GUI.outputText("<br><br>You feel a tightening up in your colon and your [asshole] sucks into itself. You feel sharp pain at first but that thankfully fades. Your ass seems to have dried and tightened up.");
    ...
    static humanTFs() {
        ...
        //Text go!
        GUI.clearOutput();
        GUI.outputText("You crack open the small clay jar to reveal a lightly colored paste that smells surprisingly delicious. You begin eating it with your fingers, wishing all the while for some crackers... ");
        liveData.player.slimeFeed();
        liveData.player.refillHunger(20);
        if (liveData.player.humanScore() > 4) {
            GUI.outputText("<br><br>You blink and the world twists around you. You feel more like yourself than you have in a while, but exactly how isn't immediately apparent. Maybe you should take a look at yourself?");
        }
        else {
            GUI.outputText("<br><br>You cry out as the world spins around you. You're aware of your entire body sliding and slipping, changing and morphing, but in the sea of sensation you have no idea exactly what's changing. You nearly black out, and then it's over. Maybe you had best have a look at yourself and see what changed?");
        }
        //------------
        // MAJOR TFs
        //------------
        ...
        if (liveData.player.isTaur() && changes < changeLimit && UTIL.rand(4) == 0) {
            GUI.outputText("<br><br>Your quadrupedal hind-quarters seizes, overbalancing your surprised front-end and causing you to stagger and fall to your side. Pain lances throughout, contorting your body into a tightly clenched ball of pain while tendons melt and bones break, melt, and regrow. When it finally stops, <b>you look down to behold your new pair of human legs</b>!");
        ...
        if (liveData.player.isGoo() && changes < changeLimit && UTIL.rand(4) == 0) {
            GUI.outputText("<br><br>Your lower body rushes inward, molding into two leg-like shapes that gradually stiffen up. In moments they solidify into normal-looking legs, complete with regular, human feet. <b>You now have normal feet!</b>");
            liveData.player.lowerBody = ENUM.LowerBodyType.LOWER_BODY_TYPE_HUMAN;
            liveData.player.legCount = 2;
            changes++;
        }
        ...
        if (liveData.player.isNaga() && changes < changeLimit && UTIL.rand(4) == 0) {
            GUI.outputText("<br><br>You collapse as your sinuous snake-tail tears in half, shifting into legs. The pain is immense, particularly where your new feet are forming. <b>You have human legs again.</b>");
            liveData.player.lowerBody = ENUM.LowerBodyType.LOWER_BODY_TYPE_HUMAN;
            liveData.player.legCount = 2;
            changes++;
        }
        ...
        if (liveData.player.isBiped() && liveData.player.lowerBody != ENUM.LowerBodyType.LOWER_BODY_TYPE_HUMAN && changes < changeLimit && UTIL.rand(4) == 0) {
            GUI.outputText("<br><br>You collapse as your legs shift and twist. By the time the pain subsides, you notice that you have normal legs and normal feet. <b>You now have normal feet!</b>");
            liveData.player.lowerBody = ENUM.LowerBodyType.LOWER_BODY_TYPE_HUMAN;
            liveData.player.legCount = 2;
            changes++;
        }
        ...
        if (liveData.player.findPerk(PerkLib.Incorporeality) >= 0 && changes < changeLimit && UTIL.rand(10) == 0) {
            GUI.outputText("<br><br>You feel a strange sensation in your [legs] as they start to feel more solid. They become more opaque until finally, you can no longer see through your [legs]. <br><b>(Perk Lost: Incorporeality!)</b>");
        ...
        if (liveData.player.skinTone != "tan" && liveData.player.skinTone != "olive" && liveData.player.skinTone != "dark" && liveData.player.skinTone != "light" && changes < changeLimit && UTIL.rand(5) == 0) {
            ...
            if (liveData.player.skinType == ENUM.SkinType.SKIN_TYPE_FUR)
                ...
                GUI.outputText("your " + liveData.player.skinDesc);
            ...
            GUI.outputText(liveData.player.skinTone + " colored.</b>");
        }
        ...
        if (liveData.player.skinType != ENUM.SkinType.SKIN_TYPE_PLAIN && (liveData.player.earType == ENUM.EarType.EARS_HUMAN || liveData.player.earType == ENUM.EarType.EARS_ELFIN) && UTIL.rand(4) == 0 && changes < changeLimit) {
            GUI.outputText("<br><br>A slowly-building itch spreads over your whole body, and as you idly scratch yourself, you find that your " + liveData.player.skinFurScales() + " ");
            if (liveData.player.skinType == ENUM.SkinType.SKIN_TYPE_SCALES)
                GUI.outputText("are");
            else
                GUI.outputText("is");
        ...
        if (liveData.player.faceType != ENUM.FaceType.FACE_HUMAN && changes < changeLimit && UTIL.rand(4) == 0) {
            GUI.outputText("<br><br>Sudden agony sweeps over your " +
                liveData.player.face() +
        ...
        if (liveData.player.tongueType != ENUM.TongueType.TONGUE_HUMAN && changes < changeLimit && UTIL.rand(4) == 0) {
            GUI.outputText("<br><br>You feel something strange inside your face as your tongue shrinks and recedes until it feels smooth and rounded. <b>You realize your tongue has changed back into human tongue!</b>");
        ...
        if (liveData.player.earType != ENUM.EarType.EARS_HUMAN && liveData.player.faceType == ENUM.FaceType.FACE_HUMAN && changes < changeLimit && UTIL.rand(4) == 0) {
            GUI.outputText("<br><br>Ouch, your head aches! It feels like your ears are being yanked out of your head, and when you reach up to hold your aching noggin, you find they've vanished! Swooning and wobbling with little sense of balance, you nearly fall a half-dozen times before <b>a pair of normal, human ears sprout from the sides of your head.</b> You had almost forgotten what human ears felt like!");
        ...
        if (changes < changeLimit && liveData.player.hairType == ENUM.HairType.HAIR_ANEMONE && UTIL.rand(3) == 0) {
            //-insert anemone hair removal into them under whatever criteria you like, though hair removal should precede abdomen growth; here's some sample text:
            GUI.outputText("<br><br>You feel something strange going in on your head. You reach your hands up to feel your tentacle-hair, only to find out that the tentacles have vanished and replaced with normal hair. <b>Your hair is normal again!</b>");
            liveData.player.hairType = ENUM.HairType.HAIR_NORMAL;
            changes++;
        }
        //Remove goo hair
        if (changes < changeLimit && liveData.player.hairType == ENUM.HairType.HAIR_GOO && UTIL.rand(3) == 0) {
            GUI.outputText("<br><br>Your gooey hair begins to fall out in globs, eventually leaving you with a bald head. Your head is not left bald for long, though. Within moments, a full head of hair sprouts from the skin of your scalp. <b>Your hair is normal again!</b>");
            //Turn hair growth on.
        ...
        if (liveData.player.antennae > ENUM.AntennaeType.ANTENNAE_NONE && UTIL.rand(3) == 0 && changes < changeLimit) {
            GUI.outputText("<br><br>The muscles in your brow clench tightly, and you feel a tremendous pressure on your upper forehead. When it passes, you touch yourself and discover your antennae have vanished!");
            liveData.player.antennae = ENUM.AntennaeType.ANTENNAE_NONE;
            changes++;
        }
        //Removes horns
        ...
        if (liveData.player.wingType > ENUM.WingType.WING_TYPE_NONE && UTIL.rand(5) == 0 && changes < changeLimit) {
            if (liveData.player.wingType == ENUM.WingType.WING_TYPE_SHARK_FIN)
                GUI.outputText("<br><br>A wave of tightness spreads through your back, and it feels as if someone is stabbing a dagger into your spine. After a moment the pain passes, though your fin is gone!");
            else
                GUI.outputText("<br><br>A wave of tightness spreads through your back, and it feels as if someone is stabbing a dagger into each of your shoulder-blades. After a moment the pain passes, though your wings are gone!");
            liveData.player.wingType = ENUM.WingType.WING_TYPE_NONE;
            changes++;
        }
        //Removes tail
        if (liveData.player.tailType > ENUM.TailType.TAIL_TYPE_NONE && UTIL.rand(5) == 0 && changes < changeLimit) {
            GUI.outputText("<br><br>You feel something shifting in your backside. Then something detaches from your backside and it falls onto the ground. <b>You no longer have a tail!</b>");
        ...
        if (UTIL.rand(2) == 0 && changes < changeLimit && liveData.player.tallness > 84) {
            GUI.outputText("<br><br>Your skin crawls, making you close your eyes and shiver. When you open them again the world seems... different. After a bit of investigation, you realize you've become shorter!<br>");
        ...
        if (liveData.player.cocks.length > 1 && UTIL.rand(3) == 0 && changes < changeLimit) {
            liveData.player.removeCock(1);
            GUI.outputText("<br><br>You have a strange feeling as your crotch tingles. " +
        ...
        if (liveData.player.balls > 2 && UTIL.rand(3) == 0 && changes < changeLimit) {
            if (liveData.player.ballSize > 2) {
                ...
                liveData.player.ballSize -= 1;
                GUI.outputText("<br><br>Your scrotum slowly shrinks, settling down at a smaller size. <b>Your " + liveData.player.ballsDescriptLight() + " are smaller now.</b><br><br>");
            }
            else {
                liveData.player.balls = 2;
                GUI.outputText("<br><br>Your scrotum slowly shrinks until they seem to have reached a normal size. <b>You can feel as if your extra balls fused together, leaving you with a pair of balls.</b><br><br>");
            }
            changes++;
        }
        ...
        if (liveData.player.hasCock() && changes < changeLimit) {
            if (UTIL.rand(3) == 0 && liveData.player.cocks[0].cockType != ENUM.CockType.HUMAN) {
                GUI.outputText("<br><br>A strange tingling begins behind your " +
                    liveData.player.cockDescript(0) +
        ...
        if (liveData.player.hasCock() && liveData.player.biggestCockLength() > 12 && UTIL.rand(3) == 0 && changes < changeLimit) {
            ...
            if (liveData.player.cocks.length == 1)
                GUI.outputText("<br><br>You feel a tingling sensation as your cock shrinks to a smaller size!");
            else
                GUI.outputText("<br><br>You feel a tingling sensation as the largest of your cocks shrinks to a smaller size!");
            liveData.player.cocks[idx].cockLength -= (UTIL.rand(10) + 2) / 10;
            if (liveData.player.cocks[idx].cockThickness > 1.5) {
                GUI.outputText(" Your " + liveData.player.cockDescript(idx) + " definitely got a bit thinner as well.");
        ...
        if (liveData.player.averageNipplesPerBreast() > 1 && UTIL.rand(4) == 0 && changes < changeLimit) {
            GUI.outputText("<br><br>A tightness arises in your nipples as three out of four on each breast recede completely, the leftover nipples migrating to the middle of your breasts. <b>You are left with only one nipple on each breast.</b>");
        ...
        if (liveData.player.hasVagina() && UTIL.rand(3) == 0 && changes < changeLimit) {
            GUI.outputText("<br><br>The vague numbness in your skin sinks slowly downwards, and you put a hand on your lower stomach as the sensation centers itself there. ");
            liveData.player.modStats(["sen", -2]);
            //High fertility:
            if (liveData.player.fertility >= 30)
                GUI.outputText("It feels like your overcharged reproductive organs have simmered down a bit.");
            //Average fertility:
            else if (liveData.player.fertility >= 10)
                GUI.outputText("You feel like you have dried up a bit inside; you are left feeling oddly tranquil.");
            //[Low/No fertility:
            else {
                GUI.outputText("Although the numbness makes you feel serene, the hummus has no effect upon your ");
                if (liveData.player.fertility > 0)
                    GUI.outputText("mostly ");
                GUI.outputText("sterile system.");
                if (liveData.player.cor > 70)
                    GUI.outputText(" For some reason the fact that you cannot function as nature intended makes you feel helpless and submissive. Perhaps the only way to be a useful creature now is to find a dominant, fertile being willing to plow you full of eggs? You shake the alien, yet oddly alluring thought away.");
            }
        ...
        if (liveData.player.hasCock() && liveData.player.cumMultiplier > 5 && UTIL.rand(3) == 0 && changes < changeLimit) {
            GUI.outputText("<br><br>You feel a strange tingling sensation in your ");
            if (liveData.player.balls > 0)
                GUI.outputText("balls");
            else
                GUI.outputText("groin");
            GUI.outputText(" as you can feel the density reducing. You have a feeling you're going to produce less cum now.");
    ...
    static impTFs() {
        ...
        //Consumption text
        if (liveData.player.hasCock())
            GUI.outputText("The food tastes strange and corrupt - you can't really think of a better word for it, but it's unclean.");
        else
            GUI.outputText("The food tastes... corrupt, for lack of a better word.");
        ...
        if (changes < changeLimit && UTIL.rand(10) == 0 && liveData.player.skinTone != "red") {
            if (liveData.player.skinType == ENUM.SkinType.SKIN_TYPE_FUR)
                GUI.outputText("<br><br>Underneath your fur, your skin ");
            else
                GUI.outputText("<br><br>Your " + liveData.player.skinDesc + " ");
            if (UTIL.rand(2) == 0)
    ...
    static pigTFs(boar) {
        ...
        GUI.outputText("<br><br>As you down the last of your truffle, your entire body begins to convulse violently. Your vision becomes blurry, and you black out.");
        GUI.outputText("<br><br>When you awaken, you are greeted by a large dog licking at your face. The dog seems oddly familiar. \"<i>Bessy, whatcha doin’ girl?</i>\" a voice calls. The voice seems familiar as well. A funny-looking pig on two legs soon appears at the dog’s side. \"<i>Now, now, what do we have here?</i>\" The pig inspects you for a moment, eventually finding a hint of pigtail truffle on your snout.");
        GUI.outputText("<br><br>\"<i>Ah no...</i>\" he says sadly, shaking his head. \"<i>Come with me little " + player.mf("guy", "gal") + ",  I’ve got a place for ya.</i>\"  He then leads you to his shack, nestled in a small clearing in a nearby forest. \"<i>You don’t need ‘ta worry about a thing.  Come ‘ta think of it...</i>\"  he taps his chin for a moment,  \"<i>I know what I could use you for. You could be my own personal truffle hog! The more truffles, the better!</i>\"");
        GUI.outputText("<br><br>You take wonderfully to your new job. Finding truffles is fun, and the funny pig takes great care of you. You couldn’t ask for better. Sure, the world is full of demons and the like, but here, you’re safe, and that’s all you care about.");
        COMBAT.gameOver();
        return;
        }
        }*/
        ...
        if (UTIL.rand(3) == 0 && changes < changeLimit && liveData.player.thickness < 75) {
            GUI.outputText(liveData.player.modThickness(75, 3));
            changes++;
        }
        //Decrease muscle tone
        if (UTIL.rand(3) == 0 && changes < changeLimit && liveData.player.gender >= 2 && liveData.player.tone > 20) {
            GUI.outputText(liveData.player.modTone(20, 4));
            changes++;
        }
        //Increase hip rating
        if (UTIL.rand(3) == 0 && changes < changeLimit && liveData.player.gender >= 2 && liveData.player.hipRating < 15) {
            GUI.outputText("<br><br>Your gait shifts slightly to accommodate your widening " + liveData.player.hipDescript() + ". The change is subtle, but they're definitely broader.");
            liveData.player.hipRating++;
            changes++;
        }
        //Increase ass rating
        if (UTIL.rand(3) == 0 && changes < changeLimit && liveData.player.buttRating < 12) {
            GUI.outputText("<br><br>When you stand back, up your [ass] jiggles with a good bit of extra weight.");
        ...
        if (UTIL.rand(3) == 0 && changes < changeLimit && liveData.player.balls > 0 && liveData.player.ballSize < 4) {
            if (liveData.player.ballSize < 3)
                ...
                GUI.outputText("<br><br>A sudden onset of heat envelops your groin, focusing on your ballsack. Walking becomes difficult as you discover your " + (liveData.player.balls == 4 ? "quartette" : "duo") + " of testicles have enlarged again.");
            liveData.player.ballSize++;
            changes++;
        }
        //------------
        ...
        if (UTIL.rand(4) == 0 && changes < changeLimit && liveData.player.cocks.length > 0 && liveData.player.cocks[0].cockType != ENUM.CockType.PIG) {
            if (liveData.player.cocks.length == 1) {
                //Single cock
                GUI.outputText("<br><br>You feel an uncomfortable pinching sensation in your [cock]. " +
                ...
                //Multiple cocks
                GUI.outputText("<br><br>You feel an uncomfortable pinching sensation in one of your cocks. You pull open your [armor], watching as it warps and changes. As the transformation completes, you're left with a shiny pinkish red pecker ending in a prominent corkscrew at the tip. <b>You now have a pig penis!</b>");
        ...
        if (UTIL.rand(boar ? 3 : 4) == 0 && changes < changeLimit && liveData.player.earType != ENUM.EarType.EARS_PIG) {
            GUI.outputText("<br><br>You feel a pressure on your ears as they begin to reshape. Once the changes finish, you flick them about experimentally, <b>and you're left with pointed, floppy pig ears.</b>");
        ...
        if (UTIL.rand(boar ? 2 : 3) == 0 && changes < changeLimit && liveData.player.earType == ENUM.EarType.EARS_PIG && liveData.player.tailType != ENUM.TailType.TAIL_TYPE_PIG) {
            if (liveData.player.tailType > 0)
                ...
                GUI.outputText("<br><br>You feel a pinching sensation in your [tail] as it begins to warp in change. When the sensation dissipates, <b>you are left with a small, curly pig tail.</b>");
            //If you don't have a tail.
            else
                GUI.outputText("<br><br>You feel a tug at the base of your spine as it lengthens ever so slightly. Looking over your shoulder, <b>you find that you have sprouted a small, curly pig tail.</b>");
        ...
        if (UTIL.rand(boar ? 2 : 3) == 0 &&
            ...
            GUI.outputText("<br><br>There is a tingling in your [tail] as it begins to warp and change. When the sensation dissipates, <b>you are left with a small, curly pig tail.</b> This new, mismatched tail looks a bit odd on your horse lower body.");
            liveData.player.tailType = ENUM.TailType.TAIL_TYPE_PIG;
            changes++;
        }
        ...
        if (UTIL.rand(boar ? 3 : 4) == 0 &&
            ...
            if (liveData.player.isTaur())
                //Centaur
                GUI.outputText("<br><br>You scream in agony as a horrible pain racks your entire bestial lower half. Unable to take it anymore, you pass out. When you wake up, you're shocked to find that you no longer have the animal's lower body. Instead, you only have two legs. They are digitigrade and end in cloven hooves. <b>You now have pig legs!</b>");
            else if (liveData.player.lowerBody == ENUM.LowerBodyType.LOWER_BODY_TYPE_NAGA)
                //Naga
                GUI.outputText("<br><br>You scream in agony as a horrible pain racks the entire length of your snake-like coils. Unable to take it anymore, you pass out. When you wake up, you're shocked to find that you no longer have the lower body of a snake. Instead, you only have two legs. They are digitigrade and end in cloven hooves. <b>You now have pig legs!</b>");
            //Bipedal
            else
                GUI.outputText("<br><br>You scream in agony as the bones in your legs break and rearrange. Once the pain subsides, you inspect your legs, finding that they are digitigrade and ending in cloven hooves. <b>You now have pig legs!</b>");
            liveData.player.lowerBody = ENUM.LowerBodyType.LOWER_BODY_TYPE_CLOVEN_HOOFED;
            liveData.player.legCount = 2;
            changes++;
        }
        ...
        if (UTIL.rand(boar ? 2 : 3) == 0 &&
            ...
            GUI.outputText("<br><br>You cry out in pain as the bones in your face begin to break and rearrange. You rub your face furiously in an attempt to ease the pain, but to no avail. As the sensations pass, you examine your face in a nearby puddle. <b>You nearly gasp in shock at the sight of your new pig face!</b>");
            liveData.player.faceType = ENUM.FaceType.FACE_PIG;
            changes++;
        }
        ...
        if (UTIL.rand(boar ? 3 : 4) == 0 && changes < changeLimit) {
            ...
            GUI.outputText("<br><br>Your skin tingles ever so slightly as you skin's color changes before your eyes. As the tingling diminishes, you find that your skin has turned " + skinToBeChosen + ".");
            liveData.player.skinTone = skinToBeChosen;
            changes++;
        }
        if (changes == 0) {
            GUI.outputText("<br><br>Oddly, you do not feel any changes. Perhaps you're lucky? Or not.");
        }
        liveData.gameFlags[FLAG.TIMES_TRANSFORMED] += changes;
    }
    static lizardTFs() {
        ...
        //clear screen
        GUI.clearOutput();
        GUI.outputText("You uncork the vial of fluid and drink it down.  The taste is sour, like a dry wine with an aftertaste not entirely dissimilar to alcohol.  Instead of the warmth you'd expect, it leaves your throat feeling cold and a little numb.");
        //Statistical changes:
        //-Reduces speed down to 50.
        if (liveData.player.spe > 50 && changes < changeLimit && UTIL.rand(4) == 0) {
            GUI.outputText("<br><br>You start to feel sluggish and cold.  Lying down to bask in the sun might make you feel better.");
            liveData.player.dynStats(["spe", -1]);
            changes++;
        }
        //-Reduces sensitivity.
        if (liveData.player.sens > 20 && changes < changeLimit && UTIL.rand(3) == 0) {
            GUI.outputText("<br><br>The sensation of prickly pins and needles moves over your body, leaving your senses a little dulled in its wake.");
        ...
        if (liveData.player.lib < 100 && changes < changeLimit && UTIL.rand(3) == 0) {
            GUI.outputText("<br><br>A knot of fire in your gut doubles you over but passes after a few moments.  As you straighten you can feel the heat seeping into you, ");
            //(DICK)
            if (liveData.player.cocks.length > 0 && (liveData.player.gender != 3 || UTIL.rand(2) == 0)) {
                GUI.outputText("filling ");
                if (liveData.player.cocks.length > 1)
                    GUI.outputText("each of ");
                GUI.outputText("your " + liveData.player.multiCockDescriptLight() + " with the desire to breed.  You get a bit hornier when you realize your sex-drive has gotten a boost.");
            }
            //(COOCH)
            else if (liveData.player.hasVagina())
                GUI.outputText("puddling in your " + liveData.player.vaginaDescript(0) + ".  An instinctive desire to mate and lay eggs spreads through you, increasing your lust and boosting your sex-drive.");
            //(TARDS)
            else
                GUI.outputText("puddling in your featureless crotch for a split-second before it slides into your " +
        ...
        if (liveData.player.tou < 70 && changes < changeLimit && UTIL.rand(3) == 0) {
            ...
            if (liveData.player.tou < 40) {
                GUI.outputText("<br><br>Your body and skin both thicken noticeably.  You pinch your " + liveData.player.skinDesc + " experimentally and marvel at how much tougher your hide has gotten.");
                liveData.player.dynStats(["tou", 3]);
            }
            //(+2)
            else if (liveData.player.tou < 55) {
                GUI.outputText("<br><br>You grin as you feel your form getting a little more solid.  It seems like your whole body is toughening up quite nicely, and by the time the sensation goes away, you feel ready to take a hit.");
                liveData.player.dynStats(["tou", 2]);
            }
            //(+1)
            else {
                GUI.outputText("<br><br>You snarl happily as you feel yourself getting even tougher.  It's a barely discernible difference, but you can feel your " + liveData.player.skinDesc + " getting tough enough to make you feel invincible.");
        ...
        if (liveData.player.countCocksOfType(ENUM.CockType.LIZARD) == 0 && liveData.player.cockTotal() > 0 && changes < changeLimit && UTIL.rand(4) == 0) {
            ...
            GUI.outputText("<br><br>A slow tingle warms your groin.  Before it can progress any further, you yank back your " +
                liveData.player.armor.equipmentName +
                " to investigate.  Your " +
                liveData.player.cockDescript(temp2) +
                " is changing!  It ripples loosely from ");
            if (liveData.player.hasSheath())
                GUI.outputText("sheath ");
            else
                GUI.outputText("base ");
            GUI.outputText("to tip, undulating and convulsing as its color lightens, darkens, and finally settles on a purplish hue.  Your " +
                Appearance.cockNoun(ENUM.CockType.HUMAN) +
            ...
            if (liveData.player.cor < 33)
                GUI.outputText("horrifies you.");
            else if (liveData.player.cor < 66)
                GUI.outputText("is a little strange for your tastes.");
            else {
                GUI.outputText("looks like it might be more fun to receive than use on others.  ");
                if (liveData.player.hasVagina())
                    GUI.outputText("Maybe you could find someone else with one to ride?");
                else
                    GUI.outputText("Maybe you should test it out on someone and ask them exactly how it feels?");
            }
            GUI.outputText("  <b>You now have a bulbous, lizard-like cock.</b>");
            //Actually xform it nau
            if (liveData.player.hasSheath()) {
                liveData.player.cocks[temp2].cockType = ENUM.CockType.LIZARD;
                if (!liveData.player.hasSheath())
                    GUI.outputText("<br><br>Your sheath tightens and starts to smooth out, revealing ever greater amounts of your " +
                        liveData.player.cockDescript(temp2) +
        ...
        if (liveData.player.cockTotal() > 1 && liveData.player.countCocksOfType(ENUM.CockType.LIZARD) > 0 && liveData.player.cockTotal() > liveData.player.countCocksOfType(ENUM.CockType.LIZARD) && UTIL.rand(4) == 0 && changes < changeLimit) {
            GUI.outputText("<br><br>A familiar tingle starts in your crotch, and before you can miss the show, you pull open your " + liveData.player.armor.equipmentName + ".  As if operating on a cue, ");
            for (temp2 = 0; temp2 < liveData.player.cocks.length; temp2++) {
                //Stop loopahn when dick be found
                if (liveData.player.cocks[temp2].cockType != ENUM.CockType.LIZARD)
                    break;
            ...
            if (liveData.player.cockTotal() == 2)
                GUI.outputText("your other dick");
            else
                GUI.outputText("another one of your dicks");
            GUI.outputText(" starts to change into the strange reptilian shape you've grown familiar with.  It warps visibly, trembling and radiating pleasurable feelings back to you as the transformation progresses.  ");
            if (liveData.player.cumQ() < 50)
                GUI.outputText("pre-cum oozes from the tip");
            else if (liveData.player.cumQ() < 700)
                GUI.outputText("Thick pre-cum rains from the tip");
            else
                GUI.outputText("A wave of pre-cum splatters on the ground");
            GUI.outputText(" from the pleasure of the change.  In moments <b>you have a bulbous, lizard-like cock.</b>");
        ...
        if (liveData.player.countCocksOfType(ENUM.CockType.LIZARD) == 1 && liveData.player.cocks.length == 1 && UTIL.rand(4) == 0 && changes < changeLimit) {
            GUI.outputText("<br><br>A knot of pressure forms in your groin, forcing you off your " +
        ...
        if (liveData.player.countCocksOfType(ENUM.CockType.LIZARD) == liveData.player.cockTotal() && changes < changeLimit && liveData.player.findStatusEffect(StatusEffects.Infested) >= 0) {
            GUI.outputText("<br><br>Like rats from a sinking ship, worms escape from your body in a steady stream.  Surprisingly, the sensation is remarkably pleasant, similar to the pleasure of sexual release in a way.  Though they seem inexhaustible, the tiny, cum-slimed invertebrates slow to a trickle.  The larger worm-kin inside you stirs as if disturbed from a nap, coming loose from whatever moorings it had attached itself to in the interior of your form.  It slowly works its way up your urethra, stretching to an almost painful degree with every lurching motion.  Your dick bloats out around the base, stretched like the ovipositor on a bee-girl in order to handle the parasitic creature, but thankfully, the ordeal is a brief one.");
            if (liveData.player.balls > 1)
                GUI.outputText("  The remaining " + UTIL.num2Text(liveData.player.balls - 1) + " slither out the pre-stretched holes with ease, though the last one hangs from your tip for a moment before dropping to the ground.");
            GUI.outputText("  The white creature joins its kin on the ground and slowly slithers away.  Perhaps they prefer mammals? In any event, <b>you are no longer infected with worms</b>.");
        ...
        if (liveData.player.biggestTitSize() >= 1 && liveData.player.gender == 1 && changes < changeLimit && UTIL.rand(3) == 0) {
            ...
            if (liveData.player.biggestTitSize() > 8) {
                GUI.outputText("<br><br>The flesh on your chest tightens up, losing nearly half its mass in the span of a few seconds.  With your center of balance shifted so suddenly, you stagger about trying not to fall on your ass.  You catch yourself and marvel at the massive change in breast size.");
                //Half tit size
            }
            //(NOT HUEG < 4)
            else
                GUI.outputText("<br><br>In an instant, your chest compacts in on itself, consuming every ounce of breast-flesh.  You're left with a  smooth, masculine torso, though your nipples remain.");
            //(BOTH – no new PG)
            GUI.outputText("  With the change in weight and gravity, you find it's gotten much easier to move about.");
        ...
        if (liveData.player.biggestLactation() >= 1 && changes < changeLimit && UTIL.rand(4) == 0) {
            if (liveData.player.totalNipples() == 2)
                GUI.outputText("<br><br>Both of your");
            else
                GUI.outputText("<br><br>All of your many");
            GUI.outputText(" nipples relax.  It's a strange feeling, and you pull back your top to touch one.  It feels fine, though there doesn't seem to be any milk leaking out.  You give it a squeeze and marvel when nothing ");
            if (liveData.player.hasFuckableNipples())
                GUI.outputText("but sexual fluid ");
            GUI.outputText("escapes it.  <b>You are no longer lactating.</b>  That makes sense, only mammals lactate!  Smiling, you muse at how much time this will save you when cleaning your gear.");
        ...
        if (liveData.player.hasVagina() && liveData.player.findPerk(PerkLib.Oviposition) < 0 && changes < changeLimit && UTIL.rand(5) == 0 && liveData.player.lizardScore() > 3) {
            GUI.outputText("<br><br>Deep inside yourself there is a change.  It makes you feel a little woozy, but passes quickly.  Beyond that, you aren't sure exactly what just happened, but you are sure it originated from your womb.<br>");
            ...
            if (liveData.player.hornType != ENUM.HornType.HORNS_DRACONIC_X2 && liveData.player.hornType != ENUM.HornType.HORNS_DRACONIC_X4_12_INCH_LONG) {
                ...
                if (liveData.player.horns > 0) {
                    ...
                    if (liveData.player.hornType == ENUM.HornType.HORNS_DEMON && liveData.player.horns > 4) {
                        GUI.outputText("<br><br>Your horns condense, twisting around each other and merging into larger, pointed protrusions.  By the time they finish you have four draconic-looking horns, each about twelve inches long.");
                        liveData.player.horns = 12;
                        liveData.player.hornType = ENUM.HornType.HORNS_DRACONIC_X4_12_INCH_LONG;
                    }
                    else {
                        GUI.outputText("<br><br>You feel your horns changing and warping, and reach back to touch them.  They have a slight curve and a gradual taper.  They must look something like the horns the dragons in your village's legends always had.");
                ...
                if (liveData.player.hornType == ENUM.HornType.HORNS_DRACONIC_X2) {
                    if (liveData.player.horns < 12) {
                        if (UTIL.rand(2) == 0) {
                            ...
                            GUI.outputText("<br><br>Your head aches as your horns grow a few inches longer.  They get even thicker about the base, giving you a menacing appearance.");
                            liveData.player.horns += 2 + UTIL.rand(4);
                        }
                        if (liveData.player.horns >= 12)
                            GUI.outputText("  <b>Your horns settle down quickly, as if they're reached their full size.</b>");
                        changes++;
                    }
                    //maxxed out, new row
                    else {
                        //--Next horn growth adds second row and brings length up to 12\"
                        GUI.outputText("<br><br>A second row of horns erupts under the first, and though they are narrower, they grow nearly as long as your first row before they stop.  A sense of finality settles over you.  <b>You have as many horns as a lizan can grow.</b>");
                        liveData.player.hornType = ENUM.HornType.HORNS_DRACONIC_X4_12_INCH_LONG;
                        changes++;
                    }
                }
        ...
        if (liveData.gameFlags[FLAG.HAIR_GROWTH_STOPPED_BECAUSE_LIZARD] == 0 && changes < changeLimit && UTIL.rand(4) == 0) {
            GUI.outputText("<br><br>Your scalp tingles oddly.  In a panic, you reach up to your " + liveData.player.hairDescript() + ", but thankfully it appears unchanged.<br><br>");
            GUI.outputText("(<b>Your hair has stopped growing.</b>)");
            changes++;
            liveData.gameFlags[FLAG.HAIR_GROWTH_STOPPED_BECAUSE_LIZARD]++;
        }
        //Remove beard!
        if (liveData.player.hasBeard() && changes < changeLimit && UTIL.rand(3) == 0) {
            GUI.outputText("<br><br>Your " + liveData.player.beardDescript() + " feels looser and looser until finally, your beard falls out.  ");
            GUI.outputText("(<b>You no longer have a beard!</b>)");
        ...
        if (liveData.player.lowerBody != ENUM.LowerBodyType.LOWER_BODY_TYPE_LIZARD && changes < changeLimit && UTIL.rand(5) == 0) {
            //Hooves -
            if (liveData.player.lowerBody == ENUM.LowerBodyType.LOWER_BODY_TYPE_HOOFED)
                GUI.outputText("<br><br>You scream in agony as you feel your hooves crack and break apart, beginning to rearrange.  Your legs change to a digitigrade shape while your feet grow claws and shift to have three toes on the front and a smaller toe on the heel.");
            //TAURS -
            else if (liveData.player.isTaur())
                GUI.outputText("<br><br>Your lower body is wracked by pain!  Once it passes, you discover that you're standing on digitigrade legs with lizard-like claws.");
            //feet types -
                ...
                GUI.outputText("<br><br>You scream in agony as you feel the bones in your legs break and begin to rearrange. They change to a digitigrade shape while your feet grow claws and shift to have three toes on the front and a smaller toe on the heel.");
            //Else –
            else
                GUI.outputText("<br><br>Pain rips through your " +
                    liveData.player.legs() +
        ...
        if (liveData.player.tailType != ENUM.TailType.TAIL_TYPE_LIZARD && liveData.player.lowerBody == ENUM.LowerBodyType.LOWER_BODY_TYPE_LIZARD && changes < changeLimit && UTIL.rand(5) == 0) {
            //No tail
            if (liveData.player.tailType == ENUM.TailType.TAIL_TYPE_NONE)
                GUI.outputText("<br><br>You drop onto the ground as your spine twists and grows, forcing the flesh above your " +
                    liveData.player.assDescript() +
            ...
            //Yes tail
                ...
                GUI.outputText("<br><br>You drop to the ground as your tail twists and grows, changing its shape in order to gradually taper to a point.  It flicks back and forth, prehensile and totally under your control.  <b>You now have a reptilian tail.</b>");
        ...
        if (liveData.player.earType != ENUM.EarType.EARS_LIZARD && liveData.player.tailType == ENUM.TailType.TAIL_TYPE_LIZARD && liveData.player.lowerBody == ENUM.LowerBodyType.LOWER_BODY_TYPE_LIZARD && changes < changeLimit && UTIL.rand(5) == 0) {
            GUI.outputText("<br><br>Tightness centers on your scalp, pulling your ears down from their normal, fleshy shape into small, scaley bumps with holes in their centers.  <b>You have reptilian ears!</b>");
        ...
        if (liveData.player.skinType != ENUM.SkinType.SKIN_TYPE_SCALES &&
            ...
            if (liveData.player.skinType == ENUM.SkinType.SKIN_TYPE_FUR) {
                ...
                GUI.outputText("<br><br>You scratch yourself, and come away with a large clump of " +
                    liveData.player.furColor +
            ...
            //(no fur)
                ...
                GUI.outputText("<br><br>You idly reach back to scratch yourself and nearly jump out of your " +
                    liveData.player.armor.equipmentName +
        ...
        if (liveData.player.faceType != ENUM.FaceType.FACE_LIZARD &&
            ...
            changes < changeLimit &&
            UTIL.rand(5) == 0) {
            GUI.outputText("<br><br>Terrible agony wracks your " +
                liveData.player.face() +
    ...
    static harpyTFs(type) {
        ...
        if (liveData.player.spe < 100 && UTIL.rand(3) == 0) {
            ...
            if (liveData.player.spe >= 75)
                GUI.outputText("<br><br>A familiar chill runs down your spine. Your muscles feel like well oiled machinery, ready to snap into action with lightning speed.");
            else
                GUI.outputText("<br><br>A chill runs through your spine, leaving you feeling like your reflexes are quicker and your body faster.");
        ...
        if (liveData.player.tou > 50 && UTIL.rand(3) == 0 && changes < changeLimit) {
            changes++;
            if (UTIL.rand(2) == 0)
                GUI.outputText("<br><br>A nice, slow warmth rolls from your gut out to your limbs, flowing through them before dissipating entirely. As it leaves, you note that your body feels softer and less resilient.");
            else
                GUI.outputText("<br><br>You feel somewhat lighter, but consequently more fragile.  Perhaps your bones have changed to be more harpy-like in structure?");
            liveData.player.dynStats(["tou", -1]);
        }
        //antianemone corollary:
        if (changes < changeLimit && liveData.player.hairType == 4 && UTIL.rand(2) == 0) {
            //-insert anemone hair removal into them under whatever criteria you like, though hair removal should precede abdomen growth; here's some sample text:
            GUI.outputText("<br><br>As you down the seed, your head begins to feel heavier.  Reaching up, you notice your tentacles becoming soft and somewhat fibrous.  Pulling one down reveals that it feels soft and fluffy, almost feathery; you watch as it dissolves into many thin, feathery strands.  <b>Your hair is now like that of a harpy!</b>");
        ...
        if (liveData.player.str < 70 && UTIL.rand(3) == 0 && changes < changeLimit) {
            ...
            if (liveData.player.str < 40)
                GUI.outputText("<br><br>Shivering, you feel a feverish sensation that reminds you of the last time you got sick. Thankfully, it passes swiftly, leaving slightly enhanced strength in its wake.");
            //(hi str – 50+)
            else
                GUI.outputText("<br><br>Heat builds in your muscles, their already-potent mass shifting slightly as they gain even more strength.");
            //Faster until 40 str.
            if (liveData.player.str < 40)
        ...
        if ((liveData.player.lib < 90 || UTIL.rand(3) == 0) && UTIL.rand(3) == 0 && changes < changeLimit) {
            ...
            if (liveData.player.lib < 40) {
                GUI.outputText("<br><br>A passing flush colors your " + liveData.player.face() + " for a second as you daydream about sex. You blink it away, realizing the item seems to have affected your libido.");
                if (liveData.player.hasVagina())
                    GUI.outputText(" The moistness of your " + liveData.player.vaginaDescript() + " seems to agree.");
                else if (liveData.player.hasCock())
                    GUI.outputText(" The hardness of " + liveData.player.sMultiCockDesc() + " seems to agree.");
                liveData.player.changeLust(5);
            }
            //(sub 75 lib)
            else if (liveData.player.lib < 75)
                GUI.outputText("<br><br>Heat, blessed heat, works through you from head to groin, leaving you to shudder and fantasize about the sex you could be having right now.<br><br>");
            //(hi lib)
            else if (liveData.player.lib < 90)
                GUI.outputText("<br><br>Sexual need courses through you, flushing your skin with a reddish hue while you pant and daydream of the wondrous sex you should be having right now.<br><br>");
            //(90+)
            else
                GUI.outputText("<br><br>You groan, something about the seed rubbing your libido in just the right way to make you horny. Panting heavily, you sigh and fantasize about the sex you could be having.<br><br>");
            //(fork to fantasy)
            if (liveData.player.lib >= 40) {
                ...
                if (liveData.player.hasCock() && (liveData.player.gender != 3 || UTIL.rand(2) == 0)) {
                    //(male 1)
                    if (UTIL.rand(2) == 0) {
                        GUI.outputText("In your fantasy you're winging through the sky, " +
                        ...
                        if (liveData.player.cockTotal() > 1) {
                            GUI.outputText("The extra penis");
                            if (liveData.player.cockTotal() > 2)
                                GUI.outputText("es rub ");
                            else
                                GUI.outputText("rubs ");
                            GUI.outputText("the skin over her taut, empty belly, drooling your need atop her.  ");
                            GUI.outputText("You jolt from the vision unexpectedly, finding your " +
                                liveData.player.sMultiCockDesc() +
        ...
        if (liveData.player.gender == 0 || (!liveData.player.hasVagina() && changes < changeLimit && UTIL.rand(3) == 0)) {
            ...
            //(balls)
            if (liveData.player.balls > 0)
                GUI.outputText("<br><br>An itch starts behind your " +
                    liveData.player.ballsDescriptLight() +
            ...
            //(dick)
                ...
                GUI.outputText("<br><br>An itch starts on your groin, just below your " +
                    liveData.player.multiCockDescriptLight() +
            ...
            //(neither)
                ...
                GUI.outputText("<br><br>An itch starts on your groin and fades before you can take action. Curious about the intermittent sensation, <b>you peek under your " +
                    liveData.player.armor.equipmentName +
        ...
        //Cannot happen at same time as row removal
            ...
            //(H+)
                ...
                GUI.outputText("<br><br>Your chest pinches tight, wobbling dangerously for a second before the huge swell of your bust begins to shrink into itself. The weighty mounds jiggle slightly as they shed cup sizes like old, discarded coats, not stopping until they're " +
        ...
        if (changes < changeLimit && liveData.player.breastRows[0].breastRating < 2 && UTIL.rand(3) == 0) {
            changes++;
            GUI.outputText("<br><br>Your chest starts to tingle, the " +
                liveData.player.skinDesc +
                " warming under your " +
                liveData.player.armor.equipmentName +
            ...
            if (liveData.player.breastRows[0].breastRating < 1)
                GUI.outputText("  <b>You have breasts now!</b>");
            liveData.player.breastRows[0].breastRating = 2;
        }
        ...
        if (changes < changeLimit && liveData.player.hasCock() && liveData.player.countCocksOfType(ENUM.CockType.AVIAN) < liveData.player.cockTotal() && UTIL.rand(type == 1 ? 4 : 10) == 0) {
            ...
            changes++;
            GUI.outputText("<br><br>You feel a strange tingling sensation in your cock as erection forms. You " +
        ...
        if (liveData.player.femininity < 85 && changes < changeLimit && UTIL.rand(3) == 0) {
            changes++;
            GUI.outputText(liveData.player.modFem(85, 3 + UTIL.rand(5)));
        }
        //-Skin color change – tan, olive, dark, light
        if (liveData.player.skinTone != "tan" && liveData.player.skinTone != "olive" && liveData.player.skinTone != "dark" && liveData.player.skinTone != "light" && changes < changeLimit && UTIL.rand(5) == 0) {
            changes++;
        ...
        if (liveData.player.hipRating >= 15 && changes < changeLimit && UTIL.rand(3) == 0) {
            GUI.outputText("<br><br>Your gait shifts inward, your " + liveData.player.hipDescript() + " narrowing significantly. They remain quite thick, but they're not as absurdly wide as before.");
            liveData.player.hipRating--;
            changes++;
        }
        //-Big booty
        if (liveData.player.buttRating < 8 && changes < changeLimit && UTIL.rand(3) == 0) {
            liveData.player.buttRating++;
            changes++;
            GUI.outputText("<br><br>A slight jiggle works through your rear, but instead of stopping it starts again. You can actually feel your " +
                liveData.player.armor.equipmentName +
        ...
        if (liveData.player.buttRating >= 14 && changes < changeLimit && UTIL.rand(4) == 0) {
            changes++;
            liveData.player.buttRating--;
            GUI.outputText("<br><br>A feeling of tightness starts in your " +
                liveData.player.buttDescript() +
        ...
        if (liveData.player.thickness > 25 && changes < changeLimit && UTIL.rand(3) == 0) {
            GUI.outputText(liveData.player.modThickness(25, 3 + UTIL.rand(4)));
            changes++;
        }
        //Remove odd eyes
        if (changes < changeLimit && UTIL.rand(5) == 0 && liveData.player.eyeType > ENUM.EyeType.EYES_HUMAN) {
        ...
        if (liveData.player.lowerBody != ENUM.LowerBodyType.LOWER_BODY_TYPE_HARPY && changes < changeLimit && (type == 1 || liveData.player.tailType == ENUM.TailType.TAIL_TYPE_HARPY) && UTIL.rand(4) == 0) {
            ...
            //goo
                ...
                GUI.outputText("<br><br>Your gooey undercarriage loses some of its viscosity, dumping you into the puddle that was once your legs. As you watch, the fluid pulls together into a pair of distinctly leg-like shapes, solidifying into a distinctly un-gooey form. You've even regained a pair of feet!  ");
            liveData.player.lowerBody = ENUM.LowerBodyType.LOWER_BODY_TYPE_HARPY;
            liveData.player.legCount = 2;
            changes++;
            //(cont)
            GUI.outputText("While humanoid in shape, they have two large, taloned toes on the front and a single claw protruding from the heel. The entire ensemble is coated in " +
                liveData.player.hairColor +
        ...
        if (liveData.player.tailType != ENUM.TailType.TAIL_TYPE_HARPY && changes < changeLimit && (type == 1 || liveData.player.wingType == ENUM.WingType.WING_TYPE_FEATHERED_LARGE) && UTIL.rand(4) == 0) {
            //(tail)
            if (liveData.player.tailType > ENUM.TailType.TAIL_TYPE_NONE)
                GUI.outputText("<br><br>Your tail shortens, folding into the crack of your " +
                    liveData.player.buttDescript() +
            ...
            //(no tail)
                ...
                GUI.outputText("<br><br>A tingling tickles the base of your spine, making you squirm in place. A moment later, it fades, but a fan of feathers erupts from your " +
                    liveData.player.skinDesc +
        ...
        if (liveData.player.wingType == ENUM.WingType.WING_TYPE_NONE && changes < changeLimit && (type == 1 || liveData.player.armType == ENUM.ArmType.ARM_TYPE_HARPY) && UTIL.rand(4) == 0) {
            GUI.outputText("<br><br>Pain lances through your back, the muscles knotting oddly and pressing up to bulge your " +
                liveData.player.skinDesc +
        ...
        if (liveData.player.wingType != ENUM.WingType.WING_TYPE_FEATHERED_LARGE && liveData.player.wingType > ENUM.WingType.WING_TYPE_NONE && changes < changeLimit && UTIL.rand(4) == 0) {
            if (liveData.player.wingType != ENUM.WingType.WING_TYPE_SHARK_FIN)
                GUI.outputText("<br><br>Sensation fades from your " +
                    liveData.player.wingDesc +
                ...
                GUI.outputText("<br><br>Sensation fades from your large fin slowly but surely, leaving it a dried out husk that breaks off to fall on the ground. Your back closes up to conceal the loss, as smooth and unbroken as the day you entered the portal.");
            liveData.player.wingType = ENUM.WingType.WING_TYPE_NONE;
        ...
        if (liveData.player.armType != ENUM.ArmType.ARM_TYPE_HARPY && changes < changeLimit && (type == 1 || liveData.player.hairType == 1) && UTIL.rand(4) == 0) {
            GUI.outputText("<br><br>You smile impishly as you lick the last bits of the nut from your teeth, but when you go to wipe your mouth, instead of the usual texture of your " +
                liveData.player.skinDesc +
        ...
        if (liveData.player.hairType != 1 && changes < changeLimit && (type == 1 || liveData.player.faceType == ENUM.FaceType.FACE_HUMAN) && UTIL.rand(4) == 0) {
            GUI.outputText("<br><br>A tingling starts in your scalp, getting worse and worse until you're itching like mad, the feathery strands of your hair tickling your fingertips while you scratch like a dog itching a flea. When you pull back your hand, you're treated to the sight of downy fluff trailing from your fingernails. A realization dawns on you - you have feathers for hair, just like a harpy!");
        ...
        if (changes == 0)
            GUI.outputText("<br><br>Aside from being a tasty treat, it doesn't seem to do anything to you this time.");
        liveData.player.refillHunger(10);
        liveData.gameFlags[FLAG.TIMES_TRANSFORMED] += changes;
    }
    ...
    static minotaurTFs() {
        ...
        let temp3 = 0;
        //Set up output
        GUI.outputText("You drink the bubbling red fluid, tasting the tangy iron after-taste.");
        //STATS
        //Strength h
        if (UTIL.rand(3) == 0 && changes < changeLimit) {
            //weaker characters gain more
            if (liveData.player.str <= 50) {
                GUI.outputText("<br><br>Painful aches ripple through your body, flooding you with pain as your muscles flex and bulge, growing much stronger and more well-defined.");
            ...
            //stronger characters gain less
                ...
                GUI.outputText("<br><br>Your muscles grow tighter, bulging outwards powerfully as you get even stronger!");
            }
            //Chance of speed drop
            if (UTIL.rand(2) == 0 && liveData.player.str > 50) {
                GUI.outputText("<br><br>You begin to feel that the size of your muscles is starting to slow you down.");
                liveData.player.dynStats(["spe", -1]);
            }
            changes++;
        }
        ...
        if (UTIL.rand(3) == 0 && changes < changeLimit) {
            //weaker characters gain more
            if (liveData.player.tou <= 50) {
                GUI.outputText("<br><br>Your hide... skin... whatever... you can feel it getting tougher as it thickens perceptibly.");
                //very weak players gain more
            ...
            //stronger characters gain less
                ...
                //faster from 50-75
                    ...
                    liveData.player.dynStats(["tou", 1]);
                GUI.outputText("<br><br>Your tough hide grows slightly thicker.");
            }
            //chance of less sensitivity
            if (UTIL.rand(2) == 0 && liveData.player.sens > 10) {
                if (liveData.player.tou > 75) {
                    GUI.outputText("<br><br>It becomes much harder to feel anything through your leathery skin.");
                    liveData.player.dynStats(["sen", -3]);
                }
                if (liveData.player.tou <= 75 && liveData.player.tou > 50) {
                    GUI.outputText("<br><br>The level of sensation from your skin diminishes noticeably.");
                    liveData.player.dynStats(["sen", -2]);
                }
                if (liveData.player.tou <= 50) {
                    GUI.outputText("<br><br>Your sense of touch diminishes due to your tougher hide.");
                    liveData.player.dynStats(["sen", -3]);
                }
            }
            changes++;
        ...
        if (!liveData.gameFlags[FLAG.HYPER_HAPPY]) {
            ...
            if (liveData.player.vaginas.length > 0) {
                if (liveData.player.vaginas[0].vaginalLooseness > ENUM.VaginalLoosenessType.VAGINA_LOOSENESS_TIGHT) {
                    ...
                    GUI.outputText("<br><br>A tightness in your groin is the only warning you get before your <b>" + liveData.player.vaginaDescript(0) + " disappears forever</b>!");
                    //Goodbye womanhood!
                    liveData.player.removeVagina(0, 1);
                    if (liveData.player.cocks.length == 0) {
                        GUI.outputText("  Strangely, your clit seems to have resisted the change, and is growing larger by the moment... shifting into the shape of a small ribbed minotaur-like penis!  <b>You now have a horse-cock!</b>");
                        liveData.player.createCock();
            ...
            //Shrink boobages till they are normal
                ...
                if (liveData.player.breastRows.length == 1) {
                    ...
                    if (liveData.player.breastRows[0].breastRating >= 1) {
                        ...
                        //Talk about shrinkage
                        if (temp == 1)
                            GUI.outputText("<br><br>You feel a weight lifted from you, and realize your " + liveData.player.breastDescript(0) + " have shrunk to " + liveData.player.breastCup(0) + "s.");
                        if (temp == 2)
                            GUI.outputText("<br><br>You feel significantly lighter.  Looking down, you realize your breasts are MUCH smaller, down to " + liveData.player.breastCup(0) + "s.");
                        changes++;
                    }
        ...
        if (changes < changeLimit && UTIL.rand(2) == 0 && liveData.player.cocks.length > 0) {
            ...
            if (selectedCock != -1) {
                ...
                if (liveData.player.cocks[selectedCock].cockThickness < 5) {
                    ...
                    GUI.outputText("  To your delight and surprise, you discover it has grown slightly thicker as well!");
                }
                //Just length...
                else {
                    //Increase by 2 + UTIL.rand(8), and store the actual amount in temp
        ...
        if (liveData.player.cocks.length > 0 && UTIL.rand(2) == 0 && changes < changeLimit) {
            ...
            if (selectedCockValue != -1) {
                ...
                GUI.outputText("<b>  You now have a");
                if (liveData.player.countCocksOfType(ENUM.CockType.HORSE) > 1)
                    GUI.outputText("nother");
                GUI.outputText(" horse-penis.</b>");
                changes++;
            }
        }
        //Males go into rut
        ...
        if (UTIL.rand(4) == 0 && changes < changeLimit && liveData.player.findStatusEffect(StatusEffects.Dysfunction) < 0) {
            if (liveData.player.cocks.length > 0)
                GUI.outputText("<br><br>Your " + liveData.player.cockDescript(0) + " tingles abruptly, then stops.  Worried, you reach down to check it, only to discover that it feels... numb.  It will be very hard to masturbate like this.");
            else if (liveData.player.hasVagina())
                GUI.outputText("<br><br>Your " + liveData.player.vaginaDescript(0) + " tingles abruptly, then stops.  Worried, you reach down to check it, only to discover that it feels... numb.  It will be very hard to masturbate like this.");
            if (liveData.player.cocks.length > 0 || liveData.player.hasVagina()) {
        ...
        if (liveData.player.earType == ENUM.EarType.EARS_COW && liveData.player.lowerBody == ENUM.LowerBodyType.LOWER_BODY_TYPE_HOOFED && liveData.player.tallness >= 90 && changes < changeLimit && UTIL.rand(3) == 0) {
            if (liveData.player.faceType != ENUM.FaceType.FACE_COW_MINOTAUR) {
                GUI.outputText("<br><br>Bones shift and twist painfully as your visage twists and morphs to resemble that of the beast whose blood you now drink.  <b>You now have a minotaur-like face.</b>");
                changes++;
                liveData.player.faceType = ENUM.FaceType.FACE_COW_MINOTAUR;
            }
        }
                ...
                if (liveData.player.hornType == ENUM.HornType.HORNS_COW_MINOTAUR) {
                    ...
                    if (liveData.player.vaginas.length > 0) {
                        if (liveData.player.horns > 4) {
                            GUI.outputText("<br><br>You feel a pressure in your head around your horns, but they don't grow any larger.  ");
                    ...
                    //Males horns get 'uge.
                        ...
                        if (temp == 0)
                            changes--;
                        if (temp == 1)
                            GUI.outputText("<br><br>An aching pressure builds in your temples as you feel your horns push another inch of length from your skull.  ");
                        if (temp == 2)
                            GUI.outputText("<br><br>A powerful headache momentarily doubles you over.  With painful slowness, you feel your horns push another two inches of length out from your brow, gradually thickening as they grow.  ");
                        if (temp == 3)
                            GUI.outputText("<br><br>Agony overwhelms you as a headache of terrifying intensity sweeps through your skull.  You squeeze your eyes shut from the pain, but it does little to help.  The torture intensifies before finally diminishing as you feel an inch or two of new horn force its way out of your forehead.  The headache remains despite this, and desperate for relief, you grab hold of your horns and tug, pulling another inch of new horn free.  At last the pain fades, leaving you with significantly enhanced head-spikes.  ");
                        if (liveData.player.horns < 3)
                            GUI.outputText("They are the size of tiny nubs.");
                        if (liveData.player.horns >= 3 && liveData.player.horns < 6)
                            GUI.outputText("They are similar to what you would see on a young bull.");
                        if (liveData.player.horns >= 6 && liveData.player.horns < 12)
                            GUI.outputText("They look like the horns on a grown bull, big enough and dangerous enough to do some damage.");
                        if (liveData.player.horns >= 12 && liveData.player.horns < 20)
                            GUI.outputText("They are large and wicked looking.");
                        if (liveData.player.horns >= 20)
                            GUI.outputText("They are huge, heavy, and tipped with dangerous points.");
        ...
        if (UTIL.rand(4) == 0)
            GUI.outputText(liveData.player.modFem(5, 10));
        if (UTIL.rand(4) == 0)
            GUI.outputText(liveData.player.modTone(85, 3));
        if (UTIL.rand(4) == 0)
            GUI.outputText(liveData.player.modThickness(70, 4));
        //Default
        if (changes == 0) {
            GUI.outputText("<br><br>Minotaur-like vitality surges through your body, invigorating and arousing you!<br>");
            if (liveData.player.balls > 0) {
                GUI.outputText("Your balls feel as if they've grown heavier with the weight of more sperm.<br>");
                liveData.player.hoursSinceCum += 200;
            }
            liveData.player.changeHP(50, true);
    ...
    static snakeTFs() {
        ...
        GUI.outputText("Pinching your nose, you quickly uncork the vial and bring it to your mouth, determined to see what effects it might have on your body. Pouring in as much as you can take, you painfully swallow before going for another shot, emptying the bottle.");
        //(if outside combat)
        if (!COMBAT.inCombat())
            GUI.outputText("  Minutes pass as you start wishing you had water with you, to get rid of the aftertaste.");
        //+ speed to 70!
        if (liveData.player.spe < 70 && UTIL.rand(2) == 0) {
            liveData.player.dynStats(["spe", 2 - liveData.player.spe / 10 / 5]);
            GUI.outputText("<br><br>Your muscles quiver, feeling ready to strike as fast as a snake!");
            if (liveData.player.spe < 40)
                GUI.outputText("  Of course, you're nowhere near as fast as that.");
            changes++;
        }
        ...
        if (changes == 0 && UTIL.rand(3) == 0) {
            if (liveData.player.tongueType != ENUM.TongueType.TONGUE_SNAKE && changes < changeLimit) {
                if (liveData.player.tongueType == ENUM.TongueType.TONGUE_HUMAN) {
                    GUI.outputText("<br><br>Your taste-buds start aching as they swell to an uncomfortably large size. " +
                    ...
                    GUI.outputText("  The soreness stops and you stick out your tongue to try and see what would have made it feel the way it did. " +
                    ...
                    GUI.outputText("  <b>The scents in the air are much more noticeable to you with your snake-like tongue.</b>");
                }
                else {
                    GUI.outputText("<br><br>Your inhuman tongue shortens, pulling tight in the very back of your throat.");
        ...
        if (changes == 0 && liveData.player.tongueType == ENUM.TongueType.TONGUE_SNAKE && liveData.player.faceType != ENUM.FaceType.FACE_SNAKE_FANGS && UTIL.rand(3) == 0 && changes < changeLimit) {
            GUI.outputText("<br><br>Without warning, you feel your canine teeth jump almost an inch in size, clashing on your gums, cutting yourself quite badly. As you attempt to find a new way to close your mouth without dislocating your jaw, you notice that they are dripping with a bitter, khaki liquid.  Watch out, and <b>try not to bite your tongue with your poisonous fangs!</b>");
            if (liveData.player.faceType != ENUM.FaceType.FACE_HUMAN &&
                ...
                GUI.outputText("  As the change progresses, your " +
                    liveData.player.face() +
        ...
        if (changes == 0 && liveData.player.faceType == ENUM.FaceType.FACE_SNAKE_FANGS && liveData.player.lowerBody != ENUM.LowerBodyType.LOWER_BODY_TYPE_NAGA && UTIL.rand(4) == 0 && changes < changeLimit) {
            GUI.outputText("<br><br>You find it increasingly harder to keep standing as your legs start feeling weak.  You swiftly collapse, unable to maintain your own weight.");
            ...
            //FUCK NO
            GUI.outputText("<br><br>Trying to get back up, you realize that the skin on the inner sides of your thighs is merging together like it was being sewn by an invisible needle.");
            GUI.outputText("  The process continues through the length of your " +
                liveData.player.legs() +
            ...
            GUI.outputText("<br><br>Wondering what happened to your sex, you pass your hand down the front of your body until you find a large, horizontal slit around your pelvic area, which contains all of your sexual organs.");
            if (liveData.player.balls > 0 && liveData.player.ballSize > 10)
                GUI.outputText("  You're happy not to have to drag those testicles around with you anymore.");
            GUI.outputText("  But then, scales start to form on the surface of your skin, slowly becoming visible, recoloring all of your body from the waist down in a snake-like pattern. The feeling is... not that bad actually, kind of like callous, except on your whole lower body. The transformation complete, you get up, standing on your newly formed snake tail. You can't help feeling proud of this majestic new body of yours.");
        ...
        GUI.outputText("<br><br>As the liquid takes effect, ", false);
        //(if multicock)
        if (player.cockTotal() > 1) GUI.outputText("one of ", false);
        GUI.outputText("your " + player.multiCockDescriptLight() + " starts to throb painfully and swell to its full size.  With a horrifying ripping sensation, your cock splits down the middle, the pain causing you to black out momentarily.", false);
        GUI.outputText("When you awaken, you quickly look down to see that where ", false);
        //(if multicock)
        if (player.cockTotal() > 1) GUI.outputText("one of ", false);
        GUI.outputText("your " + player.multiCockDescriptLight() + " was, you now have two pointed reptilian cocks, still stiff and pulsing.", false);
        }*/
        //Default change - blah
        if (changes == 0)
            GUI.outputText("<br><br>Remakarbly, the snake-oil has no effect.  Should you really be surprised at snake-oil NOT doing anything?");
    ...
    static slimeTFs() {
        ...
        if (liveData.player.lib < 80) {
            ...
            GUI.outputText("<br><br>Blushing and feeling horny, you make sure to rub it over your chest and erect nipples, letting the strange slimy fluid soak into you.");
        }
        //sensitivity moves towards 50
        if (liveData.player.sens < 50) {
            GUI.outputText("<br><br>The slippery slime soaks into your " + liveData.player.skinDesc + ", making it tingle with warmth, sensitive to every touch.");
            liveData.player.dynStats(["sen", 1]);
        }
        else if (liveData.player.sens > 50) {
            GUI.outputText("<br><br>The slippery slime numbs your " + liveData.player.skinDesc + " slightly, leaving behind only gentle warmth.");
            liveData.player.dynStats(["sen", -1]);
        }
        ...
        if (liveData.player.wingType > ENUM.WingType.WING_TYPE_NONE) {
            if (liveData.player.wingType == ENUM.WingType.WING_TYPE_SHARK_FIN)
                GUI.outputText("<br><br>You sigh, feeling a hot wet tingling down your back.  It tickles slightly as you feel your fin slowly turn to sludge, dripping to the ground as your body becomes more goo-like.");
            else
                GUI.outputText("<br><br>You sigh, feeling a hot wet tingling down your back.  It tickles slightly as you feel your wings slowly turn to sludge, dripping to the ground as your body becomes more goo-like.");
            liveData.player.wingType = ENUM.WingType.WING_TYPE_NONE;
        }
        //Goopy hair
        if (liveData.player.hairType != 3) {
            ...
            if (liveData.player.hairLength <= 0) {
                GUI.outputText("<br><br>Your head buzzes pleasantly, feeling suddenly hot and wet.  You instinctively reach up to feel the source of your wetness, and discover you've grown some kind of gooey hair.  From time to time it drips, running down your back to the crack of your " +
                    liveData.player.buttDescript() +
                    ".");
                ...
                if (liveData.player.hairColor.indexOf("rubbery") == -1 && liveData.player.hairColor.indexOf("latex-textured") == -1) {
                    GUI.outputText("<br><br>Your head buzzes pleasantly, feeling suddenly hot and wet.  You instinctively reach up to feel the source of your wetness, and discover your hair has become a slippery, gooey mess.  From time to time it drips, running down your back to the crack of your " +
                        liveData.player.buttDescript() +
                        ".");
                }
                //Latexy stuff
                else {
                    GUI.outputText("<br><br>Your oddly inorganic hair shifts, becoming partly molten as rivulets of liquid material roll down your back.  How strange.");
                }
            }
            if (liveData.player.hairColor != "green" && liveData.player.hairColor != "purple" && liveData.player.hairColor != "blue" && liveData.player.hairColor != "cerulean" && liveData.player.hairColor != "emerald") {
                GUI.outputText("  Stranger still, the hue of your semi-liquid hair changes to ");
                ...
                GUI.outputText(liveData.player.hairColor + ".");
            }
            liveData.player.changeLust(10);
        }
        //1.Goopy skin
        if (liveData.player.hairType == 3 && (liveData.player.skinDesc != "skin" || liveData.player.skinAdj != "slimy")) {
            if (liveData.player.skinType == ENUM.SkinType.SKIN_TYPE_PLAIN)
                GUI.outputText("<br><br>You sigh, feeling your " + liveData.player.armor.equipmentName + " sink into you as your skin becomes less solid, gooey even.  You realize your entire body has become semi-solid and partly liquid!");
            ...
            if (liveData.player.skinTone != "green" && liveData.player.skinTone != "purple" && liveData.player.skinTone != "blue" && liveData.player.skinTone != "cerulean" && liveData.player.skinTone != "emerald") {
                GUI.outputText("  Stranger still, your skintone changes to ");
        ...
        if (liveData.player.skinAdj == "slimy" && liveData.player.skinDesc == "skin" && liveData.player.lowerBody != ENUM.LowerBodyType.LOWER_BODY_TYPE_GOO) {
            GUI.outputText("<br><br>Your viewpoint rapidly drops as everything below your " +
                liveData.player.buttDescript() +
        ...
        if (!liveData.player.hasVagina()) {
            GUI.outputText("<br><br>A wet warmth spreads through your slimey groin as a narrow gash appears on the surface of your groin.  <b>You have grown a vagina.</b>");
            liveData.player.createVagina();
        ...
        if (liveData.player.vaginalCapacity() < 9000) {
            ...
            GUI.outputText("<br><br>Your gel-like body swells up from the intake of additional slime.  If you had to guess, you'd bet you were about two inches taller.");
        ...
        //Big slime girl
            ...
            if (liveData.player.findStatusEffect(StatusEffects.SlimeCraving) < 0) {
                GUI.outputText("<br><br>You feel a growing gnawing in your gut.  You feel... hungry, but not for food.  No, you need something wet and goopy pumped into you.  You NEED it.  You can feel it in your bones.  <b>If you don't feed that need... you'll get weaker and maybe die.</b>");
        ...
        if (UTIL.rand(2) == 0)
            GUI.outputText(liveData.player.modFem(85, 3));
        if (UTIL.rand(2) == 0)
            GUI.outputText(liveData.player.modThickness(20, 3));
        if (UTIL.rand(2) == 0)
            GUI.outputText(liveData.player.modTone(15, 5));
    }
    static trapOil() {
        GUI.clearOutput();
        let changes = 0;
        ...
        if (liveData.player.cor < 30)
            GUI.outputText("hesitantly ");
        else if (liveData.player.cor > 70)
            GUI.outputText("eagerly ");
        GUI.outputText("rub it into your arms and chest.  The substance is warm, coating and ever so slightly numbing; it quickly sinks into your skin, leaving you feeling smooth and sleek.");
        //Speed Increase:
        if (liveData.player.spe < 100 && UTIL.rand(3) == 0 && changes < changeLimit) {
            GUI.outputText("<br><br>You feel fleet and lighter on your toes; you sense you could dodge, dart or skip away from anything.");
            liveData.player.dynStats(["spe", 1]);
            changes++;
        }
        //Strength Loss:
        else if (liveData.player.str > 40 && UTIL.rand(3) == 0 && changes < changeLimit) {
            GUI.outputText("<br><br>A sense of helplessness settles upon you as your limbs lose mass, leaving you feeling weaker and punier.");
            liveData.player.dynStats(["str", -1]);
            changes++;
        }
        //Sensitivity Increase:
        if (liveData.player.sens < 70 && liveData.player.hasCock() && UTIL.rand(3) == 0 && changes < changeLimit) {
            GUI.outputText("<br><br>A light breeze brushes over you and your skin tingles.  You have become more sensitive to physical sensation.");
            liveData.player.dynStats(["sen", 5]);
            changes++;
        }
        //Libido Increase:
        if (liveData.player.lib < 70 && liveData.player.hasVagina() && UTIL.rand(3) == 0 && changes < changeLimit) {
            GUI.outputText("<br><br>You feel your blood quicken and rise, and a desire to... hunt builds within you.");
            liveData.player.dynStats(["lib", 2]);
        ...
        if (liveData.player.thickness > 40 && UTIL.rand(3) == 0 && changes < changeLimit) {
            GUI.outputText("<br><br>You feel an odd tightening sensation in your midriff, as if you were becoming narrower and lither.  You frown downwards, and then turn your arms around, examining them closely.  Is it just you or have you lost weight?");
        ...
        if (liveData.player.hipRating >= 10 && UTIL.rand(4) == 0 && changes < changeLimit) {
            GUI.outputText("<br><br>You touch your thighs speculatively.  It's not just your imagination; you've lost a bit of weight around your waist.");
            liveData.player.hipRating--;
        ...
        if (liveData.player.hipRating < 6 && UTIL.rand(4) == 0 && changes < changeLimit) {
            GUI.outputText("<br><br>You touch your thighs speculatively.  You think you may have gained a little weight around your waist.");
            liveData.player.hipRating++;
            changes++;
        }
        //Breast Loss: (towards A cup)
        if (liveData.player.biggestTitSize() > 1 && UTIL.rand(4) == 0 && changes < changeLimit) {
            GUI.outputText("<br><br>You gasp as you feel a compressing sensation in your chest and around your [fullChest].  The feeling quickly fades however, leaving you feeling like you have lost a considerable amount of weight from your upper body.");
            let temp = 0;
        ...
        if (liveData.player.biggestTitSize() < 1 || (liveData.player.breastRows[0].breastRating < 1 && UTIL.rand(4) == 0 && changes < changeLimit)) {
            GUI.outputText("<br><br>You feel a vague swelling sensation in your [fullChest], and you frown downwards.  You seem to have gained a little weight on your chest.  Not enough to stand out, but- you cup yourself carefully- certainly giving you the faintest suggestion of boobs.");
        ...
        if (liveData.player.longestCockLength() >= 3.5 && liveData.player.hasCock() && UTIL.rand(2) == 0 && changes < changeLimit) {
            GUI.outputText("<br><br>You flinch and gasp as your " + liveData.player.multiCockDescriptLight() + " suddenly become");
            if (liveData.player.cockTotal() == 1)
                GUI.outputText("s");
            GUI.outputText(" incredibly sensitive and retract into your body.  Anxiously you pull down your underclothes to examine your nether regions.  To your relief ");
            if (liveData.player.cockTotal() == 1)
                GUI.outputText("it is");
            else
                GUI.outputText("they are");
            GUI.outputText(" still present, and as you touch ");
            if (liveData.player.cockTotal() == 1)
                GUI.outputText("it");
            else
                GUI.outputText("them");
            GUI.outputText(", the sensitivity fades, however - a blush comes to your cheeks - ");
            if (liveData.player.cockTotal() == 1)
                GUI.outputText("it seems");
            else
                GUI.outputText("they seem");
            GUI.outputText(" to have become smaller.");
            let temp = 0;
        ...
        if (liveData.player.balls > 0 && liveData.player.hasCock() && (liveData.player.ballSize > 1 || liveData.player.findStatusEffect(StatusEffects.Uniball) < 0) && UTIL.rand(4) == 0 && changes < changeLimit) {
            GUI.outputText("<br><br>You feel a delicate tightening sensation around your [balls].  The sensation upon this most sensitive part of your anatomy isn't painful, but the feeling of your balls getting smaller is intense enough that you stifle anything more than a sharp intake of breath only with difficulty.");
            liveData.player.ballSize--;
            ...
            if (liveData.player.ballSize < 1 && liveData.player.findStatusEffect(StatusEffects.Uniball) < 0) {
                GUI.outputText("  You whimper as once again, your balls tighten and shrink.  Your eyes widen when you feel the gentle weight of your testicles pushing against the top of your [hips], and a few hesitant swings of your rear confirm what you can feel - you've tightened your balls up so much they no longer hang beneath your " +
                    liveData.player.multiCockDescriptLight() +
        ...
        if (liveData.player.ass.analWetness < 5 && UTIL.rand(4) == 0 && changes < changeLimit) {
            if (liveData.player.ass.analWetness < 4)
                GUI.outputText("<br><br>Your eyes widen in shock as you feel oily moisture bead out of your [asshole].  Your asshole has become wetter and more pliable.");
            //Anal Wetness Increase Final (always loose):
            else
                GUI.outputText("<br><br>You moan as clear, odorless oil dribbles out of your [asshole], this time in enough quantity to stain your [armor].  Your back passage feels incredibly sensitive, wet and accommodating.  Your ass is ready to be plowed by anything, and always will be.");
            liveData.player.ass.analWetness++;
        ...
        if (liveData.player.hasVagina() && UTIL.rand(4) == 0 && changes < changeLimit) {
            ...
            //[Low/No fertility:
                ...
                GUI.outputText("Although the numbness makes you feel serene, the trap oil has no effect upon your ");
                if (liveData.player.fertility > 0)
                    GUI.outputText("mostly ");
                GUI.outputText("sterile system.");
        ...
        if (liveData.player.gender == 1) {
            ...
            if (UTIL.rand(4) == 0 && changes < changeLimit) {
                if (liveData.player.femininity < 70 && liveData.player.femininity >= 60) {
                    GUI.outputText("<br><br>You laugh as you feel your features once again soften, before stopping abruptly.  Your laugh sounded more like a girly giggle than anything else.  Feeling slightly more sober, you touch the soft flesh of your face prospectively.  The trap oil has changed you profoundly, making your innate maleness... difficult to discern, to say the least.  You suspect you could make yourself look even more like a girl now if you wanted to.");
                ...
                //Femininity Increase:
                    ...
                    GUI.outputText("<br><br>Your face softens as your features become more feminine.");
                    liveData.player.femininity += 10;
                    changes++;
                }
            }
            ...
            if (liveData.player.tone > 20 && UTIL.rand(4) == 0 && changes < changeLimit) {
                GUI.outputText("<br><br>You sink a finger into your arm inquiringly.  You seem to have lost some of your muscle definition, leaving you looking softer.");
        ...
        //Female Effects
            ...
            if (liveData.player.femininity > 30 && UTIL.rand(4) == 0 && changes < changeLimit) {
                ...
                if (liveData.player.femininity < 30) {
                    ...
                    GUI.outputText("<br><br>You laugh as you feel your features once again soften, before stopping abruptly.  Your laugh sounded more like a boyish crow than anything else.  Feeling slightly more sober, you touch the defined lines of your face prospectively.  The trap oil has changed you profoundly, making your innate femaleness... difficult to discern, to say the least.  You suspect you could make yourself look even more like a boy now if you wanted to.");
                    if (liveData.player.findPerk(PerkLib.Androgyny) < 0) {
                        liveData.player.createPerk(PerkLib.Androgyny, 0, 0, 0, 0);
                        GUI.outputText("<br><br>(<b>Perk Gained: Androgyny</b>)");
                    }
                    ...
                    GUI.outputText("<br><br>Your face becomes more set and defined as your features turn more masculine.");
                }
                changes++;
            }
            //Muscle tone gain:
            if (liveData.player.tone < 80 && UTIL.rand(4) == 0 && changes < changeLimit) {
                GUI.outputText("<br><br>You flex your arm in interest.  Although you have become thinner, your muscles seem to have become more defined.");
                liveData.player.tone += 10;
                changes++;
            }
        }
        ...
        if (liveData.gameFlags[FLAG.HAS_BLACK_NIPPLES] == 0 && UTIL.rand(6) == 0 && changes < changeLimit) {
            GUI.outputText("<br><br>A tickling sensation plucks at your nipples and you cringe, trying not to giggle.  Looking down you are in time to see the last spot of flesh tone disappear from your [nipples].  They have turned an onyx black!");
        ...
        if (changes == 0) {
            GUI.outputText("<br><br>Well... that didn't amount to much.");
        }
        liveData.gameFlags[FLAG.TIMES_TRANSFORMED] += changes;
    }
    //----------
