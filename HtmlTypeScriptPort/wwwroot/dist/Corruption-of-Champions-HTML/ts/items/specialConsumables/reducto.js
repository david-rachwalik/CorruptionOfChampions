...
ConsumableEffects.reductoMenu = function () {
    ...
    if (player.balls > 0 && player.ballSize > 1)
        addButton(0, "Balls", Reducto.reductoBalls);
    if (player.breastRows.length > 0 && player.biggestTitSize() > 0)
        addButton(1, "Breasts", Reducto.reductoBreasts);
    if (player.buttRating > 1)
        addButton(2, "Butt", Reducto.reductoButt);
    if (player.vaginas.length > 0 && player.clitLength > 0.25)
        addButton(3, "Clit", Reducto.reductoClit);
    if (player.cockTotal() > 0 && player.biggestCockArea() > 6)
        addButton(4, "Cock", Reducto.reductoCock);
    if (player.hipRating > 2)
        addButton(5, "Hips", Reducto.reductoHips);
    if (player.nippleLength > 0.25)
        addButton(6, "Nipples", Reducto.reductoNipples);
    if (player.horns > 2)
        addButton(7, "Horns", Reducto.shrinkHorns);
