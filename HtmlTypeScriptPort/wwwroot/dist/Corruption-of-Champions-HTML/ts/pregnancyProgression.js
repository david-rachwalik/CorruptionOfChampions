    ...
    updatePregnancy() {
        ...
        if (liveData.player.pregnancyType == ENUM.PregnancyType.PREGNANCY_AMILY) {
            switch (liveData.player.pregnancyEventNum) {
                case 1:
                    GUI.outputText("<b>You wake up feeling bloated, and your belly is actually looking a little puffy. At the same time, though, you have the oddest cravings... you could really go for some mixed nuts. And maybe a little cheese, too.</b><br><br>");
                ...
                case 3:
                    ...
                    if (liveData.gameFlags[FLAG.AMILY_FOLLOWER] == 1)
                        GUI.outputText('  Amily smiles at you reassuringly. "<i>We do have litters, dear, this is normal.</i>"');
            ...
            if (liveData.player.pregnancyIncubation == 0 && liveData.player.pregnancyType == ENUM.PregnancyType.PREGNANCY_AMILY) {
                ...
                if (liveData.gameFlags[FLAG.AMILY_FOLLOWER] == 2 || liveData.gameFlags[FLAG.AMILY_CORRUPTION_PATH] > 0)
                    liveData.player.knockUpForce(ENUM.PregnancyType.PREGNANCY_MOUSE, liveData.player.pregnancyIncubationFlag);
                if (liveData.gameFlags[FLAG.AMILY_VISITING_URTA] == 1 || liveData.gameFlags[FLAG.AMILY_VISITING_URTA] == 2)
                    liveData.player.knockUpForce(ENUM.PregnancyType.PREGNANCY_MOUSE, liveData.player.pregnancyIncubationFlag);
            ...
            if (liveData.player.pregnancyIncubation == 0 && liveData.player.pregnancyType == ENUM.PregnancyType.PREGNANCY_AMILY) {
                ...
                if (liveData.player.vaginas[0].vaginalWetness == ENUM.VaginalWetnessType.VAGINA_WETNESS_DRY)
        ...
        if (liveData.player.pregnancyType == FLAG.PREGNANCY_IMP) {
            ...
            if (liveData.player.pregnancyIncubation <= 216 * 60 && liveData.player.pregnancyIncubation >= 180 * 60) {
                ...
                if (liveData.player.cor < 40)
                    GUI.outputText("You are distressed by your unwanted pregnancy, and your inability to force this thing out of you.</b><br><br>");
                if (liveData.player.cor >= 40 && liveData.player.cor < 75)
                    GUI.outputText("Considering the size of the creatures you've fucked, you hope it doesn't hurt when it comes out.</b><br><br>");
                if (liveData.player.cor >= 75)
                    GUI.outputText("You think dreamily about the monstrous cocks that have recently been fucking you, and hope that your offspring inherit such a pleasure tool.</b><br><br>");
                if (this.statControl == 0) {
                    this.statControl = 1;
            ...
            if (liveData.player.pregnancyIncubation <= 72 * 60 && liveData.player.pregnancyIncubation >= 48 * 60) {
                ...
                if (liveData.player.cor < 40)
                    GUI.outputText("making it difficult to function.</b>");
                if (liveData.player.cor >= 40 && liveData.player.cor < 75)
                    GUI.outputText("and you wonder how much longer you have to wait.</b>");
                if (liveData.player.cor >= 75)
                    GUI.outputText("and you're eager to give birth, so you can get impregnated again by corrupted or monstrous cum filling out your eager womb.</b>");
                GUI.outputText("<br><br>");
                if (this.statControl == 1) {
                    this.statControl = 2;
            ...
            if (liveData.player.pregnancyIncubation <= 48 * 60) {
                ...
                if (liveData.player.cor < 40)
                    GUI.outputText("Afterwards you feel somewhat disgusted with yourself.</b>");
                if (liveData.player.cor >= 40 && liveData.player.cor < 75)
                    GUI.outputText("You estimate you'll give birth in the next few days.</b>");
                if (liveData.player.cor >= 75)
                    GUI.outputText("You find yourself daydreaming about birthing demons repeatedly, each time being re-impregnated by your hordes of lusty adolescent children.</b>");
                GUI.outputText("<br><br>");
                displayedUpdate = true;
            }
        }
        ...
        if (liveData.player.buttPregnancyType == FLAG.PREGNANCY_BEE_EGGS) {
            ...
            if (liveData.player.buttPregnancyIncubation <= 20 * 60) {
                ...
                if (liveData.player.cockTotal() > 0)
                    GUI.outputText("The aroma seems to permeate your very being, slowly congregating in your ");
                if (liveData.player.cockTotal() == 1) {
                    ...
                    if (liveData.player.dogCocks() == 1)
                        GUI.outputText(", each inhalation making it thicker, harder, and firmer.  You suck in huge lungfuls of air, desperate for more, until your " + liveData.player.cockDescript(0) + " is twitching and dripping, its knot swollen to the max.  ");
        ...
        if (liveData.player.buttPregnancyIncubation == 0 && liveData.player.buttPregnancyType == FLAG.PREGNANCY_BEE_EGGS) {
            ...
            if (liveData.player.cockTotal() > 0)
                GUI.outputText("The muzzy feeling that fills your head seems to seep downwards, making your equipment hard and tight.  ");
            if (liveData.player.vaginas.length > 0)
                GUI.outputText("Your " + liveData.player.vaginaDescript(0) + " becomes engorged and sensitive.  ");
            GUI.outputText("Your hand darts down to the amber, scooping up a handful of the sticky stuff.  You wonder what your hand is doing as it brings it up to your mouth, which instinctively opens.  You shudder in revulsion as you swallow the sweet-tasting stuff, your mind briefly wondering why it would do that.  The stuff seems to radiate warmth, quickly pushing those nagging thoughts away as you scoop up more.<br><br>");
            GUI.outputText("A sudden slip from below surprises you; a white sphere escapes from your anus along with another squirt of honey.  Your drugged brain tries to understand what's happening, but it gives up, your hands idly slathering honey over your loins.  The next orb pops out moments later, forcing a startled moan from your mouth.  That felt GOOD.  You begin masturbating to the thought of laying more eggs... yes, that's what those are.  You nearly cum as egg number three squeezes out.  ");
            if (liveData.player.averageLactation() >= 1 && liveData.player.biggestTitSize() > 2)
                GUI.outputText("Seeking even greater sensation, your hands gather the honey and massage it into your " +
            ...
            GUI.outputText("Some time later, your mind begins to return, brought to wakefulness by an incredibly loud buzzing...  You sit up and see a pile of dozens of eggs resting in a puddle of sticky honey.  Most are empty, but a few have hundreds of honey-bees emptying from them, joining the massive swarms above you.  ");
            if (liveData.player.cor < 35)
                GUI.outputText("You are disgusted, but glad you were not stung during the ordeal.  You stagger away and find a brook to wash out your mouth with.");
            if (liveData.player.cor >= 35 && liveData.player.cor < 65)
                GUI.outputText("You are amazed you could lay so many eggs, and while the act was strange there was something definitely arousing about it.");
            if (liveData.player.cor >= 65 && liveData.player.cor < 90)
                GUI.outputText("You stretch languidly, noting that most of the drugged honey is gone.  Maybe you can find the Bee again and remember to bottle it next time.");
            if (liveData.player.cor >= 90)
                GUI.outputText("You lick your lips, savoring the honeyed residue on them as you admire your thousands of children.  If only every night could be like this...<br>");
            this.buttStatControl = 0;
            liveData.player.buttKnockUpForce(); //Clear Butt Pregnancy
            liveData.player.orgasm();
            liveData.player.dynStats(["int", 1], ["lib", 4], ["sen", 3]);
            if (liveData.player.buttChange(20, true))
                GUI.outputText("<br>");
...
//# sourceMappingURL=pregnancyProgression.js.map