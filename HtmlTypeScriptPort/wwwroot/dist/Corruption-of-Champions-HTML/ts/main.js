...
class MAIN {
    ...
    static errorEdgeNeverWorks() {
        GUI.clearOutput();
        GUI.outputText('<font size="64"><b>:(</b></font><br>Unfortunately, it appears that Microsoft Edge won\'t be able to run this game properly due to issue with local storage. I will look for possible workarounds. Until then, please switch to Firefox or Chrome.');
        GUI.menu();
        GUI.hideMenus();
    }
    //------------
    ...
    static settingsScreenMain() {
        ...
        if (liveData.use12Hours)
            GUI.outputText("Time Format: <b>12 hours</b><br>&nbsp; 12-hour format will be used. Time will use AM/PM.<br><br>");
        else
            GUI.outputText("Time Format: <b>24 hours</b><br>&nbsp; 24-hour format will be used.<br><br>");
        //Set buttons
        GUI.menu();
    ...
    static toggleTimeFormat() {
        if (liveData.use12Hours)
            liveData.use12Hours = false;
        else
            liveData.use12Hours = true;
        this.settingsScreenMain();
    }
    //Font settings
    static fontSettings() {
    ...
    static changeMainFontSize(biggerSmaller) {
        if (liveData.mainFontSizeIndex == undefined)
            liveData.mainFontSizeIndex = 4; //Fix font size.
        ...
        if (liveData.mainFontSizeIndex < 0)
            liveData.mainFontSizeIndex = 0;
        if (liveData.mainFontSizeIndex > 8)
            liveData.mainFontSizeIndex = 8;
        this.applyFontSettings();
        this.fontSettings();
    }
    static applyFontSettings() {
