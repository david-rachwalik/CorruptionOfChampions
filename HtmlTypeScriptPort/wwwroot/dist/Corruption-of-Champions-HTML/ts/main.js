...
function mainMenu() {
    ...
    playerMenu = mainMenu;
    if (gameStarted)
        addButton(0, "Resume", Camp.doCamp);
    else
        addButtonDisabled(0, "Resume", "Please load a game or start a new game first.");
...
function fontSettings() {
    ...
    if (mainFontSizeIndex < 8)
        addButton(5, "Main Size+", changeMainFontSize, "bigger");
    if (mainFontSizeIndex > 0)
        addButton(10, "Main Size-", changeMainFontSize, "smaller");
...
function changeMainFontSize(biggerSmaller) {
    if (mainFontSizeIndex == undefined)
        mainFontSizeIndex = 4; //Fix font size.
