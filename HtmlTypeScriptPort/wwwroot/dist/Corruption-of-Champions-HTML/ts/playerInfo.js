...
function attributeMenu() {
    ...
    if (liveData.player.statPoints > 0) {
        if (liveData.player.str < 100)
            GUI.addButton(0, "STR +", addAttribute, "str");
        if (liveData.player.tou < 100)
            GUI.addButton(1, "TOU +", addAttribute, "tou");
        if (liveData.player.spe < 100)
            GUI.addButton(2, "SPE +", addAttribute, "spe");
        if (liveData.player.inte < 100)
            GUI.addButton(3, "INTE +", addAttribute, "int");
    }
    //Subtract attributes
    if (tempStr > 0)
        GUI.addButton(5, "STR -", subtractAttribute, "str");
    if (tempTou > 0)
        GUI.addButton(6, "TOU +", subtractAttribute, "tou");
    if (tempSpe > 0)
        GUI.addButton(7, "SPE +", subtractAttribute, "spe");
    if (tempInt > 0)
        GUI.addButton(8, "INTE +", subtractAttribute, "int");
    //Reset & Done
...
function finishAttributes() {
    ...
    if (tempStr > 0) {
        if (tempStr >= 3)
            outputText("Your muscles feel significantly stronger from your time adventuring.<br>");
        else
            outputText("Your muscles feel slightly stronger from your time adventuring.<br>");
    }
    if (tempTou > 0) {
        if (tempTou >= 3)
            outputText("You feel tougher from all the fights you have endured.<br>");
        else
            outputText("You feel slightly tougher from all the fights you have endured.<br>");
    }
    if (tempSpe > 0) {
        if (tempSpe >= 3)
            outputText("Your time in combat has driven you to move faster.<br>");
        else
            outputText("Your time in combat has driven you to move slightly faster.<br>");
    }
    if (tempInt > 0) {
        if (tempInt >= 3)
            outputText("Your time spent fighting the creatures of this realm has sharpened your wit.<br>");
        else
...
function appearanceScreen() {
    ...
    switch (liveData.player.faceType) {
        ...
        case FACE_FERRET_MASK:
            ...
            if (liveData.player.skinType == SKIN_TYPE_FUR)
                outputText(" Under your " + liveData.player.skinFurScales() + " you have a human-shaped head with " + liveData.player.skin(true, false) + ".");
            if (liveData.player.skinType == SKIN_TYPE_SCALES)
                outputText(" Your face is fairly human in shape, but is covered in " + liveData.player.skin() + ".");
            if (liveData.player.faceType == FACE_SHARK_TEETH)
            ...
            else if (liveData.player.faceType == FACE_BUNNY)
        ...
        case FACE_FERRET:
            if (liveData.player.skinType == SKIN_TYPE_PLAIN)
                ...
                outputText(" Your face is coated in " + liveData.player.furColor + " fur with [skin] underneath, an adorable cross between human and ferret features. It is complete with a wet nose and whiskers.");
            break;
        case FACE_RACCOON_MASK:
            if (liveData.player.skinType != SKIN_TYPE_FUR && liveData.player.skinType != SKIN_TYPE_SCALES) {
                ...
                if ((liveData.player.furColor == "black" || liveData.player.furColor == "midnight") && (liveData.player.skinType == SKIN_TYPE_FUR || liveData.player.skinType == SKIN_TYPE_SCALES))
                    ...
                    outputText(" Your " + liveData.player.skinFurScales() + " are decorated with a sly-looking raccoon mask, and under them");
        ...
        case FACE_RACCOON:
            ...
            else if (liveData.player.skinType == SKIN_TYPE_SCALES)
        ...
        case FACE_FOX:
            ...
            if (liveData.player.skinType == SKIN_TYPE_PLAIN)
                outputText(" Oddly enough, there's no fur on your animalistic muzzle, just " + liveData.player.skinFurScales() + ".");
            else if (liveData.player.skinType == SKIN_TYPE_FUR)
                outputText(" A coat of " + liveData.player.skinFurScales() + " decorates your muzzle.");
            else if (liveData.player.skinType == SKIN_TYPE_SCALES)
                outputText(" Strangely, " + liveData.player.skinFurScales() + " adorn every inch of your animalistic visage.");
            break;
        case FACE_BUCKTEETH:
            outputText(" Your face is generally human in shape and structure, with " + liveData.player.skin());
            if (liveData.player.skinType == SKIN_TYPE_FUR || liveData.player.skinType == SKIN_TYPE_SCALES)
                outputText(" under your " + liveData.player.skinFurScales());
            outputText(" and mousey buckteeth.");
            break;
        case FACE_MOUSE:
            outputText(" You have a snubby, tapered mouse's face, with whiskers, a little pink nose, and ");
            if (liveData.player.skinType != SKIN_TYPE_FUR && liveData.player.skinType != SKIN_TYPE_SCALES)
                outputText(liveData.player.skin());
            else
                outputText(liveData.player.skin() + " under your " + liveData.player.skinFurScales());
            outputText(". Two large incisors complete it.");
            break;
        case FACE_SNAKE_FANGS:
            if (liveData.player.skinType == SKIN_TYPE_PLAIN || liveData.player.skinType == SKIN_TYPE_GOO)
                outputText(" You have a fairly normal face, with " + liveData.player.skin() + ". The only oddity is your pair of dripping fangs which often hang over your lower lip.");
            if (liveData.player.skinType == SKIN_TYPE_FUR)
                outputText(" Under your " + liveData.player.skinFurScales() + " you have a human-shaped head with " + liveData.player.skin(true, false) + ". In addition, a pair of fangs hang over your lower lip, dripping with venom.");
            if (liveData.player.skinType == SKIN_TYPE_SCALES)
                outputText(" Your face is fairly human in shape, but is covered in " + liveData.player.skinFurScales() + ". In addition, a pair of fangs hang over your lower lip, dripping with venom.");
            break;
        case FACE_HORSE:
            if (liveData.player.skinType == SKIN_TYPE_PLAIN || liveData.player.skinType == SKIN_TYPE_GOO)
                outputText(" Your face is equine in shape and structure. The odd visage is hairless and covered with " + liveData.player.skinFurScales() + ".");
            if (liveData.player.skinType == SKIN_TYPE_FUR)
                outputText(" Your face is almost entirely equine in appearance, even having " + liveData.player.skinFurScales() + ". Underneath the fur, you believe you have " + liveData.player.skin(true, false) + ".");
            if (liveData.player.skinType == SKIN_TYPE_SCALES)
                outputText(" You have the face and head structure of a horse, overlaid with glittering " + liveData.player.skinFurScales() + ".");
            break;
        case FACE_DOG:
            if (liveData.(player.skinType == SKIN_TYPE_PLAIN || liveData.player.skinType == SKIN_TYPE_GOO))
                outputText(" You have a dog-like face, complete with a wet nose. The odd visage is hairless and covered with " + liveData.player.skinFurScales() + ".");
            if (liveData.player.skinType == SKIN_TYPE_FUR)
                outputText(" You have a dog's face, complete with wet nose and panting tongue. You've got " + liveData.player.skinFurScales() + ", hiding your " + liveData.player.skin(true, false) + " underneath your furry visage.");
            if (liveData.player.skinType == SKIN_TYPE_SCALES)
                outputText(" You have the facial structure of a dog, wet nose and all, but overlaid with glittering " + liveData.player.skinFurScales() + ".");
            break;
        case FACE_CAT:
            if (liveData.player.skinType == SKIN_TYPE_PLAIN || liveData.player.skinType == SKIN_TYPE_GOO)
            ...
            if (liveData.player.skinType == SKIN_TYPE_FUR)
                outputText(" You have a cat-like face, complete with moist nose and whiskers. Your " + liveData.player.skinDesc + " is " + liveData.player.furColor + ", hiding your " + liveData.player.skin(true, false) + " underneath.");
            if (liveData.player.skinType == SKIN_TYPE_SCALES)
                outputText(" Your facial structure blends humanoid features with those of a cat. A moist nose and whiskers are included, but overlaid with glittering " + liveData.player.skinFurScales() + ".");
            if (liveData.player.eyeType != EYES_BLACK_EYES_SAND_TRAP)
        ...
        case FACE_COW_MINOTAUR:
            ...
            if (liveData.player.skinType == SKIN_TYPE_FUR)
                outputText(" You have a face resembling that of a minotaur, with cow-like features, particularly a squared off wet nose. Your " +
        ...
        case FACE_LIZARD:
            ...
            if (liveData.player.skinType == SKIN_TYPE_SCALES)
                outputText(" Your face is that of a lizard, complete with a toothy maw and pointed snout. Reflective " + liveData.player.skinFurScales() + " complete the look, making you look quite fearsome.");
            break;
        case FACE_DRAGON:
            outputText(" Your face is a narrow, reptilian muzzle. It looks like a predatory lizard's, at first glance, but with an unusual array of spikes along the under-jaw. It gives you a regal but fierce visage. Opening your mouth reveals several rows of dagger-like sharp teeth. The fearsome visage is decorated by " +
                liveData.player.skinFurScales() + ".");
            break;
        case FACE_KANGAROO:
            ...
            if (liveData.player.skinType == SKIN_TYPE_PLAIN)
                ...
                outputText("covered with " + liveData.player.skinFurScales(), false);
        ...
        case FACE_BOAR:
            ...
            if (liveData.player.skinType == SKIN_TYPE_FUR)
                outputText("with " + liveData.player.skinTone + " skin underneath your " + liveData.player.furColor + " fur");
            outputText(", complete with tusks and a snout that is always wiggling.");
            break;
        case FACE_RHINO:
            outputText(" Your face is like that of a rhino");
            if (liveData.player.skinType == SKIN_TYPE_PLAIN)
                outputText(", with " + liveData.player.skin() + ", complete with a long muzzle and a horn on your nose.");
            else
                outputText(" with a long muzzle and a horn on your nose. Oddly, your face is also covered in " + liveData.player.skinFurScales() + ".");
            break;
        case FACE_ECHIDNA:
            outputText(" Your odd visage consists of a long, thin echidna snout.");
            if (liveData.player.skinType == SKIN_TYPE_PLAIN)
                outputText(" The " + liveData.player.skin() + " that is revealed by your lack of fur looks quite unusual.");
            else if (liveData.player.skinType == SKIN_TYPE_FUR)
                outputText(" It's covered in " + liveData.player.skinFurScales() + ".");
            else if (liveData.player.skinType == SKIN_TYPE_SCALES)
                outputText(" It's covered in " + liveData.player.skinFurScales() + ", making your face even more unusual.");
            break;
        case FACE_DEER:
            ...
            if (liveData.player.skinType == SKIN_TYPE_PLAIN)
                ...
                outputText(" It's covered in " + liveData.player.skinFurScales() + " that covers your " + liveData.player.skinTone + " skin underneath.");
            else if (liveData.player.skinType == SKIN_TYPE_SCALES)
                outputText(" It's covered in " + liveData.player.skinFurScales() + ", making your face looks more unusual.");
            break;
        default:
    ...
    //Eyes
    if (liveData.player.eyeType == EYES_FOUR_SPIDER_EYES)
        outputText(" In addition to your primary two eyes, you have a second, smaller pair on your forehead.");
    else if (liveData.player.eyeType == EYES_BLACK_EYES_SAND_TRAP)
    ...
    //not bald
        ...
        if (liveData.player.earType == EARS_HUMAN)
            outputText(" Your " + liveData.player.hairDescript() + " looks good on you, accentuating your features well.");
        else if (liveData.player.earType == EARS_FERRET)
            outputText(" A pair of small, rounded ferret ears burst through the top of your " + liveData.player.hairDescript() + ".");
        else if (liveData.player.earType == EARS_HORSE)
            outputText(" The " + liveData.player.hairDescript() + " on your head parts around a pair of very horse-like ears that grow up from your head.");
        else if (liveData.player.earType == EARS_DOG)
            outputText(" The " + liveData.player.hairDescript() + " on your head is overlapped by a pair of pointed dog ears.");
        else if (liveData.player.earType == EARS_COW)
            outputText(" The " + liveData.player.hairDescript() + " on your head is parted by a pair of rounded cow ears that stick out sideways.");
        else if (liveData.player.earType == EARS_ELFIN)
            outputText(" The " + liveData.player.hairDescript() + " on your head is parted by a pair of cute pointed ears, bigger than your old human ones.");
        else if (liveData.player.earType == EARS_CAT)
            outputText(" The " + liveData.player.hairDescript() + " on your head is parted by a pair of cute, fuzzy cat ears, sprouting from atop your head and pivoting towards any sudden noises.");
        else if (liveData.player.earType == EARS_LIZARD)
            outputText(" The " + liveData.player.hairDescript() + " atop your head makes it nigh-impossible to notice the two small rounded openings that are your ears.");
        else if (liveData.player.earType == EARS_BUNNY)
            outputText(" A pair of floppy rabbit ears stick up out of your " + liveData.player.hairDescript() + ", bouncing around as you walk.");
        else if (liveData.player.earType == EARS_KANGAROO)
            outputText(" The " + liveData.player.hairDescript() + " atop your head is parted by a pair of long, furred kangaroo ears that stick out at an angle.");
        else if (liveData.player.earType == EARS_FOX)
            outputText(" The " + liveData.player.hairDescript() + " atop your head is parted by a pair of large, adept fox ears that always seem to be listening.");
        else if (liveData.player.earType == EARS_DRAGON)
            outputText(" The " + liveData.player.hairDescript() + " atop your head is parted by a pair of rounded protrusions with small holes on the sides of your head serve as your ears. Bony fins sprout behind them.");
        else if (liveData.player.earType == EARS_RACCOON)
            outputText(" The " + liveData.player.hairDescript() + " on your head parts around a pair of egg-shaped, furry raccoon ears.");
        else if (liveData.player.earType == EARS_MOUSE)
            outputText(" The " + liveData.player.hairDescript() + " atop your head is funneled between and around a pair of large, dish-shaped mouse ears that stick up prominently.");
        //<mod> Mod-added ears
        else if (liveData.player.earType == EARS_PIG)
            outputText(" The " + liveData.player.hairDescript() + " on your head is parted by a pair of pointy, floppy pig ears. They often flick about when you’re not thinking about it.");
        else if (liveData.player.earType == EARS_RHINO)
            outputText(" The " + liveData.player.hairDescript() + " on your head is parted by a pair of tubular rhino ears.");
        else if (liveData.player.earType == EARS_ECHIDNA)
            outputText(" Your " + liveData.player.hairDescript() + " makes it near-impossible to see the small, rounded openings that are your ears.");
        else if (liveData.player.earType == EARS_DEER)
            outputText(" The " + liveData.player.hairDescript() + " on your head parts around a pair of deer-like ears that grow up from your head.");
        //</mod>
        if (liveData.player.antennae == ANTENNAE_BEE) {
            if (liveData.player.earType == EARS_BUNNY)
    ...
    //Tongue
    if (liveData.player.tongueType == TONGUE_SNAKE)
        outputText(" A snake-like tongue occasionally flits between your lips, tasting the air.");
    else if (liveData.player.tongueType == TONGUE_DEMONIC)
    ...
    else if (liveData.player.tongueType == TONGUE_ECHIDNA)
    ...
    switch (liveData.player.hornType) {
        case HORNS_DEMON:
            if (liveData.player.horns == 2)
                outputText(" A small pair of pointed horns has broken through the " + liveData.player.skinDesc + " on your forehead, proclaiming some demonic taint to any who see them.");
            if (liveData.player.horns == 4)
                outputText(" A quartet of prominent horns has broken through your " + liveData.player.skinDesc + ". The back pair are longer, and curve back along your head. The front pair protrude forward demonically.");
            if (liveData.player.horns == 6)
                outputText(" Six horns have sprouted through your " + liveData.player.skinDesc + ", the back two pairs curve backwards over your head and down towards your neck, while the front two horns stand almost eight inches long upwards and a little forward.");
        ...
        case HORNS_RHINO:
            if (liveData.player.horns >= 2) {
                if (liveData.player.faceType == FACE_RHINO)
            ...
            if (liveData.player.femininity > 50) {
                ...
                if (liveData.player.hipRating >= 20) {
                    ...
                    if (liveData.player.balls > 0)
                        ...
                        outputText(liveData.player.multiCockDescript() + " plenty of room to swing");
                    else if (liveData.player.hasVagina())
                        outputText(liveData.player.vaginaDescript() + " a nice, wide berth");
                    else
                        outputText("vacant groin plenty of room");
                    outputText(", and");
                }
            }
        }
    ...
    switch (liveData.player.tailType) {
        ...
        case TAIL_TYPE_KANGAROO:
            ...
            if (liveData.player.skinType == SKIN_TYPE_GOO)
                outputText("gooey, " + liveData.player.skinTone);
            else
                outputText("furry, " + liveData.player.furColor, false);
        ...
        case TAIL_TYPE_FOX:
            if (liveData.player.tailVenom <= 1)
                outputText(" A swishing " + liveData.player.furColor + " fox's brush extends from your " + liveData.player.assDescript() + ", curling around your body - the soft fur feels lovely.");
            else
                outputText(" " + Num2Text(liveData.player.tailVenom) + " swishing " + liveData.player.furColor + " fox's tails extend from your " + liveData.player.assDescript() + ", curling around your body - the soft fur feels lovely.");
            break;
    ...
    //LOWERBODY SPECIAL
    if (liveData.player.lowerBody == LOWER_BODY_TYPE_HUMAN)
        outputText(" " + Num2Text(liveData.player.legCount) + " normal human legs grow down from your waist, ending in normal human feet.");
    ...
    else if (liveData.player.lowerBody == LOWER_BODY_TYPE_CAT)
        outputText(" " + Num2Text(liveData.player.legCount) + " digitigrade legs grow downwards from your waist, ending in soft, padded cat-paws.");
    ...
    if (liveData.player.findPerk(PerkLib.Incorporeality) >= 0)
        outputText(" Of course, your " + liveData.player.legs() + " are partially transparent due to their ghostly nature."); // isn't goo transparent anyway?
    outputText("<br>");
    ...
    if (liveData.player.buttPregnancyType == PREGNANCY_FROG_GIRL || liveData.player.buttPregnancyType == PREGNANCY_SATYR || liveData.player.isPregnant()) {
        ...
        //Satur preggos - only shows if bigger than regular pregnancy or not pregnancy
            ...
            if (liveData.player.buttPregnancyIncubation < 125 && liveData.player.buttPregnancyIncubation >= 75) {
                ...
                if (liveData.player.pregnancyType == PREGNANCY_IZMA ||
                ...
                else if (liveData.player.pregnancyType != PREGNANCY_MARBLE)
                    outputText("<br><b>Your belly protrudes unnaturally far forward, bulging with the unclean spawn of some monster or beast.</b>");
                else
                    outputText("<br><b>Your belly protrudes unnaturally far forward, bulging outwards with Marble's precious child.</b>");
            }
        }
        //URTA PREG
            ...
            if (liveData.player.buttPregnancyIncubation >= 8)
                ...
                outputText("<b>You're stuffed so full with eggs that your belly looks obscenely distended, huge and weighted with the gargantuan eggs crowding your gut. They make your gait a waddle and your gravid tummy wobble obscenely.</b>");
        }
            ...
            if (liveData.player.pregnancyIncubation <= 100)
                ...
                outputText("It still feels like there’s a solid ball inside your womb.</b>");
        }
        else {
    ...
    if (liveData.player.breastRows.length == 0) {
        ...
        //Centimeter display
            ...
            outputText(Math.floor(liveData.player.breastRows[0].nippleLength * 10) / 10 + "-inch "); //Inches display
        ...
        if (liveData.player.breastRows[0].milkFullness > 75)
            outputText(" Your " + liveData.player.breastDescript(temp) + " are painful and sensitive from being so stuffed with milk. You should release the pressure soon.");
        if (liveData.player.breastRows[0].breastRating >= 1)
            outputText(" You could easily fill a " + liveData.player.breastCup(temp) + " bra.");
        //Done with tits.  Move on.
        outputText("<br>");
    }
    //many rows
    ...
    if (liveData.player.cocks.length == 1) {
        ...
        if (useMetrics)
            outputText("<br>Your " + liveData.player.cockDescript(temp) + " is " + Math.round(liveData.player.cocks[temp].cockLength * 10 * 2.54) / 10 + " cm long and ");
        else
            outputText("<br>Your " + liveData.player.cockDescript(temp) + " is " + Math.round(liveData.player.cocks[temp].cockLength * 10) / 10 + " inches long and ");
        if (Math.round(10 * liveData.player.cocks[temp].cockThickness) / 10 < 10) {
            if (useMetrics) {
                if (Math.round(liveData.player.cocks[temp].cockThickness * 10 * 2.54) / 10 == 1)
                    outputText(Math.round(liveData.player.cocks[temp].cockThickness * 10 * 2.54) / 10 + " centimetre thick.");
                else
                    outputText(Math.round(liveData.player.cocks[temp].cockThickness * 10 * 2.54) / 10 + " centimetres thick.");
            }
            else {
                if (Math.round(liveData.player.cocks[temp].cockThickness * 10) / 10 == 1)
                    outputText(Math.round(liveData.player.cocks[temp].cockThickness * 10) / 10 + " inch thick.");
                else
                    outputText(Math.round(liveData.player.cocks[temp].cockThickness * 10) / 10 + " inches thick.");
            }
        }
        else
        ...
        if (liveData.player.cocks[temp].cockType == CockTypesEnum.DOG || liveData.player.cocks[temp].cockType == CockTypesEnum.FOX || liveData.player.cocks[temp].cockType == CockTypesEnum.FOX) {
            if (liveData.player.cocks[temp].knotMultiplier >= 1.8)
                ...
                outputText(" A large bulge of flesh nestles just above the bottom of your " + liveData.player.cockDescript(temp) + ", to ensure it stays where it belongs during mating.");
            else if (liveData.player.cocks[temp].knotMultiplier > 1)
                outputText(" A small knot of thicker flesh is near the base of your " + liveData.player.cockDescript(temp) + ", ready to expand to help you lodge it inside a female.");
            //List thickness
        ...
        if (liveData.player.findStatusEffect(StatusEffects.Infested) >= 0)
            outputText(" Every now and again a slimy worm coated in spunk slips partway out of your " + liveData.player.cockDescript(0) + ", tasting the air like a snake's tongue.");
    ...
    if (liveData.player.cocks.length > 1) {
        ...
        if (liveData.player.isTaur())
            ...
            outputText("<br>Where a penis would normally be located, you have instead grown " + liveData.player.multiCockDescript() + "!<br>");
        while (temp < liveData.player.cocks.length) {
            ...
            if (rando == 0) {
                ...
                if (Math.floor(liveData.player.cocks[temp].cockThickness) >= 2)
                    ...
                    if (liveData.player.cocks[temp].cockThickness == 1)
                        ...
                        outputText(Math.round(liveData.player.cocks[temp].cockThickness * 10) / 10 + " inches wide.");
                }
            }
            if (rando == 1) {
            ...
            if (rando == 3) {
                ...
                if (Math.floor(liveData.player.cocks[temp].cockThickness) >= 2)
                    ...
                    if (Math.round(liveData.player.cocks[temp].cockThickness * 10) / 10 == 1)
                        ...
                        outputText(Math.round(liveData.player.cocks[temp].cockThickness * 10) / 10 + " inches in diameter.");
                }
            }
            //horse cock flavor
            if (liveData.player.cocks[temp].cockType == CockTypesEnum.HORSE) {
            ...
            if (liveData.player.cocks[temp].knotMultiplier > 1) {
                if (liveData.player.cocks[temp].knotMultiplier >= 1.8)
                    outputText(" The obscenely swollen lump of flesh near the base of your " + liveData.player.cockDescript(temp) + " looks almost comically mismatched for your " + liveData.player.cockDescript(temp) + ".");
                else if (liveData.player.cocks[temp].knotMultiplier >= 1.4)
                    outputText(" A large bulge of flesh nestles just above the bottom of your " + liveData.player.cockDescript(temp) + ", to ensure it stays where it belongs during mating.");
                else
                    outputText(" A small knot of thicker flesh is near the base of your " + liveData.player.cockDescript(temp) + ", ready to expand to help you lodge your " + liveData.player.cockDescript(temp) + " inside a female.");
                //List knot thickness
        ...
        //Worm flavor
        if (liveData.player.findStatusEffect(StatusEffects.Infested) >= 0)
            outputText("Every now and again slimy worms coated in spunk slip partway out of your " + liveData.player.multiCockDescriptLight() + ", tasting the air like tongues of snakes.<br>");
        //DONE WITH COCKS, moving on!
    }
    //Of Balls and Sacks!
    if (liveData.player.balls > 0) {
        if (liveData.player.findStatusEffect(StatusEffects.Uniball) >= 0) {
            if (liveData.player.skinType != SKIN_TYPE_GOO)
                outputText("Your [sack] clings tightly to your groin, holding " + liveData.player.ballsDescript() + " snugly against you.");
            else if (liveData.player.skinType == SKIN_TYPE_GOO)
                outputText("Your [sack] clings tightly to your groin, dripping and holding " + liveData.player.ballsDescript() + " snugly against you.");
        }
        else if (liveData.player.cockTotal() == 0) {
            if (liveData.player.skinType == SKIN_TYPE_PLAIN)
                outputText("A " + liveData.player.sackDescript() + " with " + liveData.player.ballsDescript() + " swings heavily under where a penis would normally grow.");
            if (liveData.player.skinType == SKIN_TYPE_FUR)
                outputText("A fuzzy " + liveData.player.sackDescript() + " filled with " + liveData.player.ballsDescript() + " swings low under where a penis would normally grow.");
            if (liveData.player.skinType == SKIN_TYPE_SCALES)
                outputText("A scaley " + liveData.player.sackDescript() + " hugs your " + liveData.player.ballsDescript() + " tightly against your body.");
            if (liveData.player.skinType == SKIN_TYPE_GOO)
                outputText("An oozing, semi-solid sack with " + liveData.player.ballsDescript() + " swings heavily under where a penis would normally grow.");
        }
        else {
            if (liveData.player.skinType == SKIN_TYPE_PLAIN)
                outputText("A " + liveData.player.sackDescript() + " with " + liveData.player.ballsDescript() + " swings heavily beneath your " + liveData.player.multiCockDescriptLight() + ".");
            if (liveData.player.skinType == SKIN_TYPE_FUR)
                outputText("A fuzzy " + liveData.player.sackDescript() + " filled with " + liveData.player.ballsDescript() + " swings low under your " + liveData.player.multiCockDescriptLight() + ".");
            if (liveData.player.skinType == SKIN_TYPE_SCALES)
                outputText("A scaley " + liveData.player.sackDescript() + " hugs your " + liveData.player.ballsDescript() + " tightly against your body.");
            if (liveData.player.skinType == SKIN_TYPE_GOO)
    ...
    if (liveData.player.vaginas.length > 0) {
        ...
        if (liveData.player.vaginas.length == 1) {
            if (useMetrics)
                outputText("You have a " + liveData.player.vaginaDescript(0) + ", with a " + Math.floor(liveData.player.vaginas[0].clitLength * 10 * 2.54) / 10 + "-centimetre clit");
            else
                outputText("You have a " + liveData.player.vaginaDescript(0) + ", with a " + Math.floor(liveData.player.vaginas[0].clitLength * 10) / 10 + "-inch clit");
        ...
        if (liveData.player.vaginas.length > 1) {
            if (useMetrics)
                outputText("You have " + liveData.player.vaginas.length + " " + liveData.player.vaginaDescript(0) + "s, with " + Math.floor(liveData.player.vaginas[0].clitLength * 10 * 2.54) / 10 + "-centimetre clits each. ");
            else
                outputText("You have " + liveData.player.vaginas.length + " " + liveData.player.vaginaDescript(0) + "s, with " + Math.floor(liveData.player.vaginas[0].clitLength * 10) / 10 + "-inch clits each. ");
        }
        if (liveData.player.lib < 50 && player.lust < 50) {
            ...
            if (liveData.player.vaginas[0].vaginalWetness >= VAGINA_WETNESS_WET) {
                if (liveData.player.vaginas[0].vaginalLooseness < VAGINA_LOOSENESS_LOOSE)
                    outputText("your " + liveData.player.vaginaDescript(0) + ". ");
                if (liveData.player.vaginas[0].vaginalLooseness >= VAGINA_LOOSENESS_LOOSE && liveData.player.vaginas[0].vaginalLooseness < VAGINA_LOOSENESS_GAPING_WIDE)
                    outputText("your " + liveData.player.vaginaDescript(0) + ", its lips slightly parted. ");
                if (liveData.player.vaginas[0].vaginalLooseness >= VAGINA_LOOSENESS_GAPING_WIDE)
                    outputText("the massive hole that is your " + liveData.player.vaginaDescript(0) + ". ");
            }
        }
    ...
    //Piercings!
    if (liveData.player.eyebrowPierced > 0)
        outputText("<br>A solitary " + liveData.player.eyebrowPShort + " adorns your eyebrow, looking very stylish.");
    if (liveData.player.earsPierced > 0)
        outputText("<br>Your ears are pierced with " + liveData.player.earsPShort + ".");
    if (liveData.player.nosePierced > 0)
        outputText("<br>A " + liveData.player.nosePShort + " dangles from your nose.");
    if (liveData.player.lipPierced > 0)
        outputText("<br>Shining on your lip, a " + liveData.player.lipPShort + " is plainly visible.");
    if (liveData.player.tonguePierced > 0)
        outputText("<br>Though not visible, you can plainly feel your " + liveData.player.tonguePShort + " secured in your tongue.");
    if (liveData.player.nipplesPierced == 3)
        outputText("<br>Your " + liveData.player.nippleDescript(0) + "s ache and tingle with every step, as your heavy " + liveData.player.nipplesPShort + " swings back and forth.");
    else if (liveData.player.nipplesPierced > 0)
        outputText("<br>Your " + liveData.player.nippleDescript(0) + "s are pierced with " + liveData.player.nipplesPShort + ".");
    ...
    if (liveData.player.hasVagina()) {
        if (liveData.player.vaginas[0].labiaPierced > 0)
            outputText("<br>Your " + liveData.player.vaginaDescript(0) + " glitters with the " + liveData.player.vaginas[0].labiaPShort + " hanging from your lips.");
        if (liveData.player.vaginas[0].clitPierced > 0)
            outputText("<br>Impossible to ignore, your " + liveData.player.clitDescript() + " glitters with its " + liveData.player.vaginas[0].clitPShort + ".");
    }
    //MONEY!
    if (liveData.player.gems == 0)
        outputText("<br><br><b>Your money-purse is devoid of any currency.");
    if (liveData.player.gems > 1)
        outputText("<br><br><b>You have " + formatNumber(Math.floor(liveData.player.gems)) + " shining gems, collected in your travels.");
    if (liveData.player.gems == 1)
        outputText("<br><br><b>You have " + formatNumber(Math.floor(liveData.player.gems)) + " shining gem, collected in your travels.");
    GUI.doNext(playerMenu);
}
