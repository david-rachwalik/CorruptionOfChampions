...
class PlayerInfo {
    ...
    static attributeMenu() {
        ...
        if (liveData.player.statPoints > 0) {
            if (liveData.player.str < 100)
                GUI.addButton(0, "STR +", this.addAttribute, "str");
            if (liveData.player.tou < 100)
                GUI.addButton(1, "TOU +", this.addAttribute, "tou");
            if (liveData.player.spe < 100)
                GUI.addButton(2, "SPE +", this.addAttribute, "spe");
            if (liveData.player.inte < 100)
                GUI.addButton(3, "INTE +", this.addAttribute, "int");
        }
        //Subtract attributes
        if (liveData.player.tempStr > 0)
            GUI.addButton(5, "STR -", this.subtractAttribute, "str");
        if (liveData.player.tempTou > 0)
            GUI.addButton(6, "TOU +", this.subtractAttribute, "tou");
        if (liveData.player.tempSpe > 0)
            GUI.addButton(7, "SPE +", this.subtractAttribute, "spe");
        if (liveData.player.tempInt > 0)
            GUI.addButton(8, "INTE +", this.subtractAttribute, "int");
        //Reset & Done
    ...
    static subtractAttribute(attribute) {
        switch (attribute) {
            case "str":
                liveData.player.tempStr--;
                break;
    ...
    static finishAttributes() {
        GUI.clearOutput();
        if (liveData.player.tempStr > 0) {
            if (liveData.player.tempStr >= 3)
                GUI.outputText("Your muscles feel significantly stronger from your time adventuring.<br>");
            else
                GUI.outputText("Your muscles feel slightly stronger from your time adventuring.<br>");
        }
        if (liveData.player.tempTou > 0) {
            if (liveData.player.tempTou >= 3)
                GUI.outputText("You feel tougher from all the fights you have endured.<br>");
            else
                GUI.outputText("You feel slightly tougher from all the fights you have endured.<br>");
        }
        if (liveData.player.tempSpe > 0) {
            if (liveData.player.tempSpe >= 3)
                GUI.outputText("Your time in combat has driven you to move faster.<br>");
            else
                GUI.outputText("Your time in combat has driven you to move slightly faster.<br>");
        }
        if (liveData.player.tempInt > 0) {
            if (liveData.player.tempInt >= 3)
                GUI.outputText("Your time spent fighting the creatures of this realm has sharpened your wit.<br>");
            else
                GUI.outputText("Your time spent fighting the creatures of this realm has sharpened your wit slightly.<br>");
        }
    ...
    static perkConfirmation() {
        const inputValue = document.getElementById("perkselect").value;
        if (inputValue == "null") {
            this.perkBuyMenu();
            return;
        }
    ...
    static statsScreen() {
        ...
        if (liveData.player.teaseLevel < 5)
            ...
            combatStats += "<b>Tease Skill Level:</b> " + liveData.player.teaseLevel + " (Exp: MAX)<br>";
        if (combatStats.length > 0)
            GUI.outputText("<b><u>Combat Stats</u></b><br>" + combatStats + "<br><br>");
        //Body Stats
        let bodyStats = "";
        ...
        if (bodyStats.length > 0)
            GUI.outputText("<b><u>Body Stats</u></b><br>" + bodyStats + "<br><br>");
        GUI.doNext(liveData.playerMenu);
        // Addiction Stats
        let addictStats = "";
        // Mino Cum Addiction
        if (liveData.gameFlags[FLAG.EVER_DRANK_MINOCUM] > 0 || liveData.gameFlags[FLAG.MINOTAUR_CUM_ADDICTION_TRACKER] > 0 || liveData.player.findPerk(PerkLib.MinotaurCumAddict) >= 0 || liveData.player.findPerk(PerkLib.MinotaurCumResistance) >= 0) {
            ...
            else if (liveData.player.findPerk(PerkLib.MinotaurCumResistance) >= 0)
                addictStats += "<b>Minotaur Cum:</b> 0% (Immune)<br>";
            else
                addictStats += "<b>Minotaur Cum:</b> 100+%<br>";
        }
        if (addictStats != "")
            GUI.outputText("<br><b><u>Addictions</u></b><br>" + addictStats);
        // End Addiction Stats
    }
    //------------
    // PERKS
    ...
    static appearanceScreen() {
        GUI.clearOutput();
        let temp = 0;
        if (liveData.player.race() != "human")
            GUI.outputText("You began your journey as a human, but gave that up as you explored the dangers of this realm. ");
        GUI.outputText("You are a " + Math.floor(liveData.player.tallness / 12) + " foot " + (liveData.player.tallness % 12) + " inch tall " + liveData.player.maleFemaleHerm() + " " + liveData.player.race() + ", with " + liveData.player.bodyType() + ".");
        ...
        switch (liveData.player.faceType) {
            ...
            case ENUM.FaceType.FACE_FERRET:
                if (liveData.player.skinType == ENUM.SkinType.SKIN_TYPE_PLAIN)
                    ...
                    GUI.outputText(" Your face is coated in " + liveData.player.furColor + " fur with [skin] underneath, an adorable cross between human and ferret features. It is complete with a wet nose and whiskers.");
                break;
            case ENUM.FaceType.FACE_RACCOON_MASK:
                if (liveData.player.skinType != ENUM.SkinType.SKIN_TYPE_FUR && liveData.player.skinType != ENUM.SkinType.SKIN_TYPE_SCALES) {
                    ...
                    if ((liveData.player.skinTone == "ebony" || liveData.player.skinTone == "black") && (liveData.player.skinType == ENUM.SkinType.SKIN_TYPE_PLAIN || liveData.player.skinType == ENUM.SkinType.SKIN_TYPE_GOO))
                        ...
                        GUI.outputText(", though it is decorated with a sly-looking raccoon mask over your eyes.");
                }
                else {
                    if ((liveData.player.furColor == "black" || liveData.player.furColor == "midnight") && (liveData.player.skinType == ENUM.SkinType.SKIN_TYPE_FUR || liveData.player.skinType == ENUM.SkinType.SKIN_TYPE_SCALES))
                        ...
                        GUI.outputText(" Your " + liveData.player.skinFurScales() + " are decorated with a sly-looking raccoon mask, and under them");
            ...
            case ENUM.FaceType.FACE_RACCOON:
                ...
                if (liveData.player.skinType == ENUM.SkinType.SKIN_TYPE_PLAIN)
                    GUI.outputText(" It looks a bit strange with only the skin and no fur.");
            ...
            case ENUM.FaceType.FACE_MOUSE:
                ...
                if (liveData.player.skinType != ENUM.SkinType.SKIN_TYPE_FUR && liveData.player.skinType != ENUM.SkinType.SKIN_TYPE_SCALES)
                    ...
                    GUI.outputText(liveData.player.skin() + " under your " + liveData.player.skinFurScales());
            ...
            case ENUM.FaceType.FACE_COW_MINOTAUR:
                if (liveData.player.skinType == ENUM.SkinType.SKIN_TYPE_PLAIN || liveData.player.skinType == ENUM.SkinType.SKIN_TYPE_GOO)
                    GUI.outputText(" You have a face resembling that of a minotaur, with cow-like features, particularly a squared off wet nose. Despite your lack of fur elsewhere, your visage does have a short layer of " + liveData.player.furColor + " fuzz.");
                if (liveData.player.skinType == ENUM.SkinType.SKIN_TYPE_FUR)
                    GUI.outputText(" You have a face resembling that of a minotaur, with cow-like features, particularly a squared off wet nose. Your " +
            ...
            case ENUM.FaceType.FACE_DRAGON:
                GUI.outputText(" Your face is a narrow, reptilian muzzle. It looks like a predatory lizard's, at first glance, but with an unusual array of spikes along the under-jaw. It gives you a regal but fierce visage. Opening your mouth reveals several rows of dagger-like sharp teeth. The fearsome visage is decorated by " +
                    liveData.player.skinFurScales() +
                    ".");
                break;
            case ENUM.FaceType.FACE_KANGAROO:
                ...
                if (liveData.player.skinType == ENUM.SkinType.SKIN_TYPE_PLAIN)
                    ...
                    GUI.outputText("covered with " + liveData.player.skinFurScales());
            ...
            case ENUM.FaceType.FACE_RHINO:
                ...
                if (liveData.player.skinType == ENUM.SkinType.SKIN_TYPE_PLAIN)
                    ...
                    GUI.outputText(" with a long muzzle and a horn on your nose. Oddly, your face is also covered in " + liveData.player.skinFurScales() + ".");
                break;
            case ENUM.FaceType.FACE_ECHIDNA:
                ...
                if (liveData.player.skinType == ENUM.SkinType.SKIN_TYPE_PLAIN)
                    ...
                    GUI.outputText(" It's covered in " + liveData.player.skinFurScales() + ".");
                else if (liveData.player.skinType == ENUM.SkinType.SKIN_TYPE_SCALES)
        ...
        else if (liveData.player.eyeType == ENUM.EyeType.EYES_BLACK_EYES_SAND_TRAP)
            GUI.outputText(" Your eyes are solid spheres of inky, alien darkness.");
        //Hair
        //if bald
        if (liveData.player.hairLength == 0) {
            if (liveData.player.skinType == ENUM.SkinType.SKIN_TYPE_FUR)
                GUI.outputText(" You have no hair, only a thin layer of fur atop of your head. ");
            else
            ...
            if (liveData.player.earType == ENUM.EarType.EARS_HORSE)
                GUI.outputText(" A pair of horse-like ears rise up from the top of your head.");
            else if (liveData.player.earType == ENUM.EarType.EARS_FERRET)
                GUI.outputText(" A pair of small, rounded ferret ears sit on top of your head.");
            else if (liveData.player.earType == ENUM.EarType.EARS_DOG)
                GUI.outputText(" A pair of dog ears protrude from your skull, flopping down adorably.");
            else if (liveData.player.earType == ENUM.EarType.EARS_COW)
                GUI.outputText(" A pair of round, floppy cow ears protrude from the sides of your skull.");
            else if (liveData.player.earType == ENUM.EarType.EARS_ELFIN)
                GUI.outputText(" A pair of large pointy ears stick out from your skull.");
            else if (liveData.player.earType == ENUM.EarType.EARS_CAT)
                GUI.outputText(" A pair of cute, fuzzy cat ears have sprouted from the top of your head.");
            else if (liveData.player.earType == ENUM.EarType.EARS_PIG)
                GUI.outputText(" A pair of pointy, floppy pig ears have sprouted from the top of your head.");
                ...
                GUI.outputText(" A pair of large, adept fox ears sit high on your head, always listening.");
                ...
                GUI.outputText(" A pair of vaguely egg-shaped, furry raccoon ears adorns your head.");
            else if (liveData.player.earType == ENUM.EarType.EARS_MOUSE)
                GUI.outputText(" A pair of large, dish-shaped mouse ears tops your head.");
            //<mod>
            else if (liveData.player.earType == ENUM.EarType.EARS_PIG)
                GUI.outputText(" A pair of pointy, floppy pig ears have sprouted from the top of your head.");
            else if (liveData.player.earType == ENUM.EarType.EARS_RHINO)
                GUI.outputText(" A pair of open tubular rhino ears protrude from your head.");
            else if (liveData.player.earType == ENUM.EarType.EARS_ECHIDNA)
                GUI.outputText(" A pair of small rounded openings appear on your head that are your ears.");
            else if (liveData.player.earType == ENUM.EarType.EARS_DEER)
                GUI.outputText(" A pair of deer-like ears rise up from the top of your head.");
            //</mod>
        ...
        //not bald
            ...
            if (liveData.player.earType == ENUM.EarType.EARS_HUMAN)
                ...
                GUI.outputText(" The " + liveData.player.hairDescript() + " atop your head makes it nigh-impossible to notice the two small rounded openings that are your ears.");
            else if (liveData.player.earType == ENUM.EarType.EARS_BUNNY)
                ...
                GUI.outputText(" The " + liveData.player.hairDescript() + " atop your head is parted by a pair of long, furred kangaroo ears that stick out at an angle.");
            else if (liveData.player.earType == ENUM.EarType.EARS_FOX)
                GUI.outputText(" The " + liveData.player.hairDescript() + " atop your head is parted by a pair of large, adept fox ears that always seem to be listening.");
            else if (liveData.player.earType == ENUM.EarType.EARS_DRAGON)
                ...
                GUI.outputText(" The " + liveData.player.hairDescript() + " atop your head is funneled between and around a pair of large, dish-shaped mouse ears that stick up prominently.");
            //<mod> Mod-added ears
            else if (liveData.player.earType == ENUM.EarType.EARS_PIG)
            ...
            if (liveData.player.antennae == ENUM.AntennaeType.ANTENNAE_BEE) {
                if (liveData.player.earType == ENUM.EarType.EARS_BUNNY)
                    ...
                    GUI.outputText(" Floppy antennae also grow from just behind your hairline, bouncing and swaying in the breeze.");
            }
        }
        //Beards!
        if (liveData.player.beardLength > 0) {
            ...
            if (liveData.player.beardStyle != ENUM.BeardType.BEARD_GOATEE) {
                ...
                if (UTIL.rand(2) == 0)
                    ...
                    GUI.outputText("chin and cheeks");
            }
            else {
        ...
        if (liveData.player.tongueType == ENUM.TongueType.TONGUE_SNAKE)
            GUI.outputText(" A snake-like tongue occasionally flits between your lips, tasting the air.");
        ...
        switch (liveData.player.hornType) {
            case ENUM.HornType.HORNS_DEMON:
                if (liveData.player.horns == 2)
                    GUI.outputText(" A small pair of pointed horns has broken through the " + liveData.player.skinDesc + " on your forehead, proclaiming some demonic taint to any who see them.");
                ...
                if (liveData.player.horns == 6)
                    GUI.outputText(" Six horns have sprouted through your " +
                ...
                if (liveData.player.horns >= 8)
                    GUI.outputText(" A large number of thick demonic horns sprout through your " +
                        liveData.player.skinDesc +
            ...
            case ENUM.HornType.HORNS_COW_MINOTAUR:
                ...
                if (liveData.player.horns >= 12 && liveData.player.horns < 20)
                    GUI.outputText(" Two very large and dangerous looking horns sprout from your head, curving forward and over a foot long. They have dangerous looking points.");
                if (liveData.player.horns >= 20)
                    GUI.outputText(" Two huge horns erupt from your forehead, curving outward at first, then forwards. The weight of them is heavy, and they end in dangerous looking points.");
                break;
            case ENUM.HornType.HORNS_DRACONIC_X2:
                if (liveData.useMetrics)
                    GUI.outputText(" A pair of " + UTIL.num2Text(Math.floor(liveData.player.horns * 2.54)) + " centimetre horns grow from the sides of your head, sweeping backwards and adding to your imposing visage.");
                else
                    GUI.outputText(" A pair of " + UTIL.num2Text(liveData.player.horns) + " inch horns grow from the sides of your head, sweeping backwards and adding to your imposing visage.");
                break;
            ...
            case ENUM.HornType.HORNS_GOAT:
                if (liveData.player.horns == 1)
                    GUI.outputText(" A pair of stubby goat horns sprout from the sides of your head.");
                else
                    GUI.outputText(" A pair of tall-standing goat horns sprout from the sides of your head. They are curved and patterned with ridges.");
                break;
            case ENUM.HornType.HORNS_RHINO:
                if (liveData.player.horns >= 2) {
                    if (liveData.player.faceType == ENUM.FaceType.FACE_RHINO)
                        GUI.outputText(" A second horn sprouts from your forehead just above the horn on your nose.");
                    else
                        GUI.outputText(" A single horn sprouts from your forehead. It is conical and resembles a rhino's horn.");
        ...
        switch (liveData.player.wingType) {
            ...
            case ENUM.WingType.WING_TYPE_DRACONIC_LARGE:
                GUI.outputText(" Magnificent wings sprout from your shoulders. When unfurled they stretch further than your arm span, and a single beat of them is all you need to set out toward the sky. They look a bit like bat's wings, but the membranes are covered in fine, delicate scales and a wicked talon juts from the end of each bone.");
                break;
        ...
        if (liveData.player.isTaur()) {
            if (liveData.player.lowerBody == ENUM.LowerBodyType.LOWER_BODY_TYPE_HOOFED)
                ...
                GUI.outputText(" Where your legs would normally start you have grown the body of a feral animal, with all " + UTIL.num2Text(liveData.player.legCount) + " legs.");
        }
            ...
            if (liveData.player.thickness > 70) {
                ...
                if (liveData.player.hipRating < 6) {
                    if (liveData.player.tone < 65)
                        GUI.outputText(" buried under a noticeable muffin-top, and");
                    else
                        GUI.outputText(" that blend into your pillar-like waist, and");
                }
                if (liveData.player.hipRating >= 6 && liveData.player.hipRating < 10)
                    GUI.outputText(" that blend into the rest of your thick form, and");
                ...
                if (liveData.player.hipRating >= 15 && liveData.player.hipRating < 20)
                    GUI.outputText(" that sway and emphasize your thick, curvy shape, and");
                if (liveData.player.hipRating >= 20)
                    GUI.outputText(" that sway hypnotically on your extra-curvy frame, and");
            }
            else if (liveData.player.thickness < 30) {
                GUI.outputText(" You have " + liveData.player.hipDescript());
                if (liveData.player.hipRating < 6)
                    GUI.outputText(" that match your trim, lithe body, and");
                if (liveData.player.hipRating >= 6 && liveData.player.hipRating < 10)
                    GUI.outputText(" that sway to and fro, emphasized by your trim body, and");
                if (liveData.player.hipRating >= 10 && liveData.player.hipRating < 15)
                    GUI.outputText(" that swell out under your trim waistline, and");
                if (liveData.player.hipRating >= 15 && liveData.player.hipRating < 20)
                    GUI.outputText(", emphasized by your narrow waist, and");
                if (liveData.player.hipRating >= 20)
                    GUI.outputText(" that swell disproportionately wide on your lithe frame, and");
            }
            //STANDARD
            else {
                GUI.outputText(" You have " + liveData.player.hipDescript());
                ...
                if (liveData.player.femininity > 50) {
                    if (liveData.player.hipRating >= 6 && liveData.player.hipRating < 10)
                        GUI.outputText(" that draw the attention of those around you, and");
                    if (liveData.player.hipRating >= 10 && liveData.player.hipRating < 15)
                        GUI.outputText(" that make you walk with a sexy, swinging gait, and");
                    if (liveData.player.hipRating >= 15 && liveData.player.hipRating < 20)
                        GUI.outputText(" that make it look like you've birthed many children, and");
                    if (liveData.player.hipRating >= 20)
                        GUI.outputText(" that make you look more like an animal waiting to be bred than any kind of human, and");
                }
                else {
                    if (liveData.player.hipRating >= 6 && liveData.player.hipRating < 10)
                        GUI.outputText(" that give you a graceful stride, and");
                    if (liveData.player.hipRating >= 10 && liveData.player.hipRating < 15)
                        GUI.outputText(" that add a little feminine swing to your gait, and");
                    if (liveData.player.hipRating >= 15 && liveData.player.hipRating < 20)
                        GUI.outputText(" that force you to sway and wiggle as you move, and");
                    if (liveData.player.hipRating >= 20) {
                        GUI.outputText(" that give your ");
                        if (liveData.player.balls > 0)
                            GUI.outputText("balls plenty of room to breathe");
                        else if (liveData.player.hasCock())
                            GUI.outputText(liveData.player.multiCockDescript() + " plenty of room to swing");
                        else if (liveData.player.hasVagina())
                            GUI.outputText(liveData.player.vaginaDescript() + " a nice, wide berth");
                        else
                            GUI.outputText("vacant groin plenty of room");
                        GUI.outputText(", and");
                    }
                }
            }
        ...
        if (liveData.player.isTaur()) {
            ...
            if (liveData.player.tone < 65) {
                ...
                if (liveData.player.buttRating < 4)
                    GUI.outputText(" is lean, from what you can see of it.");
                if (liveData.player.buttRating >= 4 && liveData.player.buttRating < 6)
                    GUI.outputText(" looks fairly average.");
                if (liveData.player.buttRating >= 6 && liveData.player.buttRating < 10)
                    GUI.outputText(" is fairly plump and healthy.");
                if (liveData.player.buttRating >= 10 && liveData.player.buttRating < 15)
                    GUI.outputText(" jiggles a bit as you trot around.");
                if (liveData.player.buttRating >= 15 && liveData.player.buttRating < 20)
                    GUI.outputText(" jiggles and wobbles as you trot about.");
                if (liveData.player.buttRating >= 20)
                    GUI.outputText(" is obscenely large, bordering freakish, even for a horse.");
            }
            //GIRL LOOK AT DAT BOOTY
            else {
                GUI.outputText(" Your " + liveData.player.buttDescript());
                if (liveData.player.buttRating < 4)
                    GUI.outputText(" is barely noticable, showing off the muscles of your haunches.");
                if (liveData.player.buttRating >= 4 && liveData.player.buttRating < 6)
                    GUI.outputText(" matches your toned equine frame quite well.");
                if (liveData.player.buttRating >= 6 && liveData.player.buttRating < 10)
                    GUI.outputText(" gives hints of just how much muscle you could put into a kick.");
                if (liveData.player.buttRating >= 10 && liveData.player.buttRating < 15)
                    GUI.outputText(" surges with muscle whenever you trot about.");
                if (liveData.player.buttRating >= 15 && liveData.player.buttRating < 20)
                    GUI.outputText(" flexes its considerable mass as you move.");
                if (liveData.player.buttRating >= 20)
                    GUI.outputText(" is stacked with layers of muscle, huge even for a horse.");
            }
        }
        //Non-horse PCs
        else {
            ...
            if (liveData.player.tone < 60) {
                GUI.outputText(" your " + liveData.player.buttDescript());
                if (liveData.player.buttRating < 4)
                    GUI.outputText(" looks great under your gear.");
                if (liveData.player.buttRating >= 4 && liveData.player.buttRating < 6)
                    GUI.outputText(" has the barest amount of sexy jiggle.");
                if (liveData.player.buttRating >= 6 && liveData.player.buttRating < 10)
                    GUI.outputText(" fills out your clothing nicely.");
                if (liveData.player.buttRating >= 10 && liveData.player.buttRating < 15)
                    GUI.outputText(" wobbles enticingly with every step.");
                if (liveData.player.buttRating >= 15 && liveData.player.buttRating < 20)
                    GUI.outputText(" wobbles like a bowl full of jello as you walk.");
                if (liveData.player.buttRating >= 20)
                    GUI.outputText(" is obscenely large, bordering freakish, and makes it difficult to run.");
            }
            //FITBUTT
            else {
                GUI.outputText(" your " + liveData.player.buttDescript());
                if (liveData.player.buttRating < 4)
                    GUI.outputText(" molds closely against your form.");
                if (liveData.player.buttRating >= 4 && liveData.player.buttRating < 6)
                    GUI.outputText(" contracts with every motion, displaying the detailed curves of its lean musculature.");
                if (liveData.player.buttRating >= 6 && liveData.player.buttRating < 10)
                    GUI.outputText(" fills out your clothing nicely.");
                if (liveData.player.buttRating >= 10 && liveData.player.buttRating < 15)
                    GUI.outputText(" stretches your gear, flexing it with each step.");
                if (liveData.player.buttRating >= 15 && liveData.player.buttRating < 20)
                    GUI.outputText(" threatens to bust out from under your kit each time you clench it.");
                if (liveData.player.buttRating >= 20)
                    GUI.outputText(" is marvelously large, but completely stacked with muscle.");
            }
        }
        //TAILS
        switch (liveData.player.tailType) {
            ...
            case ENUM.TailType.TAIL_TYPE_SPIDER_ADBOMEN:
                ...
                if (liveData.player.tailVenom > 50 && liveData.player.tailVenom < 80)
                    GUI.outputText(" Your bulging arachnid posterior feels fairly full of webbing.");
                if (liveData.player.tailVenom >= 80 && liveData.player.tailVenom < 100)
                    GUI.outputText(" Your arachnid rear bulges and feels very full of webbing.");
                if (liveData.player.tailVenom == 100)
                    GUI.outputText(" Your swollen spider-butt is distended with the sheer amount of webbing it's holding.");
                break;
            case ENUM.TailType.TAIL_TYPE_BEE_ABDOMEN:
                ...
                if (liveData.player.tailVenom > 50 && liveData.player.tailVenom < 80)
                    GUI.outputText(" A single drop of poison hangs from your exposed stinger.");
                if (liveData.player.tailVenom >= 80 && liveData.player.tailVenom < 100)
                    GUI.outputText(" Poisonous bee venom coats your stinger completely.");
                if (liveData.player.tailVenom == 100)
                    GUI.outputText(" Venom drips from your poisoned stinger regularly.");
                break;
            ...
            case ENUM.TailType.TAIL_TYPE_KANGAROO:
                ...
                if (liveData.player.skinType == ENUM.SkinType.SKIN_TYPE_GOO)
                    GUI.outputText("gooey, " + liveData.player.skinTone);
                else
                    GUI.outputText("furry, " + liveData.player.furColor);
            ...
            case ENUM.TailType.TAIL_TYPE_FOX:
                if (liveData.player.tailVenom <= 1)
                    GUI.outputText(" A swishing " + liveData.player.furColor + " fox's brush extends from your " + liveData.player.assDescript() + ", curling around your body - the soft fur feels lovely.");
                else
                    GUI.outputText(" " + UTIL.Num2Text(liveData.player.tailVenom) + " swishing " + liveData.player.furColor + " fox's tails extend from your " + liveData.player.assDescript() + ", curling around your body - the soft fur feels lovely.");
                break;
        ...
        //Horse body is placed higher for readability purposes
            ...
            GUI.outputText(" In place of legs you have a shifting amorphous blob. Thankfully it's quite easy to propel yourself around on. The lowest portions of your " +
            ...
            GUI.outputText(" " + UTIL.Num2Text(liveData.player.legCount) + " digitigrade legs grow down from your " + liveData.player.hipDescript() + ", ending in clawed feet. There are three long toes on the front, and a small hind-claw on the back.");
        ...
        if (liveData.player.buttPregnancyType == ENUM.PregnancyType.PREGNANCY_FROG_GIRL || liveData.player.buttPregnancyType == ENUM.PregnancyType.PREGNANCY_SATYR || liveData.player.isPregnant()) {
            ...
            if (liveData.player.buttPregnancyType == ENUM.PregnancyType.PREGNANCY_SATYR && liveData.player.buttPregnancyIncubation > liveData.player.pregnancyIncubation) {
                if (liveData.player.buttPregnancyIncubation < 125 && liveData.player.buttPregnancyIncubation >= 75) {
                    ...
                    if (liveData.player.pregnancyType == ENUM.PregnancyType.PREGNANCY_IZMA ||
                        ...
                        GUI.outputText("<br><b>Your belly protrudes unnaturally far forward, bulging outwards with Marble's precious child.</b>");
                }
            }
            //URTA PREG
                ...
                if (liveData.player.buttPregnancyIncubation >= 8)
                    ...
                    GUI.outputText("<b>You're stuffed so full with eggs that your belly looks obscenely distended, huge and weighted with the gargantuan eggs crowding your gut. They make your gait a waddle and your gravid tummy wobble obscenely.</b>");
            }
                ...
                else if (liveData.player.pregnancyIncubation <= 140)
                    GUI.outputText("It feels like it's full of thick syrup or jelly.</b>");
                else
                    GUI.outputText("It still feels like there's a solid ball inside your womb.</b>");
            }
            else {
        ...
        if (liveData.player.gills)
            GUI.outputText("A pair of feathery gills are growing out just below your neck, spreading out horizontally and draping down your chest. They allow you to stay in the water for quite a long time. ");
        ...
        if (liveData.player.breastRows.length == 0) {
            ...
            if (liveData.useMetrics)
                GUI.outputText(Math.floor(liveData.player.breastRows[0].nippleLength * 2.54 * 10) / 10 + "-cm ");
            //Centimeter display
            else
                GUI.outputText(Math.floor(liveData.player.breastRows[0].nippleLength * 10) / 10 + "-inch "); //Inches display
            ...
            if (liveData.player.breastRows[0].milkFullness > 75)
                GUI.outputText(" Your " + liveData.player.breastDescript(temp) + " are painful and sensitive from being so stuffed with milk. You should release the pressure soon.");
            if (liveData.player.breastRows[0].breastRating >= 1)
                GUI.outputText(" You could easily fill a " + liveData.player.breastCup(temp) + " bra.");
            //Done with tits.  Move on.
            GUI.outputText("<br>");
        }
        //many rows
            ...
            while (temp < liveData.player.breastRows.length) {
                ...
                if (liveData.useMetrics)
                    breastListText += Math.floor(liveData.player.breastRows[temp].nippleLength * 2.54 * 10) / 10 + "-cm ";
                //Centimeter
                else
                    breastListText += Math.floor(liveData.player.breastRows[temp].nippleLength * 10) / 10 + "-inch "; //Inches
                ...
                if (liveData.player.breastRows[temp].breastRating >= 1)
                    breastListText += " They could easily fill a " + liveData.player.breastCup(temp) + " bra.";
                if (liveData.player.breastRows[temp].milkFullness > 75)
                    breastListText += " Your " + liveData.player.breastDescript(temp) + " are painful and sensitive from being so stuffed with milk. You should release the pressure soon.";
                breastListText += "</li>";
                temp++;
            }
            breastListText += "</ul>";
        ...
        if (liveData.player.lowerBody == ENUM.LowerBodyType.LOWER_BODY_TYPE_NAGA && liveData.player.gender > 0) {
            ...
            if (liveData.player.gender == 3 || liveData.player.totalCocks() > 1)
                GUI.outputText("es are ");
            else
                GUI.outputText(" is ");
        ...
        if (liveData.player.cocks.length == 1) {
            if (liveData.player.isTaur())
                GUI.outputText("<br>Your equipment has shifted to lie between your hind legs, like a feral animal.");
            if (liveData.useMetrics)
                GUI.outputText("<br>Your " + liveData.player.cockDescript(temp) + " is " + Math.round(liveData.player.cocks[temp].cockLength * 10 * 2.54) / 10 + " cm long and ");
            else
                GUI.outputText("<br>Your " + liveData.player.cockDescript(temp) + " is " + Math.round(liveData.player.cocks[temp].cockLength * 10) / 10 + " inches long and ");
            if (Math.round(10 * liveData.player.cocks[temp].cockThickness) / 10 < 10) {
                if (liveData.useMetrics) {
                    if (Math.round(liveData.player.cocks[temp].cockThickness * 10 * 2.54) / 10 == 1)
                        ...
                        GUI.outputText(Math.round(liveData.player.cocks[temp].cockThickness * 10 * 2.54) / 10 + " centimetres thick.");
                }
                else {
                    if (Math.round(liveData.player.cocks[temp].cockThickness * 10) / 10 == 1)
                        ...
                        GUI.outputText(Math.round(liveData.player.cocks[temp].cockThickness * 10) / 10 + " inches thick.");
                }
            }
            else
                GUI.outputText(UTIL.num2Text(Math.round(10 * liveData.player.cocks[temp].cockThickness) / 10) + " inches wide.");
            //Horsecock flavor
            ...
            if (liveData.player.cocks[temp].cockType == ENUM.CockType.DOG || liveData.player.cocks[temp].cockType == ENUM.CockType.FOX || liveData.player.cocks[temp].cockType == ENUM.CockType.FOX) {
                if (liveData.player.cocks[temp].knotMultiplier >= 1.8)
                    GUI.outputText(" The obscenely swollen lump of flesh near the base of your " + liveData.player.cockDescript(temp) + " looks almost too big for your cock.");
                    ...
                    GUI.outputText(" A small knot of thicker flesh is near the base of your " + liveData.player.cockDescript(temp) + ", ready to expand to help you lodge it inside a female.");
                //List thickness
            ...
            if (liveData.player.cocks[temp].cockType == ENUM.CockType.TENTACLE) {
                GUI.outputText(" The entirety of its green surface is covered in perspiring beads of slick moisture. It frequently shifts and moves of its own volition, the slightly oversized and mushroom-like head shifting in coloration to purplish-red whenever you become aroused.");
            }
            //Cat cock flavor
            if (liveData.player.cocks[temp].cockType == ENUM.CockType.CAT) {
                GUI.outputText(" It ends in a single point, much like a spike, and is covered in small, fleshy barbs. The barbs are larger at the base and shrink in size as they get closer to the tip. Each of the spines is soft and flexible, and shouldn't be painful for any of your partners.");
            }
            //Snake cock flavor
            ...
            if (liveData.player.cocks[temp].cockType == ENUM.CockType.BEE) {
                GUI.outputText(" It's a long, smooth black shaft that's rigid to the touch. Its base is ringed with a layer of four inch long soft bee hair. The tip has a much finer layer of short yellow hairs. The tip is very sensitive, and it hurts constantly if you don't have bee honey on it.");
            }
            //Pig flavor
        ...
        if (liveData.player.cocks.length > 1) {
            ...
            if (liveData.player.isTaur())
                GUI.outputText("<br>Between hind legs of your bestial body you have grown " + liveData.player.multiCockDescript() + "!<br>");
            else
                GUI.outputText("<br>Where a penis would normally be located, you have instead grown " + liveData.player.multiCockDescript() + "!<br>");
            while (temp < liveData.player.cocks.length) {
                //middle cock description
                if (rando == 0) {
                    if (temp == 0)
                        GUI.outputText("--Your first ");
                    else
                        GUI.outputText("--Your next ");
                    ...
                    if (Math.floor(liveData.player.cocks[temp].cockThickness) >= 2)
                        ...
                        if (liveData.player.cocks[temp].cockThickness == 1)
                            GUI.outputText("one inch wide.");
                        else
                            GUI.outputText(Math.round(liveData.player.cocks[temp].cockThickness * 10) / 10 + " inches wide.");
                    }
                }
                if (rando == 1) {
                ...
                if (rando == 2) {
                    if (temp > 0)
                        GUI.outputText("--Another of your ");
                    else
                        GUI.outputText("--One of your ");
                    GUI.outputText(liveData.player.cockDescript(temp) + "s is " + Math.round(10 * liveData.player.cocks[temp].cockLength) / 10 + " inches long and ");
                    if (Math.floor(liveData.player.cocks[temp].cockThickness) >= 2)
                ...
                if (rando == 3) {
                    ...
                    if (Math.floor(liveData.player.cocks[temp].cockThickness) >= 2)
                        ...
                        if (Math.round(liveData.player.cocks[temp].cockThickness * 10) / 10 == 1)
                            GUI.outputText("one inch in diameter.");
                        else
                            GUI.outputText(Math.round(liveData.player.cocks[temp].cockThickness * 10) / 10 + " inches in diameter.");
                    }
                }
                //horse cock flavor
                if (liveData.player.cocks[temp].cockType == ENUM.CockType.HORSE) {
                ...
                if (liveData.player.cocks[temp].cockType == ENUM.CockType.DOG || liveData.player.cocks[temp].cockType == ENUM.CockType.FOX) {
                    ...
                    if (liveData.player.cocks[temp].cockType == ENUM.CockType.DOG)
                        GUI.outputText("dog's cock.");
                    else
                        GUI.outputText("fox's cock.");
                }
                //Demon cock flavor
                if (liveData.player.cocks[temp].cockType == ENUM.CockType.DEMON) {
                    GUI.outputText(" The crown is ringed with a circle of rubbery protrusions that grow larger as you get more aroused. The entire thing is shiny and covered with tiny, sensitive nodules that leave no doubt about its demonic origins.");
                ...
                if (liveData.player.cocks[temp].knotMultiplier > 1) {
                    if (liveData.player.cocks[temp].knotMultiplier >= 1.8)
                        ...
                        GUI.outputText(" A small knot of thicker flesh is near the base of your " + liveData.player.cockDescript(temp) + ", ready to expand to help you lodge your " + liveData.player.cockDescript(temp) + " inside a female.");
                    //List knot thickness
        ...
        if (liveData.player.balls > 0) {
            if (liveData.player.findStatusEffect(StatusEffects.Uniball) >= 0) {
                ...
                if (liveData.player.skinType == ENUM.SkinType.SKIN_TYPE_PLAIN)
                    GUI.outputText("A " + liveData.player.sackDescript() + " with " + liveData.player.ballsDescript() + " swings heavily under where a penis would normally grow.");
                if (liveData.player.skinType == ENUM.SkinType.SKIN_TYPE_FUR)
                    GUI.outputText("A fuzzy " + liveData.player.sackDescript() + " filled with " + liveData.player.ballsDescript() + " swings low under where a penis would normally grow.");
                if (liveData.player.skinType == ENUM.SkinType.SKIN_TYPE_SCALES)
                ...
                if (liveData.player.skinType == ENUM.SkinType.SKIN_TYPE_SCALES)
                    GUI.outputText("A scaley " + liveData.player.sackDescript() + " hugs your " + liveData.player.ballsDescript() + " tightly against your body.");
                if (liveData.player.skinType == ENUM.SkinType.SKIN_TYPE_GOO)
                    GUI.outputText("An oozing, semi-solid sack with " + liveData.player.ballsDescript() + " swings heavily beneath your " + liveData.player.multiCockDescriptLight() + ".");
            }
        ...
        if (liveData.player.vaginas.length > 0) {
            ...
            if (liveData.player.vaginas.length == 1) {
                if (liveData.useMetrics)
                    GUI.outputText("You have a " + liveData.player.vaginaDescript(0) + ", with a " + Math.floor(liveData.player.vaginas[0].clitLength * 10 * 2.54) / 10 + "-centimetre clit");
                else
                    GUI.outputText("You have a " + liveData.player.vaginaDescript(0) + ", with a " + Math.floor(liveData.player.vaginas[0].clitLength * 10) / 10 + "-inch clit");
                if (liveData.player.vaginas[0].virgin)
                    GUI.outputText(" and an intact hymen");
                GUI.outputText(". ");
            }
            if (liveData.player.vaginas.length > 1) {
                if (liveData.useMetrics)
                    ...
                    GUI.outputText("You have " + liveData.player.vaginas.length + " " + liveData.player.vaginaDescript(0) + "s, with " + Math.floor(liveData.player.vaginas[0].clitLength * 10) / 10 + "-inch clits each. ");
            }
        ...
        //Genderless lovun'
        if (liveData.player.cockTotal() == 0 && liveData.player.vaginas.length == 0)
            GUI.outputText("<br>You have a curious lack of any sexual endowments.<br>");
        //BUNGHOLIO
        ...
        // }
        //MONEY!
        if (liveData.player.gems == 0)
            GUI.outputText("<br><br><b>Your money-purse is devoid of any currency.");
