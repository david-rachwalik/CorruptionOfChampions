...
function attributeMenu() {
    ...
    if (player.statPoints > 0) {
        if (player.str < 100)
            addButton(0, "STR +", addAttribute, "str");
        if (player.tou < 100)
            addButton(1, "TOU +", addAttribute, "tou");
        if (player.spe < 100)
            addButton(2, "SPE +", addAttribute, "spe");
        if (player.inte < 100)
            addButton(3, "INTE +", addAttribute, "int");
    }
    //Subtract attributes
    if (tempStr > 0)
        addButton(5, "STR -", subtractAttribute, "str");
    if (tempTou > 0)
        addButton(6, "TOU +", subtractAttribute, "tou");
    if (tempSpe > 0)
        addButton(7, "SPE +", subtractAttribute, "spe");
    if (tempInt > 0)
        addButton(8, "INTE +", subtractAttribute, "int");
    //Reset & Done
...
function statsScreen() {
    ...
    if (player.teaseLevel < 5)
        combatStats += "<b>Tease Skill Level:</b> " + player.teaseLevel + " (Exp: " + player.teaseXP + " / " + (10 + (player.teaseLevel + 1) * 5 * (player.teaseLevel + 1)) + ")<br>";
    else
        combatStats += "<b>Tease Skill Level:</b> " + player.teaseLevel + " (Exp: MAX)<br>";
        ...
        case FACE_RACCOON_MASK:
            if (player.skinType != SKIN_TYPE_FUR && player.skinType != SKIN_TYPE_SCALES) {
                ...
                if (((player.furColor == "black" || player.furColor == "midnight") && (player.skinType == SKIN_TYPE_FUR || player.skinType == SKIN_TYPE_SCALES)))
                    ...
                    outputText(" Your " + player.skinFurScales() + " are decorated with a sly-looking raccoon mask, and under them");
            ...
            if (rando == 0) {
                ...
                if (Math.floor(player.cocks[temp].cockThickness) >= 2)
                    ...
                    if (player.cocks[temp].cockThickness == 1)
                        ...
                        outputText(Math.round(player.cocks[temp].cockThickness * 10) / 10 + " inches wide.");
                }
            }
            if (rando == 1) {
            ...
            if (rando == 3) {
                ...
                if (Math.floor(player.cocks[temp].cockThickness) >= 2)
                    ...
                    if (Math.round(player.cocks[temp].cockThickness * 10) / 10 == 1)
                        ...
                        outputText(Math.round(player.cocks[temp].cockThickness * 10) / 10 + " inches in diameter.");
                }
            }
            //horse cock flavor
            if (player.cocks[temp].cockType == CockTypesEnum.HORSE) {
