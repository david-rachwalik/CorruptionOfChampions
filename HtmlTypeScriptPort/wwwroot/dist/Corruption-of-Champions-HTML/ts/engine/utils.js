...
class UTIL {
    ...
    static num2Text(num) {
        if (num >= 0 && num <= 10)
            return this.NUMBER_WORDS_NORMAL[num];
        return num.toString();
    }
    static num2Text2(num) {
        if (num < 0)
            return num.toString(); //Can't really have the -10th of something
        if (num <= 10)
            return this.NUMBER_WORDS_POSITIONAL[num];
        switch (num % 10) {
            case 1:
    ...
    static Num2Text(num) {
        if (num >= 0 && num <= 10)
            return this.NUMBER_WORDS_CAPITAL[num];
        return num.toString();
    }
    //Comma display
    ...
    //Capitalize letters
    static capitalizeFirstLetter(msg) {
        return msg.charAt(0).toUpperCase() + msg.slice(1);
    }
    static capitalize(msg) {
        //Alternate function
...
UTIL.NUMBER_WORDS_POSITIONAL = ["zeroth", "first", "second", "third", "fourth", "fifth", "sixth", "seventh", "eighth", "ninth", "tenth"];
export { UTIL };
