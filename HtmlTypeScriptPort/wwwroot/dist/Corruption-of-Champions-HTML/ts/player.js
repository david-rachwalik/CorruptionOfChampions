...
Player.prototype.race = function () {
    ...
    if (this.horseScore() >= 3) {
        if (this.isTaur())
            race = "centaur-morph";
        else if (this.hornType == HORNS_UNICORN)
            race = "unicorn-morph";
        else
            race = "equine-morph";
    }
...
Player.prototype.ferretScore = function () {
    ...
    if (this.lowerBody == LOWER_BODY_TYPE_FERRET)
        counter++;
    if (this.skinType == SKIN_TYPE_FUR && counter > 0)
        counter++;
    return counter;
};
...
Player.prototype.lengthChange = function (amount, ncocks) {
    ...
    if (amount <= 1 && amount > 0) {
        if (this.cocks.length == 1)
            outputText("Your " + player.cockDescript(0) + " has grown slightly longer.", false);
        if (this.cocks.length > 1) {
            if (ncocks == 1)
                outputText("One of your " + player.multiCockDescriptLight() + " grows slightly longer.", false);
            if (ncocks > 1 && ncocks < this.cocks.length)
                outputText("Some of your " + player.multiCockDescriptLight() + " grow slightly longer.", false);
            if (ncocks == this.cocks.length)
                outputText("Your " + player.multiCockDescriptLight() + " seem to fill up... growing a little bit larger.", false);
        }
    }
    if (amount > 1 && amount < 3) {
        if (this.cocks.length == 1)
            outputText("A very pleasurable feeling spreads from your groin as your " + player.cockDescript(0) + " grows permanently longer - at least an inch - and leaks pre-cum from the pleasure of the change.", false);
        if (this.cocks.length > 1) {
            if (ncocks == this.cocks.length)
                outputText("A very pleasurable feeling spreads from your groin as your " + player.multiCockDescriptLight() + " grow permanently longer - at least an inch - and leak plenty of pre-cum from the pleasure of the change.", false);
            if (ncocks == 1)
                outputText("A very pleasurable feeling spreads from your groin as one of your " + player.multiCockDescriptLight() + " grows permanently longer, by at least an inch, and leaks plenty of pre-cum from the pleasure of the change.", false);
            if (ncocks > 1 && ncocks < this.cocks.length)
                outputText("A very pleasurable feeling spreads from your groin as " + num2Text(cocks) + " of your " + player.multiCockDescriptLight() + " grow permanently longer, by at least an inch, and leak plenty of pre-cum from the pleasure of the change.", false);
        }
    }
    if (amount >= 3) {
        if (this.cocks.length == 1)
            outputText("Your " + this.cockDescript(0) + " feels incredibly tight as a few more inches of length seem to pour out from your crotch.", false);
        if (this.cocks.length > 1) {
            if (ncocks == 1)
                outputText("Your " + this.multiCockDescriptLight() + " feel incredibly tight as one of their number begins to grow inch after inch of length.", false);
            if (ncocks > 1 && ncocks < this.cocks.length)
                outputText("Your " + this.multiCockDescriptLight() + " feel incredibly number as " + num2Text(ncocks) + " of them begin to grow inch after inch of added length.", false);
            if (ncocks == this.cocks.length)
                outputText("Your " + this.multiCockDescriptLight() + " feel incredibly tight as inch after inch of length pour out from your groin.", false);
        }
    }
    ...
    if (amount > 0) {
        if (this.cocks[0].cockLength >= 8 && this.cocks[0].cockLength - amount < 8) {
            if (this.cocks.length == 1)
                outputText("  <b>Most men would be overly proud to have a tool as long as yours.</b>", false);
            if (this.cocks.length > 1)
                outputText("  <b>Most men would be overly proud to have one cock as long as yours, let alone " + this.multiCockDescript() + ".</b>", false);
        }
        if (this.cocks[0].cockLength >= 12 && this.cocks[0].cockLength - amount < 12) {
            if (this.cocks.length == 1)
                outputText("  <b>Your " + this.cockDescript(0) + " is so long it nearly swings to your knee at its full length.</b>", false);
            if (this.cocks.length > 1)
                outputText("  <b>Your " + this.multiCockDescriptLight() + " are so long they nearly reach your knees when at full length.</b>", false);
        }
        if (this.cocks[0].cockLength >= 16 && this.cocks[0].cockLength - amount < 16) {
            if (this.cocks.length == 1)
                outputText("  <b>Your " + this.cockDescript(0) + " would look more at home on a large horse than you.</b>", false);
            if (this.cocks.length > 1)
                outputText("  <b>Your " + this.multiCockDescriptLight() + " would look more at home on a large horse than on your body.</b>", false);
            if (this.biggestTitSize() >= BREAST_CUP_C) {
                if (this.cocks.length == 1)
                    outputText("  You could easily stuff your " + this.cockDescript(0) + " between your breasts and give yourself the titty-fuck of a lifetime.", false);
                if (this.cocks.length > 1)
                    outputText("  They reach so far up your chest it would be easy to stuff a few cocks between your breasts and give yourself the titty-fuck of a lifetime.", false);
            }
            else {
                if (this.cocks.length == 1)
                    outputText("  Your " + this.cockDescript(0) + " is so long it easily reaches your chest.  The possibility of autofellatio is now a foregone conclusion.", false);
                if (this.cocks.length > 1)
                    outputText("  Your " + this.multiCockDescriptLight() + " are so long they easily reach your chest.  Autofellatio would be about as hard as looking down.", false);
            }
        }
        if (this.cocks[0].cockLength >= 20 && this.cocks[0].cockLength - amount < 20) {
            if (this.cocks.length == 1)
                outputText("  <b>As if the pulsing heat of your " + this.cockDescript(0) + " wasn't enough, the tip of your " + this.cockDescript(0) + " keeps poking its way into your view every time you get hard.</b>", false);
            if (this.cocks.length > 1)
                outputText("  <b>As if the pulsing heat of your " + this.multiCockDescriptLight() + " wasn't bad enough, every time you get hard, the tips of your " + this.multiCockDescriptLight() + " wave before you, obscuring the lower portions of your vision.</b>", false);
            if (this.cor > 40 && this.cor <= 60) {
                if (this.cocks.length > 1)
                    outputText("  You wonder if there is a demon or beast out there that could take the full length of one of your " + this.multiCockDescriptLight() + "?", false);
                if (this.cocks.length == 1)
                    outputText("  You wonder if there is a demon or beast out there that could handle your full length.", false);
            }
            ...
            if (this.cor > 80) {
                if (this.cocks.length > 1)
                    outputText("  You find yourself fantasizing about impaling nubile young champions on your " + this.multiCockDescriptLight() + " in a year's time.", false);
            }
        }
    }
    ...
    if (amount < 0 && amount >= -1) {
        if (this.cocks.length == 1)
            outputText("Your " + this.multiCockDescriptLight() + " has shrunk to a slightly shorter length.", false);
        if (this.cocks.length > 1) {
            if (ncocks == this.cocks.length)
                outputText("Your " + this.multiCockDescriptLight() + " have shrunk to a slightly shorter length.", false);
            if (ncocks > 1 && ncocks < this.cocks.length)
                outputText("You feel " + num2Text(ncocks) + " of your " + this.multiCockDescriptLight() + " have shrunk to a slightly shorter length.", false);
            if (ncocks == 1)
                outputText("You feel " + num2Text(ncocks) + " of your " + this.multiCockDescriptLight() + " has shrunk to a slightly shorter length.", false);
        }
    }
    if (amount < -1 && amount > -3) {
        if (this.cocks.length == 1)
            outputText("Your " + this.multiCockDescriptLight() + " shrinks smaller, flesh vanishing into your groin.", false);
        if (this.cocks.length > 1) {
            if (ncocks == this.cocks.length)
                outputText("Your " + this.multiCockDescriptLight() + " shrink smaller, the flesh vanishing into your groin.", false);
            if (ncocks == 1)
                outputText("You feel " + num2Text(ncocks) + " of your " + this.multiCockDescriptLight() + " shrink smaller, the flesh vanishing into your groin.", false);
            if (ncocks > 1 && ncocks < this.cocks.length)
                outputText("You feel " + num2Text(ncocks) + " of your " + this.multiCockDescriptLight() + " shrink smaller, the flesh vanishing into your groin.", false);
        }
    }
    if (amount <= -3) {
        if (this.cocks.length == 1)
            outputText("A large portion of your " + this.multiCockDescriptLight() + "'s length shrinks and vanishes.", false);
        if (this.cocks.length > 1) {
            if (ncocks == this.cocks.length)
                outputText("A large portion of your " + this.multiCockDescriptLight() + " receeds towards your groin, receding rapidly in length.", false);
            if (ncocks == 1)
                outputText("A single member of your " + this.multiCockDescriptLight() + " vanishes into your groin, receding rapidly in length.", false);
            if (ncocks > 1 && this.cocks.length > ncocks)
                outputText("Your " + this.multiCockDescriptLight() + " tingles as " + num2Text(ncocks) + " of your members vanish into your groin, receding rapidly in length.", false);
        }
    }
};
...
Player.prototype.armorDescript = function (nakedText) {
    ...
    if (this.armor.equipmentName != "naked")
        textArray.push(this.armor.name);
...
Player.prototype.clearStatuses = function () {
    ...
    if (this.findStatusEffect(StatusEffects.Shielding) >= 0)
        this.removeStatusEffect(StatusEffects.Shielding);
    if (this.findStatusEffect(StatusEffects.HolliConstrict) >= 0)
        this.removeStatusEffect(StatusEffects.HolliConstrict);
    if (this.findStatusEffect(StatusEffects.LustStones) >= 0)
        this.removeStatusEffect(StatusEffects.LustStones);
    if (monster.findStatusEffect(StatusEffects.Sandstorm) >= 0)
        monster.removeStatusEffect(StatusEffects.Sandstorm);
    if (this.findStatusEffect(StatusEffects.Sealed) >= 0)
        this.removeStatusEffect(StatusEffects.Sealed);
    if (this.findStatusEffect(StatusEffects.Berzerking) >= 0)
        this.removeStatusEffect(StatusEffects.Berzerking);
    if (monster.findStatusEffect(StatusEffects.TailWhip) >= 0)
        monster.removeStatusEffect(StatusEffects.TailWhip);
    if (this.findStatusEffect(StatusEffects.UBERWEB) >= 0)
        this.removeStatusEffect(StatusEffects.UBERWEB);
    if (this.findStatusEffect(StatusEffects.DriderKiss) >= 0)
        this.removeStatusEffect(StatusEffects.DriderKiss);
    if (this.findStatusEffect(StatusEffects.WebSilence) >= 0)
        this.removeStatusEffect(StatusEffects.WebSilence);
    if (this.findStatusEffect(StatusEffects.GooArmorSilence) >= 0)
        this.removeStatusEffect(StatusEffects.GooArmorSilence);
    if (this.findStatusEffect(StatusEffects.Whispered) >= 0)
        this.removeStatusEffect(StatusEffects.Whispered);
    ...
    while (this.findStatusEffect(StatusEffects.Blind) >= 0)
        this.removeStatusEffect(StatusEffects.Blind);
    if (this.findStatusEffect(StatusEffects.SheilaOil) >= 0)
        this.removeStatusEffect(StatusEffects.SheilaOil);
    ...
    if (this.findStatusEffect(StatusEffects.Bind) >= 0)
        this.removeStatusEffect(StatusEffects.Bind);
    ...
    if (this.findStatusEffect(StatusEffects.Silence) >= 0)
        this.removeStatusEffect(StatusEffects.Silence);
    if (this.findStatusEffect(StatusEffects.StoneLust) >= 0)
        this.removeStatusEffect(StatusEffects.StoneLust);
    this.removeStatusEffect(StatusEffects.FirstAttack);
    if (this.findStatusEffect(StatusEffects.TemporaryHeat) >= 0)
        this.removeStatusEffect(StatusEffects.TemporaryHeat);
    if (this.findStatusEffect(StatusEffects.NoFlee) >= 0)
        this.removeStatusEffect(StatusEffects.NoFlee);
    if (this.findStatusEffect(StatusEffects.Poison) >= 0)
        this.removeStatusEffect(StatusEffects.Poison);
    if (this.findStatusEffect(StatusEffects.IsabellaStunned) >= 0)
        this.removeStatusEffect(StatusEffects.IsabellaStunned);
    if (this.findStatusEffect(StatusEffects.Stunned) >= 0)
        this.removeStatusEffect(StatusEffects.Stunned);
    if (this.findStatusEffect(StatusEffects.Confusion) >= 0)
        this.removeStatusEffect(StatusEffects.Confusion);
    if (this.findStatusEffect(StatusEffects.ThroatPunch) >= 0)
        this.removeStatusEffect(StatusEffects.ThroatPunch);
    if (this.findStatusEffect(StatusEffects.KissOfDeath) >= 0)
        this.removeStatusEffect(StatusEffects.KissOfDeath);
    if (this.findStatusEffect(StatusEffects.AcidSlap) >= 0)
        this.removeStatusEffect(StatusEffects.AcidSlap);
    ...
    if (this.findStatusEffect(StatusEffects.DemonSeed) >= 0)
        this.removeStatusEffect(StatusEffects.DemonSeed);
    if (this.findStatusEffect(StatusEffects.InfestAttempted) >= 0)
        this.removeStatusEffect(StatusEffects.InfestAttempted);
    ...
    if (this.findStatusEffect(StatusEffects.ChargeWeapon) >= 0)
        this.removeStatusEffect(StatusEffects.ChargeWeapon);
    ...
    if (this.findStatusEffect(StatusEffects.BasiliskCompulsion) >= 0)
        this.removeStatusEffect(StatusEffects.BasiliskCompulsion);
    if (this.findStatusEffect(StatusEffects.BasiliskSlow) >= 0) {
        this.modStats("spe", this.statusEffectValue(StatusEffects.AnemoneVenom, 1));
        this.removeStatusEffect(StatusEffects.BasiliskSlow);
    }
    if (this.findStatusEffect(StatusEffects.GiantGrabbed) >= 0)
        this.removeStatusEffect(StatusEffects.GiantGrabbed);
    if (this.findStatusEffect(StatusEffects.GiantBoulder) >= 0)
        this.removeStatusEffect(StatusEffects.GiantBoulder);
    ...
    while (this.findStatusEffect(StatusEffects.IzmaBleed) >= 0)
        this.removeStatusEffect(StatusEffects.IzmaBleed);
    ...
    if (this.findStatusEffect(StatusEffects.KnockedBack) >= 0)
        this.removeStatusEffect(StatusEffects.KnockedBack);
    if (this.findStatusEffect(StatusEffects.RemovedArmor) >= 0)
        this.removeStatusEffect(StatusEffects.RemovedArmor);
    if (this.findStatusEffect(StatusEffects.JCLustLevel) >= 0)
        this.removeStatusEffect(StatusEffects.JCLustLevel);
    if (this.findStatusEffect(StatusEffects.MirroredAttack) >= 0)
        this.removeStatusEffect(StatusEffects.MirroredAttack);
    if (this.findStatusEffect(StatusEffects.Tentagrappled) >= 0)
        this.removeStatusEffect(StatusEffects.Tentagrappled);
    if (this.findStatusEffect(StatusEffects.TentagrappleCooldown) >= 0)
        this.removeStatusEffect(StatusEffects.TentagrappleCooldown);
    if (this.findStatusEffect(StatusEffects.ShowerDotEffect) >= 0)
        this.removeStatusEffect(StatusEffects.ShowerDotEffect);
    if (this.findStatusEffect(StatusEffects.VineHealUsed) >= 0)
...
Player.prototype.itemCount = function (itype) {
    ...
    for (var i = 0; i < this.itemSlots.length; i++) {
        if (this.itemSlots[i].itype == itype)
            count += this.itemSlots[i].quantity;
    }
    return count;
};
...
Player.prototype.emptySlot = function () {
    for (var i = 0; i < this.itemSlots.length; i++) {
        if ((this.itemSlots[i].itype == undefined || this.itemSlots[i].itype == Items.NOTHING) && i < this.getMaxSlots())
            return i;
    }
    return -1;
};
...
Player.prototype.changeXP = function (amount) {
    player.XP += amount;
    if (player.XP < 0)
        player.XP = 0; //Keep from going into negative.
    if (player.XP > 9999)
        player.XP = 9999;
    refreshStats();
};
Player.prototype.changeGems = function (amount) {
    player.gems += amount;
    if (player.gems < 0)
        player.gems = 0; //Keep from going into negative.
    if (player.gems > Number.MAX_VALUE)
        player.gems = Number.MAX_VALUE;
    refreshStats();
};
//-----------
// NEW GAME PLUS
