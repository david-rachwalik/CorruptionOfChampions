...
class Player extends Creature {
    ...
    sharkScore() {
        ...
        if (this.tailType == ENUM.TailType.TAIL_TYPE_SHARK)
            sharkCounter++;
        if (this.skinType == ENUM.SkinType.SKIN_TYPE_PLAIN && (this.skinTone == "rough gray" || this.skinTone == "orange and black striped"))
            sharkCounter++;
        return sharkCounter;
    }
    ...
    echidnaScore() {
        ...
        if (this.earType == ENUM.EarType.EARS_ECHIDNA)
            echidnaCounter++;
        if (this.tailType == ENUM.TailType.TAIL_TYPE_ECHIDNA)
            echidnaCounter++;
        if (this.faceType == ENUM.FaceType.FACE_ECHIDNA)
            echidnaCounter++;
        if (this.tongueType == ENUM.TongueType.TONGUE_ECHIDNA)
            echidnaCounter++;
        if (this.lowerBody == ENUM.LowerBodyType.LOWER_BODY_TYPE_ECHIDNA)
            echidnaCounter++;
        if (echidnaCounter >= 2 && this.skinType == ENUM.SkinType.SKIN_TYPE_FUR)
            echidnaCounter++;
    ...
    lengthChange(amount, ncocks) {
        ...
        if (amount <= 1 && amount > 0) {
            if (this.cocks.length == 1)
                GUI.outputText("Your " + this.cockDescript(0) + " has grown slightly longer.");
            if (this.cocks.length > 1) {
                if (ncocks == 1)
                    GUI.outputText("One of your " + this.multiCockDescriptLight() + " grows slightly longer.");
                if (ncocks > 1 && ncocks < this.cocks.length)
                    GUI.outputText("Some of your " + this.multiCockDescriptLight() + " grow slightly longer.");
                if (ncocks == this.cocks.length)
                    GUI.outputText("Your " + this.multiCockDescriptLight() + " seem to fill up... growing a little bit larger.");
            }
        }
        if (amount > 1 && amount < 3) {
            if (this.cocks.length == 1)
                GUI.outputText("A very pleasurable feeling spreads from your groin as your " + this.cockDescript(0) + " grows permanently longer - at least an inch - and leaks pre-cum from the pleasure of the change.");
            if (this.cocks.length > 1) {
                if (ncocks == this.cocks.length)
    ...
    armorDescript(nakedText = "gear") {
        ...
        if (this.armor.equipmentName != "naked")
            textArray.push(this.armor.equipmentName);
    ...
    clearStatuses() {
        ...
        if (liveData.monster.findStatusEffect(StatusEffects.Sandstorm) >= 0)
            liveData.monster.removeStatusEffect(StatusEffects.Sandstorm);
        ...
        if (liveData.monster.findStatusEffect(StatusEffects.TailWhip) >= 0)
            liveData.monster.removeStatusEffect(StatusEffects.TailWhip);
    ...
    changeXP(amount) {
        this.XP += amount;
        if (this.XP < 0)
            this.XP = 0; //Keep from going into negative.
        if (this.XP > 9999)
            this.XP = 9999;
        GUI.refreshStats();
    }
    changeGems(amount) {
        this.gems += amount;
        if (this.gems < 0)
            this.gems = 0; //Keep from going into negative.
        if (this.gems > Number.MAX_VALUE)
            this.gems = Number.MAX_VALUE;
        GUI.refreshStats();
    }
    //-----------
    // NEW GAME PLUS
    ...
    multiCockDescript() {
        ...
        while (currCock <= totCock - 1) {
            ...
            if (same && currCock > 0 && this.cocks[currCock].cockType != this.cocks[currCock - 1].cockType)
                same = false;
            currCock++;
        }
        //Crunch averages
        ...
        if (currCock == 1) {
            if (dogCocks == 1)
                return Appearance.cockNoun(ENUM.CockType.DOG);
            if (horseCocks == 1)
                return Appearance.cockNoun(ENUM.CockType.HORSE);
            if (normalCocks == 1)
                return this.cockDescript(0);
        ...
        if (currCock == 3) {
            ...
            if (same) {
                ...
                //Tentacles
                if (this.cocks[0].cockType > 2)
                    descript += ", " + Appearance.cockNoun(this.cocks[0].cockType) + "s"; // Not sure what's going on here, referencing index *may* be a bug.
            }
            else {
    ...
    cockAdjectives(i_cockLength, i_cockThickness, i_cockType) {
        let description = "";
        let rando = 0;
        let descripts = 0;
