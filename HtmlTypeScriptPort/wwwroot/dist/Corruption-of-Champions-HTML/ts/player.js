    ...
    lengthChange(amount, ncocks) {
        ...
        if (amount <= 1 && amount > 0) {
            if (this.cocks.length == 1)
                outputText("Your " + player.cockDescript(0) + " has grown slightly longer.");
            if (this.cocks.length > 1) {
                if (ncocks == 1)
                    outputText("One of your " + player.multiCockDescriptLight() + " grows slightly longer.");
                if (ncocks > 1 && ncocks < this.cocks.length)
                    outputText("Some of your " + player.multiCockDescriptLight() + " grow slightly longer.");
                if (ncocks == this.cocks.length)
                    outputText("Your " + player.multiCockDescriptLight() + " seem to fill up... growing a little bit larger.");
            }
        }
        if (amount > 1 && amount < 3) {
            if (this.cocks.length == 1)
                outputText("A very pleasurable feeling spreads from your groin as your " + player.cockDescript(0) + " grows permanently longer - at least an inch - and leaks pre-cum from the pleasure of the change.");
            if (this.cocks.length > 1) {
                if (ncocks == this.cocks.length)
            ...
            if (this.cocks[0].cockLength >= 20 && this.cocks[0].cockLength - amount < 20) {
                ...
                if (this.cocks.length > 1)
                    outputText("  <b>As if the pulsing heat of your " +
                        ...
                        this.multiCockDescriptLight() +
                        " wave before you, obscuring the lower portions of your vision.</b>", false);
