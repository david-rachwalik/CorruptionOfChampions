...
class Appearance {
    ...
    static hairDescription(i_creature) {
        ...
        if (i_creature.skinType == 1 && i_creature.hairLength > 3 && UTIL.rand(2) == 0) {
            if (i_creature.hairType == ENUM.HairType.HAIR_FEATHER)
                description += "feather-";
            else if (i_creature.hairType == ENUM.HairType.HAIR_GHOST)
                description += "transparent ";
            else if (i_creature.hairType == ENUM.HairType.HAIR_GOO)
                description += "goo-";
            else if (i_creature.hairType == ENUM.HairType.HAIR_ANEMONE)
                description += "tentacle-";
            else if (i_creature.hairType == ENUM.HairType.HAIR_QUILL)
                description += "quill-";
            description += "mane";
            return description;
        }
    ...
    static beardDescription(i_creature) {
        let description = "";
        let options;
        //
        // LENGTH ADJECTIVE!
    ...
    static hipDescription(i_character) {
        ...
        //Taurs
        if (i_character.isTaur() && UTIL.rand(3) == 0)
            description += "flanks";
        //Nagas have sides, right?
        else if (i_character.isNaga() && UTIL.rand(3) == 0)
            description += "sides";
    ...
    static cockAdjective(cockType, length, girth, lust, cumQ, isPierced, hasSock, isGooey) {
        ...
        if (isGooey && UTIL.rand(4) == 0)
            return UTIL.randomChoice("goopey", "gooey", "slimy");
        //Length 1/3 chance
        if (UTIL.rand(3) == 0) {
            if (length < 3)
                return UTIL.randomChoice("little", "toy-sized", "mini", "budding", "tiny");
            if (length < 5)
                return UTIL.randomChoice("short", "small");
            if (length < 7)
        ...
        //Hornyness 1/2
            ...
            if (lust > 90) {
                ...
                if (cumQ < 50)
                    return UTIL.randomChoice("throbbing", "pulsating"); //Weak as shit cum
                if (cumQ < 200)
                    return UTIL.randomChoice("dribbling", "leaking", "drooling"); //lots of cum? drippy.
                ...
                if (cumQ < 50)
                    return UTIL.randomChoice("turgid", "blood-engorged", "rock-hard", "stiff", "eager"); //Weak as shit cum
                if (cumQ < 200)
                    return UTIL.randomChoice("turgid", "blood-engorged", "rock-hard", "stiff", "eager", "fluid-beading", "slowly-oozing"); //A little drippy
                return UTIL.randomChoice("dribbling", "drooling", "fluid-leaking", "leaking"); //uber drippy
            }
        }
        //Girth - fallback
        if (girth <= 0.75)
            return UTIL.randomChoice("thin", "slender", "narrow");
        if (girth <= 1.2)
            return "ample";
        if (girth <= 1.4)
            return UTIL.randomChoice("ample", "big");
        if (girth <= 2)
            return UTIL.randomChoice("broad", "meaty", "girthy");
        if (girth <= 3.5)
    ...
    static vaginaDescript(i_creature, i_vaginaIndex = 0, forceDesc = false) {
        //Main function
    ...
    static buttDescription(i_character) {
        ...
        if (i_character.buttRating <= 1) {
            if (i_character.tone >= 60)
                ...
                description = UTIL.randomChoice(options);
                //Soft PC's buns!
                if (i_character.tone <= 30 && UTIL.rand(3) == 0)
    ...
    static assholeDescript(i_creature, forceDesc = false) {
        //Main function
        let description = "";
    ...
    static biggestBreastSizeDescript(creature) {
        ...
        if (UTIL.rand(2) == 0)
            descript += this.breastSize(creature.breastRows[temp142].breastRating);
        //Nouns!
        temp14 = UTIL.rand(10);
        if (temp14 == 0)
            descript += "breasts";
