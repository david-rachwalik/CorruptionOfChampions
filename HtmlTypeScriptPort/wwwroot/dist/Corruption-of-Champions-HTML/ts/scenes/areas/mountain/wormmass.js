    ...
    wormEntice() {
        ...
        if (UTIL.rand(2) == 0) {
            if (liveData.player.lust < 50)
                GUI.outputText("The worm colony stands before you and begins secreting a significant amount of slime. You are perplexed as to why the worms have done this. You shrug your shoulders and remain on guard.<br>");
            else
                GUI.outputText("The worm colony shambles over to you and attempts to grapple you. Quickly sidestepping the clumsy movements of the creature, you avoid what could have been a horrible fate as the mass falls over and splatters in its failed attempt to engulf you.<br>");
            return;
        }
        //SUCCESS
        if (liveData.player.lust < 50) {
            GUI.outputText("The worm colony stands before you and begins secreting a significant amount of slime. Inexplicably, you find that your " +
...
class WormsScene {
    ...
    static infest1() {
        ...
        if (liveData.gameFlags[FLAG.EVER_INFESTED] == 0)
            liveData.gameFlags[FLAG.EVER_INFESTED] = 1;
    ...
    static wormToggle() {
        //TODO Sprite spriteSelect(76);
        GUI.clearOutput();
        GUI.outputText("While wandering, you come across a crudely illustrated sign.  It depicts an imp in obvious discomfort, covered in white little worms.  It looks as if one of them is even crawling into the imp's penis!\n\nHow do you feel about that?");
    ...
    static wormsRun() {
        ...
        if (liveData.player.spe > UTIL.rand(35)) {
            GUI.outputText("Your instincts overwhelm you and you immediately turn around and run like hell in the opposite direction. You look behind you as your heart feels as if it is about to burst only to discover that the creature did not follow you. You take a moment to catch your breath and consider yourself fortunate.");
            GUI.doNext(Camp.returnToCampUseOneHour);
        }
        else {
