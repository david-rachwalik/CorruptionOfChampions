...
function Minotaur(axe) {
    ...
    this.createCock(rand(13) + 24, 2 + rand(3), ENUM.CockType.HORSE);
    this.balls = 2;
...
MinotaurScene.joinBeingAMinoCumSlut = function () {
    ...
    if (player.tallness <= 96)
        outputText(" despite your immense height");
    ...
    if (player.hasVagina())
        outputText("swirls around your clitoris");
    else
        outputText("presses against your perineum");
    outputText(".");
    ...
    if (player.hasCock())
        outputText(", once again ignoring your manhood completely");
    outputText(", dives tongue first into your wet ");
    if (player.hasVagina())
        outputText("quim");
    else
        outputText("back door");
    ...
    if (player.hasVagina())
        outputText("sodden box");
    else
        outputText("moist butthole");
    ...
    if (player.hasVagina())
        outputText("within your womb, knocked up by this manly beast");
    else
        outputText("within your bowels");
    ...
    player.orgasm();
    player.modStats("lib", 0.5, "sen", -3, "cor", 1);
...
MinotaurScene.minoVictoryRapeChoices = function () {
    ...
    if (player.lust >= 33) {
        ...
        if (player.hasCock() && x >= 0)
            addButton(0, "Use Cock", MinotaurScene.bumRapeaMinotaur);
        // Checking for vaginal fuck
        if (player.hasVagina())
            addButton(1, "Use Vagina", MinotaurScene.girlRapeAMinotaur);
        // Checking for herm scene. Centaurs can't do the herm scene
        if (player.gender == 3 && x >= 0 && !player.isTaur())
            addButton(2, "Use Both", MinotaurScene.minotaurGetsRapedByHerms);
        // Checking for tentacle fuck
        if (x >= 0 && player.cockTotal() > 1 && player.countCocksOfType(ENUM.CockType.TENTACLE) > 0)
            addButton(3, "TentacleDick", MinotaurScene.rapeMinotaurTentacles);
        ...
        if (player.hasCock()) {
            if (player.cockThatFits(80) >= 0)
                addButton(7, "MakeHimSuck", MinotaurScene.minotaurBlowjob);
        }
    }
    // Non lusty options
    // Are you an addict?
    ...
    if (player.findPerk(PerkLib.Feeder) >= 0)
        addButton(8, "Breastfeed", MinotaurScene.minotaurDrinksMilkNewsAtEleven);
    ...
    //Not an addict
        ...
        if (monster.lust >= monster.maxLust())
            outputText("You smile in satisfaction as the " +
                monster.name +
                " drops down on all fours and begins masturbating feverishly.  Sadly you realize your own needs have not been met.  Of course you could always fuck the eager bull...<br><br>What do you do?", true);
        else
            outputText("You smile in satisfaction as the " + monster.name + " collapses, unable to continue fighting.  Sadly you realize your own needs have not been met.  Of course you could always rape the poor thing...<br><br>What do you do?", true);
    }
    //Not able to rape but a feeder
    else if (player.findStatusEffect(StatusEffects.Feeder) >= 0 && gameFlags[SFW_MODE] == 0) {
        if (monster.lust >= monster.maxLust())
            outputText("You smile in satisfaction as the " + monster.name + " collapses, unable to continue fighting.  Sadly you realize your own need to breastfeed has not been met.  You could always let the poor thing have a drink...<br><br>Do you?", true);
        else
            outputText("You smile in satisfaction as the " +
                monster.name +
                " collapses, unable to continue fighting.  Sadly you realize your own need to breastfeed has not been met.  You could always let the poor thing have a drink...<br><br>What do you do?", true);
    }
    //No rape, no feeder
    else {
        if (monster.lust >= monster.maxLust())
            outputText("You smile in satisfaction as the " + monster.name + " drops down on all fours and begins masturbating feverishly.", true);
        else
            outputText("You smile in satisfaction as the " + monster.name + " collapses, unable to continue fighting.", true);
        cleanupAfterCombat();
    }
    if (x < 0 && player.hasCock())
        outputText("<br>Sadly, you're too well endowed to penetrate the minotaur.", false);
    if (player.gender == 3 && player.isTaur())
        outputText("<br>If you had a different body type you might be able to penetrate him while taking him, but as a centaur that's not an option.", false);
};
...
MinotaurScene.minoRapeIntro = function () {
    ...
    if (monster.HP <= 0) {
        ...
        if (player.vaginas.length > 0)
            outputText("  Glancing back at you with hope in his eyes, the minotaur's " + monster.cockDescriptShort(0) + " twitches as he lays his gaze upon your " + player.vaginaDescript(0) + ".", false);
        else
            outputText("  Glancing back at you with eyes full of despair, the minotaur seems to realize you won't be helping the predicament his " + monster.ballsDescriptLight() + " are in.", false);
    }
    //Monster lost to lust
    else {
        outputText("You rip off the minotaur's loincloth with a savage grin, slapping his ass and admiring the jiggle of his " +
            monster.ballsDescriptLight() +
            " as they wobble from the force of your blow.  The cow-man moos softly with desire, ropes of pre splattering the ground from his arousal.   The musk pouring off him and his puddle of slick pre-cum is overpowering, making it difficult to think as you feel your blood rush to your groin ", false);
        //NIPPULAR ENJOYMENT
        if (player.totalNipples() > 1)
            outputText(" and nipples", false);
        outputText(".", false);
        //Vagina or not flavor texts
        if (player.vaginas.length > 0)
            outputText("  Glancing back at you with hope in its eyes, the minotaur's " + monster.cockDescriptShort(0) + " twitches as he lays his gaze upon your " + player.vaginaDescript(0) + ".", false);
        else
            outputText("  Glancing back at you with eyes full of despair, the minotaur doubts you'll help relieve the predicament his " + monster.ballsDescriptLight() + " are in.", false);
    }
};
//Male Rape
MinotaurScene.bumRapeaMinotaur = function () {
    ...
    if (player.cumQ() < 25)
        outputText("dribbling a bit of slick pre-cum into his fur.  ", false);
    if (player.cumQ() >= 25 && player.cumQ() < 250)
        outputText("leaking thin ropes of pre-cum over his muscular ass.  ", false);
    ...
    outputText("With little warning, you push inside, amazed to feel his sphincter relax completely.  Pushing forward with ease, you slide all " +
        num2Text(Math.round(player.cocks[0].cockLength)) +
        ...
        player.cockDescript(x) +
        " and the intoxicating musk filling your head, you lose control, your hips pistoning rapidly of their own accord, pumping harder and harder with every thrust, exhausting your body as you overexert yourself.<br><br>", false);
    ...
    if (player.cumQ() <= 25) {
        outputText("With long deep strokes you bury your " +
            player.cockDescript(x) +
            " inside him, jizz spurting into his backside.  The bull-man moans in helpless pleasure and pain as he continues jerking himself, unable to cum.  Eventually you pull away with a satisfied sigh.  The minotaur is a mess, still masturbating furiously and helpless with desire to orgasm.", false);
    }
    if (player.cumQ() > 25 && player.cumQ() <= 250) {
        outputText("With long deep strokes you bury your " +
            player.cockDescript(x) +
            ...
            player.cockDescript(x) +
            " as you finish pumping a few long last blasts into him.  The minotaur is a mess, masturbating furiously and dripping with pre-cum, on the edge of orgasm.  It looks like he might even manage to get himself off with enough time.", false);
    }
    if (player.cumQ() > 250 && player.cumQ() <= 500) {
        outputText("With long deep strokes you bury your " +
            player.cockDescript(x) +
            ...
            monster.ballsDescriptLight() +
            " empty, the strong scent making you swoon dizzily, euphoric with the minotaur's drug-like cum-musk and your own orgasm.<br><br>Eventually you pull free, giggling in pleasure as your victim falls unconscious in a small lake of his own spooge. ", false);
...
MinotaurScene.girlRapeAMinotaur = function () {
    ...
    outputText("<br><br>With a quick shove, you roll the panting minotaur onto his back, exposing his pulsing " +
        monster.cockDescriptShort(0) +
        " to the open air.  A simple flourish later, you peel back your " +
        player.armorName +
        " to expose your " +
        player.vaginaDescript(0) +
        ", already wetter than usual from the scent in the air.   You walk forward, straddling the oblivious minotaur as he masturbates furiously, muscles bulging with exertion and covered in sweat.  Your eyes zero in as a dollop of pre escapes his ribbed horse-like member, locating the source of the scent.<br><br>", false);
    //Too small
    if (player.vaginalCapacity() < monster.cockArea(0)) {
        outputText("Restraint evaporates as you lower yourself, your aching " +
            player.vaginaDescript(0) +
            ...
            player.vaginaDescript(0) +
            " roughly.  The need to be speared on the beast's meat is overwhelming urged on by constantly leaking pre-cum currently pumping into your love-canal.  With strength born of unnatural lust, you grip his flanks and drive yourself downwards, feeling yourself stretch painfully as you take in his length.  ", false);
    }
    //Good match
    ...
    if (player.vaginalCapacity() >= monster.cockArea(0) * 1.5) {
        outputText("Restraint evaporates as you lower yourself, your aching " +
            ...
            player.vaginaDescript(0) +
            ", allowing him to drool his musky pre-seed deep inside your wide love-canal.  Sinking down slowly, your easily encompass all of his equine-like member, enjoying the feeling of his three thick cock-ridges as they slip through your lips.  Eventually you take all of him, wishing he was just a bit bigger...", false);
    }
    //Resume sexings, p2
    outputText("<br><br>At last you feel the minotaur's meaty hands grab you by the ", false);
    if (player.isTaur())
        outputText("hindquarters, forcefully grinding your vagina along his length like a toy.  ", false);
    else
        outputText(player.hipDescript() + ", forcefully grinding up and down along his length like a cheap sex-toy.  ", false);
    //Size variants
    //Too small
    if (player.vaginalCapacity() < monster.cockArea(0))
        outputText("Every ridge of his " + monster.cockDescriptShort(0) + " presses tightly against your walls, smearing more of his wondrous pre-seed into your body with every stroke.  ", false);
    //Just right
    if (player.vaginalCapacity() >= monster.cockArea(0) && player.vaginalCapacity() * 1.5 < monster.cockArea(0))
        outputText("Every stroke of his " + monster.cockDescriptShort(0) + " deposits a bit more of his wondrous preseed into your body, spreading a relaxing warmth throughout your groin.  ", false);
    //Too big
    if (player.vaginalCapacity() * 1.5 >= monster.cockArea(0))
        outputText("Every stroke of his " + monster.cockDescriptShort(0) + " slides easily into your " + player.vaginaDescript(0) + ", allowing him to slowly build up a puddle of his wondrous druggy pre-seed into your aching womb.  ", false);
    //Resume!
    outputText("The minotaur picks up the pace, heartbeat thudding through his rigid meat, making it convulse and vibrate as he slams you down on it over and over again.  You feel his balls twitching against your ass as his cock visibly thickens, bulging out at the base and slowly fattening inside you until it feels fit to burst.  With a throaty roar he cums at last, fountaining seed directly into your womb.  Your eyes roll back as it fills you, swooning as you feel the liquid pleasure seeping into you.  Your " +
        ...
        monster.cockDescriptShort(0) +
        ", just as you both reach total sexual satisfaction.<br><br>", false);
    //Check for stetchiness!
    player.cuntChange(monster.cockArea(0), true);
    //ITS OVER
    outputText("He passes out with a contented sigh, balls shrunk down to the size of baseballs and his cock still dribbling cum.  You pull yourself to your feet unsteadily, drunk with pleasure as you stagger away, clamping your " +
        player.vaginaDescript(0) +
        " down unconsciously to hold in his wonderful seed.", false);
    if (player.vaginalCapacity() < monster.cockArea(0))
        outputText("<br><br>Mixed with the sheer sexual pleasure, you feel a diffuse pain and numbness coming from your much-abused " +
            player.vaginaDescript(0) +
            ", but decide with a smile that it was well worth it as his cum numbs away the last of the pain.", false);
    player.dynStats("spe", -0.5, "int", -0.5, "sen", 1.5, "cor", 1);
    //Preggers chance!
...
MinotaurScene.minotaurGetsRapedByHerms = function () {
    ...
    player.minoCumAddiction(5);
    player.slimeFeed();
    outputText("<br><br>With little care for the beast's feelings, you grip his cock with both hands, pulling it towards yourself, bending it up at an awkward angle that makes him give a trembling moo of pain.   You edge closer, your own " +
        ...
        player.vaginaDescript(0) +
        ", smearing it against your cunt-lips.<br><br>", false);
    outputText("With a twisted grin and a grunt of pain, you push forward, slipping his " +
        monster.cockDescriptShort(0) +
        " into your " +
        player.vaginaDescript(0) +
        " as your " +
    ...
    if (player.vaginas[0].vaginalLooseness < VAGINA_LOOSENESS_GAPING)
        outputText("A small suppressed part of your mind dimly registers that this should be fairly painful, but the lust and drug-like pre-cum wash that thought away with waves of pleasure.  ", false);
    if (player.vaginas[0].vaginalLooseness >= VAGINA_LOOSENESS_GAPING)
        outputText("The intense feelings seem to grow more and more powerful as he stays inside you, his drug-like pre-cum making your " + player.vaginaDescript(0) + " tingle from the inside-out.", false);
    outputText("Your " +
        player.cockDescript(0) +
...
MinotaurScene.rapeMinotaurTentacles = function () {
    ...
    if (player.cocks.length > 1 && player.countCocksOfType(ENUM.CockType.TENTACLE) == 1) {
    ...
    var counter = 0;
    while (counter < player.cockTotal()) {
        if (player.cocks[counter].cockType == ENUM.CockType.TENTACLE) {
            x = counter;
            break;
        }
        counter++;
    ...
    clearOutput();
    if (monster.HP <= 0)
        outputText("The minotaur collapses backwards after your last blow. Defeated and tired, he can barely stand up as you approach.  ", false);
    else {
        outputText("The minotaur drops his arm", false);
        if (monster.weapon == "axe")
            outputText(", letting his axe clatter to the floor", false);
    ...
    if (player.cor < 25)
        outputText("You cry, petting his hair, smoothing it across his horns, justifying to yourself that you are raping this beast because this is how this world works as you maneuver your " + player.multiCockDescript() + " towards his tailhole.  ", false);
    if (player.cor >= 25 && player.cor < 50)
        outputText("You smoothly guide your " + player.multiCockDescript() + " toward his tailhole.  ", false);
    if (player.cor >= 50 && player.cor < 80)
        outputText("You roughly grab him, slamming him close to your " + player.multiCockDescript() + " - just how this beast likes it.  ", false);
    if (player.cor >= 80)
        outputText("You grin and roughly pull his horns towards your " + player.multiCockDescript() + ", ready to roughly ride this beast.  ", false);
    //Mixed dix
    if (player.cockTotal() != player.countCocksOfType(ENUM.CockType.TENTACLE)) {
        mismatched = true;
        ...
        while (temp > 0 && temp2 == -1) {
            temp--;
            if (player.cocks[temp].cockType != ENUM.CockType.TENTACLE)
                temp2 = temp;
        }
        //failsafe
        if (temp2 == -1)
        ...
        while (temp > 0) {
            ...
            if (player.cocks[temp].cockLength > player.cocks[temp2].cockLength && player.cocks[temp].cockType != ENUM.CockType.TENTACLE) {
                temp2 = temp;
            }
        }
    ...
    //Not mismatched
        ...
        temp2 = 0;
        outputText("One of your prehensile dicks coils in on itself as it nears the minotaur's tailhole, readying itself. It then suddenly springs itself into his fleshy orifice, wiggling and writhing in deeper and deeper until all " +
            player.cocks[0].cockLength +
            " inches of it have submerged itself inside his intestines. You and he both shudder as your cock starts weaving from side to side, making slithering motions inside him.  ", false);
    }
    //Vaginassss
    if (player.vaginas.length >= 1) {
        outputText("With your " +
            ...
            player.vaginaDescript(0) +
            " to take the minotaur's enormous length into yourself.  You feel his member deliciously slide in, and feel the beast's hot breath radiate as he starts panting in lust.  ", false);
        player.cuntChange(monster.cockArea(0), true, false, true);
    }
    //Multivaginas...wtf
    if (player.vaginas.length > 1 && !mismatched && player.cocks.length > 2)
        outputText("Your tentacle cocks writhe upon themselves, each turning to an unattended " +
            player.vaginaDescript(1) +
            " on your body, and quickly filling up the cavernous depths. You groan in ecstasy at the self-fucking you are receiving as all your pussies start leaking their wonderful fluids.  ", false);
    //more dicks than available holes
    if (player.countCocksOfType(ENUM.CockType.TENTACLE) == 2 || player.countCocksOfType(ENUM.CockType.TENTACLE) == 3)
    ...
    if (player.countCocksOfType(ENUM.CockType.TENTACLE) > 3)
        outputText("Your remaining tentacle dicks wave around the minotaur, tending to his balls, weaving around his limbs, and generally rubbing and throbbing all over him, spreading pre-cum around and through him, leaving both of you moaning in pleasure.  ", false);
    //Cum
...
MinotaurScene.rapeMinotaurTentacle = function () {
    ...
    outputText("The minotaur's balls begin to spasm, so you wrap the base of your tentacle around him tightly, preventing his " +
        monster.cockDescriptShort(0) +
    ...
    if (player.cocks.length > 2 && temp != 1) {
        temp = 1;
        outputText("It's not enough! Your other dicks demand attention too! Fortunately, he has a large hole for you to abuse... you stuff as many of your " +
            player.multiCockDescript() +
            " as you can, the rest hanging underneath him, slapping against his sack. ");
    }
    if (player.clitLength > 6) {
        outputText("Eager for more stimulation, you muse that your clit's big enough to fuck with, so why not!? You take your fingers, spit on them, and slide them into place, opening him up wider to accept your " +
            player.clitDescript() +
            ". It's so sensitive, sliding into that tight hole, and the sensations are driving you wild as you hump against him with your dick-like clit.  ", false);
    }
    //Free pussy!
    if (player.vaginas.length > 0) {
        temp = 2;
        outputText("You're a woman, and it's about time you got some pussy action too!  You slide your " +
            player.vaginaDescript(0) +
            " onto " +
            player.cockDescript(x) +
            "-wrapped minotaur-prick, further driving the beast mad with lust.  He plaintatively groans, struggling to get his dick free or to cum, you aren't quite sure.  ", false);
        outputText("Eventually, the moment is right.  You smile as you feel a boiling heat building in your groin, surging through your " +
            player.multiCockDescript() +
            " in a tide of white-hot pleasure.  You groan and shiver, lost in the pleasure until you begin to calm down.  ", false);
        if (player.cumQ() < 50)
            outputText("Your cock made quite a mess of things, and when you pull him out, your " + player.vaginaDescript(0) + " makes a wet sucking sound.  ", false);
        if (player.cumQ() >= 50 && player.cumQ() < 400)
            outputText("Your cock made quite a mess of things inside your " +
                player.vaginaDescript(0) +
                ".  As you pull free, cum drips freely down your thighs.  The heavenly scent of minotaur spunk fills your nose, and begin scooping the mixed jizz up to sample.  ", false);
        if (player.cumQ() >= 400)
            outputText("Your cock made quite a mess of things inside your " +
                player.vaginaDescript(0) +
                ".  As you pull free, a river of cum pours from betwixt your thighs.  The heavenly scent of minotaur spunk fills your nose, and you begin scooping the mixed jizz up to sample.  ", false);
    }
    //Free ass!
    else {
        temp = 2;
        outputText("You're feeling a little kinky so you step in front of him, offering him a little bit more, pushing yourself down onto his meaty dick.  You slowly push down, enjoying every inch as it travels down your " +
            player.assholeDescript() +
            ", the pre leaking from it acting as a lubricant as his tentacle-wrapped cock works its way inside you.  ", false);
        //Ass cum
        outputText("Eventually, the moment is right.  You smile as you feel a boiling heat building in your groin, surging through your " +
            player.multiCockDescript() +
            " in a tide of white-hot pleasure.  You groan and shiver, lost in the pleasure until you begin to calm down.  ", false);
        if (player.cumQ() < 50)
            outputText("Your cock in his ass made a mess of things, and when you pull out, it makes a wet sucking sound, trying to keep you inside.  ", false);
        if (player.cumQ() >= 50 && player.cumQ() < 400)
            outputText("Your cock in his ass spills forth, creating a puddle which will mat his fur later.  ", false);
        if (player.cumQ() >= 400)
            outputText("Your cock is blasted free from his ass by the force of your ejaculation, splattering rivers of spoo over the ground.  A river of the stickiness pours from his violated asshole, sticking in the fur of his thighs.  ", false);
    }
    //Cum time
    if (temp < 2) {
        if (player.cumQ() < 25)
...
MinotaurScene.minoUrethralPen = function () {
    ...
    //Infested version!
    if (gameFlags[INFESTED] == 5) {
        //TODO Change Infested Status Effect to GameFlag
        outputText("Grinning maliciously, you shove the minotaur onto his back, his massive equine dong slapping him in the chest as he lands with a grunt. Fondling your " +
            player.cockDescript(x) +
        ...
        outputText('The brute breaks out of his defeated daze with a look of abject horror as he feels unnatural squirming on his flare, bolting upright and loosing a terrified "<i>MOO!!</i>" as he scrambles to distance himself from your infested semen. You cackle with sick delight at his meager attempts to escape and grasp the head of his cock firmly, causing him to grunt with pain as you yank him back over to you by his flared rod and line the head of your ' +
            player.cockDescript(x) +
            " up with his urethra. His eyes lock with yours, a pleading, desperate look in them as your balls surge from the anticipation. Unable to hold back against the unholy squirming of your infested cum, you grin evilly and ram your throbbing, infested prick deep into his shaft.<br><br>", false);
        outputText("The minotaur writhes and moans, openly panicking but too weak to resist, as you saw your " +
            player.cockDescript(x) +
            ...
            player.face() +
            " as the first of your worms pour out of your tip and directly into the beast's urethra.<br><br>", false);
        ...
        outputText("The minotaur shivers, eyes wide and mouth open with breathless screams as his scrotum begins to visibly squirm, then bloat, swelling with the churning parasites. Without having to waste their effort on pinning the brute down, your worms get right down to business doing what they were made for, turning this minotaur into a constantly-cumming, worm infested, puddle of fuck - a slave to the endless orgasms your pets provide, just like you. The thought makes you cum one last time, a huge spurt of wormy jism making his balls swell obscenely before you finally pull out. Sated but exhausted, you flop down on your " +
            ...
            player.sackDescript() +
            " wriggle happily while you watch the squirming, newly infested minotaur writhe in the dirt of the cave, bringing a wicked smile to your face.<br><br>", false);
        ...
        outputText("Overcome with lust and fatigue, the minotaur falls to the ground as you unhurriedly remove your " +
            player.armorName +
            ". The minotaur's eyes roam all over your body, his massive cock hard and throbbing. You slowly begin to stroke your own hard cock, pre-cum beginning to drool thickly from the head and over your hand. You slide your hand up and down your long shaft, lubing it with your juices as you decide what to do with the fallen brute before you. An idea forms in your mind and your cock throbs with anticipation, your imagination already working out the entertaining details.<br><br>", true);
        outputText("With a hand covered in pre-cum, you roughly grab his fat prick and you smear it onto his flared cockhead, feeling it throb in your hand from the sensation. You press the head of your " +
            player.cockDescript(x) +
...
MinotaurScene.minotaurBlowjob = function () {
    ...
    if (!player.isTaur())
        outputText("peeling away your " + player.armorName + " and ", false);
    outputText("revealing your " +
        player.multiCockDescriptLight() +
        " to the defeated bull.  The beast looks to you in both arousal and shock, mouth parting slightly at the idea of another creature taking charge.  Within the musky stench of its dripping shaft you nearly lose yourself, and ", false);
    if (!player.isTaur())
        outputText("taking the bull by the horns", false);
    else
        outputText("pinning his horns in the crook of your forelegs", false);
    outputText(', you guide its mouth closer to your member as if on autopilot.  "<i>Let\'s see just how good you are at milking,</i>" you blurt; the minotaur moos in defiance but is soon silenced when the head of your ' +
        Appearance.cockNoun(player.cocks[x].cockType) +
        " presses into its bovine mouth.<br><br>", false);
    ...
    if (monster.lust >= monster.maxLust())
        outputText("his own burning lusts", false);
    else if (player.findPerk(PerkLib.ArousingAura) >= 0)
        outputText("the power of your corruption", false);
    else
        outputText("the salty taste of your pre-cum", false);
    ...
    if (!player.isTaur())
        outputText("finding your hands leaving his horns for the back of his head ", false);
    ...
    if (!player.isTaur())
        outputText("your hand", false);
    else
        outputText("a forehoof", false);
...
MinotaurScene.takeMinoCumDirectly = function () {
    ...
    if (monster.HP < 1)
        outputText("The beast-man has fallen on his back, comatose and insensate, though his shaft has risen to full erectness, displacing his loincloth.", false);
    else
        outputText("The beast-man is on his back with his legs splayed, masturbating hopelessly.  His anguished moans tell you all you need to know – he'll never get off alone.", false);
    outputText("  You smile, blushing in anticipation, readying yourself for the coming bliss.<br><br>", false);
    outputText("You shed your " +
        player.armorName +
        " and step over its waist, trembling slightly with anticipation of your next fix.  Taking his flared drug-nozzle in hand, you hold it steady and lower yourself down, feeling his slippery narcotic pre-cum smearing over your hole.  ", false);
    //Too tight!
    ...
    outputText("You bottom out, sighing happily as you feel his hot fluids leaking inside you, radiating pure pleasure out from your core.   Bestial hands grab your " +
        player.nippleDescript(0) +
        "s and begin gently twisting them, taking surprising care for all their strength.  Spurred on by the feelings radiating from your ", false);
    if (player.biggestTitSize() < 1)
        outputText("chest", false);
    else
        outputText(player.breastDescript(0), false);
    ...
    outputText("An hour or so later, you pick yourself up out of a puddle of mud that reeks of sex.  Still feeling groggy and euphoric, you get dressed, not bothering to clean up.  You have to work to keep your " +
        player.assholeDescript() +
        " clenched, lest you lose some of the drug making you so happy.", false);
...
MinotaurScene.minoGetsTitFucked = function () {
    ...
    if (monster.HP < 1)
        outputText("defeated.  ", false);
    else
        outputText("masturbating uncontrollably.  ", false);
    outputText("He's yours for the moment, and for right now, you want to see if that throbbing horse-cock will splatter its heavenly spunk all over your " +
        player.allBreastsDescript() +
        " and " +
        player.face() +
        ".  You really could use a fix, just enough to fuzz over all the worry and stress you've had to deal with.<br><br>", false);
    if (monster.lust >= monster.maxLust())
        outputText("You push his hands away and y", false);
    else
        outputText("Y", false);
    outputText("ou lean over him, feeling your generous breast-flesh jiggle, your " +
        player.nippleDescript(0) +
        "s tingling with desire as they rub across the horse-like minotaur dong.   The beast's pre starts forming into large drops along his flared head, and you waste no time smearing it through your cleavage, lubricating the space between your tits and breathing deeply of its perfect scent.", false);
    if (player.totalCocks() > 0)
        outputText("  Your " + player.cockDescript(0) + " grows harder and more aroused than ever before while your ", false);
    else
        outputText("  Your body aches with arousal while your ", false);
    if (player.hasVagina())
        outputText(player.vaginaDescript(0) + " grows hotter and hotter, begging to be taken.", false);
    else
        outputText(player.assholeDescript() + " puckers slightly, as if begging to be taken by another monstrous beast.", false);
    outputText("<br><br>", false);
    ...
    if (player.hasVagina())
        outputText("cunt.  ", false);
    else
        outputText("ass.  ", false);
    outputText("Out to prove him wrong by letting him release all over your face, you wrap your " +
        player.allBreastsDescript() +
        " tightly around him and begin sliding your body along his length.  The constant drizzling pre-cum easily lubricates you, and soon your orbs are shiny and slick with his drug-like fluid.<br><br>", false);
    outputText("As you begin shaking your breasts up and down, stroking his length, you feel the skin of his three rings of prepuce sliding the soaked skin of your tits.   You lick around the edge of his crown when it presents itself to your lips, making sure to take in a nice fat dollop of his thick cream.  It makes you dizzy for a moment, and you nearly slide off, but the sensation passes.  You need more.  You squeeze your " +
        player.allBreastsDescript() +
        " tighter around him, squeezing out a few more drops as you feel the manhood begin to swell, growing harder and larger as the minotaur's orgasm approaches.<br><br>", false);
    ...
    player.dynStats("sen", 0.5, "cor", 0.7);
    cleanupAfterCombat();
    player.orgasm();
};
...
MinotaurScene.AddictNagaOnMinotaur = function () {
    ...
    //[if (monster.lust >= 99)
    if (monster.lust >= monster.maxLust())
        outputText("You slither towards the fallen monster, wishing to taste his addictive semen.  He's currently engaged in stroking his enormous shaft up and down with both of his hands, but offers little resistance when you push him over to a sitting position.  You pull his hands away and replace them with yours, running your fingers up and down along his thickness, working yourself up with anticipation.  He leans back and exhales loudly, clearly appreciative.<br><br>", false);
    else
        outputText("You slither towards the fallen monster, wishing to taste his addictive semen.  You notice that, despite the beating you just gave him, his huge cock is fully erect.  Delighted, you gently run both of your hands up and down its long shaft.  He exhales loudly, signaling his appreciation.<br><br>", false);
    ...
    if (player.hasCock())
        outputText("furiously stroke your own " + player.multiCockDescriptLight() + ". Your muffled vocalizations of pleaure weakly join his own.<br><br>", false);
    else
        outputText("part the folds of your " + player.vaginaDescript(0) + ", sliding it inside and masturbating with a furious vigor. Your muffled vocalizations of pleaure weakly join his own.<br><br>", false);
...
MinotaurScene.minotaurDrinksMilkNewsAtEleven = function () {
    //spriteSelect(44);
    clearOutput();
    outputText("You kneel next to the exhausted minotaur, lifting his head up to your " +
...
MinotaurScene.layEggsInAMinotaurSpiderLike = function () {
    ...
    if (player.cor < 50)
        outputText("<br><br>You carefully unwrap his limbs, then pull his snout around to force him to look at you.");
    else
        outputText("<br><br>Chuckling at the thought of the fun the other mountain denizens will have with your well-trussed side of beef, you grab his snout and force him to look you in the eyes.");
...
MinotaurScene.layBeeEggsInCowbutt = function () {
    ...
    //(If Silly mode:
    if (silly)
        outputText('<br><br>Looking down at the mass of bull-meat filled with eggs, you lean down and whisper into his ear, "<i>And that\'s how you make Scotch Eggs, cowboy.</i>"');
    player.dumpEggs();
    cleanupAfterCombat();
    player.orgasm();
};
...
MinotaurScene.getRapedByMinotaur = function (autoRape) {
    ...
    if (player.vaginas.length > 0) {
        ...
        if (player.averageVaginalWetness() < 2)
            outputText("You aren't very wet, and fear the damage this beast will inflict on your " + player.vaginaDescript(0) + ".  ", false);
        else
    ...
    if (player.isTaur())
        outputText("<br><br>He positions himself behind your rear legs, and roughly impales you onto his shaft, forcing himself as far into you as he can manage. You cry out, and you feel your stomach distending to accommodate his incredible size. Grabbing your ass, he begins to violently pound your backside with his massive member.  ", false);
    else
        outputText("<br><br>He lifts you into the air, hefting your insignificant weight with little effort, and roughly impales you onto his shaft, forcing himself as far into you as he can manage. You cry out, and looking down you can see your stomach distending to accommodate his incredible size. Using you like a human cock-sleeve, he simply holds you by the torso and begins lifting you up and down.  ", false);
    //Vag stretch texts
    if (player.hasVagina())
    ...
    if (player.cockTotal() == 1)
        outputText("The bull-man bends forward a little, and grabs your " + player.cockDescript(0) + " in a crushing grip. He makes short jerking motions as he keeps thrusting into you.", false);
    if (player.cockTotal() > 1)
        outputText("The bull-man bends forward a little, and grabs one of your " + player.cockDescript(0) + "s in a crushing grip. He makes short jerking motions as he keeps thrusting into you.", false);
    if (player.vaginas.length > 0) {
        if (player.vaginalCapacity() < monster.cockArea(0))
            outputText("<br><br>You clench your teeth in pain as his thick ribbed cock ravages your tight pussy. After a while, a kind of numbness sets in, and you start to feel dim waves of pleasure piercing through the curtain of pain. You hear grunts of pleasure and satisfaction coming from the beast's mouth.", false);
        if (player.vaginalCapacity() >= monster.cockArea(0) && player.vaginalCapacity() < monster.cockArea(0) * 1.5)
            outputText("<br><br>You feel his monstrous ribbed cock deliciously rubbing against your love-canal, sending maddening waves of pleasure to your brain as the beast keeps shoving into you. Oblivious to anything but your own enjoyment, you moan loudly like a whore, as the beast chuckles and grunts from his own pleasure.", false);
        if (player.vaginalCapacity() >= monster.cockArea(0) * 1.5)
            outputText("<br><br>You feel his monstrous cock sliding easily in and out of your wide fuck-hole, hearing the beast grunt in dissatisfaction. He suddenly clouts your left buttock, which makes you tense your muscles, including your vagina's, and you start to feel the ribs on his cock rubbing against your wall. The pleasure it procures you makes you tense your muscles harder, causing the pleasure to swell again. The minotaur grunts in approval, and continues his violent pounding as you start moaning.", false);
    }
    ...
    if (player.cockTotal() > 0 && player.sens + rand(40) > 50) {
        ...
        if (player.cumQ() >= 500)
            outputText("erupting a thick torrent of seed that seems to go on forever, spurred by the constant pressure of the huge minotaur cock.  You paint the cave wall with cum, the beast egging you on as it roughly jacks your " +
                player.cockDescript(0) +
                ".  You are vaguely aware of your orgasm dragging on and on, until eventually your orgasm stops, leaving a sizable puddle of cum on the floor.  ", false);
        player.dynStats("lib", 0.5, "sen", 1, "cor", 1);
    }
    ...
    if (player.hasVagina())
        outputText("pussy.", false);
    else
        outputText("asshole.", false);
    cleanupAfterCombat();
    player.orgasm();
...
MinotaurScene.getOralRapedByMinotaur = function () {
    //spriteSelect(44);
    clearOutput();
    outputText("The huge bull-man grabs you around the waist with one hand and flips you over. You hang upside down in his grip, your " +
        player.legs() +
        " splayed in the air.  The minotaur rubs your face across his sweat-slick balls, smearing his musk into your skin. Each of the minotaur's testicles is bigger than a man's head, and the skin of his sack is pulled taut over his cum-filled balls.<br><br>", false);
    outputText("The minotaur traps your " +
        player.legs() +
        ...
        player.legs() +
        " held in one of the minotaur's hands while his other roughly gropes your stretched-out body.", false);
    if (player.hasBreasts())
        outputText("  The bull-man grabs your " + player.biggestBreastSizeDescript() + ", squeezing and mauling them with his calloused hand.", false);
    if (player.cockTotal() >= 1) {
    ...
    //new pg
    outputText("Wasting no time, the minotaur grinds his glans past your lips and tries to force the tip of his " +
        monster.cockDescriptShort(0) +
    ...
    if (player.hasBreasts())
        outputText(", painfully grinding your " + player.nippleDescript(0) + " against his wiry fur", false);
    if (player.balls > 0)
        outputText(", and you can feel the minotaur's hot breath on your " + player.ballsDescriptLight(), false);
    ...
    //new pg
    outputText("With no warning the minotaur yanks your " +
        player.legs() +
    ...
    //new pg
    outputText("The minotaur bellows and slams its hips forward, forcing the ridge past your lips. You're almost out of air and the edges of your vision are starting to fade. All you can see is the minotaur's belly. Its " +
        monster.ballsDescriptLight() +
    ...
    //last pg
    outputText("The minotaur grunts a few last times.  Finished with you, it lifts your " +
        player.leg() +
