...
MinotaurScene.minoVictoryRapeChoices = function () {
    ...
    if (player.lust >= 33) {
        ...
        if (x >= 0 && player.cockTotal() > 1 && player.countCocksOfType(CockTypesEnum.TENTACLE) > 0)
    ...
    //Not an addict
        ...
        if (monster.lust >= monster.maxLust())
            outputText("You smile in satisfaction as the " + monster.name + " drops down on all fours and begins masturbating feverishly.  Sadly you realize your own needs have not been met.  Of course you could always fuck the eager bull...<br><br>What do you do?", true);
        else
    ...
    //Not able to rape but a feeder
        ...
        if (monster.lust >= monster.maxLust())
            ...
            outputText("You smile in satisfaction as the " + monster.name + " collapses, unable to continue fighting.  Sadly you realize your own need to breastfeed has not been met.  You could always let the poor thing have a drink...<br><br>What do you do?", true);
    }
    //No rape, no feeder
    else {
        if (monster.lust >= monster.maxLust())
...
MinotaurScene.girlRapeAMinotaur = function () {
    ...
    if (player.vaginalCapacity() < monster.cockArea(0))
        outputText("<br><br>Mixed with the sheer sexual pleasure, you feel a diffuse pain and numbness coming from your much-abused " + player.vaginaDescript(0) + ", but decide with a smile that it was well worth it as his cum numbs away the last of the pain.", false);
...
MinotaurScene.rapeMinotaurTentacles = function () {
    ...
    if (player.vaginas.length > 1 && !mismatched && player.cocks.length > 2)
        outputText("Your tentacle cocks writhe upon themselves, each turning to an unattended " + player.vaginaDescript(1) + " on your body, and quickly filling up the cavernous depths. You groan in ecstasy at the self-fucking you are receiving as all your pussies start leaking their wonderful fluids.  ", false);
    //more dicks than available holes
    if ((player.countCocksOfType(CockTypesEnum.TENTACLE) == 2 || player.countCocksOfType(CockTypesEnum.TENTACLE) == 3))
...
MinotaurScene.rapeMinotaurTentacle = function () {
    ...
    if (player.vaginas.length > 0) {
        ...
        if (player.cumQ() >= 50 && player.cumQ() < 400)
            outputText("Your cock made quite a mess of things inside your " + player.vaginaDescript(0) + ".  As you pull free, cum drips freely down your thighs.  The heavenly scent of minotaur spunk fills your nose, and begin scooping the mixed jizz up to sample.  ", false);
        if (player.cumQ() >= 400)
            outputText("Your cock made quite a mess of things inside your " + player.vaginaDescript(0) + ".  As you pull free, a river of cum pours from betwixt your thighs.  The heavenly scent of minotaur spunk fills your nose, and you begin scooping the mixed jizz up to sample.  ", false);
    }
    //Free ass!
    else {
        temp = 2;
...
MinotaurScene.getRapedByMinotaur = function (autoRape) {
    ...
    if (player.cockTotal() > 0 && (player.sens + rand(40) > 50)) {
        ...
        if (player.cumQ() >= 500)
            outputText("erupting a thick torrent of seed that seems to go on forever, spurred by the constant pressure of the huge minotaur cock.  You paint the cave wall with cum, the beast egging you on as it roughly jacks your " + player.cockDescript(0) + ".  You are vaguely aware of your orgasm dragging on and on, until eventually your orgasm stops, leaving a sizable puddle of cum on the floor.  ", false);
        player.dynStats("lib", .5, "sen", 1, "cor", 1);
    }
