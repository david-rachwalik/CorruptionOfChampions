...
class DemonPack extends Creature {
        ...
        //Determine if cat'ed
            ...
            if (temp <= 0) {
                ...
                if (!liveData.monster.plural)
                    ...
                    GUI.outputText("You deflect " + liveData.monster.a + liveData.monster.name + " " + liveData.monster.weapon.verb + ".");
            }
            else {
                if (temp <= 5)
    ...
    lustAttack() {
        if (liveData.player.lust < 35) {
            GUI.outputText("The " + liveData.monster.name + " press in close against you and although they fail to hit you with an attack, the sensation of their skin rubbing against yours feels highly erotic.");
        }
            ...
            if (liveData.player.cocks.length > 0)
                GUI.outputText(liveData.player.multiCockDescriptLight() + " hardening ");
            else if (liveData.player.vaginas.length > 0)
                GUI.outputText(liveData.player.vaginaDescript(0) + " get wetter ");
            GUI.outputText("in response to all the friction.");
        }
        else {
            GUI.outputText("As the " +
            ...
            if (liveData.player.gender == 1)
                GUI.outputText(liveData.player.multiCockDescriptLight() + " towards the nearest inviting hole.");
            if (liveData.player.gender == 2)
                GUI.outputText(liveData.player.vaginaDescript(0) + " towards the nearest swinging cock.");
            if (liveData.player.gender == 3)
                GUI.outputText("aching cock and thirsty pussy towards the nearest thing willing to fuck it.");
            if (liveData.player.gender == 0)
                GUI.outputText("groin, before remember there is nothing there to caress.");
        }
...
class OasisScene {
    ...
    static oasisRunAway() {
        ...
        if (liveData.player.spe > 15 && liveData.player.spe / 2 > UTIL.rand(40)) {
            GUI.outputText("You bolt out from under your bush and scramble away over the sand. Before long the swishing sounds of pursuit fade away and looking back you see the few demons with the gusto to follow you tramping back to the oasis.");
            GUI.doNext(Camp.returnToCampUseOneHour);
        }
        else {
            GUI.outputText("You scramble away from the demons, but are too late. A swift demon with canine features tackles you to the ground.  Luckily he loses his grip as you tumble onto the sand and you slither free, stand up and wheel to face the host of leering demons which begin to advance with malicious intent.");
            COMBAT.startCombat(new DemonPack());
    ...
    static oasisTalkAccept() {
        //TODO spriteSelect(46);
        //You slut!
        GUI.outputText("The leader smiles in genuine delight and excited chatter rises up from the group of demons. 'This is excellent. It has been so long since we last had one of your kind join us.' Behind him the demons begin to slide free of their tattered rags, hardening, dampening and licking their lips. As the leader steps forward to caress the curves and angles of your body you begin to suspect that the hunger this feast is to satisfy is not for food, but all that is forgotten as the demons swarm silently around you and you stumble back onto the hot sand, ");
        if (liveData.player.isTaur())
            GUI.outputText("your rear legs losing their balance and sending you crashing on your flank.  ");
        else
            GUI.outputText("legs falling open in the process.  ");
        GUI.outputText("Suddenly the silence is broken by a shrill screeching laugh, then a howl and the movement of the demons begins to accelerate. The deep bass laugh of the demon leader breaks over you like a crashing wave and the demons shriek with frenzied lust as they take you on the sand of the oasis.");
        //Count voluntary submissions
        liveData.gameFlags[FLAG.OASIS_DEMONS_ACCEPT]++;
        GUI.doNext(OasisScene.oasisSexing);
    }
    // Oasis Sex Scenes
    static oasisSexing() {
        //TODO spriteSelect(46);
        ...
        if (liveData.player.cocks.length > 0) {
            ...
            GUI.outputText("your " + liveData.player.multiCockDescriptLight() + " and begin to slide up and down before another demon pushes them out of the way and you ");
            if (liveData.player.cocks[0].cockLength > 25)
                GUI.outputText("feel your " +
                ...
                GUI.outputText("see a smaller and younger demon lowering her very tight pussy onto your " + liveData.player.cockDescript(0) + ". It's a tight fit, but her almost-virginal pussy is dripping wet. She bottoms her pussy out ");
                if (liveData.player.cocks[0].cockLength > 17)
                    GUI.outputText("with hardly any of you inside her ");
                else if (liveData.player.cocks[0].cockLength > 12)
                    GUI.outputText("about halfway down your " + liveData.player.cockDescript(0) + " ");
                else if (liveData.player.cocks[0].cockLength > 6)
                    GUI.outputText("with almost all of you inside of her ");
                else if (liveData.player.cocks[0].cockLength <= 6)
                    GUI.outputText("as her pussy lips touch the base of your " + liveData.player.cockDescript(0) + " ");
                GUI.outputText("and begins to slide herself up and down your shaft in complete ecstasy, moaning like a cheap whore. She seems relatively uncorrupted for a denizen of this realm and is firm, tight and free of fur. Only a pair of horns betrays her taint. The thought that you are perhaps one of her first cocks sends tingles down your spine.  ");
                if (liveData.player.cocks[0].cockLength > 6) {
                    GUI.outputText("The realisation that her pussy is slowly stretching over your " + liveData.player.cockDescript(0) + " gives you a thrill that can only be described as positively evil.  ");
                    liveData.player.dynStats(["cor", 1]);
                }
                GUI.outputText("Soon the slender cock-whore begins to cum and she starts on what is about to be the first of several shuddering orgasms. You smile evilly as she cums around your " +
                    liveData.player.cockDescript(0) +
                    " and when her eyes roll back to the front of her head you know that there is more to come.  ");
            }
            //Orgasmzzzzzzz
            GUI.outputText("Soon you're shaking and pumping your seed deep into her pussy as she screams out one of her many orgasms and you cum deep inside her for the first time.  ");
            //Multizz
            if (liveData.player.cocks.length > 1) {
                ...
                if (liveData.player.cocks.length == 2)
                    GUI.outputText(liveData.player.cockDescript(1) + " is");
                else
                    GUI.outputText("cocks are");
        ...
        if (liveData.player.vaginas.length > 0) {
            ...
            //Not gaping!
                ...
                GUI.outputText("Eventually one of the demons wins out and sets the tip of his hefty dog-cock at the entrance to your pussy. He rams his member into your " +
                    liveData.player.vaginaDescript(0) +
                ...
                GUI.outputText("The two cocks pump you hard until you feel one of them shooting your " +
                    liveData.player.vaginaDescript(0) +
            ...
            //Orgasmzzzzz
            GUI.outputText("<br><br>The feeling of immense fullness gets better and better as the cum surging into your " +
                liveData.player.vaginaDescript(0) +
        ...
        //Buttbutt buuuuuttt
        if (liveData.player.gender > 0)
            GUI.outputText("However, the demons are interested in every part of you, not just your crotch.  ");
        GUI.outputText("Soon you feel hands and dicks grabbing and jabbing at your " +
        ...
        if (liveData.player.vaginas.length > 0)
            GUI.outputText("You feel the cocks in your ass and pussy rubbing into each other as your lower body becomes a temple of friction. The feeling of so much demon stuffing you as full as possible is almost unbearable, and you are inches away from blacking out as you come again.  ");
        GUI.outputText("The cock fucks you hard and fast for a dozen strokes and then you feel it begin to pump its cum into your stuffed ass until it can't come any more. It slips out of your " +
            liveData.player.buttDescript() +
        ...
        //More fucking!
        GUI.outputText("Around you you see the demons wrapped up in the frenzy of fucking that they've fallen into. No hole is unfilled and not a single cock isn't sunk into some being's flesh. The moans and screams are almost contagious and you feel them clouding your brain making you want more, harder.");
        liveData.player.dynStats(["int", -1], ["lib", 1]);
        //Titzen!
        if (liveData.player.breastRows.length > 0) {
            if (liveData.player.biggestTitSize() > 1) {
                GUI.outputText("  Demonic hands grab and squeeze at your " + liveData.player.allBreastsDescript() + " and lips fasten around your " + liveData.player.nippleDescript(0) + " and begin to lick and suck like there's no tomorrow.");
            }
        }
        //Newline for oral!
        GUI.outputText("<br><br>Soon even your mouth is taken by a demoness lowering her slick honeypot down onto your lips. You lick and suck as she moans like a whore atop your head. It's impossible to count how many times she comes and so you just relax into a rhythm of licking and sucking, interrupted only by your own bone-creaking orgasms as the demonic attentions to the rest of your body drive you over the edge time and time again.");
        //Non-preggers text!
        if (liveData.player.pregnancyIncubation == 0 && liveData.player.gender > 1) {
            //Newline for potential preggers?
            GUI.outputText("<br><br>");
            GUI.outputText("You do your best to keep a vague mental catalogue of what has been in where, but eventually it becomes impossible to remember the type or number of demonic dicks that have filled you with their cum. The sand below your ass is wet with seed that has spilled out of your overflowing " +
                liveData.player.vaginaDescript(0) +
        ...
        //If you got here by losing combat!
            ...
            GUI.outputText("The demons fuck you like animals until you can't come any more. Every one of your orifices is filled and you pump out orgasm after orgasm until you black out from the abuse.");
            COMBAT.cleanupAfterCombat();
            liveData.player.orgasm();
    ...
    static oasisBadEnd() {
        ...
        GUI.outputText("Your arms are also tied up behind your back, resting uncomfortably in the hot desert sand. You can see the tribe is packing up to get ready to move on, and you struggle to sit up against your bindings.<br><br>");
        GUI.outputText('"<i>I see that you are awake, slave,</i>" the leader says after watching you get up, walking closer to you with a wicked smirk on his lips. He stares down at you with a look of satisfaction on his face. "<i>Thought you would wake up in time to sneak off on us again, did you? Well, that won\'t be happening anymore.</i>"<br><br>');
        GUI.outputText("\"<i>What the hell are you talking about? I'm no one's slave!</i>\" you yell in indignation, only to cry out in pain as the tribe leader's hand comes down and smacks you across the face. Your head jerks back from the force of the blow, successfully silencing you.<br><br>");
        GUI.outputText("\"<i>There will no longer be any of that back talk, slave. As for what I'm talking about... After all of our encounters with you, my tribesmen and I have made a decision.</i>\" The leader's smirk widens as he reaches down and grips your face tightly, forcing you to look up at him. His eyes look you over appraisingly as he pulls out a black collar with a long chain attached and fastens it to your neck. His other hand reaches down to one of your " +
        ...
        if (liveData.player.gender == 3) {
            if (liveData.player.isTaur())
                GUI.outputText('"<i>Everyone will have a great time playing with you,</i>" the leader says, smirking wider still as he stares at your centaur body. He reaches out and gives your ' +
                    liveData.player.buttDescript() +
                ...
                GUI.outputText("His smirk widens even more as he stares down at your uncovered " +
                    liveData.player.cockDescript(0) +
    ...
    */
    static demonWin() {
        if (liveData.monster.HP <= 0) {
            GUI.outputText("You strike out and the last of the demons tumbles to the ground with a thud. You stand there for a second surrounded by dead or unconscious demons feeling like a god of battle. Then you realize that if a god of battle does exist he lives on a demonic plane like this, so to avoid insulting him you take your hands off your hips and your " +
                liveData.player.legs() +
                " off the head of the demon leader before you start to search the bodies.");
            liveData.player.dynStats(["lus", 1]);
        }
        else {
            GUI.outputText("The demons stop attacking, and reach out to touch your body. Some are already masturbating like it's the only thing in the world and you know that right now, if you wanted to, you could make each and every one of them fuck you.");
        }
    ...
    static demonLose() {
        if (liveData.player.gender == 0) {
            ...
            GUI.outputText("with arousal.  You make a grab for the nearest demon and catch a handful of jiggly breast. You try desperately to use your other arm to pull her closer to slake your thirst but you both go tumbling to the ground. The demonic leader laughs out loud and the rest of the tribe falls on you, grabbing for anything it can find.");
            GUI.doNext(OasisScene.oasisSexing);
        }
    }
}
