...
class SandTrap extends Creature {
    constructor() {
        ...
        if (UTIL.rand(3) == 0)
            liveData.gameFlags[FLAG.FERTILE_SANDTRAP] = 1;
        //Name and references
        this.a = "the ";
        ...
        this.tallness = UTIL.rand(8) + 150; //Height in inches
        this.skinTone = "fair";
        this.skinType = 0;
        this.skinAdj = "";
        this.skinDesc = "skin";
        ...
        //Cocks
        this.createCock(10, 2, ENUM.CockType.HUMAN);
        this.balls = 2;
        this.ballSize = 4;
    ...
    trapLevel(amount = 0) {
        if (amount != 0) {
            liveData.gameFlags[FLAG.TRAP_LEVEL] = liveData.gameFlags[FLAG.TRAP_LEVEL] + amount;
            if (liveData.gameFlags[FLAG.TRAP_LEVEL] < 1)
                liveData.gameFlags[FLAG.TRAP_LEVEL] = 1;
            if (liveData.gameFlags[FLAG.TRAP_LEVEL] > 4)
                liveData.gameFlags[FLAG.TRAP_LEVEL] = 4;
        }
        return liveData.gameFlags[FLAG.TRAP_LEVEL];
    }
    sandTrapWait() {
        GUI.clearOutput();
        //TODO spriteSelect(97);
        // First, mark that the player climbed...
        liveData.gameFlags[FLAG.CLIMBED_TRAP_THIS_ROUND] = 1;
        GUI.outputText("Current Trap Level is " + liveData.gameFlags[FLAG.TRAP_LEVEL] + "<br>Current Climb Status is " + liveData.gameFlags[FLAG.CLIMBED_TRAP_THIS_ROUND] + "<br>");
        GUI.outputText("Instead of attacking, you turn away from the monster and doggedly attempt to climb back up the pit, digging all of your limbs into the soft powder as you climb against the sandslide.");
        // Are you at the top?
        // TODO Bit of a strange thing here. You can't climb out from the spell, but you can flee successfully at the top.
        if (this.trapLevel() == 4) {
            GUI.outputText("<br><br>You eye the ground above you.  The edge of the pit is too sheer, the ground too unstable... although it looks like you can fight against the currents carrying you further down, it seems impossible to gain freedom with the sand under the monster's spell.");
        }
        // Otherwise, try to climb out
        else {
            //Strength check success: [Player goes up one level, does not go down a level this turn]
            if (liveData.player.str / 10 + UTIL.rand(20) > 10) {
                GUI.outputText('<br><br>Sweat beads your forehead - trying to clamber out of this pit is like running against the softest treadmill imaginable.  Nonetheless, through considerable effort you see you\'ve managed to pull further clear of the sandtrap\'s grasp.  "<i>Watching you squirm around like that gets me so hot,</i>" it calls up to you.  Turning around you see that the creature is rubbing its hands all over its lean body whilst watching you struggle.  "<i>Such an energetic little mating dance, just for me... mmm, prey who do that are always the best!</i>"');
                this.trapLevel(2);
            }
            else {
    ...
    doAI() {
        ...
        if (liveData.gameFlags[FLAG.TRAP_LEVEL] == 4 && liveData.gameFlags[FLAG.CLIMBED_TRAP_THIS_ROUND] == 0)
            this.grabAttack();
        ...
        if (liveData.gameFlags[FLAG.CLIMBED_TRAP_THIS_ROUND] == 0) {
            GUI.outputText("<br><br>Rivulets of sand run past you as you continue to sink deeper into both the pit and the sand itself.");
            this.trapLevel(-1);
        }
        //If player did climb, reset the status effect for the next round.
        else
            liveData.gameFlags[FLAG.CLIMBED_TRAP_THIS_ROUND] = 0;
    ...
    sandTrapPheremones() {
        //TODO spriteSelect(97);
        GUI.outputText("The sandtrap puckers its lips.  For one crazed moment you think it's going to blow you a kiss... but instead it spits clear fluid at you!   You desperately try to avoid it, even as your lower half is mired in sand.");
        //Avoid the Pheramone trap
        if (liveData.player.spe / 10 + UTIL.rand(20) > 10) {
            // TODO add this or statement -  || player.getEvasionRoll(false))
            GUI.outputText("  Moving artfully with the flow rather than against it, you are able to avoid the trap's fluids, which splash harmlessly into the dune.");
        }
        // Got caught
        else {
            var damage = 10 + liveData.player.lib / 10;
            GUI.outputText("  Despite ducking away from the jet of fluid as best you can, you cannot avoid some of the stuff splashing upon your arms and face.  The substance feels oddly warm and oily, and though you quickly try to wipe it off it sticks resolutely to your skin and the smell hits your nose.  Your heart begins to beat faster as warmth radiates out from it; you feel languid, light-headed and sensual, eager to be touched and led by the hand to a sandy bed...  Shaking your head, you try to stifle what the foreign pheromones are making you feel.");
            damage = Math.round(damage);
            liveData.player.changeLust(damage);
            GUI.outputText("<b>You take " + damage + " in lust damage!</b>");
        }
    }
    //Grab Attack
    grabAttack() {
        //TODO spriteSelect(97);
        GUI.outputText("The sandtrap smiles at you winningly as it thrusts its hands into the sifting granules.  The sand beneath you suddenly seems to lose even more of its density; you're sinking up to your thighs!");
        //Quicksand attack fail:
        if (liveData.player.spe / 10 + UTIL.rand(20) > 10) {
            //TODO Add this - || player.getEvasionRoll(false))
            GUI.outputText("  Acting with alacrity, you manage to haul yourself free of the area affected by the sandtrap's spell, and set yourself anew.");
        }
        //Quicksand attack success: (Speed and Strength loss, ability to fly free lost)
        else {
            GUI.outputText("  You can't get free in time and in a panic you realize you are now practically wading in sand.  Attempting to climb free now is going to be very difficult.");
            if (liveData.player.canFly())
                GUI.outputText("  You try to wrench yourself free by flapping your wings, but it is hopeless.  You are well and truly snared.");
    ...
    sandTrapLoss() {
        //SFW Mode?
        if (liveData.gameFlags[FLAG.SFW_MODE] > 0) {
            //No rape in SFW mode.
            GUI.clearOutput();
        ...
        if (liveData.gameFlags[FLAG.INFESTED] == 1) {
            GUI.outputText("<br><br>The sand trap seems bemused by the insects your body houses...");
            COMBAT.cleanupAfterCombat();
            return;
        }
        //Lose to Sand Trap while in Bad End mode.
        if (liveData.gameFlags[FLAG.SANDTRAP_LOSS_REPEATS] >= 2 && liveData.player.eyeType == ENUM.EyeType.EYES_BLACK_EYES_SAND_TRAP && liveData.player.wingType == ENUM.WingType.WING_TYPE_GIANT_DRAGONFLY) {
            SandTrapScene.loseLastFightWithSandTrap();
            return;
        }
        // Otherwise...
        liveData.gameFlags[FLAG.SANDTRAP_LOSS_REPEATS]++;
        //TODO Do we need a clear output here?
        GUI.outputText("<br><br>");
        //TODO Sprite spriteSelect(97);
        // Sunk too far into the trap
        if (this.trapLevel() == 1) {
            GUI.outputText("You are sunk to your belly in the depthless sand at the bottom of the pit, and are still falling fast.  The sun above you is blotted out by a shape which leans downwards towards you, smiling triumphantly.  Desperately, you try to keep your arms free so you can swing a blow at it, but with consummate ease, the sandtrap grabs your wrists with one set of hands while pushing you downwards with the others.  Within moments you are up to your armpits in the stuff, staring helplessly up at the strange androgynous creature which has you entirely at its mercy.");
            liveData.player.lust = liveData.player.maxLust(); // Kludge to get the combat to end cleanly.
        }
        //PC lust loss:
        else
            GUI.outputText("You feel as radiant and molten as the sun above you... you just want to sink into the warm sand surrounding you forever.  Why are you struggling against it again?  You can't remember; with a sigh, you fall backwards onto the soft powder and allow yourself to be carried right down to the bottom.  The sandtrap chuckles softly as it envelopes you in its waiting arms.  \"<i>Good " +
                liveData.player.mf("boy", "girl") +
                '...</i>"');
        if (liveData.player.hasCock())
            GUI.doNext(SandTrapScene.cockLoseToSandTrap);
        else if (liveData.player.hasVagina())
            GUI.doNext(SandTrapScene.chicksLoseToSandTrap);
        else
            GUI.doNext(SandTrapScene.genderlessLoseToSandTrap);
    }
}
//------------
...
class SandTrapScene {
    ...
    static sandTrapWin() {
        ...
        liveData.gameFlags[FLAG.SANDTRAP_LOSS_REPEATS] = 0;
        //PC HP victory:
        if (liveData.monster.HP < 1)
            GUI.outputText("The sandtrap groans and collapses forwards into the dirt.  With the creature no longer controlling it, the sand underneath you feels a great deal more stable... climbing out won't be too difficult.");
        //PC lust victory:
        else
            GUI.outputText("The sand around you stops sinking.  Overpowered with lust, the sandtrap moans and gives up control over itself and its pit to desperately run one set of hands over its flat chest whilst the other pushes into the sand to masturbate... whatever it has down there.  With your foe in this state, climbing out won't be so difficult now.");
        if (liveData.player.lust >= 33) {
            ...
            GUI.menu();
            //TODO Track Down this Status Effect and see if it needs replacing: if (player.isNaga() && player.findStatusEffect(StatusEffects.Naga) >= 0 && player.gender > 0 && player.faceType == FACE_SNAKE_FANGS) GUI.addButton(0, "Naga3Some", this.nagaThreesomeWithSandTrap);
            if (liveData.player.hasCock() && liveData.player.str >= 60)
                GUI.addButton(1, "UseYourCock", this.cockWin); //stickWanginSandgina
            if (liveData.player.hasVagina())
                GUI.addButton(2, "RideVaginal", this.vaginaWin); //rideDatSandTarp
            if (liveData.player.gender > 0)
                GUI.addButton(3, "Handjob", this.handjobWin); // useSandTarpsHands
            //TODO Paizuri Scene - if (player.hasVagina() && player.biggestTitSize() >= 4 && liveData.player.armorName == "lusty maiden's armor") bikiniTits = createCallBackFunction2((player.armor as LustyMaidensArmor).lustyMaidenPaizuri,player,monster);
    ...
    static cockWin() {
        ...
        GUI.outputText("<br><br>You come to your senses some time later.  Your ");
        if (liveData.player.balls > 0)
            GUI.outputText("balls ache and your ");
        GUI.outputText("vision swims a bit as you slowly recede from the sandtrap, dripping shared sex as you go.  You give the top half of the creature a careful berth as you pick up your [armor] and slowly dress, but it is in no shape for retaliation.  The black haired androgyne seems to be in a deeply satisfied post-coital trance, humming and fluttering softly to itself as it trails patterns in the sand.  It blinks unsteadily at you as you begin to clamber your way out of the hole.  Behind you you hear sand being flung as it begins to go about the task of re-immersing itself.");
        GUI.outputText('<br><br>"<i>Thank you for your seed, lion,</i>" its sleepy words reach you as you pull yourself clear of the hollow and step out towards camp.  "<i>Perhaps next time we meet I will return the favor, hmm?</i>"  In your wearily tranquil fug you can\'t tell if it intends this as an invitation or a threat.');
    ...
    static vaginaWin() {
        ...
        //TODO spriteSelect(97);
        //Requirement: Vagina
        GUI.outputText("You bend into the sand and allow the slope to slowly carry you down to the bottom and the defeated sandtrap.  Its six black eyes regard you with a mixture of lust and resentment as you slowly discard your [armor], reveling in the hot desert sun upon your naked " +
    ...
    static handjobWin() {
        ...
        if (liveData.player.gender == 1) {
            ...
            if (liveData.player.cocks[0].cockLength < 6) {
                ...
                GUI.outputText("  Your small size doesn't seem to bother it; if anything, judging from the red rising in its thin cheeks and its shortening breath, the girliness of your cock actually seems to excite it.  It pauses and, whilst you are both panting lightly, it slowly and gently circles your head with its index finger whilst it still holds the entirety of your cock with the rest of its hand.");
            }
            //Cock 6-16 inches:
            else if (liveData.player.cocks[0].cockLength < 16) {
                //[no balls:
                if (liveData.player.balls == 0)
                    GUI.outputText("<br><br>The sandtrap glides its hand up and down your " + liveData.player.cockDescript(0) + ", picking up a warm smooth rhythm.");
                //Balls:
                else
                    GUI.outputText("<br><br>The sandtrap glides its hand up and down your " +
                        liveData.player.cockDescript(0) +
            ...
            //Cock more than 16 inches:
                ...
                GUI.outputText("<br><br>The sandtrap quickly comes to the realisation it needs both hands for this.  Still cupping your behind, it moves one oily hand up the top end of your massive shaft, while the other works the other half.  You sigh with deep satisfaction, enjoying the all-too-rare sensation of having every inch of your " +
                    liveData.player.cockDescript(0) +
            ...
            if (liveData.player.cockTotal() == 1) {
                GUI.outputText("  The sandtrap pumps you faster and faster until its slippery hands are a blur of motion.  It clutches onto your butt cheeks so tightly you hazily realize the indents of its slender digits will be with you for hours afterwards.");
                GUI.outputText("<br><br>The warm, oily double sensation is unbearably good and although you try to hold on to savor it as long as possible, it isn't long before you reach a toe-curling peak and, with a wordless bark, spurt out line after line of jizz.  The trap flinches slightly when your first cumshots hit it in the face and chest, but then goes back to gently rubbing you, guiding you up and down your warm, slippery orgasm.");
            }
            //Two cocks:
            else if (liveData.player.cockTotal() == 2) {
                GUI.outputText("  As the sandtrap begins to pump you faster, your disregarded " +
            ...
            //Three cocks:
            else if (liveData.player.cockTotal() == 3) {
                GUI.outputText("<br><br>As the sandtrap begins to pump you faster, your two other, disregarded cocks bob in hard, eager concert with your " +
                    liveData.player.cockDescript(0) +
                ...
                if (liveData.player.balls > 0)
                    GUI.outputText("  It grins up at you shyly and, with its final hand, cups your [balls].  ]You laugh with an almost hysterical edge to the sky at how mindboggling getting a triple handjob feels, and the trap giggles slightly as well despite itself.");
                GUI.outputText("<br><br>The warm, oily tripled sensation is unbearably good and although you try to hold on to savor it as long as possible, it isn't long before you reach a toe-curling peak and, with a wordless bark, spurt out line after line of jizz.  The trap flinches slightly when your first shots hit it in the face and chest, but then goes back to gently rubbing you, guiding you up and down your warm, slippery multiple orgasm.");
            }
            //Four cocks:
            else if (liveData.player.cockTotal() == 4) {
                GUI.outputText("<br><br>As the sandtrap begins to pump you faster, your three other, disregarded cocks bob in hard, eager concert with your " +
                    liveData.player.cockDescript(0) +
                ...
                GUI.outputText("<br><br>The warm, oily quadrupled sensation is unbearably good and although you try to hold on to savor it as long as possible, it isn't long before you reach a toe-curling peak and, with a wordless bark, spurt out line after line of jizz.  The trap flinches slightly when your first cumshots hit it in the face and chest, but then goes back to gently rubbing you, guiding you up and down your warm, slippery multiple orgasm as you spurt like a cum sprinkler in front of it.");
            }
            //More than four cocks:
            else {
                GUI.outputText("<br><br>As the sandtrap begins to pump you faster, your many other, disregarded cocks bob in hard, eager concert with your " +
                    liveData.player.cockDescript(0) +
        ...
        //Female:
            ...
            GUI.outputText("<br><br>The sandtrap is a fast learner.  After stroking and probing your dripping tunnel for a bit the creature makes you gasp again as it curls one of its long, slender digits and finds your G-spot.  The trap begins to finger your most sensitive point with a vengeance, curling its fingers and stroking outwards in a come-hither gesture.  Coupled with the attention it is lavishing on your clit, the sensation is eye-crossingly good and you begin to grunt and moan with need, thrusting your sex into its hands, coating them with your own lubricant.");
            if (liveData.player.tone < 60)
                GUI.outputText("  It picks up the pace gradually and soon it is doubly frigging you as fast as its fingers will allow, its other hands sinking deep into your soft, pillowy behind as it clutches onto you.");
            else
                GUI.outputText("  It picks up the pace gradually and soon it is doubly frigging you as fast as its fingers will allow, its other hands clutching onto your tight, muscled behind so tightly you hazily realize the indents of its slender digits will be with you for hours afterwards.");
            GUI.outputText("<br><br>You grab the sandtrap's black hair and cum with startling force, gushing fluid onto the trap and the sand beneath you as your cunt flexes around its fingers, which continue to curl and flick to send spasms of ecstatic pleasure up your spine.");
        }
        //Herm:
        else if (liveData.player.gender == 3) {
            GUI.outputText('<br><br>One pair of the creature\'s hands slowly brush up your inner thighs whilst the other two feel around your "<i>armor</i>", clumsily loosening clasps and belts as it finds them.  One hand finds your ' +
                liveData.player.cockDescript(0) +
            ...
            if (liveData.player.cocks[0].cockLength < 6) {
                ...
                if (liveData.player.balls == 0)
                    ...
                    GUI.outputText("<br><br>The sandtrap only needs one hand to pump your diminutive cock; it barely moves its wrist as it jerks you, occasionally wriggling its oily fingers along the bottom of your " +
                        liveData.player.cockDescript(0) +
                        " to incite you even further, whilst it uses its free hand to gently cup your [balls].");
                GUI.outputText("  Your small size doesn't seem to bother it; if anything, judging from the red rising in its thin cheeks and its shortening breath, the girliness of your cock actually seems to excite it.  It pauses and, while you are both panting lightly, it slowly and gently circles your head with its index finger, still holding the entirety of your cock with the rest of its hand.");
            }
            //Cock 6-16 inches:
            else if (liveData.player.cocks[0].cockLength < 16) {
                if (liveData.player.balls == 0)
                ...
                GUI.outputText("<br><br>The sandtrap quickly comes to the realisation it needs both hands for this.  Still cupping your behind, it moves one oily hand up the top end of your massive shaft, whilst the other works the other half.  You sigh with deep satisfaction, enjoying the all-too-rare sensation of having every inch of your " +
                    liveData.player.cockDescript(0) +
                    " immersed in pleasure.  The creature screws its face up and looks away in distaste at the size of your straining dick, which is in danger of poking it in the nose.  As you enjoy the warm, slippery sensation inundating you, you find you couldn't care less what it thinks.");
            }
            //One cock:
            if (liveData.player.cockTotal() == 1) {
                GUI.outputText("<br><br>The sandtrap pumps you faster and faster until its slippery hands are a blur of motion, lavishing attention upon your clit and pussy, eventually pumping and flicking in furious rhythm.  It clutches onto your butt cheeks so tightly you hazily realize the indents of its slender digits will be with you for hours afterwards.");
            ...
            //Two cocks:
                ...
                GUI.outputText("<br><br>As the sandtrap begins to pump you faster, your disregarded " +
                    liveData.player.cockDescript(1) +
                ...
                GUI.outputText("<br><br>The warm, oily triple sensation is unbearably good and although you try to hold on to savor it as long as possible, it isn't long before you reach a toe-curling peak and, with a harsh moan, spurt out line after line of jizz whilst gushing femcum coat its hands and your thighs.  The trap flinches slightly when your first shots hit it in the face and chest, but then goes back to gently rubbing and frigging you as you ride your glorious orgasms out.");
            }
            //hree cocks:
            else if (liveData.player.cockTotal() == 3) {
                GUI.outputText("<br><br>As the sandtrap begins to pump you faster, your two other, disregarded cocks bob in eager concert with your " +
                    liveData.player.cockDescript(0) +
                ...
                GUI.outputText("<br><br>The warm, oily sensation is unbearably good and although you try to hold on to savor it as long as possible, it isn't long before you reach a toe-curling peak and, with a harsh moan, spurt out line after line of jizz whilst gushing femcum coat its hands and your thighs.  The trap flinches slightly when your first shots hit it in the face and chest, but then goes back to gently rubbing you as you ride your glorious orgasms out, spurting like a cum sprinkler in front of it.");
            }
            //More than three cocks:
            else {
                GUI.outputText("<br><br>As the sandtrap begins to pump you faster, your many other, disregarded cocks begin to harden in eager concert with your " +
                    liveData.player.cockDescript(0) +
    ...
    static cockLoseToSandTrap() {
        ...
        //6 inches to 15:
        else if (liveData.player.cocks[0].cockLength < 15)
        ...
        //More than 15 inches:
            ...
            GUI.outputText("<br><br>The creature owns an incredibly tight sex which grasp you possessively once it is on you.  Sweat beads the sandtrap's elegant brow as it forces itself brutally down your huge shaft; when it finally stops with a sizable portion of your " +
                liveData.player.cockDescript(0) +
        ...
        if (liveData.player.cumQ() > 1000)
            GUI.outputText("  The sandtrap gasps and coos as you ejaculate over and over again, and beneath you feel your cum wet the sands as it beads around the clenching grip it has you in.");
        if (liveData.player.hasVagina())
            GUI.outputText("  Ignored entirely, your " + liveData.player.vaginaDescript(0) + " quivers and orgasms in tandem, flexing and wetting fruitlessly against the dry water pressing against it.");
        GUI.outputText("<br><br>The sandtrap finally releases you, humming and fluttering sounds of pleasure as it draws its unseen cunt away and retracts its prong from your mouth and back into its slit, trailing oil and saliva as it goes.  You don't really notice these things or even if it is trying to say anything to you in that bizarre voice it has, still deep within the encasing sand and deep within your muddling daze, and your limbs feel like any returning strength has been sucked right back out of you by the creature's milking cunt.  You do not struggle when the sandtrap takes you into its arms and begins to turn you around.  It fondles your [chest] with one hand as its others busily work at pulling your bottom half this way and that, until your [butt] is poked outwards in supplication.  Only then does it dozily occur to you that the creature is not done with you yet.");
        GUI.outputText('<br><br>"<i>You\'hhve done well for me so farhh,</i>" chuckles the sandtrap lowly, bending over you and threading one set of arms under your ');
        if (!liveData.player.isTaur())
            GUI.outputText("armpits");
        else
            GUI.outputText("forelegs");
        GUI.outputText('.  You try and turn your head to see what it is doing but it gently but insistently pushes your face into the yielding sand.  "<i>You arrrrhe a fine little drone.  I wwill thank you for yourrh hard work... with the fruits of ssame hard work.</i>" What does it mean by...?');
        GUI.outputText("<br><br>The self-same tube which sucked away your cum pushes into your sphincter, sliding easily into your anal passage with its wet weight.  You arch your back and open your mouth... but the complete relaxation the sandtrap's oil has instilled in you stops your body from clenching against the invader, and you feel no pain.");
        liveData.player.buttChange(30, true, true, false);
        GUI.outputText("  The creature pushes your head down, making soothing shushing noises as it feeds more of its bulbous length into your ass.");
        if (liveData.player.analCapacity() < 40)
            GUI.outputText("  Although the oil which coats your thoughts and reflexes with sensual supplication makes things easier, the feeling is still incredibly intense, and you grit your teeth as something round travels down the creature's ovipositor and stretches your tight hole wide.");
        else
            GUI.outputText("  Your accommodating ass feels like it was made for this purpose, and with the creature's oil soothing your thoughts and reflexes you savor the creature taking you like this as if it were your quim.  You moan as something round travels down the creature's ovipositor and into your wet ass, the sensation making [eachCock] hard again.");
        GUI.outputText("<br><br>The sandtrap continues to make its soothing, humming sounds and caresses your [chest] and [hair] whilst it holds you firmly in its sandy grasp, sinking egg after egg into your [butt].  It is a process which seems to go on for minutes on end, and the sensation of having your ass bred like this starts to drive you inexorably towards another peak.  You groan as the slick pressure in your bowels makes you cum, your cock");
        if (liveData.player.hasVagina())
            GUI.outputText(" and cunt");
        GUI.outputText(" flexing out more moisture into the sand beneath you.  The oil-induced serenity and the sexual marathon the sandtrap has put you through are too much and, even with the creature still relentlessly pumping away at you, you pass out.");
        GUI.outputText("<br><br>You awaken a while later, wearily getting to your feet and looking around.  You are standing in a featureless stretch of desert... there is no suggestion of the sandtrap, or indeed that you are in the same place where it caught you.  A fair amount of time has passed though, judging by the sky above you.  Perhaps it was all a particularly lucid mirage?  A sensation of... fullness in your abdomen suggests otherwise.  Clutching your bowels uneasily, you make your way back to camp.");
        //monster.createStatusEffect(StatusEffects.Fertilized,0,0,0,0); // TODO TEST THIS
        //this.sandTrapPregChance(); // TODO Set Up Pregnancy
    ...
    static chicksLoseToSandTrap() {
        ...
        if (liveData.monster.findStatusEffect(StatusEffects.Fertilized) >= 0)
            this.sandTrapPregChance(); //TODO Pregnancy
        liveData.player.dynStats(["lib", 1]);
        liveData.player.slimeFeed();
        //reduce lust, increase lib, slimefeed, reset hours since cum
        COMBAT.cleanupAfterCombat();
    ...
    static genderlessLoseToSandTrap() {
        GUI.clearOutput();
        //TODO Sprite spriteSelect(97);
        GUI.outputText("<br><br>The sandtrap holds your hands whilst it pushes you further down with its other set of arms, until only your head is above the sand.  Below the surface you try to weakly move your limbs, try to work yourself out of this situation but it is impossible; the sand feels impossibly heavy and is packed against you.  The sandtrap seems to have no such difficulty.  It towers above you, moving with sinuous grace.  You feel something wet touch your thigh and you try to flinch, but aside from flexing your muscles you cannot move.");
        GUI.outputText('<br><br>"<i>Hwhat a strange creature you arrrhe,</i>" says the sandtrap, looking at you with vague bafflement as it gently strokes your face.  It seems the more excited it gets and the less it feels the need to pretend, the more fluttery and broken its voice becomes; it is like you are listening to a hive of bees that just happens to be forming words.  ');
        //(Trap has fertilised eggs:
        if (liveData.monster.findStatusEffect(StatusEffects.Fertilized) >= 0)
            GUI.outputText('"<i>Not a worker OR a drone! Wwwell, that isz ok.  Even ants who hhhave nothing can be someone for somebody.  And hhyou can be mommy for my childrrhen!</i>"');
        else
            GUI.outputText('"<i>Not a worker OR a drone! Wwwell, that isz ok.  Hwwe can still have fun, can\'t we?</i>"');
        GUI.outputText('<br><br>You are helpless to ignore the fact it has a genital slit where its human cock would be, particularly as the sandtrap places its hands behind your head and proceeds to rub your face into it.  Its smooth, supple human flesh trades with its rough, leathery insect hide against your cheeks and forehead, and as its excitement grows you feel the slit slide open and something long, warm, and oily presses insistently into your face.  The creature pulls back momentarily, and with a feeling of deep trepidation you take it in... a black ten-inch insect cock prong, thicker at the base than its dull tip, dripping with a clear, viscous fluid.  Smiling tenderly, it presses the tip of its gleaming black prong against your lips.  "<i>Drink.  It will make thingsz so much easier for hwyou if hwyou do.</i>" The creature is not exactly giving you much choice in the matter.  Packed tightly inside its sand and incapable of resisting, you steel yourself, close your eyes, open your mouth and accept it.');
        ...
        liveData.player.buttChange(30, true, true, false);
        GUI.outputText("<br><br>The creature makes soothing shushing noises into your mouth and feeds more of its bulbous length into your ass.  ");
        if (liveData.player.analCapacity() < 30)
            GUI.outputText("Although the oil which coats your thoughts and reflexes with sensual supplication makes things easier, the feeling is still incredibly intense, and you grit your teeth as something round travels down the creature's ovipositor and stretches your tight ass wide.");
        else
            GUI.outputText("Your accommodating ass feels like it was made for this purpose, and with the creature's oil soothing your thoughts and reflexes you savor the creature taking your [asshole] like this as if it were a quim.  You moan as something round travels down the creature's ovipositor and into your wet ass.");
        GUI.outputText("  Slowly the creature begins to pick up its rhythm again, sliding in and out of your butt as it breeds you.");
        ...
        //TODO Pregnancy this.sandTrapPregChance();
        liveData.player.dynStats(["lib", 1]);
        liveData.player.slimeFeed();
        //reduce lust, increase lib, slimefeed, reset hours since cum
        COMBAT.cleanupAfterCombat();
    ...
    static sandTrapPregChance() {
        ...
        if (liveData.gameFlags[FLAG.FERTILE_SANDTRAP] == 1)
            ...
            liveData.player.buttKnockUp(ENUM.PregnancyType.PREGNANCY_SANDTRAP, ENUM.IncubationType.INCUBATION_SANDTRAP);
    }
    static birfSandTarps() {
        //TODO Change this function name once found in PregnancyAdvance.as
        //TODO spriteSelect(97);
        GUI.outputText("<br>Your eyes widen as a gout of oil suddenly gushes from your ass.  Before panic can set in, an incredible light-headedness overtakes you.  Dreamily, you discard your [armor] and squat.  More oil oozes out of you, and in your hazy euphoria, you scoop some of it up and rub it dreamily into your " +
        ...
        //[Male:
        if (liveData.player.gender == 1) {
            GUI.outputText("  You roll your eyes to the sky and moan, [eachCock] growing hard as you push out the egg.");
            //[(mans and qualified horses only)]
            if (!liveData.player.isTaur() || liveData.player.tallness * (5 / 6) < liveData.player.cocks[liveData.player.longestCock()].cockLength)
                GUI.outputText("  Your oily hands descend upon your cock, and you massage your shaft as you feel the pressure in your bowels intensify again.");
        }
        //Female:
        else if (liveData.player.gender == 2) {
            GUI.outputText("  You roll your eyes to the sky and moan, your " + liveData.player.vaginaDescript(0) + " moistening as you push out the egg.");
            //[(no fukken horses from here)]
            if (!liveData.player.isTaur())
                GUI.outputText("  Your oily hands push softly into your cleft, fingering your needy " + liveData.player.clitDescript() + " as you feel the pressure in your bowels intensify again.");
        }
        //Herm:
        else if (liveData.player.gender == 3) {
            GUI.outputText("  You roll your eyes to the sky and moan, [eachCock] growing hard and your [vagina] moistening as you push out the egg.");
            //[(no horses)]
            if (!liveData.player.isTaur())
                GUI.outputText("  Your oily hands descend upon your genitals and you begin to slowly pump your shaft and finger your needy " + liveData.player.clitDescript() + " as you feel the pressure in your bowels intensify again.");
        }
        GUI.outputText("<br><br>You nearly cum as egg number two squeezes out.  You DO cum when egg number three stretches you wide, blowing your mind into the empty sky.  Each egg seems to come out closer on the heels of the one before, and each time your conscious mind loses more of its ability to do anything but wallow in oil and pleasure.");
        GUI.outputText("<br><br>You are brought to your senses by something flitting past your eyes.  You wearily brush your face and pick yourself up.  Behind you, leaking and lying in a translucent ooze, is a multitude of black, oily eggshells.  Of more note are the pale blurs which keep zipping past your head.  They look rather like the fairies which inhabit the forest, except they have six black eyes, are flat-chested and are male... no, female...?  No, male... you can't tell.  They are lithe and beautiful and have tiny, undeveloped insect abdomens hanging above their trim butts and below their dragonfly wings.  They whirr in place for a moment before keeling around and about each other excitedly like hoverflies, chattering to each other in a tongue so fast it is beyond your comprehension.");
        //Libido <30:
        if (liveData.player.lib < 33)
            GUI.outputText("<br><br>You pick yourself up wearily, flap the flytraps you have birthed away and make your way back to camp.  This whole experience has been deeply unnerving, and you vow to make sure you don't have to repeat it.");
        //Libido 30-65:
        else if (liveData.player.lib < 66)
            GUI.outputText("<br><br>You spend a moment enjoying your post-natal haze, then haul yourself out of it, flap the flytraps you have birthed away, and make your way back to camp.  Though this experience has been deeply unnerving, you can't help but acknowledge it has also been incredibly erotic.");
        //Libido >65:
        else
            GUI.outputText("<br><br>You smile lazily, then lie back and glory in the sensual haze the oil has left you in.  After you have spent many minutes lying listening to the happy twittering of your flytrap children above you, you reluctantly get up.  You only hope that you get to experience the unearthly wonder of birthing these strange creatures again, and again, and again.");
    ...
    static encounterASandTrap() {
        ...
        // Mark we've met one
        liveData.gameFlags[FLAG.TIMES_ENCOUNTERED_SAND_TRAPS]++;
        // Very first sandtrap encounter
        if (liveData.gameFlags[FLAG.TIMES_ENCOUNTERED_SAND_TRAPS] == 1) {
            //femininity <= 65:
            if (liveData.player.femininity <= 65) {
                GUI.outputText('You are exploring the endlessly undulating landscape of the desert when you hear an agonized cry for help over the nearest dune.  You quickly scramble up it and are confronted by the sight of a small, flat lake of exceptionally fine sand, overlooked on each side by tall dunes.  Trapped in the middle of it is a young woman with black hair, up to her armpits in the sand.  "<i>Oh, thank the Gods!</i>" she cries, spotting you.  "<i>I\'m sinking, please help me!</i>"  The word \'quicksand\' flashes through your mind like fire and you take a nervous step back, attempting to quickly evaluate the situation.');
                if (liveData.player.inte > 65)
                    GUI.outputText("  Something about this scene puts you on edge.  How come you've never encountered quicksand in this desert before, with no water and its steady, stubborn, but not violent winds?  There's something odd about the woman, too - although she can't be a sand witch with that hair, you can't quite make her features out clearly through the heat haze.");
                GUI.outputText("  You think you could just about reach her with your hand if you were careful.  If you're going to save her, it has to be now; even in the short time you've been here she has sunk down to her collarbone.  She stares at you with plaintive despair.");
            }
            else {
                GUI.outputText('You are exploring the endlessly undulating landscape of the desert when you hear an agonised cry for help over the nearest dune.  You quickly scramble up it and are confronted by the sight of a small, flat lake of exceptionally fine sand, overlooked on each side by tall dunes.  Trapped in the middle of it is a young man with black hair, up to his armpits in the sand.  "<i>Oh, thank the Gods!</i>" he cries, spotting you.  "<i>I\'m sinking, please help me!</i>"  The word \'quicksand\' flashes through your mind like fire and you take a nervous step back, attempting to quickly evaluate the situation.  ');
                if (liveData.player.inte > 65)
                ...
                GUI.outputText("You think you could just about reach him with your hand if you were careful.  If you're going to save him, it has to be now; even in the short time you've been here he has sunk down to his collarbone.  He stares at you with plaintive despair.");
            }
            GUI.menu();
