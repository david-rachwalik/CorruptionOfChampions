...
Naga.nagaTailWhip = function () {
    ...
    if ((player.findPerk(PerkLib.Evade) && rand(6) == 0)) {
        ...
        var damage = 10;
        if (player.armor.defense < 10)
            damage += 10 - player.armor.defense;
        else
            damage += rand(3);
    ...
    if (player.hasCock && (!player.hasVagina() || rand(2) == 0)) { //Cocks
        ...
        //[Cock isn't too big]
            ...
            if (player.totalCocks() == 1)
                outputText("Gently she starts to stroke the length of your " + player.cockDescript(0) + " with one hand while circling the tip with the other. ");
...
NagaScene.nagaRapeChoice = function () {
    ...
    if (player.canOvipositBee() && player.gender > 0)
    ...
    if (player.lust >= 33 && gameFlags[SFW_MODE] <= 0) {
        ...
        menu();
        if (player.gender == 0)
            addButton(0, "Yes", NagaScene.nagaVictoryGenderless, null, null, null, "TO BE ADDED.");
        if (player.gender == 1)
            addButton(0, "Yes", NagaScene.nagaVictoryMale, null, null, null, "TO BE ADDED.");
        if (player.gender == 2)
            addButton(0, "Yes", NagaScene.nagaVictoryFemale, null, null, null, "TO BE ADDED.");
        if (player.gender == 3) {
    ...
    if (player.totalCocks() > 0) {
        ...
        if (player.isTaur()) {
            ...
            //[SUPER CUM]
                ...
                outputText("you explode with huge fountains of cum spurting from your " + player.cockDescript(0) + ", gushing out from where the two of you are joined to rain on the sands below, ");
        ...
        //Uncorrupted  
            ...
            if (player.cockArea(0) <= monster.vaginalCapacity()) {
                ...
                //[normal cum load]
                if (player.cumQ() < 250)
                    outputText("shooting thick ropes of sperm deep inside her as the both of you wail with excitement.");
                //[lots of cum]
                ...
                //[Spermageddon]
                    ...
                    outputText("your " + player.cockDescript(0) + " erupting, blasting a violent torrent of semen deep inside her petite frame, filling her up completely as both your voices rise in screams of pleasure.");
            }
            //[If cock too large]
            else {
        ...
        //Corrupted (event raises fatigue by 20)
            ...
            if (player.cumQ() > 250)
                outputText("  Your spooge explodes from the naga's abused cunt, flooding the desert sands as you black out.<br><br>");
            else
                outputText("<br><br>");
    ...
    if (player.hasVagina()) {
        ...
        if (player.isTaur()) {
            ...
            if (monster.lust >= monster.maxLust()) {
                ...
                if (player.gender == 3)
                    outputText("  As her tail gently squeezes your body, it draws your own " + player.multiCockDescriptLight() + " up against your body, her soft underbelly stimulating the sensitive skin.");
                ...
                //(If PC is Herm i.e. Has a cock(s)){
                if (player.gender == 3)
                    outputText(" and your " + player.multiCockDescriptLight() + " constantly stimulated by the tail wrapped around your body");
...
NagaScene.gooNagaRape = function () {
    ...
    if (player.totalCocks() > 0) {
        if (player.cockTotal() == 1)
            outputText("your " + player.cockDescript(0), false);
        else
            outputText("one of your " + player.multiCockDescriptLight(), false);
    ...
    if (player.gender == 1) {
        ...
        //(normal)
        if (player.cumQ() < 250)
            outputText("you come all over the naga's stomach.");
        else if (player.cumQ() < 1000)
            outputText("you come all over the naga's face and stomach.");
        //(if THE LEVEYS HAVE BROKE RUN FOR THE HILLS)
        else
            outputText("you come violently, covering the naga in a layer of your sperm.");
    }
    //(if female)
    else if (player.gender == 2) {
        ...
        if (player.vaginas[0].vaginalWetness == VAGINA_WETNESS_SLAVERING)
            outputText(", your girlcum gushing out and pooling on the stomach of the naga");
        outputText(".");
    }
    //(if herm)
    if (player.gender == 3) {
        ...
        if (player.vaginas[0].vaginalWetness < VAGINA_WETNESS_SLAVERING)
            ...
            outputText("gushing out to form a pool on the naga's stomach");
        ...
        //(lots of jizz)
            ...
            outputText(" completely covering the naga in a layer of your spooge.");
    }
    //(if genderless)
    if (player.gender == 0) {
...
NagaScene.beePositANagaPlease = function () {
    ...
    if (player.cor < 33)
        outputText("<br><br>With nothing to do but enjoy the tender moment,");
    else if (player.cor < 66)
        outputText("<br><br>Figuring you may as well enjoy the extra stimulation,");
    else
        outputText("<br><br>Reluctantly kissing her back, if only because it makes your egg-spout throb,");
...
NagaScene.nagaRapesPlayer = function () {
    ...
    if ((player.findPerk(PerkLib.BimboBrains) >= 0 || player.findPerk(PerkLib.FutaFaculties) >= 0) && player.hasVagina()) {
        ...
        if (player.lactationQ() >= 50 && player.biggestLactation() >= 1)
            outputText("Your warm milk is leaking from your " + player.nippleDescript(0) + "s the entire time, coating your skin and soaking her hands. At one point, she withdraws a hand and licks the milk from it. You can only assume that she liked it, as she begins to focus even more on your breasts.<br><br>");
        //-------
        ...
        //[If lactating]
        if (player.lactationQ() >= 50 && player.biggestLactation() >= 1)
            outputText("As her mouth sucks and plays with your nipple, your warm, sweet milk begins to flow forth once again, filling her mouth with your delectable cream. Her hand is also absolutely covered by it, and the excess drips down to the dry sand below, absorbed in seconds by the thirsty earth.<br><br>");
        //------
        ...
        //[If Lactating]
        if (player.lactationQ() >= 50 && player.biggestLactation() >= 1)
            outputText("She shares a mouthful of your delicious milk with you.<br><br>");
        //------
    ...
    //too)
        ...
        //(if defeated by lust)
        if (player.lust >= player.maxLust())
            outputText("all your resistance seems to do is make your " + player.cockDescript(0) + " harder.");
        //if defeated by KO)
        else
        ...
        if (player.cor > 75)
            outputText("  The thought somehow makes your " + player.cockDescript(0) + " throb even more.");
        outputText("<br><br>");
        ...
        //(Cock too big)
            ...
            //(Normal)
            if (player.cumQ() < 250)
                outputText("The naga manages to hold the warm cum within her mouth, only causing her cheeks to bulge a little.");
            //(Lots of cum)
            else if (player.cumQ() < 1000)
                outputText("The naga's cheeks balloon as her mouth is filled with your hot, gooey sperm, forced to swallow some of it as her cheeks reach their full capacity.");
            //(Cumception, we need to go deeper)
            else
                outputText("Hot semen blasts from the naga's nose as her mouth is completely filled in seconds. Much of your warm cum is forced down her throat and into her stomach, causing it to distend slightly within her thin frame.");
    ...
    //b) female //http://nekomimichan.org/mg/src/12844585127.jpg
        ...
        //(if uncorrupted)
            ...
            outputText("and while you know you probably won't like it, your " + player.vaginaDescript(0) + " says otherwise.  ");
