...
TentacleBeast.tentacleEntwine = function () {
    ...
    if (gameFlags[TENTACLE_BIND] == 0) {
        ...
        //Fail
            ...
            outputText("While you attempt to avoid the onslaught of pseudopods, one catches you around your " +
                player.foot() +
                " and drags you to the ground. You attempt to reach for it to pull it off only to have all of the other tentacles grab you in various places and immobilize you in the air. You are trapped and helpless!!!<br><br>", false);
            //Male/Herm Version:
            if (player.hasCock())
                outputText("The creature, having immobilized you, coils a long tendril about your penis. You shudder as the creature begins stroking your cock like a maid at a dairy farm in an attempt to provoke a response from you. Unable to resist, your " +
                    player.cockDescript(0) +
                    " easily becomes erect, signaling to the creature that you are responsive to harsher stimulation.<br>", false);
            //Female Version:
            else if (player.hasVagina())
                outputText("The creature quickly positions a long tentacle with a single sucker over your clitoris. You feel the power of the suction on you, and your body quickly heats up.  Your clit engorges, prompting the beast to latch the sucker onto your " +
                    player.clitDescript() +
                    ".<br>", false);
            //Genderless
            else
                outputText("The creature quickly positions a long tentacle against your " + player.assholeDescript() + ". It circles your pucker with slow, delicate strokes that bring unexpected warmth to your body.<br>", false);
...
TentacleBeastScene.tentacleLossRape = function () {
    ...
    if (player.countCocksOfType(ENUM.CockType.HORSE) > 0 && player.cocks[0].cockLength > 15 && player.cocks[0].cockThickness >= 3) {
        if (player.cor < 75 && player.lust < 100)
            outputText("It grabs you before you can get away!<br><br>While you attempt to resist the abomination, its raw muscle mass is too much. ", false);
        ...
        if (player.balls > 0)
            outputText(", and you feel your " + player.ballsDescript() + " rise and drop in unison to the muscular contractions pumping freshly made cum into position for release", false);
        ...
        if (player.balls > 0)
            outputText("Your balls have overfilled the creature!<br><br>", false);
        else
            outputText("Your cum has overfilled the creature!<br><br>", false);
        ...
        player.dynStats("str", 0.5, "spe", -0.5, "int", -1, "lib", 5, "sen", 1, "cor", 1);
        monster.HP = 0;
        if (player.HP == 0)
            player.HP++;
        if (inCombat()) {
    ...
    if (player.hasCock()) {
        ...
        //[has testicles:
        if (player.balls > 0)
            outputText("Your " + player.ballsDescriptLight() + " tighten up against you in preparation for their inevitable release, ready to spray their boiling load into the beast.  ", false);
    ...
    if (player.hasVagina()) {
        outputText("Your " +
            ...
            player.vaginaDescript(0) +
            " remains, rippling with waves of seed that spray inside you in massive, hot globules.  The sticky substance flooding your love canal pushes you over the edge and you orgasm again, spraying more as you cry out in pleasure.<br><br>", false);
    }
    ...
    if (player.hasCock() && player.cumQ() < 1500 && player.ass.analLooseness < 4) {
        ...
        //end (loss)
        player.dynStats("tou", 1, "int", -0.5, "lib", 2, "sen", 1, "cor", 0.5);
        if (inCombat()) {
            cleanupAfterCombat();
            player.orgasm();
        }
    ...
    if (player.hasCock() && player.cumQ() < 1500 && player.ass.analLooseness >= 0) {
        outputText("Just as you think it's over, the tentacle inside your " +
            player.assholeDescript() +
            " begins to swell massively, causing another surge of cum to leave you, and another, and another.  It continues to violate your ass until you black out from exhaustion, the number of loads you've released no longer countable.", false);
        //end (loss)
        player.dynStats("tou", 1, "int", -0.5, "lib", 2, "sen", 1, "cor", 0.5);
        if (inCombat()) {
            cleanupAfterCombat();
    ...
    if (player.hasCock() && player.cumQ >= 1500) {
        ...
        if (player.balls > 0)
            outputText(player.sackDescript() + " and ", false);
        ...
        if (player.balls > 0)
            outputText("r " + player.ballsDescriptLight(), false);
        ...
        //end (victory)
        player.dynStats("tou", 0.5, "spe", -0.5, "int", -0.5, "lib", 1, "sen", 1, "cor", 1);
        monster.HP = 0;
        if (player.HP == 0)
            player.HP++;
        if (inCombat()) {
    ...
    if (player.biggestLactation() >= 3.5 && player.gender > 0) {
        ...
        outputText("You shudder as your " +
        ...
        if (player.gender == 2) {
            ...
            if (player.vaginalCapacity() < 30)
                outputText("The beast senses your excitement and with beguiling speed swiftly impales your " +
                    player.vaginaDescript(0) +
            ...
            //[Female/Loose-Moist Cunt]
                ...
                outputText("The beast senses your excitement and with beguiling speed swiftly impales your " +
                    ...
                    player.vaginaDescript(0) +
                    ", filling the air with lewd squishing sounds.<br>", false);
        ...
        if (player.gender == 3) {
            ...
            if (player.vaginalCapacity() < 30)
                outputText("The beast senses your excitement and with beguiling speed swiftly impales your " +
                    ...
                    player.cockDescript(0) +
                    " deeper into its tight fuck hole.<br><br>", false);
            //[Herm/Loose-Wet Cunt]
            else
                outputText("The beast senses your excitement and with beguiling speed swiftly impales your " +
                    player.vaginaDescript(0) +
        ...
        outputText("You slowly become aware that the beast has slowed its assault on your genitals and soon stops altogether, withdrawing entirely.  The beast lets out an audible gurgle and you smile as you feel the tentacles re-double their assault on your " +
            ...
            ".  The stimulation causes even more milk to gush down the tentacles length.  After a few moments of the increased assault the beast groans and releases you, the tentacles popping off your nipples audibly, spraying your milk about as they release you.<br><br>", false);
        //[Female/Herm]
        if (player.gender >= 2)
            outputText("Your " +
                player.allBreastsDescript() +
                ...
                player.vaginaDescript(0) +
                " with the living dildo.  The idea of turning the tables on the raping beast spurs you on to new heights and you cum quickly around the knobbly shaft, your cunt spasming and milking the bumpy tentacle hard.  As you finish with the tentacle the beast gives a final gurgle and retreats into the forest.", false);
        //[Male]
        else
            outputText("You feel your " +
                ...
                player.cockDescript(0) +
                " slips free the beast lets out a final gurgle of defeat and slithers away into the forest.", false);
        player.dynStats("tou", 0.5, "spe", -0.5, "int", -0.5, "lib", 1, "sen", 1, "cor", 1);
    ...
    if (player.gender == 1) {
        ...
        if (player.cockTotal() > 1)
            outputText("at the base of your " + player.multiCockDescriptLight() + ".", false);
        ...
        if (player.cockTotal() == 1)
            outputText("your member immediately becomes fully erect and pre-cum leaks liberally from your tip.", false);
        else
            outputText("your members immediately become fully erect, pre-cum drizzling from the tips.", false);
        ...
        if (player.cockTotal() == 1)
            outputText("In your daze you see a monstrous, hollow tentacle poised over your furious cock. You scream in shock and protest, but your cries fall upon deaf ears. The tentacle descends upon your penis, now begging for release, and clamps down upon your pubic mound, fully encapsulating your member.", false);
        else
            outputText("In your daze you see " +
                player.cockTotal() +
                ...
                ", all begging for release, and clamps down upon your pubic mound, fully encapsulating your dicks.", false);
    }
    if (player.gender == 2) {
        player.slimeFeed();
        player.dynStats("spe", -1, "int", -1, "lib", 5, "sen", 3, "lus", 20, "cor", 1);
        if (player.cor < 75)
            outputText("It grabs you before you can get away!<br><br>While you struggle valiantly, the beast's raw might is more than a match for you. ", false);
        outputText("Tentacles burst from the mass and bind your arms, legs, and midriff. ", false);
        if (player.cor < 75)
            outputText("You struggle to break free, but the creature only constricts you further, ensuring your immobility. ", false);
        outputText("A quick flex of the tentacles securing your legs leaves you spreadeagled before the maw of the horror.  ", false);
        if (player.cor < 75)
            outputText("Fearing for your life, you scream and struggle for help, but only the apathetic sounds of nature respond.", false);
        outputText("<br><br>" +
            (player.totalBreasts() + 1) +
            " thorny tendrils appear and pierce your breasts and groin. A sharp pain and a burning sensation tear through you, overriding the previous wave of pleasure. You feel fluids being injected into you and a distinctive, agonizing misery flows into your veins.  Your breasts and ", false);
    ...
    if (player.gender == 3) {
        ...
        if (player.cockTotal() > 1)
            outputText(player.multiCockDescriptLight() + " immediately become fully erect and leak pre-cum liberally from their tips.  ", false);
        else
            outputText("member immediately becomes fully erect and pre-cum leaks liberally from your tip.  ", false);
        ...
        if (player.cockTotal() > 1)
            outputText("furious cocks.  ", false);
        else
            outputText("furious cock.  ", false);
        outputText("You scream in shock and protest, but your cries fall upon deaf ears. The tentacle descends upon your ", false);
        if (player.cockTotal() > 1)
            outputText(player.multiCockDescriptLight() + ", now begging for release, and clamps down around your pubic mound, fully encapsulating your members.  ", false);
        else
            outputText(player.cockDescript(0) + ", now begging for release, and clamps down upon your pubic mound, fully encapsulating your member.", false);
    }
    //Call page 2!
...
TentacleBeastScene.tentacleRapeContinuation = function () {
    ...
    if (player.gender == 1) {
        ...
        if (player.cockTotal() == 1)
            outputText("using a sucking-stroking motion on your helpless " +
                player.multiCockDescriptLight() +
                ". The swelling of the ass tentacle pressures your prostate in a paradoxically pleasurable and painful manner. You realize, much to your terror, that this beast is MILKING you of your semen!", false);
        else
            outputText("using a sucking-stroking motion on your " +
                player.multiCockDescriptLight() +
                ".  The swelling of the ass tentacle pressures your prostate in a paradoxical pleasurable and painful manner.  You realize, much to your terror, that this beast is MILKING you of your semen!", false);
    ...
    // Female (Kicks to special scene)
        ...
        if (player.vaginas.length == 1)
            ...
            outputText(player.vaginas.length +
        ...
        if (player.vaginas.length > 1)
            ...
            outputText("clit. ", false);
        player.cuntChange(player.vaginalCapacity() * 0.76, true);
        outputText(" Your body betrays your resistance as pleasure hammers you from crotch to head. After some time, you begin bucking your hips in tandem to the creature's thrusts, drunk with pleasure. As you peak for your orgasm, you feel the creature bottom out inside your womb. Oceans of hot cum flood your " +
    ...
    // Herm
        ...
        if (player.cockTotal() == 1) {
            outputText("A sharp tug tells you that the creature has sealed itself upon your " +
                player.cockDescript(0) +
                ". You see " +
                player.totalBreasts() +
                ...
                player.vaginaDescript(0) +
                ".  ");
            player.cuntChange(player.vaginalCapacity() * 0.76, true, false, true);
            outputText("With freakish coordination, the beast sucks your " +
                player.cockDescript(0) +
                ...
                player.vaginaDescript(0) +
                " faster, leading to a chain of orgasms.<br><br>", false);
            ...
            outputText("The creature's milking tentacles stop moving and you feel the dick-tentacle press sharply against your womb. You feel the thunderous force of hot fluid lance into your body as the creature cums repeatedly inside you, triggering yet another orgasm. The creature cums in surges and shoots repeatedly inside you. Within moments, excess cum spews out of your " +
                player.vaginaDescript(0) +
                " as it cannot hold anymore, but the creature keeps cumming.<br><br>", false);
            ...
            outputText("A sharp tug tells you that the creature has sealed itself upon your " +
                ...
                player.vaginaDescript(0) +
                ".");
            player.cuntChange(player.vaginalCapacity() * 0.76, true, true, false);
            outputText("  With freakish coordination, the beast sucks your " +
                player.multiCockDescriptLight() +
                " and tits while hammering away at your " +
                player.vaginaDescript(0) +
...
TentacleBeastScene.tentacleRapeContinuationForFemales = function () {
    ...
    if (player.vaginas.length == 1) {
        ...
        outputText("Satisfied, the creature drops you smartly and withdraws its limbs from you and lumbers away.  Covered completely in cum, you see that your " +
    ...
    if (player.clitLength < 7 && player.clitLength >= 3.5 && player.findPerk(PerkLib.BigClit) >= 0) {
        player.clitLength += 0.1 + player.cor / 100;
...
TentacleBeastScene.centaurLossGendered = function () {
    ...
    if (player.hasCock()) {
        if (player.cockArea(player.smallestCockIndex()) <= 50) {
            ...
            if (player.cockTotal() > 1)
                outputText("  The creature seems surprised at first to discover such a large brace of cocks, testing their texture and wrapping around each individually.  Your " +
                    player.multiCockDescriptLight() +
                    " responds by wriggling about and tempting the beast to continue its exploration, but the gesture is futile and they're abandoned, though not for long.", false);
            outputText("<br><br>", false);
            outputText("A peculiar sensation rolls over it as an unseen tentacle engulfs you, rippling and milking your " +
                player.cockDescript(0) +
        ...
        // has cock, not engulfable:
            ...
            outputText("More aphrodisiac-toxin pours into you, causing " +
                player.sMultiCockDesc() +
                " to expand. Something bumps up against the tip but can't seem to fit around your " +
                player.cockDescript(0) +
                ".  It continues trying for a while, sending pangs of pleasure down the length.  The tentacle eventually gives up and latches onto the tip, positioned right at the opening to your urethra.  It sucks up your pre-cum as it drips from you, accompanied by a loud suckling noise.", false);
            //[With testicles:
            if (player.balls > 0)
                outputText("The sucking reaches all the way to your " + player.ballsDescriptLight() + ", a spectacularly strange sensation that nevertheless feels wonderful.", false);
            outputText("<br><br>", false);
        }
    }
    // has vagina:
    if (player.hasVagina()) {
        outputText("A squirming tentacle forces its way inside your " +
            player.vaginaDescript(0) +
            ", undulating and squirming as it works its way deeper and deeper.  Your body responds by pumping out more fluid, making the passage of the monstrous thing easier.", false);
        player.cuntChange(32, true, true, false);
        if (player.hasCock()) {
            if (player.cockArea(player.smallestCockIndex()) <= 50)
                outputText("  Your humping appears to not affect the creatures continuing efforts, despite the force of your body.", false);
        }
    ...
    if (player.biggestTitSize() >= 1 && player.lactationQ() <= 0) {
        ...
        if (player.totalBreasts() == 2)
            outputText("  The anus-like tips affix to them.", false);
        else
            outputText("  The anus-like tips attach to one pair as more appear in order to take the others.", false);
    ...
    if (player.ass.analLooseness >= 4) {
        outputText("Your " +
            player.assholeDescript() +
            " makes an inviting target for the squirming mass and it's quick to capitalize.  A particularly bulbous appendage slides deep inside, roiling about in a way that not even your well-trained hole has been treated to.", false);
        if (player.hasCock())
            outputText("  A series of undulating lumps pass over your prostate, pushing out a splash of pre-cum.", false);
    ...
    if (player.biggestTitSize() >= 1 && player.lactationQ() > 0 && player.lactationQ() < 1000) {
        outputText("Roving tentacles latch onto your " +
            player.allBreastsDescript() +
    ...
    if (player.hasCock()) {
        ...
        if (player.cumQ() <= 500)
            outputText("pushing the creature to the edge of its fluid-containing abilities.", false);
        else
            outputText("quickly overfilling the tentacle attached to it; it explodes off of you, freeing your spunk to spray from both you and the retreating beast.  ", false);
    }
...
TentacleBeastScene.centaurLossGenderless = function () {
    ...
    if (gameFlags[TENTACLE_GENDERLESS_CENTAUR] == 0 || player.balls == 0) {
        ...
        if (player.cor < 50 && player.lust < 70)
            outputText("dread", false);
        else
            outputText("anticipation", false);
    ...
    //(Followup scene, if pc has seen above at least once, is unsexed centaur and has balls: -Z)
        ...
        outputText("The tentacle, now joined by a second, hunts around your " +
            player.ballsDescriptLight() +
            ", seeking any organs that might serve as a release valve for their contents.  You stare at it as it searches, quite certain you know what's coming next.<br><br>", false);
        ...
        outputText("Your eyes bulge out as one of the feelers which had been still during your argument pulls away from your " +
            player.sackDescript() +
            " and then returns with a sharp slap; as your vision pinks over under the wave of nausea, the creature releases your legs and you collapse into what can only be assumed is a centaur fetal position.<br><br>", false);
...
TentacleBeastScene.nagaLossGenderless = function () {
    ...
    outputText("Out of nowhere tentacles bind your arms and tail, holding you firm in a matter of seconds.  You struggle to free yourself but can do nothing against the strength of the beast holding you in your current state.  More of the appendages start teasing around your body, as if looking for something.  A handful test the entrance to your " +
        player.assholeDescript() +
        " but evidently that's not what they're after.<br><br>", false);
...
TentacleBeastScene.futaTentacleBadEnd = function () {
    ...
    outputText("The last rays of light fade as the creature closes its beak, trapping you inside. You begin flailing and fighting in sheer panic at the prospect of being eaten alive. As you struggle, countless tentacles wrap around your arms and legs, essentially binding you inside the creature. A thick tentacle forces its way down your mouth and you feel the familiar sensation of salty lust being emptied into your mouth. Your " +
        player.cockDescript(0) +
        " instantly becomes erect, triggering a tentacle to encapsulate your member completely. As this occurs, another limb buries itself deep within your ass.<br><br>", false);
...
TentacleBeastScene.futaTentacleEpilogue = function () {
    ...
    //[Never met Giacomo]
        ...
        outputText('"<i>We cannot leave this unfortunate ' +
            player.mf("man", "woman") +
            ...
            player.mf("man", "woman") +
            ' recover.</i>", the doctor states flatly.<br><br>', false);
