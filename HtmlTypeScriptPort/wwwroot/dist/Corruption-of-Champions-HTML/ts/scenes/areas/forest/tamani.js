...
class Tamani extends Creature {
    ...
    tamaniHypnoTease() {
        ...
        if (selector == 0)
            GUI.outputText('Tamani smiles and shifts her leather straps, pulling one into the puffy gash that is her vagina.  She groans out loud, sliding the studded leather band into her outer lips and sawing it along her clit.  Her whole body blushes as she pulls it free, running a fingertip up the now wet strip of leather, "<i>Mmm, can\'t you see how much my pussy needs a man inside it?  Be a good husband and fuck Tamani full!  You know you want to.</i>"<br><br>');
        if (selector == 1)
            GUI.outputText('Tamani saunters up to you, sliding her fingers down to each side of her pussy and spreading them.  Your eyes are drawn to her honeyed tunnel, unable to look away she gets closer.  She whispers, "<i>Your cock knows what it needs.  Just be a good husband and obey your dick, it KNOWS how badly you need mistress\'s pussy.</i>"<br><br>');
        if (selector == 2)
            GUI.outputText("Tamani turns around and bends down, pressing her hands into the dirt as she kicks her legs apart.  Your stare open-mouthed at her bouncy ass-cheeks and the tantalizingly wet entrance of her slit.  She smirks and offers, \"<i>You've cum so many times inside me, why resist when you can give in and feel that pleasure again today?  Come on husband, don't make Tamani beg...</i>\"<br><br>");
        //Low Hypnosis
        if (liveData.gameFlags[FLAG.TAMANI_TIMES_HYPNOTIZED] < 5) {
            selector = UTIL.rand(3);
            if (selector == 0)
                GUI.outputText("You reluctantly pull your stare away from the heavenly entrance between her legs.  There's an urge to walk over to her and plunge yourself inside her over and over, but you dismiss it.");
            if (selector == 1)
                GUI.outputText("You find it hard to pull your gaze from her inviting twat, but you manage.  You shake your head, clearing away thoughts of fertilizing your wife.  Her rhetoric must be getting to you.");
            if (selector == 2)
                GUI.outputText("No matter the case, her actions shifted a fair bit of your blood-flow to your groin.");
        }
        //Medium Hypnosis
            ...
            if (selector == 0) {
                ...
                if (liveData.player.totalCocks() > 1)
                    GUI.outputText("Each of y");
                else
                    GUI.outputText("Y");
                GUI.outputText("our " + liveData.player.multiCockDescriptLight());
                if (liveData.player.lust > 80)
                    GUI.outputText(" drips pre-cum");
                else if (liveData.player.lust > 40)
                    GUI.outputText(" grows harder");
                else
                    GUI.outputText(" hardens");
                ...
                if (liveData.player.lust < 70)
                    GUI.outputText("  Still, you don't want to fuck her right now!");
            }
            else {
                ...
                if (liveData.player.lust <= 41)
                    GUI.outputText(" thicken perceptibly");
                else if (liveData.player.lust <= 81)
                    GUI.outputText(" twitch eagerly");
                else
                    GUI.outputText("drip pre-cum");
                GUI.outputText(", responding to the overly sensual goblin's body.  You start to approach her, but stop yourself, realizing you were about to pick up your wife and fuck her on the spot.  You know she's not really your wife, but you have a hard time thinking of her as anything else, save for maybe your mistress.");
                if (liveData.player.lust < 70)
                    GUI.outputText("  Regardless, you're resolute in your desire not to fuck her right now!");
            }
        }
        //High Hypnosis
        else {
            ...
            if (selector == 0) {
                ...
                GUI.outputText("You wrench your gaze from the juicy mound before you with great difficulty.  The desire to submit to your wife and fuck her on the spot rages through your body, melting your resistance into liquid lust and pooling it in your groin.  ");
                if (liveData.player.totalCocks() > 1)
                    GUI.outputText("Each of y");
                else
                    GUI.outputText("Y");
        ...
        if (liveData.gameFlags[FLAG.TAMANI_TIMES_HYPNOTIZED] == undefined)
            liveData.gameFlags[FLAG.TAMANI_TIMES_HYPNOTIZED] = 0; // Kludge for save fix.
...
class TamaniScene {
    ...
    static tamaniVictoryMenu() {
        ...
        if (liveData.player.lust >= 33 && liveData.player.totalCocks() > 0 && liveData.gameFlags[FLAG.SFW_MODE] <= 0) {
            ...
            if (liveData.player.cockThatFits(liveData.monster.analCapacity()) >= 0)
                GUI.addButton(1, "Buttfuck", this.tamaniAnalSex);
            else
                GUI.addButtonDisabled(1, "Buttfuck", liveData.player.cockTotal() == 1 ? "Your cock is too big to fit in Tamani's ass." : "None of your cocks will fit in Tamani's ass.");
            if (!liveData.tamanipreg.isPregnant() && liveData.player.canOvipositSpider())
                GUI.addButton(2, "Lay Eggs", this.tamaniBeaten);
    ...
    static tamaniSexWon() {
        ...
        if (liveData.player.cockArea(x) <= 90) {
            ...
            if (liveData.player.totalCocks() > 1)
                GUI.outputText(" and tight asshole");
            ...
            GUI.outputText("You rock back and forth methodically, treating Tamani like a tight cock-sleeve.  The goblin slut's hands rub her belly, not even attempting to pull her face out of the mud as she moans and giggles like a whore.  You keep working her cunt like a ");
            ...
            if (liveData.player.totalCocks() == 1)
                GUI.outputText("pussy squeezing tightly as she cums hard.<br><br>");
            else
                GUI.outputText("holes squeezing tightly as she cums hard.<br><br>");
            if (liveData.player.totalCocks() > 1) {
                GUI.outputText("Each of your " +
                    liveData.player.multiCockDescriptLight() +
                ...
                if (liveData.player.cumQ() >= 250)
                    GUI.outputText("  She pants, turning her head in the mud to watch as her belly visibly inflates, stuffed totally full of cum. ");
                if (liveData.player.cumQ() >= 500)
                    GUI.outputText("  In no time at all a river of jism pours from her, pooling below as you overwhelm her body's capacity to store spunk.");
            }
            else {
                GUI.outputText("Your " +
                    liveData.player.multiCockDescriptLight() +
            ...
            if (liveData.player.balls > 0)
                GUI.outputText("sexy balls have ");
            else
                GUI.outputText("sweet cock has ");
        ...
        //Too big? Jerk off with feet and bukkake
            ...
            if (liveData.player.totalCocks() > 1)
                GUI.outputText("any of your massive members");
            else
                GUI.outputText("your massive member");
            GUI.outputText(", so you grab her by the ankles and wrap her soft-soled feet about yourself.   You start jerking yourself off, using Tamani as a cute but expendable masturbation aid.  She chews a fingernail and massages her " +
                this.tamaniChest() +
                " as she watches you, doing her best to put on a show.  The little slut seems to like it.<br><br>");
            GUI.outputText('Her feet start to grow slick with your sweat and pre-cum, sliding effortlessly along the length of your shaft as you continue to bring yourself towards orgasm.  You watch while Tamani dips her fingers into the slick folds of her hungry cunt, getting off on being used in such a perverse manner.  She coos, "<i>How do my feet feel ' +
            ...
            if (liveData.player.cumQ() >= 500)
                GUI.outputText("  She sputters, blowing some of the caked up semen off her face so she can breathe.  The slut gathers up the goopy mess as you continue to paint her, alternatively devouring it with her mouth and shoveling drippy handfuls into her eager cunt.");
    ...
    static tamaniAnalSex() {
        ...
        GUI.outputText('The slutty creature\'s moans turn to shouts as the sensation overwhelms her, and she bucks wildly back against you, face still in the dirt and tongue lolling out of her mouth.  "<i>Oh, fuck yes, harder, HARDER!</i>" she bursts forth wildly.  You oblige and pick up the intensity, absolutely ravaging her tight ass now with the motions of your ' +
            liveData.player.cockDescript(x) +
    ...
    static spareTamani() {
        GUI.clearOutput();
        GUI.outputText('You tell her that should she ever cross paths again, she\'s going to have a really bad time. "<i>Yes...</i>" Tamani sniffles, tears welling up in her face. She gets up and leaves her satchel on the ground before running into the foliage, never to be seen again.');
    ...
    static postTamaniRemoval() {
        ...
        if (liveData.tamanipreg.isPregnant())
            liveData.tamanipreg.knockUpForce(0, 0); //Clear Tamani Pregnancy.
        COMBAT.cleanupAfterCombat();
    }
    //Tamani Ovipositing Scene
    static tamaniBeaten() {
        GUI.clearOutput();
        GUI.outputText("With the goblin defeated, you're finally free to tend to your body's true desires, even if they may be different than the ones the goblin was intent on stoking.  You shed your [armor] like a second skin, shrugging out of the offending garments with ease.  With your nude form revealed, Tamani ");
        if (liveData.player.hasCock())
            GUI.outputText("licks her lips in anticipation, eyes locked on [eachCock]");
        else
            GUI.outputText("frowns in displeasure, noting your lack of a penis");
        GUI.outputText(".  She's clueless to the growing size that dangles beneath your spider-half, the secret drider treasure that you usually keep retracted.  A dollop of green goo drips from the tip of the semi-hard organ, still hidden from the goblin matron.");
        ...
        if (liveData.gameFlags[FLAG.TIMES_OVIPOSITED_TAMANI] == 0)
            GUI.outputText("my baby-" + liveData.player.mf("daddy", "momma"));
        else
            GUI.outputText("my egg-obsessed arachnophile");
        GUI.outputText(".</i>\"  Soon, the purple-haired skank has assumed the position, though she has the temerity to look back over her shoulder and lick her lips.  Her smouldering gaze is just beckoning for cock, but in this case, she'll be making do with your ovipositor.  You look away from her hungry eyes down to the green bubble-butt, admiring the swell of her cheeks, her puckered anus, and best of all, her sopping wet nethers.");
        ...
        if (liveData.gameFlags[FLAG.TIMES_OVIPOSITED_TAMANI] > 0)
            GUI.outputText(", or in your case even more eggs");
        ...
        if (liveData.player.hasCock())
            GUI.outputText("  White coats the goblin from her shoulders up, the only evidence of your male satisfaction.  You were so focused on the act of impregnation and the unique pleasure that it provides that you barely noticed [eachCock] erupting in sympathetic pleasure.");
    ...
    static tamaniSexLost() {
        ...
        if (liveData.player.HP < 1)
            GUI.outputText("lie there, unable to fight back.  ");
        else
            GUI.outputText("touch yourself, giving in to the tiny goblin's desire to breed.  ");
        GUI.outputText("She advances, her bare feet padding across the soil as she rummages in a faded leather pouch for something.  Her eyes light up as she finds whatever it is she was looking for.  In her hand is a ribbed silver ring, covered on every surface with numerous carved runes.  You whimper and back up, knowing she intends to fit it over your " +
            liveData.player.cockDescript(0) +
            ", but Tamani only 'tsks' and shakes her head.<br><br>");
        GUI.outputText("\"<i>You were such a naughty boy, teasing me with that juicy dick and then pretending like you didn't want this.  I'll have to punish you for being bad, that's all there is to it,</i>\" she explains, pushing your hands away and fitting the ring around your flare.  ");
        if (liveData.player.cocks[0].cockThickness >= 5)
            GUI.outputText("Impossibly, despite your titanic girth, the ring somehow expands to squeeze onto you, though it's painfully tight.<br><br>");
        else
            GUI.outputText("Somehow the ring fits onto you perfectly, as if it was made exactly for your " + liveData.player.cockDescript(0) + ", though it remains painfully tight.<br><br>");
        GUI.outputText("\"<i>Don't fret, " +
        ...
        if (liveData.player.balls > 0)
            GUI.outputText("cups your " + liveData.player.ballsDescriptLight() + " tightly");
        else
            GUI.outputText("caresses you around the ring");
        GUI.outputText(' and asks, "<i>So are you sorry for making me wait for my cream ' +
            liveData.player.mf("stud", "slut") +
        ...
        if (liveData.player.balls > 0)
            GUI.outputText("Your balls are getting VERY swollen and tender.");
        else
            GUI.outputText("Your body aches inside, feeling bloated and ready to pop.");
        if (liveData.player.cor < 50)
            GUI.outputText("  You have no choice, you'll say what she wants you to say.<br><br>");
        else
            GUI.outputText("  Being denied like this was kind of fun, but you REALLY need to get off now, so you'll say what she wants you to say.<br><br>");
        ...
        if (liveData.player.cor > 50)
            GUI.outputText("blushing hotly and doing your best to look like a submissive toy.<br><br>");
        else
            GUI.outputText("blushing in shame and embarrassment, but meaning every word.<br><br>");
        GUI.outputText("She takes the empty bottle and presses it against your " +
        ...
        if (liveData.player.cumQ() >= 250)
            GUI.outputText("  The bottle quickly overflows, but Tamani happily sets it aside and slides her own cunt under you, letting you pump the rest of your seed into her fertile womb.");
        if (liveData.player.cumQ() >= 500)
            GUI.outputText("  She coos happily when you still manage to pump enough jism into her to bloat her belly, but even with your impressive fertility, the orgasm has to end.");
        GUI.outputText("<br><br>");
        GUI.outputText("Tamani removes the ring once you have finished.  She seals the bottle and places it in her pouch as you drop to the ground, exhausted.  All you can do is watch as she walks away, her ass swaying confidently from side to side.  Your last thought before you pass out is how much easier it would've been to just fuck her.");
    ...
    static tamaniSexLetHer() {
        ...
        if (COMBAT.inCombat()) {
            ...
            if (liveData.player.isTaur()) {
                if (liveData.player.HP < 1)
                    GUI.outputText("You stumble, legs weaving drunkenly as you try to keep your feet.  ");
                else
                    GUI.outputText("You stumble, legs weaving drunkenly as " + liveData.player.sMultiCockDesc() + " ache and drool with need.  ");
                GUI.outputText("Tamani sidles up to you and pats your flank, cooing, \"<i>There there horsey, everything's going to be okay.  Momma Tamani's going to take really good care of you.</i>\"  Before you know it your " +
                    liveData.player.armor.equipmentName +
            ...
            //Not taur
                ...
                if (liveData.player.HP < 1)
                    GUI.outputText("You collapse, unable to fight back any longer.  For her part, Tamani seems disappointed with the outcome, but approaches your prone form in spite of your sorry appearance.");
                else
                    GUI.outputText("You tear off your clothes in a frenzy of lust and stagger towards Tamani, openly fondling your groin.   She smirks and pushes you hard, throwing you off balance and onto your back.");
                ...
                if (liveData.player.cockTotal() == 1)
                    GUI.outputText("an erection emerges");
                else
                    GUI.outputText("erections sprout up like weeds");
        ...
        //Voluntary submission
            ...
            //Taur
            if (liveData.player.isTaur()) {
                GUI.outputText("You meekly undress and ask Tamani how she would like to take you today.  She titters and slowly walks around you, taking her time, taking in your formidable bestial body and your " +
                    liveData.player.multiCockDescriptLight() +
        ...
        if (liveData.player.isTaur()) {
            ...
            GUI.outputText("Such is your arousal you find you can support her easily, but the sensation makes you cringe, particularly as Tamani sets about her work in earnest. Hanging onto your stomach she slathers your head with attention, her tingling lips and wet tongue all over your tip until you can barely stand it, beading pre-cum into her hungry mouth.<br><br>");
            //If <18 Inches
            if (liveData.player.cocks[0].cockLength <= 18) {
                GUI.outputText("Before you can get too far she stops and agonizingly shifts her attention; you feel her transfer her weight to her hands, and then the rough but soft bottoms of her feet work their way down to the other end of your " +
                    liveData.player.cockDescript(0) +
            ...
            //If >18 Inches
                ...
                GUI.outputText("Before you can get too far she stops and agonizingly shifts her attention; you feel her rough but soft bottoms of her feet work their way down to the other end of your " +
                    liveData.player.cockDescript(0) +
                ...
                if (liveData.player.balls == 0)
                    ...
                    GUI.outputText("You find yourself wondering vaguely where a goblin learns these kinds of gymnastics, before the goblin in question drives all thoughts out of your head by beginning to mash her soft soles into your " +
                        liveData.player.ballsDescriptLight() +
                        ".<br><br>");
                ...
                if (liveData.player.cumQ() < 1200) {
                    GUI.outputText("With a ragged sigh, you reach your peak. Your first blast of jizz goes straight into Tamani's mouth, but you feel the goblin nimbly and hastily changing her position so that the very tip of your " +
                        liveData.player.cockDescript(0) +
                    ...
                    GUI.outputText("When you have finished, you feel an intense feeling of relief as Tamani finally lets go of your underside and drops to the floor. She happily paws off the jizz coating her face and attempts to stuff more into her dripping cunt as she waddles around to face you. \"<i>Who knew that riding a horsie could be so much fun! We're going to have to do that again, stud. Next time I might make it a gallop instead of a canter. Won't that be nice? Until next time, big " +
                        liveData.player.mf("boy", "girl") +
                        ".</i>\" She swaggers off, leaving you to wonder if you'll ever be in a fit enough state to let her near your cock again, and where the nearest whereabouts of a gallon of water is.<br><br>");
                }
                //High cum:
                else {
                    GUI.outputText("With a bellowing roar, you cum with mind blowing force. Your churning balls blast out a river of gooey fluid, the first glorious arc of which flies straight past your human front and paints the tree in front of you. Totally insensate, you can do nothing for entire minutes but stand there and cum, over and over again, sweat dripping off your frame, until the entire area underneath you is covered with your spooge.<br><br>");
                    ...
                    if (liveData.player.balls > 0)
                        GUI.outputText("Your " + liveData.player.ballsDescriptLight() + " ache and you feel very thirsty indeed.");
                    else
                        GUI.outputText("You feel very thirsty indeed.");
                    GUI.outputText(" It's only when you manage to drag your mind away from your own privations that you suddenly wonder where Tamani went. She isn't still attached to you, is she? It is then that a small lump in the center of the vast puddle in front of you stands up and begins to happily wipe herself down, taking care to stuff as much of the seed plastering her into herself as she can.<br><br>");
                    GUI.outputText('"<i>Wheeeeeeeee,</i>" she says, with a grin which almost splits her face in half. "<i>Who knew that riding a horsie could be so much fun! We\'re going to have to do that again, stud. Next time I might make it a gallop instead of a canter. Won\'t that be nice? Until next time, big  ' +
                        liveData.player.mf("boy", "girl") +
                        ".</i>\" She swaggers off, leaving you to wonder if you'll ever be in a fit enough state to let her near your cock again, and where the nearest whereabouts of a gallon of water is.<br><br>");
                }
            }
        ...
        //Not-taurs
            ...
            GUI.outputText("She gets down on all fours, crawling up your legs until her gloss-covered lips reach your " +
                liveData.player.multiCockDescriptLight() +
            ...
            if (liveData.player.cockArea(0) <= 90) {
                GUI.outputText("Tamani turns about, dragging her wet gash across you and giving you a nice view of her plump butt-cheeks.  The soft skin of her palms squeezes around you as she lifts you up, positioning your " +
                    liveData.player.cockDescript(0) +
                ...
                if (liveData.player.cocks[0].cockLength >= 12)
                    GUI.outputText("Her body visibly stretches around you, and you silently thank whatever gods or demons adapted goblins to be able to fulfill this role.  ");
                ...
                if (liveData.player.biggestTitSize() >= 2)
                    GUI.outputText("Your " + liveData.player.allBreastsDescript() + " bounce and jiggle obscenely as the goblin twists and pulls, abusing your nipples.");
                else
                    GUI.outputText("Your " +
                        liveData.player.nippleDescript(0) +
                ...
                GUI.outputText('Tamani plays your body like a fiddle, keeping you near the edge as she noisily orgasms over your shaft again and again.  You can tell she\'s starting to tire, so she gives you a kind look and begins to pick up the tempo.   She whispers softly, "<i>Go ahead, cream my honeypot until I can barely walk.</i>"<br><br>');
                GUI.outputText("That's all the encouragement you need.  ");
                if (liveData.player.balls > 0)
                    GUI.outputText("Your " + liveData.player.ballsDescriptLight() + " twitch powerfully, venting their contents, releasing a jet of hot cum into the goblin.");
                else
                    GUI.outputText("Your body's internal muscles twitch, pushing a jet of hot cum into the goblin.");
                GUI.outputText("  Tamani wriggles happily, giving your nipples one last tweak before releasing them to play with her own.  Rope after rope of your goopey jism paints the inside of her canal, and the happy slut only begs for more the whole time.");
                if (liveData.player.cumQ() >= 250)
                    GUI.outputText('  After a while you can feel the pressure building inside her, starting to give her a bit of a belly.  Tamani rubs it and begs, "<i>More!</i>"');
                if (liveData.player.cumQ() >= 500)
                    GUI.outputText("  Your body fulfills her requests and more, stuffing her until she's positively pregnant and your spunk is squirting from her entrance with each wave of sticky spooge.");
                else
                    GUI.outputText("  Sadly, you only manage a few more spurts before your orgasm ends.");
                GUI.outputText("<br><br>A sudden draft of cold air blows across your suddenly released member, making you shiver.   You look over at Tamani, watching your seed run down her thighs as she primps her hair and does a quick touch-up of her make-up.  She finishes up and blows you a kiss, bidding you farewell, \"<i>I'll be back for more if this one doesn't take");
                if (liveData.player.cumQ() < 250)
                    GUI.outputText(".  I really enjoy our fucks " + liveData.player.mf("stud", "hun") + ", and I can't wait to 'bump' into you again.");
                else {
                    GUI.outputText(".  You make sure and let your ");
                    if (liveData.player.balls > 0)
                        GUI.outputText(liveData.player.ballsDescriptLight() + " fill back up with cum, ok?");
                    else
                        GUI.outputText(liveData.player.cockDescript(0) + " fills back up with cum, ok?");
                }
            ...
            //(Doesnt fit)
                ...
                GUI.outputText("Tamani turns about, dragging her wet gash across you and giving you a nice view of her plump butt-cheeks.  She looks over her shoulder and sucks on one of her lacquered fingernails, looking equal parts mischievous and sexy.   You rock your hips under her until she gives your ");
                if (liveData.player.balls > 0)
                    GUI.outputText(liveData.player.ballsDescriptLight());
                else
                    GUI.outputText(liveData.player.cockDescript(0));
                GUI.outputText(" a rough slap.  Whimpering, you still your rebellious hips and allow her to play you like the world's most sensitive fiddle.  Tamani pulls your " +
                    liveData.player.cockHead() +
                    " up and rocks herself against it, openly moaning as you dribble pre into her sopping gash.<br><br>");
                GUI.outputText('"<i>Oooh, you\'re too big for little ol\' me,</i>" moans the goblin, easing back a bit, "<i>but I have something that might make you cum just as hard.   I\'ll just make sure to catch all your cum with my cunny!</i>"    She pulls out a hot-pink dildo and gives it a lick.  You watch in horror as a drop of thick fluid leaks from the tip.  No doubt Tamani intends for you to take take the entire thing in one way or another.<br><br>');
                GUI.outputText("You pout, protesting meekly as she ");
                if (liveData.player.balls > 0)
                    GUI.outputText("lifts up your " + liveData.player.ballsDescriptLight());
                else
                    GUI.outputText("leans forwards, looking between your legs");
                GUI.outputText(", and places the dildo's leaking tip against your " +
                ...
                GUI.outputText("Tamani rocks back and forth, sliding herself against your " +
                    liveData.player.cockDescript(0) +
                ...
                if (liveData.player.hasVagina())
                    GUI.outputText("cock's base ");
                else
                    GUI.outputText("prostate ");
                GUI.outputText(' with delicious intensity.  You quiver underneath her, panting and moaning, totally subservient to the tiny girl giving your body the sexual workout of a life-time.  She slides back, her cunt squishing back and forth against your prick-head, driving you wild.  She coos, "<i>Go ahead and cum stud,</i>" while squeezing your ');
                if (liveData.player.balls > 0)
                    GUI.outputText("balls");
                else
                    GUI.outputText("crotch");
                GUI.outputText(', "<i>let out all that hot sticky spunk for Mistress Tamani.</i>"<br><br>');
                GUI.outputText("Your body immediately and happily complies, squirting a jet of thick white jism onto the goblin's jiggling ass-cheeks.  She slides down, aligning her cunt with your swollen cock-tip, taking the next squirt directly between her lips as she rubs against you.  Tamani slaps the dildo in your " +
                    liveData.player.assholeOrPussy() +
                ...
                if (liveData.player.cumQ() >= 250) {
                    ...
                    if (liveData.player.cumQ() >= 500)
                        GUI.outputText(" as you keep cumming, splattering her tits and face, soaking the little goblin slut");
                    GUI.outputText(".  ");
                }
                ...
                GUI.outputText('Tamani eases up off of you, dripping a mixture of sexual fluids and stretching as if she had just completed a long workout.  The slutty goblin winks at you and waves, "<i>Thanks for the cum.  Be sure and take some time to refill.  I wanna be soaked again next time!</i>"<br><br>');
                //Combat end:
                if (COMBAT.inCombat()) {
                    //TODO CHECK ORDER
                    GUI.outputText("You black out, exhausted from the ordeal.");
    ...
    static tamaniHypnosis() {
        ...
        GUI.outputText("Tamani crooks her finger and you come running, already feeling yourself straining against your " +
            liveData.player.armor.equipmentName +
            ' for a chance to fuck your wife and mistress.  She giggles, "<i>Ohhh, how sweet, you\'re so ready to do your duty.</i>"<br><br>');
        GUI.outputText("The goblin traces her fingers over the bulge in your " +
            liveData.player.armor.equipmentName +
        ...
        GUI.outputText('She smirks and bends over, looking back up at you from between her legs.  She offers, "<i>Go ahead and stick it in.  A horny ' +
        ...
        if (primary >= 0) {
            ...
            if (liveData.player.hasSheath())
                ...
                GUI.outputText("crotch");
            GUI.outputText("<br><br>");
            GUI.outputText("You wait a second, adjusting to the feeling of being completely contained by such a warm, wet hole before you slowly push her away.  The slick walls of her tunnel clench around you as you pull her back, massaging the parts of you still inside her.  ");
            if (cocks > 1) {
                if (primary > 0)
                    secondary = 0;
                else
                ...
                GUI.outputText("She moans and pants, babbling about how good you are to her the whole time.");
            GUI.outputText("  You don't stop until your " + liveData.player.cockHead(primary) + " is the only thing inside her.<br><br>");
            GUI.outputText("  The second thrust is even better than the first.  It's slippery, hot, and passionate, and the idea of slowing down or stopping for any reason is inconceivable.  Tamani pants with each thrust, pawing at her bouncing breasts, grabbing one by the nipple with a free hand and moaning loudly");
            if (cocks > 0)
                GUI.outputText(" around your " + liveData.player.cockDescript(secondary));
            GUI.outputText(". Her tiny form jiggles, ");
            if (liveData.player.tallness > 55)
                GUI.outputText("lifting off the ground as you get into it, giving yourself over to her sex.");
            else
                GUI.outputText("nearly falling over as you give your body over to her sex.");
            GUI.outputText("  She visibly orgasms, shaking and trembling from head to toe as her pussy milks your " + liveData.player.cockDescript(primary) + ".  It only lasts a few seconds, though they feel like heaven, before her body goes nerveless.<br><br>");
            ...
            if (liveData.player.skinType == ENUM.SkinType.SKIN_TYPE_FUR)
                GUI.outputText("matted");
            else
                GUI.outputText("slicked");
            GUI.outputText(" " +
            ...
            if (cocks == 1)
                GUI.outputText("her tongue hanging out as she pants nonsensical pleasure-noises.");
            else
                GUI.outputText("and her mouth is too full of " + liveData.player.cockDescript(secondary) + " to do anything but gurgle out nonsensical pleasure-noises.");
            ...
            if (liveData.player.hasKnot(primary))
                GUI.outputText("Your knot balloons inside her, locking her in place");
            else if (liveData.player.cocks[primary].cockType == ENUM.CockType.HORSE)
                GUI.outputText("Your flare widens, actually holding itself inside Tamani's womb");
            ...
            else if (liveData.player.cocks[primary].cockType == ENUM.CockType.TENTACLE)
                GUI.outputText("Your tentacle-cock's tip widens, becoming mushroom-like inside Tamani's womb");
            else
                GUI.outputText("Your sensitive head swells slightly in time with the contractions of your orgasm");
            GUI.outputText(" as you begin to fill your wife's womb with seed.   The orgasm is intense");
            if (liveData.player.cumQ() < 50)
                GUI.outputText(", but brief, and after a few moments of spasming pleasure, the flood of cum slows to a trickle.");
                ...
                if (liveData.player.cumQ() < 1000)
                    GUI.outputText("  Though your orgasm tapers off, the additional seed squirts from between her thighs, trickling down to a slow drip of viscous white goo.");
                else
                    GUI.outputText("  Trapped inside her and cumming as hard as ever, you raise the pressure inside the goblin until your seed is washing out of her cunt around you, puddling on the ground.");
                if (liveData.player.cumQ() >= 1000 && liveData.player.cumQ() < 4000)
                    GUI.outputText("  It does eventually end, but not until a sizable puddle has formed between the pair of you.");
                else
                    GUI.outputText("  It lasts seemingly forever, and by the time you're done, the two of you are standing in a puddle inches deep.");
            }
            if (cocks > 1) {
                ...
                if (liveData.player.cumQ() > 300) {
                    ...
                    if (liveData.player.cumQ() > 600)
                        GUI.outputText("  A few seconds later she pulls back coughing, and the perky goblin takes the rest of your load in her face.");
                }
                else
                    GUI.outputText(".");
            }
            ...
            if (liveData.player.tallness > 55)
                GUI.outputText(" in spite of the height differential");
            GUI.outputText(".  Tamani regains her senses and pushes you back, waddling slightly after the intense fuck.   White jism and clear feminine fluids drool down her thighs, and your mistress couldn't be happier.  She pets your still-dripping " +
            ...
            GUI.outputText("A surge of pride spreads through you at her praise, and you lean down to give your wife a long french kiss before heading back to camp.  On the way back your head slowly clears, and you wonder what came over you back there?!");
            this.tamaniKnockUp();
            liveData.player.orgasm();
        ...
        //[DOESNT FIT]
            ...
            if (liveData.player.biggestTitSize() < 1)
                GUI.outputText("on your chest ");
            else
                GUI.outputText("between your " + liveData.player.breastDescript(0) + " ");
            GUI.outputText(' and commands, "<i>Stop, you\'re just too big of a man for Tamani to do it the old fashioned way.</i>"<br><br>');
            GUI.outputText("Disgruntled but obedient, you stop your vaginal assault and release your wife.  It seems she has another way for your aching " +
                liveData.player.cockDescript(0) +
            ...
            GUI.outputText("You do as you are told, like a good husband, and Tamani stays close, bound to you by a flexible, artificial umbilical.  She sits next to you, propping her feet up onto your " +
                liveData.player.cockDescript(0) +
            ...
            GUI.outputText("With both her feet locked together, wrapped around most of your sensitive " +
            ...
            if (liveData.player.cor < 33)
                ...
                GUI.outputText("command");
            GUI.outputText(' her not to stop.  Tamani starts to giggle, but bursts into a moan as her fingers slide along her easily visible clit. She pants, "<i>Hurry up and come for Tamani baby, I wanna feel you pumping your cum inside me!</i>"<br><br>');
            GUI.outputText("Obedient until the end, you obey your tiny mistress and reach your climax.  You gasp out loud, twitching between her feet as your cum slowly boils out from your body.  Tamani watches in fascination as your urethra bulges out, actually shifting the position of her feet to make room for your jism.  White goop rockets out of your urethra, pumping through the tube and into your wife's waiting gash.  She squeals and dribbles clear fluid underneath the tube, actually getting off from the feeling of being impregnated.");
            if (liveData.player.cumQ() > 250)
                GUI.outputText("  In no time, you've filled her to capacity, and white mixes in with the clear sexual fluids that are dripping from the goblin's cunt.");
            if (liveData.player.cumQ() > 700)
                GUI.outputText("  A few more blasts visibly bloat her belly, and Tamani pulls her hands away from her pussy to stroke the slowly rounding dome on her abdomen.");
            if (liveData.player.cumQ() > 1500) {
                ...
                if (liveData.player.cumQ() > 3000)
                    GUI.outputText(", and by the time you finish, both of you are in a thick puddle of spunk");
                GUI.outputText(".");
            }
            if (cocks > 1) {
                if (cocks == 2)
                    GUI.outputText("  Your other" + liveData.player.cockDescript(1));
                else
                    GUI.outputText("  Each of your other " + liveData.player.multiCockDescriptLight());
            ...
            GUI.outputText("<br><br>");
            GUI.outputText("Tamani smiles as she pulls the tube out and stands up.  White jism and clear feminine fluids drool down her thighs, and your mistress couldn't be happier.  She peels her device off your still-dripping " +
                liveData.player.cockDescript(0) +
            ...
            GUI.outputText("A surge of pride spreads through you at her praise, and you get up to give your wife a long french kiss before heading back to camp.  On the way back your head slowly clears, and you wonder what came over you back there?!");
            this.tamaniKnockUp();
            liveData.player.orgasm();
            liveData.player.dynStats(["int", -0.5], ["sen", -1]);
        }
    ...
    static tamaniFemaleEncounter() {
        ...
        GUI.clearOutput();
        GUI.outputText("A goblin leaps out from behind a rock outcropping.  She keeps her arms folded across her " +
            this.tamaniChest() +
    ...
    static tamaniFemaleNo() {
        ...
        if (liveData.player.cor < 33)
            GUI.outputText("What a slut.");
        else if (liveData.player.cor < 66)
            GUI.outputText("How odd.");
        else
            GUI.outputText("You hope she misses a few.");
        GUI.doNext(Camp.returnToCampUseOneHour);
    }
    ...
    static preferTamaniFemdom() {
        ...
        GUI.outputText('"<i>You\'re into girls, huh?</i>" Tamani laughs, turning around and giving her fat butt a playful swat.  You watch as she does it a second time, laughing more at you than <i>with</i> you now, and then turns back around.  "<i>Tell you what then, slut! I\'ve had crap luck today finding a good stud, so I\'ll make you a deal.</i>"');
    ...
    static declineTamaniFacesits() {
        ...
        GUI.outputText('<br><br>The curvy goblin kicks you with a snarl, making you instinctively grab at one [leg] and hop around on the other - until she kicks it too, knocking you down.  "<i>Fine, bitch. Have it your way. But if I find you taking <b>my</b> cocks again, you\'re going to be in trouble!</i>"  She darts off before you can get a word in edgewise, leaving you alone.');
    ...
    static tamaniFirstTimeConsentual() {
        ...
        this.tamaniKnockUp();
        GUI.clearOutput();
        GUI.outputText("You almost can't believe your good fortune.  Finally you meet a creature willing to talk instead of just launching into violent rape.   Her direct advances were so crude and overtly sexual that you felt yourself stiffening before she could even finish her offer.   Your decision was made by the tent in your " +
            liveData.player.armor.equipmentName +
            ".  You'll give Tamani exactly what you both want.<br><br>");
        GUI.outputText("Her hips sway seductively as she approaches with her right hand dipping into the moist honeypot of her sex.  You disrobe, throwing your " +
            liveData.player.armor.equipmentName +
        ...
        GUI.outputText("She breaks the kiss and smiles, licking the shining purple lipstick she wears as she whispers in your ear, \"<i>Mmmhmm, I knew your juicy cock just couldn't resist a wet and ready pussy like mine.  I made sure to lace my lipstick with fertility enhancing chemicals too, so we'll be nice and messy.</i>\"<br><br>");
        GUI.outputText("As if to emphasize her point, she curls her toes around your " +
            liveData.player.cockDescript(0) +
        ...
        if (liveData.player.balls > 0)
            GUI.outputText("Your " + liveData.player.ballsDescriptLight() + " swell with seed, spurring your desire to new heights.");
        else
            GUI.outputText("Something inside you swells up with seed, spurring your desire to new heights.");
        GUI.outputText("  You NEED to fuck her pussy full - NOW.<br><br>");
        //(FITS)
        if (liveData.player.cockArea(0) <= 90) {
            GUI.outputText("Tamani breaks the kiss and gives you a coy smile as she shimmies down your body, dropping her moist cunt onto your " +
                liveData.player.cockDescript(0) +
                "'s " +
                liveData.player.cockHead() +
            ...
            if (liveData.player.balls > 0)
                GUI.outputText(" and drips from your " + liveData.player.ballsDescriptLight());
            ...
            if (liveData.player.cor > 66)
                GUI.outputText("  Your " + liveData.player.cockDescript(0) + " twitches at the thought, ready and wanting more.");
        }
        //(TOO BIG)
        else {
            GUI.outputText("Tamani breaks your sloppy kiss and shimmies down your body, clutching tightly to your " +
                liveData.player.cockDescript(0) +
                " and " +
                liveData.player.skinDesc +
    ...
    static tamaniFirstTimeRefusal() {
        //TODO Tamani Sprite spriteSelect(56);
        GUI.clearOutput();
        GUI.outputText("Tamani's eyes widen in surprise, \"<i>Don't let the size fool you, big " +
            liveData.player.mf("boy", "girl") +
    ...
    static tamaniMaleRepeatEncounter() {
        ...
        if (liveData.tamanipreg.isPregnant() || liveData.gameFlags[FLAG.TAMANI_NUMBER_OF_DAUGHTERS] > 0)
            GUI.outputText("While exploring, you're startled by the feeling of tiny hands stroking the insides of your thighs.  You look down and find Tamani there, grinning wolfishly,  \"<i>Ready for another fuck, big " +
                liveData.player.mf("boy", "girl") +
                '?"<br><br>');
        // Else check for hypnoslut scene (large hypnosis number, then 50% chance)
        else
            GUI.outputText("While exploring, you're startled by the feeling of tiny hands stroking the insides of your thighs.  You look down and find Tamani the goblin there, grinning with desire, \"<i>Ready to stuff me with cum?  I'm not taking no for an answer this time.</i>\"<br><br>");
        if (liveData.gameFlags[FLAG.TAMANI_TIMES_HYPNOTIZED] > 19 && UTIL.rand(2) == 0) {
            this.tamaniHypnosis();
            return;
        }
    ...
    static tamaniKnockUp() {
        ...
        if (liveData.player.findPerk(PerkLib.MaraesGiftStud) >= 0)
            liveData.gameFlags[FLAG.TAMANI_PREGNANCY_COUNT] += 3; //TODO Check findPerk Thing
        if (cum >= 50 && UTIL.rand(2) == 0)
            liveData.gameFlags[FLAG.TAMANI_PREGNANCY_COUNT]++;
        if (cum >= 100 && UTIL.rand(2) == 0)
            liveData.gameFlags[FLAG.TAMANI_PREGNANCY_COUNT]++;
        if (cum >= 200 && UTIL.rand(2) == 0)
            liveData.gameFlags[FLAG.TAMANI_PREGNANCY_COUNT]++;
        if (cum >= 300 && UTIL.rand(2) == 0)
            liveData.gameFlags[FLAG.TAMANI_PREGNANCY_COUNT]++;
        if (cum >= 400 && UTIL.rand(2) == 0)
            liveData.gameFlags[FLAG.TAMANI_PREGNANCY_COUNT]++;
        if (cum >= 500 && UTIL.rand(2) == 0)
            liveData.gameFlags[FLAG.TAMANI_PREGNANCY_COUNT]++;
        if (cum >= 600 && UTIL.rand(2) == 0)
            liveData.gameFlags[FLAG.TAMANI_PREGNANCY_COUNT]++;
    }
    static tamaniPregnantEncounter() {
        //TODO SPRITE spriteSelect(56);
        GUI.clearOutput();
        GUI.outputText("Tamani strolls out from behind a boulder, and wow is she ever pregnant.  It doesn't diminish the look of lust in her eyes when she meets your gaze, but her hands do keep rubbing the outside of her belly, only pausing to squeeze drops of milk from her nipples.  Her leather straps seem to fit her even better than before, accentuating her expanding curves and looking fantastic on her pregnant form.\n\n");
    ...
    static tamaniPregnantFuck() {
        //TODO Sprite spriteSelect(56);
        GUI.clearOutput();
        GUI.outputText("Tamani's eyes light up and she pounces you, somehow managing to jump up and latch onto your chest despite the weight of her burgeoning pregnancy.  The two of you overbalance as her weight carries you to the ground, flat on your back.   Your " +
            liveData.player.buttDescript() +
        ...
        if (liveData.player.biggestTitSize() >= 2)
            ...
            GUI.outputText("chest");
        GUI.outputText(".\n\n");
        GUI.outputText("You grab her under the arms and lift her up, examining her milk-laden nipples.  They shine with moisture, practically begging to be sucked.  You nuzzle them, gently sucking one into your mouth and drinking down the nutritious beverage.  Tamani grinds her hips against one of your " +
            liveData.player.nippleDescript(0) +
        ...
        //(Very large:
        if (liveData.gameFlags[FLAG.TAMANI_TIMES_IMPREGNATED] > 6)
            GUI.outputText("  They're almost too big for you to handle; they probably make it hard for the poor girl to walk.");
        GUI.outputText("\n\n");
        GUI.outputText("Tired of the teasing, Tamani yanks her nipples free of your mouth and kisses you fiercely.  Her lips are hot, almost feverish, and taste of sweet cherries.  Fiery warmth races through your body, collecting at your groin as her tongue curls around yours.  She sucks on your lower lip before pulling back and giving you an all-too-knowing smile.  Your " +
            liveData.player.cockDescript(0) +
        ...
        GUI.outputText("Her pregnancy swollen belly and growing breasts rub along your shaft as she kisses and squeezes it.  You're so turned on it hurts, and you KNOW it's from whatever she put in her lipstick this time.  You beg her, \"<i>Please, fuck me, I'm so horny it hurts!</i>\"\n\n");
        //(Fits) - pregnant capacity is lowered by about 50%
        if (liveData.player.cockArea(0) <= 50) {
            GUI.outputText("Tamani pulls herself up to her feet and grabs your " +
                liveData.player.cockDescript(0) +
            ...
            GUI.outputText("You cum with near-painful intensity.  Tamani is actually lifted up by the force of your ejaculation.  Cum squirts from her abused fuckhole as she slips back down, rubbing her belly with both hands and tittering girlishly.  The waves of seed you're putting out seem far beyond your normal ability, and you try to manage a glare at her, blaming her, but she just licks her lips and winks in between orgasmic moans.    Cum squirts from her, streaming down her thighs and puddling under your " +
                liveData.player.buttDescript() +
                ".  At last you finish, leaving her looking even more pregnant than before.  Your ");
            if (liveData.player.balls > 0)
                GUI.outputText(liveData.player.ballsDescriptLight());
            else
                GUI.outputText("genitals");
            GUI.outputText(" ache from the explosive discharge, leaving you sore and exhausted.\n\n");
            GUI.outputText("For her part, Tamani staggers to her feet, letting you flop free as a river of your cum pours between her legs, soaking your lower body.  She staggers over to your face, pleasure-drunk, and leans down to give you another sloppy kiss.\n\n");
            GUI.outputText('"<i>Thanks for the good fuck, ' +
        ...
        //(Doesn't Fit)
            ...
            if (liveData.player.balls > 0)
                GUI.outputText("massaging your balls with her feet");
            else
                GUI.outputText("jacking the base of your " + liveData.player.cockDescript(0) + " with her feet");
            GUI.outputText(" while her hands caress and tease you.   Tamani pulls herself downwards, pressing the " +
                liveData.player.cockHead() +
            ...
            GUI.outputText("Tamani comes over to you and gives you a sloppy goodbye kiss, sending an immediate surge of hardness and desire to your groin.  She looks back and giggles, then waddles off, patting her pouches and dripping with your spooge.");
            liveData.player.orgasm();
        }
        GUI.doNext(Camp.returnToCampUseOneHour);
    }
    ...
    static tamaniBirthScene() {
        ...
        if (liveData.gameFlags[FLAG.TAMANI_NUMBER_OF_DAUGHTERS] > 0)
            GUI.outputText("with her daughters, "); //She always has at least two daughters if she has any
        GUI.outputText("laying against a scrubby tree with her legs spread wide and surrounded by a small puddle.  Her " +
            this.tamaniChest() +
        ...
        if (liveData.gameFlags[FLAG.TAMANI_PREGNANCY_COUNT] > 2) {
            ...
            if (liveData.gameFlags[FLAG.TAMANI_PREGNANCY_COUNT] > 3) {
                ...
                if (liveData.gameFlags[FLAG.TAMANI_PREGNANCY_COUNT] == 5) {
                    ...
                    GUI.outputText(", and more than you can count");
            }
            GUI.outputText(".");
        }
        GUI.outputText("\n\n");
        ...
        GUI.outputText("  She looks up at you and whispers breathily, \"<i>Come fuck me a little later, I'll give you even more beautiful little girls.  These " +
