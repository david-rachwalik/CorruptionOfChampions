...
Tamani.tamaniHypnoTease = function () {
    ...
    if (gameFlags[TAMANI_TIMES_HYPNOTIZED] < 5) {
        selector = rand(3);
        if (selector == 0)
            outputText("You reluctantly pull your stare away from the heavenly entrance between her legs.  There's an urge to walk over to her and plunge yourself inside her over and over, but you dismiss it.", false);
        if (selector == 1)
            outputText("You find it hard to pull your gaze from her inviting twat, but you manage.  You shake your head, clearing away thoughts of fertilizing your wife.  Her rhetoric must be getting to you.", false);
        if (selector == 2)
            outputText("No matter the case, her actions shifted a fair bit of your blood-flow to your groin.", false);
    }
    //Medium Hypnosis
        ...
        if (selector == 0) {
            ...
            if (player.lust > 80)
                outputText(" drips pre-cum", false);
            else if (player.lust > 40)
                outputText(" grows harder", false);
            else
                outputText(" hardens", false);
            ...
            if (player.lust < 70)
                outputText("  Still, you don't want to fuck her right now!", false);
        }
        else {
            ...
            if (player.lust <= 41)
                outputText(" thicken perceptibly", false);
            else if (player.lust <= 81)
                outputText(" twitch eagerly", false);
            else
                outputText("drip pre-cum", false);
            ...
            if (player.lust < 70)
                outputText("  Regardless, you're resolute in your desire not to fuck her right now!", false);
        }
    }
    //High Hypnosis
    else {
    ...
    if (gameFlags[TAMANI_TIMES_HYPNOTIZED] == undefined)
        gameFlags[TAMANI_TIMES_HYPNOTIZED] = 0; // Kludge for save fix.
...
TamaniScene.tamaniVictoryMenu = function () {
    ...
    if (player.lust >= 33 && player.totalCocks() > 0 && gameFlags[SFW_MODE] <= 0) {
        ...
        if (player.cockThatFits(monster.analCapacity()) >= 0)
            addButton(1, "Buttfuck", TamaniScene.tamaniAnalSex);
        else
            addButtonDisabled(1, "Buttfuck", player.cockTotal() == 1 ? "Your cock is too big to fit in Tamani's ass." : "None of your cocks will fit in Tamani's ass.");
        if (!tamanipreg.isPregnant() && player.canOvipositSpider())
            addButton(2, "Lay Eggs", TamaniScene.tamaniBeaten);
...
TamaniScene.tamaniSexWon = function () {
    ...
    if (player.cockArea(x) <= 90) {
        ...
        if (player.totalCocks() > 1)
            outputText(" and tight asshole", false);
        ...
        if (player.totalCocks() == 1)
            outputText("pussy squeezing tightly as she cums hard.<br><br>", false);
        else
            outputText("holes squeezing tightly as she cums hard.<br><br>", false);
        if (player.totalCocks() > 1) {
            ...
            if (player.cumQ() >= 250)
                outputText("  She pants, turning her head in the mud to watch as her belly visibly inflates, stuffed totally full of cum. ", false);
            if (player.cumQ() >= 500)
                outputText("  In no time at all a river of jism pours from her, pooling below as you overwhelm her body's capacity to store spunk.", false);
        }
        else {
        ...
        if (player.balls > 0)
            outputText("sexy balls have ", false);
        else
            outputText("sweet cock has ", false);
    ...
    //Too big? Jerk off with feet and bukkake
        ...
        if (player.totalCocks() > 1)
            outputText("any of your massive members", false);
        else
            outputText("your massive member", false);
        ...
        if (player.cumQ() >= 250)
            outputText("  You continue working your " + player.cockDescript(x) + " with her supple feet, watching your dripping spooge froth and bubble as you continue to pump more onto the sassy wench.", false);
        if (player.cumQ() >= 500)
            outputText("  She sputters, blowing some of the caked up semen off her face so she can breathe.  The slut gathers up the goopy mess as you continue to paint her, alternatively devouring it with her mouth and shoveling drippy handfuls into her eager cunt.", false);
...
TamaniScene.postTamaniRemoval = function () {
    ...
    if (tamanipreg.isPregnant())
        tamanipreg.knockUpForce(0, 0); //Clear Tamani Pregnancy.
    cleanupAfterCombat();
};
...
TamaniScene.tamaniBeaten = function () {
    ...
    if (gameFlags[TIMES_OVIPOSITED_TAMANI] == 0)
    ...
    if (player.fertilizedEggs() == 0)
        tamanipreg.knockUpForce(PREGNANCY_DRIDER_EGGS, 72);
    player.dumpEggs();
    cleanupAfterCombat();
    player.orgasm();
};
...
TamaniScene.tamaniLoss = function () {
    if (player.HP <= 0) {
        if (player.totalCocks() > 0) {
            if (rand(2) == 0)
                TamaniScene.tamaniSexLost();
            else
                TamaniScene.tamaniSexLetHer();
        }
        else {
...
TamaniScene.tamaniSexLost = function () {
    ...
    if (player.HP < 1)
        outputText("lie there, unable to fight back.  ", false);
    else
        outputText("touch yourself, giving in to the tiny goblin's desire to breed.  ", false);
    ...
    if (player.cocks[0].cockThickness >= 5)
        outputText("Impossibly, despite your titanic girth, the ring somehow expands to squeeze onto you, though it's painfully tight.<br><br>", false);
    else
        outputText("Somehow the ring fits onto you perfectly, as if it was made exactly for your " + player.cockDescript(0) + ", though it remains painfully tight.<br><br>", false);
    ...
    if (player.biggestTitSize() >= 6)
        outputText("but can't manage to see your bloated prick past the wall of titflesh.<br><br>", false);
    else
        outputText("at your bloated prick, looking almost overinflated from all the blood that tight ring has forced into it.<br><br>", false);
    ...
    if (player.balls > 0)
        outputText("cups your " + player.ballsDescriptLight() + " tightly", false);
    else
        outputText("caresses you around the ring", false);
    ...
    if (player.balls > 0)
        outputText("Your balls are getting VERY swollen and tender.", false);
    else
        outputText("Your body aches inside, feeling bloated and ready to pop.", false);
    if (player.cor < 50)
        outputText("  You have no choice, you'll say what she wants you to say.<br><br>", false);
    else
        outputText("  Being denied like this was kind of fun, but you REALLY need to get off now, so you'll say what she wants you to say.<br><br>", false);
    ...
    if (player.cor > 50)
        outputText("blushing hotly and doing your best to look like a submissive toy.<br><br>", false);
    else
        outputText("blushing in shame and embarrassment, but meaning every word.<br><br>", false);
    ...
    if (player.cumQ() >= 250)
        outputText("  The bottle quickly overflows, but Tamani happily sets it aside and slides her own cunt under you, letting you pump the rest of your seed into her fertile womb.", false);
    if (player.cumQ() >= 500)
        outputText("  She coos happily when you still manage to pump enough jism into her to bloat her belly, but even with your impressive fertility, the orgasm has to end.", false);
    outputText("<br><br>", false);
...
TamaniScene.tamaniSexLetHer = function () {
    ...
    if (inCombat()) {
        ...
        if (player.isTaur()) {
            if (player.HP < 1)
                outputText("You stumble, legs weaving drunkenly as you try to keep your feet.  ", false);
            else
                outputText("You stumble, legs weaving drunkenly as " + player.sMultiCockDesc() + " ache and drool with need.  ", false);
        ...
        //Not taur
            ...
            if (player.HP < 1)
                outputText("You collapse, unable to fight back any longer.  For her part, Tamani seems disappointed with the outcome, but approaches your prone form in spite of your sorry appearance.", false);
            else
                outputText("You tear off your clothes in a frenzy of lust and stagger towards Tamani, openly fondling your groin.   She smirks and pushes you hard, throwing you off balance and onto your back.", false);
            outputText("  She wiggles her " + TamaniScene.tamaniChest() + ", and despite your efforts to control your body, ", false);
            if (player.cockTotal() == 1)
                outputText("an erection emerges", false);
            else
                outputText("erections sprout up like weeds", false);
    ...
    if (player.isTaur()) {
        ...
        if (player.cocks[0].cockLength <= 18) {
            ...
            //No balls:
            if (player.balls == 0)
                outputText("You find yourself wondering vaguely where a goblin learns these kinds of gymnastics.<br><br>", false);
            else
                outputText("You find yourself wondering vaguely where a goblin learns these kinds of gymnastics, before the goblin in question drives all thoughts out of your head by mashing her soft soles into your " + player.ballsDescriptLight() + ".<br><br>", false);
        ...
        //If >18 Inches
            ...
            if (player.balls == 0)
                ...
                outputText("You find yourself wondering vaguely where a goblin learns these kinds of gymnastics, before the goblin in question drives all thoughts out of your head by beginning to mash her soft soles into your " + player.ballsDescriptLight() + ".<br><br>", false);
            ...
            //High cum:
                ...
                if (player.balls > 0)
                    outputText("Your " + player.ballsDescriptLight() + " ache and you feel very thirsty indeed.", false);
                else
                    outputText("You feel very thirsty indeed.", false);
    ...
    //Not-taurs
        ...
        if (player.cockArea(0) <= 90) {
            ...
            if (player.cocks[0].cockLength >= 12)
                outputText("Her body visibly stretches around you, and you silently thank whatever gods or demons adapted goblins to be able to fulfill this role.  ", false);
            ...
            if (player.biggestTitSize() >= 2)
                outputText("Your " + player.allBreastsDescript() + " bounce and jiggle obscenely as the goblin twists and pulls, abusing your nipples.", false);
            else
                outputText("Your " + player.nippleDescript(0) + "s burn with pain as the goblin twists and pulls on them, abusing them.  Pleasure and pain mix together, overwhelming you, reducing your world to little more than the slap of genital against genital and the painful torture happening to your chest.<br><br>", false);
            ...
            if (player.balls > 0)
                outputText("Your " + player.ballsDescriptLight() + " twitch powerfully, venting their contents, releasing a jet of hot cum into the goblin.", false);
            else
                outputText("Your body's internal muscles twitch, pushing a jet of hot cum into the goblin.", false);
            ...
            if (player.cumQ() >= 250)
                outputText("  After a while you can feel the pressure building inside her, starting to give her a bit of a belly.  Tamani rubs it and begs, \"<i>More!</i>\"", false);
            if (player.cumQ() >= 500)
                outputText("  Your body fulfills her requests and more, stuffing her until she's positively pregnant and your spunk is squirting from her entrance with each wave of sticky spooge.", false);
            else
                outputText("  Sadly, you only manage a few more spurts before your orgasm ends.", false);
            ...
            if (player.cumQ() < 250)
                outputText(".  I really enjoy our fucks " + player.mf("stud", "hun") + ", and I can't wait to 'bump' into you again.", false);
            else {
                outputText(".  You make sure and let your ", false);
                if (player.balls > 0)
                    outputText(player.ballsDescriptLight() + " fill back up with cum, ok?", false);
                else
                    outputText(player.cockDescript(0) + " fills back up with cum, ok?", false);
            }
        ...
        //(Doesnt fit)
            ...
            if (player.balls > 0)
                outputText("lifts up your " + player.ballsDescriptLight(), false);
            else
                outputText("leans forwards, looking between your legs", false);
            ...
            if (player.cumQ() >= 250) {
                ...
                if (player.cumQ() >= 500)
                    outputText(" as you keep cumming, splattering her tits and face, soaking the little goblin slut", false);
                outputText(".  ", false);
            }
...
TamaniScene.tamaniHypnosis = function () {
    ...
    if (primary >= 0) {
        ...
        if (cocks > 1) {
            ...
            outputText("She moans and pants, babbling about how good you are to her the whole time.", false);
        ...
        if (cocks > 0)
            outputText(" around your " + player.cockDescript(secondary), false);
        outputText(". Her tiny form jiggles, ", false);
        if (player.tallness > 55)
            outputText("lifting off the ground as you get into it, giving yourself over to her sex.", false);
        else
            outputText("nearly falling over as you give your body over to her sex.", false);
        ...
        if (player.skinType == SKIN_TYPE_FUR)
            outputText("matted", false);
        else
            outputText("slicked", false);
        ...
        if (cocks == 1)
            outputText("her tongue hanging out as she pants nonsensical pleasure-noises.", false);
        else
            outputText("and her mouth is too full of " + player.cockDescript(secondary) + " to do anything but gurgle out nonsensical pleasure-noises.", false);
        ...
        if (player.hasKnot(primary))
            outputText("Your knot balloons inside her, locking her in place", false);
        else if (player.cocks[primary].cockType == CockTypesEnum.HORSE)
            outputText("Your flare widens, actually holding itself inside Tamani's womb", false);
        else if (player.cocks[primary].cockType == CockTypesEnum.DEMON)
            outputText("The corrupted nodules along your " + player.cockDescript(primary) + " pulsate in rippling, wave-like motions, massaging the interior of Tamani's twat", false);
        else if (player.cocks[primary].cockType == CockTypesEnum.TENTACLE)
            outputText("Your tentacle-cock's tip widens, becoming mushroom-like inside Tamani's womb", false);
        else
            outputText("Your sensitive head swells slightly in time with the contractions of your orgasm", false);
        outputText(" as you begin to fill your wife's womb with seed.   The orgasm is intense", false);
        if (player.cumQ() < 50)
            outputText(", but brief, and after a few moments of spasming pleasure, the flood of cum slows to a trickle.", false);
        else if (player.cumQ() < 300)
            outputText(" and voluminous, forcing more and more seed into Tamani's uterus until she sloshes with each new squirt.  After a few long moments of spasming pleasure, the flood of cum slows to a trickle.", false);
        else {
            ...
            if (player.cumQ() < 1000)
                outputText("  Though your orgasm tapers off, the additional seed squirts from between her thighs, trickling down to a slow drip of viscous white goo.", false);
            else
                outputText("  Trapped inside her and cumming as hard as ever, you raise the pressure inside the goblin until your seed is washing out of her cunt around you, puddling on the ground.", false);
            if (player.cumQ() >= 1000 && player.cumQ() < 4000)
                outputText("  It does eventually end, but not until a sizable puddle has formed between the pair of you.", false);
            else
                outputText("  It lasts seemingly forever, and by the time you're done, the two of you are standing in a puddle inches deep.", false);
        }
        if (cocks > 1) {
            ...
            if (player.cumQ() > 300) {
                ...
                if (player.cumQ() > 600)
                    outputText("  A few seconds later she pulls back coughing, and the perky goblin takes the rest of your load in her face.", false);
            }
            else
                outputText(".", false);
        }
        ...
        if (player.tallness > 55)
            outputText(" in spite of the height differential", false);
    ...
    //[DOESNT FIT]
        ...
        if (player.biggestTitSize() < 1)
            outputText("on your chest ", false);
        else
            outputText("between your " + player.breastDescript(0) + " ", false);
        ...
        if (player.cor < 33)
            outputText("beg", false);
        else if (player.cor < 66)
            outputText("ask", false);
        else
            outputText("command", false);
        ...
        if (player.cumQ() > 250)
            outputText("  In no time, you've filled her to capacity, and white mixes in with the clear sexual fluids that are dripping from the goblin's cunt.", false);
        if (player.cumQ() > 700)
            outputText("  A few more blasts visibly bloat her belly, and Tamani pulls her hands away from her pussy to stroke the slowly rounding dome on her abdomen.", false);
        if (player.cumQ() > 1500) {
            outputText("  You just keep cumming, forcing the spooge to squirt out around the tube, puddling on the ground", false);
            if (player.cumQ() > 3000)
                outputText(", and by the time you finish, both of you are in a thick puddle of spunk", false);
            outputText(".", false);
        }
        if (cocks > 1) {
            if (cocks == 2)
                outputText("  Your other" + player.cockDescript(1), false);
            else
                outputText("  Each of your other " + player.multiCockDescriptLight(), false);
...
TamaniScene.encounterTamani = function () {
    ...
    //Dudezillaz:
        ...
        switch (tamanipreg.pregnancyEventNum) {
            case 2:
                TamaniScene.tamaniPregnantEncounter();
                break; //She's moderately pregnant
            case 3:
                TamaniScene.tamaniBirthScene();
...
TamaniScene.tamaniFemaleYes = function () {
    ...
    if (player.cor < 50)
        outputText("Gross.<br><br>", false);
    else
        outputText("Getting cock-slapped would've been kind of hot...<br><br>", false);
...
TamaniScene.tamaniFemaleNo = function () {
    ...
    if (player.cor < 33)
        outputText("What a slut.", false);
    else if (player.cor < 66)
        outputText("How odd.", false);
    else
        outputText("You hope she misses a few.", false);
    doNext(Camp.returnToCampUseOneHour);
};
...
TamaniScene.tamaniFirstTimeConsentual = function () {
    ...
    if (player.biggestTitSize() > 1)
        outputText(player.allBreastsDescript(), false);
    else
    ...
    if (player.balls > 0)
        outputText("Your " + player.ballsDescriptLight() + " swell with seed, spurring your desire to new heights.", false);
    else
        outputText("Something inside you swells up with seed, spurring your desire to new heights.", false);
    ...
    if (player.cockArea(0) <= 90) {
        ...
        if (player.balls > 0)
            outputText(" and drips from your " + player.ballsDescriptLight(), false);
        ...
        if (player.cockArea(0) > 30)
            outputText(" and watching her belly bulge from your size", false);
        ...
        //[CORRUPT]
        if (player.cor > 66)
            outputText("  Your " + player.cockDescript(0) + " twitches at the thought, ready and wanting more.", false);
    }
    //(TOO BIG)
    else {
        ...
        if (player.hasSheath()) {
            ...
            if (player.balls > 0)
                outputText("licks all over your balls", false);
            else if (player.hasVagina())
                outputText("sneaks between your folds to tease your now-hardening clit", false);
            else
                outputText("licks the sensitive " + player.skinTone + " " + player.skinDesc + " of your inner thighs", false);
        }
        outputText("<br><br>", false);
        ...
        if (player.balls > 0 && player.hoursSinceCum > 200)
            outputText(", visibly draining your " + player.ballsDescriptLight() + " down to their normal size", false);
...
TamaniScene.tamaniMaleRepeatEncounter = function () {
    ...
    if (tamanipreg.isPregnant() || gameFlags[TAMANI_NUMBER_OF_DAUGHTERS] > 0)
        outputText("While exploring, you're startled by the feeling of tiny hands stroking the insides of your thighs.  You look down and find Tamani there, grinning wolfishly,  \"<i>Ready for another fuck, big " + player.mf("boy", "girl") + "?\"<br><br>", false);
...
TamaniScene.tamaniKnockUp = function () {
    ...
    if (player.findPerk(PerkLib.MaraesGiftStud) >= 0)
        gameFlags[TAMANI_PREGNANCY_COUNT] += 3; //TODO Check findPerk Thing
    if (cum >= 50 && rand(2) == 0)
        gameFlags[TAMANI_PREGNANCY_COUNT]++;
    if (cum >= 100 && rand(2) == 0)
        gameFlags[TAMANI_PREGNANCY_COUNT]++;
    if (cum >= 200 && rand(2) == 0)
        gameFlags[TAMANI_PREGNANCY_COUNT]++;
...
TamaniScene.tamaniPregnantFuck = function () {
    ...
    //(Very large:
    if (gameFlags[TAMANI_TIMES_IMPREGNATED] > 6)
        outputText("  They're almost too big for you to handle; they probably make it hard for the poor girl to walk.", false);
    outputText("\n\n", false);
    ...
    //(Doesn't Fit)
        ...
        if (player.balls > 0)
            outputText("massaging your balls with her feet", false);
        else
            outputText("jacking the base of your " + player.cockDescript(0) + " with her feet", false);
...
TamaniScene.tamaniBirthScene = function () {
    ...
    if (gameFlags[TAMANI_NUMBER_OF_DAUGHTERS] > 0)
        outputText("with her daughters, ", false); //She always has at least two daughters if she has any
    ...
    if (gameFlags[TAMANI_PREGNANCY_COUNT] > 2) {
        ...
        if (gameFlags[TAMANI_PREGNANCY_COUNT] > 3) {
            ...
            if (gameFlags[TAMANI_PREGNANCY_COUNT] == 5) {
                ...
                outputText(", and more than you can count", false);
        }
        outputText(".", false);
    }
    outputText("\n\n", false);
