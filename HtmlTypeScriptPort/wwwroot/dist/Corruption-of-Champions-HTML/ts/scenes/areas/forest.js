...
import { TentacleBeastScene } from "./forest/tentacleBeast.js";
import { TamaniScene } from "./forest/tamani.js";
// import { JojoScene } from "../npcs/jojo.js"
import { BeeGirlScene } from "./forest/beeGirl.js";
...
class AreasForest {
    static explore() {
        ...
        if ((liveData.player.cor >= 25 || liveData.player.level >= 4 || liveData.gameFlags[FLAG.JOJO_CORRUPTION_STAGE] != 0) && liveData.gameFlags[FLAG.JOJO_CAMP] == 0 /* && flags[JOJO_DEAD_OR_GONE] == 0*/)
            choice[choice.length] = 1; //Jojo
        if (liveData.player.level >= 2)
            ...
            default:
                if (liveData.player.cor < 80) {
                    ...
                    if (liveData.player.gender == 1)
                        GUI.outputText("stroking your half-erect " + liveData.player.multiCockDescriptLight() + " as you daydream about fucking all kinds of women, from weeping tight virgins to lustful succubi with gaping, drooling fuck-holes.");
                    if (liveData.player.gender == 2)
                        GUI.outputText("idly toying with your " + liveData.player.vaginaDescript(0) + " as you daydream about getting fucked by all kinds of monstrous cocks, from minotaurs' thick, smelly dongs to demons' towering, bumpy pleasure-rods.");
                    if (liveData.player.gender == 3)
                        GUI.outputText("stroking alternatively your " +
                    ...
                    if (liveData.player.gender == 0)
                        GUI.outputText("daydreaming about sex-demons with huge sexual attributes, and how you could please them.");
                    GUI.outputText(" ");
                    if (liveData.player.tou < 50)
                        liveData.player.modStats(["tou", 0.5]);
                    if (liveData.player.lib < 60)
                        liveData.player.modStats(["lib", 0.25]);
