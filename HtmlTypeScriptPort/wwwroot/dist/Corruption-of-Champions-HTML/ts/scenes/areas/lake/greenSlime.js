...
class GreenSlime extends Creature {
    ...
    lustAttack() {
        GUI.outputText("The creature surges forward slowly with a swing that you easily manage to avoid.  You notice traces of green liquid spurt from the creature as it does, forming a thin mist that makes your skin tingle with excitement when you inhale it. ");
        liveData.player.changeLust(liveData.player.lib / 10 + 8, true);
    }
    lustReduction() {
        GUI.outputText("The creature collapses backwards as its cohesion begins to give out, and the faint outline of eyes and a mouth form on its face.  Its chest heaves as if it were gasping, and the bolt upright erection it sports visibly quivers and pulses before relaxing slightly. ");
        this.changeLust(-13, true);
    }
}
//------------
// SCENES
//------------
class GreenSlimeScene {
    static encounterSlime() {
        GUI.clearOutput();
        liveData.gameFlags[FLAG.TIMES_MET_OOZE]++;
        GUI.displaySprite("greenslime");
        //High int starts on even footing.
        if (liveData.player.inte >= 25) {
            GUI.outputText("A soft shuffling sound catches your attention and you turn around, spotting an amorphous green mass sliding towards you!  Realizing it's been spotted, the ooze's mass surges upwards into a humanoid form with thick arms and wide shoulders.  The beast surges forward to attack!");
            COMBAT.startCombat(new GreenSlime());
            return;
        }
        //High speed starts on even footing.
        else if (liveData.player.spe >= 30) {
            GUI.outputText("You feel something moist brush the back of your ankle and instinctively jump forward and roll, coming up to face whatever it is behind you.  The nearly silent, amorphous green slime that was at your feet surges vertically, its upper body taking the form of a humanoid with thick arms and wide shoulders, which attacks!");
            COMBAT.startCombat(new GreenSlime());
            if (liveData.gameFlags[FLAG.FACTORY_SHUTDOWN] == 1)
                GUI.outputText("<br><br><b>You are amazed to encounter a slime creature with the factory shut down - most of them have disappeared.</b>");
            return;
        }
        //High strength gets stunned first round.
        else if (liveData.player.str >= 40) {
            GUI.outputText("Without warning, you feel something moist and spongy wrap around your ankle, nearly pulling you off balance.  With a ferocious tug, you pull yourself free and turn to face your assailant.  It is a large green ooze that surges upwards to take the form of humanoid with wide shoulders and massive arms.  It shudders for a moment, and its featureless face shifts into a green version of your own! The sight gives you pause for a moment, and the creature strikes!");
            if (liveData.gameFlags[FLAG.FACTORY_SHUTDOWN] == 1)
                GUI.outputText("<br><br><b>You are amazed to encounter a slime creature with the factory shut down - most of them have disappeared.</b>");
            COMBAT.startCombat(new GreenSlime());
            GUI.outputText("<br><br>");
            liveData.monster.attack();
            return;
        }
        else {
            GUI.outputText("Without warning, you feel something moist and spongy wrap around your ankle, pulling you off balance!  You turn and try to pull your leg away, struggling against a large green ooze for a moment before your foot comes away with a *schlorp* and a thin coating of green fluid.  The rest of the ooze rises to tower over you, forming a massive green humanoid torso with hugely muscled arms and wide shoulders.  Adrenaline rushes into your body as you prepare for combat, and you feel your heart skip a beat as your libido begins to kick up as well!");
            liveData.player.modStats(["lib", 1]);
            liveData.player.changeLust(10, true);
            COMBAT.startCombat(new GreenSlime());
        }
    ...
    static greenSlimeWin() {
        ...
        if (liveData.player.findStatusEffect(StatusEffects.Feeder) >= 0 && liveData.gameFlags[FLAG.SFW_MODE] <= 0) {
            ...
            if (liveData.player.lust >= 33 && liveData.player.gender > 0) {
                ...
                GUI.addButton(0, "B.Feed", GreenSlimeScene.rapeOozeWithMilk);
                GUI.addButton(1, "Rape", GreenSlimeScene.slimeVictoryRape);
                GUI.addButton(5, "Leave", COMBAT.cleanupAfterCombat);
            }
            //Rapes not on the table.
            else {
                GUI.outputText("<br><br>Your nipples ache with the desire to forcibly breastfeed the gelatinous beast.  Do you?");
                GUI.doYesNo(GreenSlimeScene.rapeOozeWithMilk, COMBAT.cleanupAfterCombat);
            }
        }
        //Not a breastfeeder
        else if (liveData.player.lust >= 33 && liveData.player.gender > 0 && liveData.gameFlags[FLAG.SFW_MODE] <= 0) {
            GUI.outputText("  Sadly you realize your own needs have not been met.  Of course, you could always play with the poor thing... Do you rape it?");
            GUI.doYesNo(GreenSlimeScene.slimeVictoryRape, COMBAT.cleanupAfterCombat);
        }
        COMBAT.cleanupAfterCombat();
    }
    static slimeVictoryRape() {
        //Service for lower corruption
        if (liveData.player.cor <= 33) {
            GUI.clearOutput();
            if (liveData.player.lust < 60)
                GreenSlimeScene.serviceLowCorruption();
            else
                GreenSlimeScene.serviceLowCorruptionHighLust();
        }
        //higher corruption raaaaeeeep
            ...
            if (liveData.player.gender == 1 || (liveData.player.gender == 3 && UTIL.rand(2) == 0))
                GreenSlimeScene.maleRapesOoze();
            //female or futa
            else if (liveData.player.gender > 0)
                GreenSlimeScene.femaleRapesOoze();
        }
    }
    static rapeOozeWithMilk() {
        GUI.clearOutput();
        GUI.outputText("You look over the ooze, wondering what to do about your need to nurse now that it has lost cohesion. After a while of puzzling things out, you decide to wing it, " +
            liveData.player.clothedOrNaked("removing the top of your " + liveData.player.armorName + " and ") +
            "pressing the mess of a monster to your " +
        ...
        if (liveData.player.gender == 1) {
            GUI.outputText("caressing your " + liveData.player.multiCockDescriptLight());
            if (liveData.player.balls > 0) {
                GUI.outputText(", and slipping slightly further down to engulf your " + liveData.player.ballsDescriptLight() + " as well. ");
            }
            else
                GUI.outputText(". ");
        }
        // [if female-
            ...
            GUI.outputText("manipulating your " + liveData.player.clitDescript() + ". ");
        }
        //[If Herm-
            ...
            if (liveData.player.balls > 0)
                ...
                GUI.outputText("and almost dripping down to coat your " + liveData.player.clitDescript() + " to add to your pleasure. ");
        }
        //[If Genderless-
        else {
            GUI.outputText("caressing the blank spot where your genitalia should be" + liveData.player.clothedOrNaked(" despite your " + liveData.player.armorName + " still being in place") + ". ");
            GUI.outputText("<br><br>");
            GUI.outputText("The barrage of pleasurable feelings causes you to fall over onto your " + liveData.player.buttDescript() + " and just soak in them, your hands ");
        }
        //[if male or genderless-
        if (liveData.player.gender < 1)
            GUI.outputText("gripping the ground as the slime has left you nothing to do; adding your hands to the mix feels like it'd be an insult to the creature's expert work.  ");
        //if duder
        else if (liveData.player.gender == 1)
            GUI.outputText("Your hands grip the ground as the slime has left you nothing to do; adding your fingers to the mix feels like it'd be an insult to the creature's expert work.  ");
        //[if female or Herm-
        else
            GUI.outputText("Your hands fly to your " + liveData.player.vaginaDescript(0) + " and frantically plunge your fingers in and out of it.  You wish the slime would engulf it as well.  ");
        GUI.outputText("You cum many times into the mass, but that's not what truly matters to you. It's an extremely welcome, mind-shatteringly satisfying bonus, but not the main event. What does matter is the slow, meticulous draining of milk from both of your " +
            liveData.player.breastDescript(0) +
        ...
        if (liveData.player.gender == 1) {
            ...
            if (liveData.player.balls > 0)
                GUI.outputText(" and " + liveData.player.ballsDescript());
            GUI.outputText(" and ");
        }
        //[if female-
        else if (liveData.player.gender == 2)
            GUI.outputText(liveData.player.clitDescript() + " and ");
        //[if Herm-
        else if (liveData.player.gender == 3) {
            GUI.outputText(liveData.player.multiCockDescriptLight());
            if (liveData.player.balls > 0)
                GUI.outputText(", " + liveData.player.ballsDescriptLight() + ",");
            GUI.outputText(" and " + liveData.player.clitDescript() + " and ");
        }
        GUI.outputText(liveData.player.breastDescript(0) + ".<br><br>");
        GUI.outputText("Unfortunately, you seem to have done a little too much to it in your battle before. The ooze slides off, leaving you hanging on the edge of orgasm. Deciding you just won't stand for this, you scoop up most the mess left by the monster and use it as a masturbation aid, achieving sweet release by ");
        if (liveData.player.gender == 1)
            GUI.outputText("feverishly jacking yourself off with it.  The cool feel of it contrasts enough to send you over the edge relatively quickly, and you release your cum into it.  You drop most of its bulk ");
        //[if female-
        else if (liveData.player.gender == 2)
            GUI.outputText("rapidly entering it in and out of your " +
        ...
        //[if herm-
            ...
            GUI.outputText("feverishly jacking yourself off with it while rapidly entering it in and out of your " +
                liveData.player.vaginaDescript(0) +
        ...
        //[if genderless-
            ...
            GUI.outputText("rubbing it over your " + liveData.player.assholeDescript() + ".  The cool feel of it contrasts enough to send you over the edge relatively quickly, and you drop it as you feel your muscles spasm with the phantom orgasm ");
        GUI.outputText("while using the rest of it to coat your " + liveData.player.nippleDescript(0) + " in order to give it the last of your milk.<br><br>");
        GUI.outputText("Now empty, you leave the ooze in the pile it's in and walk away, feeling somewhat disappointed by the lackluster ending of it all, but overall satisfied with the fact that you've nursed.<br><br>");
    ...
    static femaleRapesOoze() {
        liveData.player.slimeFeed();
        GUI.outputText("You feel a stirring inside your feminine side as you eye the slime's throbbing erection, and decide to take advantage of its current state to satisfy your own urges.  You strip off your clothes and walk forward, straddling the creature's thighs and running a hand over its member.  You pull it in against your body and stroke the soft, velvety, and just slightly moist shaft,");
        if (liveData.player.cocks.length > 0) {
            GUI.outputText(" your " + liveData.player.vaginaDescript(0) + " growing wet and your " + liveData.player.cockDescript(0) + " growing firm as you imagine it inside you.  ");
        }
        else {
            GUI.outputText(" growing wet as you imagine it inside of you.  ");
        }
        // !!NOTE
        ...
        if (liveData.player.biggestTitSize() > 8) {
            GUI.outputText("You grin and lean forward slightly, letting your " +
        ...
        else if (liveData.player.biggestTitSize() > 3) {
            GUI.outputText("You smile mischievously and lean down, pressing your chest down around the creature's member.  You slowly start to move back and forth and the slime responds positively by slowly moving its hips in time with your bod, all the while throbbing and pulsing magnificently.  You watch with wide eyes as the shaft shifts, changing into a perfectly sculpted human cock, a full foot and a half long.  Sensing the thing getting close you ease yourself off and sit back up, your chest now covered with a green fluid.<br><br>");
        }
        else if (liveData.player.biggestTitSize() >= 0) {
            GUI.outputText("You pull the member tight against your body and run your hands along its length, rubbing it over your belly as the shaft throbs and pulses beneath your touch.  You lean forward a little and rub the tip of it against your " +
                liveData.player.allBreastsDescript() +
                ", smiling as it leaves a little bit of greenish fluid behind.  After enduring your ministrations for a short time the creature begins moving its hips in time with you.  You hold in a slight gasp as the ooze's shaft shifts, changing into a perfectly sculpted human cock, a full foot and a half long.<br><br>");
        }
        /*
        else
        {
        ...
        GUI.outputText("<<ERROR: breast size check returned invalid number -" + String(player.biggestTitSize()) + "- at green slime' player win + vagina + high lust;>>", false);
        }*/
        GUI.outputText("Before long you cannot take the temptation anymore and lift your hind end into the air, moaning softly as you rub the tip of its cock against your " + liveData.player.vaginaDescript(0) + ".");
        //defloration text
        if (liveData.player.vaginas[0].virgin) {
            //low corruption reveres this loss
            if (liveData.player.cor <= 20) {
                GUI.outputText("  You hesitate for a moment as you hover over the creature, a sudden surge of nervousness comes over you.  This is a moment that only comes once.  You let your weight push the creature into you, slowly at first, then forcing yourself down as far you can go.  You gasp at the initial pain, but it quickly dissipates as the creature's fluids coat your interior with a cool, soothing fluid.  ");
            }
            //higher corruption tosses it aside
            else {
                GUI.outputText("  You smile as you ram yourself onto the creature, finally removing your virginity.  You quickly start moving and give a pleased gasp as the creature's fluids coat your interior with a cool, soothing fluid.  ");
            }
        }
        else if (liveData.player.vaginalCapacity() >= 18) {
            GUI.outputText("  You enthusiastically lower yourself onto the slime's cock, letting out a small gasp of surprise as it slides inside of you.  The creature quickly understands your intentions and reaches up with its hands, beginning to pump you furiously.  ");
        }
        else {
            GUI.outputText("  You hesitate at the sheer size of the slime's cock and the creature, sensing your hesitation, reaches up with its strong arms and grips your ass, forcefully pushing you onto itself.  ");
        }
        GUI.outputText("The thing's cock, though rigid, somehow manages to squish and expand wherever needed to conform perfectly to your insides, adapting to your " + liveData.player.vaginaDescript(0) + " with ease as you ride it.");
        //penis arousal text
        if (liveData.player.cockTotal() > 0) {
            if (liveData.player.cockTotal() == 1) {
                GUI.outputText("  Your " + liveData.player.cockDescript(0) + " grows hard as you are penetrated, and with the creature handling the movement you reach down to jack yourself off.");
            }
            else {
                //might be kind of interesting to include text that identifies the cocks you use, but is possibly pointless.
                GUI.outputText("  Your cock grows hard as you are penetrated, and with the creature handling the movement you reach down, jerking off a different cock with each hand.");
            }
        }
        GUI.outputText("<br><br>");
        GUI.outputText("The slime pumps you hard, eliciting moans of arousal as you lean forward, finally giving in to instinct.  You find that it can easily support your weight as you lean on the thing's chest, using it as leverage as you buck your hips against it.  After a while the slime begins to react almost perfectly to your moaning and panting, changing its pace and size to incredible effect.  You feel exceptionally wet through the entire process, the creature's lubricant mixing with your own.  It has an aphrodisiac effect, intensifying your sensation and making your " +
            liveData.player.vaginaDescript(0) +
            " feel hot as you fuck it.  ");
        GUI.outputText("Before long you collapse entirely, resting your entire weight on the creature as it pulls your " +
        ...
        if (liveData.player.cockTotal() > 0) {
            if (liveData.player.cockTotal() > 1) {
                GUI.outputText("Your " + liveData.player.multiCockDescriptLight() + " explode into simultaneous orgasm, releasing ");
                GUI.outputText(UTIL.num2Text(liveData.player.cocks.length));
                GUI.outputText(" thick streams of semen onto you and the creature, your mind blanking at the massive relief.  You vaguely notice your ejaculate sinking into the creature's body and getting absorbed as you give yourself to pleasure.  ");
            }
            else {
                GUI.outputText("Your " +
        ...
        GUI.outputText("You feel a rush of liquid flood your insides, ");
        // !! NOTE
    ...
    static maleRapesOoze() {
        GUI.outputText("Noting your own erectness, you decide that the unusual nature of the thing in front of you is of, at best, minor concern, and resolve to fuck it to satisfy your own desires.<br><br>");
        GUI.outputText("You approach the creature and examine it, finding no real orifices to speak of.  ");
        // Random check here for oral or anal
        // oral rape text
        if (UTIL.rand(2) == 0) {
            //MULTICAWK
            if (liveData.player.cockTotal() > 1)
                GUI.outputText("Shrugging, you decide to go with the closest thing available and walk towards its head.  You spend a moment deciding which of your organs to ravage the creature with, eventually settling on the largest,  ");
            else
                GUI.outputText("Shrugging, you decide to go with the closest thing available and walk towards its head, pulling out your " +
                    liveData.player.cockDescript(0) +
            ...
            if (liveData.player.balls > 1)
                GUI.outputText("anointing the slime's face with your " + liveData.player.sackDescript() + " as you do.  ");
            //Ball-less bitches
            else
                GUI.outputText("probing at the 'mouth' of the creature with your free hand.  ");
            //Continue the setup...
            GUI.outputText("You pull back for a moment and aim your penis downward, resting the ");
            //!! PENISTYPE
            switch (liveData.player.cocks[0].cockType) {
                case 0: //human cock
                    GUI.outputText("tip");
                    break;
                case 1: //horse cock
                    GUI.outputText("flared tip");
                    break;
                case 2: //dog cock
                    GUI.outputText("pointed tip");
                    break;
                case 4: //tentacle cock
                    GUI.outputText("rounded tip");
                    break;
                default:
                    //error text
                    GUI.outputText("tip");
            ...
            GUI.outputText(" on the inside of its mouth and pushing.  You feel the material give and push harder, the membrane material that covers the slime's exterior stubbornly refusing to give.  Finally, after some frustration, you grab the creature by what would be its neck and try to pull it back to your pelvis, thrusting forward at the same time.  It finally gives, and you see your " +
                liveData.player.cockDescript(0) +
                " surge into its mouth");
            //This series deliberately not else-if statements.
            if (liveData.player.cocks[0].cockLength >= 12)
                GUI.outputText(", down its throat,");
            if (liveData.player.cocks[0].cockLength >= 24)
                GUI.outputText(" into its torso,");
            if (liveData.player.cocks[0].cockLength >= 48)
                GUI.outputText(" and all the way through to its hips,");
            GUI.outputText(" and grunt approvingly.  You begin to slowly pull yourself out and then thrust yourself back in, feeling a moistness as some fluid surrounds your cock.  ");
            //player doesn't care about monster's wellbeing
            if (liveData.player.cor >= 60)
                GUI.outputText("You wonder for a moment if you've broken the creature's membranous skin, then shrug and decide you don't care.");
            //player shows concern for monster
            else
                GUI.outputText("You wonder for a moment if you've broken the creature's membranous skin, but the continuing presence of the creature's throbbing erection implies that it's in relatively little distress.");
            //New PG
            GUI.outputText("<br><br>");
            //Start fucking in earnest
            GUI.outputText("You start to thrust with more vigor, thrusting until your hips mash against the ooze's skin, feeling yourself build to orgasm even faster than usual as your " +
                liveData.player.cockDescript(0) +
            ...
            //Balls ftw
            if (liveData.player.balls > 1)
                GUI.outputText(" as your " + liveData.player.ballsDescriptLight() + " slap against its face");
            GUI.outputText(".  One of its arms reaches up to meekly rub at its own erection.  You grin as this fuels your own lust, and you start to thrust even harder.");
            ...
            //New PG for cumming!
            GUI.outputText("<br><br>");
            GUI.outputText("Before long you find yourself shouting as you pump your seed into the creature, watching as the semen is caught by unseen currents and dispersed into its body.");
            //vaginal orgasm text
            if (liveData.player.vaginas.length > 0)
                GUI.outputText("  This is quickly followed by a quivering between your nether-lips as your " + liveData.player.vaginaDescript(0) + " quivers in orgasm shortly afterwards.");
            GUI.outputText("  You pull your " +
                liveData.player.cockDescript(0) +
        ...
        //anal rape text
            ...
            //Multipricks
            if (liveData.player.cockTotal() > 1) {
                GUI.outputText("You shrug at the lack of obvious orifices and decide to go for the most common one, choosing the ");
                if (liveData.player.cockTotal() == 2)
                    GUI.outputText("larger");
                else
                    GUI.outputText("largest");
                GUI.outputText(" of your" + liveData.player.multiCockDescriptLight() + " as you grab the slime and roughly try to turn it over.  Surprisingly it maintains its cohesion and flops onto its side, its erection flopping wetly onto the ground.  ");
            }
            //Single dicks
            else
                GUI.outputText("You shrug at the lack of obvious orifices and decide to go for the most common one, pulling out your " +
                    liveData.player.cockDescript(0) +
            ...
            GUI.outputText("  You position your member around where you think the asshole would be and push, smiling as the membrane moves aside and you penetrate into a tight, deep hole.  ");
            if (liveData.player.cocks[0].cockLength >= 24)
                GUI.outputText("By the time you finally bottom out, your " + liveData.player.cockDescript(0) + " is nearly halfway through its torso.");
            else if (liveData.player.cocks[0].cockLength >= 48)
                GUI.outputText("By the time you finally bottom out, your " + liveData.player.cockDescript(0) + " reaches nearly into the slime's neck.");
            else if (liveData.player.cocks[0].cockLength >= 60)
                GUI.outputText("Your penis keeps going deeper and deeper, through the chest, neck, and head, before finally stretching the slime as far as it needs to go to accommodate the full length of your " + liveData.player.cockDescript(0) + ".");
            GUI.outputText("  With a satisfied smile you begin pumping, slowly at first before picking up speed.  The slime's anus feels better than anything you've ever had before, apparently coming with its own lubrication but still being tight, hugging your cock like a custom-fit fuck toy.  ");
            //vaginal arousal text
            if (liveData.player.vaginas.length > 0)
                GUI.outputText("Your " + liveData.player.vaginaDescript(0) + " grows wet as you work the thing's hole, every slap of your hips against the thing's ass sending a pang of pleasure through your nethers.  ");
            GUI.outputText("After only a few minutes you realize that you can't hold out for much longer, as whatever is inside the creature seems to be making you extra sensitive.  Deciding to make the most of it, you double your speed and grit your teeth, enjoying the slime for all it's worth before ");
            if (liveData.player.cockTotal() > 1)
                GUI.outputText("your " + liveData.player.multiCockDescriptLight() + " explodes into orgasm, your load filling the space inside the creature and spraying in thick streams across its back.");
            else
                GUI.outputText("exploding inside it, your load filling the space around your cock with a white, milky fluid.");
            //vaginal orgasm text
            if (liveData.player.vaginas.length > 0)
                GUI.outputText("  As you pump your jizz into the creature an almost electrical shock runs through your body, starting from your " + liveData.player.vaginaDescript(0) + " and radiating outwards.");
            GUI.outputText("  The thing seems to clamp down on you even tighter as you pull out, and you relish the sensation.  Its tight ass leaves your " +
                liveData.player.cockDescript(0) +
    ...
    static greenSlimeLose() {
        //WORMS
        /*
            if (pcCameWorms) {
                GUI.outputText("<br><br>The slime doesn't even seem to notice.<br><br>");
            }
            GUI.doNext(game.lake.greenSlimeScene.slimeLoss);
            */
        GreenSlimeScene.slimeLoss();
    }
    static slimeLoss() {
        if (liveData.gameFlags[FLAG.SFW_MODE] == 1)
            return; //No rape in SFW mode.
        GUI.clearOutput();
        GUI.doNext(liveData.playerMenu);
        if (liveData.player.gender == 2 || (liveData.player.gender == 3 && UTIL.rand(2) == 0)) {
            let temp = UTIL.rand(3);
            if (temp == 0)
                GreenSlimeScene.oozeRapesYouVaginally();
            if (temp == 1)
                GreenSlimeScene.oozeRapesYouOrally();
            if (temp == 2)
                GreenSlimeScene.oozeButtRapesYou();
        }
        //Everybody else
        else {
            let temp = UTIL.rand(2);
            if (temp == 0)
                GreenSlimeScene.oozeRapesYouOrally();
            if (temp == 1)
                GreenSlimeScene.oozeButtRapesYou();
    ...
    static oozeRapesYouVaginally() {
        liveData.player.slimeFeed();
        GUI.outputText("You collapse under the beating from the slime's soft but heavy fists, dazed and disoriented.  The creature lunges at you with surprising speed, grabbing you by the ankles and pulling you towards it.  You try to pull away as it pulls you up against its trunk, momentarily fearful that it's going to try and absorb you.  ");
        //shy result
        if (liveData.player.cor <= 20) {
            GUI.outputText("This fear quickly disappears as a large, human cock grows from the creature's trunk, and is replaced by an entirely different fear as you realize what it is planning.  The slime wraps its arms around your " +
                ...
                " and along your " +
                liveData.player.vaginaDescript(0) +
                ".  Its skin is soft and velvety, slightly moist, and leaves a thin trace of green fluid behind.<br><br>");
            GUI.outputText("You begin to feel a tingle in your nethers as your " +
                liveData.player.vaginaDescript(0) +
            ...
            //Ginormo coochie
            else if (liveData.player.vaginalCapacity() <= 30)
                GUI.outputText("The thing slowly presses against you to penetrate and you grab at the grass beneath you.  A soft moan escapes your lips as the cock slides into your " +
                    liveData.player.vaginaDescript(0) +
            ...
            GUI.outputText("You arch your back slightly as the slime begins moving, slowly pumping your " +
                liveData.player.vaginaDescript(0) +
                ".  As it picks up speed a small amount of the thing's greenish fluid builds up around your lower mouth, making your skin tingle and filling you with an unusual arousal.<br><br>");
            //Cocks
            if (liveData.player.cockTotal() > 0) {
                //Multi biiiiitches
                if (liveData.player.cockTotal() > 1) {
                    GUI.outputText("Your " + liveData.player.multiCockDescript() + " rapidly grows erect under this assault and begins to throb.  Having now released yourself to instinct you reach to jerk off, ");
                    if (liveData.player.cockTotal() > 2)
                        GUI.outputText("switching between cocks as you rapidly stroke yourself in time with the creature's movements.  ");
                    else
                        GUI.outputText("rapidly stroking one cock with each hand, working yourself in time with the creature's movements.  ");
                }
                //Single dick
                else
                    GUI.outputText("Your " +
            ...
            GUI.outputText("After a short time the creature penetrates you fully again and stops, and you realize that at some point you had begun moving your hips with it.  You gasp in surprise as the slime's cock begins churning inside of you, massaging you from the inside in a uniquely pleasurable fashion.<br><br>");
            GUI.outputText("It doesn't take much of this to bring you to orgasm.  Your entire body shudders and you arch your back as shocks of pleasure run through you.  The creature pulls you tightly against it and swells just slightly as its movement inside you slows and then stops, then swells at the tip for just a second before releasing a flood of thin, cool fluid inside of you.  It forces its way around the creature's cock and spills out of you.  ");
            //Dicks again!
            if (liveData.player.cockTotal() > 0) {
                //Multi
                if (liveData.player.cockTotal() > 1)
                    GUI.outputText("Your " + liveData.player.multiCockDescript() + " climaxes as well, sending rope after rope of hot, thick cum onto your face and chest.");
                //Single
                else
                    GUI.outputText("Your member climaxes as well, and you pull a hand to cover your eyes and face as your jism explodes onto yourself.  Your other hand slides to the base of your " +
                        liveData.player.cockDescript(0) +
        ...
        //standard result
            ...
            if (liveData.player.vaginas[0].virgin) {
                ...
                liveData.player.cuntChange(15, true);
            }
            //TIGHT
            else if (liveData.player.vaginalCapacity() < 18)
                GUI.outputText("It finally begins to penetrate you and you grab at the grass beneath you, excited and anxious for it to try and squeeze itself into you.  You bite your bottom lip hold in a moan loudly as the cock slides into your " +
                    liveData.player.vaginaDescript(0) +
                    ", its girth squeezing down to comfortably slide into you.  You let out a pleasant sigh as it penetrates deeper into your body, filling the contours of your interior perfectly until it finally bottoms out.  What remains of its length seems to slide back into the creatures body as it pulls your hips against it's trunk.  ");
            //Loose
            else {
                GUI.outputText("It finally begins to penetrate and you grab at the grass beneath you, delighted that its cock seems large enough to fill you");
                //SLUT!
                if (liveData.player.vaginalCapacity() >= 30)
                    GUI.outputText(" at least partly");
                GUI.outputText(".  A deep, throaty moan escapes your lips as the cock slides into your " +
                    liveData.player.vaginaDescript(0) +
                    ...
                    liveData.player.vaginaDescript(0));
                if (liveData.player.vaginalCapacity() >= 30)
                    GUI.outputText(" almost perfectly.<br>");
                else
                    GUI.outputText(" perfectly.<br>");
            }
            GUI.outputText("You arch your back in pleasure as the slime begins moving, slowly pumping your wet " +
                liveData.player.vaginaDescript(0) +
            ...
            if (liveData.player.cockTotal() > 0) {
                ...
                //single cock
                    ...
                    GUI.outputText("Your own member finally reaches its climax as well, and you open your mouth as jism explodes onto your face and chest.  Your hands slide to the base of your " +
                        liveData.player.cockDescript(0) +
        ...
        if (liveData.player.cor >= 70) {
            ...
            if (liveData.player.cockTotal() > 0) {
                if (liveData.player.lust > 40)
                    ...
                    GUI.outputText("You feel your " + liveData.player.multiCockDescriptLight() + " begin to harden just slightly as you work the creature, a surge of arousal flowing through your body.");
            }
            if (liveData.player.vaginas.length > 0) {
                GUI.outputText("Your " + liveData.player.vaginaDescript(0) + " ");
                if (liveData.player.cockTotal() > 0)
                    GUI.outputText("also ");
                GUI.outputText("grows wet as you work the creature, a surge of arousal flowing through your body.  You rub your thighs together and moan into the creatures cock, desperate to be touched by something.");
    ...
    static oozeButtRapesYou() {
        liveData.player.slimeFeed();
        GUI.outputText("You collapse under the beating from the slime's soft but heavy fists, dazed and disoriented.  You weakly try to resist as the slime rolls you onto your stomach and lifts your " +
        ...
        //Girls get fondled.
        if (liveData.player.vaginas.length > 0)
            GUI.outputText(liveData.player.vaginaDescript(0) + " and " + liveData.player.assholeDescript());
        //No coochie
        else {
            //Ballsy
            if (liveData.player.balls >= 2)
                GUI.outputText(liveData.player.assholeDescript() + " and " + liveData.player.ballsDescript());
            //No balls, no pussy, just butt
            else
                GUI.outputText(liveData.player.assholeDescript());
        }
        GUI.outputText(" before settling over your rear entrance.  ");
        // virgins just assume it wont fit
        if (liveData.player.analCapacity() < 18 || liveData.player.ass.analLooseness == 0) {
            ...
            if (liveData.player.cor <= 50)
                GUI.outputText("You feel a bit of fear as you tremble under the creature's weight, worried about the damage that its massive size might do to your ass.  ");
            else if (liveData.player.cor < 100)
                GUI.outputText("In spite of your worries about the creature's size, you can't help but feel a little bit excited at the idea of having this beast take you from behind.  ");
            //and something special for the total whorebags that have somehow not already wrecked their asses.
            else
                GUI.outputText("You almost lick your lips in anticipation and push your hips back greedily, eager to feel the monster's cock stretch your asshole wide.  ");
        }
        GUI.outputText("You grimace slightly and it pushes at your sphincter, then penetrates.  You let out a slight gasp in spite of yourself as its cock warps, deforming to stretch your ass without pushing you beyond your limits.  It pushes itself several inches inside you and stops, pausing as you catch your breath and collect yourself.");
        //De-virgin
        liveData.player.buttChange(liveData.monster.cockArea(0), true);
        GUI.outputText("<br><br>");
        GUI.outputText("With a deliberate slowness the slime pulls out of you and then thrusts back inside, this time going deeper than before.  It continues like this and you realize that the penetration is entirely painless, the creature's pliability and moistness allowing it to easily glide inside you.  Before long you feel it bottom out inside you, its soft member curving perfectly through your insides as it trunk presses against your behind.  The slime begins fucking you with short, shallow strokes and you feel a moistness build inside your ass as the creature's fluid begins to accumulate.  It begins to quicken and deepen its strokes as you feel a heat build inside of you, the creature's fluid apparently having an aphrodisiac effect.<br><br>");
        GUI.outputText("Before long you find yourself moaning under the beast's attack, pleasure washing through your nethers in spite of yourself as it pumps your ass.  ");
        //cock arousal
        if (liveData.player.cockTotal() > 0)
            GUI.outputText("You feel pangs of pleasure run through your body as its member repeatedly strokes your prostate from the inside, stimulating it.  ");
        GUI.outputText("Its dick shifts slightly, continually growing and shrinking in tiny amounts in a continual, heavy throb.  ");
        //masturbation begin
        //Cawkz!
        if (liveData.player.cockTotal() > 0) {
            //Single
            if (liveData.player.cockTotal() == 1)
                GUI.outputText("You quickly grow erect under the creature's embrace as it pistons into your ass, and before long you stand at full attention.  You weakly rub yourself with one hand as it pumps you, and quickly approach orgasm.  ");
            //Multi!
            else
                GUI.outputText("Your " +
                    liveData.player.multiCockDescript() +
        ...
        if (liveData.player.vaginas.length > 0) {
            ...
            if (liveData.player.cockTotal() > 0)
                GUI.outputText("The creature finally plunges a pair of fingers into your " +
                    liveData.player.vaginaDescript(0) +
            ...
            //vaginal-only orgasm
                ...
                GUI.outputText("The creature finally plunges a pair of fingers into your " + liveData.player.vaginaDescript(0) + ", the sudden shock finally pushing you to orgasm.  ");
        }
        //mangasm
        else if (liveData.player.cockTotal() > 0)
            GUI.outputText("The additional pressure against your prostate finally pushes you over the edge and you come hard, spraying your load against the ground.  Thick ropes of jizm spray onto the ground as the feeling wracks your body.  ");
        GUI.outputText("You shudder beneath the creature as it slowly begins to pull out of you, which brings another moan to your lips.  When it finally pops free of your body you feel a deluge of fluid rush from your ass and onto the ground.  You recover yourself as the creature retreats back into the waters, leaving nothing but a trail of slime leading to the water's edge.");
        liveData.player.dynStats(["sen", 4]);
        COMBAT.cleanupAfterCombat();
        liveData.player.orgasm();
    }
    static serviceLowCorruptionHighLust() {
        liveData.player.slimeFeed();
        GUI.outputText("You find yourself unable to tear your eyes away from the creature as it undulates almost hypnotically in front of you, the heat in your crotch rising.  The rigid protrusion at its middle visibly pulses and throbs, and with a small grin you step closer to the ooze and strip off your clothing.<br><br>");
        GUI.outputText("You straddle the creature's chest and playfully run a hand over the tip of its featureless member, which is moist and soft to the touch.");
        //Tall peeps
        if (liveData.player.tallness >= 82)
            GUI.outputText("  You grip the head of the member firmly with one of your large hands and tease the tip with your thumb for a moment before squeezing and sliding your hand to its base.");
        //Short peeps
        else
            GUI.outputText("  You run a finger up and down the creature's member a few times before tightly wrapping your hands around the tip and running them to its base.");
        GUI.outputText("  The erection is spongy and pliable to the touch, and with every pulse it seems to fill the cracks between your fingers.<br><br>");
        //If player has a cunny
        if (liveData.player.vaginas.length > 0) {
            GUI.outputText("As you rub the creature's shaft you toy with the idea of inserting the thing into your " +
                liveData.player.vaginaDescript(0) +
            ...
            if (!liveData.player.vaginas[0].virgin)
                GUI.outputText("for that familiar full feeling ");
            GUI.outputText("and paranoid about allowing this creature inside of you.");
            //If you be hella tight
            if (liveData.player.vaginalCapacity() <= 18)
                GUI.outputText("  On top of that, you are unsure if the thick, foot and a half long appendage would even fit inside of you.");
            GUI.outputText("<br><br>"); //this line present to ensure proper paragraph structure
        }
        //Penises foreplay
        if (liveData.player.cockTotal() > 0) {
            //Multicock folk get in their own way
            if (liveData.player.cockTotal() > 1)
                GUI.outputText("Your own erect " + liveData.player.multiCockDescriptLight() + " rub against the creature's as you work, getting in the way somewhat.  ");
            //Big single cocked folk get in their own way too!
            else if (liveData.player.cocks[0].cockThickness >= 36)
                GUI.outputText("Your own erect " + liveData.player.cockDescript(0) + " rubs against the creature's as you work, getting in the way somewhat.  ");
            //Single dick contemplation
            if (liveData.player.cockTotal() == 1)
                GUI.outputText("You feel your " +
                    liveData.player.cockDescript(0) +
        ...
        //Next PG
        //Get it hard
        GUI.outputText("As you continue to work the creature's mass its erection gains more and more definition, slowly but surely turning into an unmistakable human penis.  ");
        //And here begins the giant goddamn height split.  Hoo boy.
        //Smaller characters!
        if (liveData.player.tallness < 82) {
            //Hand gets coated in sensitive slime<br>
            GUI.outputText("A light green fluid begins to coat your hand as you stroke the slime's cock, making your fingers feel warm and sending shivers of arousal through your entire body.<br><br>");
            //Male or sometimes herm...
            if (liveData.player.gender == 1 || (liveData.player.gender == 3 && UTIL.rand(2) == 0)) {
                //multiCock - mutual masturbation + cum
                if (liveData.player.cocks.length > 1) {
                    if (liveData.player.cocks.length > 0)
                        GUI.outputText("Eventually your own arousal becomes unbearable and you reach down to work your one of your " +
                            liveData.player.multiCockDescriptLight() +
                    ...
                    if (liveData.player.vaginas.length > 0)
                        GUI.outputText("Your " + liveData.player.vaginaDescript(0) + " quivers and shares in the sensation as you grind yourself against the slime.  ");
                    //Create cumz
                ...
                //Single cocks
                    ...
                    //'Small' cocks
                    if (liveData.player.cocks[0].cockLength <= 30) {
                        GUI.outputText("Eventually your own arousal becomes unbearable and you reach down to work your own " +
                            liveData.player.cockDescript(0) +
                    ...
                    //FUGGIN' 'UGE PRICKS!
                    else if (liveData.player.cocks[0].cockLength >= 31) {
                        GUI.outputText("Eventually your own arousal becomes unbearable and you reach down to grope at yourself as well, working the monster's swollen, throbbing head with one of your hands.  Unable to grip it properly you moan a little and hold yourself down against the creature's skin, moving your hips back and forth to get off.  You begin to moan louder as the same greenish secretion that coats your hands begins to cover your " +
                            liveData.player.cockDescript(0) +
                        ...
                        GUI.outputText("Instinctively you hold onto the creature's own shaft for support as you do and with a final mighty pulse it also comes, spraying its thin green fluid in a tall gout into the air.  You feel it splash onto your back and " +
            ...
            //female or herm version (if the herm didn't trigger male)
                ...
                //Small clitties
                    ...
                    GUI.outputText("You moan and lean back slightly as you start to flick your " +
                        liveData.player.clitDescript() +
                ...
                GUI.outputText("At the same time you continue to work the head of the slime beneath you, slowly twisting your hand around the top of its large erection.  Before long the mass starts to slim and bulge, becoming more defined before taking on the distinct image of a human penis.  The sight of it further excites you and you begin to work it harder, feeling it start to throb in earnest beneath your touch.  ");
                //Tit stuff
                if (liveData.player.breastRows.length > 0 && liveData.player.biggestTitSize() > 0) {
                    GUI.outputText("You gasp a little as the slime's massive arms reach up from behind you to cup your " + liveData.player.allBreastsDescript() + ", and the soft material ");
                    //Tig ol' bitties
                    if (liveData.player.biggestTitSize() > 8)
                        GUI.outputText("barely contains them.");
                    //Middling Boobiliciousness
                    else if (liveData.player.biggestTitSize() > 3)
                        GUI.outputText("gently covers them.");
                    //Tiny ta-tas.
                    else
                        GUI.outputText("easily smothers them.");
                    //Get nips played with and nearly cum
                    GUI.outputText("  You feel the thing tweak your nipples and you moan a little as the ooze begins a gentle, rhythmic massage, teasing your " +
                        liveData.player.nippleDescript(0) +
                ...
                //No titties or too small to bother with
                    ...
                    GUI.outputText("You gasp a little as the slime's massive arms suddenly grab your " +
                        liveData.player.hipDescript() +
                ...
                if (liveData.player.cockTotal() > 0) {
                    ...
                    if (liveData.player.cockTotal() > 1) {
                        ...
                        if (liveData.player.biggestTitSize() == 0)
                            GUI.outputText("You delicately grasp the head of one of your cocks, your hand slick with the fluids of yourself and of the creature.  The combination of the creature's movements and your own quickly overwhelms you.");
                    }
                    //Single prick
                    else {
                        GUI.outputText("You feel your " + liveData.player.cockDescript(0) + " surge to full size as the slime works you over.  It begins to throb and ache for release, a need made only more pressing by your desire to have it fondled.  ");
                        //Play with it if your boobs aren't being manhandled
                        if (liveData.player.biggestTitSize() == 0)
                            GUI.outputText("You delicately grasp the head of your " +
                                liveData.player.cockDescript(0) +
                ...
                GUI.outputText("<br><br>");
                //Female cum!
                GUI.outputText("Finally, you feel a tsunami of pleasure begin to wash over you as you orgasm, and as your entire body shakes you unconsciously try to squeeze the creature between your legs.  It pulls back slightly and with a final mighty pulse, a green fluid explodes from the tip of its " +
                    liveData.monster.cockDescriptShort(0) +
                ...
                if (liveData.player.cockTotal() > 0) {
                    //Multidicked herms
                    if (liveData.player.cockTotal() > 1)
                        GUI.outputText("You let out a strangled gasp as your body, already straining from exertion, shudders again as your " + liveData.player.multiCockDescript() + " explodes into orgasm, showering both you and the creature in jizz.  ");
                    //Single cocked herms
                    else
                        GUI.outputText("You let out a strangled gasp as your body, already straining from exertion, shudders again as your " + liveData.player.cockDescript(0) + " explodes into orgasm, sending a jet of your warm jizz into the air.  ");
                }
                GUI.outputText("When you open your eyes you notice that the creature's erection has disappeared and the almost predatory calm it possessed when you first encountered it has returned, though different somehow.");
                //New PG
                GUI.outputText("<br><br>");
            }
            ...
            GUI.outputText("After a few moments the creature slides out from under you, slinking back into the waters of the lake as you recover from the ordeal.  When your wits return, there is nothing to do but stop back by camp.<br>");
        }
        //PHEW! that was a big one.  now...  we do it all again for large players, whee! and this is why this thing is so goddamn long.
        else if (liveData.player.tallness >= 82) {
            GUI.outputText("A light green fluid begins to coat your hand as you stroke the slime's cock, making your fingers feel warm and sending shivers of arousal through your entire body.  ");
            //Males/Herms
            if (liveData.player.gender == 1 || (liveData.player.gender == 3 && UTIL.rand(2) == 0)) {
                //Multicock foreplay
                if (liveData.player.cockTotal() > 1)
                    GUI.outputText("Eventually your own arousal becomes unbearable and you reach down to work one of the shafts in your " +
                        liveData.player.multiCockDescriptLight() +
                ...
                //SingleCock
                    ...
                    if (liveData.player.cocks[0].cockType == ENUM.CockType.HUMAN)
                        GUI.outputText("a near exact replica of your own " + liveData.player.cockDescript(0) + "!  ");
                    //Cocks are different so it's a 'human' penis
                    else
                        GUI.outputText("a distinctly human penis.  ");
                }
                //vaginal arousal & cum text for herms
                if (liveData.player.vaginas.length > 0) {
                    GUI.outputText("The intense sensations ripple through your entire body, and your " +
                        liveData.player.vaginaDescript(0) +
                    ...
                    GUI.outputText("<br><br>It isn't long before you can no longer hold yourself back and with a groan, you explode, showering the creature with your seed as your " +
                        liveData.player.multiCockDescript() +
                        " and " +
                        liveData.player.vaginaDescript(0) +
            ...
            //Female/Herm Masturbation
                ...
                if (liveData.player.breastRows.length > 0 && liveData.player.biggestTitSize() > 0) {
                    ...
                    if (liveData.player.biggestTitSize() > 8)
                        GUI.outputText("Its hand expands, struggling to cover all of your " + liveData.player.allBreastsDescript() + ", ");
                    //Middling ta-tas get covered
                    else if (liveData.player.biggestTitSize() > 3)
                        GUI.outputText("Its hand expands and gently covers your " + liveData.player.allBreastsDescript() + ", ");
                    //Small tits?  Pwned.
                    else
                        GUI.outputText("Its hand easily smothers your " + liveData.player.allBreastsDescript() + ", ");
                    GUI.outputText("and you stop for a moment until the slime tweaks one of your " +
                        liveData.player.nippleDescript(0) +
                    ...
                    GUI.outputText("A bead of realization dawns on you just before you feel it hit, the surprise making you squeeze and twist at the head of the slime's cock.  ");
                }
                //Non titty foreplay
                else
                    GUI.outputText("rapidly moistening " +
                        liveData.player.vaginaDescript(0) +
                ...
                if (liveData.player.cockTotal() > 0) {
                    ...
                    if (liveData.player.biggestTitSize() == 0 || liveData.player.breastRows.length == 0) {
                        GUI.outputText("You begin to moan as the creature rubs the " + liveData.player.vaginaDescript(0) + " beneath your " + liveData.player.multiCockDescript() + ", working you with incredible skill, but sadly ignoring your needy ");
                        //multi!
                        if (liveData.player.cockTotal() > 1)
                            GUI.outputText("penii ");
                        //Single
                        else
                            GUI.outputText("penis ");
                        //SURPRIZES!
                        GUI.outputText("as it does.  What comes next is an utter surprise, making you unconsciously wring the head of the slime's cock.  ");
                    }
                    //Clit engulph'ed!
                    GUI.outputText("The creature's smooth, gel-like body envelopes your " +
                        liveData.player.clitDescript() +
                ...
                //Just clits, no cocks.
                    ...
                    //since players without breasts are wholly surprised here, this needs to be acknowledged.
                    if (liveData.player.biggestTitSize() == 0 || liveData.player.breastRows.length == 0)
                        GUI.outputText("You begin to moan as the creature rubs your " +
                            liveData.player.vaginaDescript(0) +
                ...
                GUI.outputText("You barely notice as the creature's erection recedes and it slides out from under you, leaving you utterly satisfied.  When you do finally recover your senses you find no other traces of its presence.");
            }
        }
        liveData.player.dynStats(["sen", 3]);
        COMBAT.cleanupAfterCombat();
    ...
    static serviceLowCorruption() {
        GUI.outputText("You seem unable to pull your eyes away from the creature, clearly helpless and distressed in its current state.  Carefully, you step forward, the slime barely registering your presence any longer.  It momentarily grimaces as if in agony, and you decide that you can't simply leave it like this.");
        GUI.outputText("You sit next to the blob and gently touch its throbbing member.  The surface membrane is moist and almost velvety in texture, not entirely smooth but not truly rough either.  You slowly run a hand along its length, glancing at the creature's face as you do so.  It seems calmer somehow, more in control of its breathing.<br><br>");
        //Big peoples!
        if (liveData.player.tallness >= 82) {
            GUI.outputText("You try gently wrapping one of your massive hands around its erection, and squeezing.");
        }
        //Not so big peoples
        else {
            GUI.outputText("You try to wrap one of your hands around the erection, but end up having to use both to get a firm grip.");
        }
        GUI.outputText("  In spite of its appearance the thing is very soft and spongy, although a strong pulse periodically makes it surge in your hand, pushing itself into the cracks between your fingers.  Looking back at the thing's face you slowly begin to pump up and down the massive erection, which grows moist and then a little slick as some of the creature's thin green fluid seeps out onto your hands.  You feel slightly flush and start moving faster, the slime reacting positively to your ministrations.  The rest of its body seems to pulse as well, the definition in its body returning as you continue.  It even begins moving its hips in time with you, its facial features slowly becoming less and less pronounced as its erection gains more definition, clearly forming into a " +
            liveData.monster.cockDescriptShort(0) +
            ".<br><br>");
        //Big peoples!
        if (liveData.player.tallness >= 82) {
            GUI.outputText("The throbbing of the slime's " +
                liveData.monster.cockDescriptShort(0) +
...
export { GreenSlime, GreenSlimeScene };
