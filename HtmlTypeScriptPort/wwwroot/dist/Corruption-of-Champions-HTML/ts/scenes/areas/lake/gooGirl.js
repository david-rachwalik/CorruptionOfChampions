...
GooGirl.prototype.doAI = function () {
    var chooser;
    if (monster.findStatusEffect(StatusEffects.Acid) >= 0 && rand(3) == 0 || rand(3) == 0)
        chooser = 0; //Extra chance if acidic.
    else if (rand(5) == 0)
        chooser = 1;
    else if (rand(3) == 0)
        chooser = 2;
...
GooGirl.gooGalAttack = function () {
    ...
    if (damage <= 0) {
        damage = 0;
        if (monster.findStatusEffect(StatusEffects.Acid) >= 0) {
            if (rand(player.armor.defense + player.tou) < player.armor.defense)
                outputText("tries to slap you, but the acid-bearing slap spatters weakly off your " + player.armor.equipmentName + ". ");
            else
            ...
            if (rand(player.armor.defense + player.tou) < player.armor.defense)
                outputText(", her attack slapping fruitlessly against your " + player.armor.equipmentName + ". ");
            else
    ...
    //everyone else
        ...
        if (monster.findStatusEffect(StatusEffects.Acid) >= 0) {
            ...
            if (player.findStatusEffect(StatusEffects.Acid) < 0)
                player.createStatusEffect(StatusEffects.Acid, 0, 0, 0, 0);
        }
        else
...
GooGirl.gooEngulph = function () {
    ...
    if (player.findStatusEffect(StatusEffects.Bind) < 0)
        player.createStatusEffect(StatusEffects.Bind, BIND_TYPE_GOO, 0, 0, 0);
    monster.fatigue += 25;
    //combatRoundOver();
};
// NEEDS TESTING
GooGirl.prototype.outputDefaultTeaseReaction = function (lustDelta) {
    if (monster.lust <= 99) {
        if (lustDelta <= 0)
            outputText("<br><br>The goo-girl looks confused by your actions, as if she's trying to understand what you're doing.", false);
        else if (lustDelta < 13)
            outputText("<br><br>The curious goo has begun stroking herself openly, trying to understand the meaning of your actions by imitating you.", false);
        else
            outputText("<br>The girl begins to understand your intent. She opens and closes her mouth, as if panting, while she works slimy fingers between her thighs and across her jiggling nipples.", false);
    }
    else
        outputText("<br>It appears the goo-girl has gotten lost in her mimicry, squeezing her breasts and jilling her shiny " + monster.skinTone + " clit, her desire to investigate you forgotten.", false);
};
//-----------
...
GooGirlScene.beatUpGoo = function () {
    ...
    // Else choose from these options
        ...
        if (player.findPerk(PerkLib.Feeder) >= 0)
            addButton(3, "Breastfeed", GooGirlScene.victoryRapeAGooGalAsFeeder);
        if (player.canOvipositBee())
            addButton(4, "Lay Eggs", GooGirlScene.layBeeEggsInGoo);
...
GooGirlScene.gooMaleRape = function (type) {
    ...
    if (player.longestCockLength() >= 24 && player.shortestCockLength() < 24) {
        if (type == 1)
            x = player.longestCock();
        else
            x = player.shortestCockIndex();
    }
...
GooGirlScene.victoryRapeAGooGalAsFeeder = function () {
    ...
    if (player.hasVagina())
        outputText("Even without stimulation, your clit practically vibrates under your climax, squeezing your body to its limits until everything fades into a pearl haze and you lose yourself to the euphoria.<br><br>", false);
    else if (player.hasCock())
        if (player.hasVagina())
            outputText("Even without stimulation, your cock practically vibrates under your climax, squeezing your body to its limits until everything fades into a pearl haze and you lose yourself to the euphoria.<br><br>", false);
        else if (player.hasVagina())
            outputText("Even without stimulation, your tingling anus practically vibrates under your climax, squeezing to its limits until everything fades into a pearl haze and you lose yourself to the euphoria.<br><br>", false);
...
GooGirlScene.getBeatByGooGirl = function () {
    ...
    if (gameFlags[GOOGIRL_CONSECUTIVE_LOSSES] >= 5 && player.gooScore() >= 4)
...
GooGirlScene.hermLoseToGooGal = function () {
    ...
    if (player.biggestLactation() >= 2)
        outputText("  The mouths sucking at your " + player.allBreastsDescript() + " ripple against your flesh until milk sprays from your mammaries, her body suckling the entirety of your lactate-heavy chest, cleavage to teat.", false);
...
GooGirlScene.femaleLoseToGooGal = function () {
    ...
    if (player.pregnancyIncubation <= 280 && player.pregnancyIncubation >= 1) {
        ...
        if (player.lactationQ() <= 1000)
            outputText("  She drains you in no time, hands hungrily sucking for more in goopy kisses on your overtaxed nipples.", false);
        //[medium milk only:
        else if (player.lactationQ() <= 2000)
            outputText("  She drinks gratefully, nursing you with her permeable body until the pout of her belly sloshes heavily with the pale ivory of your motherly bounty.", false);
        else
            outputText("  You bite your lip and let go, torrents of rich, pearl fluid rushing through her arms and inflating the surprised girl before your eyes. The milk gushing into her fills the goo from base to breast, giving the once-" + monster.gooColor5() + " girl an opalescent shine, her over-filled body bloated as if she were carrying triplets.", false);
    ...
    //[If the player isn't pregnant]
        ...
        if (player.vaginalCapacity() <= 30)
            outputText("She bottoms out quickly, filling every inch of your pussy with her squished form, wriggling inside of you happily, her membrane molding to your birth canals every crevasse.", false);
        else
            outputText("More and more of her ooze thrusts into you, shivers of overstimulation tearing through your mind like fists through wet parchment. Your eyes roll aimlessly as the pressure makes it difficult to breathe. With a squelching slurp, the last of the girl's form is pulled inside your slavering pussy.", false);
        ...
        if (player.pregnancyIncubation >= 1)
            outputText("  Everything she pumped into you runs back out after.  For some reason or another, it didn't take.", false);
        else {
...
GooGirlScene.gooGirlBadEnd2 = function () {
    ...
    if (player.hasCock())
        outputText("A pseudopod extends from your crotch, a gooey penis the size of a minotaur. It snakes through the air toward the girl, its tip flaring to expose a wide, urethra-like opening.  ", false);
    else
