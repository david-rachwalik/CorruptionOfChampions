...
CalluScene.ottahGirl = function () {
    ...
    menu();
    if (player.lust < 33)
        outputText("<br><br>You aren't aroused enough to fuck her.");
    else {
        //(If cocksize above 48")
        if (player.hasCock()) {
            if (player.shortestCockLength() > 48)
                outputText("<br><br>Unfortunately, you don't think she can quite handle your cock.");
            else
                addButton(0, "Fuck Her", CalluScene.ottergirlLikesDongs);
        }
        if (player.hasVagina() || !player.hasCock())
            addButton(1, "Facesitting", CalluScene.ottersForGals);
    }
    if (gameFlags[MET_OTTERGIRL] > 1)
        addButton(2, "Get Fish", CalluScene.getSomeFishYaFatty);
...
CalluScene.ottergirlLikesDongs = function () {
    ...
    while (temp < player.cockTotal()) {
        if (player.cocks[temp].cockLength < 48) {
            if (y < 0)
                y = temp;
            else if (player.cocks[temp].cockLength > player.cocks[y].cockLength)
                y = temp;
        }
        temp++;
    }
    ...
    if (player.cocks[x].cockLength < 6)
        outputText("<br><br>\"<i>Well butter my buns and call me a biscuit, ain't this a cute little thing,</i>\" she remarks, inspecting your tiny cock. \"<i>I ain't never seen one this small. I just wanna wrap it up in a little bow and cuddle with it. You sure it ain't a clit, darlin'?</i>\"");
    //(6"-10")
    else if (player.cocks[x].cockLength < 10)
        outputText("<br><br>\"<i>Just packin' the average model, eh? Nothin' wrong with that,</i>\" she remarks while inspecting your cock.");
    //(10"-24")
    else if (player.cocks[x].cockLength < 24)
        outputText('<br><br>"<i>Oh my, now that\'s a manly piece of meat right there,</i>" she remarks, inspecting your oversized cock. "<i>I could enjoy that bad boy all day.</i>"');
    //(24"-48")
    else
        outputText('<br><br>"<i>Whoa nellie,</i>" she says, her eyes going wide as they feast upon your giant cock. "<i>That. That right there, darlin\', is one grade-A trouser snake. I\'ve seen centaurs that\'d look like geldings next to you.</i>"');
    ...
    outputText("<br><br>You feel slender but powerful fingers wrap around your cock, followed shortly after by a pair of lips. They encircle your " +
        player.cockHead(x) +
    ...
    //[demonic]
    else if (player.cocks[x].cockType == ENUM.CockType.DEMON)
        outputText("to the demonic nodules ringing your cock, ");
    //[anemone]
    else if (player.cocks[x].cockType == ENUM.CockType.ANEMONE)
    ...
    outputText('<br><br>After several minutes of this, Callu relinquishes her hold on your member and says, "<i>Mm, I reckon that\'ll work just fine.</i>" She sits up and positions herself over your ' +
    ...
    outputText("<br><br>Surprisingly, you can still feel a throbbing around your " +
        player.cockDescript(x) +
    ...
    outputText("<br><br>A sudden, strange high-pitched sound suddenly rings out and your head bolts upright, only to see Callu reeling in a fish. She looks it over, nods once to herself and tucks it away in an ice chest cleverly buried under the sand right next to the two of you. Afterwards she stands up, letting your dick fall out of her. Your " +
        player.cockDescript(x) +
