...
class CalluScene {
    static ottahGirl() {
        ...
        if (liveData.gameFlags[FLAG.MET_OTTERGIRL] == 1) {
            ...
            //Silly Mode:
            if (liveData.silly)
                GUI.outputText(" No tragic backstory to go through? No annoying combat encounter? Just meet and fuck? My god, what has this world come to?");
        ...
        //Repeats
            ...
            GUI.outputText("Your explorations of the lake lead you back to Callu, the otter girl. She sits lazily on the beach; fishing rod in hand, as usual. She gives a friendly wave as you approach, and pats the sandy patch of lakeside next to her.");
            GUI.outputText('<br><br>"<i>Well ain\'t you a sight for sore eyes.</i>" You sit down next to her and relax, just sitting and watching the makeshift bobber tip and sway in the water. "<i>You up for a fish\'n\'fuck then?</i>" she asks suddenly, brushing a strand of her sun bleached blonde hair out of her face.');
            GUI.outputText("<br><br>Well, are you?");
            //[Facesitting] [Fuck Her] [Fish] [Skedaddle]
        }
        GUI.menu();
        if (liveData.player.lust < 33)
            GUI.outputText("<br><br>You aren't aroused enough to fuck her.");
        else {
            //(If cocksize above 48")
            if (liveData.player.hasCock()) {
                if (liveData.player.shortestCockLength() > 48)
                    GUI.outputText("<br><br>Unfortunately, you don't think she can quite handle your cock.");
                else
                    GUI.addButton(0, "Fuck Her", CalluScene.ottergirlLikesDongs);
            }
            if (liveData.player.hasVagina() || !liveData.player.hasCock())
                GUI.addButton(1, "Facesitting", CalluScene.ottersForGals);
        }
        if (liveData.gameFlags[FLAG.MET_OTTERGIRL] > 1)
            GUI.addButton(2, "Get Fish", CalluScene.getSomeFishYaFatty);
    ...
    static ottergirlLikesDongs() {
        ...
        GUI.outputText("The moment you agree, a sly smile spreads across her face. She jams the end of her fishing pole into the sand like a post, to prevent it from going anywhere, and stands up. There's no tease, no ceremony as she strips out of her bikini bottoms and tosses them aside. Her newly revealed mound has only the barest tuft of pubic hair, a little wisp of blonde hair amongst the sparse brown fur.");
        GUI.outputText("<br><br>You move forward, intent on groping Callu's little breasts still hidden beneath the bikini top, but she holds up a hand and says, \"<i>Whoa there darlin', that ain't how a fish'n'fuck works. You just lay down, and I'll take care of everything. And make sure you're as naked as a newborn babe.</i>\"");
        GUI.outputText("<br><br>Strange, but you oblige, stripping off your [armor] and gear and tossing them aside. Callu instructs you to lay down on the beach next to her fishing pole, which you likewise oblige. The otter-girl straddles your stomach, facing away from you, though her thick, heavy tail is thankfully kept away from your face.");
        var x;
        var y = -1;
        var temp = 0;
        while (temp < liveData.player.cockTotal()) {
            if (liveData.player.cocks[temp].cockLength < 48) {
                if (y < 0)
                    y = temp;
                else if (liveData.player.cocks[temp].cockLength > liveData.player.cocks[y].cockLength)
                    y = temp;
            }
            temp++;
        }
        if (y < 0)
            y = liveData.player.smallestCockIndex();
        x = y;
        //(Under 6")
        if (liveData.player.cocks[x].cockLength < 6)
            GUI.outputText("<br><br>\"<i>Well butter my buns and call me a biscuit, ain't this a cute little thing,</i>\" she remarks, inspecting your tiny cock. \"<i>I ain't never seen one this small. I just wanna wrap it up in a little bow and cuddle with it. You sure it ain't a clit, darlin'?</i>\"");
        //(6"-10")
        else if (liveData.player.cocks[x].cockLength < 10)
            GUI.outputText("<br><br>\"<i>Just packin' the average model, eh? Nothin' wrong with that,</i>\" she remarks while inspecting your cock.");
        //(10"-24")
        else if (liveData.player.cocks[x].cockLength < 24)
            GUI.outputText('<br><br>"<i>Oh my, now that\'s a manly piece of meat right there,</i>" she remarks, inspecting your oversized cock. "<i>I could enjoy that bad boy all day.</i>"');
        //(24"-48")
        else
            GUI.outputText('<br><br>"<i>Whoa nellie,</i>" she says, her eyes going wide as they feast upon your giant cock. "<i>That. That right there, darlin\', is one grade-A trouser snake. I\'ve seen centaurs that\'d look like geldings next to you.</i>"');
        ...
        GUI.outputText("<br><br>You feel slender but powerful fingers wrap around your cock, followed shortly after by a pair of lips. They encircle your " +
        ...
        if (liveData.player.cocks[x].cockType == ENUM.CockType.HORSE)
            GUI.outputText("to your flare, sucking, teasing and ");
        //[canine]
        else if (liveData.player.hasKnot(x))
            GUI.outputText("to the base of your cock, planting sloppy kisses on your knot, ");
        //[demonic]
        else if (liveData.player.cocks[x].cockType == ENUM.CockType.DEMON)
            GUI.outputText("to the demonic nodules ringing your cock, ");
        //[anemone]
        else if (liveData.player.cocks[x].cockType == ENUM.CockType.ANEMONE)
            GUI.outputText("to the little wriggling tentacles ringing the head and base of your cock, ");
        else
            GUI.outputText("to the sensitive little spot on the underside of the head, ");
        ...
        if (liveData.player.balls > 0 && liveData.player.hasVagina())
            GUI.outputText("balls and cunt");
        ...
        GUI.outputText('<br><br>After several minutes of this, Callu relinquishes her hold on your member and says, "<i>Mm, I reckon that\'ll work just fine.</i>" She sits up and positions herself over your ' +
        ...
        //(dicks 10" or less)
        if (liveData.player.cocks[x].cockLength < 10)
            GUI.outputText("until she comes to rest on your lap");
        //(10"-24")
        else if (liveData.player.cocks[x].cockLength < 24)
            GUI.outputText("slowly devouring your entire cock, until she finally comes to rest on your lap");
        else
            GUI.outputText("an excruciatingly long process as feet worth of hard cockmeat disappear into her snatch. There's a small moment of resistance, followed by a soft squelch and a sudden \"<i>Oooh</i>\" from Callu. With no small amount of trepidation, you realize you've just penetrated into her womb. You can't tell from the way she's facing, but you're certain her stomach has to be bulging outwards at this point");
        GUI.outputText(".");
        GUI.outputText("<br><br>With your entire ");
        if (liveData.player.cocks[x].cockThickness >= 3)
            GUI.outputText("impressive ");
        GUI.outputText("girth within her she settles down on your lap, stretching her legs out before retrieving her fishing rod. \"<i>Now don't you go movin' about, darlin',</i>\" Callu says over her shoulder. \"<i>Don't wanna go scarin' the fish away. I'll let ya go after I catch a few good ones.</i>\"");
        GUI.outputText("<br><br>Surprisingly, you can still feel a throbbing around your " +
            liveData.player.cockDescript(x) +
        ...
        if (liveData.player.cumQ() >= 250) {
            ...
            if (liveData.player.cumQ() < 500)
                GUI.outputText("some of it begins to dribble down your cock, forming a puddle right under your ass cheeks");
            else if (liveData.player.cumQ() < 1000)
                GUI.outputText("it begins to spray out of the edges of your cock, like water coming out of a blocked faucet");
            else if (liveData.player.cumQ() < 2000)
                GUI.outputText("Callu's stomach begins to visibly inflate, even from your point of view");
            else
                GUI.outputText("Callu's stomach inflates to a huge degree. She suddenly looks to be about eight months pregnant, though she doesn't seem bothered by this in the least");
            GUI.outputText(".");
        }
        ...
        if (liveData.player.cumQ() >= 2000)
            GUI.outputText(", despite the huge belly she now sports");
    ...
    static ottersForGals() {
        ...
        if (liveData.player.hasFuckableNipples())
            GUI.outputText("You even slip a finger or two inside, stretching your nipple-cunts out with deliciously pleasurable results. ");
        GUI.outputText("Combined with Callu's tender tongue ");
        if (liveData.player.hasVagina())
            GUI.outputText("paying lip service to your wet cunt");
        else
            GUI.outputText("doing a cave dive in your rear");
        GUI.outputText(", you can't hold out much longer. All the tingling in your body seems to get forced through your veins, coalescing in a single spot within your groin. The pressure builds and builds as orgasmic energies begin overflowing. Your legs and arms tremble, your head wobbles uncertainly, and you can't even guess at what your spine is attempting to do.");
        ...
        if (liveData.player.hasVagina())
            GUI.outputText("Your snatch clenches and clamps down on thin air, flooding Callu's face with your feminine juices in a tasty, refreshing spray.");
        else
            GUI.outputText("Your asshole clenches and spasms randomly, aching to be filled by something, anything in your quest for release.");
