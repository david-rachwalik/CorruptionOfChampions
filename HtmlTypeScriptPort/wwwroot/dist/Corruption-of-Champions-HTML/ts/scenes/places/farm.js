...
class PlacesFarm {
    static farmExploreEncounter() {
        ...
        //Repeat Offender
            ...
            if (liveData.gameFlags[FLAG.KELT_KILLED] >= 1) {
                ...
                GUI.outputText("<br><br>You hold your hands up, knowing full-well what the angry bitch is on about. She angrily says \"<i>You've fucking killed Kelt the centaur! He may be rude and I don't like him but still, what you've done is wrong. You're not welcome on my farm anymore! Leave. <b>Now.</b></i>\"");
            ...
            else if (liveData.gameFlags[FLAG.WHITNEY_FLIPPED_OUT_OVER_KELLY] > 0)
                GUI.outputText("You aren't welcome on the farm proper, but you can see Kelly cantering about the fields, looking for you.");
            else
                GUI.outputText("Whitney's farm is remarkably large for such a humble operation. What do you want to do?");
            GUI.menu();
    ...
    static exploreFarm() {
        ...
        if (explore == 0) {
            ...
            GUI.outputText("<br><br>Afterwards, the both of you lie back against a tree, panting heavily and exchanging pleasantries. Once you've both had a chance to rest, she bids you farewell and returns to her labors, leaving you to journey home to camp.");
            GUI.doNext(Camp.returnToCampUseOneHour);
            return;
        }
        //Other stuff
        if (explore == 1) {
            GUI.outputText("After wandering around for a while, you find yourself atop a slight rise looking out over the farm and the distant lake. Despite the corruption you know is slowly consuming this land, being here now makes you feel so at peace you wish it could go on forever.");
        ...
        //Cows
        if (explore == 2) {
            GUI.outputText("Your explorations take you to the cow pasture. There's no bull here, so the cows are all placidly grazing, building up milk for Whitney to gather. One turns to face you, and you get the odd feeling that it's trying to tell you something.<br><br>");
            ...
            if (liveData.player.horseCocks() > 0) {
                ...
                if (liveData.player.cor > 60)
                    GUI.outputText("It makes you smile.");
                if (liveData.player.cor < 30)
                    GUI.outputText("It disgusts you.");
                //[else/mid-corruption]
                else if (liveData.player.cor <= 60)
                    GUI.outputText("You aren't sure how you feel about that.");
    ...
    static workFarm() {
        ...
        if (UTIL.rand(4) == 0) {
            GUI.displaySprite("whitney");
            GUI.outputText('You find Whitney getting a scythe out of her tool shed. "<i>Do you know how to muck out a stable?</i>" she asks when you offer to help. You admit that you did a lot of that while growing up in your village. After passing you a rake, shovel, and pitchfork, she leads you to the milking barn.');
            ...
            if (liveData.player.str <= 50)
                ...
                liveData.player.modStats(["str", UTIL.rand(2)]);
            GUI.doNext(Camp.returnToCampUseOneHour);
        }
        else {
            GUI.displaySprite("whitney");
            GUI.outputText("You ask Whitney if she could use help with anything and she points towards the pepper fields, \"<i>Ya mind gathering up some peppers for an hour or two?  I'm gonna need a few for supper tonight.  I'll even let you keep the best one!</i>\"\n\n");
            GUI.outputText("You nod and borrow a basket, and set off towards the fields.  The next two hours are a blur of sweat and hard work as you prowl between the rows of plants, picking as many ripe red peppers as you can find.  When you finish, you drop the basket by Whitney's door, but not before taking your pepper.\n");
