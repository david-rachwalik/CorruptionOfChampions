...
Places.Farm.farmExploreEncounter = function () {
    ...
    if (gameFlags[MET_WHITNEY] < 2) {
        if (gameFlags[MET_WHITNEY] == 0) {
            ...
            if (gameFlags[MET_WHITNEY] == 2)
                outputText("<b>You've been to the farm enough to easily find it. You can return by selecting it from the places menu (and will no longer encounter it during random lake exploration)</b>.<br><br>");
        }
    ...
    //Repeat Offender
        ...
        else if (gameFlags[WHITNEY_FLIPPED_OUT_OVER_KELLY] > 0)
