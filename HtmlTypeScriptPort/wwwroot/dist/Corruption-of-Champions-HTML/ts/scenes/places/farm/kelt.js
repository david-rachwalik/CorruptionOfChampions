...
function bowSkill(amount) {
    ...
    //Clamp values
    if (player.bowSkill > 100)
        player.bowSkill = 100;
    if (player.bowSkill < 0)
        player.bowSkill = 0;
}
    ...
    //Repeated encounter
        ...
        if (gameFlags[KELT_SUBMISSIVENESS] >= 100) {
            if (((player.lib + player.lust) < 30 && player.inte >= 50) || gameFlags[KELT_BAD_END_WARNING] == 0) {
                ...
                KeltScene.keltSubmissiveBadEnd();
            return;
        }
        //Centaur bad end
...
KeltScene.keltMainEncounter = function () {
    ...
    //(Submissive 0-30:
    if (gameFlags[KELT_SUBMISSIVENESS] <= 30)
        outputText("You grind your teeth in irritation, but swallow your pride enough to ask him for help. ", false);
    //(Submissive 30-70:
    else if (gameFlags[KELT_SUBMISSIVENESS] <= 70)
        outputText("You nod reluctantly, and Kelt grins. He may be arrogant, but he is rather good. ", false);
    //(Submissive 70-100:
    else
        outputText("You nod enthusiastically, almost begging him to teach you. ", false);
    ...
    //(Naked Off)
        ...
        //(Otherwise:
            ...
            //(Otherwise:
                ...
                outputText("You ignore his barbed comments as best as you can, and soon enough, he leaves the matter alone, instead critiquing your archery skills.\r\r", false);
        }
    }
...
KeltScene.keltMainEncounter3 = function () {
    ...
    if (player.findStatusEffect(StatusEffects.NakedOn) < 0) {
        ...
        //(25% Chance:
        if (temp == 0)
            outputText("\"<i>Oh, yes. Looks like you're shaping up to be a pretty accurate archer. Just make sure you only fight blind giants. You know, massive targets that have no chance to dodge. You'll do fine.</i>\"\r\r", false);
        //(25% Chance:
        if (temp == 1)
            outputText("\"<i>Ha! You want to become an archer? Here's a hint... stop thinking about sucking cock for two seconds and AIM.</i>\"\r\r", false);
        //(25% Chance:
        if (temp == 2)
            outputText("\"<i>See the red dot, in the middle of the target? Imagine that's your ass, and the arrow is a big, fat, minotaur cock. That should help you hit the target.</i>\"\r\r", false);
        //(25% Chance:
        if (temp == 3) {
            if (player.race() != "centaur")
                outputText("\"<i>If you were a centaur, I'd recommend suicide. Since you're a " + player.race() + ", I'd say your best option is to fuck off.</i>\"\r\r", false);
            else
                outputText("\"<i>As a centaur, I'd recommend suicide. Really, it's that or man the fuck up.</i>\"\r\r", false);
        }
    ...
    //NAKERS
        ...
        if (player.inHeat && player.gender > 1) {
            ...
            if (player.statusEffectv2(StatusEffects.Kelt) <= 30)
                outputText("You struggle as best as you can, but Kelt weighs a good deal more than you do. As his thrusting hips anxiously press his cock to your nether-lips, you realize this is going to happen, whether you want it to or not. The thought fills you with an undeniable shiver of pleasure.\r\r", false);
            //(Submissive, 30-70:
            ...
            //(Submissive, 0-30:
            if (player.statusEffectv2(StatusEffects.Kelt) <= 30)
                outputText("You shiver and groan, unable to help yourself. It is clear that Kelt has every intention of breeding you, and you are helpless to stop the urges of your body. Terrifying images of being raped daily by this cruel beast fill your head... of your belly swelling with his young again and again. You let out a moaning cry, and orgasm helplessly even as Kelt laughs.\r\r", false);
            //(Submissive, 30-70:
            else if (player.statusEffectv2(StatusEffects.Kelt) <= 70)
        ...
        if (temporary == 3) {
            ...
            if (player.biggestTitSize() <= 5)
                outputText("\"<i>Ha! Even with your itty-bitty-titty, you have to admit to a certain... weakness, is it? Awfully sensitive, aren't they? Ooh, am I making the little girl wet? Naughty slut!</i>\"\r\r", false);
            //(Medium Size, E-HHH:
            else if (player.biggestTitSize() <= 11)
                outputText("\"<i>Just look at these mommy melons! You want to be an archer? I'm amazed you don't slap yourself in the tits with every shot! Easy target to grab onto. But hey, I bet you like it that way. Like it when people grope these fat titties of yours?</i>\"\r\r", false);
            //(Big Size, Watermelon and beyond:
            else
                outputText("\"<i>Look at you! You're a miracle of science, you are... any other creature would break their back, trying to haul these milk bags around! Tell the truth, now... which one of your parents was actually a cow?</i>\"\r\r", false);
    ...
    if (player.findStatusEffect(StatusEffects.BlowjobOn) >= 0) {
        ...
        if (player.lust >= 75 || gameFlags[KELT_SUBMISSIVENESS] >= 90 && rand(2) == 0) {
            ...
            if (player.totalCocks() > 0)
                outputText(" Your " + player.cockDescript(0) + " is rock hard, but he ignores it almost contemptuously. You almost feel ashamed of it, compared to the slowly-growing manhood between his legs.", false);
            //(Vagina:
            if (player.hasVagina())
            ...
            //(Vagina:
            if (player.hasVagina())
                outputText(" Your poor, neglected pussy quivers with delight, convulsing without even being touched. Thick juices run freely down your leg, dripping off to splatter the hay below you as you moan like a bitch in heat.", false);
            //(Genderless:
            if (player.gender == 0)
                outputText(" Never before has a complete lack of genitalia been so frustrating... or pleasurable. Not being able to physically cum, the explosions of climax simply rack up within your body... waves of cascading pleasure with no release and no mercy.", false);
            ...
            if (player.looseness(false) < 3)
                outputText("\"<i>Fuck! About time! Sure got a tight little ass back here, but don't worry, slut. We'll stretch it out in no time. I promise you this... you'll be getting a lot more of my cock from now on, so you'd better be ready!</i>\"\r\r", false);
