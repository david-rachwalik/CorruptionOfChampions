...
Inventory.useItem = function (item, fromSlot) {
    ...
    if (!item.useItem())
        Inventory.itemGoNext(); //Items should return true if they have provided some form of sub-menu.
...
Inventory.takeItemFull = function (itype, showUseNow, source) {
    ...
    if (showUseNow)
        addButton(13, "Use Now", Inventory.useItemNow, itype, source);
...
Inventory.returnItemToInventory = function (item, showNext) {
...
Inventory.stashMenu = function () {
    ...
    */
    ...
    if (gameFlags[HAS_WEAPON_RACK] == 1) {
        ...
        if (Inventory.weaponRackFilled())
            addButton(3, "W.Rack Take", Inventory.takeFromWeaponRack);
    }
    //Armor Rack
    if (gameFlags[HAS_ARMOR_RACK] == 1) {
        ...
        if (Inventory.armorRackFilled())
            addButton(6, "A.Rack Take", Inventory.takeFromArmorRack);
    }
    //Shield Rack
    if (gameFlags[HAS_EQUIPMENT_RACK] == 1) {
        ...
        if (Inventory.shieldRackFilled())
            addButton(8, "S.Rack Take", Inventory.takeFromShieldRack);
    }
    /* Jewelry Box Code
...
Inventory.takeFromStorage = function (Array, startSlot, endSlot, text) {
    ...
    for (var x = startSlot; x < endSlot; x++, button++) {
        if (player.itemSlots[x].quantity > 0)
            addButton(button, (player.itemSlots[x].itype.shortName + " x" + player.itemSlots[x].quantity), Inventory.pickFrom, x);
    }
    addButton(14, "Back", Inventory.stashMenu);
};
