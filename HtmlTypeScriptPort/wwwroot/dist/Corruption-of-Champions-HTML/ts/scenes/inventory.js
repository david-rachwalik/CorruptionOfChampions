...
class Inventory {
    ...
    static inventoryMenu() {
        ...
        if (liveData.player.keyItems.length > 0)
            GUI.outputText("<b><u>Key Items:</u></b><br>");
        for (let x = 0; x < liveData.player.keyItems.length; x++)
            GUI.outputText(liveData.player.keyItems[x].ktype.id + "<br>");
    ...
    static replaceItem(itype, slotNum) {
        ...
        if (liveData.player.itemSlots[slotNum].itype == itype)
            ...
            if (liveData.player.itemSlots[slotNum].quantity == 1)
                ...
                GUI.outputText("You throw away " + liveData.player.itemSlots[slotNum].itype.longName + "(x" + liveData.player.itemSlots[slotNum].quantity + ") and replace it with " + itype.longName + ".");
