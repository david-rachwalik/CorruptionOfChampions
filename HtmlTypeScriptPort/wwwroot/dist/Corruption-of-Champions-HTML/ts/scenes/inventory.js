...
class Inventory {
    ...
    inventoryMenu() {
        ...
        for (x = 0; x < player.keyItems.length; x++)
            outputText(player.keyItems[x].ktype.id + "<br>");
    ...
    takeItemFull(itype, showUseNow, source) {
        ...
        for (x = 0; x < 10; x++) {
            if (player.itemSlots[x].itype != Items.NOTHING && x < player.getMaxSlots())
                addButton(x, player.itemSlots[x].itype.shortName + " x" + player.itemSlots[x].quantity, this.replaceItem, itype, x);
        }
        if (source != null) {
        ...
        if (showUseNow)
            addButton(13, "Use Now", this.useItemNow, itype, source);
    ...
    returnItemToInventory(item, showNext) {
        ...
        //Items with sub menus should return to the inventory screen if the player decides not to use them
            ...
            callNext(); //When putting items back in your stash we should skip to the take from stash menu
    }
        ...
        */
        ...
        if (gameFlags[HAS_WEAPON_RACK] == 1) {
            ...
            if (this.weaponRackFilled())
                addButton(3, "W.Rack Take", this.takeFromWeaponRack);
        }
        //Armor Rack
        if (gameFlags[HAS_ARMOR_RACK] == 1) {
            ...
            if (this.armorRackFilled())
                addButton(6, "A.Rack Take", this.takeFromArmorRack);
        }
        //Shield Rack
        if (gameFlags[HAS_EQUIPMENT_RACK] == 1) {
            ...
            if (this.shieldRackFilled())
                addButton(8, "S.Rack Take", this.takeFromShieldRack);
        }
        /* Jewelry Box Code
        ...
        if (this.itemAnyInStorage(player.itemSlots, 10, 18)) {
            ...
            for (x = 10; x < 19; x++) {
                if (player.itemSlots[x].quantity > 0)
                    itemList[itemList.length] = player.Itemslots[x].itype.longName;
    ...
    */
    ...
    takeFromStorage(Array, startSlot, endSlot, text) {
        ...
        for (var x = startSlot; x < endSlot; x++, button++) {
            if (player.itemSlots[x].quantity > 0)
                addButton(button, player.itemSlots[x].itype.shortName + " x" + player.itemSlots[x].quantity, this.pickFrom, x);
        }
        addButton(14, "Back", this.stashMenu);
    }
    pickFrom(slotNum) {
