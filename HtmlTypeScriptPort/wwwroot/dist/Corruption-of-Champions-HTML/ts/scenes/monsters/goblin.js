...
class Goblin extends Creature {
    ...
    goblinTeaseAttack() {
        ...
        if (liveData.monster.refName == "goblin" || liveData.monster.refName == "goblin assassin" || liveData.monster.refName == "Tamani") {
            if (det == 0)
                GUI.outputText(UTIL.capitalize(liveData.monster.a) + liveData.monster.refName + ' runs her hands along her leather-clad body and blows you a kiss. "<i>Why not walk on the wild side?</i>" she asks.');
            if (det == 1)
                GUI.outputText(UTIL.capitalize(liveData.monster.a) + liveData.monster.refName + " grabs her heel and lifts it to her head in an amazing display of flexibility.  She caresses her snatch and gives you a come hither look.");
            if (det == 2)
                GUI.outputText(UTIL.capitalize(liveData.monster.a) + liveData.monster.refName + " bends over, putting on a show and jiggling her heart-shaped ass at you.  She looks over her shoulder and sucks on her finger, batting her eyelashes.");
        }
        else if (liveData.monster.refName == "goblin warrior") {
            if (det == 0)
                GUI.outputText(UTIL.capitalize(liveData.monster.a) + liveData.monster.refName + ' runs her hands along her metal-clad body and blows you a kiss. "<i>Why not walk on the wild side?</i>" she asks.');
            if (det == 1)
                GUI.outputText(UTIL.capitalize(liveData.monster.a) +
            ...
            if (det == 0)
                GUI.outputText(UTIL.capitalize(liveData.monster.a) + liveData.monster.refName + ' runs her hands along her leather-clad body and blows you a kiss. "<i>Why not walk on the wild side?</i>" she asks.');
            if (det == 1)
                GUI.outputText(UTIL.capitalize(liveData.monster.a) + liveData.monster.refName + " grabs her heel and lifts it to her head in an amazing display of flexibility.  She lifts her loincloth and caresses her snatch and gives you a come hither look.");
            if (det == 2)
                GUI.outputText(UTIL.capitalize(liveData.monster.a) + liveData.monster.refName + " bends over, putting on a show and jiggling her heart-shaped ass at you.  She looks over her shoulder and sucks on her finger, batting her eyelashes.");
        }
        else if (liveData.monster.refName == "goblin elder") {
            if (det == 0)
                GUI.outputText(UTIL.capitalize(liveData.monster.a) + liveData.monster.refName + ' runs her hands along her bone-clad body and blows you a kiss. "<i>Why not walk on the wild side?</i>" she asks.');
            if (det == 1)
                GUI.outputText(UTIL.capitalize(liveData.monster.a) + liveData.monster.refName + " grabs her heel and lifts it to her head in an amazing display of flexibility.  She lifts her loincloth and caresses her snatch and gives you a come hither look.");
            if (det == 2)
                GUI.outputText(UTIL.capitalize(liveData.monster.a) + liveData.monster.refName + " bends over, putting on a show and jiggling her heart-shaped ass at you.  She looks over her shoulder and sucks on her finger, batting her eyelashes.");
        }
        var lustDmg = UTIL.rand(liveData.player.lib / 10) + 8;
        if (liveData.monster.refName == "goblin assassin")
            lustDmg *= 1.4;
        if (liveData.monster.refName == "goblin warrior")
            lustDmg *= 1.6;
        if (liveData.monster.refName == "goblin shaman")
            lustDmg *= 1.6;
        if (liveData.monster.refName == "goblin elder")
            lustDmg *= 2;
        lustDmg = Math.round(lustDmg);
        GUI.outputText("  The display distracts you long enough to prevent you from taking advantage of her awkward pose, leaving you more than a little flushed.  ");
        liveData.player.changeLust(lustDmg, true);
    }
    // This drug attack is used across all Goblins, including Tamani and Daughters.
    goblinDrugAttack() {
        var temp2 = UTIL.rand(2);
        ...
        if (liveData.monster.refName == "Tamani")
            temp2 = UTIL.rand(5);
        ...
        if (color != "blue") {
            if (liveData.monster.refName == "Tamani's daughters")
                ...
                GUI.outputText(UTIL.capitalize(liveData.monster.a) + liveData.monster.refName + " uncorks a glass bottle full of " + color + " fluid and swings her arm, flinging a wave of fluid at you.");
        }
        //Drink blue pots
            ...
            if (this.HPRatio() < 1) {
                ...
                GUI.outputText("  There doesn't seem to be any effect.<br>");
            GUI.outputText("<br>");
            return;
        }
        else {
            ...
            if (this.HP < this.maxHP()) {
                ...
                if (liveData.monster.name == "Tamani")
                    liveData.monster.changeHP((liveData.monster.maxHP() / 4) * multiplier);
            }
            else
                GUI.outputText("  There doesn't seem to be any effect.<br>");
            return;
        ...
        if ((liveData.player.findPerk(PerkLib.Evade) >= 0 && UTIL.rand(10) <= 3) || UTIL.rand(100) < liveData.player.spe / 5) {
            ...
            if (color == "red") {
                ...
                //Green poison
                GUI.outputText("<br>The greenish fluids splash over you, making you feel slimy and gross.  Nausea plagues you immediately - you have been poisoned!<br>");
                if (liveData.player.findStatusEffect(StatusEffects.Poison) < 0)
                    liveData.player.createStatusEffect(StatusEffects.Poison, 0, multiplier, 0, 0);
            }
            else if (color == "white") {
                //sticky flee prevention
                GUI.outputText("<br>You try to avoid it, but it splatters the ground around you with very sticky white fluid, making it difficult to run.  You'll have a hard time escaping now!<br>");
...
class GoblinScene {
    // WIN SCENES
    static victoryAgainstGoblin() {
        GUI.clearOutput();
        if (liveData.monster.HP <= 0)
            GUI.outputText("The goblin falls down and smashes her titties against the ground. She looks up at you and sniffles.");
        else {
            GUI.outputText("The goblin groans and drops onto her back.  Her legs spread wide, displaying amazing flexibility as one hand dives into her cunt and the other begins twisting her pierced nipples, one at a time.  The display manages to stir your loins.");
        ...
        if (liveData.player.lust >= 33 && liveData.player.gender > 0) {
            ...
            if (liveData.player.hasCock()) {
                if (liveData.player.cockThatFits(liveData.monster.vaginalCapacity()) >= 0) {
                    GUI.addButton(0, "Dick Fuck", GoblinScene.gatsGoblinBoners, null, null, null, "TO BE ADDED");
                    GUI.addButton(4, "Jog Fuck", GoblinScene.gobboGetsRapedMaleFits, null, null, null, "TO BE ADDED");
                }
                if (liveData.player.cockArea(liveData.player.biggestCockIndex()) > liveData.monster.vaginalCapacity()) {
                    GUI.addButton(1, "DickTooBig", GoblinScene.manRapesGoblinTooBig, null, null, null, "TO BE ADDED");
                }
                if (liveData.player.cockArea(liveData.player.biggestCockIndex()) > liveData.monster.vaginalCapacity() && liveData.player.cor > 80) {
                    ...
                    GUI.addButton(2, "CorruptBig", GoblinScene.rapeAGoblinCorruptTooBig, null, null, null, "TO BE ADDED");
                }
                if (liveData.player.cockThatFits(liveData.monster.analCapacity()) >= 0 && (liveData.player.cor > 70 - liveData.player.corruptionTolerance() || liveData.gameFlags[FLAG.MEANINGLESS_CORRUPTION] >= 1)) {
                    GUI.addButton(3, "Dick In Ass", GoblinScene.goblinButtSex, null, null, null, "TO BE ADDED");
                }
                if (liveData.player.findStatusEffect(StatusEffects.Feeder) >= 0) {
                    //CHECK THIS STATUS EFFECT!
                    GUI.addButton(5, "Breastfeed", GoblinScene.giveGoblinAMilkMustache, null, null, null, "TO BE ADDED");
                }
                if (liveData.player.tailType == ENUM.TailType.TAIL_TYPE_SPIDER_ADBOMEN && liveData.player.cockThatFits(liveData.monster.vaginalCapacity()) >= 0) {
                    GUI.addButton(6, "SpiderCondom", GoblinScene.goblinCondomed, 0, null, null, "TO BE ADDED"); // Pass argument to do Spider condom
                }
                else if (liveData.player.hasItem(Items.Consumables.Condom) && liveData.player.cockThatFits(liveData.monster.vaginalCapacity()) >= 0) {
                    GUI.addButton(6, "Use Condom", GoblinScene.goblinCondomed, 1, null, null, "TO BE ADDED"); // Pass argument to do normal condom
                }
            }
            if (liveData.player.hasVagina()) {
                GUI.addButton(7, "Pussies", GoblinScene.goblinFemaleRape, null, null, null, "TO BE ADDED");
            }
            if (liveData.player.canOvipositSpider()) {
                GUI.addButton(8, "Lay Eggs", GoblinScene.goblinDriderEggs, null, null, null, "TO BE ADDED");
            }
        }
        ...
        if (liveData.player.canOvipositSpider() || liveData.player.findStatusEffect(StatusEffects.Feeder) >= 0) {
            ...
            if (liveData.player.findStatusEffect(StatusEffects.Feeder) >= 0)
                ...
                GUI.outputText("your abdomen aches with the desire to impregnate her full of insect eggs.  Do you?</b>");
            if (liveData.player.findStatusEffect(StatusEffects.Feeder) >= 0) {
                //CHECK THIS STATUS EFFECT!
                GUI.addButton(5, "Breastfeed", GoblinScene.giveGoblinAMilkMustache, null, null, null, "TO BE ADDED");
            }
    ...
    static gatsGoblinBoners() {
        ...
        GUI.outputText('"<i>W-woah!  Hey stud, whaddya think you\'re doing back there?</i>" she yelps, more surprised than scared at your sudden appearance.  Instead of answering, you decide to grab your cock');
        if (liveData.player.cockTotal() > 1)
            GUI.outputText("s");
        ...
        if (liveData.player.cockTotal() == 1)
            ...
            GUI.outputText("them");
        GUI.outputText(" against the bare flesh of her ass, whilst your victim anxiously awaits your next move.  You take your time massaging the goblin's slutty ass with your bare hands before sliding your " +
        ...
        if (liveData.player.cockTotal() == 1)
            GUI.outputText("your dick inside of the panting whore, pushing her forwards violently as you enter her tight snatch");
            ...
            if (liveData.player.cockTotal() >= 4)
                GUI.outputText("s");
            GUI.outputText(" remaining sandwiched in between her asscheeks");
        }
        GUI.outputText(".<br><br>");
        GUI.outputText("You roughly pound against the goblin girl, maintaining a firm grip on her hips while she squeals with delight.  The sound of your groin slapping against her echoes throughout the area, followed by your grunting and the goblin's moans of ecstasy.  Your victim struggles to lift herself up by her arms, only to collapse back down from the feeling of you invading her insides.<br><br>");
        GUI.outputText("Eventually you begin to feel yourself coming to a climax, your movements getting faster and faster as you build up to your release.  The goblin below you has already lost herself to the pleasure of your " +
            liveData.player.cockDescript(x) +
        ...
        if (liveData.player.cockTotal() == 2)
            GUI.outputText("s");
        GUI.outputText(".  You slowly release yourself from her tight body, finishing off by covering her curved back and pert rump with the rest of your seed.<br><br>");
        GUI.outputText("You pick yourself back up, jerking yourself slowly as cum dribbles from your " +
            liveData.player.cockDescript(x) +
    ...
    // Male jog fuck
    static gobboGetsRapedMaleFits() {
        var x = liveData.player.cockThatFits(liveData.monster.vaginalCapacity());
        if (x < 0)
            x = liveData.player.biggestCockIndex();
        GUI.clearOutput();
        ...
        if (liveData.player.cockArea(x) > liveData.monster.vaginalCapacity() * 0.8) {
            GUI.outputText('You pick up the defeated goblin, looking her over. She crosses her arms across her chest pitifully and asks, "<i>What now?</i>" with her eyes darting down when she thinks you won\'t notice. A grimace temporarily crossing her face at the size of your ' +
                liveData.player.cockDescript(x) +
                ...
                " as possible.<br><br>");
            GUI.outputText("You start walking, letting your movements work with gravity, allowing you to penetrate her with little difficulty. Those puffy wet walls clench you tightly as she slides down, ");
            if (liveData.player.cocks[0].cockType == ENUM.CockType.DEMON)
                GUI.outputText("rubbing painfully against your demonic nubs");
            else if (liveData.player.hasKnot(0))
                GUI.outputText("stretching painfully around your knot");
            ...
            GUI.outputText(". With each and every step she slides down further, stretching her to capacity, until she sits almost completely impaled on you, grabbing your ");
            if (liveData.player.biggestTitSize() >= 1)
                GUI.outputText(liveData.player.allBreastsDescript());
            else
                GUI.outputText("torso");
            GUI.outputText(" to help support herself.  A steady pulse of motion massages you in time with the green girl's breathing.  You realize just how much of her body must be devoted to accommodating monstrous members, no wonder goblins are so fragile in a fight!<br><br>");
            GUI.outputText('She pants happily, her tongue rolling free from her mouth as she comments, "<i>So full. . .</i>"  Still wincing from the monster inside her she begins to cheer you on, "<i>oooh go-ah-faster! I wanna bounce!</i>"<br><br>');
            GUI.outputText("It's all the encouragement you need, and you break into a run, feeling her lithe form bounce on your " +
                liveData.player.cockDescript(x) +
            ...
            if (liveData.player.biggestTitSize() >= 1)
                ...
                GUI.outputText("skin");
            GUI.outputText(" as she hangs on, clenching and smashing her ample tits against you. You run hard, feeling her bounce and wriggle as her cunt and rapid breathing squeezing and milking you like you never before. You're sure if you could feel like this every time you took a jog, you'd be in great shape.<br><br>");
            GUI.outputText('"<i>Ooh fuck stud, bounce me! Yeah just like that,</i>" she moans, "<i>Are you gonna cum? Omigod please cum, I need you to fill me up just like this!</i>"<br><br>');
            GUI.outputText("The familiar tightness of a coming orgasm grows in your groin, tightening as you near release. You pick up the pace, full out sprinting, letting the girl bounce and jiggle as she clings to you, supported entirely by your " +
                liveData.player.cockDescript(x) +
                ". ");
            if (liveData.player.balls > 0)
                GUI.outputText("Your " + liveData.player.ballsDescriptLight() + " tighten, releasing the seed of your orgasm.  ");
            GUI.outputText("The howl of a powerful orgasm fills your ears as your cumming sets off the little green cock-sleeve. One of her hands lets go, and starts rubbing her belly while she kisses and licks your belly-button.");
            if (liveData.player.cumQ() >= 100)
                GUI.outputText("  Your enhanced body easily stuffs her full of cream, pudging her belly out slightly, your seed staying embedded in her womb with nowhere to escape, her cunt plugged tightly with your " + liveData.player.cockDescript(x) + ".");
            if (liveData.player.cumQ() >= 500)
                GUI.outputText("  The orgasm is so potent that by the time you wind down, she looks to be sporting a pregnancy the size of a medicine ball.  Your cum is trapped inside her, unable to find any gap between her walls and your " +
                    liveData.player.cockDescript(x) +
                    ".");
            else if (liveData.player.cumQ() >= 250)
                GUI.outputText("  The orgasm is so potent that by the time you wind down, she looks heavily pregnant.  Your cum is unable to find any gap between her walls and your " + liveData.player.cockDescript(x) + ".");
            GUI.outputText("<br><br>");
            ...
            if (liveData.player.cumQ() >= 250)
                GUI.outputText("pours out in a river");
            else
                GUI.outputText("leaks");
            GUI.outputText(' from her now-gaping twat. She rubs her belly and blows you a kiss, still trying to catch her breath. You smirk and begin redressing. Once finished, you start walking away, but she calls out one last time to you, "<i>MMMmm I hope you don\'t mind if I find you again. I need more of your baby batter so I can give you lots of beautiful sluts to fuck!</i>"<br><br>');
            if (liveData.player.cor > 50)
                GUI.outputText("Chuckling");
            else
                GUI.outputText("Shuddering");
            GUI.outputText(", you make your way back to camp, satisfied.");
        }
        //(FITS – Get ridden)
        else {
            GUI.outputText("You pick up the defeated goblin, looking her over.  She crosses her arms across her chest pitifully and asks, \"<i>What now?</i>\" with her eyes darting down when she thinks you won't notice.  You muse to yourself 'great minds think alike' and lower her down towards your " +
                liveData.player.cockDescript(x) +
                ...
                liveData.player.hipDescript() +
                " as possible.<br><br>");
            GUI.outputText("You start walking, letting the movements work with gravity to allow you to effortlessly penetrate her.  Those puffy wet walls clench you tightly as she slides down ");
            if (liveData.player.cocks[0].cockType == ENUM.CockType.DEMON)
                GUI.outputText("rubbing perfectly against your demonic nubs");
            else if (liveData.player.hasKnot(0))
                GUI.outputText("stretching tightly around your knot");
            else if (liveData.player.cocks[0].cockType == ENUM.CockType.HORSE || liveData.player.cocks[0].cockType > 3)
            ...
            if (liveData.player.biggestTitSize() >= 1)
                ...
                GUI.outputText("torso");
            GUI.outputText(" to help support herself.   A steady pulse of motion massages you in time with the green girl's breathing, making you realize just how much of her body must be devoted to accommodating monstrous members.<br><br>");
            ...
            GUI.outputText("It's all the encouragement you need, and you break into a run, feeling her lithe form bounce on your " + liveData.player.cockDescript(x) + ", drawing out a cacophony of happy wails and moans.  Her tiny fists dig into your ");
            if (liveData.player.biggestTitSize() >= 1)
                GUI.outputText("tits");
            else
                GUI.outputText("skin");
            GUI.outputText(" as she hangs on, clenching and smashing her ample tits against you.  You run hard, feeling her bounce and wriggle as her cunt and rapid breathing begin squeezing and milking you like never before.  You're sure if you could feel like this every time you took a jog, you'd be in great shape.<br><br>");
            GUI.outputText('"<i>Ooh fuck stud, bounce me! Yeah just like that,</i>" she moans, "<i>Are you gonna cum? Omigod please cum, I need you to fill me like this!</i>"<br><br>');
            GUI.outputText("The familiar tightness of a coming orgasm grows in your groin, tightening as you near release.  You pick the pace, full out sprinting, letting the girl bounce and jiggle as she clings to you, supported entirely by your " +
                liveData.player.cockDescript(x) +
                ".  ");
            if (liveData.player.balls > 0)
                GUI.outputText("Your " + liveData.player.ballsDescriptLight() + " tighten, releasing the seed of your orgasm.  ");
            ...
            if (liveData.player.cumQ() >= 500)
                GUI.outputText("  The orgasm is so potent that by the time you wind down, she looks heavily pregnant and your cum squirts out of any gap it can find between her walls and your " + liveData.player.cockDescript(x) + ".");
            GUI.outputText("<br><br>");
            GUI.outputText("You pant and stop, pulling the stuffed goblin off you and setting her on the ground, smiling in satisfaction as your cum ");
            if (liveData.player.cumQ() >= 250)
                GUI.outputText("pours out in a river");
    ...
    static goblinFemaleRape() {
        ...
        // Are you a naga?
            ...
            //[No fangs:
                ...
                GUI.outputText("You smile at her and the fear disappears. She's still wary though, and you make sure to grab her with your tail before she changes her mind about you.");
            GUI.outputText("<br><br>");
            GUI.outputText("You wrap yourself tightly around your struggling prey, in the process removing her slutty 'clothes'. The incapacitated goblin whimpers plaintively at you and you respond by giving her cunt a smack with your hand. Pulling your hand away, you're surprised at how wet it is. You wipe it dry on her face and bring your head down to her ear.<br><br>");
            //[Corrupt characters:
            if (liveData.player.cor > 60) {
                GUI.outputText("You hiss something incoherent to the terrified woman, who starts to quiver in your grip. Laughing, your fingers slide into her mouth and she begins to suck on them in an attempt to appease you. Her tiny tongue feels wonderful; clearly she's very experienced at this.  ");
                GUI.outputText("Using your fingers, you spread open her mouth. She's confused but can't resist as you fiddle with something beside you. She realizes what's about to happen too late, as you pull off a number of shiny flasks from her pouches. As punishment for trying to poison you, you start emptying bottle after bottle into her mouth, stroking her throat and forcing her to drink them down.<br><br>");
            }
            //[Non-corrupt characters:
            else {
                GUI.outputText("You ask her if she's sorry for trying to poison you as your fingers slide around her face. She nods vehemently at you, too constricted or afraid to answer. You smile pleasantly at her and feel her body relax a little in your grip.  Your fingers slide into her mouth and she sucks on them eagerly, clearly not wanting to anger you. Her tongue is talented and you enjoy the experience for a little bit until you decide you've toyed with her enough.<br><br>");
                GUI.outputText("Opening your fingers, you open her mouth and prevent it from closing. The confused goblin tries to see what you're doing beside her but can't. When you bring up a handful of shiny flasks from her pouches though, her body tenses again and she whimpers at you.<br><br>");
                GUI.outputText("Your pity for the creature doesn't extend quite far enough to prevent you from punishing her though, and you pick out some of the less potent looking concoctions from the bunch. While stroking her throat gently to make sure she swallows you pour in vial after vial.<br><br>");
            }
            GUI.outputText("The effects don't take long to materialize, and soon the slut is a purplish hue and desperately trying to grind against your coils. She pants and moans in frustration as her dripping cunt can't get enough pleasure from your smooth underbelly, while you wait, enjoying the sensations.<br><br>");
            GUI.outputText("You slide your tail up to her cunt and tweak her clit with the tip. She immediately releases a gush of fluids, thoroughly coating your already slick and sticky coils in more of her cum. You waggle your finger in front of her face to tell her off; you're not done with her yet.<br><br>");
            GUI.outputText("With a smooth motion your tail slides inside her, causing her to moan in pleasure and buck her hips. You squirm about inside, stretching her so wide you suspect she'll be gaping for quite a while. The tip of your tail hits the end of her love canal and you're surprised to find her uterus is pulsating, trying to grip at something that's not there. Curious, you move your tail up to the opening, which grabs you and drags you inside her womb. You smile at the pleasure-overloaded goblin and begin stroking at the walls. She clamps down hard on you and screams out in ecstasy as her eyes roll back into her head.<br><br>");
            GUI.outputText("With no warning you pull out your entire tail, feeling a massive gush of fluids sliding out behind you. The goblin is barely conscious, so you look around inside her pouch for something to help. Nothing seems to look like it would help your cause though, and you're beginning to regret being so vicious, when you notice a particularly tiny flask at the very bottom.<br><br>");
            GUI.outputText("You pull it out and examine it. Not much more than a centimeter long, it's filled with a white fluid. There's a label, but the lettering is far too small to read. You think you can make out the word \"<i>wake</i>\", but aren't entirely sure.<br><br>");
            GUI.outputText("Shrugging, you dump the contents into the goblin's mouth, little more than a drop that's quickly absorbed into her tongue. For a moment nothing happens, then the tiny figure starts to shake violently. Not sure what to do, you simply stay as you are, holding her tight. The shaking stops as suddenly as it started though, and you think you might have killed her.<br><br>");
            GUI.outputText("That's proven very false in a moment though, as her eyes fly open and she yells out \"<i>WHOOOOOO!</i>\". You're startled at the sudden change in demeanor, but decide to roll with it, shoving your " +
                liveData.player.vaginaDescript(0) +
                " into the evidently very energetic little creature.<br><br>");
            GUI.outputText('You hear a muffled yell of "<i>YOU GOT IT BOSS!</i>" as she starts licking and gently chewing at you. The feeling is wonderful and you can\'t help but wonder what was in the vial, but the thought is wiped from your mind as you cum, spraying all over her.<br><br>');
            GUI.outputText("You orgasm repeatedly, the goblin not tiring and the residue of the various substances you poured into her still coating her lips and tongue, making you not feel like stopping. Eventually you grow tired, releasing the goblin from your coils. She lands on her feet, does a pirouette, runs about the clearing for a bit (all while giggling like a madwoman), then collapses face first onto her 'clothes'.<br><br>");
            GUI.outputText("Thoroughly confused about what just happened, you decide not to test fate by sticking around near the heavily drugged creature and make for camp as soon as you've grabbed your things.");
            COMBAT.cleanupAfterCombat();
            liveData.player.orgasm();
        }
        // Normal female rape.
            ...
            GUI.outputText('You pick up the goblin, sitting her onto your knee and restraining both her arms behind her back with your left hand.  You tear a piece of supple leather off of her slutty garments and use it to bind her arms there.  She looks up at you, her eyes wide and frightened at the thought of being at the mercy of a larger creature.  In spite of it all, she seems more than a little turned on, if the juices staining your knee are any indication.  You look down at her and remark, "<i>So the little skank has a submissive streak huh?</i>"<br><br>');
            GUI.outputText("She blushes red and the flow of feminine fluid thickens as she nods.  You know she'll probably enjoy whatever sexual act you could perform with her, so you may as well surprise her.  A cruel idea forms in the back of your mind - getting revenge for her attempts to drug you.  You easily rip the pouches from her belt and pull out a few random bottles.  The goblin, understanding your intent, begins struggling to free herself, but you easily restrain the smaller woman and force the drugs into her mouth, one after the another.  When she tries to spit them out, you massage her throat, triggering her instinct to swallow.<br><br>");
            GUI.outputText("By the third bottle she's given up on struggling and is instead trying to grind her cunt into your knee.  Her mouth is open wide and her entire face is flushed reddish-purple with desire.  You finish force-feeding her the remaining bottles and shove her, catching her ankles to hold her over your leg with her ass in the air.  The scent of her arousal and the vapors from all the drugs and aphrodisiacs give you a bit of a 'contact high', at least in the sense that your " +
                liveData.player.vaginaDescript(0) +
                " ");
            if (liveData.player.vaginas[0].vaginalWetness < ENUM.VaginalWetnessType.VAGINA_WETNESS_WET)
                GUI.outputText("grows puffy and moist");
            else if (liveData.player.vaginas[0].vaginalWetness < ENUM.VaginalWetnessType.VAGINA_WETNESS_DROOLING)
                GUI.outputText("drips with feminine moisture");
            else if (liveData.player.vaginas[0].vaginalWetness < ENUM.VaginalWetnessType.VAGINA_WETNESS_SLAVERING)
                GUI.outputText("slowly begins to soak your thighs");
            else
                GUI.outputText("drools with need, puddling under you");
            GUI.outputText(".  Your arousal can wait.  This slut needs to be punished.<br><br>");
            GUI.outputText("*<b>THWACK</b>!* Your hand smacks her nicely rounded ass.  You pull back, feeling drops of wetness thanks to her prominent pussy-lips.  The little bitch's cunt is so inflamed with need that she's practically squirting from a simple spank.  You swat her again, watching her entire body tense and feeling her fluids splatter you.<br><br>");
            GUI.outputText('"<i>YOU GOT ME WET, YOU CUNT!!  BAD BITCH!</i>" you scold, slapping her even harder.<br><br>');
            GUI.outputText("The goblin squeals, though in delight or pain you can't be sure.  You start spanking her harder and harder, turning her ass from green to red with the repeated abuse.  Her entire body begins convulsing and squirting, splattering your arm with her honey.  She got off on it!  Well, after that kind of show, she's going to get you off too - or else!<br><br>");
            GUI.outputText("You pull her off your leg and shove her face into your " +
                liveData.player.vaginaDescript(0) +
            ...
            if (liveData.player.vaginas[0].vaginalWetness > ENUM.VaginalWetnessType.VAGINA_WETNESS_DROOLING)
                GUI.outputText("splattering her with your fluids");
                ...
                GUI.outputText("leaving the taste of pussy on her tongue");
            GUI.outputText(".  ");
            if (liveData.player.biggestLactation() >= 3.5)
                GUI.outputText("Milk explodes from your nipples, soaking the petite slut.  ");
            else if (liveData.player.biggestLactation() > 2)
                GUI.outputText("Twin streams of milk soak the slut's hair, running down her face like white tears.  ");
            else if (liveData.player.biggestLactation() >= 1)
                GUI.outputText("Milk dribbles from your nipples, falling into the little slut's hair.  ");
            GUI.outputText("Her face has a strange dopey smile on it, and she looks like she's in some strange state in between consciousness and sleep.  You grab her by the hair and toss her on the ground, watching her body twitch and jump as her orgasm continues to rack her body.  It looks like it's starting to slow down, the little twat ought to be fine.<br><br>");
            GUI.outputText("You casually dress, ignoring the pants and moans from the blissed-out goblin, and prepare to leave.   Taking one last look over your shoulder, you realize her fluids have made a puddle bigger than her.  She'll probably have a hell of a hangover when she wakes up.  You ");
            if (liveData.player.cor > 50)
                GUI.outputText("smirk");
            else
                GUI.outputText("sigh");
            GUI.outputText(" and saunter off, feeling ");
            if (liveData.player.cor < 50)
                GUI.outputText("a bit guilty about overdoing it.");
            else
                GUI.outputText("thoroughly satisfied with your revenge.");
            COMBAT.cleanupAfterCombat();
            liveData.player.orgasm();
        }
    }
    // Dick too big
    static manRapesGoblinTooBig() {
        GUI.clearOutput();
        ...
        if (liveData.player.cockArea(x) < 80)
            GUI.outputText("nearly ");
        GUI.outputText(" absurd size to your victim.  The goblin-girl's eyes grow to the size of dinner plates in shock as she takes in the view.   Knowing you'll try regardless of the size-mismatch, she spreads her legs and settles herself more comfortably on the ground.<br><br>");
        GUI.outputText("You ");
        if (liveData.player.cor < 50)
            GUI.outputText("shrug and guess you may as well try since she's ready");
        else
            GUI.outputText("grin, happy to try and stretch her around yourself");
        GUI.outputText(".  The ");
        if (liveData.player.cocks[0].cockType == ENUM.CockType.HORSE)
            GUI.outputText("flare");
        else
            GUI.outputText("head");
        GUI.outputText(" of your " +
            liveData.player.cockDescript(x) +
        ...
        if (liveData.player.cocks[0].cockType == ENUM.CockType.HORSE)
            ...
            GUI.outputText("crown");
        GUI.outputText(", but after a moment or two of progress the tiny passage will stretch no more, and you're sure you can't force any more in without hurting her.<br><br>");
        GUI.outputText("Disgruntled with the tease, you pull out and slide it onto her torso, pinning her underneath your " +
            liveData.player.cockDescript(x) +
        ...
        if (liveData.player.hasKnot(0))
            GUI.outputText(" hooking the heels of her feet just behind your knot");
        GUI.outputText(".<br><br>");
        GUI.outputText('"<i>She\'s good at this,</i>" you muse as she begins grinding underneath you, using her legs to piston her entire body up and down your length, her arms and hands rubbing and caressing you with surprising passion.  The feeling is intense - these goblins know how to please a ');
        if (liveData.player.gender == 3)
            GUI.outputText("herm");
        else
            GUI.outputText("man");
        GUI.outputText(", that's for sure!  You start dripping with excitement, soaking the goblin's face with an errant drop.  Her smile only broadens when she blinks it away.   A trail of warm wetness licks its way up your tip as the goblin greedily begins to devour your pre, going so far as to lick it from your urethra.  Your " +
            liveData.player.hipDescript() +
            " twitch, lifting her off the ground as she clings to your member.<br><br>");
        GUI.outputText("You'd never think such an act would be so enjoyable, but it's just too good.  You lose control, blasting a load of hot seed over the goblin's face.  She sputters and tries to wipe the spunk from her eyes when your next blast hits her square in the forehead, unbalancing the sprightly woman.   She thumps down hard on the ground after losing her grip on your spasming " +
            liveData.player.cockDescript(x) +
            ".  You step back, dick bobbling in the air as your orgasm finishes, ");
        if (liveData.player.cumQ() < 75)
            GUI.outputText("splattering her a few more times with potent seed.  ");
        else if (liveData.player.cumQ() < 250)
            GUI.outputText("painting her with a thick layer of seed.  ");
        else
            GUI.outputText("soaking her and leaving her in a thick puddle of seed. ");
        GUI.outputText('The green slut seems to handle it pretty well, even going so far as to scoop up your spunk and rub it into her cunt as she masturbates.  She licks her lips as she watches you redress, a sultry smile on her cum-painted face, "<i>You tasted as good as I thought stud!  Maybe shrink that bad-boy down and come visit me for a better visit next time ok?  Hopefully by then all this baby batter I\'m cramming into my box will give me a nice belly for you to rub!</i>"<br><br>');
    ...
    static rapeAGoblinCorruptTooBig() {
        var x = liveData.player.biggestCockIndex();
        GUI.clearOutput();
        //spriteSelect(24);
        //GUI.outputText(images.showImage("goblin-win-male-corruptedfuck"));
        GUI.outputText("You begin to remove your " + liveData.player.armorName + ", looking down on your helpless would-be-attacker and soon-to-be victim while licking your lips hungrily. Your " + liveData.player.multiCockDescriptLight());
        if (liveData.player.cockTotal() == 1)
            GUI.outputText(" is");
        else
            GUI.outputText(" are all");
        GUI.outputText(" far more aware of the situation than she is as you stoop down and strip her of every scrap of her admittedly sparse clothing. While you look her over, ");
        if (liveData.player.cockTotal() > 1)
            GUI.outputText("one of your " + liveData.player.multiCockDescriptLight() + " comes to rest on top of her and the fact that it's ");
        else
            GUI.outputText("your " + liveData.player.cockDescript(x) + " comes to rest on top of her and the fact that it's ");
        if (liveData.player.cocks[0].cockLength < 20)
            GUI.outputText("about as long as her entire torso");
        else
            GUI.outputText("bigger than she is");
        GUI.outputText(" gives you a wicked idea.<br><br>");
        GUI.outputText("You have a seat, legs wide, on the ground and hold the little goblin whore with her relatively tiny slit resting at the tip of your " +
            liveData.player.cockDescript(x) +
        ...
        GUI.outputText("Both of you dripping with sweat from your respective exertions, you slowly begin to realize the combination of her furious struggling and the tightness of her " +
            liveData.monster.vaginaDescript() +
        ...
        GUI.outputText("The goblin relaxes a little when she feels you no longer pressing her down onto your " +
            liveData.player.cockDescript(x) +
        ...
        GUI.outputText("You reposition your fucktoy so that now she's facing away from you and again bring your " +
            liveData.player.cockDescript(x) +
        ...
        GUI.outputText("You ram home the rest of your " + liveData.player.cockDescript(x) + " left outside of your newly christened dick jockey. It's enough to orgasm the bitch, and you haven't even gotten started. You ");
        if (liveData.player.cocks[0].cockLength < 20)
            GUI.outputText("can feel the massive bulge in her midsection");
        else
            GUI.outputText("stretch her so much that you can see the bulge even from this angle");
        GUI.outputText(" and it drives your lust even higher. You withdraw more cock from the hole than your cum starved slut has body, causing her eyes to roll into the back of her head as her " +
            liveData.monster.vaginaDescript() +
        ...
        if (liveData.player.biggestTitSize() >= 1) {
            ...
            if (liveData.player.biggestLactation() >= 1) {
                ...
                if (liveData.player.biggestLactation() < 2)
                    GUI.outputText("occasionally drip milky tears onto her face");
                if (liveData.player.biggestLactation() < 3)
                    GUI.outputText("weep streams of milk onto her features");
                else
                    GUI.outputText("plaster her with gouts of fluid");
            }
        }
        GUI.outputText(". Her hair is matted with");
        if (liveData.player.biggestLactation() >= 1)
            GUI.outputText(" milk and");
        ...
        if (liveData.player.hasVagina()) {
            ...
            if (liveData.player.vaginas[0].vaginalWetness <= ENUM.VaginalWetnessType.VAGINA_WETNESS_SLICK)
                GUI.outputText("juices ");
            else if (liveData.player.vaginas[0].vaginalWetness <= ENUM.VaginalWetnessType.VAGINA_WETNESS_DROOLING)
                GUI.outputText("floods ");
            else
                GUI.outputText("explodes ");
            if (liveData.player.cockTotal() > 1)
                GUI.outputText("and the rest of your " + liveData.player.multiCockDescriptLight() + " drench her, ");
        }
        else if (liveData.player.cockTotal() > 1) {
            GUI.outputText("while your other " + Appearance.cockNoun(ENUM.CockType.HUMAN));
            if (liveData.player.cockTotal() > 2)
                GUI.outputText(" drench her, ");
            else
                GUI.outputText(" drenches her, ");
        }
        GUI.outputText("the bitch is too tight! Like a natural cock-ring! ");
        if (liveData.player.balls >= 2)
            GUI.outputText("Your " + liveData.player.ballsDescriptLight() + " are trying but, h");
        else
            GUI.outputText("H");
        GUI.outputText("er spasming " + liveData.monster.vaginaDescript() + " is clamping down so hard on your " + liveData.player.cockDescript(x) + " that it can't release.<br><br>");
        GUI.outputText("The moment passes and you're left unsatisfied.  This only serves to piss you off as your cum receptacle fails in its one duty. You, however, are undaunted. In fact you redouble your efforts. If this size queen slut wants to deny you your pay off while getting off herself, well, you'll just have to cum her into oblivion the next go-round.<br><br>");
        GUI.outputText("For what seems like hours you almost literally screw the brains out of her little green head, working yourself back up to the brink. You consider pulling out this time, but decide against it. At least two loads worth at once; it'll work, and the bitch has it coming.<br><br>");
        ...
        GUI.outputText("Your " + liveData.player.cockDescript(x) + " is pressed so firmly against her skin that you can see the cum working its way up and out of your shaft and bloating your goblin toy with only the first shot. ");
        if (liveData.player.cockTotal() > 2)
        ...
        if (liveData.player.cockTotal() == 2)
            GUI.outputText("Your remaining penis blasts geysers into the air, coating you both with spunk. ");
        GUI.outputText("The same tightness that prevented your cumming the first time now ensures that none of the copious amount of seed you blast into her escapes until you pull out. It's a good thing your corruption was so effective, as she is beginning to look pregnant enough to hold a beach ball.<br><br>");
        GUI.outputText("Finally you blow your last wad into this latest piece of ass, shoving her off of your " + liveData.player.cockDescript(x) + " and letting her fall to the ground. The impact sends torrents of cum sluicing out of her.<br><br>");
        GUI.outputText("You stand and prepare to leave, looking down at the goblin slut you just finished with.<br><br>");
        GUI.outputText('"<i>I hope my daughters are a better fuck than their bitch mother,</i>" you say. "<i>Tell\'em to find me if they want to get split like a log too.</i>"');
    ...
    static goblinButtSex() {
        ...
        GUI.outputText('"<i>Fuck me, stud!</i>" she begs. Though defeated, she has to gall to make demands. "<i>Pump me full of your baby batter!</i>" Her defeat doesn\'t seem to do much to silence her tongue.<br><br>');
        GUI.outputText("You're insulted. Who emerged victorious from the prior battle? You could have left her there to wallow in a pool of her own juices if you weren't so damn horny yourself. So what do you do? After some silent pondering, you get a deliciously wicked idea.<br><br>");
        GUI.outputText("After quickly removing your " +
        ...
        GUI.outputText("With a sneer, you take the crown of your " + liveData.player.cockDescript(x) + " and press it not against her drenched pussy lips, but her asshole!<br><br>");
        ...
        GUI.outputText("You say nothing to the trembling woman in your grasp. No words could describe the joy you feel from crushing her dreams. By the elders, this world has truly corrupted you, and you love it!<br><br>");
        GUI.outputText("Without any reservations, you slam the goblin whore onto your " +
            liveData.player.cockDescript(x) +
        ...
        GUI.outputText("Protesting, the goblin squirms more, even going as far as to beat her fists into your chest; she's so feeble at the moment that her strikes actually tickle. Her puny assault is enough to knock you out of your carnal daze. You grin manically as you grasp her tiny waist and hammer her! Your " +
            liveData.player.cockDescript(x) +
            " quickly adjusts to her stretching asshole as you thrust harder and deeper!<br><br>");
        GUI.outputText('"<i>This isn\'t what I wanted!</i>" she cries, "<i>This isn\'t what I wanted!</i>" If your ' + liveData.player.cockDescript(x) + " expanding her ass wasn't enough, she can feel your ");
        if (liveData.player.balls > 0)
            GUI.outputText(liveData.player.sackDescript());
        else
            GUI.outputText(liveData.player.hipDescript());
        GUI.outputText(" smacking her plump rump with each thrust, as if to tease her.<br><br>");
        GUI.outputText("The tight confines of the goblin's asshole prove too much for you. Your body convulses wildly as you unload a massive load in her. Spent, you throw the little whore onto the ground; you have no further use for her at the moment.<br><br>");
        GUI.outputText("As you pick up your " +
            liveData.player.armorName +
    ...
    static giveGoblinAMilkMustache() {
        GUI.clearOutput();
        GUI.outputText("You slowly walk up to the downed goblin, gently telling her that everything will be all right now. She looks at you a bit incredulously and spreads her legs, obviously hoping that you will satisfy the urges that she has. You shake your head at her and instead cup your hands under your " +
        ...
        GUI.outputText('She struggles against your grip, trying to get something, anything inside her needy pussy while yelling "<i>Come on ' +
        ...
        GUI.outputText("After a while, you feel the goblin fall asleep in your arms. Even then, she still continues suckling gently on your " +
            liveData.player.nippleDescript(0) +
    ...
    static goblinCondomed(mode) {
        //spriteSelect(24);
        var x = liveData.player.cockThatFits(liveData.monster.vaginalCapacity());
        GUI.clearOutput();
        //GUI.outputText(images.showImage("goblin-win-male-goblincondomed"));
        GUI.outputText('Defeated, the goblin girl\'s knees give out and she sinks backward, lying on her back with her emerald ankles suspended above her head. "Use me," she begs, "humiliate, degrade, and debase me! Just, whatever you do, fill me!" As you strip off your ' +
            liveData.player.armorName +
        ...
        GUI.outputText("Impatiently, she thrusts her legs out and hooks her toes around your lower body, trying to pull you closer while still keeping her needy hole accessible. Her olive feet clench around your flesh, her soles firm and muscular on your " +
    ...
    static goblinDriderEggs() {
        ...
        if (liveData.player.hasCock()) {
            ...
            GUI.outputText("  Y");
        if (liveData.player.hasVagina())
            GUI.outputText("our [vagina] clenching in time with hers as your copious fluids drench your spider half.");
        GUI.outputText("<br><br>Then the next egg rolls forward, and the next, and the next...");
        GUI.outputText("<br><br>You keep her pinned against your body as you fill her up, one orb at a time, each sphere bloating her stretched stomach a little further, until she's so full you can feel your eggs through the taut skin of her belly.  The goblin is nearly unconscious, insensibly gurgling as the pleasure of her instant pregnancy numbs her mind.  You pull her off with a loud wet plop, her twitching snatch leaking an unending stream of her own clear fluids as well as a sticky string of your green egg-mucus.");
        GUI.outputText("<br><br>Laying her down in the shade, you put your clothes back on, glad to be free of the extra weight and ready to continue your adventure.");
    ...
    static defeatAgainstGoblin() {
        ...
        if (liveData.player.gender == 0) {
            GUI.clearOutput();
            GUI.outputText("You collapse in front of the goblin, too wounded to fight.  She giggles and takes out a tube of lipstick smearing it whorishly on your face.  You pass into unconsciousness immediately.  It must have been drugged.");
            COMBAT.cleanupAfterCombat();
            return;
        }
    ...
    static goblinRapesPlayer() {
        ...
        if (liveData.player.findPerk(PerkLib.BimboBrains) >= 0 || liveData.player.findPerk(PerkLib.FutaFaculties) >= 0) {
            ...
            if (liveData.player.hasVagina()) {
                ...
                if (liveData.player.hasFuckableNipples())
                    GUI.outputText(", fingering your wet nipplecunts and pinching the now-puffy areolas tightly, sending uncontrollable waves of pleasure from your chest to your groin.");
                //-[If not]
                else
                    GUI.outputText(" and slides her fingers up to your nipples, pinching and twisting them, torturing you with pain and pleasure.");
                GUI.outputText("<br><br>");
                GUI.outputText("The goblin, almost drooling, says \"<i>Baby, we're both horny, but fingers and tongues just aren't enough for me. Lucky for you, I've got just the thing.</i>\"<br><br>");
                GUI.outputText("For a moment, your dumb, bimbo mind struggles to think of whatever she could, like, mean by that. Oh well, you giggle airheadedly and just let it happen.<br><br>");
                GUI.outputText('The goblin reaches into another one of her pouches and pulls out a dildo that flops about in her hand as if it were glad to be free of the pouch. Slapping you on the cheek and grinning mischeviously, she offers, "<i>Why don\'t we bury one end in each of our cunts? Just open your mouth and help me get it warmed up for us, okay?</i>"<br><br>');
                GUI.outputText("For a moment, you blink dumbly, trying to think why for, like, one tiny second you wanted to say 'no', but then you forget about it, let the horniness flow, and naturally pop your mouth open into a perfect-lip lined 'O', shooting her a involuntarily slutty look.<br><br>");
                GUI.outputText("She instantly plugs your sex-doll-like mouth with the bulging dildo.  It plumps up somehow, forcing your jaw open and pinning your tongue to the bottom of your mouth - this is like, so much easier than having to hold it open yourself. A trickle of fluid escapes its tip. You swallow it reflexively - oh wait, was that like cum or like what? Your little brain is confused for a moment, at least 'til the smarter slut explains it to you.<br><br>");
                GUI.outputText("\"<i>Yummy isn't it? I made this myself. It's made of the best stuff - it reacts with fluids to puff up and fill ANY hole perfectly. Which, by the looks of you, will probably be helpful.  You look like you've taken a few in your day.</i>\" She winks, \"<i>Oh, and even better... it has a little tube inside full of aphrodisiacs that'll slowly leak out into your wet body!</i>\"<br><br>");
                GUI.outputText("Saliva coats your lips and dribbles down onto your boobs when the goblin pulls the thick, soaked dong out of your mouth. The part that was stuffed down your throat is swollen up to nearly twice as wide as the half still in her hand. The goblin quickly corrects that, slurping the second half down into her throat, taking at least eight inches into her mouth with no sign of gagging. She's like, totally good at this! She pulls it out, watching it puff up. She blushes, turning her slightly-dimpled, green cheeks a shade of purple. The dildo slaps your twat cruelly as she drops part of it onto your mons. With a few practiced motions, she shoves it inside you, stuffing you full of artificial cock, bringing your mind back to all the times you'd fucked yourself just like this. Your easy, wandering mind is brought back to the present by a small slap on the dildo, which sends vibrations right into your " +
                    liveData.player.vaginaDescript(0) +
                    ".<br><br>");
                GUI.outputText("Your green slut counterpart stands up and steps over your crotch, positioning herself perpendicular to you. The warm wetness of her dripping cunt splashes your thighs when she works the free end of the double-dong into her own slick twat. Your pussy squelches against hers wetly as they meet in the middle, your " +
                    liveData.player.clitDescript() +
                ...
                GUI.outputText("By now your passage feels, like, more stuffed than ever! Every motion the tiny slut makes is amplified directly into the fuck-stick plugging your drooling pussy. Judging by how wonderful it feels rubbing and twisting against your well-experienced vaginal walls, the aphrodisiac is definitely working. You moan and spread your legs wide, giving the tiny dominatrix free reign over your body in its natural position - open and ready.<br><br>");
                GUI.outputText("She wiggles against you harder, throwing her head back and running her fingers through her purple hair, shouting out encouragement all the while, \"<i>Mmm, you like this, don't you slut? My dildo fits your slutty cunt well, doesn't it? I bet that's rare. Keep wiggling those hips - the aphrodisiac is gravity fed, and with you on the bottom you'll be blissed into unconsciousness soon. Just don't cum before me hun, I want to feel release WITH you.</i>\"<br><br>");
                GUI.outputText("It isn't hard these days, but you lose yourself in the sweet sensations of the bloated dildo that joins your simmering groins, pleasure whisking away whatever little intelligence you have left. Rocking back and forth, scissoring relentlessly against your green mistress, you moan, drowning yourself in a sea of drug-enhanced pleasure. The goblin cries out and thrashes in sudden orgasm, twisting the fat dildo violently around inside your slut-hole. The juices of her orgasm react with the toy, stretching you to a perfect level of vaginal gape - it's sooooo rare to, like, find someone who can stretch you out! Your bodies thrash together, wracked by twin orgasms that leave you smeared with a mixture of sweat and girl-cum.<br><br>");
                GUI.outputText("Later, the wet goblin audibly pops off the dildo. She stumbles, bow-legged, before teasing your bimbo-clit and yanking her toy free of your needy pussy. Your lips gape wider than ever, but you've gotten used to your pussy getting stretched by now...");
                liveData.player.cuntChange(liveData.player.vaginalCapacity(), true, true, false);
                GUI.outputText("<br><br>");
                GUI.outputText('She plants a kiss on your lips and mutters, "<i>Can\'t forget this,</i>" as she puts her dildo away. You find yourself smiling and watching her strap-covered form jiggle pleasantly as she bounds away from you into the distance. Your eyelids drift closed and your lips go numb as her drugged lipstick begins to put you out.<br><br>');
        ...
        // Standard rape...
        GUI.outputText("The goblin saunters up to you, wiggling her hips with a needy, sexual sway.  She opens a small pouch and pulls out a tube of lipstick, pausing to apply it to her lips.  She caps off the tube and blows you a kiss with a wet smacking sound as she steps up to your ");
        if (liveData.player.HP < 1)
            GUI.outputText("defeated");
        else
            GUI.outputText("trembling");
        GUI.outputText(" form.  Your breath is knocked from your body as she drops all her weight onto your middle, making her ass and well-rounded tits jiggle enticingly.   She reaches up to twist her nipple-studs, grinding the sopping wetness of her crotch up and down your belly.<br><br>");
        ...
        if (liveData.player.cockTotal() > 0) {
            ...
            if (liveData.player.cocks[0].cockLength <= 7)
                GUI.outputText("as your bulge begins to press between her soft ass-cheeks");
                ...
                GUI.outputText("as your bulge grows up her back and creeps towards her shoulders steadily");
            GUI.outputText(".  ");
        }
        if (liveData.player.hasVagina()) {
            ...
            if (liveData.player.vaginas[0].vaginalWetness <= ENUM.VaginalWetnessType.VAGINA_WETNESS_NORMAL)
                ...
                GUI.outputText("It rapidly forms into a puddle as your " + liveData.player.vaginaDescript(0) + " does its best to show just how ready you are.  ");
        }
        ...
        if (liveData.player.hasFuckableNipples())
            GUI.outputText("slipping her thumbs inside the tender cunts and pinching against them tightly");
        else
            GUI.outputText("twisting and tweaking");
        ...
        if (liveData.player.totalCocks() > 0 && (!liveData.player.hasVagina() || UTIL.rand(2) == 0)) {
            ...
            if (liveData.player.cockArea(0) > liveData.monster.vaginalCapacity()) {
                GUI.outputText("She lifts her body up high, grabbing your " +
                ...
                if (liveData.player.balls > 0)
                    ...
                    GUI.outputText("the base of your " + liveData.player.cockDescript(0));
                GUI.outputText(" before she buries it deep into your " +
                ...
                GUI.outputText("The lewd little creature body-fucking your titanic tool is just so passionate and sensual that you can't hold the budding pressure in your loins.  You're sure you must be leaking pre-cum, but the squishing wet cunt wrapped around your cock-tip has you so slathered in fuck-juice you'd never be able to tell.  You tremble, struggling to hold back and ");
                if (liveData.player.cor > 50)
                    GUI.outputText("prolong your pleasure");
                else
                    GUI.outputText("prevent the monstrous girl from getting what she wants");
                GUI.outputText('.  The goblin looks back over her shoulder, narrows her eyes, and pulls back to say, "<i>Don\'t even think about holding back stud.</i>"<br><br>');
                GUI.outputText("She works her fingers into the void her tongue left behind, ");
                if (liveData.player.hasVagina())
                    GUI.outputText("caressing your " + liveData.player.clitDescript());
                else
                    GUI.outputText("pressing tightly against your prostate");
                GUI.outputText(' as she whispers, "<i>Go ahead, make my twat a swollen cum-dump.  I NEED you to FILL me with ALL of your cream.  Stuff me full of your fuck-juice and I promise I\'ll give you a dozen slutty daughters to fill with jizz every night.</i>"<br><br>');
                GUI.outputText("Her fingers and words have the desired effect, drawing out a tremendously pleasurable orgasm.  Your hips rock and buck against her lithe body, forcing her to cling on to your spasming form as globules of cum force their way up your massive urethra.  You can feel them explode into the tiny girl's sopping tunnel, immediately soaking into her womb.");
                if (liveData.player.cumQ() > 100)
                ...
                if (liveData.player.cumQ() >= 250)
                    GUI.outputText("  Her belly distorts visibly as her uterus is packed full of cum, making her look heavily pregnant already.  She coos in delight at the sight of her swollen abdomen.");
                GUI.outputText("  Satisfied, your orgasm tapers off into tiny dribbles.  The goblin slips off you, looking a bit bowlegged, but utterly pleased.<br><br>");
                GUI.outputText('She waves, "<i>Thanks for the spunk hun!  It ');
                if (liveData.player.cumQ() < 100)
                    GUI.outputText("wasn't much, but I'll make do");
                else if (liveData.player.cumQ() < 250)
                    GUI.outputText("should be plenty to make a few new wet-behind-the-ears sluts for you to fuck");
                else
                    GUI.outputText("was better than I could have dreamed.  I'm going to find you again stud");
                GUI.outputText('!</i>"<br><br>');
                GUI.outputText("She giggles again and leans over to kiss you on the lips, smearing her thick bubbly lips across your own and leaving you tasting the bubble-gum of her lipstick.  You find yourself smiling dreamily and slipping into unconsciousness... there must have been something in that lipstick!");
                COMBAT.cleanupAfterCombat();
                liveData.player.orgasm();
                return;
            }
            //Dick fits...
                ...
                GUI.outputText("The goblin-girl doesn't waste time with any more foreplay, she just arches her back like a cat about to get the cream, and slides her plush ass towards your " +
                    liveData.player.cockDescript(0) +
                ...
                if (liveData.player.cocks[0].cockType == ENUM.CockType.HORSE)
                    ...
                    GUI.outputText("cock-head");
                GUI.outputText(", spreading around you ever-so-slowly as she sinks further and further down.  She licks her glossy lips and blows you a kiss as she violently pushes the rest of the way down, impaling herself on every inch of your length.<br><br>");
                GUI.outputText('She arches her back further, proudly displaying the bulge your manhood has made in her tight little tummy.  She coos while fiddling with her clit, "<i>Mmmm I\'m so glad that fit.  The last mate I found was too big to stuff my greedy hole, and that\'s never as fun.</i>"  She looks wistful for a moment as her hips begin grinding up and down, "<i>He did fill me fit to burst though.  Can you do that for me?  Can you fuck me pregnant?</i>" she asks.<br><br>');
                GUI.outputText("You're too turned on to do anything besides groan and nod, imprisoned by desire and her tight wet pussy.  You can feel it rippling around you, squeezing and milking in rhythmic motions as if it's trying to drain you dry.  Girlish giggles erupt from the goblin when your hips start grinding back against her, bouncing her up and down and making her leather-wrapped tits jiggle for you.  Light reflects off her piercings with hypnotic intensity as they bounce to and fro.<br><br>");
                GUI.outputText("The sultry slut pulls out a flask from somewhere behind her and uncorks it, releasing a puff of red smoke that smells of cherries.<br><br>");
                GUI.outputText('"<i>Drink your medicine for me stud, I need to make sure you\'ve got a full load for me,</i>" she orders, stretching to press the flask to your lips.  ');
                if (liveData.player.cor < 50)
                    ...
                    GUI.outputText("You lick your lips and open your mouth, happily taking in the cherry-flavored drug.");
                GUI.outputText("  A sensation of dizziness flows through you, along with relaxing waves of gentle warmth that make it easy to relax and let that cute green whore take your shaft.<br><br>");
        ...
        // Female rape, or herm pussy rape.
            ...
            GUI.outputText("The goblin says, \"<i>Baby we're both horny, but fingers and tongues just aren't enough for me. Lucky for you, I've got just the thing.</i>\"<br><br>");
            GUI.outputText('She reaches into a tiny bulging pouch on her hip and pulls out a dildo that flops about in her hand as if was glad to be free.  Slapping you on the cheek and grinning impishly, she offers, "<i>Why don\'t we bury one end in each of our cunts?  Just open your mouth and help me get it warmed up for us, OK?</i>"<br><br>');
            if (liveData.player.cor < 33)
                GUI.outputText("You shake your head violently, clearly indicating 'No', but when you open your mouth to vocalize your complaint, your lips are swiftly plugged with bulging sex-toy.");
            else if (liveData.player.cor < 66)
                GUI.outputText("You open your mouth to stammer out your concerns, but she plugs the hole with the floppy artificial dong, turning your speech into surprised muffles.");
            else
                GUI.outputText("You lick your lips coyly then open your mouth into a welcoming 'O'.  The sex-toy slips straight into the hole, muffling the sounds of your happiness with the arrangement.");
            GUI.outputText("  It 'plumps' up somehow - perhaps in reaction to your spit - forcing your jaw open and pinning your tongue to the bottom of your mouth.  A trickle of fluid escapes its tip, nearly gagging you before your throat reflexively drinks it down.  What did you just swallow?<br><br>");
            GUI.outputText("\"<i>Yummy yummy isn't it?  I made this myself.  It's made up of the best stuff - it reacts with fluids to puff up and fill ANY hole perfectly.  Even better, it has a reservoir stuffed with aphrodisiacs that'll slowly leak out.  Do you feel warm yet hun?</i>\" she asks.<br><br>");
            GUI.outputText("You sputter a bit when she pulls the thick spit-soaked dong out of your mouth.  The part that was stuffed down your throat is swollen up nearly twice as wide as the half in her hand.  The goblin slurps the other half into her throat, taking at least eight inches into her mouth with no sign of discomfort.  She pulls it out, watching it begin to puff up and blushing, turning her slightly-dimpled cheeks purple.   The dildo slaps your twat cruelly as she drops part of it onto your mons.  With a few expert motions, she shoves it inside you, stuffing you full of artificial cock.  The goblin giggles again and slaps the outer half of the dong, making it flop about and sending vibrations directly to your core.<br><br>");
            GUI.outputText("The green slut stands up and steps over your crotch, positioning herself at a ninety degree angle to you.   The warm wetness of her readiness splashes your thighs when she works the free end of the double-dong into her own aching twat.  She slides down its length, easily taking the remaining length up her juicy cunt.  Your " +
                liveData.player.vaginaDescript(0) +
            ...
            if (liveData.player.clitLength >= 7)
                GUI.outputText("Of course, the sheer size of your clit makes it difficult for the goblin to handle in the normal way - it keeps slipping between up her breasts.  The tiny green tart's eyes light up with a devilish idea.  She pulls out a vial of pink slime and dumps it over her breasts, pushing them around your clit and smothering them in slippery flesh.   You cry out in delight, overwhelmed by the feelings radiating from your over-sized pleasure-buzzer as it is mercilessly worked by the tiny woman.<br><br>");
            GUI.outputText("By now your passage feels as stuffed as it ever has been, crammed totally full of the squishy expanded double-dong.  Every motion the tiny slut makes is amplified directly into the fuck-stick plugging your " +
                liveData.player.vaginaDescript(0) +
            ...
            GUI.outputText("Later, the wet goblin audibly pops off the dildo.  She stumbles, bow-legged, before teasing your " + liveData.player.clitDescript() + " and yanking her toy free.  Your lips ");
            if (liveData.player.vaginas[0].vaginalLooseness <= ENUM.VaginalLoosenessType.VAGINA_LOOSENESS_GAPING)
                GUI.outputText("gape apart momentarily");
            else
                GUI.outputText("gape wider than ever, but only for a moment");
            GUI.outputText(".<br><br>");
            GUI.outputText('She plants a kiss on your lips and mutters, "<i>Can\'t forget this,</i>" as she puts her dildo away.  You find yourself smiling and watching her strap-covered form jiggle pleasantly as she bounds away from you into the distance.  Your eyelids drift closed and your lips go numb as her drugged lipstick puts you out.  ');
