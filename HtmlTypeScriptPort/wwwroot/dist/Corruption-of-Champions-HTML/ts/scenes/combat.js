...
class COMBAT {
    ...
    static flee() {
        ...
        if (liveData.monster.refName == "Marae") {
            ...
            if (!liveData.player.canFly())
                GUI.outputText("You may not be able to swim fast enough. ");
            else
                GUI.outputText("You grit your teeth with effort as you try to fly away but the tentacles grab your " + liveData.player.legs() + " and pull you down. ");
        ...
        if (liveData.monster.refName == "Ember") {
            ...
            if (!liveData.player.canFly())
                GUI.outputText(" running");
            else
                GUI.outputText(", flapping as hard as you can");
        ...
        if (liveData.monster.refName == "lizan rogue") {
            ...
            GUI.outputText("Gritting your teeth with effort, you beat your wings quickly and lift off! ");
        //Nonflying PCs
        else {
            //In prison!
            /*if (prison.inPrison) {
            ...
            /*else */ if (liveData.player.findStatusEffect(StatusEffects.NoFlee) >= 0) {
                if (liveData.monster.refName == "goblin")
                    GUI.outputText("You try to flee but get stuck in the sticky white goop surrounding you.<br><br>");
                else
                    GUI.outputText("You put all your skills at running to work and make a supreme effort to escape, but are unable to get away!<br><br>");
                success = false;
            }
            //Nonstuck!
            else
                GUI.outputText("You turn tail and attempt to flee! ");
        }
        ...
        if (liveData.player.canFly())
            escapeMod -= 20;
        if (liveData.player.tailType == ENUM.TailType.TAIL_TYPE_RACCOON && liveData.player.earType == ENUM.EarType.EARS_RACCOON && liveData.player.findPerk(PerkLib.Runner) >= 0)
            escapeMod -= 25;
        if (liveData.monster.findStatusEffect(StatusEffects.Stunned) >= 0)
            escapeMod -= 50;
        else {
        ...
        if (liveData.monster.name == "anemone") {
            ...
            if (liveData.player.lust < 60) {
                GUI.outputText("Marshalling your thoughts, you frown at the strange girl and turn to march up the beach. After twenty paces inshore you turn back to look at her again. The anemone is clearly crestfallen by your departure, pouting heavily as she sinks beneath the water's surface.");
                success = true;
            }
            //Speed dependent
            else {
                ...
                //Run failed:
                    ...
                    GUI.outputText('You try to shake off the fog and run but the anemone slinks over to you and her tentacles wrap around your waist. <i>"Stay?"</i> she asks, pressing her small breasts into you as a tentacle slides inside your ' +
        ...
        if (success !== undefined && liveData.player.spe > UTIL.rand(liveData.monster.spe + escapeMod)) {
            ...
            /*else */ if (liveData.player.canFly())
                ...
                GUI.outputText("Using your running skill, you build up a head of steam and jump, then spread your arms and flail your tail wildly; your opponent dogs you as best " +
            ...
            //Non-fliers flee
                ...
                GUI.outputText(UTIL.capitalize(liveData.monster.a) + liveData.monster.refName + " rapidly disappears into the shifting landscape behind you.");
            if (liveData.monster.name == "Izma") {
                GUI.outputText('<br><br>As you leave the tigershark behind, her taunting voice rings out after you. "<i>Oooh, look at that fine backside! Are you running or trying to entice me? Haha, looks like we know who\'s the superior specimen now! Remember: next time we meet, you owe me that ass!</i>" Your cheek tingles in shame at her catcalls.');
            }
            success = true;
        }
        //Runner perk chance
        ...
        //FAIL FLEE
            ...
            if (liveData.player.canFly()) {
                ...
                if (liveData.monster.plural)
                    GUI.outputText(UTIL.capitalize(liveData.monster.a) + liveData.monster.refName + " manage to grab your " + liveData.player.legs() + " and drag you back to the ground before you can fly away!");
                else
                    GUI.outputText(UTIL.capitalize(liveData.monster.a) + liveData.monster.refName + " manages to grab your " + liveData.player.legs() + " and drag you back to the ground before you can fly away!");
            }
            else if (liveData.player.tailType == ENUM.TailType.TAIL_TYPE_RACCOON && liveData.player.earType == ENUM.EarType.EARS_RACCOON && liveData.player.findPerk(PerkLib.Runner) >= 0)
                // >>>>>>[P] FAIL
                ...
                if (liveData.player.balls > 0 && liveData.player.ballSize >= 24) {
                    if (liveData.player.ballSize < 48)
                        GUI.outputText("With your " + liveData.player.ballsDescriptLight() + " swinging ponderously beneath you, getting away is far harder than it should be. ");
                    else
                        GUI.outputText("With your " + liveData.player.ballsDescriptLight() + " dragging along the ground, getting away is far harder than it should be. ");
                }
                ...
                if (liveData.player.biggestTitSize() >= 35 || liveData.player.buttRating >= 20 || liveData.player.hipRating >= 20) {
                    ...
                    if (liveData.player.biggestTitSize() >= 35 && liveData.player.biggestTitSize() < 66) {
                        if (liveData.player.hipRating >= 20) {
                            ...
                            if (liveData.player.buttRating >= 20)
                                GUI.outputText(liveData.player.buttDescript() + " and ");
                    ...
                    //FOR PLAYERS WITH MASSIVE BREASTS
                        ...
                        if (liveData.player.hipRating >= 20) {
                            ...
                            if (liveData.player.buttRating >= 20)
                                GUI.outputText("causing the fat of your " + liveData.player.skinTone + liveData.player.buttDescript() + " to wobble heavily, ");
                            ...
                            GUI.outputText("Your " + liveData.player.chestDesc() + " nearly drag along the ground, preventing you from moving quick enough to get escape.");
                    }
                    //FOR PLAYERS WITH EITHER GIANT HIPS OR BUTT BUT NOT THE BREASTS
                        ...
                        if (liveData.player.buttRating >= 20)
                            GUI.outputText("causing your " + liveData.player.skinTone + liveData.player.buttDescript() + " to wobble obscenely ");
                        GUI.outputText("and forcing your body into an awkward gait that slows you down, preventing you from escaping.");
                    }
                    //JUST DA BOOTAH
                    else if (liveData.player.buttRating >= 20)
                        GUI.outputText("Your " + liveData.player.skinTone + liveData.player.buttDescript() + " wobbles so heavily that you're unable to move quick enough to escape.");
                }
                //NORMAL RUN FAIL MESSAGES
                else if (liveData.monster.plural)
                    GUI.outputText(UTIL.capitalize(liveData.monster.a) + liveData.monster.refName + " stay hot on your heels, denying you a chance at escape!");
                else
                    GUI.outputText(UTIL.capitalize(liveData.monster.a) + liveData.monster.refName + " stays hot on your heels, denying you a chance at escape!");
            }
        }
        ...
        if (liveData.monster.refName == "sandtrap") {
            ...
            switch (liveData.player.statusEffectValue(StatusEffects.Bind, 1)) {
                case BindType.BIND_TYPE_GOO:
                    GUI.outputText("You writhe uselessly, trapped inside the goo girl's warm, seething body. Darkness creeps at the edge of your vision as you are lulled into surrendering by the rippling vibrations of the girl's pulsing body around yours.");
            ...
            GUI.outputText("You decide not to take any action this round.<br><br>");
    ...
    static fantasize() {
        ...
        if (liveData.player.armor == Items.Armor.GooArmor) {
            ...
            GUI.outputText("You fantasize about grabbing " +
                liveData.monster.a +
        ...
        if (lustGain >= 20)
            GUI.outputText("The fantasy is so vivid and pleasurable you wish it was happening now. You wonder if " + liveData.monster.a + liveData.monster.refName + " can tell what you were thinking.<br><br>");
        else
            GUI.outputText("<br>");
        ...
        switch (liveData.player.statusEffectValue(StatusEffects.Bind, 1)) {
            ...
            case BindType.BIND_TYPE_TENTACLE:
                ...
                if (UTIL.rand(80) < 33 + liveData.player.str / 2) {
                    ...
                    GUI.outputText("As the creature attempts to adjust your position in its grip, you free one of your " +
                    ...
                    if (liveData.player.cocks.length > 0)
                        ...
                        GUI.outputText("The creature continues probing at your asshole and has now latched " +
    ...
    static cleanupAfterCombat(nextFunc = UTIL.nullFunc) {
        if (liveData.monster.HP <= 0 || liveData.monster.lust >= liveData.monster.maxLust()) {
            // if (nextFunc == undefined) nextFunc = Camp.returnToCampUseOneHour
            if (nextFunc == UTIL.nullFunc)
    ...
    static magicMenu() {
        ...
        GUI.menu();
        //White Spells
        if (liveData.player.spells.blind)
            GUI.addButton(0, "Blind", this.spellBlind);
        if (liveData.player.spells.chargeWeapon)
            GUI.addButton(1, "Charge Weapon", this.spellChargeWeapon);
        if (liveData.player.spells.whitefire)
            GUI.addButton(2, "Whitefire", this.spellWhitefire);
        //Black Spells
        if (liveData.player.spells.arouse)
            GUI.addButton(5, "Arouse", this.spellArouse);
        if (liveData.player.spells.heal)
            GUI.addButton(6, "Heal", this.spellHeal);
        if (liveData.player.spells.might)
            GUI.addButton(7, "Might", this.spellMight);
        //Special
        if (liveData.player.findPerk(PerkLib.CleansingPalm) >= 0)
            GUI.addButton(10, "CleansingPalm", this.spellCleansingPalm);
        GUI.addButton(14, "Back", COMBAT.battleMenu);
    }
    //White Spells
    static spellBlind() { }
    static spellChargeWeapon(silent) {
        if (silent) {
    ...
    static spellWhitefire() { }
    //Black Spells
    static spellArouse() { }
    static spellHeal() { }
    static spellMight(silent = undefined) { }
    //Special Spells
    static spellCleansingPalm() { }
    //------------
    // P. SPECIAL
    //------------
    static physicalSpecials() {
        GUI.menu();
        GUI.addButton(14, "Back", COMBAT.battleMenu);
    }
    //------------
    ...
    // SPEC UTIL
    //------------
    static spellCost(cost) {
        var temp = cost;
        return temp;
    }
    static physicalCost(cost) {
        var temp = cost;
        return temp;
    }
    static spellMod() {
