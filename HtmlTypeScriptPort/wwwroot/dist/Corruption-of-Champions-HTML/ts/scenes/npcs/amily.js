...
AmilyScene.start = function () {
    ...
    if (gameFlags[AMILY_CORRUPT_FLIPOUT] > 0 && player.cor > 25) {
        ...
        if (player.hasKeyItem(KeyItems.PotentMixture) < 0 && gameFlags[CREATE_POTENT_MIXTURE] < 3) {
            ...
            if (player.hasKeyItem(KeyItems.PotentMixture) && gameFlags[AMILY_CORRUPTION_PATH] == 0)
                AmilyScene.amilyCorrupt1();
            else if (player.hasKeyItem(KeyItems.PotentMixture) && gameFlags[AMILY_CORRUPTION_PATH] == 1)
                AmilyScene.amilyCorrupt2();
            else if (player.hasKeyItem(KeyItems.PotentMixture) && gameFlags[AMILY_CORRUPTION_PATH] == 2)
                AmilyScene.amilyCorrupt3();
            else
                AmilyScene.rapeCorruptAmily4Meeting(); //rapeCorruptAmily4Meeting();
            return;
        }
    }
    ...
    if (gameFlags[AMILY_PC_GENDER] == player.gender) {
        ...
        if (gameFlags[AMILY_BIRTH_TOTAL] + gameFlags[PC_TIMES_BIRTHED_AMILYKIDS] >= 5 && gameFlags[AMILY_VILLAGE_ENCOUNTERS_DISABLED] == 0) {
            if (gameFlags[AMILY_AFFECTION] < 40)
                AmilyScene.amilyBadEnding();
            else
                AmilyScene.amilyBecomesFollower();
            return;
        }
    }
    // Male Meeting
...
AmilyScene.amilyStandardMeeting = function () {
    ...
    if (gameFlags[AMILY_AFFECTION] < 15 || player.gender == 0) {
        ...
        if (gameFlags[AMILY_MET_AS] == 2 && player.gender == 2)
    ...
    // Amily is starting to REALLY like you... AMILY AFFECTION 40+
        ...
        if (player.hasCock()) {
            ...
            if (gameFlags[AMILY_WANG_LENGTH] > 0 && player.pregnancyIncubation == 0)
...
AmilyScene.amilyLesbianLetHerGo = function () {
    ...
    if (gameFlags[AMILY_AFFECTION] > 10)
        gameFlags[AMILY_AFFECTION] = 10;
    doNext(Camp.returnToCampUseOneHour);
};
...
AmilyScene.amilyNewGenderConfrontation = function () {
    ...
    //[Female...
        ...
        if (player.gender == 1) {
            ...
            if (gameFlags[AMILY_AFFECTION] < 15) {
                ...
                if (gameFlags[PC_TIMES_BIRTHED_AMILYKIDS] > 0)
                    gameFlags[AMILY_OFFER_ACCEPTED] = 1;
            }
            //Medium Affection:
            else if (gameFlags[AMILY_AFFECTION] < 40) {
    ...
    //Herm toooooooo
        ...
        if (player.gender == 1) {
            ...
            if (gameFlags[AMILY_AFFECTION] < 15) {
                ...
                if (gameFlags[AMILY_HERM_QUEST] == 2)
                    gameFlags[AMILY_OFFER_ACCEPTED] = 1;
            }
            //Medium Affection:
            else if (gameFlags[AMILY_AFFECTION] < 40) {
    ...
    //Genderless tooo
        ...
        //[High Affection]
            ...
            if (player.hasCock())
                outputText("bringing your " + player.cockDescript(0) + " to erection");
            if (player.hasVagina() && player.hasCock())
                outputText(" and ");
            if (player.hasVagina())
                outputText("earning a clench from your " + player.vaginaDescript(0));
...
AmilyScene.amilySexHappens = function () {
    ...
    //Moderate Affection Sex:
        ...
        if (amily.pregnancyEventNum >= 1 && amily.pregnancyEventNum <= 5)
...
AmilyScene.determineAmilySexEvent = function () {
    ...
    if (gameFlags[AMILY_CONFESSED_LESBIAN] > 0 && player.gender == 2) {
        if (gameFlags[AMILY_WANG_LENGTH] > 0) {
            ...
            if (!amily.isPregnant())
                AmilyScene.amilyHermOnFemalePC();
            //else 50/50
            else {
                if (rand(2) == 0)
...
AmilyScene.girlyGirlMouseSex = function () {
    ...
    //(If first time:
    if (gameFlags[AMILY_TIMES_FUCKED_FEMPC] == 0)
...
AmilyScene.amilyHermOnFemalePC = function () {
    ...
    //" + player.chestDesc() + "
    if (gameFlags[AMILY_HERM_TIMES_FUCKED_BY_FEMPC] == 0)
        outputText("\"<i>I can't believe this is actually happening... I've grown a cock and I'm about to use it on another woman.</i>\" Amily mutters to herself, though it's very evident that she likes what she sees, unable to resist staring at your " + player.chestDesc() + " or your " + player.vaginaDescript() + ".<br><br>");
    else
    ...
    if (player.gender == 3) {
        if (gameFlags[AMILY_HERM_TIMES_FUCKED_BY_FEMPC] == 0)
            outputText("\"<i>How on earth did I let myself get talked into this? If you've got both a cock and a pussy, then what's wrong with you just filling me with that cock?</i>\" Amily mutters to herself. Despite her words, though, her gaze is fixed squarely on your " + player.vaginaDescript() + ".<br><br>");
        //else
        else
            outputText("\"<i>You know, it's not all bad, us both being this way... but remember that I want a turn at that, too.</i>\" She states, staring hungrily at your " + player.multiCockDescriptLight() + ".<br><br>");
    }
    ...
    if (gameFlags[AMILY_HERM_TIMES_FUCKED_BY_FEMPC] == 0)
        ...
        outputText("Amily grips your " + player.hipDescript() + ", gathering her courage, and then plunges her penis into your depths. Cautiously at first, she begins to thrust herself back and forth, growing faster and harder as her resolve builds.");
...
AmilyScene.amilyPreggoChance = function () {
    ...
    if (gameFlags[AMILY_FOLLOWER] == 1) {
        ...
        if (gameFlags[AMILY_ALLOWS_FERTILITY] == 0)
            return;
    }
    ...
    if (gameFlags[FOLLOWER_AT_FARM_AMILY] != 0)
        return;
...
AmilyScene.amilyLaborWatch = function () {
    ...
    if (gameFlags[AMILY_NOT_FURRY] == 0)
        outputText(", mousy figure – pink, hairless and ", false);
    else
...
AmilyScene.amilyLaborHelp = function () {
    ...
    var amilyKid = "girl";
    if ((player.gender == 3 || gameFlags[AMILY_WANG_LENGTH] > 0) && rand(2) == 0)
        amilyKid = "herm";
    if (player.gender == 3 && gameFlags[AMILY_WANG_LENGTH] > 0)
        amilyKid = "herm";
...
AmilyScene.cookAmilyASnack = function () {
    ...
    if (gameFlags[CREATE_POTENT_MIXTURE] > 0) {
        ...
        //(else)
            ...
            if (player.hasVagina())
                outputText("your " + player.vaginaDescript() + " moistens itself ");
            ...
            //Consume items
            if (player.hasItem(Items.Consumables.LustDraft))
                player.destroyItems(Items.Consumables.LustDraft, 1);
            else
                player.destroyItems(Items.Consumables.FuckDraft, 1);
    ...
    //First Time Cooking the Drug
        ...
        //(else)
            ...
            if (player.hasCock())
                outputText(player.cockDescript(0) + " jumps up to attention");
            if (player.hasCock() && player.hasVagina())
                outputText(", and your ");
            if (player.hasVagina())
                outputText(player.vaginaDescript() + " nearly juices itself");
...
AmilyScene.amilyCorrupt2 = function () {
    ...
    //(else)
        ...
        if (player.hasVagina())
            outputText(player.vaginaDescript() + " growing wet");
        outputText(".<br><br>");
        ...
        if (player.hasVagina())
            outputText(" dripping " + player.vaginaDescript());
...
AmilyScene.amilyCorrupt3 = function () {
    ...
    if (player.hasVagina())
        outputText(" moistening your " + player.vaginaDescript(), false);
...
AmilyScene.amilyCorruptionRape = function () {
    ...
    //2nd rape scene
        ...
        if (player.gender == 1)
            doNext(AmilyScene.rapeCorruptAmily2Male);
        else if (player.gender == 2)
            doNext(AmilyScene.rapeCorruptAmily2Female);
        else if (player.gender == 3) {
            menu();
    ...
    //3nd rape scene
        ...
        if (player.gender == 1)
            doNext(AmilyScene.rapeCorruptAmily3Male);
        else if (player.gender == 2)
            doNext(AmilyScene.rapeCorruptAmily3Female);
        else if (player.gender == 3) {
            menu();
...
AmilyScene.rapeCorruptAmily1 = function () {
    ...
    if (player.cockTotal() > 0) {
        if (player.cockTotal() > 1)
            outputText(player.multiCockDescriptLight() + " are");
        else
            outputText(player.cockDescript(0) + " is");
        outputText(" already hard");
        if (player.hasVagina())
            outputText(", and your ");
    }
    if (player.hasVagina())
        outputText(player.vaginaDescript() + " is already wet");
    ...
    //Cocks!
        ...
        doNext(AmilyScene.rapeCorruptAmily1Male);
    //Cunts!
    else
        doNext(AmilyScene.rapeCorruptAmily1Female);
};
//COMPLETE
AmilyScene.rapeCorruptAmily1Male = function () {
    ...
    if (player.balls > 0)
        outputText(player.ballsDescriptLight() + " churn");
    else
        outputText(player.cockDescript(x) + " throb");
...
AmilyScene.rapeCorruptAmily2Male = function () {
    ...
    //[(If PC is huge.)
    if (player.cockThatFits(61) < 0)
        outputText("  One of Amily's hands settles on your " + player.cockDescript(x) + ", stroking whatever she doesn't manage to swallow.");
    outputText("<br><br>");
    ...
    //(else if PC has a pussy)
        ...
        outputText("Amily shoves her fingers into your " + player.vaginaDescript() + ".  ");
...
AmilyScene.rapeCorruptAmily2Female = function () {
    ...
    //[(if PC is a squirter)
    if (player.wetness() >= 5)
        outputText("  Your juices rocket into her face, painting her " + ((gameFlags[AMILY_NOT_FURRY] == 0) ? "muzzle" : "face") + " in your femcum as she giggles with happiness and pleasure.");
...
AmilyScene.rapeCorruptAmily3Male = function () {
    ...
    //[(if PC has balls)
    if (player.balls > 0)
        outputText("  Your " + player.ballsDescriptLight() + " churn with cum, growing heavier with every nasty detail that goes through your mind.");
...
AmilyScene.rapeCorruptAmily4Meeting = function () {
    ...
    if (player.gender == 3) {
        ...
        doNext(AmilyScene.rapeCorruptAmily4Female);
    else
        doNext(AmilyScene.rapeCorruptAmily4Male);
};
//COMPLETE
AmilyScene.rapeCorruptAmily4Male = function () {
    ...
    //[(if PC is huge)
    if (player.cockArea(0) > 50)
        outputText("  You smirk, pleased; with your size, you'd probably choke whomever tried to swallow your " + player.cockDescript(0) + " to death. But Amily doesn't choke or gag, she's only capable of moaning in delight with each inch that she manages to fit inside her hungry maw.");
    outputText("<br><br>");
    ...
    if (player.isNaga())
        ...
        outputText("put a " + player.foot() + " on her head.");
...
AmilyScene.rapeCorruptAmily4Epilogue = function () {
    ...
    if (player.pregnancyType == PREGNANCY_AMILY)
        player.knockUpForce(PREGNANCY_MOUSE, player.pregnancyIncubation);
...
AmilyScene.amilyDefurrify = function () {
    ...
    if (player.hasItem(Items.Consumables.BlackEgg))
        player.destroyItems(Items.Consumables.BlackEgg, 1);
    else
        player.destroyItems(Items.Consumables.LBlackEgg, 1);
    if (AmilyScene.amilyCorrupt()) {
        if (player.hasItem(Items.Consumables.SuccubiMilk))
            ...
            player.destroyItems(Items.Consumables.SuccubiMilkPurified);
    }
    else
        player.destroyItems(Items.Consumables.SuccubiMilkPurified);
