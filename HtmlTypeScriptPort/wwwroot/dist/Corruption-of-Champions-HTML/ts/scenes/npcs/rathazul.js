...
RathazulScene.campRathazul = function () {
    ...
    //Rat is definitely not sexy!
    if (player.lust > 50)
        player.changeLust(-((player.lust - 50) / 5), false);
    //Introduction
...
RathazulScene.rathazulWorkOffer = function () {
    ...
    //Marae bark armor
    if (player.hasKeyItem("Tentacled Bark Plates") >= 0 || player.hasKeyItem("Divine Bark Plates") >= 0)
        showArmorMenu = true;
    ...
    if (totalOffers > 0) {
        ...
        menu();
        if (showArmorMenu)
            addButton(0, "Armor", RathazulScene.rathazulArmorMenu, null, null, null, "Ask Rathazul to make an armour for you.");
        if (dyes) {
        ...
        if (purify)
            addButton(4, "Purify", RathazulScene.purifySomething, null, null, null, "Ask him to purify any tainted potions. <br><br>Cost: 20 Gems.");
...
RathazulScene.craftCarapace = function () {
    ...
    if (player.cockTotal() > 0 && player.biggestCockArea() >= 40)
    ...
    if (player.biggestTitSize() >= 8)
        outputText("Your " + player.biggestBreastSizeDescript() + " barely fit into the breastplate, leaving you displaying a large amount of jiggling cleavage.<br><br>");
