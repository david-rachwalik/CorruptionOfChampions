...
class RathazulScene {
    static encounterRathazul() {
        ...
        if (liveData.gameFlags[FLAG.RATHAZUL_MET] > 0) {
            if (liveData.gameFlags[FLAG.RATHAZUL_CAMP] > 0)
                GUI.outputText("You walk over to Rathazul's corner of the camp. He seems as busy as usual, with his nose buried deep in some tome or alchemical creation, but he turns to face you as soon as you walk within a few paces of him.<br><br>");
            else
                GUI.outputText("You spy the familiar sight of the alchemist Rathazul's camp along the lake. The elderly rat seems to be oblivious to your presence as he scurries between his equipment, but you know him well enough to bet that he is entirely aware of your presence.<br><br>");
        }
        else {
            GUI.outputText('You encounter a hunched figure working as you come around a large bush. Clothed in tattered robes that obscure most his figure, you can nontheless see a rat-like muzzle protruding from the shadowy hood that conceals most of his form. A simple glance behind him confirms your suspicions - this is some kind of rat-person. He seems oblivious to your presence as he stirs a cauldron of viscous fluid with one hand; a neat stack of beakers and phials sit in the dirt to his left. You see a smile break across his aged visage, and he says, "<i>Come closer child. I will not bite.</i>"<br><br>Apprehensive of the dangers of this unknown land, you cautiously approach.<br><br>"<i>I am Rathazul the Alchemist. Once I was famed for my miracle cures. Now I idle by this lake, helpless to do anything but measure the increasing amounts of corruption that taint its waters,</i>" he says as he pulls back his hood, revealing the entirety of his very bald and wrinkled head.<br><br>');
        ...
        if (!offered) {
            GUI.outputText('He sighs dejectedly, "<i>I am not sure what I can do for you, youngling. This world is fraught with unimaginable dangers, and you\'re just scratching the surface of them.</i>"<br><br>You nod and move on, leaving the depressed alchemist to his sadness.');
            GUI.doNext(Camp.returnToCampUseOneHour);
        }
    }
    static rathazulMoveToCamp() {
        GUI.clearOutput();
        GUI.outputText('Rathazul smiles happily back at you and begins packing up his equipment.  He mutters over his shoulder, "<i>It will take me a while to get my equipment moved over, but you head on back and I\'ll see you within the hour.  Oh my, yes.</i>"\n\nHe has the look of someone experiencing hope for the first time in a long time.');
    ...
    static campRathazul() {
        ...
        GUI.outputText('Rathazul looks up from his equipment and gives you an uncertain smile.<br><br>"<i>Oh, don\'t mind me,</i>" he says, "<i>I\'m just running some tests here. Was there something you needed, ' + liveData.player.name + '?</i>"<br><br>');
        ...
        if (!offered) {
            ...
            if (liveData.gameFlags[FLAG.RATHAZUL_CAMP] > 0)
                GUI.doNext(Camp.campFollowersMenu);
            else
    ...
    static rathazulWorkOffer() {
        ...
        if (pCounter > 0) {
            if (pCounter == 1)
                GUI.outputText('The rat mentions, "<i>I see you have at least one tainted item on you... for 20 gems I could remove most of the taint, making it a good deal safer to use. Of course, who knows what kind of freakish transformations it would cause...</i>"<br><br>');
            else
                GUI.outputText('The rat mentions, "<i>I see you have a number of demonic items on your person. For 20 gems I could attempt to remove the taint from one of them, rendering it a good deal safer for consumption. Of course it would not remove most of the transformative properties of the item...</i>"<br><br>');
            purify = true;
        ...
        if (liveData.player.gems >= 50) {
            ...
            if (liveData.gameFlags[FLAG.RATHAZUL_PURCHASE_COUNTER] >= 8)
                GUI.outputText("I should be able to make exotic-colored dyes if you're interested. ");
        ...
        if (liveData.gameFlags[FLAG.RATHAZUL_PURCHASE_COUNTER] >= 5) {
            GUI.outputText('The rat mentions, "<i>You know, I could make something new if you\'re willing to hand over two of vials labeled "Equinum", one vial of minotaur blood and one hundred gems. Or five bottles of Lactaid and two bottles of purified LaBova along with 250 gems.</i>"<br><br>');
        }
        //Reducto
        if (liveData.gameFlags[FLAG.RATHAZUL_CAMP] > 0 && liveData.gameFlags[FLAG.RATHAZUL_PURCHASE_COUNTER]) {
            GUI.outputText("The rat hurries over to his supplies and produces a container of paste, looking rather proud of himself, \"<i>Good news everyone! I've developed a paste you could use to shrink down any, ah, oversized body parts. The materials are expensive though, so I'll need ");
        ...
        if (totalOffers > 0) {
            ...
            GUI.menu();
            if (showArmorMenu)
                GUI.addButton(0, "Armor", this.rathazulArmorMenu, null, null, null, "Ask Rathazul to make an armour for you.");
            if (dyes) {
            ...
            if (purify)
                GUI.addButton(4, "Purify", this.purifySomething, null, null, null, "Ask him to purify any tainted potions. <br><br>Cost: 20 Gems.");
    ...
    static craftSilkArmor() {
        ...
        GUI.outputText('The rat limps over to his equipment, spider-silk in hand. With efficient, practiced motions, he runs a few tests. As he finishes, he sighs and explains, "<i>This will be harder than I thought. The webbing is highly resistant to most of my alchemic reagents. To even begin to work with such material I will need a number of rare, expensive elements. I would need 500 gems to even start such a project.</i>"<br><br>');
    ...
    static commissionSilkArmorForReal() {
        GUI.clearOutput();
        GUI.outputText('You sort 500 gems into a pouch and toss them to Rathazul, along with the rest of the webbing. The wizened alchemist snaps the items out of the air with lightning-fast movements and goes to work immediately. He bustles about with enormous energy, invigorated by the challenging task before him. It seems Rathazul has completely forgotten about you, but as you turn to leave, he calls out, "<i>What did you want me to make? A mage\'s robe or some nigh-impenetrable armor? Or undergarments if you want.</i>"<br><br>');
        GUI.menu();
        if (liveData.player.hasItem(Items.Materials.SpiderSilk, 5)) {
    ...
    static chooseArmorOrRobes(armorType) {
        ...
        if (liveData.gameFlags[FLAG.RATHAZUL_CAMP] > 0)
            GUI.outputText("You turn back to the center of your camp");
        else
            GUI.outputText("You head back to your camp");
    ...
    static collectRathazulArmor() {
        ...
        if (liveData.gameFlags[9999] == 1)
            GUI.outputText("armor");
        else if (liveData.gameFlags[9999] == 2)
            GUI.outputText("robe");
        else
            GUI.outputText("undergarment");
        GUI.outputText(' is finished!</i>"<br><br>');
        let itype;
        switch (liveData.gameFlags[9999]) {
            case 1: //Armor
                //GUI.outputText(images.showImage("rathazul-craft-silkarmor"));
                GUI.outputText("A glittering white suit of armor sits atop a crude armor rack, reflecting the light that plays across its surface beautifully. You definitely didn't expect anything like this! It looks nearly identical to a set of light platemail, though instead of having a cold metal surface, the armor feels slightly spongy, with just a little bit of give in it.<br><br>");
                GUI.outputText('While you marvel at the strange equipment, Rathazul explains, "<i>When you said you wanted armor, I realized I could skip a few of the alchemical processes used to soften material. The savings let me acquire a cheap metal set of armor to use as a base, and I molded half the armor around each piece, then removed it and created the outer, defensive layers with the rest of the webbing. Unfortunately, I didn\'t have enough silk for a solid codpiece, but I did manage to make a you thin loincloth from the leftover scraps - for modesty.</i>"<br><br>');
                itype = Items.Armor.SpiderSilkArmor;
                break;
            case 2: //Robes
                //GUI.outputText(images.showImage("rathazul-craft-silkrobes"));
                GUI.outputText("Hanging from a small rack is a long, flowing robe. It glitters brightly in the light, the pearl-white threads seeming to shimmer and shine with every ripple the breeze blows through the soft fabric. You run your fingers over the silken garment, feeling the soft material give at your touch. There's a hood with a golden border embroidered around the edge. For now, it hangs limply down the back, but it would be easy to pull up in order to shield the wearer's eyes from harsh sunlight or rainy drizzle. The sleeves match the cowl, circled with intricate threads laid out in arcane patterns.<br><br>");
                GUI.outputText("Rathazul gingerly takes down the garment and hands it to you. \"<i>Don't let the softness of the material fool you. This robe is tougher than many armors, and the spider-silk's properties may even help you in your spell-casting as well.</i>\"<br><br>");
                itype = Items.Armor.SpiderSilkRobes;
                break;
            case 3: //Bra
                //GUI.outputText(images.showImage("rathazul-craft-silkbra"));
                GUI.outputText("On a table is a pair of white bra. It glitters brightly in the light, the pearl-white threads seeming to shimmer and shine with every ripple the breeze blows through the soft fabric. You run your fingers over the silken garment, feeling the soft material give at your touch. <br><br>");
            ...
            case 4: //Panties
                ...
                GUI.outputText("On a table is a pair of white panties. It glitters brightly in the light, the pearl-white threads seeming to shimmer and shine with every ripple the breeze blows through the soft fabric. You run your fingers over the silken garment, feeling the soft material give at your touch. <br><br>");
            ...
            case 5: //Loincloth
                ...
                GUI.outputText("On a table is a white loincloth. It glitters brightly in the light, the pearl-white threads seeming to shimmer and shine with every ripple the breeze blows through the soft fabric. You run your fingers over the silken garment, feeling the soft material give at your touch. <br><br>");
    ...
    static craftDragonscaleArmor() {
        ...
        if (liveData.player.hasItem(Items.Materials.DragonScale, 5)) {
            ...
            GUI.outputText("<br><br>You realize you're still a bit short on dragonscales for the armor but you can have undergarments made instead.");
    ...
    static craftDragonscaleArmorForReal(type) {
        if (type == 0 || type == 1) {
            //Armor or robes
        ...
        switch (type) {
            case 0: //Armor
                ...
                GUI.outputText('The rat takes the scales and works on his bench for an hour while you wait. Once he has finished, Ratzhul is beaming with pride, "<i>I think you\'ll be pleased. Go ahead and take a look.</i>"<br><br>He hands you the armor. ');
                GUI.outputText('The armor is red and the breastplate has nicely decorated pauldrons to give an imposing looks. You touch the armor and feel the scaly texture. "<i>It\'s quite flexible and should offer very good protection,</i>" Rathazul says.');
                itype = Items.Armor.DragonscaleArmor;
                break;
            case 1: //Robes
                //GUI.outputText(images.showImage("rathazul-craft-dragonscalerobes"));
                GUI.outputText('The rat takes the scales and works on his bench for an hour while you wait. Once he has finished, Ratzhul is beaming with pride, "<i>I think you\'ll be pleased. Go ahead and take a look.</i>"<br><br>He hands you the robes. ');
            ...
            case 2: //Bra
                ...
                GUI.outputText('The rat takes the scales and works on his bench for an hour while you wait. Once he has finished, Ratzhul is beaming with pride, "<i>I think you\'ll be pleased. Go ahead and take a look.</i>"<br><br>He hands you the bra. ');
            ...
            case 3: //Thong
                ...
                GUI.outputText('The rat takes the scales and works on his bench for an hour while you wait. Once he has finished, Ratzhul is beaming with pride, "<i>I think you\'ll be pleased. Go ahead and take a look.</i>"<br><br>He hands you the thong. ');
                GUI.outputText("It's nicely textured with dragon scales. \"<i>I've used leather straps to maintain the flexibility. It should be comfortable and protective,</i>\" Rathazul says.");
                itype = Items.Undergarments.DragonscaleThone;
                break;
            case 4: //Loincloth
                //GUI.outputText(images.showImage("rathazul-craft-dragonscaleloincloth"));
                GUI.outputText('The rat takes the scales and works on his bench for an hour while you wait. Once he has finished, Ratzhul is beaming with pride, "<i>I think you\'ll be pleased. Go ahead and take a look.</i>"<br><br>He hands you the loincloth. ');
                GUI.outputText("It's nicely textured with dragon scales. \"<i>I've used leather straps to maintain the flexibility. It should be comfortable and protective,</i>\" Rathazul says.");
    ...
    static craftMaraeArmor(divine) {
        ...
        if (!divine) {
            ...
            GUI.outputText("You show him the pieces of thick bark with tentacles attached. <br><br> \"<i>My, my. That's definitely the strangest thing I've ever seen. But as you've requested, I'll make armor for you,</i>\" the old rat says. He takes the pile of bark, taking care to avoid touching the still-alive tentacles. He works on his bench for an hour while you wait. <br><br>");
            GUI.outputText("Once he has finished, Ratzhul is beaming with both pride and shame, \"<i>I think you'll be pleased. Go ahead and take a look. I'm not working on this type of armor again. I nearly got surprised by tentacles.</i>\"<br><br>He hands you the armor. <br><br>");
            ...
            if (liveData.player.mf("m", "f") == "f")
                GUI.outputText(" with stockings and garters");
            ...
            GUI.outputText('You show him the pieces of glowing white thick bark attached. <br><br> "<i>My, my. I heard a voice from Marae instructing me to make the armor for you,</i>" the old rat says. He takes the pile of bark and works on his bench for an hour while you wait. <br><br>');
            GUI.outputText("Once he has finished, Ratzhul is beaming with both pride and shame, \"<i>I think you'll be pleased. Go ahead and take a look. I'm not working on this type of armor again. It took me many attempts to bend the bark plates to get them right.</i>\"<br><br>He hands you the armor. <br><br>");
            GUI.outputText("The plates are white like snow. The armor includes a breastplate, pauldrons, full arm guards, and knee-high boots. You notice there are no pants. As you turn to ask him where the pants are, you see him scratching his head and hastily rustling in drawers. He mutters under his breath, \"<i>I'm sorry, I'm sorry, I got so focused on working on the pauldrons that I forgot to make any leg coverings! Here, this should look good with it, and it won't restrict your movements.</i>\" He hands you a silken loincloth");
            if (liveData.player.mf("m", "f") == "f")
                GUI.outputText(" with stockings and garters");
            GUI.outputText('. He still manages to look somewhat pleased with himself in spite of the blunder, even bragging a little bit, "<i>Let me show you the different lengths of string I used.</i>"<br><br>');
            if (liveData.player.cockTotal() > 0 && liveData.player.biggestCockArea() >= 40)
