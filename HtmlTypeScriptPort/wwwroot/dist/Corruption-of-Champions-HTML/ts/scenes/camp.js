...
class Camp {
    ...
    static doCamp() {
        ...
        if (liveData.time.days < 10)
            GUI.outputText("Your campsite is fairly simple at the moment.  Your tent and bedroll are set in front of the rocks that lead to the portal.  You have a small fire pit as well.  ");
        ...
        //}
        if (liveData.time.days >= 20)
            GUI.outputText("You've even managed to carve some artwork into the rocks around the camp's perimeter.<br><br>");
        else
        ...
        if (liveData.time.hours < 6 || liveData.time.hours > 20)
            ...
            if (liveData.time.hours == 19)
                GUI.outputText("The sun is close to the horizon, getting ready to set. ");
            if (liveData.time.hours == 20)
                GUI.outputText("The sun has already set below the horizon. The sky glows orange. ");
        ...
        if (this.followersCount() > 0)
            GUI.addButton(2, "Followers", this.campFollowersMenu, null, null, null, "Check up on any followers or companions who are joining you in or around your camp. You'll probably just end up sleeping with them.");
        if (this.loversCount() > 0)
            GUI.addButton(3, "Lovers", this.campLoversMenu, null, null, null, "Check up on any lovers you have invited so far and interact with them.");
        if (this.slavesCount() > 0)
            GUI.addButton(4, "Slaves", this.campSlavesMenu, null, null, null, "Check up on any slaves you have received and interact with them.");
    ...
    */
    static campFollowersMenu() {
        ...
        GUI.displaySprite();
        GUI.menu();
        if (liveData.gameFlags[FLAG.JOJO_CAMP] == 1)
            GUI.addButton(0, "Jojo", JojoScene.jojoCamp, null, null, null, "Go find Jojo around the edges of your camp and meditate with him or talk about watch duty.");
        if (liveData.gameFlags[FLAG.RATHAZUL_CAMP] > 0)
            GUI.addButton(1, "Rathazul", RathazulScene.campRathazul, null, null, null, "Visit with Rathazul to see what alchemical supplies and services he has available at the moment.");
    ...
    static returnToCampUseCustomMinutes(minutes) {
    ...
    static followersCount() {
        var count = 0;
        if (liveData.gameFlags[FLAG.JOJO_CAMP] > 0 && liveData.gameFlags[FLAG.JOJO_CORRUPTION_STAGE] < 5)
            count++;
        if (liveData.gameFlags[FLAG.RATHAZUL_CAMP] > 0)
            count++;
        return count;
