...
class Camp {
    ...
    static doCamp() {
        ...
        GUI.hideUpDown();
        GUI.displaySprite();
        GUI.setMenuButton("buttonMain", "Main Menu", mainMenu);
        ...
        //}
        if (liveData.time.days >= 20)
            outputText("You've even managed to carve some artwork into the rocks around the camp's perimeter.<br><br>");
        else
            outputText("<br><br>");
        ...
        //outputText("AmilyMet = " + liveData.gameFlags[AMILY_MET] + "<br>");
        ...
        GUI.menu();
        GUI.addButton(0, "Explore", Areas.GenericExploration.exploreMenu, null, null, null, "Explore to find new regions and visit any discovered regions.");
    ...
    */
    static campFollowersMenu() {
        ...
        GUI.displaySprite();
        GUI.menu();
        if (liveData.gameFlags[JOJO_CAMP] == 1)
    ...
    static campSlavesMenu() {
        ...
        GUI.displaySprite();
        GUI.menu();
        if (liveData.gameFlags[JOJO_CAMP] == 2)
    ...
    static doMasturbate() {
        ...
        liveData.advanceMinutes(30 - Math.floor(liveData.player.sen / 4));
        GUI.doNext(this.doCamp);
    }
    static doSleep() {
        //For now
    ...
    static followersCount() {
        var count = 0;
        if (liveData.gameFlags[JOJO_CAMP] > 0 && liveData.gameFlags[JOJO_CORRUPTION_STAGE] < 5)
            count++;
        if (liveData.gameFlags[RATHAZUL_CAMP] > 0)
            count++;
        return count;
