...
Camp.doCamp = function () {
    ...
    if (Camp.followersCount() > 0)
        addButton(2, "Followers", Camp.campFollowersMenu, null, null, null, "Check up on any followers or companions who are joining you in or around your camp. You'll probably just end up sleeping with them.");
    if (Camp.loversCount() > 0)
        addButton(3, "Lovers", Camp.campLoversMenu, null, null, null, "Check up on any lovers you have invited so far and interact with them.");
    if (Camp.slavesCount() > 0)
        addButton(4, "Slaves", Camp.campSlavesMenu, null, null, null, "Check up on any slaves you have received and interact with them.");
...
Camp.campFollowersMenu = function () {
    ...
    displaySprite();
    menu();
    if (gameFlags[JOJO_CAMP] == 1)
        addButton(0, "Jojo", JojoScene.jojoCamp, null, null, null, "Go find Jojo around the edges of your camp and meditate with him or talk about watch duty.");
    if (gameFlags[RATHAZUL_CAMP] > 0)
        addButton(1, "Rathazul", RathazulScene.campRathazul, null, null, null, "Visit with Rathazul to see what alchemical supplies and services he has available at the moment.");
...
Camp.campSlavesMenu = function () {
    ...
    displaySprite();
    menu();
    if (gameFlags[JOJO_CAMP] == 2)
        addButton(0, "Jojo", JojoScene.jojoCampCorrupt, null, null, null, "Call your corrupted pet into camp in order to relieve your desires in a variety of sexual positions? He's ever so willing after your last encounter with him.");
    addButton(14, "Back", Camp.doCamp);
};
//ACTIONS
...
Camp.followersCount = function () {
    var count = 0;
    if (gameFlags[JOJO_CAMP] > 0 && gameFlags[JOJO_CORRUPTION_STAGE] < 5)
        count++;
    if (gameFlags[RATHAZUL_CAMP] > 0)
        count++;
    return count;
