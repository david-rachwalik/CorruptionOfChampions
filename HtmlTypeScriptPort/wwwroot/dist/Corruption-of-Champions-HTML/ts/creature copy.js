...
Creature.prototype.dynStats = function () {
...
Creature.prototype.damagePercent = function (displayMode, applyModifiers) {
    ...
    if (applyModifiers) {
        if (player.weapon == Items.Weapons.JewelRapier || player.weapon == Items.Weapons.SPEAR || (player.weapon.name.indexOf("staff") != -1 && player.findPerk(PerkLib.StaffChanneling) >= 0))
            armorMod = 0;
        if (player.weapon == Items.Weapons.Katana)
            armorMod -= 5;
...
Creature.prototype.totalCocks = function () {
    //Alternate
...
Creature.prototype.isPregnant = function () {
    return this.pregnancyType != 0;
};
Creature.prototype.isButtPregnant = function () {
...
Creature.prototype.allVaginaDescript = function () {
    if (player.vaginas.length == 1)
        return this.vaginaDescript(rand(player.vaginas.length - 1));
    if (player.vaginas.length > 1)
        return this.vaginaDescript(rand(player.vaginas.length - 1)) + "s";
...
Creature.prototype.minoCumAddiction = function (raw) {
    ...
    if (gameFlags[MINOTAUR_CUM_ADDICTION_TRACKER] < 0)
        gameFlags[MINOTAUR_CUM_ADDICTION_TRACKER] = 0;
    if (gameFlags[MINOTAUR_CUM_ADDICTION_STATE] < 0)
        gameFlags[MINOTAUR_CUM_ADDICTION_STATE] = 0;
    if (gameFlags[MINOTAUR_CUM_ADDICTION_TRACKER] > 120)
        gameFlags[MINOTAUR_CUM_ADDICTION_TRACKER] = 120;
    ...
    if (gameFlags[MINOTAUR_CUM_ADDICTION_TRACKER] >= 60)
        raw /= 2;
    if (gameFlags[MINOTAUR_CUM_ADDICTION_TRACKER] >= 80)
        raw /= 2;
    if (gameFlags[MINOTAUR_CUM_ADDICTION_TRACKER] >= 90)
        raw /= 2;
    if (player.findPerk(PerkLib.MinotaurCumResistance) >= 0)
        raw *= 0;
    //If in withdrawl, readdiction is potent!
    if (gameFlags[MINOTAUR_CUM_ADDICTION_STATE] == 3)
        raw += 10;
    if (gameFlags[MINOTAUR_CUM_ADDICTION_STATE] == 2)
        raw += 5;
    ...
    if (this.findPerk(PerkLib.MinotaurCumResistance) >= 0)
        gameFlags[MINOTAUR_CUM_ADDICTION_TRACKER] = 0; //Never get addicted!
    if (gameFlags[MINOTAUR_CUM_ADDICTION_TRACKER] > 120)
        gameFlags[MINOTAUR_CUM_ADDICTION_TRACKER] = 120;
    if (gameFlags[MINOTAUR_CUM_ADDICTION_TRACKER] < 0)
        gameFlags[MINOTAUR_CUM_ADDICTION_TRACKER] = 0;
...
//# sourceMappingURL=creature%20copy.js.map