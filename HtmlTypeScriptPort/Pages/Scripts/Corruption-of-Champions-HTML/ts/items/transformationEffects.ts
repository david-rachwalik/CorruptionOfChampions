...
abstract class TransformationEffects {
    static beeTFs(type: number): void {
        let pure = type == 1
        let special = type == 2
        let changes = 0
        let changeLimit = 1
        ...
        if (changes < changeLimit && pure) {
            ...
            changes++
            if (liveData.player.cor > 80) GUI.outputText("Your head aches, as if thunder was echoing around your skull. ")
            else if (liveData.player.cor > 60) GUI.outputText("You feel a headache forming just behind your eyes. In no time flat it reaches full strength. ")
            else if (liveData.player.cor > 40) GUI.outputText("A wave of stinging pain slices through your skull. ")
            else if (liveData.player.cor > 20) GUI.outputText("A prickling pain spreads throughout your skull. ")
            else GUI.outputText("You feel a mildly unpleasant tingling inside your skull. ")
            if (liveData.player.cor > 0) GUI.outputText("It quickly passes, leaving you more clearheaded")
            ...
            if (liveData.player.cor > 0 && changes < changeLimit && UTIL.rand(1.5) == 0 && liveData.player.lib > 40) {
                ...
                changes++
            } else if (liveData.player.cor > 0) GUI.outputText(".")
        }
        //bee item corollary:
        if (changes < changeLimit && liveData.player.hairType == 4 && UTIL.rand(2) == 0) {
            //-insert anemone hair removal into them under whatever criteria you like, though hair removal should precede abdomen growth; here's some sample text:
            GUI.outputText(
                "<br><br>As you down the sticky-sweet honey, your head begins to feel heavier. Reaching up, you notice your tentacles becoming soft and somewhat fibrous. Pulling one down reveals that it feels and smells like the honey you just ate; you watch as it dissolves into many thin strands coated in the sugary syrup. <b>Your hair is back to normal (well, once you wash the honey out)!</b>"
            )
        ...
        if (changes < changeLimit && liveData.player.hairColor != "shiny black" && liveData.player.hairColor != "black and yellow" && liveData.player.hairLength > 10 && UTIL.rand(5) == 0) {
            ...
            if (UTIL.rand(9) == 0) liveData.player.hairColor = "black and yellow"
            else liveData.player.hairColor = "shiny black"
        ...
        if (changes < changeLimit && liveData.player.bRows() > 2 && UTIL.rand(3) == 0 && !liveData.hyperHappy) {
            changes++
            GUI.outputText(
                "<br><br>You stumble back when your center of balance shifts, and though you adjust before you can fall over, you're left to watch in awe as your bottom-most " +
        ...
        if (liveData.player.averageNipplesPerBreast() > 1 && changes < changeLimit && UTIL.rand(4) == 0) {
            ...
            if (liveData.player.biggestTitSize() < 1) GUI.outputText("'breast'.")
            else GUI.outputText("breast.")
            changes++
        ...
        if (changes < changeLimit && liveData.player.tailType == ENUM.TailType.TAIL_TYPE_BEE_ABDOMEN && liveData.player.tailRecharge < 15 && UTIL.rand(2)) {
            ...
            if (liveData.player.tailVenom > 100) liveData.player.tailVenom = 100
        ...
        if (special) {
            ...
            if (!liveData.player.hasCock()) {
                ...
                GUI.outputText(
                    "<br><br>The effects of the honey move towards your groin, and into your " +
                ...
                if (liveData.player.cocks[0].cArea() <= 20) GUI.outputText("It now swings as low as your knees!")
                else if (liveData.player.cocks[0].cArea() <= 50) GUI.outputText("While erect, your massive member fills the lower half of your vision.")
                else GUI.outputText("Your member is now simply huge, you wonder what in the world could actually take your massive size now?")
                GUI.outputText("</b>")
                ...
                GUI.outputText(
                    "<br><br>Your huge member suddenly starts to hurt, especially the tip of the thing. At the same time, you feel your length start to get incredibly sensitive and the base of your shaft starts to itch. You tear off your " +
                        liveData.player.armor.equipmentName +
                ...
                if (mult > 0) {
                    ...
                    GUI.outputText(
                        "<br><br>The effects of the honey don't seem to focus on your groin this time and you have a feeling that your " +
                            liveData.player.cockDescript(0) +
            ...
            if (liveData.player.femininity >= 60 || liveData.player.femininity <= 40) {
                ...
                if (liveData.player.femininity >= 60) liveData.player.femininity -= 3
                else liveData.player.femininity += 3
            }
    ...
    static canineTFs(type: number): void {
        let temp = 0
        let temp2 = 0
        let temp3 = 0
        ...
        switch (type) {
            case 0: //Normal
                if (crit) GUI.outputText("The pepper tastes particularly potent, searingly hot and spicy.")
                else GUI.outputText("The pepper is strangely spicy but very tasty.")
                break
            case 1: //Oversized
        ...
        if (type == 3) {
            ...
            if (liveData.player.cor < 50) GUI.outputText(" You shake your head, blushing hotly. Where did that thought come from?")
        }
        if (liveData.player.str < 50 && UTIL.rand(3) == 0) {
            liveData.player.modStats(["str", crit])
            if (crit > 1) GUI.outputText("<br><br>Your muscles ripple and grow, bulging outwards.")
            else GUI.outputText("<br><br>Your muscles feel more toned.")
            changes++
        }
        if (liveData.player.spe < 30 && UTIL.rand(3) == 0 && changes < changeLimit) {
            liveData.player.modStats(["spe", crit])
            if (crit > 1) GUI.outputText("<br><br>You find your muscles responding quicker, faster, and you feel an odd desire to go for a walk.")
            else GUI.outputText("<br><br>You feel quicker.")
            changes++
        }
        ...
        if (changes < changeLimit && liveData.player.hairType == 1 && UTIL.rand(4) == 0) {
            ...
            if (liveData.player.hairLength >= 6)
                GUI.outputText(
                    "<br><br>A lock of your downy-soft feather-hair droops over your eye. Before you can blow the offending down away, you realize the feather is collapsing in on itself. It continues to curl inward until all that remains is a normal strand of hair. <b>Your hair is no longer feathery!</b>"
                )
            //(short)
            else
                GUI.outputText(
        ...
        if (changes < changeLimit && UTIL.rand(5) == 0 && liveData.player.eyeType > ENUM.EyeType.EYES_HUMAN) {
            if (liveData.player.eyeType == ENUM.EyeType.EYES_BLACK_EYES_SAND_TRAP) {
                ...
                GUI.outputText(
                    "<br><br>You blink and stumble, a wave of vertigo threatening to pull your " + liveData.player.feet() + " from under you. As you steady and open your eyes, you realize something seems different. Your vision is changed somehow."
                )
            ...
            //If player doesnt have 2 dogdicks
                ...
                if (liveData.player.countCocksOfType(ENUM.CockType.DOG) == 0) {
                    ...
                    if (liveData.player.cockTotal() == 0) {
                        ...
                        GUI.outputText(
                            "<br><br>" +
                                liveData.player.clothedOrNakedLower(
                                    "A painful lump forms on your groin, nearly doubling you over as it presses against your " + liveData.player.armor.equipmentName + ". You rip open your gear and",
                                    "An erecting sensation forms in your groin. You"
                                ) +
                    ...
                    //2 dicks+ - convert first 2 to doggie-dom
                        ...
                        GUI.outputText(
                            "<br><br>Your crotch twitches, and you pull open your " +
                                liveData.player.armor.equipmentName +
        ...
        if (type == 4) {
            ...
            if (liveData.player.cockTotal() > 0) {
                ...
                if (liveData.player.countCocksOfType(ENUM.CockType.DOG) > 0) {
                    ...
                    while (temp > 0) {
                        temp--
                        if (liveData.player.cocks[temp].cockType == ENUM.CockType.DOG && liveData.player.cocks[temp].knotMultiplier < liveData.player.cocks[temp2].knotMultiplier) temp2 = temp
                    }
                    //Have smallest knotted cock selected.
                    temp3 = ((UTIL.rand(2) + 5) / 20) * crit
                    if (liveData.player.cocks[temp2].knotMultiplier >= 1.5) temp3 /= 2
                    if (liveData.player.cocks[temp2].knotMultiplier >= 1.75) temp3 /= 2
                    if (liveData.player.cocks[temp2].knotMultiplier >= 2) temp3 /= 5
                    liveData.player.cocks[temp2].knotMultiplier += temp3
                    GUI.outputText("<br><br>")
                    if (temp3 < 0.06) GUI.outputText("Your " + Appearance.cockNoun(ENUM.CockType.DOG) + " feels unusually tight in your sheath as your knot grows.")
                    if (temp3 >= 0.06 && temp3 <= 0.12) GUI.outputText("Your " + Appearance.cockNoun(ENUM.CockType.DOG) + " pops free of your sheath, thickening nicely into a bigger knot.")
                    if (temp3 > 0.12) GUI.outputText("Your " + Appearance.cockNoun(ENUM.CockType.DOG) + " surges free of your sheath, swelling thicker with each passing second. Your knot bulges out at the base, growing far beyond normal.")
            ...
            else GUI.outputText("<br><br>A slight wave of nausea passes through you. It seems this pepper does not quite agree with your body.")
        }
        //GROW BALLS
        if (type == 5) {
            if (liveData.player.balls <= 1) {
                GUI.outputText(
                    "<br><br>A spike of pain doubles you up, nearly making you vomit. You stay like that, nearly crying, as a palpable sense of relief suddenly washes over you. You look down and realize you now have a small sack, complete with two relatively small balls."
                )
                ...
                if (liveData.player.ballSize > 10) liveData.player.ballSize -= 0.5
                //Texts
                if (liveData.player.ballSize <= 2)
                    GUI.outputText(
                        "<br><br>A flash of warmth passes through you and a sudden weight develops in your groin. You pause to examine the changes and your roving fingers discover your " +
                            liveData.player.ballsDescriptLight() +
                            " have grown larger than a human's."
                    )
                if (liveData.player.ballSize > 2)
                    GUI.outputText(
                        "<br><br>A sudden onset of heat envelops your groin, focusing on your " + liveData.player.sackDescript() + ". Walking becomes difficult as you discover your " + liveData.player.ballsDescriptLight() + " have enlarged again."
                    )
        ...
        if (liveData.player.cocks.length > 0) {
            ...
            if (type != 4 && liveData.player.countCocksOfType(ENUM.CockType.DOG) > 0 && ((changes < changeLimit && UTIL.rand(1.4) == 0) || type == 1)) {
                ...
                if (liveData.player.cocks[temp2].knotMultiplier >= 2) temp3 /= 5
                liveData.player.cocks[temp2].knotMultiplier += temp3
                if (temp3 < 0.06) GUI.outputText("<br><br>Your " + liveData.player.cockDescript(temp2) + " feels unusually tight in your sheath as your knot grows.")
                if (temp3 >= 0.06 && temp3 <= 0.12) GUI.outputText("<br><br>Your " + liveData.player.cockDescript(temp2) + " pops free of your sheath, thickening nicely into a bigger knot.")
                if (temp3 > 0.12) GUI.outputText("<br><br>Your " + liveData.player.cockDescript(temp2) + " surges free of your sheath, swelling thicker with each passing second. Your knot bulges out at the base, growing far beyond normal.")
                liveData.player.modStats(["sen", 0.5], ["lus", 5 * crit])
                changes++
            }
            ...
            if (liveData.player.countCocksOfType(ENUM.CockType.DOG) < liveData.player.cocks.length && ((changes < changeLimit && UTIL.rand(1.6)) || type == 1) == 0) {
                ...
                if (liveData.player.cocks[temp3].cockType == ENUM.CockType.HORSE) {
                    ...
                    if (liveData.player.cocks[temp3].cockLength > 6) liveData.player.cocks[temp3].cockLength -= 2
                    else liveData.player.cocks[temp3].cockLength -= 0.5
                ...
                if (liveData.player.cocks[temp3].cockType == ENUM.CockType.DEMON) {
                    GUI.outputText(
                        "<br><br>Your " + liveData.player.cockDescript(temp3) + " color shifts red for a moment and begins to swell at the base, but within moments it smooths out, retaining its distinctive demonic shape, only perhaps a bit thicker."
                    )
            ...
            if (liveData.player.cumMultiplier < 2 && UTIL.rand(2) == 0 && changes < changeLimit) {
                ...
                if (temp < liveData.player.cumMultiplier + 0.05 * crit) {
                    ...
                    //Flavor text
                    if (liveData.player.balls == 0) GUI.outputText("<br><br>You feel a churning inside your gut as something inside you changes.")
                    if (liveData.player.balls > 0) GUI.outputText("<br><br>You feel a churning in your " + liveData.player.ballsDescriptLight() + ". It quickly settles, leaving them feeling somewhat more dense.")
                    if (crit > 1) GUI.outputText(" A bit of milky pre dribbles from your " + liveData.player.multiCockDescriptLight() + ", pushed out by the change.")
                }
                changes++
            }
            //Oversized pepper
            if (type == 1) {
                ...
                //Multicock
                    ...
                    liveData.player.modStats(["sen", 1], ["lus", 10])
                    if (liveData.player.cocks[temp].cockThickness <= 2) liveData.player.cocks[temp].thickenCock(1)
                }
                if (temp2 > 2)
                    GUI.outputText(
                        "<br><br>Your " +
                ...
                if (temp2 > 1 && temp2 <= 2)
                    GUI.outputText(
                        "<br><br>Aching pressure builds within your crotch, suddenly releasing as an inch or more of extra dick-flesh spills out. A dollop of pre beads on the head of your enlarged " +
                            liveData.player.cockDescript(temp) +
                            " from the pleasure of the growth."
                    )
                if (temp2 <= 1) GUI.outputText("<br><br>A slight pressure builds and releases as your " + liveData.player.cockDescript(temp) + " pushes a bit further out of your crotch.")
            }
        }
        //Female Stuff
        //Multiboobages
            ...
            if (liveData.player.breastRows[0].breastRating > 0 && liveData.player.vaginas.length > 0) {
                ...
                if (liveData.player.breastRows.length < 3 && UTIL.rand(2) == 0 && changes < changeLimit) {
                    ...
                    if (liveData.player.breastRows.length == 2) {
                        ...
                        if (liveData.player.breastRows[0].breastRating - 1 == 0) GUI.outputText("<br><br>A second set of breasts forms under your current pair, stopping while they are still fairly flat and masculine looking.")
                        else GUI.outputText("<br><br>A second set of breasts bulges forth under your current pair, stopping as they reach " + liveData.player.breastCup(temp) + "s.")
                    ...
                    if (liveData.player.breastRows.length > 2 && liveData.player.breastRows[0].breastRating > liveData.player.breastRows.length) {
                        ...
                        if (liveData.player.breastRows[temp].breastRating < 0) liveData.player.breastRows[temp].breastRating = 0
                        if (liveData.player.breastRows[temp - 1].breastRating < 0) liveData.player.breastRows[temp - 1].breastRating = 0
                        ...
                        else GUI.outputText("<br><br>Your abdomen tingles and twitches as a new row of " + liveData.player.breastCup(temp) + " " + liveData.player.breastDescript(temp) + " sprouts below your others.")
                        GUI.outputText(" A sensitive nub grows on the summit of each new tit, becoming a new nipple.")
                        changes++
                    }
                ...
                //If already has max doggie breasts!
                    ...
                    while (temp > 1 && temp2 == 0) {
                        ...
                        if (liveData.player.breastRows[temp].breastRating + 1 < liveData.player.breastRows[temp - 1].breastRating) {
                            ...
                            if (temp == 0) GUI.outputText("first ")
                            if (temp == 1) GUI.outputText("second ")
                            if (temp == 2) GUI.outputText("third ")
                            if (temp == 3) GUI.outputText("fourth ")
                            if (temp == 4) GUI.outputText("fifth ")
                            if (temp > 4) GUI.outputText("")
        ...
        if (changes < changeLimit && liveData.player.dogScore() >= 3 && UTIL.rand(4) == 0) {
            ...
            if (liveData.player.gender <= 1 || (liveData.player.gender == 3 && UTIL.rand(2) == 0)) {
                ...
                //break1
                if (liveData.player.cor < 33 || !liveData.player.hasCock()) GUI.outputText("<br>You shake your head to clear the unwanted fantasy from your mind, repulsed by it.")
                else {
                    GUI.outputText(
                    ...
                    liveData.player.modStats(["lus", 5 + liveData.player.lib / 20])
                    //Break 2
                    if (liveData.player.cor < 66) GUI.outputText("<br>You blink a few times, the fantasy fading as you master yourself. That daydream was so strange, yet so hot.")
                    else {
                        GUI.outputText(
                        ...
                        if (liveData.player.cor < 80) {
                            if (liveData.player.vaginas.length > 0) GUI.outputText("<br>You reluctantly pry your hand from your aching " + liveData.player.vaginaDescript(0) + " as you drag yourself out of your fantasy.")
                            else GUI.outputText("<br>You reluctantly pry your hand from your aching " + liveData.player.cockDescript(0) + " as you drag yourself out of your fantasy.")
                        } else {
                            GUI.outputText(
        ...
        if (type == 3 && liveData.player.hairColor != "midnight black") {
            if (liveData.player.skinType == ENUM.SkinType.SKIN_TYPE_FUR) GUI.outputText("<b><br><br>Your fur and hair tingles, growing in thicker than ever as darkness begins to spread from the roots, turning it midnight black.</b>")
            else GUI.outputText("<b><br><br>Your " + liveData.player.skinDesc + " itches like crazy as fur grows out from it, coating your body. It's incredibly dense and black as the middle of a moonless night.</b>")
        ...
        if (UTIL.rand(3) == 0 && liveData.player.lowerBody != ENUM.LowerBodyType.LOWER_BODY_TYPE_DOG && liveData.player.tailType == ENUM.TailType.TAIL_TYPE_DOG && liveData.player.earType == ENUM.EarType.EARS_DOG && changes < changeLimit) {
            //Feet -> paws
            if (liveData.player.lowerBody == ENUM.LowerBodyType.LOWER_BODY_TYPE_HUMAN) GUI.outputText("<br><br>You scream in agony as you feel the bones in your feet break and begin to rearrange. <b>You now have paws</b>.")
            //Hooves -> Paws
            ...
            else GUI.outputText("<br><br>Your lower body is wracked by pain! Once it passes, you discover that you're standing on fur-covered paws! <b>You now have paws</b>.")
        ...
        if (UTIL.rand(3) == 0 && changes < changeLimit && liveData.player.tailType != ENUM.TailType.TAIL_TYPE_DOG) {
            ...
            if (liveData.player.tailType >= ENUM.TailType.TAIL_TYPE_COW) GUI.outputText("<br><br>You feel your backside shift and change, flesh molding and displacing into a long puffy tail! ")
            changes++
        ...
        if (liveData.player.skinType == ENUM.SkinType.SKIN_TYPE_FUR && changes < changeLimit && UTIL.rand(3) == 0) {
            GUI.outputText(
                "<br><br>You become more... solid. Sinewy. A memory comes unbidden from your youth of a grizzled wolf you encountered while hunting, covered in scars, yet still moving with an easy grace. You imagine that must have felt something like this."
            )
    ...
    static cowTFs(tainted: boolean, enhanced: boolean): void {
        liveData.player.slimeFeed()
        //Changes done
        let changes = 0
        //Change limit
        ...
        if (enhanced) GUI.outputText("Pro Bova")
        else GUI.outputText("La Bova")
        ...
        if (changes < changeLimit && UTIL.rand(3) == 0) {
            ...
            if (temp <= 0) temp = 0
                ...
                if (UTIL.rand(2) == 0) GUI.outputText("<br><br>There is a slight pain as you feel your muscles shift somewhat.  Their appearance does not change much, but you feel much stronger.")
                else GUI.outputText("<br><br>You feel your muscles tighten and clench as they become slightly more pronounced.")
        ...
        if (changes < changeLimit && UTIL.rand(3) == 0) {
            ...
            if (temp <= 0) temp = 0
                ...
                if (UTIL.rand(2) == 0) GUI.outputText("<br><br>You feel your insides toughening up; it feels like you could stand up to almost any blow.")
                else GUI.outputText("<br><br>Your bones and joints feel sore for a moment, and before long you realize they've gotten more durable.")
        ...
        if (liveData.player.cocks.length > 0 && UTIL.rand(2) == 0 && !liveData.gameFlags[FLAG.HYPER_HAPPY]) {
            ...
            while (temp2 > 0) {
                temp2--
                if (liveData.player.cocks[temp].cockLength <= liveData.player.cocks[temp2].cockLength) temp = temp2
            }
            //Shrink said cock
            ...
            if (liveData.player.cocks[temp].cockLength < 2) {
                ...
                if (liveData.player.cockTotal() == 1 && !liveData.player.hasVagina()) {
                    ...
                    if (liveData.player.balls > 0) GUI.outputText("  At the same time, your " + liveData.player.ballsDescriptLight() + " fall victim to the same sensation; eagerly swallowed whole by your crotch.")
                    GUI.outputText(
        ...
        if (((tainted && liveData.player.biggestTitSize() <= 11) || (!tainted && liveData.player.biggestTitSize() <= 5)) && changes < changeLimit && (UTIL.rand(3) == 0 || enhanced)) {
            if (UTIL.rand(2) == 0) GUI.outputText("<br><br>Your " + liveData.player.breastDescript(0) + " tingle for a moment before becoming larger.")
            else GUI.outputText("<br><br>You feel a little weight added to your chest as your " + liveData.player.breastDescript(0) + " seem to inflate and settle in a larger size.")
        ...
        if (enhanced) {
            ...
            if (liveData.player.breastRows[0].nipplesPerBreast == 1) {
                ...
                if (liveData.player.breastRows.length >= 3 && liveData.player.breastRows[2].nipplesPerBreast == 1) {
                    ...
                    if (liveData.player.bRows() == 3) GUI.outputText("third row of " + liveData.player.breastDescript(2) + " mutates along with its sisters, sprouting into a wonderland of nipples.")
                        ...
                        if (liveData.player.bRows() >= 5) liveData.player.breastRows[4].nipplesPerBreast = 4
                        if (liveData.player.bRows() >= 6) liveData.player.breastRows[5].nipplesPerBreast = 4
                        if (liveData.player.bRows() >= 7) liveData.player.breastRows[6].nipplesPerBreast = 4
                        if (liveData.player.bRows() >= 8) liveData.player.breastRows[7].nipplesPerBreast = 4
                        if (liveData.player.bRows() >= 9) liveData.player.breastRows[8].nipplesPerBreast = 4
                    }
            ...
            //QUAD DAMAGE IF WEIRD SHIT BROKE BEFORE
                ...
                if (UTIL.rand(2) == 0) GUI.outputText("<br><br>A wave of pleasure passes through your chest as your " + liveData.player.breastDescript(0) + " start leaking milk from a massive jump in production.")
                else GUI.outputText("<br><br>Something shifts inside your " + liveData.player.breastDescript(0) + " and they feel MUCH fuller and riper.  You know that you've started producing much more milk.")
        ...
        //If breasts are already lactating and the player is not lactating beyond a reasonable level, they start lactating more:
            ...
            if (tainted && liveData.player.breastRows[0].lactationMultiplier > 1 && liveData.player.breastRows[0].lactationMultiplier < 5 && changes < changeLimit && (UTIL.rand(3) == 0 || enhanced)) {
                if (UTIL.rand(2) == 0) GUI.outputText("<br><br>A wave of pleasure passes through your chest as your " + liveData.player.breastDescript(0) + " start producing more milk.")
                else GUI.outputText("<br><br>Something shifts inside your " + liveData.player.breastDescript(0) + " and they feel fuller and riper.  You know that you've started producing more milk.")
            ...
            if (!tainted) {
                ...
                if ((liveData.player.breastRows[0].lactationMultiplier > 2 && liveData.player.findStatusEffect(StatusEffects.Feeder) >= 0) || liveData.player.breastRows[0].lactationMultiplier > 5) {
                    ...
                    else GUI.outputText("<br><br>The insides of your breasts suddenly feel bloated.  There is a spray of milk from them, and they settle closer to a more natural level of lactation.")
                    changes++
                    liveData.player.dynStats(["sen", 0.5])
        ...
        if (((enhanced && liveData.player.tallness < 96) || liveData.player.tallness < 84) && changes < changeLimit && UTIL.rand(2) == 0) {
            ...
            if (liveData.player.tallness > 74) temp = Math.floor(temp / 2)
            //Never 0
            if (temp == 0) temp = 1
            //Flavor texts.  Flavored like 1950's cigarettes. Yum.
            if (temp < 5) GUI.outputText("<br><br>You shift uncomfortably as you realize you feel off balance.  Gazing down, you realize you have grown SLIGHTLY taller.")
            if (temp >= 5 && temp < 7) GUI.outputText("<br><br>You feel dizzy and slightly off, but quickly realize it's due to a sudden increase in height.")
            if (temp == 7) GUI.outputText("<br><br>Staggering forwards, you clutch at your head dizzily.  You spend a moment getting your balance, and stand up, feeling noticeably taller.")
        ...
        if (enhanced && (liveData.player.skinDesc != "fur" || liveData.player.furColor != "black and white spotted")) {
            if (liveData.player.skinDesc != "fur")
                ...
                GUI.outputText(
                    "<br><br>A ripple spreads through your fur as some patches darken and others lighten.  After a few moments you're left with a black and white spotted pattern that goes the whole way up to the hair on your head!  <b>You've got cow fur!</b>"
                )
            liveData.player.skinDesc = "fur"
        ...
        if (liveData.gameFlags[FLAG.HAS_BLACK_NIPPLES] == 1 && changes < changeLimit && UTIL.rand(3) == 0) {
            GUI.outputText(
                "<br><br>Something invisible brushes against your " +
                    liveData.player.nippleDescript(0) +
        ...
        if (UTIL.rand(3) == 0) GUI.outputText(liveData.player.modFem(79, 3))
        if (UTIL.rand(3) == 0) GUI.outputText(liveData.player.modThickness(70, 4))
        if (UTIL.rand(5) == 0) GUI.outputText(liveData.player.modTone(10, 5))
        liveData.player.refillHunger(20)
    }

    static demonTFs(type: number, purified: boolean): void {
        if (type == 1) {
            //Incubi Draft
        }
        if (type == 2) {
    ...
    static succubiDelight(tainted: boolean): void {
        liveData.player.slimeFeed()
        let changes = 0
        let crit = 1
        //Determine crit multiplier (x2 or x3)
        if (UTIL.rand(4) == 0) crit += UTIL.rand(2) + 1
        let changeLimit = 1
        ...
        if (liveData.player.cor < 33) GUI.outputText(" This stuff is gross, why are you drinking it?")
        //high corruption
        if (liveData.player.cor >= 66) GUI.outputText(" You lick your lips, marvelling at how thick and sticky it is.")
        //Corruption increase
        if ((liveData.player.cor < 50 || UTIL.rand(2)) && tainted) {
            ...
            if (liveData.player.cor < 50) temp++
            if (liveData.player.cor < 40) temp++
            if (liveData.player.cor < 30) temp++
            //Corrupts the very corrupt slower
            if (liveData.player.cor >= 90) temp = 0.5
        ...
        if (UTIL.rand(1.5) == 0 && changes < changeLimit && liveData.player.balls > 0) {
            ...
            if (liveData.player.ballSize > 10) liveData.player.ballSize -= 0.5
            //And MUCH slower
            if (liveData.player.ballSize > 24) liveData.player.ballSize -= 0.3
            //Texts
            if (liveData.player.ballSize <= 2)
                GUI.outputText(
                    "<br><br>A flash of warmth passes through you and a sudden weight develops in your groin. You pause to examine the changes and your roving fingers discover your " +
        ...
        if (liveData.player.balls < 2 && changes < changeLimit && UTIL.rand(4) == 0) {
            if (liveData.player.balls == 0) {
                liveData.player.balls = 2
                GUI.outputText(
                    "<br><br>Incredible pain scythes through your crotch, doubling you over. You stagger around, struggling to pull open your " +
                        liveData.player.armor.equipmentName +
        ...
        if (changes < changeLimit && UTIL.rand(2) == 0 && liveData.player.cocks.length > 0) {
            if (liveData.player.cumMultiplier < 6 && UTIL.rand(2) == 0 && changes < changeLimit) {
                ...
                if (temp < liveData.player.cumMultiplier + 0.4 * crit) {
                    ...
                    //Flavor text
                    if (liveData.player.balls == 0) GUI.outputText("<br><br>You feel a churning inside your body as something inside you changes.")
                    if (liveData.player.balls > 0) GUI.outputText("<br><br>You feel a churning in your " + liveData.player.ballsDescriptLight() + ". It quickly settles, leaving them feeling somewhat more dense.")
                    if (crit > 1) GUI.outputText(" A bit of milky pre dribbles from your " + liveData.player.multiCockDescriptLight() + ", pushed out by the change.")
                    liveData.player.modStats(["lib", 1])
                }
    ...
    static equineTFs(): void {
        ...
        if (
            ...
            if (UTIL.rand(4) == 0 && liveData.gameFlags[FLAG.HORSE_WARNING] == 1 && liveData.player.findPerk(PerkLib.TransformationResistance) < 0) {
                ...
                //If player has no cocks
                    ...
                    GUI.outputText(
                        "<b>One year after...</b><br><br>As you graze small plants in the open plains that became your home, you hear a noise on your right side. As you raise your head to check where the noise comes from, preparing to run from a potential predator, you see a strange creature. It stands on two feet, its furless pink skin appearing beneath its clothes. "
                    )
                    if (liveData.player.gender == 0 || liveData.player.gender == 1) GUI.outputText("He is clearly a male, but you are somewhat confused as you can see not one but three bulges where his manhood would be.<br><br>")
                    if (liveData.player.gender == 2) GUI.outputText("She is clearly a female, as you can see her six breasts jiggle as she walks towards you, small stains appearing on her shirt where her nipples are.<br><br>")
                    if (liveData.player.gender == 3) GUI.outputText("You are somewhat confused as you can see a bulge near her thighs but also huge boobs jiggling as she walks, and you can't say if she's a male or female.<br><br>")
        ...
        if ((liveData.player.gender == 1 || liveData.player.gender == 3) && UTIL.rand(1.5) == 0 && changes < changeLimit) {
            ...
            if (liveData.player.countCocksOfType(ENUM.CockType.HORSE) + liveData.player.countCocksOfType(ENUM.CockType.DEMON) < liveData.player.cocks.length) {
                ...
                if (liveData.player.cocks.length == 1) {
                    ...
                    liveData.player.orgasm()
                    if (temp3 == 1) GUI.outputText(" <b>Your penis has transformed into a horse's!</b>")
                }
                //MULTICOCK
                else {
                    ...
                    if (liveData.player.hasSheath()) GUI.outputText(" Your sheath tingles and begins growing larger as the cock's base shifts to lie inside it.")
                    else
                        GUI.outputText(
            ...
            //Players cocks are all horse-type - increase size!
                ...
                if (temp2 <= 1) GUI.outputText("A slight pressure builds and releases as your " + liveData.player.cockDescript(temp) + " pushes a bit further out of your sheath.")
                changes++
            }
            ...
            if (UTIL.rand(2) == 0 && changes < changeLimit && liveData.player.countCocksOfType(ENUM.CockType.HORSE) > 0) {
                ...
                GUI.outputText(".")
                if (liveData.player.cor < 30) GUI.outputText(" You shudder in revulsion at the strange thoughts and vow to control yourself better.")
                if (liveData.player.cor >= 30 && liveData.player.cor < 60) GUI.outputText(" You wonder why you thought such odd things, but they have a certain appeal.")
                if (liveData.player.cor >= 60 && liveData.player.cor < 90) GUI.outputText(" You relish your twisted fantasies, hoping to dream of them again.")
                if (liveData.player.cor >= 90) GUI.outputText(" You flush hotly and give a twisted smile, resolving to find a fitting subject to rape and relive your fantasies.")
            ...
            if (UTIL.rand(2) == 0 && changes < changeLimit && liveData.player.ballSize <= 3 && liveData.player.countCocksOfType(ENUM.CockType.HORSE) > 0) {
                if (liveData.player.balls == 0) {
                    ...
                    liveData.player.ballSize = 1
                    GUI.outputText(
                        "<br><br>A nauseating pressure forms just under the base of your maleness. With agonizing pain the flesh bulges and distends, pushing out a rounded lump of flesh that you recognize as a testicle! A moment later relief overwhelms you as the second drops into your newly formed sack."
                    )
                    ...
                    if (liveData.player.ballSize > 2)
                        GUI.outputText(
                            "<br><br>A sudden onset of heat envelops your groin, focusing on your " + liveData.player.sackDescript() + ". Walking becomes difficult as you discover your " + player.ballsDescriptLight() + " have enlarged again."
                        )
                    liveData.player.modStats(["lib", 1], ["lus", 3])
                }
                changes++
                ...
                if (UTIL.rand(2) == 0 && changes < changeLimit) {
                    ...
                    if (liveData.player.breastRows.length == 1) {
                        ...
                        if (liveData.player.breastRows[0].breastRating > 3) {
                            ...
                            if (temp == 1) GUI.outputText("<br><br>You feel a weight lifted from you, and realize your " + liveData.player.breastDescript(0) + " have shrunk to a " + liveData.player.breastCup(0) + ".")
                            if (temp == 2) GUI.outputText("<br><br>You feel significantly lighter. Looking down, you realize your breasts are MUCH smaller, down to " + liveData.player.breastCup(0) + "s.")
                            changes++
                        }
                    }
                    //multiple
                        ...
                        if (liveData.player.biggestTitSize() > 3) GUI.outputText("<br>")
                        while (temp3 > 0) {
                            temp3--
                            if (liveData.player.breastRows[temp3].breastRating > 3) {
                                ...
                                temp2++
                                GUI.outputText("<br>")
                                if (temp3 < liveData.player.breastRows.length - 1) GUI.outputText("...and y")
        ...
        if (liveData.player.earType != ENUM.EarType.EARS_HORSE && liveData.player.tailType == ENUM.TailType.TAIL_TYPE_HORSE && changes < changeLimit && UTIL.rand(3) == 0) {
            if (liveData.player.earType == -1) GUI.outputText("<br><br>Two painful lumps sprout on the top of your head, forming into tear-drop shaped ears, covered with short fur. ")
            if (liveData.player.earType == ENUM.EarType.EARS_HUMAN) GUI.outputText("<br><br>Your ears tug painfully on your face as they begin shifting, moving upwards to the top of your head and transforming into a upright animalistic ears. ")
            if (liveData.player.earType == ENUM.EarType.EARS_DOG) GUI.outputText("<br><br>Your ears change shape, morphing into from their doglike shape into equine-like ears! ")
        ...
        if (UTIL.rand(3) == 0) GUI.outputText(liveData.player.modTone(60, 1))
        //FAILSAFE CHANGE
        if (changes == 0) {
            GUI.outputText("<br><br>Inhuman vitality spreads through your body, invigorating you!<br>")
            liveData.player.changeHP(20, true)
    ...
    static felineTFs(): void {
        ...
        if (liveData.player.str < 40 && UTIL.rand(3) == 0 && changes < changeLimit) {
            ...
            if (UTIL.rand(2) == 0) GUI.outputText("<br><br>Your muscles feel taut, like a coiled spring, and a bit more on edge.")
            else GUI.outputText("<br><br>You arch your back as your muscles clench painfully. The cramp passes swiftly, leaving you feeling like you've gotten a bit stronger.")
            liveData.player.modStats(["str", 1])
            changes++
        ...
        if (UTIL.rand(4) == 0 && changes < changeLimit) {
            ...
            if (liveData.player.inte < 15)
                //low intelligence
                GUI.outputText(
                    "<br><br>You feel like something is slipping away from you but can't figure out exactly what's happening. You scrunch up your " +
                ...
                if (UTIL.rand(2) == 0) {
                    ...
                    temp = UTIL.rand(3)
                    if (temp == 0) GUI.outputText("toss a ball around or something")
                    else if (temp == 1) GUI.outputText("play with some yarn")
                    else if (temp == 2) GUI.outputText("take a nap and stop worrying")
                } else GUI.outputText("in the sun and let your troubles slip away")
                GUI.outputText(".")
            } //High intelligence
            else
                GUI.outputText(
                    "<br><br>You start to feel a bit dizzy, but the sensation quickly passes. Thinking hard on it, you mentally brush away the fuzziness that seems to permeate your brain and determine that this fruit may have actually made you dumber. It would be best not to eat too much of it."
                )
            liveData.player.modStats(["int", -1])
            changes++
        }
        ...
        if (liveData.player.lib < 80 && changes < changeLimit && UTIL.rand(4) == 0) {
            ...
            if (liveData.player.countCocksOfType(ENUM.CockType.CAT) > 0) {
                ...
                if (liveData.player.cor < 33) GUI.outputText("You need to control yourself better.")
                else if (liveData.player.cor < 66) GUI.outputText("You're not sure how you feel about the fantasy.")
                else GUI.outputText("You hope to find a willing partner to make this a reality.")
            }
            //Else –
            else {
                GUI.outputText(
                    "<br><br>A rush of tingling warmth spreads through your body as it digests the fruit. You can feel your blood pumping through your extremities, making them feel sensitive and surprisingly sensual. It's going to be hard to resist getting "
                )
                if (liveData.player.lust > 60) GUI.outputText("even more ")
        ...
        if (UTIL.rand(4) == 0 && changes < changeLimit) {
            ...
            if (liveData.player.goIntoHeat(false)) {
                if (intensified) {
                    if (UTIL.rand(2) == 0) GUI.outputText("<br><br>The itch inside your " + liveData.player.vaginaDescript(0) + " is growing stronger, and you desperately want to find a nice cock to massage the inside.")
                    else
                        GUI.outputText(
                            "<br><br>The need inside your " +
        ...
        if (changes < changeLimit && UTIL.rand(4) == 0 && !liveData.hyperHappy) {
            ...
            if (!liveData.player.hasVagina() && liveData.player.biggestTitSize() > 2) temp2 = 2
            else if (liveData.player.biggestTitSize() > 4) temp2 = 4
            //IT IS!
            if (temp2 > 0) {
                ...
                for (let k = 0; k < liveData.player.breastRows.length; k++) {
                    ...
                    if (liveData.player.breastRows[k].breastRating > temp2) {
                        ...
                        if (liveData.player.breastRows[k].breastRating > 10) {
                            ...
                            if (temp3 == 0) GUI.outputText("<br><br>The " + liveData.player.breastDescript(0) + " on your chest wobble for a second, then tighten up, losing several cup-sizes in the process!")
                            else GUI.outputText(" The change moves down to your " + UTIL.num2Text2(k + 1) + " row of " + liveData.player.breastDescript(0) + ". They shrink greatly, losing a couple cup-sizes.")
                        }
                        //Small change
                        else {
                            liveData.player.breastRows[k].breastRating -= 1
                            if (temp3 == 0) GUI.outputText("<br><br>All at once, your sense of gravity shifts. Your back feels a sense of relief, and it takes you a moment to realize your " + liveData.player.breastDescript(k) + " have shrunk!")
                            else GUI.outputText(" Your " + UTIL.num2Text2(k + 1) + " row of " + liveData.player.breastDescript(k) + " gives a tiny jiggle as it shrinks, losing some off its mass.")
                        }
        ...
        if (
            ...
            if (!liveData.player.hasSheath()) {
                ...
                if (liveData.player.balls > 0) GUI.outputText(" Thankfully, your balls appear untouched.")
            } else GUI.outputText("Then, it disappears back into your sheath.")
        ...
        if (liveData.player.countCocksOfType(ENUM.CockType.CAT) > 0 && UTIL.rand(3) == 0 && changes < changeLimit && !liveData.hyperHappy) {
            ...
            if (temp == 1) {
                ...
                if (liveData.player.cocks[j].cockLength > 16) {
                    GUI.outputText(
                        "<br><br>Your " + liveData.player.cockDescript(j) + " tingles, making your sheath feel a little less tight. It dwindles in size, losing a full third of its length and a bit of girth before the change finally stops."
                    )
                ...
                if (liveData.player.cocks[j].cockLength / 5 < liveData.player.cocks[j].cockThickness && liveData.player.cocks[j].cockThickness > 1.25) liveData.player.cocks[j].cockThickness = liveData.player.cocks[j].cockLength / 6
                //Check for any more!
                temp2 = 0
                j++
                for (j; j < liveData.player.cocks.length; j++) {
                    ...
                    if (liveData.player.cocks[j].cockType == ENUM.CockType.CAT) {
                        ...
                        if (liveData.player.cocks[j].cockLength > 6) {
                            if (liveData.player.cocks[j].cockLength > 16) liveData.player.cocks[j].cockLength *= 0.66
                            else if (liveData.player.cocks[j].cockLength > 6) liveData.player.cocks[j].cockLength -= 2
                ...
                if (temp2 == 1) GUI.outputText(" Upon further inspection, all your " + Appearance.cockNoun(ENUM.CockType.CAT) + "s have shrunk!")
                changes++
            }
        }
        //------------
        ...
        if (liveData.player.earType != ENUM.EarType.EARS_CAT && UTIL.rand(5) == 0 && changes < changeLimit) {
            ...
            if (liveData.player.earType == ENUM.EarType.EARS_HUMAN) {
                if (UTIL.rand(2) == 0)
                    ...
                    GUI.outputText(
                        "<br><br>Your ears begin to tingle. You reach up with one hand and gently rub them. They appear to be growing fur. Within a few moments, they've migrated up to the top of your head and increased in size. The tingling stops and you find yourself hearing noises in a whole new way. <b>You now have cat ears.</b>"
                    )
            }
            //non human to cat:
            else {
                if (UTIL.rand(2) == 0) GUI.outputText("<br><br>Your ears change shape, morphing into pointed, feline ears! They swivel about reflexively as you adjust to them. <b>You now have cat ears.</b>")
                else
                    GUI.outputText(
        ...
        if (liveData.player.tailType != ENUM.TailType.TAIL_TYPE_CAT && liveData.player.earType == ENUM.EarType.EARS_CAT && UTIL.rand(5) == 0 && changes < changeLimit) {
            if (liveData.player.tailType == ENUM.TailType.TAIL_TYPE_NONE) {
                ...
                if (temp == 0)
                    GUI.outputText(
                        "<br><br>A pressure builds in your backside. You feel under your " +
                            liveData.player.armor.equipmentName +
                ...
                if (temp == 1) GUI.outputText("<br><br>You feel your backside shift and change, flesh molding and displacing into a long, flexible tail! <b>You now have a cat tail.</b>")
                if (temp == 2)
                    GUI.outputText(
                        "<br><br>You feel an odd tingling in your spine and your tail bone starts to throb and then swell. Within a few moments it begins to grow, adding new bones to your spine. Before you know it, you have a tail. Just before you think it's over, the tail begins to sprout soft, glossy " +
                            liveData.player.furColor +
        ...
        if (liveData.player.tailType == ENUM.TailType.TAIL_TYPE_CAT && liveData.player.earType == ENUM.EarType.EARS_CAT && UTIL.rand(5) == 0 && changes < changeLimit && liveData.player.lowerBody != ENUM.LowerBodyType.LOWER_BODY_TYPE_CAT) {
            ...
            if (liveData.player.lowerBody == ENUM.LowerBodyType.LOWER_BODY_TYPE_HOOFED) {
                ...
                if (liveData.player.isTaur()) GUI.outputText(" You feel woozy and collapse on your side. When you wake, you're no longer a centaur and your body has returned to a humanoid shape.")
            }
            //Goo to cat
            ...
            //non hoof to cat:
            else GUI.outputText("<br><br>You scream in agony as you feel the bones in your " + liveData.player.feet() + " break and begin to rearrange. When the pain fades, you feel surprisingly well-balanced. <b>You now have cat paws.</b>")
        ...
        if (
            ...
            GUI.outputText(
                "You reach down to scratch your arm absent-mindedly and pull your fingers away to find strands of " +
                    liveData.player.furColor +
        ...
        if (
            ...
            if (temp == 0)
                GUI.outputText(
                    "<br><br>Your face is wracked with pain. You throw back your head and scream in agony as you feel your cheekbones breaking and shifting, reforming into something... different. You find a puddle to view your reflection and discover <b>your face is now a cross between human and feline features.</b>"
                )
            else if (temp == 1)
                GUI.outputText(
                    "<br><br>Mind-numbing pain courses through you as you feel your facial bones rearranging. You clutch at your face in agony as your skin crawls and shifts, your visage reshaping to replace your facial characteristics with those of a feline. <b>You now have an anthropomorphic cat-face.</b>"
                )
            else
                GUI.outputText(
        ...
        if (changes < changeLimit) {
            if (UTIL.rand(2) == 0) GUI.outputText(liveData.player.modThickness(5, 2))
            if (UTIL.rand(2) == 0) GUI.outputText(liveData.player.modTone(76, 2))
            if (liveData.player.gender < 2) {
                if (UTIL.rand(2) == 0) GUI.outputText(liveData.player.modFem(65, 1))
                else GUI.outputText(liveData.player.modFem(85, 2))
            }
        }
        liveData.gameFlags[FLAG.TIMES_TRANSFORMED] += changes
    }

    static goblinTFs(): void {
        let changes = 0
        let changeLimit = 1
        if (UTIL.rand(2) == 0) changeLimit++
        if (UTIL.rand(3) == 0) changeLimit++
        ...
        if (liveData.player.str > 50) {
            liveData.player.modStats(["str", -1])
            if (liveData.player.str > 70) liveData.player.modStats(["str", -1])
            if (liveData.player.str > 90) liveData.player.modStats(["str", -2])
        ...
        if (liveData.player.tou > 50) {
            ...
            if (liveData.player.tou > 70) liveData.player.modStats(["tou", -1])
            if (liveData.player.tou > 90) liveData.player.modStats(["tou", -2])
        }
        //Speed boost
        if (UTIL.rand(3) == 0 && liveData.player.spe < 50 && changes < changeLimit) {
            liveData.player.modStats(["spe", 1 + UTIL.rand(2)])
            GUI.outputText(
                "<br><br>You feel like dancing, and stumble as your legs react more quickly than you'd think. Is the alcohol slowing you down or are you really faster? You take a step and nearly faceplant as you go off balance. It's definitely both."
            )
            changes++
        }
        //------------
        ...
        if (liveData.player.skinTone != "green" && liveData.player.skinTone != "grayish-blue" && liveData.player.skinTone != "dark green" && liveData.player.skinTone != "pale yellow" && changes < changeLimit && UTIL.rand(2) == 0) {
            if (UTIL.rand(10) != 0) liveData.player.skinTone = "dark green"
            else {
                if (UTIL.rand(2) == 0) liveData.player.skinTone = "pale yellow"
                else liveData.player.skinTone = "grayish-blue"
            }
            changes++
        ...
        if (changes < changeLimit && UTIL.rand(4) == 0 && ((liveData.player.ass.analWetness > 0 && liveData.player.findPerk(PerkLib.MaraesGiftButtslut) < 0) || liveData.player.ass.analWetness > 1)) {
            ...
            if (liveData.player.ass.analLooseness > 1) liveData.player.ass.analLooseness--
            changes++
        }
        if (changes < changeLimit && UTIL.rand(3) == 0) {
            if (UTIL.rand(2) == 0) liveData.player.modFem(85, 3)
            if (UTIL.rand(2) == 0) liveData.player.modThickness(20, 3)
            if (UTIL.rand(2) == 0) liveData.player.modTone(15, 5)
        }
        liveData.player.refillHunger(15)
    }

    static humanTFs(): void {
        let changes = 0
        let changeLimit = 1
        if (UTIL.rand(2) == 0) changeLimit++
        if (UTIL.rand(2) == 0) changeLimit++
        ...
        if (liveData.player.humanScore() > 4) {
            ...
            GUI.outputText(
                "<br><br>You cry out as the world spins around you. You're aware of your entire body sliding and slipping, changing and morphing, but in the sea of sensation you have no idea exactly what's changing. You nearly black out, and then it's over. Maybe you had best have a look at yourself and see what changed?"
            )
        }
        //------------
        // MAJOR TFs
        ...
        if (liveData.player.skinTone != "tan" && liveData.player.skinTone != "olive" && liveData.player.skinTone != "dark" && liveData.player.skinTone != "light" && changes < changeLimit && UTIL.rand(5) == 0) {
            ...
            if (liveData.player.skinType == ENUM.SkinType.SKIN_TYPE_FUR) GUI.outputText("the skin under your " + liveData.player.furColor + " " + liveData.player.skinDesc)
            else GUI.outputText("your " + liveData.player.skinDesc)
            GUI.outputText(" has changed to become ")
            let temp = UTIL.rand(4)
            if (temp == 0) liveData.player.skinTone = "tan"
            else if (temp == 1) liveData.player.skinTone = "olive"
            else if (temp == 2) liveData.player.skinTone = "dark"
            else if (temp == 3) liveData.player.skinTone = "light"
        ...
        if (liveData.player.wingType > ENUM.WingType.WING_TYPE_NONE && UTIL.rand(5) == 0 && changes < changeLimit) {
            ...
            else GUI.outputText("<br><br>A wave of tightness spreads through your back, and it feels as if someone is stabbing a dagger into each of your shoulder-blades. After a moment the pain passes, though your wings are gone!")
            liveData.player.wingType = ENUM.WingType.WING_TYPE_NONE
            changes++
        }
        //Removes tail
        ...
        if (UTIL.rand(2) == 0 && changes < changeLimit && liveData.player.tallness < 60) {
            ...
            //Slow rate of growth near ceiling
            if (liveData.player.tallness > 90) temp = Math.floor(temp / 2)
            //Never 0
            if (temp == 0) temp = 1
            //Flavor texts.  Flavored like 1950's cigarettes. Yum.
            if (temp < 5) GUI.outputText("<br><br>You shift uncomfortably as you realize you feel off balance. Gazing down, you realize you have grown SLIGHTLY taller.")
        ...
        if (liveData.player.cocks.length > 1 && UTIL.rand(3) == 0 && changes < changeLimit) {
            liveData.player.removeCock(1)
            GUI.outputText(
                "<br><br>You have a strange feeling as your crotch tingles. " +
        ...
        if (liveData.player.balls > 2 && UTIL.rand(3) == 0 && changes < changeLimit) {
            if (liveData.player.ballSize > 2) {
                if (liveData.player.ballSize > 5) liveData.player.ballSize -= 1 + UTIL.rand(3)
                liveData.player.ballSize -= 1
        ...
        if (liveData.player.hasCock() && changes < changeLimit) {
            if (UTIL.rand(3) == 0 && liveData.player.cocks[0].cockType != ENUM.CockType.HUMAN) {
                GUI.outputText(
                    "<br><br>A strange tingling begins behind your " +
                        liveData.player.cockDescript(0) +
        ...
        if (liveData.player.hasCock() && liveData.player.biggestCockLength() > 12 && UTIL.rand(3) == 0 && changes < changeLimit) {
            ...
            if (liveData.player.cocks.length == 1) GUI.outputText("<br><br>You feel a tingling sensation as your cock shrinks to a smaller size!")
            else GUI.outputText("<br><br>You feel a tingling sensation as the largest of your cocks shrinks to a smaller size!")
        ...
        if (liveData.player.hasVagina() && UTIL.rand(3) == 0 && changes < changeLimit) {
            ...
            //High fertility:
            if (liveData.player.fertility >= 30) GUI.outputText("It feels like your overcharged reproductive organs have simmered down a bit.")
            //Average fertility:
            else if (liveData.player.fertility >= 10) GUI.outputText("You feel like you have dried up a bit inside; you are left feeling oddly tranquil.")
            //[Low/No fertility:
            else {
                GUI.outputText("Although the numbness makes you feel serene, the hummus has no effect upon your ")
                if (liveData.player.fertility > 0) GUI.outputText("mostly ")
            ...
            if (liveData.player.fertility < 10 && liveData.player.gender >= 2) liveData.player.fertility = 10
            if (liveData.player.fertility < 5) liveData.player.fertility = 5
            changes++
        }
        ...
        if (liveData.player.hasCock() && liveData.player.cumMultiplier > 5 && UTIL.rand(3) == 0 && changes < changeLimit) {
            ...
            if (liveData.player.cumMultiplier < 1) liveData.player.cumMultiplier = 1
            changes++
        }
        liveData.gameFlags[FLAG.TIMES_TRANSFORMED] += changes
    }
    ...
    static pigTFs(boar: boolean): void {
        let changes = 0
        let changeLimit = 1
        let temp = 0
        let x = 0
        ...
        if (UTIL.rand(3) == 0 && changes < changeLimit && liveData.player.balls > 0 && liveData.player.ballSize < 4) {
            if (liveData.player.ballSize < 3)
                ...
                GUI.outputText(
                    "<br><br>A sudden onset of heat envelops your groin, focusing on your ballsack. Walking becomes difficult as you discover your " + (liveData.player.balls == 4 ? "quartette" : "duo") + " of testicles have enlarged again."
                )
            liveData.player.ballSize++
            changes++
        }
        ...
        if (UTIL.rand(4) == 0 && changes < changeLimit && liveData.player.cocks.length > 0 && liveData.player.cocks[0].cockType != ENUM.CockType.PIG) {
            if (liveData.player.cocks.length == 1) {
                //Single cock
                GUI.outputText(
                    "<br><br>You feel an uncomfortable pinching sensation in your [cock]. " +
                ...
                //Multiple cocks
                GUI.outputText(
                    "<br><br>You feel an uncomfortable pinching sensation in one of your cocks. You pull open your [armor], watching as it warps and changes. As the transformation completes, you're left with a shiny pinkish red pecker ending in a prominent corkscrew at the tip. <b>You now have a pig penis!</b>"
                )
        ...
        if (UTIL.rand(boar ? 2 : 3) == 0 && changes < changeLimit && liveData.player.earType == ENUM.EarType.EARS_PIG && liveData.player.tailType != ENUM.TailType.TAIL_TYPE_PIG) {
            ...
            else GUI.outputText("<br><br>You feel a tug at the base of your spine as it lengthens ever so slightly. Looking over your shoulder, <b>you find that you have sprouted a small, curly pig tail.</b>")
        ...
        if (
            ...
            //Bipedal
            else GUI.outputText("<br><br>You scream in agony as the bones in your legs break and rearrange. Once the pain subsides, you inspect your legs, finding that they are digitigrade and ending in cloven hooves. <b>You now have pig legs!</b>")
            liveData.player.lowerBody = ENUM.LowerBodyType.LOWER_BODY_TYPE_CLOVEN_HOOFED
            liveData.player.legCount = 2
            changes++
        }
    ...
    static lizardTFs(): void {
        ...
        if (liveData.player.lib < 100 && changes < changeLimit && UTIL.rand(3) == 0) {
            ...
            if (liveData.player.cocks.length > 0 && (liveData.player.gender != 3 || UTIL.rand(2) == 0)) {
                ...
                if (liveData.player.cocks.length > 1) GUI.outputText("each of ")
            ...
            //(COOCH)
            else if (liveData.player.hasVagina()) GUI.outputText("puddling in your " + liveData.player.vaginaDescript(0) + ".  An instinctive desire to mate and lay eggs spreads through you, increasing your lust and boosting your sex-drive.")
            //(TARDS)
            else
                GUI.outputText(
            ...
            if (liveData.player.lib < 50) liveData.player.dynStats(["lib", 3])
            //+2 lib if less than 75
            if (liveData.player.lib < 75) liveData.player.dynStats(["lib", 2])
            //+1 if above 75.
        ...
        if (liveData.player.countCocksOfType(ENUM.CockType.LIZARD) == 0 && liveData.player.cockTotal() > 0 && changes < changeLimit && UTIL.rand(4) == 0) {
            ...
            for (temp2 = 0; temp2 < liveData.player.cocks.length; temp2++) {
                ...
                if (liveData.player.cocks[temp2].cockType != ENUM.CockType.LIZARD) break
            }
            GUI.outputText(
                "<br><br>A slow tingle warms your groin.  Before it can progress any further, you yank back your " +
                    liveData.player.armor.equipmentName +
                    " to investigate.  Your " +
            ...
            if (liveData.player.cor < 33) GUI.outputText("horrifies you.")
            else if (liveData.player.cor < 66) GUI.outputText("is a little strange for your tastes.")
            else {
                GUI.outputText("looks like it might be more fun to receive than use on others.  ")
                if (liveData.player.hasVagina()) GUI.outputText("Maybe you could find someone else with one to ride?")
                else GUI.outputText("Maybe you should test it out on someone and ask them exactly how it feels?")
            }
            ...
            if (liveData.player.hasSheath()) {
                ...
                if (!liveData.player.hasSheath())
                    GUI.outputText(
                        "<br><br>Your sheath tightens and starts to smooth out, revealing ever greater amounts of your " +
                            liveData.player.cockDescript(temp2) +
                            "'s lower portions.  After a few moments <b>your groin is no longer so animalistic - the sheath is gone.</b>"
                    )
            } else liveData.player.cocks[temp2].cockType = ENUM.CockType.LIZARD
            changes++
        ...
        if (liveData.player.cockTotal() > 1 && liveData.player.countCocksOfType(ENUM.CockType.LIZARD) > 0 && liveData.player.cockTotal() > liveData.player.countCocksOfType(ENUM.CockType.LIZARD) && UTIL.rand(4) == 0 && changes < changeLimit) {
            ...
            if (liveData.player.cockTotal() == 2) GUI.outputText("your other dick")
            else GUI.outputText("another one of your dicks")
            ...
            if (liveData.player.cumQ() < 50) GUI.outputText("pre-cum oozes from the tip")
            else if (liveData.player.cumQ() < 700) GUI.outputText("Thick pre-cum rains from the tip")
            else GUI.outputText("A wave of pre-cum splatters on the ground")
        ...
        if (liveData.player.countCocksOfType(ENUM.CockType.LIZARD) == 1 && liveData.player.cocks.length == 1 && UTIL.rand(4) == 0 && changes < changeLimit) {
            GUI.outputText(
                "<br><br>A knot of pressure forms in your groin, forcing you off your " +
            ...
            if (liveData.player.skinType == ENUM.SkinType.SKIN_TYPE_FUR || liveData.player.skinType == ENUM.SkinType.SKIN_TYPE_SCALES) GUI.outputText(" and shedding " + liveData.player.skinDesc)
            GUI.outputText(
        ...
        if (liveData.player.biggestTitSize() >= 1 && liveData.player.gender == 1 && changes < changeLimit && UTIL.rand(3) == 0) {
            ...
            if (liveData.player.biggestTitSize() > 8) {
                GUI.outputText(
                    "<br><br>The flesh on your chest tightens up, losing nearly half its mass in the span of a few seconds.  With your center of balance shifted so suddenly, you stagger about trying not to fall on your ass.  You catch yourself and marvel at the massive change in breast size."
                )
                //Half tit size
            }
            //(NOT HUEG < 4)
            else GUI.outputText("<br><br>In an instant, your chest compacts in on itself, consuming every ounce of breast-flesh.  You're left with a  smooth, masculine torso, though your nipples remain.")
            //(BOTH – no new PG)
            ...
            for (temp2 = 0; temp2 < liveData.player.breastRows.length; temp2++) {
                if (liveData.player.breastRows[temp2].breastRating > 8) liveData.player.breastRows[temp2].breastRating /= 2
                else liveData.player.breastRows[temp2].breastRating = 0
            }
            //(+2 speed)
        ...
        if (liveData.player.biggestLactation() >= 1 && changes < changeLimit && UTIL.rand(4) == 0) {
            if (liveData.player.totalNipples() == 2) GUI.outputText("<br><br>Both of your")
            else GUI.outputText("<br><br>All of your many")
            ...
            if (liveData.player.hasFuckableNipples()) GUI.outputText("but sexual fluid ")
            ...
            //ALREADY DRAGON
                ...
                if (liveData.player.hornType == ENUM.HornType.HORNS_DRACONIC_X2) {
                    if (liveData.player.horns < 12) {
                        ...
                        if (liveData.player.horns >= 12) GUI.outputText("  <b>Your horns settle down quickly, as if they're reached their full size.</b>")
                        changes++
                    }
                    //maxxed out, new row
                    else {
        ...
        if (liveData.player.lowerBody != ENUM.LowerBodyType.LOWER_BODY_TYPE_LIZARD && changes < changeLimit && UTIL.rand(5) == 0) {
            ...
            //TAURS -
            else if (liveData.player.isTaur()) GUI.outputText("<br><br>Your lower body is wracked by pain!  Once it passes, you discover that you're standing on digitigrade legs with lizard-like claws.")
            //feet types -
            else if (
                liveData.player.lowerBody == ENUM.LowerBodyType.LOWER_BODY_TYPE_HUMAN ||
                liveData.player.lowerBody == ENUM.LowerBodyType.LOWER_BODY_TYPE_DOG ||
        ...
        if (
            ...
            if (liveData.player.skinType == ENUM.SkinType.SKIN_TYPE_FUR) {
                ...
                if (UTIL.rand(10) == 0) {
                    ...
                    else liveData.player.skinTone = "silver"
                }
                //non rare skinTone
                else {
                    let temp = UTIL.rand(5)
                    if (temp == 0) liveData.player.skinTone = "red"
                    else if (temp == 1) liveData.player.skinTone = "green"
                    else if (temp == 2) liveData.player.skinTone = "white"
                    else if (temp == 3) liveData.player.skinTone = "blue"
                    else liveData.player.skinTone = "black"
                }
                GUI.outputText(
                    "<br><br>You scratch yourself, and come away with a large clump of " +
                        liveData.player.furColor +
            ...
            //(no fur)
                ...
                GUI.outputText(
                    "<br><br>You idly reach back to scratch yourself and nearly jump out of your " +
                        liveData.player.armor.equipmentName +
        ...
        if (
            ...
            GUI.outputText(
                "<br><br>Terrible agony wracks your " +
                    liveData.player.face() +
    ...
    static harpyTFs(type: number): void {
        ...
        if (liveData.player.spe < 100 && UTIL.rand(3) == 0) {
            changes++
            if (liveData.player.spe >= 75) GUI.outputText("<br><br>A familiar chill runs down your spine. Your muscles feel like well oiled machinery, ready to snap into action with lightning speed.")
            else GUI.outputText("<br><br>A chill runs through your spine, leaving you feeling like your reflexes are quicker and your body faster.")
            //Speed gains diminish as it rises.
            if (liveData.player.spe < 40) liveData.player.dynStats(["spe", 0.5])
            if (liveData.player.spe < 75) liveData.player.dynStats(["spe", 0.5])
            liveData.player.dynStats(["spe", 0.5])
        }
        //-Toughness decrease to 50
        if (liveData.player.tou > 50 && UTIL.rand(3) == 0 && changes < changeLimit) {
            changes++
            if (UTIL.rand(2) == 0) GUI.outputText("<br><br>A nice, slow warmth rolls from your gut out to your limbs, flowing through them before dissipating entirely. As it leaves, you note that your body feels softer and less resilient.")
            else GUI.outputText("<br><br>You feel somewhat lighter, but consequently more fragile.  Perhaps your bones have changed to be more harpy-like in structure?")
            liveData.player.dynStats(["tou", -1])
        }
        //antianemone corollary:
        if (changes < changeLimit && liveData.player.hairType == 4 && UTIL.rand(2) == 0) {
            //-insert anemone hair removal into them under whatever criteria you like, though hair removal should precede abdomen growth; here's some sample text:
            GUI.outputText(
                "<br><br>As you down the seed, your head begins to feel heavier.  Reaching up, you notice your tentacles becoming soft and somewhat fibrous.  Pulling one down reveals that it feels soft and fluffy, almost feathery; you watch as it dissolves into many thin, feathery strands.  <b>Your hair is now like that of a harpy!</b>"
            )
        ...
        if (liveData.player.str < 70 && UTIL.rand(3) == 0 && changes < changeLimit) {
            changes++
            //(low str)
            if (liveData.player.str < 40) GUI.outputText("<br><br>Shivering, you feel a feverish sensation that reminds you of the last time you got sick. Thankfully, it passes swiftly, leaving slightly enhanced strength in its wake.")
            //(hi str – 50+)
            else GUI.outputText("<br><br>Heat builds in your muscles, their already-potent mass shifting slightly as they gain even more strength.")
            //Faster until 40 str.
            if (liveData.player.str < 40) liveData.player.dynStats(["str", 0.5])
        ...
        if ((liveData.player.lib < 90 || UTIL.rand(3) == 0) && UTIL.rand(3) == 0 && changes < changeLimit) {
            changes++
            if (liveData.player.lib < 90) liveData.player.dynStats(["lib", 1])
            //(sub 40 lib)
            if (liveData.player.lib < 40) {
                ...
                if (liveData.player.hasVagina()) GUI.outputText(" The moistness of your " + liveData.player.vaginaDescript() + " seems to agree.")
                else if (liveData.player.hasCock()) GUI.outputText(" The hardness of " + liveData.player.sMultiCockDesc() + " seems to agree.")
                liveData.player.changeLust(5)
            }
            //(sub 75 lib)
            else if (liveData.player.lib < 75) GUI.outputText("<br><br>Heat, blessed heat, works through you from head to groin, leaving you to shudder and fantasize about the sex you could be having right now.<br><br>")
            //(hi lib)
            else if (liveData.player.lib < 90) GUI.outputText("<br><br>Sexual need courses through you, flushing your skin with a reddish hue while you pant and daydream of the wondrous sex you should be having right now.<br><br>")
            //(90+)
            else GUI.outputText("<br><br>You groan, something about the seed rubbing your libido in just the right way to make you horny. Panting heavily, you sigh and fantasize about the sex you could be having.<br><br>")
            //(fork to fantasy)
            if (liveData.player.lib >= 40) {
                ...
                if (liveData.player.hasCock() && (liveData.player.gender != 3 || UTIL.rand(2) == 0)) {
                    ...
                    if (UTIL.rand(2) == 0) {
                        ...
                        if (liveData.player.cockTotal() > 1) {
                            ...
                            if (liveData.player.cockTotal() > 2) GUI.outputText("es rub ")
                            else GUI.outputText("rubs ")
                ...
                //Cunts!
                    ...
                    if (UTIL.rand(2) == 0) {
                        ...
                        if (liveData.player.hasFuckableNipples()) GUI.outputText("and pussy leaking over ")
                        else if (liveData.player.biggestLactation() >= 1.5) GUI.outputText("dripping milk inside ")
                        else GUI.outputText("rubbing inside ")
        ...
        if (liveData.player.gender == 0 || (!liveData.player.hasVagina() && changes < changeLimit && UTIL.rand(3) == 0)) {
            ...
            if (liveData.player.balls > 0)
                GUI.outputText(
                    "<br><br>An itch starts behind your " +
                        liveData.player.ballsDescriptLight() +
            ...
            //(dick)
                ...
                GUI.outputText(
                    "<br><br>An itch starts on your groin, just below your " +
                        liveData.player.multiCockDescriptLight() +
            ...
            //(neither)
                ...
                GUI.outputText(
                    "<br><br>An itch starts on your groin and fades before you can take action. Curious about the intermittent sensation, <b>you peek under your " +
                        liveData.player.armor.equipmentName +
        ...
        //Cannot happen at same time as row removal
            ...
            //(H+)
                ...
                GUI.outputText(
                    "<br><br>Your chest pinches tight, wobbling dangerously for a second before the huge swell of your bust begins to shrink into itself. The weighty mounds jiggle slightly as they shed cup sizes like old, discarded coats, not stopping until they're " +
        ...
        if (changes < changeLimit && liveData.player.breastRows[0].breastRating < 2 && UTIL.rand(3) == 0) {
            changes++
            GUI.outputText(
                "<br><br>Your chest starts to tingle, the " +
                    liveData.player.skinDesc +
                    " warming under your " +
                    liveData.player.armor.equipmentName +
            ...
            if (liveData.player.breastRows[0].breastRating < 1) GUI.outputText("  <b>You have breasts now!</b>")
            liveData.player.breastRows[0].breastRating = 2
        }
        ...
        if (changes < changeLimit && liveData.player.hasCock() && liveData.player.countCocksOfType(ENUM.CockType.AVIAN) < liveData.player.cockTotal() && UTIL.rand(type == 1 ? 4 : 10) == 0) {
            ...
            changes++
            GUI.outputText(
                "<br><br>You feel a strange tingling sensation in your cock as erection forms. You " +
        ...
        if (liveData.player.skinTone != "tan" && liveData.player.skinTone != "olive" && liveData.player.skinTone != "dark" && liveData.player.skinTone != "light" && changes < changeLimit && UTIL.rand(5) == 0) {
            changes++
            GUI.outputText("<br><br>It takes a while for you to notice, but <b>")
            if (liveData.player.skinType == ENUM.SkinType.SKIN_TYPE_FUR) GUI.outputText("the skin under your " + liveData.player.hairColor + " " + liveData.player.skinDesc)
            else GUI.outputText("your " + liveData.player.skinDesc)
            GUI.outputText(" has changed to become ")
            let temp = UTIL.rand(4)
            if (temp == 0) liveData.player.skinTone = "tan"
        ...
        if (liveData.player.buttRating < 8 && changes < changeLimit && UTIL.rand(3) == 0) {
            ...
            changes++
            GUI.outputText(
                "<br><br>A slight jiggle works through your rear, but instead of stopping it starts again. You can actually feel your " +
                    liveData.player.armor.equipmentName +
        ...
        if (liveData.player.buttRating >= 14 && changes < changeLimit && UTIL.rand(4) == 0) {
            ...
            liveData.player.buttRating--
            GUI.outputText(
                "<br><br>A feeling of tightness starts in your " +
                    liveData.player.buttDescript() +
        ...
        if (liveData.player.tailType != ENUM.TailType.TAIL_TYPE_HARPY && changes < changeLimit && (type == 1 || liveData.player.wingType == ENUM.WingType.WING_TYPE_FEATHERED_LARGE) && UTIL.rand(4) == 0) {
            ...
            //(no tail)
                ...
                GUI.outputText(
                    "<br><br>A tingling tickles the base of your spine, making you squirm in place. A moment later, it fades, but a fan of feathers erupts from your " +
                        liveData.player.skinDesc +
        ...
        if (liveData.player.wingType == ENUM.WingType.WING_TYPE_NONE && changes < changeLimit && (type == 1 || liveData.player.armType == ENUM.ArmType.ARM_TYPE_HARPY) && UTIL.rand(4) == 0) {
            GUI.outputText(
                "<br><br>Pain lances through your back, the muscles knotting oddly and pressing up to bulge your " +
                    liveData.player.skinDesc +
        ...
        if (changes == 0) GUI.outputText("<br><br>Aside from being a tasty treat, it doesn't seem to do anything to you this time.")
        liveData.player.refillHunger(10)
        liveData.gameFlags[FLAG.TIMES_TRANSFORMED] += changes
    }

    //Minoblood only. Minotaur Cum is in consumableEffects.js
    static minotaurTFs(): void {
        liveData.player.slimeFeed()
        //Changes done
        let changes = 0
        //Change limit
        ...
        if (UTIL.rand(3) == 0 && changes < changeLimit) {
            ...
            if (liveData.player.str <= 50) {
                ...
                if (liveData.player.str <= 20) liveData.player.dynStats(["str", 3])
                else liveData.player.dynStats(["str", 2])
            }
            //stronger characters gain less
            else {
                //small growth if over 75
                if (liveData.player.str >= 75) liveData.player.dynStats(["str", 0.5])
                //faster from 50-75
                else liveData.player.dynStats(["str", 1])
        ...
        if (UTIL.rand(3) == 0 && changes < changeLimit) {
            ...
            if (liveData.player.tou <= 50) {
                ...
                //very weak players gain more
                if (liveData.player.tou <= 20) liveData.player.dynStats(["tou", 3])
                else liveData.player.dynStats(["tou", 2])
            }
            //stronger characters gain less
            else {
                //small growth if over 75
                if (liveData.player.tou >= 75) liveData.player.dynStats(["tou", 0.5])
                //faster from 50-75
                else liveData.player.dynStats(["tou", 1])
        ...
        if (UTIL.rand(4) == 0) this.restoreArms(tfSource)
        //+hooves
        if (liveData.player.lowerBody != ENUM.LowerBodyType.LOWER_BODY_TYPE_HOOFED) {
            if (changes < changeLimit && UTIL.rand(3) == 0) {
                changes++
                ...
                if (liveData.player.lowerBody > ENUM.LowerBodyType.LOWER_BODY_TYPE_NAGA)
                    GUI.outputText("<br><br>You stagger as your " + liveData.player.feet() + " change, curling up into painful angry lumps of flesh.  They get tighter and tighter, harder and harder, until at last they solidify into hooves!")
                if (liveData.player.skinType != ENUM.SkinType.SKIN_TYPE_FUR) GUI.outputText("  A fine coat of fur grows out below your waist, itching briefly as it fills in.")
            ...
            //Shrink boobages till they are normal
                ...
                if (liveData.player.breastRows.length == 1) {
                    ...
                    if (liveData.player.breastRows[0].breastRating >= 1) {
                        ...
                        //Talk about shrinkage
                        if (temp == 1) GUI.outputText("<br><br>You feel a weight lifted from you, and realize your " + liveData.player.breastDescript(0) + " have shrunk to " + liveData.player.breastCup(0) + "s.")
                        if (temp == 2) GUI.outputText("<br><br>You feel significantly lighter.  Looking down, you realize your breasts are MUCH smaller, down to " + liveData.player.breastCup(0) + "s.")
                        changes++
                    }
                }
                //multiple
                    ...
                    temp2 = 0
                    temp3 = 0
                    if (liveData.player.biggestTitSize() >= 1) GUI.outputText("<br>")
                    while (temp3 < liveData.player.breastRows.length) {
                        if (liveData.player.breastRows[temp3].breastRating >= 1) {
                            ...
                            if (temp2 > 1) GUI.outputText("...and y")
                            else GUI.outputText("Y")
                            GUI.outputText("our " + liveData.player.breastDescript(temp3) + " shrink, dropping to " + liveData.player.breastCup(temp3) + "s.")
                        }
                        temp3++
        ...
        if (changes < changeLimit && UTIL.rand(2) == 0 && liveData.player.cocks.length > 0) {
            ...
            if (selectedCock != -1) {
                ...
                if (liveData.player.cocks[selectedCock].cockThickness < 5) {
                    ...
                    if (temp > 6) GUI.outputText("<br><br>Gasping in sudden pleasure, your " + liveData.player.cockDescript(selectedCock) + " surges free of its sheath, emerging with over half a foot of new dick-flesh.", false)
                    if (temp <= 6 && temp >= 3) GUI.outputText("<br><br>You pant in delight as a few inches of " + liveData.player.cockDescript(selectedCock) + " pop free from your sheath, the thick new horse-flesh still slick and sensitive.", false)
                    if (temp < 3) GUI.outputText("<br><br>Groaning softly, you feel a pleasurable change in your groin.  Looking down, you see [oneCock] grow slightly longer.")
                ...
                //Just length...
                    ...
                    temp = liveData.player.increaseCock(selectedCock, 2 + UTIL.rand(8))
                    //Comment on length changes
                    if (temp > 6) GUI.outputText("<br><br>Gasping in sudden pleasure, your " + liveData.player.cockDescript(selectedCock) + " surges free of its sheath, emerging with over half a foot of new dick-flesh.")
                    if (temp <= 6 && temp >= 3) GUI.outputText("<br><br>You pant in delight as a few inches of " + liveData.player.cockDescript(selectedCock) + " pop free from your sheath, the thick new horse-flesh still slick and sensitive.")
                    if (temp < 3) GUI.outputText("<br><br>Groaning softly, you feel a pleasurable change in your groin.  Looking down, you see [oneCock] grow slightly longer.")
                }
                changes++
            }
        ...
        if (liveData.player.cocks.length > 0 && UTIL.rand(2) == 0 && changes < changeLimit) {
            ...
            if (selectedCockValue != -1) {
                ...
                if (liveData.player.countCocksOfType(ENUM.CockType.HORSE) > 1) GUI.outputText("nother")
            ...
            if (liveData.player.hornType == ENUM.HornType.HORNS_COW_MINOTAUR || liveData.player.hornType == ENUM.HornType.HORNS_NONE) {
                ...
                if (liveData.player.hornType == ENUM.HornType.HORNS_COW_MINOTAUR) {
                    ...
                    //Males horns get 'uge.
                        ...
                        if (temp == 0) changes--
                        if (temp == 1) GUI.outputText("<br><br>An aching pressure builds in your temples as you feel your horns push another inch of length from your skull.  ")
                        if (temp == 2) GUI.outputText("<br><br>A powerful headache momentarily doubles you over.  With painful slowness, you feel your horns push another two inches of length out from your brow, gradually thickening as they grow.  ")
                        if (temp == 3)
                            GUI.outputText(
                                "<br><br>Agony overwhelms you as a headache of terrifying intensity sweeps through your skull.  You squeeze your eyes shut from the pain, but it does little to help.  The torture intensifies before finally diminishing as you feel an inch or two of new horn force its way out of your forehead.  The headache remains despite this, and desperate for relief, you grab hold of your horns and tug, pulling another inch of new horn free.  At last the pain fades, leaving you with significantly enhanced head-spikes.  "
                            )
                        if (liveData.player.horns < 3) GUI.outputText("They are the size of tiny nubs.")
                        if (liveData.player.horns >= 3 && liveData.player.horns < 6) GUI.outputText("They are similar to what you would see on a young bull.")
                        if (liveData.player.horns >= 6 && liveData.player.horns < 12) GUI.outputText("They look like the horns on a grown bull, big enough and dangerous enough to do some damage.")
                        if (liveData.player.horns >= 12 && liveData.player.horns < 20) GUI.outputText("They are large and wicked looking.")
                        if (liveData.player.horns >= 20) GUI.outputText("They are huge, heavy, and tipped with dangerous points.")
        ...
        if (UTIL.rand(4) == 0) GUI.outputText(liveData.player.modFem(5, 10))
        if (UTIL.rand(4) == 0) GUI.outputText(liveData.player.modTone(85, 3))
        if (UTIL.rand(4) == 0) GUI.outputText(liveData.player.modThickness(70, 4))
        //Default
        if (changes == 0) {
    ...
    static snakeTFs(): void {
        ...
        //(if outside combat)
        if (!COMBAT.inCombat()) GUI.outputText("  Minutes pass as you start wishing you had water with you, to get rid of the aftertaste.")
        //+ speed to 70!
        if (liveData.player.spe < 70 && UTIL.rand(2) == 0) {
            ...
            if (liveData.player.spe < 40) GUI.outputText("  Of course, you're nowhere near as fast as that.")
            changes++
        }
        ...
        if (changes == 0 && liveData.player.faceType == ENUM.FaceType.FACE_SNAKE_FANGS && liveData.player.lowerBody != ENUM.LowerBodyType.LOWER_BODY_TYPE_NAGA && UTIL.rand(4) == 0 && changes < changeLimit) {
            ...
            if (liveData.player.balls > 0 && liveData.player.ballSize > 10) GUI.outputText("  You're happy not to have to drag those testicles around with you anymore.")
            GUI.outputText(
        ...
        //Default change - blah
        if (changes == 0) GUI.outputText("<br><br>Remakarbly, the snake-oil has no effect.  Should you really be surprised at snake-oil NOT doing anything?")
        liveData.player.refillHunger(5)
    }

    static slimeTFs(): void {
        //Set up changes and changeLimit
        let changes = 0
        let changeLimit = 1
        ...
        if (liveData.player.wingType > ENUM.WingType.WING_TYPE_NONE) {
            ...
            else GUI.outputText("<br><br>You sigh, feeling a hot wet tingling down your back.  It tickles slightly as you feel your wings slowly turn to sludge, dripping to the ground as your body becomes more goo-like.")
            liveData.player.wingType = ENUM.WingType.WING_TYPE_NONE
        }
        //Goopy hair
        if (liveData.player.hairType != 3) {
            ...
            if (liveData.player.hairLength <= 0) {
                GUI.outputText(
                    "<br><br>Your head buzzes pleasantly, feeling suddenly hot and wet.  You instinctively reach up to feel the source of your wetness, and discover you've grown some kind of gooey hair.  From time to time it drips, running down your back to the crack of your " +
                        liveData.player.buttDescript() +
                        "."
                )
                ...
                if (liveData.player.hairColor.indexOf("rubbery") == -1 && liveData.player.hairColor.indexOf("latex-textured") == -1) {
                    GUI.outputText(
                        "<br><br>Your head buzzes pleasantly, feeling suddenly hot and wet.  You instinctively reach up to feel the source of your wetness, and discover your hair has become a slippery, gooey mess.  From time to time it drips, running down your back to the crack of your " +
                            liveData.player.buttDescript() +
                            "."
                    )
                }
            ...
            if (liveData.player.hairColor != "green" && liveData.player.hairColor != "purple" && liveData.player.hairColor != "blue" && liveData.player.hairColor != "cerulean" && liveData.player.hairColor != "emerald") {
                ...
                if (blah <= 2) liveData.player.hairColor = "green"
                else if (blah <= 4) liveData.player.hairColor = "purple"
                else if (blah <= 6) liveData.player.hairColor = "blue"
                else if (blah <= 8) liveData.player.hairColor = "cerulean"
                else liveData.player.hairColor = "emerald"
        ...
        if (liveData.player.hairType == 3 && (liveData.player.skinDesc != "skin" || liveData.player.skinAdj != "slimy")) {
            ...
            if (liveData.player.skinTone != "green" && liveData.player.skinTone != "purple" && liveData.player.skinTone != "blue" && liveData.player.skinTone != "cerulean" && liveData.player.skinTone != "emerald") {
                ...
                if (blaht <= 2) liveData.player.skinTone = "green"
                else if (blaht <= 4) liveData.player.skinTone = "purple"
                else if (blaht <= 6) liveData.player.skinTone = "blue"
                ...
                if (liveData.player.armType != ENUM.ArmType.ARM_TYPE_HUMAN || liveData.player.clawType != ENUM.ClawType.CLAW_TYPE_NORMAL) this.restoreArms(tfSource)
            }
        }
        ...
        if (UTIL.rand(2) == 0) GUI.outputText(liveData.player.modThickness(20, 3))
        if (UTIL.rand(2) == 0) GUI.outputText(liveData.player.modTone(15, 5))
    }
    ...
    static trapOil(): void {
        ...
        if (liveData.player.lib < 70 && liveData.player.hasVagina() && UTIL.rand(3) == 0 && changes < changeLimit) {
            ...
            liveData.player.dynStats(["lib", 2])
            if (liveData.player.lib < 30) liveData.player.dynStats(["lib", 2])
            changes++
        }
        //Body Mass Loss:
        if (liveData.player.thickness > 40 && UTIL.rand(3) == 0 && changes < changeLimit) {
            GUI.outputText(
                "<br><br>You feel an odd tightening sensation in your midriff, as if you were becoming narrower and lither.  You frown downwards, and then turn your arms around, examining them closely.  Is it just you or have you lost weight?"
            )
        ...
        if (liveData.player.hipRating >= 10 && UTIL.rand(4) == 0 && changes < changeLimit) {
            ...
            liveData.player.hipRating--
            if (liveData.player.hipRating > 15) liveData.player.hipRating -= 2 + UTIL.rand(3)
            changes++
        }
        ...
        if (liveData.player.biggestTitSize() > 1 && UTIL.rand(4) == 0 && changes < changeLimit) {
            GUI.outputText(
                "<br><br>You gasp as you feel a compressing sensation in your chest and around your [fullChest].  The feeling quickly fades however, leaving you feeling like you have lost a considerable amount of weight from your upper body."
            )
            let temp = 0
            while (temp < liveData.player.bRows()) {
                if (liveData.player.breastRows[temp].breastRating > 70) liveData.player.breastRows[temp].breastRating -= UTIL.rand(3) + 15
                else if (liveData.player.breastRows[temp].breastRating > 50) liveData.player.breastRows[temp].breastRating -= UTIL.rand(3) + 10
                else if (liveData.player.breastRows[temp].breastRating > 30) liveData.player.breastRows[temp].breastRating -= UTIL.rand(3) + 7
                else if (liveData.player.breastRows[temp].breastRating > 15) liveData.player.breastRows[temp].breastRating -= UTIL.rand(3) + 4
                else liveData.player.breastRows[temp].breastRating -= 2 + UTIL.rand(2)
                if (liveData.player.breastRows[temp].breastRating < 1) liveData.player.breastRows[temp].breastRating = 1
                temp++
            }
            changes++
        }
        ...
        if (liveData.player.biggestTitSize() < 1 || (liveData.player.breastRows[0].breastRating < 1 && UTIL.rand(4) == 0 && changes < changeLimit)) {
            GUI.outputText(
                "<br><br>You feel a vague swelling sensation in your [fullChest], and you frown downwards.  You seem to have gained a little weight on your chest.  Not enough to stand out, but- you cup yourself carefully- certainly giving you the faintest suggestion of boobs."
            )
        ...
        if (liveData.player.longestCockLength() >= 3.5 && liveData.player.hasCock() && UTIL.rand(2) == 0 && changes < changeLimit) {
            ...
            if (liveData.player.cockTotal() == 1) GUI.outputText("it is")
            else GUI.outputText("they are")
            ...
            if (liveData.player.cockTotal() == 1) GUI.outputText("it seems")
            else GUI.outputText("they seem")
            ...
            while (temp < liveData.player.cockTotal()) {
                if (liveData.player.cocks[temp].cockLength >= 3.5) {
                    ...
                    if (liveData.player.cocks[temp].cockLength < 6 && liveData.player.cocks[temp].cockLength >= 2.9) {
                        liveData.player.cocks[temp].cockLength -= 0.5
                        if (liveData.player.cocks[temp].cockThickness * 6 > liveData.player.cocks[temp].cockLength) liveData.player.cocks[temp].cockThickness -= 0.2
                        if (liveData.player.cocks[temp].cockThickness * 8 > liveData.player.cocks[temp].cockLength) liveData.player.cocks[temp].cockThickness -= 0.2
                        if (liveData.player.cocks[temp].cockThickness < 0.5) liveData.player.cocks[temp].cockThickness = 0.5
                    }
                    liveData.player.cocks[temp].cockLength -= 0.5
        ...
        if (liveData.player.balls > 0 && liveData.player.hasCock() && (liveData.player.ballSize > 1 || liveData.player.findStatusEffect(StatusEffects.Uniball) < 0) && UTIL.rand(4) == 0 && changes < changeLimit) {
            GUI.outputText(
                "<br><br>You feel a delicate tightening sensation around your [balls].  The sensation upon this most sensitive part of your anatomy isn't painful, but the feeling of your balls getting smaller is intense enough that you stifle anything more than a sharp intake of breath only with difficulty."
            )
            liveData.player.ballSize--
            if (liveData.player.ballSize > 8) liveData.player.ballSize--
            if (liveData.player.ballSize > 10) liveData.player.ballSize--
            if (liveData.player.ballSize > 12) liveData.player.ballSize--
        ...
        if (liveData.player.ass.analWetness < 5 && UTIL.rand(4) == 0 && changes < changeLimit) {
            if (liveData.player.ass.analWetness < 4) GUI.outputText("<br><br>Your eyes widen in shock as you feel oily moisture bead out of your [asshole].  Your asshole has become wetter and more pliable.")
            //Anal Wetness Increase Final (always loose):
            else
                GUI.outputText(
                    "<br><br>You moan as clear, odorless oil dribbles out of your [asshole], this time in enough quantity to stain your [armor].  Your back passage feels incredibly sensitive, wet and accommodating.  Your ass is ready to be plowed by anything, and always will be."
                )
            liveData.player.ass.analWetness++
            //buttChange(30,false,false,false);
            if (liveData.player.ass.analLooseness < 3) liveData.player.ass.analLooseness++
            changes++
            liveData.player.dynStats(["sen", 2])
        }
        //Fertility Decrease:
        if (liveData.player.hasVagina() && UTIL.rand(4) == 0 && changes < changeLimit) {
            ...
            if (liveData.player.fertility >= 30) GUI.outputText("It feels like your overcharged reproductive organs have simmered down a bit.")
            //Average fertility:
            else if (liveData.player.fertility >= 5) GUI.outputText("You feel like you have dried up a bit inside; you are left feeling oddly tranquil.")
            //[Low/No fertility:
            else {
            ...
            liveData.player.fertility -= 1 + UTIL.rand(3)
            if (liveData.player.fertility < 4) liveData.player.fertility = 4
            changes++
        }
        //Male Effects
        if (liveData.player.gender == 1) {
            ...
            if (UTIL.rand(4) == 0 && changes < changeLimit) {
                if (liveData.player.femininity < 70 && liveData.player.femininity >= 60) {
                    ...
                    if (liveData.player.femininity > 70) liveData.player.femininity = 70
                    changes++
                }
                //Femininity Increase:
                else {
        ...
        if (liveData.player.hasVagina() && liveData.player.vaginaType() != 5 && UTIL.rand(4) == 0 && changes < changeLimit) {
            GUI.outputText(
                "<br><br>Your [vagina] feels... odd.  You undo your clothes and gingerly inspect your nether regions.  The tender pink color of your sex has disappeared, replaced with smooth, marble blackness starting at your lips and working inwards."
            )
            //(Wet:
            if (liveData.player.wetness() >= 3) GUI.outputText("  Your natural lubrication makes it gleam invitingly.")
            //(Corruption <50:
            if (liveData.player.cor < 50) GUI.outputText("  After a few cautious touches you decide it doesn't feel any different- it does certainly look odd, though.")
            else GUI.outputText("  After a few cautious touches you decide it doesn't feel any different - the sheer bizarreness of it is a big turn on though, and you feel it beginning to shine with anticipation at the thought of using it.")
    ...
    */
    ...
    static updateClaws(clawType: ENUM.ClawType = ENUM.ClawType.CLAW_TYPE_NORMAL) {
        let clawTone = ""
    ...
    static restoreArms(tfSource: string) {
        //Set up changes and changeLimit
        let changes = 0
        let changeLimit = 1
