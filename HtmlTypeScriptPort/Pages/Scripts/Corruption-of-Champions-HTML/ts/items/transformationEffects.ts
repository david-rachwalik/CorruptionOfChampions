...
abstract class TransformationEffects {
    static beeTFs(type: number): void {
        ...
        if (changes < changeLimit && pure) {
            ...
            outputText("<br><br>")
            changes++
            if (liveData.player.cor > 80) outputText("Your head aches, as if thunder was echoing around your skull. ")
            else if (liveData.player.cor > 60) outputText("You feel a headache forming just behind your eyes. In no time flat it reaches full strength. ")
            else if (liveData.player.cor > 40) outputText("A wave of stinging pain slices through your skull. ")
            else if (liveData.player.cor > 20) outputText("A prickling pain spreads throughout your skull. ")
            else outputText("You feel a mildly unpleasant tingling inside your skull. ")
            if (liveData.player.cor > 0) outputText("It quickly passes, leaving you more clearheaded")
            ...
            if (liveData.player.cor > 0 && changes < changeLimit && UTIL.rand(1.5) == 0 && liveData.player.lib > 40) {
                ...
                changes++
            } else if (liveData.player.cor > 0) outputText(".")
        }
        //bee item corollary:
        ...
        if (changes < changeLimit && liveData.player.bRows() > 2 && UTIL.rand(3) == 0 && !liveData.hyperHappy) {
            ...
            if (liveData.player.bRows() >= 3) outputText("abdomen")
            else outputText("chest")
            outputText(". The " + liveData.player.nippleDescript(liveData.player.breastRows.length - 1) + "s even fade until nothing but ")
            if (liveData.player.skinType == ENUM.SkinType.SKIN_TYPE_FUR) outputText(liveData.player.hairColor + " " + liveData.player.skinDesc)
            else outputText(liveData.player.skinTone + " " + liveData.player.skinDesc)
        ...
        if (liveData.player.averageNipplesPerBreast() > 1 && changes < changeLimit && UTIL.rand(4) == 0) {
            ...
            if (liveData.player.biggestTitSize() < 1) outputText("'breast'.")
        ...
        if (special) {
            ...
            if (!liveData.player.hasCock()) {
                ...
                if (liveData.player.cocks[0].cArea() <= 20) outputText("It now swings as low as your knees!")
                else if (liveData.player.cocks[0].cArea() <= 50) outputText("While erect, your massive member fills the lower half of your vision.")
    ...
    static canineTFs(type: number): void {
        ...
        switch (type) {
            case 0: //Normal
                if (crit) outputText("The pepper tastes particularly potent, searingly hot and spicy.")
        ...
        if (type == 3) {
            ...
            if (liveData.player.cor < 50) outputText(" You shake your head, blushing hotly. Where did that thought come from?")
        }
        ...
        if (changes < changeLimit && UTIL.rand(5) == 0 && liveData.player.eyeType > ENUM.EyeType.EYES_HUMAN) {
            if (liveData.player.eyeType == ENUM.EyeType.EYES_BLACK_EYES_SAND_TRAP) {
                ...
                if (liveData.player.eyeType == ENUM.EyeType.EYES_FOUR_SPIDER_EYES) outputText(" Your multiple, arachnid eyes are gone!</b>")
        ...
        if (type == 4) {
            ...
            if (liveData.player.cockTotal() > 0) {
                ...
                if (liveData.player.countCocksOfType(ENUM.CockType.DOG) > 0) {
                    ...
                    outputText("<br><br>")
                    if (temp3 < 0.06) outputText("Your " + Appearance.cockNoun(ENUM.CockType.DOG) + " feels unusually tight in your sheath as your knot grows.")
                    if (temp3 >= 0.06 && temp3 <= 0.12) outputText("Your " + Appearance.cockNoun(ENUM.CockType.DOG) + " pops free of your sheath, thickening nicely into a bigger knot.")
                    if (temp3 > 0.12) outputText("Your " + Appearance.cockNoun(ENUM.CockType.DOG) + " surges free of your sheath, swelling thicker with each passing second. Your knot bulges out at the base, growing far beyond normal.")
        ...
        if (liveData.player.cocks.length > 0) {
            ...
            if (type != 4 && liveData.player.countCocksOfType(ENUM.CockType.DOG) > 0 && ((changes < changeLimit && UTIL.rand(1.4) == 0) || type == 1)) {
                ...
                if (liveData.player.cocks[temp2].knotMultiplier >= 2) temp3 /= 5
                liveData.player.cocks[temp2].knotMultiplier += temp3
                if (temp3 < 0.06) outputText("<br><br>Your " + liveData.player.cockDescript(temp2) + " feels unusually tight in your sheath as your knot grows.")
                if (temp3 >= 0.06 && temp3 <= 0.12) outputText("<br><br>Your " + liveData.player.cockDescript(temp2) + " pops free of your sheath, thickening nicely into a bigger knot.")
                if (temp3 > 0.12) outputText("<br><br>Your " + liveData.player.cockDescript(temp2) + " surges free of your sheath, swelling thicker with each passing second. Your knot bulges out at the base, growing far beyond normal.")
                liveData.player.modStats(["sen", 0.5], ["lus", 5 * crit])
                changes++
            }
            ...
            if (liveData.player.cumMultiplier < 2 && UTIL.rand(2) == 0 && changes < changeLimit) {
                ...
                if (temp < liveData.player.cumMultiplier + 0.05 * crit) {
                    ...
                    //Flavor text
                    if (liveData.player.balls == 0) outputText("<br><br>You feel a churning inside your gut as something inside you changes.")
                    if (liveData.player.balls > 0) outputText("<br><br>You feel a churning in your " + liveData.player.ballsDescriptLight() + ". It quickly settles, leaving them feeling somewhat more dense.")
                    if (crit > 1) outputText(" A bit of milky pre dribbles from your " + liveData.player.multiCockDescriptLight() + ", pushed out by the change.")
                }
                changes++
            }
            //Oversized pepper
            if (type == 1) {
                ...
                if (temp2 <= 1) outputText("<br><br>A slight pressure builds and releases as your " + liveData.player.cockDescript(temp) + " pushes a bit further out of your crotch.")
            }
        }
        //Female Stuff
        //Multiboobages
            ...
            if (liveData.player.breastRows[0].breastRating > 0 && liveData.player.vaginas.length > 0) {
                ...
                if (liveData.player.breastRows.length < 3 && UTIL.rand(2) == 0 && changes < changeLimit) {
                    ...
                    if (liveData.player.breastRows.length == 2) {
                        ...
                        if (liveData.player.breastRows[0].breastRating - 1 == 0) outputText("<br><br>A second set of breasts forms under your current pair, stopping while they are still fairly flat and masculine looking.")
                        else outputText("<br><br>A second set of breasts bulges forth under your current pair, stopping as they reach " + liveData.player.breastCup(temp) + "s.")
                    ...
                    if (liveData.player.breastRows.length > 2 && liveData.player.breastRows[0].breastRating > liveData.player.breastRows.length) {
                        ...
                        else outputText("<br><br>Your abdomen tingles and twitches as a new row of " + liveData.player.breastCup(temp) + " " + liveData.player.breastDescript(temp) + " sprouts below your others.")
                        outputText(" A sensitive nub grows on the summit of each new tit, becoming a new nipple.")
                        changes++
                    }
        ...
        if (changes < changeLimit && liveData.player.dogScore() >= 3 && UTIL.rand(4) == 0) {
            ...
            if (liveData.player.gender <= 1 || (liveData.player.gender == 3 && UTIL.rand(2) == 0)) {
                ...
                //break1
                if (liveData.player.cor < 33 || !liveData.player.hasCock()) outputText("<br>You shake your head to clear the unwanted fantasy from your mind, repulsed by it.")
                else {
                    outputText(
                    ...
                    liveData.player.modStats(["lus", 5 + liveData.player.lib / 20])
                    //Break 2
                    if (liveData.player.cor < 66) outputText("<br>You blink a few times, the fantasy fading as you master yourself. That daydream was so strange, yet so hot.")
                    else {
                        outputText(
                        ...
                        if (liveData.player.cor < 80) {
                            if (liveData.player.vaginas.length > 0) outputText("<br>You reluctantly pry your hand from your aching " + liveData.player.vaginaDescript(0) + " as you drag yourself out of your fantasy.")
                            else outputText("<br>You reluctantly pry your hand from your aching " + liveData.player.cockDescript(0) + " as you drag yourself out of your fantasy.")
                        } else {
                            outputText(
        ...
        if (type == 3 && liveData.player.hairColor != "midnight black") {
            if (liveData.player.skinType == ENUM.SkinType.SKIN_TYPE_FUR) outputText("<b><br><br>Your fur and hair tingles, growing in thicker than ever as darkness begins to spread from the roots, turning it midnight black.</b>")
            else outputText("<b><br><br>Your " + liveData.player.skinDesc + " itches like crazy as fur grows out from it, coating your body. It's incredibly dense and black as the middle of a moonless night.</b>")
        ...
        if (UTIL.rand(3) == 0 && liveData.player.lowerBody != ENUM.LowerBodyType.LOWER_BODY_TYPE_DOG && liveData.player.tailType == ENUM.TailType.TAIL_TYPE_DOG && liveData.player.earType == ENUM.EarType.EARS_DOG && changes < changeLimit) {
            //Feet -> paws
            if (liveData.player.lowerBody == ENUM.LowerBodyType.LOWER_BODY_TYPE_HUMAN) outputText("<br><br>You scream in agony as you feel the bones in your feet break and begin to rearrange. <b>You now have paws</b>.")
            //Hooves -> Paws
            else if (liveData.player.lowerBody == ENUM.LowerBodyType.LOWER_BODY_TYPE_HOOFED) outputText("<br><br>You feel your hooves suddenly splinter, growing into five unique digits. Their flesh softens as your hooves reshape into furred paws.")
        ...
        if (UTIL.rand(2) == 0 && liveData.player.earType != ENUM.EarType.EARS_DOG && liveData.player.tailType == ENUM.TailType.TAIL_TYPE_DOG && changes < changeLimit) {
            if (liveData.player.earType == -1) outputText("<br><br>Two painful nubs begin sprouting from your head, growing and opening into canine ears. ")
            ...
            if (liveData.player.earType == ENUM.EarType.EARS_HORSE) outputText("<br><br>Your equine ears twist as they transform into canine versions. ")
            if (liveData.player.earType > ENUM.EarType.EARS_DOG) outputText("<br><br>Your ears transform, becoming more canine in appearance. ")
        ...
        if (UTIL.rand(3) == 0 && changes < changeLimit && liveData.player.tailType != ENUM.TailType.TAIL_TYPE_DOG) {
            ...
            if (liveData.player.tailType >= ENUM.TailType.TAIL_TYPE_COW) outputText("<br><br>You feel your backside shift and change, flesh molding and displacing into a long puffy tail! ")
            changes++
    ...
    static cowTFs(tainted: boolean, enhanced: boolean): void {
        ...
        if (changes < changeLimit && UTIL.rand(3) == 0) {
            ...
            if (temp <= 0) temp = 0
                ...
                if (UTIL.rand(2) == 0) outputText("<br><br>There is a slight pain as you feel your muscles shift somewhat.  Their appearance does not change much, but you feel much stronger.")
        ...
        if (changes < changeLimit && UTIL.rand(3) == 0) {
            ...
            if (temp <= 0) temp = 0
                ...
                if (UTIL.rand(2) == 0) outputText("<br><br>You feel your insides toughening up; it feels like you could stand up to almost any blow.")
        ...
        if (liveData.player.cocks.length > 0 && UTIL.rand(2) == 0 && !liveData.gameFlags[FLAG.HYPER_HAPPY]) {
            ...
            if (liveData.player.cocks[temp].cockLength < 2) {
                ...
                if (liveData.player.cockTotal() == 1 && !liveData.player.hasVagina()) {
                    ...
                    if (liveData.player.balls > 0) outputText("  At the same time, your " + liveData.player.ballsDescriptLight() + " fall victim to the same sensation; eagerly swallowed whole by your crotch.")
                    outputText(
        ...
        if (((tainted && liveData.player.biggestTitSize() <= 11) || (!tainted && liveData.player.biggestTitSize() <= 5)) && changes < changeLimit && (UTIL.rand(3) == 0 || enhanced)) {
            if (UTIL.rand(2) == 0) outputText("<br><br>Your " + liveData.player.breastDescript(0) + " tingle for a moment before becoming larger.")
            else outputText("<br><br>You feel a little weight added to your chest as your " + liveData.player.breastDescript(0) + " seem to inflate and settle in a larger size.")
        ...
        if (enhanced) {
            ...
            if (liveData.player.breastRows[0].nipplesPerBreast == 1) {
                ...
                if (liveData.player.breastRows.length >= 3 && liveData.player.breastRows[2].nipplesPerBreast == 1) {
                    ...
                    if (liveData.player.bRows() == 3) outputText("third row of " + liveData.player.breastDescript(2) + " mutates along with its sisters, sprouting into a wonderland of nipples.")
            ...
            //QUAD DAMAGE IF WEIRD SHIT BROKE BEFORE
                ...
                if (UTIL.rand(2) == 0) outputText("<br><br>A wave of pleasure passes through your chest as your " + liveData.player.breastDescript(0) + " start leaking milk from a massive jump in production.")
                else outputText("<br><br>Something shifts inside your " + liveData.player.breastDescript(0) + " and they feel MUCH fuller and riper.  You know that you've started producing much more milk.")
        ...
        //If breasts are already lactating and the player is not lactating beyond a reasonable level, they start lactating more:
            ...
            if (tainted && liveData.player.breastRows[0].lactationMultiplier > 1 && liveData.player.breastRows[0].lactationMultiplier < 5 && changes < changeLimit && (UTIL.rand(3) == 0 || enhanced)) {
                if (UTIL.rand(2) == 0) outputText("<br><br>A wave of pleasure passes through your chest as your " + liveData.player.breastDescript(0) + " start producing more milk.")
                else outputText("<br><br>Something shifts inside your " + liveData.player.breastDescript(0) + " and they feel fuller and riper.  You know that you've started producing more milk.")
            ...
            if (!tainted) {
                ...
                if ((liveData.player.breastRows[0].lactationMultiplier > 2 && liveData.player.findStatusEffect(StatusEffects.Feeder) >= 0) || liveData.player.breastRows[0].lactationMultiplier > 5) {
                    if (UTIL.rand(2) == 0) outputText("<br><br>Your breasts suddenly feel less full, it seems you aren't lactating at quite the level you were.")
        ...
        if (UTIL.rand(3) == 0) outputText(liveData.player.modFem(79, 3))
        if (UTIL.rand(3) == 0) outputText(liveData.player.modThickness(70, 4))
        if (UTIL.rand(5) == 0) outputText(liveData.player.modTone(10, 5))
    ...
    static succubiDelight(tainted: boolean): void {
        ...
        if (liveData.player.cor < 33) outputText(" This stuff is gross, why are you drinking it?")
        //high corruption
        if (liveData.player.cor >= 66) outputText(" You lick your lips, marvelling at how thick and sticky it is.")
        //Corruption increase
        ...
        if (changes < changeLimit && UTIL.rand(2) == 0 && liveData.player.cocks.length > 0) {
            if (liveData.player.cumMultiplier < 6 && UTIL.rand(2) == 0 && changes < changeLimit) {
                ...
                if (temp < liveData.player.cumMultiplier + 0.4 * crit) {
                    ...
                    //Flavor text
                    if (liveData.player.balls == 0) outputText("<br><br>You feel a churning inside your body as something inside you changes.")
                    if (liveData.player.balls > 0) outputText("<br><br>You feel a churning in your " + liveData.player.ballsDescriptLight() + ". It quickly settles, leaving them feeling somewhat more dense.")
                    if (crit > 1) outputText(" A bit of milky pre dribbles from your " + liveData.player.multiCockDescriptLight() + ", pushed out by the change.")
                    liveData.player.modStats(["lib", 1])
                }
    ...
    static equineTFs(): void {
            ...
            if (UTIL.rand(4) == 0 && liveData.gameFlags[FLAG.HORSE_WARNING] == 1 && liveData.player.findPerk(PerkLib.TransformationResistance) < 0) {
                ...
                if (liveData.player.cockTotal() > 0) {
                    ...
                    if (liveData.player.countCocksOfType(ENUM.CockType.HORSE) > 0) {
                        ...
                        if (liveData.player.gender == 0 || liveData.player.gender == 3) outputText("horse ")
                        if (liveData.player.gender == 1) outputText("stallion ")
                        if (liveData.player.gender == 2) outputText("mare ")
                        outputText(
                            " with beautiful " +
                        ...
                        if (liveData.player.gender == 0 || liveData.player.gender == 1) outputText("He is clearly a male, but you are somewhat confused as you can see not one but three bulges where his manhood would be.<br><br>")
                        if (liveData.player.gender == 2) outputText("She is clearly a female, as you can see her six breasts jiggle as she walks towards you, small stains appearing on her shirt where her nipples are.<br><br>")
                        if (liveData.player.gender == 3) outputText("You are somewhat confused as you can see a bulge near her thighs but also huge boobs jiggling as she walks, and you can't say if she's a male or female.<br><br>")
                        outputText(
        ...
        if ((liveData.player.gender == 1 || liveData.player.gender == 3) && UTIL.rand(1.5) == 0 && changes < changeLimit) {
            ...
            if (liveData.player.countCocksOfType(ENUM.CockType.HORSE) + liveData.player.countCocksOfType(ENUM.CockType.DEMON) < liveData.player.cocks.length) {
                ...
                //MULTICOCK
                    ...
                    if (liveData.player.hasSheath()) outputText(" Your sheath tingles and begins growing larger as the cock's base shifts to lie inside it.")
                    else
                        outputText(
            ...
            //Players cocks are all horse-type - increase size!
                ...
                if (temp2 <= 1) outputText("A slight pressure builds and releases as your " + liveData.player.cockDescript(temp) + " pushes a bit further out of your sheath.")
                changes++
            }
            ...
            if (UTIL.rand(2) == 0 && changes < changeLimit && liveData.player.countCocksOfType(ENUM.CockType.HORSE) > 0) {
                ...
                outputText(".")
                if (liveData.player.cor < 30) outputText(" You shudder in revulsion at the strange thoughts and vow to control yourself better.")
                if (liveData.player.cor >= 30 && liveData.player.cor < 60) outputText(" You wonder why you thought such odd things, but they have a certain appeal.")
                if (liveData.player.cor >= 60 && liveData.player.cor < 90) outputText(" You relish your twisted fantasies, hoping to dream of them again.")
                if (liveData.player.cor >= 90) outputText(" You flush hotly and give a twisted smile, resolving to find a fitting subject to rape and relive your fantasies.")
                ...
                if (UTIL.rand(2) == 0 && changes < changeLimit) {
                    ...
                    if (liveData.player.breastRows.length == 1) {
                        ...
                        if (liveData.player.breastRows[0].breastRating > 3) {
                            ...
                            if (temp == 1) outputText("<br><br>You feel a weight lifted from you, and realize your " + liveData.player.breastDescript(0) + " have shrunk to a " + liveData.player.breastCup(0) + ".")
                            if (temp == 2) outputText("<br><br>You feel significantly lighter. Looking down, you realize your breasts are MUCH smaller, down to " + liveData.player.breastCup(0) + "s.")
                            changes++
                        }
                    }
                    //multiple
                        ...
                        if (liveData.player.biggestTitSize() > 3) outputText("<br>")
                        while (temp3 > 0) {
                            temp3--
                            if (liveData.player.breastRows[temp3].breastRating > 3) {
                                ...
                                temp2++
                                outputText("<br>")
                                if (temp3 < liveData.player.breastRows.length - 1) outputText("...and y")
                                else outputText("Y")
        ...
        if (liveData.player.earType != ENUM.EarType.EARS_HORSE && liveData.player.tailType == ENUM.TailType.TAIL_TYPE_HORSE && changes < changeLimit && UTIL.rand(3) == 0) {
            if (liveData.player.earType == -1) outputText("<br><br>Two painful lumps sprout on the top of your head, forming into tear-drop shaped ears, covered with short fur. ")
            if (liveData.player.earType == ENUM.EarType.EARS_HUMAN) outputText("<br><br>Your ears tug painfully on your face as they begin shifting, moving upwards to the top of your head and transforming into a upright animalistic ears. ")
            if (liveData.player.earType == ENUM.EarType.EARS_DOG) outputText("<br><br>Your ears change shape, morphing into from their doglike shape into equine-like ears! ")
            if (liveData.player.earType > ENUM.EarType.EARS_DOG) outputText("<br><br>Your ears change shape, morphing into teardrop-shaped horse ears! ")
        ...
        if (UTIL.rand(3) == 0) outputText(liveData.player.modTone(60, 1))
        //FAILSAFE CHANGE
        if (changes == 0) {
            outputText("<br><br>Inhuman vitality spreads through your body, invigorating you!<br>")
            liveData.player.changeHP(20, true)
    ...
    static felineTFs(): void {
        ...
        if (liveData.player.str < 40 && UTIL.rand(3) == 0 && changes < changeLimit) {
            ...
            if (UTIL.rand(2) == 0) outputText("<br><br>Your muscles feel taut, like a coiled spring, and a bit more on edge.")
        ...
        if (liveData.player.lib < 80 && changes < changeLimit && UTIL.rand(4) == 0) {
            ...
            if (liveData.player.countCocksOfType(ENUM.CockType.CAT) > 0) {
                ...
                if (liveData.player.cor < 33) outputText("You need to control yourself better.")
                else if (liveData.player.cor < 66) outputText("You're not sure how you feel about the fantasy.")
            ...
            //Else –
                ...
                if (liveData.player.lust > 60) outputText("even more ")
        ...
        if (UTIL.rand(4) == 0 && changes < changeLimit) {
            ...
            if (liveData.player.goIntoHeat(false)) {
                if (intensified) {
                    if (UTIL.rand(2) == 0) outputText("<br><br>The itch inside your " + liveData.player.vaginaDescript(0) + " is growing stronger, and you desperately want to find a nice cock to massage the inside.")
                    else
                        outputText(
                    ...
                    if (liveData.player.skinType == ENUM.SkinType.SKIN_TYPE_FUR) outputText("underneath your fur ")
                    outputText("as images and fantasies ")
                    if (liveData.player.cor < 50) outputText("assault ")
                ...
                for (let k = 0; k < liveData.player.breastRows.length; k++) {
                    ...
                    if (liveData.player.breastRows[k].breastRating > temp2) {
                        ...
                        if (liveData.player.breastRows[k].breastRating > 10) {
                            ...
                            if (temp3 == 0) outputText("<br><br>The " + liveData.player.breastDescript(0) + " on your chest wobble for a second, then tighten up, losing several cup-sizes in the process!")
                            else outputText(" The change moves down to your " + UTIL.num2Text2(k + 1) + " row of " + liveData.player.breastDescript(0) + ". They shrink greatly, losing a couple cup-sizes.")
                        }
                        //Small change
                        else {
                            liveData.player.breastRows[k].breastRating -= 1
                            if (temp3 == 0) outputText("<br><br>All at once, your sense of gravity shifts. Your back feels a sense of relief, and it takes you a moment to realize your " + liveData.player.breastDescript(k) + " have shrunk!")
                            else outputText(" Your " + UTIL.num2Text2(k + 1) + " row of " + liveData.player.breastDescript(k) + " gives a tiny jiggle as it shrinks, losing some off its mass.")
                        }
        ...
        if (
            ...
            if (!liveData.player.hasSheath()) {
                ...
                if (liveData.player.balls > 0) outputText(" Thankfully, your balls appear untouched.")
        ...
        if (liveData.player.countCocksOfType(ENUM.CockType.CAT) > 0 && UTIL.rand(3) == 0 && changes < changeLimit && !liveData.hyperHappy) {
            ...
            if (temp == 1) {
                ...
                if (temp2 == 1) outputText(" Upon further inspection, all your " + Appearance.cockNoun(ENUM.CockType.CAT) + "s have shrunk!")
                changes++
            }
        }
        //------------
        ...
        if (liveData.player.earType != ENUM.EarType.EARS_CAT && UTIL.rand(5) == 0 && changes < changeLimit) {
            ...
            //non human to cat:
                ...
                if (UTIL.rand(2) == 0) outputText("<br><br>Your ears change shape, morphing into pointed, feline ears! They swivel about reflexively as you adjust to them. <b>You now have cat ears.</b>")
                else
                    outputText(
        ...
        if (liveData.player.tailType == ENUM.TailType.TAIL_TYPE_CAT && liveData.player.earType == ENUM.EarType.EARS_CAT && UTIL.rand(5) == 0 && changes < changeLimit && liveData.player.lowerBody != ENUM.LowerBodyType.LOWER_BODY_TYPE_CAT) {
            ...
            if (liveData.player.lowerBody == ENUM.LowerBodyType.LOWER_BODY_TYPE_HOOFED) {
                ...
                if (liveData.player.isTaur()) outputText(" You feel woozy and collapse on your side. When you wake, you're no longer a centaur and your body has returned to a humanoid shape.")
            }
            //Goo to cat
            ...
            //non hoof to cat:
            else outputText("<br><br>You scream in agony as you feel the bones in your " + liveData.player.feet() + " break and begin to rearrange. When the pain fades, you feel surprisingly well-balanced. <b>You now have cat paws.</b>")
        ...
        if (changes < changeLimit) {
            if (UTIL.rand(2) == 0) outputText(liveData.player.modThickness(5, 2))
            if (UTIL.rand(2) == 0) outputText(liveData.player.modTone(76, 2))
            if (liveData.player.gender < 2) {
                if (UTIL.rand(2) == 0) outputText(liveData.player.modFem(65, 1))
                else outputText(liveData.player.modFem(85, 2))
            }
        }
        liveData.gameFlags[FLAG.TIMES_TRANSFORMED] += changes
    }
    ...
    static goblinTFs(): void {
        ...
        if (liveData.player.skinTone != "green" && liveData.player.skinTone != "grayish-blue" && liveData.player.skinTone != "dark green" && liveData.player.skinTone != "pale yellow" && changes < changeLimit && UTIL.rand(2) == 0) {
            ...
            if (liveData.player.skinType == ENUM.SkinType.SKIN_TYPE_FUR) outputText("skin")
            else outputText(liveData.player.skinDesc)
    ...
    static humanTFs(): void {
        ...
        if (liveData.player.skinTone != "tan" && liveData.player.skinTone != "olive" && liveData.player.skinTone != "dark" && liveData.player.skinTone != "light" && changes < changeLimit && UTIL.rand(5) == 0) {
            ...
            if (liveData.player.skinType == ENUM.SkinType.SKIN_TYPE_FUR) outputText("the skin under your " + liveData.player.furColor + " " + liveData.player.skinDesc)
            else outputText("your " + liveData.player.skinDesc)
        ...
        if (liveData.player.skinType != ENUM.SkinType.SKIN_TYPE_PLAIN && (liveData.player.earType == ENUM.EarType.EARS_HUMAN || liveData.player.earType == ENUM.EarType.EARS_ELFIN) && UTIL.rand(4) == 0 && changes < changeLimit) {
            ...
            if (liveData.player.skinType == ENUM.SkinType.SKIN_TYPE_SCALES) outputText("are")
            else outputText("is")
        ...
        if (liveData.player.hasCock() && liveData.player.biggestCockLength() > 12 && UTIL.rand(3) == 0 && changes < changeLimit) {
            ...
            if (liveData.player.cocks.length == 1) outputText("<br><br>You feel a tingling sensation as your cock shrinks to a smaller size!")
        ...
        if (changes < changeLimit && liveData.player.breastRows.length > 1 && UTIL.rand(3) == 0 && !liveData.hyperHappy) {
            ...
            if (liveData.player.breastRows.length >= 3) outputText("abdomen")
            else outputText("chest")
            outputText(". The " + liveData.player.nippleDescript(liveData.player.breastRows.length - 1) + "s even fade until nothing but ")
            if (liveData.player.skinType == ENUM.SkinType.SKIN_TYPE_FUR) outputText(liveData.player.hairColor + " " + liveData.player.skinDesc)
            else outputText(liveData.player.skinTone + " " + liveData.player.skinDesc)
        ...
        if (liveData.player.hasVagina() && UTIL.rand(3) == 0 && changes < changeLimit) {
            ...
            //High fertility:
            if (liveData.player.fertility >= 30) outputText("It feels like your overcharged reproductive organs have simmered down a bit.")
            //Average fertility:
            else if (liveData.player.fertility >= 10) outputText("You feel like you have dried up a bit inside; you are left feeling oddly tranquil.")
            //[Low/No fertility:
            else {
                outputText("Although the numbness makes you feel serene, the hummus has no effect upon your ")
                if (liveData.player.fertility > 0) outputText("mostly ")
        ...
        if (liveData.player.hasCock() && liveData.player.cumMultiplier > 5 && UTIL.rand(3) == 0 && changes < changeLimit) {
            ...
            if (liveData.player.balls > 0) outputText("balls")
    ...
    static impTFs(): void {
        ...
        if (liveData.player.findPerk(PerkLib.TransformationResistance) >= 0) changeLimit--
        //Consumption text
        if (liveData.player.hasCock()) outputText("The food tastes strange and corrupt - you can't really think of a better word for it, but it's unclean.")
        else outputText("The food tastes... corrupt, for lack of a better word.")
        ...
        if (changes < changeLimit && UTIL.rand(10) == 0 && liveData.player.skinTone != "red") {
            if (liveData.player.skinType == ENUM.SkinType.SKIN_TYPE_FUR) outputText("<br><br>Underneath your fur, your skin ")
            else outputText("<br><br>Your " + liveData.player.skinDesc + " ")
    ...
    static pigTFs(boar: boolean): void {
        ...
        if (UTIL.rand(3) == 0 && changes < changeLimit && liveData.player.balls > 0 && liveData.player.ballSize < 4) {
            ...
            else outputText("<br><br>A sudden onset of heat envelops your groin, focusing on your ballsack. Walking becomes difficult as you discover your " + (liveData.player.balls == 4 ? "quartette" : "duo") + " of testicles have enlarged again.")
            liveData.player.ballSize++
            changes++
        }
        //------------
        ...
        if (UTIL.rand(boar ? 2 : 3) == 0 && changes < changeLimit && liveData.player.earType == ENUM.EarType.EARS_PIG && liveData.player.tailType != ENUM.TailType.TAIL_TYPE_PIG) {
            ...
            else outputText("<br><br>You feel a tug at the base of your spine as it lengthens ever so slightly. Looking over your shoulder, <b>you find that you have sprouted a small, curly pig tail.</b>")
        ...
        if (
            ...
            //Bipedal
            else outputText("<br><br>You scream in agony as the bones in your legs break and rearrange. Once the pain subsides, you inspect your legs, finding that they are digitigrade and ending in cloven hooves. <b>You now have pig legs!</b>")
            liveData.player.lowerBody = ENUM.LowerBodyType.LOWER_BODY_TYPE_CLOVEN_HOOFED
            liveData.player.legCount = 2
            changes++
        }
    ...
    static lizardTFs(): void {
        ...
        if (liveData.player.lib < 100 && changes < changeLimit && UTIL.rand(3) == 0) {
            ...
            if (liveData.player.cocks.length > 0 && (liveData.player.gender != 3 || UTIL.rand(2) == 0)) {
                outputText("filling ")
                if (liveData.player.cocks.length > 1) outputText("each of ")
            ...
            //(COOCH)
            else if (liveData.player.hasVagina()) outputText("puddling in your " + liveData.player.vaginaDescript(0) + ".  An instinctive desire to mate and lay eggs spreads through you, increasing your lust and boosting your sex-drive.")
            //(TARDS)
            else
                outputText(
        ...
        if (liveData.player.countCocksOfType(ENUM.CockType.LIZARD) == 0 && liveData.player.cockTotal() > 0 && changes < changeLimit && UTIL.rand(4) == 0) {
            ...
            if (liveData.player.hasSheath()) outputText("sheath ")
            ...
            if (liveData.player.cor < 33) outputText("horrifies you.")
            else if (liveData.player.cor < 66) outputText("is a little strange for your tastes.")
            else {
                outputText("looks like it might be more fun to receive than use on others.  ")
                if (liveData.player.hasVagina()) outputText("Maybe you could find someone else with one to ride?")
        ...
        if (liveData.player.cockTotal() > 1 && liveData.player.countCocksOfType(ENUM.CockType.LIZARD) > 0 && liveData.player.cockTotal() > liveData.player.countCocksOfType(ENUM.CockType.LIZARD) && UTIL.rand(4) == 0 && changes < changeLimit) {
            ...
            if (liveData.player.cockTotal() == 2) outputText("your other dick")
            ...
            if (liveData.player.cumQ() < 50) outputText("pre-cum oozes from the tip")
            else if (liveData.player.cumQ() < 700) outputText("Thick pre-cum rains from the tip")
        ...
        if (liveData.player.countCocksOfType(ENUM.CockType.LIZARD) == 1 && liveData.player.cocks.length == 1 && UTIL.rand(4) == 0 && changes < changeLimit) {
            ...
            if (liveData.player.skinType == ENUM.SkinType.SKIN_TYPE_FUR || liveData.player.skinType == ENUM.SkinType.SKIN_TYPE_SCALES) outputText(" and shedding " + liveData.player.skinDesc)
            outputText(
        ...
        if (liveData.player.biggestLactation() >= 1 && changes < changeLimit && UTIL.rand(4) == 0) {
            if (liveData.player.totalNipples() == 2) outputText("<br><br>Both of your")
            ...
            if (liveData.player.hasFuckableNipples()) outputText("but sexual fluid ")
            ...
            //ALREADY DRAGON
                ...
                if (liveData.player.hornType == ENUM.HornType.HORNS_DRACONIC_X2) {
                    if (liveData.player.horns < 12) {
                        ...
                        if (liveData.player.horns >= 12) outputText("  <b>Your horns settle down quickly, as if they're reached their full size.</b>")
                        changes++
                    }
                    //maxxed out, new row
                    else {
        ...
        if (liveData.player.lowerBody != ENUM.LowerBodyType.LOWER_BODY_TYPE_LIZARD && changes < changeLimit && UTIL.rand(5) == 0) {
            ...
            //TAURS -
            else if (liveData.player.isTaur()) outputText("<br><br>Your lower body is wracked by pain!  Once it passes, you discover that you're standing on digitigrade legs with lizard-like claws.")
            //feet types -
            else if (
                liveData.player.lowerBody == ENUM.LowerBodyType.LOWER_BODY_TYPE_HUMAN ||
                liveData.player.lowerBody == ENUM.LowerBodyType.LOWER_BODY_TYPE_DOG ||
    ...
    static harpyTFs(type: number): void {
        ...
        if (liveData.player.spe < 100 && UTIL.rand(3) == 0) {
            changes++
            if (liveData.player.spe >= 75) outputText("<br><br>A familiar chill runs down your spine. Your muscles feel like well oiled machinery, ready to snap into action with lightning speed.")
        ...
        if (liveData.player.tou > 50 && UTIL.rand(3) == 0 && changes < changeLimit) {
            changes++
            if (UTIL.rand(2) == 0) outputText("<br><br>A nice, slow warmth rolls from your gut out to your limbs, flowing through them before dissipating entirely. As it leaves, you note that your body feels softer and less resilient.")
        ...
        if (liveData.player.str < 70 && UTIL.rand(3) == 0 && changes < changeLimit) {
            changes++
            //(low str)
            if (liveData.player.str < 40) outputText("<br><br>Shivering, you feel a feverish sensation that reminds you of the last time you got sick. Thankfully, it passes swiftly, leaving slightly enhanced strength in its wake.")
            //(hi str – 50+)
        ...
        if ((liveData.player.lib < 90 || UTIL.rand(3) == 0) && UTIL.rand(3) == 0 && changes < changeLimit) {
            ...
            if (liveData.player.lib < 40) {
                ...
                if (liveData.player.hasVagina()) outputText(" The moistness of your " + liveData.player.vaginaDescript() + " seems to agree.")
                else if (liveData.player.hasCock()) outputText(" The hardness of " + liveData.player.sMultiCockDesc() + " seems to agree.")
                liveData.player.changeLust("lus", 5)
            }
            //(sub 75 lib)
            else if (liveData.player.lib < 75) outputText("<br><br>Heat, blessed heat, works through you from head to groin, leaving you to shudder and fantasize about the sex you could be having right now.<br><br>")
            //(hi lib)
            else if (liveData.player.lib < 90) outputText("<br><br>Sexual need courses through you, flushing your skin with a reddish hue while you pant and daydream of the wondrous sex you should be having right now.<br><br>")
            //(90+)
            ...
            if (liveData.player.lib >= 40) {
                ...
                if (liveData.player.hasCock() && (liveData.player.gender != 3 || UTIL.rand(2) == 0)) {
                    ...
                    if (UTIL.rand(2) == 0) {
                        ...
                        if (liveData.player.cockTotal() > 1) {
                            ...
                            if (liveData.player.cockTotal() > 2) outputText("es rub ")
                ...
                //Cunts!
                    ...
                    if (UTIL.rand(2) == 0) {
                        ...
                        if (liveData.player.hasFuckableNipples()) outputText("and pussy leaking over ")
                        else if (liveData.player.biggestLactation() >= 1.5) outputText("dripping milk inside ")
        ...
        if (changes < changeLimit && liveData.player.breastRows[0].breastRating < 2 && UTIL.rand(3) == 0) {
            ...
            if (liveData.player.breastRows[0].breastRating < 1) outputText("  <b>You have breasts now!</b>")
            liveData.player.breastRows[0].breastRating = 2
        }
        ...
        if (liveData.player.skinTone != "tan" && liveData.player.skinTone != "olive" && liveData.player.skinTone != "dark" && liveData.player.skinTone != "light" && changes < changeLimit && UTIL.rand(5) == 0) {
            changes++
            outputText("<br><br>It takes a while for you to notice, but <b>")
            if (liveData.player.skinType == ENUM.SkinType.SKIN_TYPE_FUR) outputText("the skin under your " + liveData.player.hairColor + " " + liveData.player.skinDesc)
            else outputText("your " + liveData.player.skinDesc)
            outputText(" has changed to become ")
            let temp = UTIL.rand(4)
            if (temp == 0) liveData.player.skinTone = "tan"
    ...
    static minotaurTFs(): void {
        ...
        if (liveData.player.lowerBody != ENUM.LowerBodyType.LOWER_BODY_TYPE_HOOFED) {
            if (changes < changeLimit && UTIL.rand(3) == 0) {
                ...
                if (liveData.player.lowerBody > ENUM.LowerBodyType.LOWER_BODY_TYPE_NAGA)
                    outputText("<br><br>You stagger as your " + liveData.player.feet() + " change, curling up into painful angry lumps of flesh.  They get tighter and tighter, harder and harder, until at last they solidify into hooves!")
                if (liveData.player.skinType != ENUM.SkinType.SKIN_TYPE_FUR) outputText("  A fine coat of fur grows out below your waist, itching briefly as it fills in.")
            ...
            //Shrink boobages till they are normal
                ...
                if (liveData.player.breastRows.length == 1) {
                    ...
                    if (liveData.player.breastRows[0].breastRating >= 1) {
                        ...
                        //Talk about shrinkage
                        if (temp == 1) outputText("<br><br>You feel a weight lifted from you, and realize your " + liveData.player.breastDescript(0) + " have shrunk to " + liveData.player.breastCup(0) + "s.")
                        if (temp == 2) outputText("<br><br>You feel significantly lighter.  Looking down, you realize your breasts are MUCH smaller, down to " + liveData.player.breastCup(0) + "s.")
                        changes++
                    }
                }
                //multiple
                    ...
                    temp2 = 0
                    temp3 = 0
                    if (liveData.player.biggestTitSize() >= 1) outputText("<br>")
        ...
        if (changes < changeLimit && UTIL.rand(2) == 0 && liveData.player.cocks.length > 0) {
            ...
            if (selectedCock != -1) {
                ...
                if (liveData.player.cocks[selectedCock].cockThickness < 5) {
                    ...
                    if (temp > 6) outputText("<br><br>Gasping in sudden pleasure, your " + liveData.player.cockDescript(selectedCock) + " surges free of its sheath, emerging with over half a foot of new dick-flesh.", false)
                    if (temp <= 6 && temp >= 3) outputText("<br><br>You pant in delight as a few inches of " + liveData.player.cockDescript(selectedCock) + " pop free from your sheath, the thick new horse-flesh still slick and sensitive.", false)
                ...
                //Just length...
                    ...
                    temp = liveData.player.increaseCock(selectedCock, 2 + UTIL.rand(8))
                    //Comment on length changes
                    if (temp > 6) outputText("<br><br>Gasping in sudden pleasure, your " + liveData.player.cockDescript(selectedCock) + " surges free of its sheath, emerging with over half a foot of new dick-flesh.")
                    if (temp <= 6 && temp >= 3) outputText("<br><br>You pant in delight as a few inches of " + liveData.player.cockDescript(selectedCock) + " pop free from your sheath, the thick new horse-flesh still slick and sensitive.")
                    if (temp < 3) outputText("<br><br>Groaning softly, you feel a pleasurable change in your groin.  Looking down, you see [oneCock] grow slightly longer.")
                }
                changes++
            }
        ...
        if (liveData.player.cocks.length > 0 && UTIL.rand(2) == 0 && changes < changeLimit) {
            ...
            if (selectedCockValue != -1) {
                ...
                if (liveData.player.countCocksOfType(ENUM.CockType.HORSE) > 1) outputText("nother")
            ...
            if (liveData.player.hornType == ENUM.HornType.HORNS_COW_MINOTAUR || liveData.player.hornType == ENUM.HornType.HORNS_NONE) {
                ...
                if (liveData.player.hornType == ENUM.HornType.HORNS_COW_MINOTAUR) {
                    ...
                    //Males horns get 'uge.
                        ...
                        if (liveData.player.horns < 3) outputText("They are the size of tiny nubs.")
                        if (liveData.player.horns >= 3 && liveData.player.horns < 6) outputText("They are similar to what you would see on a young bull.")
                        if (liveData.player.horns >= 6 && liveData.player.horns < 12) outputText("They look like the horns on a grown bull, big enough and dangerous enough to do some damage.")
                        if (liveData.player.horns >= 12 && liveData.player.horns < 20) outputText("They are large and wicked looking.")
                        if (liveData.player.horns >= 20) outputText("They are huge, heavy, and tipped with dangerous points.")
        ...
        if (UTIL.rand(4) == 0) outputText(liveData.player.modFem(5, 10))
        if (UTIL.rand(4) == 0) outputText(liveData.player.modTone(85, 3))
        if (UTIL.rand(4) == 0) outputText(liveData.player.modThickness(70, 4))
        //Default
        if (changes == 0) {
    ...
    static snakeTFs(): void {
        ...
        //(if outside combat)
        if (!COMBAT.inCombat()) outputText("  Minutes pass as you start wishing you had water with you, to get rid of the aftertaste.")
        //+ speed to 70!
        if (liveData.player.spe < 70 && UTIL.rand(2) == 0) {
            ...
            if (liveData.player.spe < 40) outputText("  Of course, you're nowhere near as fast as that.")
            changes++
        }
        ...
        if (changes == 0 && liveData.player.faceType == ENUM.FaceType.FACE_SNAKE_FANGS && liveData.player.lowerBody != ENUM.LowerBodyType.LOWER_BODY_TYPE_NAGA && UTIL.rand(4) == 0 && changes < changeLimit) {
            ...
            if (liveData.player.balls > 0 && liveData.player.ballSize > 10) outputText("  You're happy not to have to drag those testicles around with you anymore.")
            outputText(
    ...
    static slimeTFs(): void {
        ...
        if (UTIL.rand(2) == 0) outputText(liveData.player.modFem(85, 3))
        if (UTIL.rand(2) == 0) outputText(liveData.player.modThickness(20, 3))
        if (UTIL.rand(2) == 0) outputText(liveData.player.modTone(15, 5))
    }
    ...
    static trapOil(): void {
        ...
        if (liveData.player.cor < 30) outputText("hesitantly ")
        else if (liveData.player.cor > 70) outputText("eagerly ")
        ...
        if (liveData.player.longestCockLength() >= 3.5 && liveData.player.hasCock() && UTIL.rand(2) == 0 && changes < changeLimit) {
            ...
            if (liveData.player.cockTotal() == 1) outputText("s")
            ...
            if (liveData.player.cockTotal() == 1) outputText("it is")
            else outputText("they are")
            outputText(" still present, and as you touch ")
            if (liveData.player.cockTotal() == 1) outputText("it")
            else outputText("them")
            outputText(", the sensitivity fades, however - a blush comes to your cheeks - ")
            if (liveData.player.cockTotal() == 1) outputText("it seems")
        ...
        if (liveData.player.ass.analWetness < 5 && UTIL.rand(4) == 0 && changes < changeLimit) {
            if (liveData.player.ass.analWetness < 4) outputText("<br><br>Your eyes widen in shock as you feel oily moisture bead out of your [asshole].  Your asshole has become wetter and more pliable.")
        ...
        if (liveData.player.hasVagina() && UTIL.rand(4) == 0 && changes < changeLimit) {
            ...
            if (liveData.player.fertility >= 30) outputText("It feels like your overcharged reproductive organs have simmered down a bit.")
            //Average fertility:
            else if (liveData.player.fertility >= 5) outputText("You feel like you have dried up a bit inside; you are left feeling oddly tranquil.")
            //[Low/No fertility:
            else {
        ...
        if (liveData.player.hasVagina() && liveData.player.vaginaType() != 5 && UTIL.rand(4) == 0 && changes < changeLimit) {
            ...
            //(Wet:
            if (liveData.player.wetness() >= 3) outputText("  Your natural lubrication makes it gleam invitingly.")
            //(Corruption <50:
            if (liveData.player.cor < 50) outputText("  After a few cautious touches you decide it doesn't feel any different- it does certainly look odd, though.")
    ...
    */
    ...
    static restoreArms(tfSource: string) {
        ...
        if (tfSource == "gooGasmic") {
            ...
            if (hasClaws || liveData.player.armType == ENUM.ArmType.ARM_TYPE_HARPY) outputText(message + "  <b>You have normal human arms again.</b>")
