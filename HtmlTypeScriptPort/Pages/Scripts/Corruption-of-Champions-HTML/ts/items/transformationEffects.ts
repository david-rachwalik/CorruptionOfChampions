ConsumableEffects.beeTFs = function (type) {
    ...
    if ((pure || special) && player.pregnancyType == PREGNANCY_FAERIE) {
        //Pure or special honey can reduce the corruption of a phouka baby
        if (gameFlags[PREGNANCY_CORRUPTION] > 1) {
        ...
        } else if (gameFlags[PREGNANCY_CORRUPTION] < 1) {
    ...
    if (changes < changeLimit && player.bRows() > 2 && rand(3) == 0 && !hyperHappy) {
        ...
        outputText(
            "<br><br>You stumble back when your center of balance shifts, and though you adjust before you can fall over, you're left to watch in awe as your bottom-most " +
                player.breastDescript(player.breastRows.length - 1) +
    ...
    if (player.averageNipplesPerBreast() > 1 && changes < changeLimit && rand(4) == 0) {
        outputText(
            "<br><br>A chill runs over your " +
                ...
                player.clothedOrNakedUpper(player.armorDescript()) +
    ...
    if (special) {
        ...
        if (!player.hasCock()) {
            ...
            if (player.cocks[0].cArea() <= 20) outputText("It now swings as low as your knees!")
            else if (player.cocks[0].cArea() <= 50) outputText("While erect, your massive member fills the lower half of your vision.")
            ...
            player.modStats("sen", 5)
        } else if (player.cocks[0].cockType != ENUM.CockType.BEE && player.race() == "bee-morph") {
            outputText(
            ...
            player.cocks[0].cockType = ENUM.CockType.BEE
...
ConsumableEffects.canineTFs = function (type) {
    ...
    if (type == 2) {
        ...
        if (player.countCocksOfType(ENUM.CockType.DOG) >= 2) {
            type = 1
        }
        //If player doesnt have 2 dogdicks
            ...
            if (player.countCocksOfType(ENUM.CockType.DOG) == 0) {
                ...
                if (player.cockTotal() == 0) {
                    ...
                    outputText(
                        "<br><br>" +
                            player.clothedOrNakedLower(
                                "A painful lump forms on your groin, nearly doubling you over as it presses against your " + player.armor.equipmentName + ". You rip open your gear and",
                    ...
                    player.cocks[0].cockType = ENUM.CockType.DOG
                    player.cocks[1].knotMultiplier = 1.7
                    player.cocks[1].cockType = ENUM.CockType.DOG
            ...
            //If player has 1 dogdicks
                ...
                if (player.cockTotal() >= 1) {
                    ...
                    if (player.cocks[0].cockType == ENUM.CockType.DOG) {
                        outputText(
                            "<br><br>Your crotch twitches, and you pull open your " +
                                player.armor.equipmentName +
                                " to get a better look. You watch in horror and arousal as your " +
        ...
        if (player.cockTotal() > 0) {
            ...
            if (player.countCocksOfType(ENUM.CockType.DOG) > 0) {
                ...
                while (temp < player.cocks.length) {
                    if (player.cocks[temp].cockType == ENUM.CockType.DOG) {
                        temp2 = temp
                        break
                    } else temp++
                }
                ...
                while (temp > 0) {
                    temp--
                    if (player.cocks[temp].cockType == ENUM.CockType.DOG && player.cocks[temp].knotMultiplier < player.cocks[temp2].knotMultiplier) temp2 = temp
                }
                ...
                player.modStats("sen", 0.5, "lus", 5 * crit)
            }
    ...
    if (type == 5) {
        if (player.balls <= 1) {
            ...
            if (player.ballSize > 2)
                outputText("<br><br>A sudden onset of heat envelops your groin, focusing on your " + player.sackDescript() + ". Walking becomes difficult as you discover your " + player.ballsDescriptLight() + " have enlarged again.")
    ...
    if (player.cocks.length > 0) {
        ...
        if (type != 4 && player.countCocksOfType(ENUM.CockType.DOG) > 0 && ((changes < changeLimit && rand(1.4) == 0) || type == 1)) {
            temp = 0
            //set temp2 to first dogdick for initialization
            while (temp < player.cocks.length) {
                if (player.cocks[temp].cockType == ENUM.CockType.DOG) {
            ...
            if (player.cocks[temp2].knotMultiplier >= 2) temp3 /= 5
            player.cocks[temp2].knotMultiplier += temp3
            if (temp3 < 0.06) outputText("<br><br>Your " + player.cockDescript(temp2) + " feels unusually tight in your sheath as your knot grows.")
            if (temp3 >= 0.06 && temp3 <= 0.12) outputText("<br><br>Your " + player.cockDescript(temp2) + " pops free of your sheath, thickening nicely into a bigger knot.")
            if (temp3 > 0.12) outputText("<br><br>Your " + player.cockDescript(temp2) + " surges free of your sheath, swelling thicker with each passing second. Your knot bulges out at the base, growing far beyond normal.")
            player.modStats("sen", 0.5, "lus", 5 * crit)
            changes++
        }
        //Cock Xform if player has free cocks.
        if (player.countCocksOfType(ENUM.CockType.DOG) < player.cocks.length && ((changes < changeLimit && rand(1.6)) || type == 1) == 0) {
            ...
            //Talk about it
            //Hooooman
            if (player.cocks[temp3].cockType == ENUM.CockType.HUMAN) {
                outputText(
                    "<br><br>Your " +
                        player.cockDescript(temp3) +
            ...
            //Horse
            if (player.cocks[temp3].cockType == ENUM.CockType.HORSE) {
                outputText(
                    "<br><br>Your " +
            ...
            //Tentacular Tuesday!
            if (player.cocks[temp3].cockType == ENUM.CockType.TENTACLE) {
                outputText(
                    "<br><br>Your " +
                        player.cockDescript(temp3) +
            ...
            temp = 0
            //Demon
            if (player.cocks[temp3].cockType == ENUM.CockType.DEMON) {
            ...
            //Xform it!
            player.cocks[temp3].cockType = ENUM.CockType.DOG
            player.cocks[temp3].knotMultiplier = 1.1
            player.cocks[temp3].thickenCock(2)
            if (temp == 1) {
                player.cocks[temp3].cockType = ENUM.CockType.DEMON
            }
            changes++
        }
        ...
        if (player.cumMultiplier < 2 && rand(2) == 0 && changes < changeLimit) {
            ...
            if (temp < player.cumMultiplier + 0.05 * crit) {
                changes--
            } else {
        ...
        if (type == 1) {
            ...
            if (temp2 > 2)
                outputText(
                    "<br><br>Your " + player.cockDescript(temp) + " tightens painfully, inches of bulging dick-flesh pouring out from your crotch as it grows longer. Thick pre forms at the pointed tip, drawn out from the pleasure of the change."
                )
            if (temp2 > 1 && temp2 <= 2)
                outputText(
    ...
    if (rand(5) == 0 && changes < changeLimit && player.faceType != FACE_DOG && player.skinType == SKIN_TYPE_FUR && player.lowerBody == LOWER_BODY_TYPE_DOG) {
    ...
    if (rand(4) == 0 && changes < changeLimit && player.lowerBody == LOWER_BODY_TYPE_DOG && player.tailType == TAIL_TYPE_DOG && player.skinType != SKIN_TYPE_FUR) {
        if (player.skinType == SKIN_TYPE_PLAIN)
            outputText("<br><br>Your skin itches intensely. You gaze down as more and more hairs break forth from your skin, quickly transforming into a soft coat of fur. <b>You are now covered in " + player.furColor + " fur from head to toe.</b>")
        if (player.skinType == SKIN_TYPE_SCALES)
            outputText("<br><br>Your scales itch incessantly. You scratch, feeling them flake off to reveal a coat of " + player.furColor + " fur growing out from below! <b>You are now covered in " + player.furColor + " fur from head to toe.</b>")
        player.skinType = SKIN_TYPE_FUR
        player.skinDesc = "fur"
...
ConsumableEffects.cowTFs = function (tainted, enhanced) {
            ...
            if (player.breastRows[2].nipplesPerBreast == 1) {
                outputText(
                    "<br><br>Your third row of " +
                        player.breastDescript(2) +
                        " tingle and itch.  You pull back your " +
                        player.armor.equipmentName +
                        " and watch in shock as your " +
                        player.nippleDescript(2) +
            ...
            if (player.breastRows[3].nipplesPerBreast == 1) {
                outputText(
                    "<br><br>Your fourth row of " +
                        player.breastDescript(3) +
                        " tingle and itch.  You pull back your " +
                        player.armor.equipmentName +
                        " and watch in shock as your " +
                        player.nippleDescript(3) +
    ...
    if (tainted && player.tailType != TAIL_TYPE_COW && changes < changeLimit && rand(3) == 0) {
        if (player.tailType == TAIL_TYPE_NONE)
            outputText(
                "<br><br>You feel the flesh above your " + player.buttDescript() + " knotting and growing.  It twists and writhes around itself before flopping straight down, now shaped into a distinctly bovine form.  You have a <b>cow tail</b>.",
                false
            )
        else {
    ...
    if (tainted && player.lowerBody != LOWER_BODY_TYPE_HOOFED && player.earType == EARS_COW) {
        if (changes < changeLimit && rand(3) == 0) {
            ...
            if (player.lowerBody == LOWER_BODY_TYPE_HUMAN)
                outputText("<br><br>You stagger as your feet change, curling up into painful angry lumps of flesh.  They get tighter and tighter, harder and harder, until at last they solidify into hooves!", false)
            if (player.lowerBody == LOWER_BODY_TYPE_DOG)
                outputText("<br><br>You stagger as your paws change, curling up into painful angry lumps of flesh.  They get tighter and tighter, harder and harder, until at last they solidify into hooves!", false)
            if (player.lowerBody == LOWER_BODY_TYPE_NAGA)
                outputText("<br><br>You collapse as your sinuous snake-tail tears in half, shifting into legs.  The pain is immense, particularly in your new feet as they curl inward and transform into hooves!", false)
            //Catch-all
            if (player.lowerBody > LOWER_BODY_TYPE_NAGA)
                outputText("<br><br>You stagger as your " + player.feet() + " change, curling up into painful angry lumps of flesh.  They get tighter and tighter, harder and harder, until at last they solidify into hooves!", false)
    ...
    if (gameFlags[HAS_BLACK_NIPPLES] == 1 && changes < changeLimit && rand(3) == 0) {
        outputText(
            "<br><br>Something invisible brushes against your " + player.nippleDescript(0) + ", making you twitch.  Undoing your clothes, you take a look at your chest and find that your nipples have turned back to their natural flesh colour."
        )
        changes++
...
ConsumableEffects.succubiDelight = function (tainted) {
    ...
    if (rand(1.5) == 0 && changes < changeLimit && player.balls > 0) {
        ...
        if (player.ballSize > 10) player.ballSize -= 0.5
        //And MUCH slower
        if (player.ballSize > 24) player.ballSize -= 0.3
        //Texts
        if (player.ballSize <= 2)
            outputText(
                "<br><br>A flash of warmth passes through you and a sudden weight develops in your groin. You pause to examine the changes and your roving fingers discover your " + player.ballsDescriptLight() + " have grown larger than a human's.",
                false
            )
        if (player.ballSize > 2)
            outputText("<br><br>A sudden onset of heat envelops your groin, focusing on your " + player.sackDescript() + ". Walking becomes difficult as you discover your " + player.ballsDescriptLight() + " have enlarged again.", false)
        ...
        if (player.balls == 0) {
            ...
            outputText(
                "<br><br>Incredible pain scythes through your crotch, doubling you over. You stagger around, struggling to pull open your " +
                    player.armor.equipmentName.equipmentName +
    ...
    if (changes < changeLimit && rand(2) == 0 && player.cocks.length > 0) {
        if (player.cumMultiplier < 6 && rand(2) == 0 && changes < changeLimit) {
            ...
            if (temp < player.cumMultiplier + 0.4 * crit) {
                changes--
            } else {
...
ConsumableEffects.equineTFs = function () {
    ...
    if ((player.gender == 1 || player.gender == 3) && rand(1.5) == 0 && changes < changeLimit) {
        ...
        if (player.countCocksOfType(ENUM.CockType.HORSE) + player.countCocksOfType(ENUM.CockType.DEMON) < player.cocks.length) {
            ...
            if (player.cocks.length == 1) {
                ...
                temp3 = 0
                if (player.cocks[0].cockType == ENUM.CockType.HUMAN) {
                    outputText(
                        "<br><br>Your " +
                            player.cockDescript(0) +
                            " begins to feel strange... " +
                            player.clothedOrNakedLower("you pull down your clothes to take a look", "you look down") +
                ...
                if (player.cocks[0].cockType == ENUM.CockType.TENTACLE) {
                    outputText(
                        "<br><br>Your " +
                            player.cockDescript(0) +
                            " begins to feel odd... " +
                ...
                temp = player.changeCockType(ENUM.CockType.HORSE)
            ...
            //MULTICOCK
                ...
                if (player.hasSheath()) outputText(" Your sheath tingles and begins growing larger as the cock's base shifts to lie inside it.")
                else
                    outputText(
        ...
        if (rand(2) == 0 && changes < changeLimit && player.countCocksOfType(ENUM.CockType.HORSE) > 0) {
            temp3 = 0
            temp2 = player.cocks.length
            while (temp2 > 0) {
                temp2--
            ...
            if (player.cor + player.lib < 50) outputText(" to have such a splendid tool. You idly daydream about cunts and pussies, your " + Appearance.cockNoun(ENUM.CockType.HORSE) + " plowing them relentlessly, stuffing them pregnant with cum")
            ...
            player.modStats("lib", 0.5, "lus", 10)
        }
        //Chance of ball growth if not 3" yet
        if (rand(2) == 0 && changes < changeLimit && player.ballSize <= 3 && player.countCocksOfType(ENUM.CockType.HORSE) > 0) {
            if (player.balls == 0) {
                player.balls = 2
                player.ballSize = 1
                outputText(
    ...
    if (player.skinType != SKIN_TYPE_FUR && changes < changeLimit && rand(4) == 0 && player.tailType == TAIL_TYPE_HORSE) {
        if (player.skinType == SKIN_TYPE_PLAIN)
            outputText("<br><br>An itchy feeling springs up over every inch of your skin. As you scratch yourself madly, you feel fur grow out of your skin until <b>you have a fine coat of " + player.hairColor + "-colored fur.</b>")
        if (player.skinType == SKIN_TYPE_SCALES) {
            player.skinDesc = "fur"
            outputText(
                "<br><br>Your " +
...
ConsumableEffects.felineTFs = function () {
    ...
    if (player.lib < 80 && changes < changeLimit && rand(4) == 0) {
        //Libido gain
        //Cat dicked folks
        if (player.countCocksOfType(ENUM.CockType.CAT) > 0) {
            temp = player.findFirstCockType(ENUM.CockType.CAT)
            outputText(
                "<br><br>You feel your " +
                    player.cockDescript(temp) +
        ...
        player.modStats("lib", 1, "sen", 0.25)
        changes++
    }
    //------------
    // SEXUAL TFs
    ...
    if (rand(4) == 0 && changes < changeLimit) {
        ...
        if (player.goIntoHeat(false)) {
            if (intensified) {
                if (rand(2) == 0) outputText("<br><br>The itch inside your " + player.vaginaDescript(0) + " is growing stronger, and you desperately want to find a nice cock to massage the inside.")
                else
                    outputText(
    ...
    //Cat dangly-doo.
    if (player.cockTotal() > 0 && player.countCocksOfType(ENUM.CockType.CAT) < player.cockTotal() && (player.earType == EARS_CAT || rand(3) > 0) && (player.tailType == TAIL_TYPE_CAT || rand(3) > 0) && changes < changeLimit && rand(4) == 0) {
        //loop through and find a non-cat wang.
        for (var i = 0; i < player.cockTotal() && player.cocks[i].cockType == ENUM.CockType.CAT; i++) {}
        outputText(
            "<br><br>Your " +
                player.cockDescript(i) +
        ...
        player.cocks[i].cockType = ENUM.CockType.CAT
        player.cocks[i].knotMultiplier = 1
        changes++
    }
    //Cat penorz shrink
    if (player.countCocksOfType(ENUM.CockType.CAT) > 0 && rand(3) == 0 && changes < changeLimit && !hyperHappy) {
        //loop through and find a cat wang.
        temp = 0
        for (var j = 0; j < player.cockTotal(); j++) {
            if (player.cocks[j].cockType == ENUM.CockType.CAT && player.cocks[j].cockLength > 6) {
                temp = 1
                break
            }
        }
    ...
    if (player.earType != EARS_CAT && rand(5) == 0 && changes < changeLimit) {
        ...
        //non human to cat:
            ...
            if (rand(2) == 0) outputText("<br><br>Your ears change shape, morphing into pointed, feline ears! They swivel about reflexively as you adjust to them. <b>You now have cat ears.</b>")
            else
                outputText(
    ...
    if (player.tailType == TAIL_TYPE_CAT && player.earType == EARS_CAT && rand(5) == 0 && changes < changeLimit && player.lowerBody != LOWER_BODY_TYPE_CAT) {
        ...
        //non hoof to cat:
        else outputText("<br><br>You scream in agony as you feel the bones in your " + player.feet() + " break and begin to rearrange. When the pain fades, you feel surprisingly well-balanced. <b>You now have cat paws.</b>")
    ...
    if (
        ...
        changes < changeLimit &&
        player.lowerBody == LOWER_BODY_TYPE_CAT &&
        (player.skinType == SKIN_TYPE_FUR || (player.skinType == SKIN_TYPE_SCALES && player.dragonneScore() >= 4)) &&
    ...
    if (changes < changeLimit) {
        ...
        if (player.gender < 2) {
            if (rand(2) == 0) outputText(player.modFem(65, 1), false)
...
ConsumableEffects.humanTFs = function () {
    ...
    if (player.wingType > WING_TYPE_NONE && rand(5) == 0 && changes < changeLimit) {
        if (player.wingType == WING_TYPE_SHARK_FIN) outputText("<br><br>A wave of tightness spreads through your back, and it feels as if someone is stabbing a dagger into your spine. After a moment the pain passes, though your fin is gone!")
    ...
    if (player.cocks.length > 1 && rand(3) == 0 && changes < changeLimit) {
        ...
        outputText(
            "<br><br>You have a strange feeling as your crotch tingles. " +
                player.clothedOrNakedLower("Opening your " + player.armor.equipmentName.equipmentName, "Looking down") +
    ...
    if (player.hasCock() && changes < changeLimit) {
        if (rand(3) == 0 && player.cocks[0].cockType != ENUM.CockType.HUMAN) {
            outputText(
            ...
            player.cocks[0].cockType = ENUM.CockType.HUMAN
            changes++
        }
    }
...
ConsumableEffects.impTFs = function () {
    ...
    if (player.findPerk(PerkLib.TransformationResistance) >= 0) changeLimit--
    //Consumption text
    if (player.hasCock() > 0) outputText("The food tastes strange and corrupt - you can't really think of a better word for it, but it's unclean.")
...
ConsumableEffects.pigTFs = function (boar) {
    ...
    if (rand(3) == 0 && changes < changeLimit && player.balls > 0 && player.ballSize < 4) {
        if (player.ballSize < 3)
            outputText(
                "<br><br>A flash of warmth passes through you and a sudden weight develops in your groin. You pause to examine the changes and your roving fingers discover your " +
                    (player.balls == 4 ? "quartette" : "duo") +
                    " of [balls] have grown larger than a human’s."
            )
        else outputText("<br><br>A sudden onset of heat envelops your groin, focusing on your ballsack. Walking becomes difficult as you discover your " + (player.balls == 4 ? "quartette" : "duo") + " of testicles have enlarged again.")
        player.ballSize++
        changes++
    }
    //------------
    ...
    if (rand(4) == 0 && changes < changeLimit && player.cocks.length > 0 && player.cocks[0].cockType != ENUM.CockType.PIG) {
        if (player.cocks.length == 1) {
            //Single cock
            outputText(
                "<br><br>You feel an uncomfortable pinching sensation in your [cock]. " +
                    player.clothedOrNakedLower("You pull open your [armor]", "You look down at your exposed groin") +
            ...
            player.cocks[0].cockType = ENUM.CockType.PIG
        } else {
            //Multiple cocks
            outputText(
            ...
            player.cocks[rand(player.cocks.length + 1)].cockType = ENUM.CockType.PIG
        }
        changes++
    }
    //Gain pig ears!
...
ConsumableEffects.lizardTFs = function () {
    ...
    if (player.countCocksOfType(ENUM.CockType.LIZARD) == 0 && player.cockTotal() > 0 && changes < changeLimit && rand(4) == 0) {
        //Find the first non-lizzy dick
        for (temp2 = 0; temp2 < player.cocks.length; temp2++) {
            //Stop loopahn when dick be found
            if (player.cocks[temp2].cockType != ENUM.CockType.LIZARD) break
        }
        outputText(
            "<br><br>A slow tingle warms your groin.  Before it can progress any further, you yank back your " + player.armor.equipmentName + " to investigate.  Your " + player.cockDescript(temp2) + " is changing!  It ripples loosely from ",
            false
        )
        ...
        if (player.hasSheath()) {
            player.cocks[temp2].cockType = ENUM.CockType.LIZARD
            if (!player.hasSheath())
                outputText(
                    "<br><br>Your sheath tightens and starts to smooth out, revealing ever greater amounts of your " +
                        player.cockDescript(temp2) +
                        "'s lower portions.  After a few moments <b>your groin is no longer so animalistic – the sheath is gone.</b>",
                    false
                )
        } else player.cocks[temp2].cockType = ENUM.CockType.LIZARD
        changes++
        player.dynStats("lib", 3, "lus", 10)
    }
    //(CHANGE OTHER DICK)
    //Requires 1 lizard cock, multiple cocks
    if (player.cockTotal() > 1 && player.countCocksOfType(ENUM.CockType.LIZARD) > 0 && player.cockTotal() > player.countCocksOfType(ENUM.CockType.LIZARD) && rand(4) == 0 && changes < changeLimit) {
    ...
    if (player.countCocksOfType(ENUM.CockType.LIZARD) == 1 && player.cocks.length == 1 && rand(4) == 0 && changes < changeLimit) {
        outputText(
        ...
        if (player.skinType == SKIN_TYPE_FUR || player.skinType == SKIN_TYPE_SCALES) outputText(" and shedding " + player.skinDesc, false)
        outputText(
        ...
        player.createCock()
        player.cocks[1].cockType = ENUM.CockType.LIZARD
    ...
    //Require mammals?
    if (player.countCocksOfType(ENUM.CockType.LIZARD) == player.cockTotal() && changes < changeLimit && player.findStatusEffect(StatusEffects.Infested) >= 0) {
        outputText(
    ...
    if (player.averageNipplesPerBreast() > 1 && changes < changeLimit && rand(4) == 0) {
        outputText(
            "<br><br>A chill runs over your " + player.allBreastsDescript() + " and vanishes.  You stick a hand under your " + player.armor.equipmentName + " and discover that your extra nipples are missing!  You're down to just one per ",
            false
        )
    ...
    if (changes < changeLimit && rand(5) == 0 && player.eyeType > EYES_HUMAN) {
        if (player.eyeType == EYES_BLACK_EYES_SAND_TRAP) {
            ...
            outputText(
                "<br><br>You blink and stumble, a wave of vertigo threatening to pull your " + player.feet() + " from under you.  As you steady and open your eyes, you realize something seems different.  Your vision is changed somehow.",
                false
            )
...
ConsumableEffects.harpyTFs = function (type) {
    ...
    if (player.spe < 100 && rand(3) == 0) {
        ...
        if (player.spe < 40) player.dynStats("spe", 0.5)
        if (player.spe < 75) player.dynStats("spe", 0.5)
        player.dynStats("spe", 0.5)
    }
    ...
    if (player.str < 70 && rand(3) == 0 && changes < changeLimit) {
        ...
        //Faster until 40 str.
        if (player.str < 40) player.dynStats("str", 0.5)
        player.dynStats("str", 0.5)
    }
    ...
    if (changes < changeLimit && player.hasCock() && player.countCocksOfType(ENUM.CockType.AVIAN) < player.cockTotal() && rand(type == 1 ? 4 : 10) == 0) {
        //2.5x chance if magic seed.
        changes++
        outputText(
            "<br><br>You feel a strange tingling sensation in your cock as erection forms. You " +
                player.clothedOrNakedLower("open up your " + player.armor.equipmentName + " and", "") +
                " look down to see " +
                (player.cockTotal() == 1 ? "your cock" : "one of your cocks") +
        ...
        for (var i = 0; i < player.cocks.length; i++) {
            if (player.cocks[i].cockType != ENUM.CockType.AVIAN) {
                player.cocks[i].cockType = ENUM.CockType.AVIAN
                return
            }
        }
    }
...
ConsumableEffects.minotaurTFs = function () {
    ...
    if (rand(3) == 0 && changes < changeLimit) {
        ...
        //stronger characters gain less
            ...
            if (player.str >= 75) player.dynStats("str", 0.5)
            //faster from 50-75
    ...
    if (rand(3) == 0 && changes < changeLimit) {
        ...
        //stronger characters gain less
            ...
            //small growth if over 75
            if (player.tou >= 75) player.dynStats("tou", 0.5)
            //faster from 50-75
    ...
    if (changes < changeLimit && rand(2) == 0 && player.ballSize <= 5 && player.countCocksOfType(ENUM.CockType.HORSE) > 0) {
        //Chance of ball growth if not 3" yet
        if (player.balls == 0) {
            player.balls = 2
            player.ballSize = 1
        ...
        if (player.vaginas.length > 0) {
            if (player.vaginas[0].vaginalLooseness > VAGINA_LOOSENESS_TIGHT) {
                ...
                if (player.cocks.length == 0) {
                    ...
                    player.cocks[0].cockType = ENUM.CockType.HORSE
                    player.clitLength = 0.25
                }
                player.genderCheck()
            }
    ...
    if (changes < changeLimit && rand(2) == 0 && player.cocks.length > 0) {
        var selectedCock = -1
        for (var i = 0; i < player.cocks.length; i++) {
            if (player.cocks[i].cockType == ENUM.CockType.HORSE && (player.cocks[i].cockLength < 36 || player.cocks[i].cockThickness < 5)) {
                selectedCock = i
                break
            }
        }
    ...
    if (player.cocks.length > 0 && rand(2) == 0 && changes < changeLimit) {
        ...
        for (var indexI = 0; indexI < player.cocks.length; indexI++) {
            if (player.cocks[indexI].cockType != ENUM.CockType.HORSE) {
        ...
        if (selectedCockValue != -1) {
            ...
            if (player.cocks[selectedCockValue].cockType == ENUM.CockType.HUMAN /* || player.cocks[selectedCockValue].cockType.Index > 2 */)
                outputText(
                    "<br><br>Your " +
                        player.cockDescript(selectedCockValue) +
                        " begins to feel strange... you pull down your pants to take a look and see it darkening as you feel a tightness near the base where your skin seems to be bunching up.  A sheath begins forming around your cock's base, tightening and pulling your cock inside its depths.  A hot feeling envelops your member as it suddenly grows into a horse penis, dwarfing its old size.  The skin is mottled brown and black and feels more sensitive than normal.  Your hands are irresistibly drawn to it, and you jerk yourself off, splattering cum with intense force.",
                    false
                )
            //Text for dogdicks
            if (player.cocks[selectedCockValue].cockType == ENUM.CockType.DOG)
                outputText(
                    "<br><br>Your " +
                        Appearance.cockNoun(ENUM.CockType.DOG) +
            ...
            player.cocks[selectedCockValue].cockType = ENUM.CockType.HORSE
            player.increaseCock(selectedCockValue, 4)
            player.dynStats("lib", 5, "sen", 4)
            player.changeLust(35)
            outputText("<b>  You now have a")
            if (player.countCocksOfType(ENUM.CockType.HORSE) > 1) outputText("nother")
    ...
    if (rand(4) == 0 && changes < changeLimit && player.findStatusEffect(StatusEffects.Dysfunction) < 0) {
        if (player.cocks.length > 0)
            outputText("<br><br>Your " + player.cockDescript(0) + " tingles abruptly, then stops.  Worried, you reach down to check it, only to discover that it feels... numb.  It will be very hard to masturbate like this.", false)
        else if (player.hasVagina())
            outputText("<br><br>Your " + player.vaginaDescript(0) + " tingles abruptly, then stops.  Worried, you reach down to check it, only to discover that it feels... numb.  It will be very hard to masturbate like this.", false)
        if (player.cocks.length > 0 || player.hasVagina()) {
    ...
    if (player.earType == EARS_COW && player.lowerBody == LOWER_BODY_TYPE_HOOFED && player.tallness >= 90 && changes < changeLimit && rand(3) == 0) {
...
ConsumableEffects.slimeTFs = function () {
    ...
    //libido up to 80
    if (player.lib < 80) {
        player.dynStats("lib", 0.5 + (90 - player.lib) / 10)
    ...
    if (player.wingType > WING_TYPE_NONE) {
        if (player.wingType == WING_TYPE_SHARK_FIN)
            outputText("<br><br>You sigh, feeling a hot wet tingling down your back.  It tickles slightly as you feel your fin slowly turn to sludge, dripping to the ground as your body becomes more goo-like.", false)
    ...
    if (player.hairType == 3 && (player.skinDesc != "skin" || player.skinAdj != "slimy")) {
        if (player.skinType == SKIN_TYPE_PLAIN)
            outputText("<br><br>You sigh, feeling your " + player.armorName + " sink into you as your skin becomes less solid, gooey even.  You realize your entire body has become semi-solid and partly liquid!", false)
    ...
    if (player.longestCockLength() >= 3.5 && player.hasCock() && rand(2) == 0 && changes < changeLimit) {
        ...
        while (temp < player.cockTotal()) {
            if (player.cocks[temp].cockLength >= 3.5) {
                ...
                if (player.cocks[temp].cockLength < 6 && player.cocks[temp].cockLength >= 2.9) {
                    player.cocks[temp].cockLength -= 0.5
                    if (player.cocks[temp].cockThickness * 6 > player.cocks[temp].cockLength) player.cocks[temp].cockThickness -= 0.2
                    if (player.cocks[temp].cockThickness * 8 > player.cocks[temp].cockLength) player.cocks[temp].cockThickness -= 0.2
                    if (player.cocks[temp].cockThickness < 0.5) player.cocks[temp].cockThickness = 0.5
                }
                player.cocks[temp].cockLength -= 0.5
