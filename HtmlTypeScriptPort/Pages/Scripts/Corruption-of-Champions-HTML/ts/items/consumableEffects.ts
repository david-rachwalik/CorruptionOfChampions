...
abstract class ConsumableEffects {
    ...
    static fishFillet() {
        clearOutput()
        if (!COMBAT.inCombat) outputText("You sit down and unwrap your fish fillet. It's perfectly flaky, allowing you to break it off in bite-sized chunks.  The salty meal disappears quickly, and your stomach gives an appreciative gurgle.")
        //(In combat?)
        else
            outputText(
    ...
    static lustDraft(fuck: boolean): void {
        ...
        if (liveData.player.lust >= liveData.player.maxLust() && !COMBAT.inCombat) {
            ...
            if (liveData.player.gender == 0) outputText("body begins to quiver with orgasmic bliss.  ")
    ...
    static vitalityTincture() {
        ...
        if (UTIL.rand(3) == 0) outputText(liveData.player.modTone(95, 3))
    ...
    static scholarsTea() {
        ...
        if (UTIL.rand(3) == 0) outputText(liveData.player.modTone(15, 1))
        //Now NERFED!
    ...
    static minotaurCum(purified: boolean): void {
        ...
        if (liveData.gameFlags[FLAG.MINOTAUR_CUM_ADDICTION_STATE] == 0 && liveData.player.findPerk(PerkLib.MinotaurCumResistance) < 0) outputText("potent musk washes over you.")
        else outputText("heavenly scent fills your nostrils.")
        if (!purified) {
            if (liveData.gameFlags[FLAG.MINOTAUR_CUM_ADDICTION_TRACKER] < 50) outputText("  It makes you feel dizzy, ditzy, and placid.")
        ...
        if (liveData.player.cocks.length > 0) {
            outputText("  ")
            if (liveData.player.cockTotal() == 1) outputText("Y")
        ...
        if (liveData.player.hasVagina()) {
            ...
            if (liveData.player.clitLength < 3) outputText("parting your lips.")
            else outputText("bursting free of your lips and bobbing under its own weight.")
            if (liveData.player.vaginas[0].vaginalWetness <= ENUM.VaginalWetnessType.VAGINA_WETNESS_NORMAL) outputText("  Wetness builds inside you as your " + liveData.player.vaginaDescript(0) + " tingles and aches to be filled.")
            ...
            else if (liveData.player.vaginas[0].vaginalWetness <= ENUM.VaginalWetnessType.VAGINA_WETNESS_DROOLING) outputText("  Wet fluids leak down your thighs as your body reacts to this new stimulus.")
