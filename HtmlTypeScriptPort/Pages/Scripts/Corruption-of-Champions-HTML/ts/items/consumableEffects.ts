...
abstract class ConsumableEffects {
    ...
    static fishFillet() {
        GUI.clearOutput()
        if (!COMBAT.inCombat) GUI.outputText("You sit down and unwrap your fish fillet. It's perfectly flaky, allowing you to break it off in bite-sized chunks.  The salty meal disappears quickly, and your stomach gives an appreciative gurgle.")
        //(In combat?)
        else
            GUI.outputText(
    ...
    static minotaurCum(purified: boolean): void {
        ...
        if (liveData.gameFlags[FLAG.MINOTAUR_CUM_ADDICTION_STATE] == 0 && liveData.player.findPerk(PerkLib.MinotaurCumResistance) < 0) GUI.outputText("potent musk washes over you.")
        ...
        if (liveData.player.hasVagina()) {
            ...
            if (liveData.player.vaginas[0].vaginalWetness <= ENUM.VaginalWetnessType.VAGINA_WETNESS_NORMAL) GUI.outputText("  Wetness builds inside you as your " + liveData.player.vaginaDescript(0) + " tingles and aches to be filled.")
            ...
            else if (liveData.player.vaginas[0].vaginalWetness <= ENUM.VaginalWetnessType.VAGINA_WETNESS_DROOLING) GUI.outputText("  Wet fluids leak down your thighs as your body reacts to this new stimulus.")
