...
let fishFillet = function () {
    ...
    //Blew up factory
    if (liveData.gameFlags[FACTORY_SHUTDOWN] == 2) liveData.player.modStats("cor", 0.5)
    //Turned off factory
    else if (liveData.gameFlags[FACTORY_SHUTDOWN] == 1) liveData.player.modStats("cor", -0.1)
    //Normal
...
let lustDraft = function (fuck) {
    ...
    if (liveData.player.lust >= liveData.player.maxLust() && !inCombat) {
        ...
        if (liveData.player.gender == 0) outputText("body begins to quiver with orgasmic bliss.  ", false)
...
//Hair Dyes
let hairDye = function (newColor: string) {
...
//Skin Oils
let skinOil = function (newColor: string) {
    if (liveData.player.skinTone == newColor) {
        outputText(
            "You " + liveData.player.clothedOrNaked("take a second to disrobe before uncorking the bottle of oil and rubbing", "uncork the bottle of oil and rub") + " the smooth liquid across your body. Once you’ve finished you feel rejuvenated. "
        )
...
//Body Lotions
let bodyLotion = function (newAdj: string) {
...
function liquidDesc(_adj: string) {
    var liquidDesc = ""
    var liquidArrays = []
    switch (_adj) {
        case "smooth":
...
let minotaurCum = function (purified) {
    ...
    if (liveData.player.cocks.length > 0) {
        outputText("  ", false)
        if (liveData.player.cockTotal() == 1) outputText("Y", false)
    ...
    if (liveData.player.hasVagina()) {
        ...
        if (liveData.player.clitLength < 3) outputText("parting your lips.", false)
        else outputText("bursting free of your lips and bobbing under its own weight.", false)
        if (liveData.player.vaginas[0].vaginalWetness <= VAGINA_WETNESS_NORMAL) outputText("  Wetness builds inside you as your " + liveData.player.vaginaDescript(0) + " tingles and aches to be filled.", false)
        else if (liveData.player.vaginas[0].vaginalWetness <= VAGINA_WETNESS_SLICK) outputText("  A trickle of wetness escapes your " + liveData.player.vaginaDescript(0) + " as your body reacts to the desire burning inside you.", false)
        else if (liveData.player.vaginas[0].vaginalWetness <= VAGINA_WETNESS_DROOLING) outputText("  Wet fluids leak down your thighs as your body reacts to this new stimulus.", false)
