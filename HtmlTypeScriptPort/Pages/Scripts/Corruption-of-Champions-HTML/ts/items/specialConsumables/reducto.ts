...
abstract class ReductoMenu {
    ...
    static reductoBalls() {
        clearOutput()
        outputText("You smear the foul-smelling paste onto your " + liveData.player.sackDescript() + ".  It feels cool at first but rapidly warms to an uncomfortable level of heat.\n\n")
        liveData.player.ballSize -= UTIL.rand(4) + 2
        if (liveData.player.ballSize < 1) liveData.player.ballSize = 1
        outputText("You feel your scrotum shift, shrinking down along with your " + liveData.player.ballsDescriptLight() + ".  Within a few seconds the paste has been totally absorbed and the shrinking stops.")
    ...
    static reductoBreasts() {
        clearOutput()
        outputText("You smear the foul-smelling ointment all over your " + liveData.player.allBreastsDescript() + ", covering them entirely as the paste begins to get absorbed into your " + liveData.player.skinDesc + ".\n")
    ...
    static reductoButt() {
        clearOutput()
        outputText("You smear the foul-smelling paste onto your " + liveData.player.buttDescript() + ".  It feels cool at first but rapidly warms to an uncomfortable level of heat.\n\n")
        if (liveData.player.buttRating >= 15) {
            ...
            outputText("You feel much lighter as your " + liveData.player.buttDescript() + " jiggles slightly, adjusting to its smaller size.")
        } else {
            liveData.player.buttRating -= UTIL.rand(3) + 1
            if (liveData.player.buttRating < 1) liveData.player.buttRating = 1
            outputText("After a few seconds your " + liveData.player.buttDescript() + " has shrunk to a much smaller size!")
        }
        liveData.player.dynStats(["lib", -2])
        liveData.player.changeLust(-10)
        Inventory.itemGoNext()
    ...
    static reductoClit() {
        clearOutput()
        outputText(
            "You carefully apply the paste to your " + liveData.player.clitDescript() + ", being very careful to avoid getting it on your " + liveData.player.vaginaDescript(0) + ".  It burns with heat as it begins to make its effects known...\n\n"
        )
        ...
        outputText("Your " + liveData.player.clitDescript() + " shrinks rapidly, dwindling down to almost half its old size before it finishes absorbing the paste.")
        liveData.player.dynStats(["sen", 2])
        liveData.player.changeLust(10)
        Inventory.itemGoNext()
    }
    ...
    static reductoCock() {
        ...
        if (liveData.player.cocks[0].cockType == ENUM.CockType.BEE) {
            ...
            if (liveData.player.cocks.length == 1) {
                outputText("Your " + liveData.player.cockDescript(0) + " twitches as it shrinks, disappearing steadily into your " + (liveData.player.hasSheath() ? "sheath" : "crotch") + " until it has lost about a third of its old size.")
                liveData.player.cocks[0].cockLength *= 2 / 3
                liveData.player.cocks[0].cockThickness *= 2 / 3
            } else {
                //MULTI
                outputText(
                    "Your " + liveData.player.multiCockDescriptLight() + " twitch and shrink, each member steadily disappearing into your " + (liveData.player.hasSheath() ? "sheath" : "crotch") + " until they've lost about a third of their old size."
                )
    ...
    static reductoHips() {
        ...
        if (liveData.player.hipRating >= 15) {
            ...
            if (liveData.player.hipRating < 1) liveData.player.hipRating = 1
    ...
    static reductoNipples() {
        clearOutput()
        outputText("You rub the paste evenly over your " + liveData.player.nippleDescript(0) + "s, being sure to cover them completely.\n\n")
        //Shrink
        if (liveData.player.nippleLength / 2 < 0.25) {
            ...
            outputText("Your " + liveData.player.nippleDescript(0) + "s get smaller and smaller, stopping when they are roughly half their previous size.")
