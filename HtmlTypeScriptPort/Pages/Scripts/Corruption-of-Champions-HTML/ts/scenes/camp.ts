...
abstract class Camp {
    ...
    static doCamp() {
        ...
        if (liveData.time.days < 10) outputText("Your campsite is fairly simple at the moment.  Your tent and bedroll are set in front of the rocks that lead to the portal.  You have a small fire pit as well.  ")
        ...
        //}
        if (liveData.time.days >= 20) outputText("You've even managed to carve some artwork into the rocks around the camp's perimeter.<br><br>")
        ...
        if (liveData.time.hours < 6 || liveData.time.hours > 20)
            ...
            if (liveData.time.hours == 19) outputText("The sun is close to the horizon, getting ready to set. ")
            if (liveData.time.hours == 20) outputText("The sun has already set below the horizon. The sky glows orange. ")
    ...
    */
    ...
    static campFollowersMenu() {
        ...
        GUI.displaySprite()
        GUI.menu()
        if (liveData.gameFlags[JOJO_CAMP] == 1) GUI.addButton(0, "Jojo", JojoScene.jojoCamp, null, null, null, "Go find Jojo around the edges of your camp and meditate with him or talk about watch duty.")
        if (liveData.gameFlags[RATHAZUL_CAMP] > 0) GUI.addButton(1, "Rathazul", RathazulScene.campRathazul, null, null, null, "Visit with Rathazul to see what alchemical supplies and services he has available at the moment.")
    ...
    static followersCount() {
        var count = 0
        if (liveData.gameFlags[JOJO_CAMP] > 0 && liveData.gameFlags[JOJO_CORRUPTION_STAGE] < 5) count++
        if (liveData.gameFlags[RATHAZUL_CAMP] > 0) count++
        return count
    }
