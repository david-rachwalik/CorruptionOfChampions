...
function Jojo() {
    ...
    if (gameFlags[JOJO_CORRUPTION_STAGE] >= 5) this.battleDesc = "He's naked, with a large tainted throbbing member bouncing at attention. A fuzzy sack with painfully large looking balls dangles between his legs."
...
Jojo.selfCorruption = function () {
    ...
    if (monster.lust >= monster.maxLust()) {
        return
    } else if (monster.lust >= 85) outputText("The mouse is panting and softly whining, each movement seeming to make his bulge more pronounced. You don't think he can hold out much longer. ")
    else if (monster.lust >= 70) outputText("The mouse is having trouble moving due to the rigid protrusion from his groin. ")
    else if (monster.lust >= 60) outputText("The mouse's eyes constantly dart over your most sexual parts, betraying his lust. ")
    else if (monster.lust >= 50) outputText("The mouse's skin remains flushed with the beginnings of arousal. ")
}
...
JojoScene.routeJojoEncounter = function () {
    ...
    if (gameFlags[JOJO_CAMP] > 0 || flags[JOJO_DEAD_OR_GONE] > 0) {
        ...
        //Mod toughness
        if (player.tou < 50) player.modStats("tou", 0.5)
        //Mod intelligence
        if (player.inte < 50) player.modStats("int", 1)
        else if (player.inte < 75) player.modStats("int", 0.5)
        return
    }
    //If Jojo is encounterable.
    if (gameFlags[JOJO_CORRUPTION_STAGE] == 0) {
        gameFlags[JOJO_CORRUPTION_STAGE] = 1
        if (player.cor < 25) JojoScene.lowCorruptionJojoEncounter()
        else JojoScene.highCorruptionJojoEncounter()
...
JojoScene.lowCorruptionJojoEncounter = function () {
    ...
    //Variant checks
    if (player.tou >= 50 && player.isBiped()) outputText(" You take a few steps back, momentarily dazed. Shaking it off, you ready your [weapon] and assume a fighting stance.<br><br>")
    else if (player.tou < 50 && player.isBiped()) outputText("The force of the blow knocks you flat on your [ass]. Shaking it off, you immediately climb to your feet and take on a fighting stance.<br><br>")
    else if (player.isTaur()) outputText("The blow does little more than leave you momentarily dazed but isn’t enough to knock you over. You shake it off and ready your [weapon] as you assume a fighting stance.<br><br>")
    // Was originally isNaga() only, but this will also cover Drider just as well
    else outputText("You recoil as you are struck, but the force of the blow does little more than leave you momentarily dazed. You assume a fighting stance, ready to defend yourself.<br><br>")
...
JojoScene.meditateInForest = function () {
    ...
    if (gameFlags[JOJO_MEDITATION_COUNTER] >= 5) {
        ...
        if (flags[FUCK_FLOWER_LEVEL] >= 4 && flags[FUCK_FLOWER_KILLED] == 0 && gameFlags[JOJO_MEDITATION_COUNTER] % 5 == 0) {
            ...
            return
        } else outputText('"<i>It seems you have quite a talent for this. We should meditate together more often.</i>"')
    }
    ...
    if (gameFlags[JOJO_MEDITATION_COUNTER] % 5 == 0) {
        ...
        return
    } else outputText("<br><br>He bows his head sadly and dismisses you.")
    doNext(Camp.returnToCampUseTwoHours)
}
...
JojoScene.jojoTalkVillage = function () {
    ...
    if (player.cor < 40) outputText("Looks like Jojo's childhood wasn't so bad... you thank the mouse morph monk for sharing his treasured memories with you now that the conversation is over.<br><br>")
    else
        outputText(
...
JojoScene.wormRemoval = function () {
    ...
    if (player.HP > player.maxHP() * 0.5) player.HP = Math.floor(player.maxHP() * 0.5)
...
JojoScene.suckJojosCock = function () {
    ...
    else outputText("<br><br>Jojo collapses on his back, winded. When he comes up, you say that was delicious and to treat you next time as well. Jojo’s too tired to say anything, but smiles in response.")
    outputText("<br><br>After a good while of rest, " + player.clothedOrNaked("the two of you get redressed and", "Jojo gets redressed and the two of you") + " return to your camp.")
...
JojoScene.loseToJojo = function () {
    ...
    if (gameFlags[JOJO_CORRUPTION_STAGE] <= 3) {
        ...
        if (player.countCocksOfType(ENUM.CockType.HORSE) > 0) player.lib += 3
        if (player.countCocksOfType(ENUM.CockType.DOG) > 0) player.lib += 2
...
JojoScene.postCombatRape = function () {
    ...
    if (gameFlags[JOJO_CORRUPTION_STAGE] == 5) outputText("fuck your violent little slut senseless. ")
    else outputText("teach the uppity monk a lesson...<br><br>")
...
JojoScene.jojosFirstRape = function () {
    ...
    if (player.gender == 1) {
        ...
        if (player.wetness() < 2) outputText("but the sensation of you grinding your folds against him momemtarily breaks his will. ")
        else outputText("but the feeling of your " + player.vaginaDescript(0) + " grinding against his hardening cock robs him of any will. ")
        ...
        if (player.cockTotal() > 0) {
            ...
            if (player.cockTotal() == 1) {
                ...
                if (player.cumQ() >= 25 && player.cumQ() < 250) {
                    if (player.countCocksOfType(ENUM.CockType.HORSE) == 1) outputText(" Your " + player.cockDescript(0) + " flares wildly as musky horse-cum erupts from it, splattering over Jojo. ")
                    ...
                    if (player.cocks[0].cockType == ENUM.CockType.HUMAN || player.cocks[0].cockType > 2) outputText("Your body tenses and cums a thick eruption far beyond what a normal human could produce. Jojo is splattered with the stuff. ")
                }
                if (player.cumQ() >= 250) {
...
JojoScene.corruptJojoBJGentle = function () {
    ...
    if (player.biggestTitSize() >= 2) outputText("You draw him to your bosom and kiss his forehead and then stand and go about your duties, leaving him to recover from the intense encounter and then retreat back into the jungle.  ")
...
JojoScene.corruptJojoVaginalSmother = function () {
    ...
    if (player.vaginas.wetness > 4) outputText("You moan as he works, your juices flowing liberally across his muzzle and into his mouth and nose, making him struggle not to drown in your pleasure as he focuses on giving you even more so.  ")
    else if (rand(2) == 0)
        outputText(
