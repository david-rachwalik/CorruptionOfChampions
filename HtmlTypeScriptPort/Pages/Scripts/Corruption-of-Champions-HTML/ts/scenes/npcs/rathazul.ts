...
abstract class RathazulScene {
    static encounterRathazul() {
        ...
        /*if (flags[MARBLE_PURIFICATION_STAGE] == 2 && liveData.gameFlags[RATHAZUL_MET] >= 0) {
        ...
        if (liveData.gameFlags[RATHAZUL_MET] > 0) {
            ...
            liveData.gameFlags[RATHAZUL_MET] = 1
        }
        //Camp offer!
        if (liveData.gameFlags[RATHAZUL_PURCHASE_COUNTER] >= 3 && liveData.gameFlags[RATHAZUL_CAMP] == 0 && liveData.player.cor < 75) {
    ...
    static rathazulMoveToCamp() {
        ...
        liveData.gameFlags[RATHAZUL_CAMP] = 1
        GUI.doNext(Camp.returnToCampUseOneHour)
    }
    ...
    static campRathazul() {
        ...
        if (!offered) {
            ...
            if (liveData.gameFlags[RATHAZUL_CAMP] > 0) GUI.doNext(Camp.campFollowersMenu)
            else GUI.doNext(liveData.playerMenu)
        }
    }

    static returnToRathazulMenu() {
        if (liveData.gameFlags[RATHAZUL_CAMP] > 0) this.campRathazul()
    ...
    static rathazulWorkOffer() {
        ...
        if (liveData.player.hasItem(Items.Materials.GreenGel)) {
            //Gel
            if (liveData.gameFlags[RATHAZUL_ARMOUR_COUNTER] == 0)
                outputText(
            ...
            else outputText('He pipes up with a bit of excitement in his voice, "<i>With just five pieces of slime-gel I could make another suit of armor...</i>"<br>')
        ...
        /*if (liveData.player.hasItem(Items.Materials.SpiderSilk) && flags[RATHAZUL_SILK_ARMOR_COUNTDOWN] + flags[UNKNOWN_FLAG_NUMBER_00275] == 0) { //SPOIDAH
            showArmorMenu = true;
            totalOffers++;
            outputText("\"<i>Oooh, is that some webbing from a giant spider or spider-morph? Most excellent! With a little bit of alchemical treatment, it is possible I could loosen the fibers enough to weave them into something truly magnificent - armor, or even a marvelous robe,</i>\" offers this.<br><br>");
        }
        ...
        if (liveData.player.gems >= 50) {
            ...
            if (liveData.gameFlags[RATHAZUL_PURCHASE_COUNTER] >= 8) outputText("I should be able to make exotic-colored dyes if you're interested. ")
        ...
        if (liveData.gameFlags[RATHAZUL_PURCHASE_COUNTER] >= 5) {
            outputText(
        ...
        //Reducto
        if (liveData.gameFlags[RATHAZUL_CAMP] > 0 && liveData.gameFlags[RATHAZUL_PURCHASE_COUNTER]) {
            outputText(
            ...
            //outputText(liveData.gameFlags[AMILY_MET_RATHAZUL] >= 2 ? "50" : 100);
        ...
        //Vines
        /*if (player.keyItemv1("Marae's Lethicite") > 0 && player.findStatusAffect(StatusAffects.DefenseCanopy) < 0 && liveData.gameFlags[RATHAZUL_CAMP] > 0) {
        ...
        if (totalOffers > 0) {
            ...
            if (liveData.gameFlags[RATHAZUL_PURCHASE_COUNTER] >= 5) GUI.addButton(7, "ProLactaid", this.rathazulMakesMilkPotion, null, null, null, "Ask him to brew a special lactation potion. <br><br>Cost: 250 Gems <br>Needs 5 Lactaids and 2 Purified LaBovas.");
            if (liveData.gameFlags[RATHAZUL_PURCHASE_COUNTER] >= 5) GUI.addButton(8, "Taurinum", this.rathazulMakesTaurPotion, null, null, null, "Ask him to brew a special potion that could aid in becoming a centaur. <br><br>Cost: 100 Gems <br>Needs 2 Equinum and 1 Minotaur Blood.");
            ...
            if (liveData.gameFlags[RATHAZUL_CAMP] > 0) GUI.addButton(14, "Leave", Camp.campFollowersMenu)
    ...
    static craftOozeArmor() {
        ...
        liveData.gameFlags[RATHAZUL_PURCHASE_COUNTER]++
    ...
    static craftCarapace() {
        ...
        if (liveData.player.cockTotal() > 0 && liveData.player.biggestCockArea() >= 40)
            outputText("The silken material does little to hide the bulge of your groin, if anything it looks a little lewd. Rathazul mumbles and looks away, shaking his head.<br><br>")
        if (liveData.player.biggestTitSize() >= 8) outputText("Your " + liveData.player.chestDesc() + " barely fit into the breastplate, leaving you displaying a large amount of jiggling cleavage.<br><br>")
    ...
    static chooseArmorOrRobes(armorType) {
        ...
        if (liveData.gameFlags[RATHAZUL_CAMP] > 0) outputText("You turn back to the center of your camp")
    ...
    static collectRathazulArmor() {
        ...
        if (liveData.gameFlags[9999] == 1) outputText("armor")
        else if (liveData.gameFlags[9999] == 2) outputText("robe")
    ...
    static craftMaraeArmor(divine) {
        ...
        if (!divine) {
            ...
            if (liveData.player.mf("m", "f") == "f") outputText(" with stockings and garters")
    ...
    static buyDyes() {
        ...
        if (liveData.gameFlags[RATHAZUL_PURCHASE_COUNTER] >= 8) {
        ...
        if (liveData.gameFlags[RATHAZUL_PURCHASE_COUNTER] >= 12) {
    ...
    static purificationByRathazulBegin() {
        outputText('Hoping the rodent-morph alchemist can assist you, you waste no time in approaching him. Rathazul looks up when he sees you, raising an eye curiously. "<i>Is something the matter, ' + liveData.player.name + '?</i>"')
        outputText(
