...
abstract class MarbleScene {
    static marbleStatusChange(affection: number, addiction: number, isAddicted = -1) {
    ...
    /*MarbleScene.encounterMarbleInitially = function() {
        GUI.clearOutput();
        GUI.displaySprite("marble");
        liveData.gameFlags[FLAG.MARBLE_MET] = 0.5;
        
        GUI.outputText("While exploring at Whitney's farm, you run across the furry southern belle almost immediately. She looks like she has a job for you.<br><br>");
        GUI.outputText("Whitney tells you that one of her barn's residents, a cow-girl named Marble, is sore from overusing the milk machines.  She asks you to go and give the cow-girl a gentler touch from a living being.<br><br>");
        //(description of barn may need to be edited, I don't know what it's supposed to look like)
        GUI.outputText("You walk in to Whitney's barn and head over to a series of small rooms for the cow-girls. You find Marble's room and knock on the door. A friendly earthy female voice calls out in response and invites you in. Inside is a rather pleasant little room. There are several shelves on the walls and a small sitting table in the corner with seating for two.  A large portion of the room is dominated by a large bed, the owner filling most of it. Lastly, you notice a mini-dresser next to the bed.  The room's owner looks over at you and starts, \"<i>Oh, I've never met you before.</i>\"\n\nAs she gets up, you are given a chance to get a good look at her.  She is over six feet tall, with long brown hair tipped with two cow horns and a pair of cow ears in place of normal human ones.  Rounding out her relatively unchanged face are a pair of deep, brown eyes.  She is wearing only a short plain skirt, so you get a full frontal view of her two HH-cup assets. They look rather sore right now, with big red circles around her puffy nipples.  Her hands and arms appear mostly human save for thick-looking nails. A soft 'clop' brings your eyes down to see that she is covered in thick, dark blond fur going from at least mid-way down her thighs to where a human's feet normally would be, in place of which are hooves.  A cow tail with a bow tied on it swings between her legs.<br><br>");
        if (liveData.gameFlags[CODEX_ENTRY_LABOVINES] <= 0) {
            liveData.gameFlags[CODEX_ENTRY_LABOVINES] = 1;
            GUI.outputText("<b>New codex entry unlocked: Lacta Bovines/Cowgirl!</b>\n\n")
        }
        //(if player height is under 5 feet)
        if (player.tallness < 60) {
            GUI.outputText("She looks down at you with a smile and says \"<i>Aww, you're so cute!  Did you come for my milk?  I'm always happy to give it, but since I'm kinda sore right now, you'll have to be gentle. Okay little one?</i>\"  She moves towards you and tries to pick you up.");
            //- player chooses resist or don't resist
            GUI.menu();
            GUI.addButton(0, "Let Her", MarbleScene.marblePicksYouUpInitially);
            GUI.addButton(1, "Don't", MarbleScene.resistMarbleInitially);
            return;
        }
        GUI.outputText("\"<i>My name's Marble, what's yours?</i>\" she asks you.  You introduce yourself and exchange a few pleasantries before she asks how she can help you.  You tell her that you actually came to help her, explaining that Whitney said she could use a gentle touch.  \"<i>Oh that would be nice</i>\", she says \"<i>Spending the night connected to the milking machine was a mistake, and now I need something gentle.</i>\"  How will you help her?", false);
    ...
    MarbleScene.resistMarbleInitially = function() {
        ...
        //(player chose resist)
        GUI.clearOutput();
        GUI.outputText("Surprised by your resistance, she pulls back and apologizes for being presumptuous.  ", false);
