...
class SandTrap extends Creature {
    constructor() {
        ...
        if (UTIL.rand(3) == 0) liveData.gameFlags[FLAG.FERTILE_SANDTRAP] = 1
        //Name and references
        this.a = "the "
    ...
    trapLevel(amount = 0) {
        if (amount != 0) {
            ...
            if (liveData.gameFlags[FLAG.TRAP_LEVEL] < 1) liveData.gameFlags[FLAG.TRAP_LEVEL] = 1
            if (liveData.gameFlags[FLAG.TRAP_LEVEL] > 4) liveData.gameFlags[FLAG.TRAP_LEVEL] = 4
        }
    ...
    sandTrapWait() {
        ...
        if (this.trapLevel() == 4) {
            GUI.outputText(
                "<br><br>You eye the ground above you.  The edge of the pit is too sheer, the ground too unstable... although it looks like you can fight against the currents carrying you further down, it seems impossible to gain freedom with the sand under the monster's spell."
            )
        }
    ...
    override doAI() {
        ...
        if (liveData.gameFlags[FLAG.TRAP_LEVEL] == 4 && liveData.gameFlags[FLAG.CLIMBED_TRAP_THIS_ROUND] == 0) this.grabAttack()
        ...
        else liveData.gameFlags[FLAG.CLIMBED_TRAP_THIS_ROUND] = 0
    ...
    grabAttack() {
        ...
        //Quicksand attack success: (Speed and Strength loss, ability to fly free lost)
            ...
            if (liveData.player.canFly()) GUI.outputText("  You try to wrench yourself free by flapping your wings, but it is hopeless.  You are well and truly snared.")
            // Autosnare. We set the climbed status so it doesn't double-pull us down
            this.trapLevel(-1)
            if (liveData.gameFlags[FLAG.CLIMBED_TRAP_THIS_ROUND] == 0) liveData.gameFlags[FLAG.CLIMBED_TRAP_THIS_ROUND] = 1
        }
    }
    ...
    sandTrapLoss() {
        ...
        if (liveData.player.hasCock()) GUI.doNext(SandTrapScene.cockLoseToSandTrap)
        else if (liveData.player.hasVagina()) GUI.doNext(SandTrapScene.chicksLoseToSandTrap)
        else GUI.doNext(SandTrapScene.genderlessLoseToSandTrap)
    }
}
...
abstract class SandTrapScene {
    ...
    static sandTrapWin() {
        ...
        if (liveData.player.lust >= 33) {
            ...
            if (liveData.player.hasCock() && liveData.player.str >= 60) GUI.addButton(1, "UseYourCock", this.cockWin) //stickWanginSandgina
            if (liveData.player.hasVagina()) GUI.addButton(2, "RideVaginal", this.vaginaWin) //rideDatSandTarp
            if (liveData.player.gender > 0) GUI.addButton(3, "Handjob", this.handjobWin) // useSandTarpsHands
    ...
    static cockWin() {
        ...
        if (liveData.player.balls > 0) GUI.outputText(", makes your " + liveData.player.ballsDescriptLight() + " swell in readiness,")
        GUI.outputText(
        ...
        //Megacock:
            ...
            GUI.outputText(
                "<br><br>The creature's moist sex is tight and you honestly doubt it can even take your obscenely massive tool.  Never say never, though.  You work your way in carefully, letting it get used to your girth, slowly working it loose enough to take your " +
                    liveData.player.cockDescript(0) +
            ...
            if (liveData.player.balls > 0) GUI.outputText("  Your " + liveData.player.ballsDescriptLight() + " slap against the creature's wet opening.  ")
            GUI.outputText(
        ...
        GUI.outputText(
            "<br><br>\"<i>Hhhhyes!  That'sz it lion, oh do me, do it to me hard!  Give me yyyyour strong spunk - uh! give hhit to me szo I can sink yyour children into evvryone I catch - yes!  Hhhhso the whole world can know hhwhat a " +
        ...
        if (liveData.player.balls > 0) GUI.outputText("balls ache and your ")
        GUI.outputText(
        ...
        GUI.outputText(
            '<br><br>"<i>Thank you for your seed, lion,</i>" its sleepy words reach you as you pull yourself clear of the hollow and step out towards camp.  "<i>Perhaps next time we meet I will return the favor, hmm?</i>"  In your wearily tranquil fug you can\'t tell if it intends this as an invitation or a threat.'
        )
    ...
    static vaginaWin() {
        ...
        GUI.outputText(
            '<br><br>"<i>Pleasze...</i>" the sandtrap manages in a tiny voice.  Its slender cheeks are red and it stares hopelessly into your eyes.  You twist your thighs and very gently tease its tip with your wet opening, before once again coming to an agonising halt.  The trap almost sobs.'
        )
        GUI.outputText('<br><br>"<i>Beg for it!</i>"')
        GUI.outputText(
            '<br><br>"<i>Pleasze lion!  I hwill do anything for you!  I hhwill do anything to serve your body!  I am hyour slave! Just... pleasze pleasze fuck me!</i>" it cries out, its fluttering voice in danger of breaking up completely.  You suppose that will do.  Holding the sandtrap down, you sink your wetness down onto its long, thin length with a pleasurable sigh.'
        )
        //Tight:
    ...
    static handjobWin() {
        ...
        if (liveData.player.gender == 1) {
            ...
            if (liveData.player.cocks[0].cockLength < 6) {
                ...
                if (liveData.player.balls == 0)
                    GUI.outputText(
                        "<br><br>The sandtrap only needs one hand to pump your diminutive cock; it barely moves its wrist as it jerks you, occasionally wriggling its oily fingers along the bottom of your " +
                            liveData.player.cockDescript(0) +
            ...
            //Cock 6-16 inches:
                ...
                //[no balls:
                if (liveData.player.balls == 0) GUI.outputText("<br><br>The sandtrap glides its hand up and down your " + liveData.player.cockDescript(0) + ", picking up a warm smooth rhythm.")
                //Balls:
                else
                    GUI.outputText(
                        "<br><br>The sandtrap glides its hand up and down your " +
                            liveData.player.cockDescript(0) +
            ...
            //Cock more than 16 inches:
                ...
                GUI.outputText(
                    "<br><br>The sandtrap quickly comes to the realisation it needs both hands for this.  Still cupping your behind, it moves one oily hand up the top end of your massive shaft, while the other works the other half.  You sigh with deep satisfaction, enjoying the all-too-rare sensation of having every inch of your " +
                        liveData.player.cockDescript(0) +
            ...
            //Three cocks:
                ...
                GUI.outputText(
                    "<br><br>As the sandtrap begins to pump you faster, your two other, disregarded cocks bob in hard, eager concert with your " +
                        liveData.player.cockDescript(0) +
                ...
                GUI.outputText(
                    "<br><br>The warm, oily tripled sensation is unbearably good and although you try to hold on to savor it as long as possible, it isn't long before you reach a toe-curling peak and, with a wordless bark, spurt out line after line of jizz.  The trap flinches slightly when your first shots hit it in the face and chest, but then goes back to gently rubbing you, guiding you up and down your warm, slippery multiple orgasm."
                )
            }
            //Four cocks:
            else if (liveData.player.cockTotal() == 4) {
                GUI.outputText(
                    "<br><br>As the sandtrap begins to pump you faster, your three other, disregarded cocks bob in hard, eager concert with your " +
                        liveData.player.cockDescript(0) +
        ...
        //Female:
            ...
            GUI.outputText(
                "<br><br>You grab the sandtrap's black hair and cum with startling force, gushing fluid onto the trap and the sand beneath you as your cunt flexes around its fingers, which continue to curl and flick to send spasms of ecstatic pleasure up your spine."
            )
        }
        //Herm:
        else if (liveData.player.gender == 3) {
            GUI.outputText(
                '<br><br>One pair of the creature\'s hands slowly brush up your inner thighs whilst the other two feel around your "<i>armor</i>", clumsily loosening clasps and belts as it finds them.  One hand finds your ' +
                    liveData.player.cockDescript(0) +
            ...
            //Cock 6-16 inches:
                ...
                GUI.outputText(
                    "<br><br>The sandtrap quickly comes to the realisation it needs both hands for this.  Still cupping your behind, it moves one oily hand up the top end of your massive shaft, whilst the other works the other half.  You sigh with deep satisfaction, enjoying the all-too-rare sensation of having every inch of your " +
                        liveData.player.cockDescript(0) +
                        " immersed in pleasure.  The creature screws its face up and looks away in distaste at the size of your straining dick, which is in danger of poking it in the nose.  As you enjoy the warm, slippery sensation inundating you, you find you couldn't care less what it thinks."
                )
            }
            ...
            if (liveData.player.cockTotal() == 1) {
                GUI.outputText(
                    "<br><br>The sandtrap pumps you faster and faster until its slippery hands are a blur of motion, lavishing attention upon your clit and pussy, eventually pumping and flicking in furious rhythm.  It clutches onto your butt cheeks so tightly you hazily realize the indents of its slender digits will be with you for hours afterwards."
                )
                GUI.outputText(
            ...
            //Two cocks:
                ...
                GUI.outputText(
                    "<br><br>As the sandtrap begins to pump you faster, your disregarded " +
                        liveData.player.cockDescript(1) +
                ...
                GUI.outputText(
                    "<br><br>The warm, oily triple sensation is unbearably good and although you try to hold on to savor it as long as possible, it isn't long before you reach a toe-curling peak and, with a harsh moan, spurt out line after line of jizz whilst gushing femcum coat its hands and your thighs.  The trap flinches slightly when your first shots hit it in the face and chest, but then goes back to gently rubbing and frigging you as you ride your glorious orgasms out."
                )
            }
            //hree cocks:
            else if (liveData.player.cockTotal() == 3) {
                GUI.outputText(
                    "<br><br>As the sandtrap begins to pump you faster, your two other, disregarded cocks bob in eager concert with your " +
                        liveData.player.cockDescript(0) +
                ...
                GUI.outputText(
                    "<br><br>The warm, oily sensation is unbearably good and although you try to hold on to savor it as long as possible, it isn't long before you reach a toe-curling peak and, with a harsh moan, spurt out line after line of jizz whilst gushing femcum coat its hands and your thighs.  The trap flinches slightly when your first shots hit it in the face and chest, but then goes back to gently rubbing you as you ride your glorious orgasms out, spurting like a cum sprinkler in front of it."
                )
            }
            //More than three cocks:
            else {
                GUI.outputText(
                    "<br><br>As the sandtrap begins to pump you faster, your many other, disregarded cocks begin to harden in eager concert with your " +
                        liveData.player.cockDescript(0) +
        ...
        //All sexes go to:
        GUI.outputText(
            "<br><br>You sigh with immense satisfaction and step back from the sandtrap.  Taking into account what an excellent job it did you opt not to rub its face in what you have made it do, particularly as you have managed to get quite a bit of your fluids on it anyway.  It watches you, still flushed and mouth slightly ajar, as you leisurely pull your [armor] back on."
        )
        GUI.outputText(
            '<br><br>"<i>Please, lion,</i>" it manages eventually.  You look at the strange, half-buried creature and notice that spots of fluid are darkening the sand around it; it is evidently perversely turned on by your domineering actions, and you have left it in no way satiated.  You curl your lip into an evil smirk.'
        )
        GUI.outputText(
            '<br><br>"<i>Not today, I think,</i>" you say over your shoulder, as you clamber out of its pit.  "<i>I think you\'ll have to try a bit harder if you want to get what you want from this lion.</i>"  Your smile broadens as a deeply frustrated moan reaches your ears from behind you...'
        )
    ...
    static cockLoseToSandTrap() {
        ...
        //6 inches to 15:
            ...
            GUI.outputText(
                "<br><br>The creature owns an incredibly tight sex which grasps you possessively once it is on you; it fits your cock like a glove.  The sandtrap sighs its fluttering sigh when it reaches your base and then slowly begins to work you, its punishing, pulsing muscles squeezing your cock up and down."
            )
        //More than 15 inches:
        else
            GUI.outputText(
                "<br><br>The creature owns an incredibly tight sex which grasp you possessively once it is on you.  Sweat beads the sandtrap's elegant brow as it forces itself brutally down your huge shaft; when it finally stops with a sizable portion of your " +
                    liveData.player.cockDescript(0) +
        ...
        //[High cum:
        if (liveData.player.cumQ() > 1000) GUI.outputText("  The sandtrap gasps and coos as you ejaculate over and over again, and beneath you feel your cum wet the sands as it beads around the clenching grip it has you in.")
        if (liveData.player.hasVagina()) GUI.outputText("  Ignored entirely, your " + liveData.player.vaginaDescript(0) + " quivers and orgasms in tandem, flexing and wetting fruitlessly against the dry water pressing against it.")

        GUI.outputText(
            "<br><br>The sandtrap finally releases you, humming and fluttering sounds of pleasure as it draws its unseen cunt away and retracts its prong from your mouth and back into its slit, trailing oil and saliva as it goes.  You don't really notice these things or even if it is trying to say anything to you in that bizarre voice it has, still deep within the encasing sand and deep within your muddling daze, and your limbs feel like any returning strength has been sucked right back out of you by the creature's milking cunt.  You do not struggle when the sandtrap takes you into its arms and begins to turn you around.  It fondles your [chest] with one hand as its others busily work at pulling your bottom half this way and that, until your [butt] is poked outwards in supplication.  Only then does it dozily occur to you that the creature is not done with you yet."
        )
        ...
        if (!liveData.player.isTaur()) GUI.outputText("armpits")
        else GUI.outputText("forelegs")
        GUI.outputText(
    ...
    static chicksLoseToSandTrap() {
        ...
        if (liveData.monster.findStatusEffect(StatusEffects.Fertilized) >= 0) this.sandTrapPregChance() //TODO Pregnancy
        liveData.player.dynStats(["lib", 1])
        liveData.player.slimeFeed()
        //reduce lust, increase lib, slimefeed, reset hours since cum
        COMBAT.cleanupAfterCombat()
    ...
    static genderlessLoseToSandTrap() {
        ...
        //TODO Sprite spriteSelect(97);
        GUI.outputText(
            "<br><br>The sandtrap holds your hands whilst it pushes you further down with its other set of arms, until only your head is above the sand.  Below the surface you try to weakly move your limbs, try to work yourself out of this situation but it is impossible; the sand feels impossibly heavy and is packed against you.  The sandtrap seems to have no such difficulty.  It towers above you, moving with sinuous grace.  You feel something wet touch your thigh and you try to flinch, but aside from flexing your muscles you cannot move."
        )
        GUI.outputText(
            '<br><br>"<i>Hwhat a strange creature you arrrhe,</i>" says the sandtrap, looking at you with vague bafflement as it gently strokes your face.  It seems the more excited it gets and the less it feels the need to pretend, the more fluttery and broken its voice becomes; it is like you are listening to a hive of bees that just happens to be forming words.  '
        )
    ...
    static sandTrapPregChance() {
        ...
        if (liveData.gameFlags[FLAG.FERTILE_SANDTRAP] == 1) liveData.player.buttKnockUp(ENUM.PregnancyType.PREGNANCY_SANDTRAP_FERTILE, ENUM.IncubationType.INCUBATION_SANDTRAP)
    ...
    static birfSandTarps() {
        ...
        //TODO spriteSelect(97);
        GUI.outputText(
            "<br>Your eyes widen as a gout of oil suddenly gushes from your ass.  Before panic can set in, an incredible light-headedness overtakes you.  Dreamily, you discard your [armor] and squat.  More oil oozes out of you, and in your hazy euphoria, you scoop some of it up and rub it dreamily into your " +
        ...
        //Female:
            ...
            if (!liveData.player.isTaur()) GUI.outputText("  Your oily hands push softly into your cleft, fingering your needy " + liveData.player.clitDescript() + " as you feel the pressure in your bowels intensify again.")
        }
        //Herm:
        else if (liveData.player.gender == 3) {
        ...
        //Libido 30-65:
            ...
            GUI.outputText(
                "<br><br>You spend a moment enjoying your post-natal haze, then haul yourself out of it, flap the flytraps you have birthed away, and make your way back to camp.  Though this experience has been deeply unnerving, you can't help but acknowledge it has also been incredibly erotic."
            )
        //Libido >65:
        else
            GUI.outputText(
                "<br><br>You smile lazily, then lie back and glory in the sensual haze the oil has left you in.  After you have spent many minutes lying listening to the happy twittering of your flytrap children above you, you reluctantly get up.  You only hope that you get to experience the unearthly wonder of birthing these strange creatures again, and again, and again."
            )
    ...
    static encounterASandTrap() {
        ...
        // Later meetings
            ...
            //Speed check fail:
                ...
                GUI.outputText(
                    "<br><br>You don't move quickly enough, however, and you may as well be running on the spot; the edge of the pit recedes as the fluid sand carries you downwards.  You struggle upright as best you can and ready yourself to fight the sandtrap, which is leering at you hungrily from the bottom of its vast pit."
                )
    ...
    static dontSaveTheSandTrap() {
        ...
        if (liveData.player.femininity <= 65) GUI.outputText("woman")
        else GUI.outputText("man")
        GUI.outputText('.  "<i>Please!</i>" ')
        if (liveData.player.femininity <= 65) GUI.outputText("she")
        else GUI.outputText("he")
        ...
        if (liveData.player.femininity <= 65) GUI.outputText("her")
        else GUI.outputText("him")
    ...
    static sandTrapBadEnd() {
        ...
        GUI.outputText(
            "<br><br>You start and come to your senses as your feet point downwards into a shifting pit.  Out in the middle of nowhere, you have led yourself to a Sandtrap.  It lazily trails its hands in the sand at the bottom of its hollow and grins up at you, its face shimmering between genders.  There is something... knowing in its smile.  You try and collect yourself a bit.  Why didn't the creature try and surprise you?  Somehow the sight of it sitting there brazenly, waiting for you, is more eerie than if it had tried to suck you into its trap."
        )
        GUI.outputText(
            '<br><br>"<i>My people have stories about wanderers like you,</i>" the androgyne says softly, its multiple black eyes looking into your own black orbs.  "<i>It has been so long since we had such a one as you, and we were so winnowed by the soulless lions of the mountains, that your kind have passed into legend.  Surface scratchers who not only bear our children, but choose to do so willingly; who drink the fluids we gift them to become of our flesh; who come back to our lands time and again, answering a call only they can hear.</i>"  The creature opens all four of its arms to you, beaming triumphantly.  "<i>You need only to take the final step, Flytrap.  Come to me, and embrace your desztiny.</i>"'
        )
        GUI.outputText(
            "<br><br>You stand at the lip of the Sandtrap's hollow, shaking your head, trying to make sense of what it is saying.  It's talking nonsense...isn't it?  Again you feel light-headed, your thoughts a lonely cloud of bees without a hive.  They have no purpose, and the Sandtrap is offering you one... but don't you already have a purpose?  Not as good a purpose as this one.  Not as right a purpose, one that perfectly suits what your body has become...  "
        )
        if (liveData.player.hasCock()) GUI.outputText("[EachCock] hardens and your thin frame feels aflame, barely understood impulses deep within you begging you to walk down and submit to the Sandtrap's will.  ")
        //Female:
        else if (liveData.player.hasVagina()) GUI.outputText("Your [vagina] moistens and your thin frame feels aflame, barely understood impulses deep within you begging you to walk down and submit to the Sandtrap's will.  ")
        GUI.outputText(
    ...
    static destiny() {
        ...
        //Male:
        if (liveData.player.hasCock()) GUI.outputText("Your cock strains upwards and you moan; feeling as sensitive as you do, you are both desperate to be touched and desperate not to be.  ")
        //Female:
        else if (liveData.player.hasVagina()) GUI.outputText("You feel but barely hear the drip of moisture on the sand, both your [asshole] and [vagina] beading with oily need.  ")
        GUI.outputText(
