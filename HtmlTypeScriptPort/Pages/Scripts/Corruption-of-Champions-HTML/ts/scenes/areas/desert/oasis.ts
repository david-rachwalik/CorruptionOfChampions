...
class DemonPack extends Creature {
    ...
    packAttack() {
        ...
        //Determine if cat'ed
            ...
            if (temp <= 0) {
                temp = 0
                if (!liveData.monster.plural) GUI.outputText("You deflect and block every " + liveData.monster.weapon.verb + " " + liveData.monster.a + liveData.monster.name + " throw at you.")
                ...
                else GUI.outputText(liveData.monster.a + liveData.monster.name + " <b>mutilates</b> you with powerful fists and " + liveData.monster.weapon.verb + "s! ")
...
abstract class OasisScene {
    ...
    static oasisSexing() {
        ...
        if (liveData.player.vaginas.length > 0) {
            //Herms only!
            if (liveData.player.gender == 3) GUI.outputText("From your cock it is only a tiny trip downward between your legs before the demons discover something new to play with.  ")
            //Girlies only!
            else GUI.outputText("The demons quickly find your " + liveData.player.vaginaDescript(0) + " and tussle eagerly for position at your entrance, first with hands and then with a wide range of demonic dicks.  ")
            //gaping cunners!
    ...
    static oasisBadEnd() {
        ...
        //[If female/herm]
        if (!liveData.player.isTaur()) GUI.outputText("Your feet are locked up in shackles, though with a chain long enough to leave you room to move or walk without problems. ")
        if (liveData.player.isTaur()) GUI.outputText("Your four legs are locked up in shackles, though with a chain long enough to leave you room to move or walk without problems. ")
        ...
        //[If female/herm]
        if (liveData.player.gender >= 2) GUI.outputText("He reaches down to grab a hold of your hair and lifts you to your feet, causing you to yelp out in pain from the sharp pull.  ")
        GUI.outputText(
