    ...
    if (player.findStatusEffect(StatusEffects.StoneLust) >= 0) {
        ...
        outputText(
            "You feel the sands shift by your " +
                ...
                player.clothedOrNakedLower("into your clothes!", "onto your " + player.legs()) +
...
SandWitchScene.allowSandWitchMagic = function () {
    ...
    if (player.hairColor == "sandy blonde") {
        ...
        if (player.biggestLactation() == 0) {
            ...
            player.modStats("lib", 0.5, "sen", 1, "lus", 10)
        }
        ...
        if (gameFlags[SAND_WITCH_RAPED] == 0) outputText("You wonder at what her robes conceal as she vanishes into the dunes.")
        if (gameFlags[SAND_WITCH_RAPED] > 0) {
            if (player.cor <= 33) outputText("You are glad to avoid servicing her again as she vanishes into the dunes.")
            else if (player.cor <= 66) outputText("You wonder if you should've resisted and tried for some sex as she departs.")
        ...
        if (player.hairColor.indexOf("sandy blonde") != -1) {
            ...
            if (player.averageNipplesPerBreast() < 1) {
                ...
                while (temp > 0) {
                    ...
                    if (player.breastRows[0].nipplesPerBreast < 1) player.breastRows[0].nippleLength = 0.2
                    player.breastRows[0].nipplesPerBreast = 1
                }
                player.dynStats("sen", 2, "lus", 1)
            }
            ...
            if (player.biggestLactation() == 0) {
                ...
                player.boostLactation(1)
                player.dynStats("lib", 0.5, "sen", 1, "lus", 10)
            }
                ...
                if (player.dogCocks() > 0 && player.cocks[0].knotMultiplier > 1.4) SandWitchScene.knotSandWitch()
                    ...
                    outputText(
                        "You place your " +
                            ...
                            num2Text(player.cocks.length + 1) +
                    ...
                    outputText(
                        "Finally finished, you pull your " + player.cockDescript(0) + " from one of her well-used slits, your " + player.cockDescript(0) + " drooling cum even now. With a grin, you walk away, happy with your little encounter.  ",
                        false
                    )
                    cleanupAfterCombat()
                    player.orgasm()
...
SandWitchScene.sandWitchCentaurBoning = function () {
    ...
    //[Corruption > 70]
        ...
        if (player.cockTotal() > 0 || (player.gender == 3 && rand(2) == 0)) {
            ...
            //[1 cock, non-tentacle]
            if (player.cockTotal() == 1 && player.countCocksOfType(ENUM.CockType.TENTACLE) == 0) outputText("With a single thrust, you push deep into one of her cunts, ", false)
            //[1 cock, tentacle]
            if (player.cockTotal() == 1 && player.countCocksOfType(ENUM.CockType.TENTACLE) == 1)
                outputText("Your " + player.cockDescript(0) + " caresses her anus teasingly, causing her to whimper in a mixture of arousal and denial.  With a single savage thrust, you push past her clenched muscles, ", false)
            //[two cocks]
            ...
            //[1 cock, tentacle]
            if (player.countCocksOfType(ENUM.CockType.TENTACLE) == 1 && player.cockTotal() == 1) {
            ...
            //[1 cock, tentacle]
            if (player.cockTotal() == 1 && player.countCocksOfType(ENUM.CockType.TENTACLE) == 1) outputText("anus", false)
            else {
                //[largest cock is not wide and/or long]
                if (player.cocks[0].cockLength >= 12 || player.cocks[0].cockThickness >= 3) {
                    //[1 cock]
...
SandWitchScene.knotSandWitch = function () {
    ...
    if (player.cumQ() >= 25 && player.cumQ() < 100)
        outputText("Your cum expands her pussy, stretching her out and leaving your " + player.cockDescript(0) + " swimming in your spunk, trapped within her tight-stretched walls by your massive knot.", false)
...
SandWitchScene.sandWitchSpanking = function () {
    ...
    if (player.findPerk(PerkLib.FireLord) >= 0 || player.findPerk(PerkLib.Hellfire) >= 0 || player.findPerk(PerkLib.Dragonfire) >= 0)
...
SandWitchScene.eggWitchForeplay = function () {
    ...
    player.dynStats("lus", 25, "cor", -0.35)
