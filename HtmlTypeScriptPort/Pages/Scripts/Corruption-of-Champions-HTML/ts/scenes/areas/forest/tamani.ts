...
TamaniScene.tamaniSexLost = function () {
    ...
    outputText(
        ' and asks, "<i>So are you sorry for making me wait for my cream ' + player.mf("stud", "slut") + "?  If you are, just say <b>I'm sorry mistress Tamani, please milk my cock whenever you want</b> and then beg me to cum.</i>\"<br><br>",
        false
    )
    ...
    player.dynStats("lib", 0.5, "sen", -1, "cor", 0.5)
    cleanupAfterCombat()
    player.orgasm()
}
...
TamaniScene.tamaniSexLetHer = function () {
    ...
    //Not-taurs
        ...
        //(Doesnt fit)
            ...
            if (player.hasVagina()) {
                outputText("  ", false)
                player.cuntChange(player.vaginalCapacity() * 0.95, true)
                ...
                player.buttChange(player.analCapacity() * 0.95, true)
...
TamaniScene.tamaniHypnosis = function () {
    ...
    if (primary >= 0) {
        ...
        outputText(
            "  She visibly orgasms, shaking and trembling from head to toe as her pussy milks your " + player.cockDescript(primary) + ".  It only lasts a few seconds, though they feel like heaven, before her body goes nerveless.<br><br>",
            false
        )
        ...
        else if (player.cocks[primary].cockType == ENUM.CockType.HORSE) outputText("Your flare widens, actually holding itself inside Tamani's womb", false)
        else if (player.cocks[primary].cockType == ENUM.CockType.DEMON) outputText("The corrupted nodules along your " + player.cockDescript(primary) + " pulsate in rippling, wave-like motions, massaging the interior of Tamani's twat", false)
        else if (player.cocks[primary].cockType == ENUM.CockType.TENTACLE) outputText("Your tentacle-cock's tip widens, becoming mushroom-like inside Tamani's womb", false)
    ...
    //[DOESNT FIT]
        ...
        outputText(player.mf("strong man's", "sexy lover's") + " " + Appearance.cockNoun(ENUM.CockType.HUMAN) + '?</i>"<br><br>', false)
...
TamaniScene.encounterTamani = function () {
    ...
    //Dudezillaz:
        ...
        switch (tamanipreg.pregnancyEventNum) {
            ...
            default:
                TamaniScene.tamaniMaleRepeatEncounter() //She's not pregnant or is only slightly pregnant
        }
    }
}
...
TamaniScene.tamaniMaleRepeatEncounter = function () {
    ...
    if (tamanipreg.isPregnant() || gameFlags[TAMANI_NUMBER_OF_DAUGHTERS] > 0)
        outputText(
            "While exploring, you're startled by the feeling of tiny hands stroking the insides of your thighs.  You look down and find Tamani there, grinning wolfishly,  \"<i>Ready for another fuck, big " + player.mf("boy", "girl") + '?"<br><br>',
            false
        )
...
TamaniScene.tamaniBirthScene = function () {
    ...
    outputText(
        "  She looks up at you and whispers breathily, \"<i>Come fuck me a little later, I'll give you even more beautiful little girls.  These " +
            num2Text(gameFlags[TAMANI_PREGNANCY_COUNT]) +
...
TamaniScene.tamaniGivesBirth = function () {
    if (tamanipreg.pregnancyType == PREGNANCY_PLAYER) {
