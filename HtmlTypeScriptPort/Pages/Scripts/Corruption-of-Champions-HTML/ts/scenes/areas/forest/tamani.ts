    ...
    tamaniHypnoTease() {
        ...
        //Medium Hypnosis
            ...
            if (selector == 0) {
                ...
                if (liveData.player.totalCocks() > 1) outputText("Each of y")
                else outputText("Y")
                outputText("our " + liveData.player.multiCockDescriptLight())
                if (liveData.player.lust > 80) outputText(" drips pre-cum")
                else if (liveData.player.lust > 40) outputText(" grows harder")
                ...
                if (liveData.player.lust < 70) outputText("  Still, you don't want to fuck her right now!")
            } else {
                outputText("Struggling, you pull your eyes back into your head and away from Tamani's gorgeous slit.  You shudder, feeling ")
                if (liveData.player.totalCocks() > 1) outputText("each of ")
                outputText("your " + liveData.player.multiCockDescriptLight())
                if (liveData.player.lust <= 41) outputText(" thicken perceptibly")
                else if (liveData.player.lust <= 81) outputText(" twitch eagerly")
                ...
                if (liveData.player.lust < 70) outputText("  Regardless, you're resolute in your desire not to fuck her right now!")
            }
        }
        //High Hypnosis
        else {
...
abstract class TamaniScene {
    ...
    static tamaniSexWon() {
        ...
        if (liveData.player.cockArea(x) <= 90) {
            ...
            if (liveData.player.totalCocks() > 1) outputText(" and tight asshole")
            ...
            if (liveData.player.totalCocks() > 1) outputText("s")
            ...
            if (liveData.player.gender == 1) outputText("man")
            ...
            if (liveData.player.totalCocks() == 1) outputText("pussy squeezing tightly as she cums hard.<br><br>")
            ...
            if (liveData.player.totalCocks() > 1) {
                ...
                if (liveData.player.cumQ() >= 250) outputText("  She pants, turning her head in the mud to watch as her belly visibly inflates, stuffed totally full of cum. ")
                if (liveData.player.cumQ() >= 500) outputText("  In no time at all a river of jism pours from her, pooling below as you overwhelm her body's capacity to store spunk.")
            } else {
                outputText(
                    "Your " +
                        liveData.player.multiCockDescriptLight() +
            ...
            if (liveData.player.balls > 0) outputText("sexy balls have ")
        ...
        //Too big? Jerk off with feet and bukkake
            ...
            if (liveData.player.totalCocks() > 1) outputText("any of your massive members")
            ...
            if (liveData.player.cumQ() >= 250) outputText("  You continue working your " + liveData.player.cockDescript(x) + " with her supple feet, watching your dripping spooge froth and bubble as you continue to pump more onto the sassy wench.")
    ...
    static tamaniBeaten() {
        ...
        if (liveData.player.hasCock()) outputText("licks her lips in anticipation, eyes locked on [eachCock]")
        ...
        if (liveData.gameFlags[FLAG.TIMES_OVIPOSITED_TAMANI] == 0) outputText("my baby-" + liveData.player.mf("daddy", "momma"))
        ...
        if (liveData.gameFlags[FLAG.TIMES_OVIPOSITED_TAMANI] > 0) outputText(", or in your case even more eggs")
    ...
    static tamaniSexLost() {
        ...
        if (liveData.player.HP < 1) outputText("lie there, unable to fight back.  ")
        ...
        if (liveData.player.cocks[0].cockThickness >= 5) outputText("Impossibly, despite your titanic girth, the ring somehow expands to squeeze onto you, though it's painfully tight.<br><br>")
        else outputText("Somehow the ring fits onto you perfectly, as if it was made exactly for your " + liveData.player.cockDescript(0) + ", though it remains painfully tight.<br><br>")
        outputText(
            "\"<i>Don't fret, " +
        ...
        if (liveData.player.biggestTitSize() >= 6) outputText("but can't manage to see your bloated prick past the wall of titflesh.<br><br>")
        ...
        if (liveData.player.balls > 0) outputText("cups your " + liveData.player.ballsDescriptLight() + " tightly")
        ...
        if (liveData.player.balls > 0) outputText("Your balls are getting VERY swollen and tender.")
        else outputText("Your body aches inside, feeling bloated and ready to pop.")
        if (liveData.player.cor < 50) outputText("  You have no choice, you'll say what she wants you to say.<br><br>")
        ...
        if (liveData.player.cor > 50) outputText("blushing hotly and doing your best to look like a submissive toy.<br><br>")
        ...
        if (liveData.player.cumQ() >= 250) outputText("  The bottle quickly overflows, but Tamani happily sets it aside and slides her own cunt under you, letting you pump the rest of your seed into her fertile womb.")
        if (liveData.player.cumQ() >= 500) outputText("  She coos happily when you still manage to pump enough jism into her to bloat her belly, but even with your impressive fertility, the orgasm has to end.")
        outputText("<br><br>")
    ...
    static tamaniSexLetHer() {
        ...
        if (COMBAT.inCombat()) {
            ...
            if (liveData.player.isTaur()) {
                if (liveData.player.HP < 1) outputText("You stumble, legs weaving drunkenly as you try to keep your feet.  ")
                else outputText("You stumble, legs weaving drunkenly as " + liveData.player.sMultiCockDesc() + " ache and drool with need.  ")
                outputText(
            ...
            //Not taur
                ...
                if (liveData.player.HP < 1) outputText("You collapse, unable to fight back any longer.  For her part, Tamani seems disappointed with the outcome, but approaches your prone form in spite of your sorry appearance.")
                ...
                if (liveData.player.cockTotal() == 1) outputText("an erection emerges")
        ...
        if (liveData.player.isTaur()) {
            ...
            if (liveData.player.cocks[0].cockLength <= 18) {
                ...
                //No balls:
                if (liveData.player.balls == 0) outputText("You find yourself wondering vaguely where a goblin learns these kinds of gymnastics.<br><br>")
                else
                    outputText(
            ...
            //If >18 Inches
                ...
                //High cum:
                    ...
                    if (liveData.player.balls > 0) outputText("Your " + liveData.player.ballsDescriptLight() + " ache and you feel very thirsty indeed.")
        ...
        //Not-taurs
            ...
            if (liveData.player.cockArea(0) <= 90) {
                ...
                if (liveData.player.cocks[0].cockLength >= 12) outputText("Her body visibly stretches around you, and you silently thank whatever gods or demons adapted goblins to be able to fulfill this role.  ")
                ...
                if (liveData.player.totalNipples() == 2) outputText("both")
                else outputText("all")
                outputText(
                    " of your " +
                ...
                if (liveData.player.biggestTitSize() >= 2) outputText("Your " + liveData.player.allBreastsDescript() + " bounce and jiggle obscenely as the goblin twists and pulls, abusing your nipples.")
                else
                    outputText(
                        "Your " +
                            liveData.player.nippleDescript(0) +
                ...
                if (liveData.player.balls > 0) outputText("Your " + liveData.player.ballsDescriptLight() + " twitch powerfully, venting their contents, releasing a jet of hot cum into the goblin.")
                ...
                if (liveData.player.cumQ() >= 250) outputText('  After a while you can feel the pressure building inside her, starting to give her a bit of a belly.  Tamani rubs it and begs, "<i>More!</i>"')
                if (liveData.player.cumQ() >= 500) outputText("  Your body fulfills her requests and more, stuffing her until she's positively pregnant and your spunk is squirting from her entrance with each wave of sticky spooge.")
                ...
                if (liveData.player.cumQ() < 250) outputText(".  I really enjoy our fucks " + liveData.player.mf("stud", "hun") + ", and I can't wait to 'bump' into you again.")
                else {
                    outputText(".  You make sure and let your ")
                    if (liveData.player.balls > 0) outputText(liveData.player.ballsDescriptLight() + " fill back up with cum, ok?")
                    else outputText(liveData.player.cockDescript(0) + " fills back up with cum, ok?")
                }
                outputText('</i>"<br><br>')
                //Combat end:
                if (COMBAT.inCombat()) {
            ...
            //(Doesnt fit)
                ...
                if (liveData.player.balls > 0) outputText(liveData.player.ballsDescriptLight())
                else outputText(liveData.player.cockDescript(0))
                outputText(
                ...
                if (liveData.player.balls > 0) outputText("lifts up your " + liveData.player.ballsDescriptLight())
                ...
                if (liveData.player.hasVagina()) outputText("cock's base ")
                ...
                if (liveData.player.cumQ() >= 250) {
                    ...
                    if (liveData.player.cumQ() >= 500) outputText(" as you keep cumming, splattering her tits and face, soaking the little goblin slut")
                    outputText(".  ")
                }
    ...
    static tamaniHypnosis() {
        ...
        if (primary >= 0) {
            ...
            if (cocks > 0) outputText(" around your " + liveData.player.cockDescript(secondary))
            outputText(". Her tiny form jiggles, ")
            if (liveData.player.tallness > 55) outputText("lifting off the ground as you get into it, giving yourself over to her sex.")
            ...
            if (liveData.player.skinType == ENUM.SkinType.SKIN_TYPE_FUR) outputText("matted")
            ...
            else outputText("and her mouth is too full of " + liveData.player.cockDescript(secondary) + " to do anything but gurgle out nonsensical pleasure-noises.")
            ...
            if (liveData.player.hasKnot(primary)) outputText("Your knot balloons inside her, locking her in place")
            else if (liveData.player.cocks[primary].cockType == ENUM.CockType.HORSE) outputText("Your flare widens, actually holding itself inside Tamani's womb")
            ...
            else if (liveData.player.cocks[primary].cockType == ENUM.CockType.TENTACLE) outputText("Your tentacle-cock's tip widens, becoming mushroom-like inside Tamani's womb")
            ...
            if (liveData.player.cumQ() < 50) outputText(", but brief, and after a few moments of spasming pleasure, the flood of cum slows to a trickle.")
                ...
                if (liveData.player.cumQ() < 1000) outputText("  Though your orgasm tapers off, the additional seed squirts from between her thighs, trickling down to a slow drip of viscous white goo.")
                ...
                if (liveData.player.cumQ() >= 1000 && liveData.player.cumQ() < 4000) outputText("  It does eventually end, but not until a sizable puddle has formed between the pair of you.")
            ...
            if (cocks > 1) {
                ...
                if (liveData.player.cumQ() > 300) {
                    ...
                    if (liveData.player.cumQ() > 600) outputText("  A few seconds later she pulls back coughing, and the perky goblin takes the rest of your load in her face.")
                } else outputText(".")
            }
            outputText("<br><br>")

            outputText("Joined at the hips, the two of you manage to stay upright, each leaning into the other")
            if (liveData.player.tallness > 55) outputText(" in spite of the height differential")
            outputText(
        ...
        //[DOESNT FIT]
            ...
            if (liveData.player.biggestTitSize() < 1) outputText("on your chest ")
            else outputText("between your " + liveData.player.breastDescript(0) + " ")
            ...
            if (liveData.player.tallness > 60) outputText("big ")
            ...
            if (liveData.player.cor < 33) outputText("beg")
            else if (liveData.player.cor < 66) outputText("ask")
            ...
            if (liveData.player.cumQ() > 250) outputText("  In no time, you've filled her to capacity, and white mixes in with the clear sexual fluids that are dripping from the goblin's cunt.")
            if (liveData.player.cumQ() > 700) outputText("  A few more blasts visibly bloat her belly, and Tamani pulls her hands away from her pussy to stroke the slowly rounding dome on her abdomen.")
            if (liveData.player.cumQ() > 1500) {
                ...
                if (liveData.player.cumQ() > 3000) outputText(", and by the time you finish, both of you are in a thick puddle of spunk")
                outputText(".")
            }
            if (cocks > 1) {
                if (cocks == 2) outputText("  Your other" + liveData.player.cockDescript(1))
                else outputText("  Each of your other " + liveData.player.multiCockDescriptLight())
    ...
    static tamaniFemaleYes() {
        ...
        if (liveData.player.cor < 50) outputText("Gross.<br><br>")
    ...
    static tamaniFemaleNo() {
        ...
        if (liveData.player.cor < 33) outputText("What a slut.")
        else if (liveData.player.cor < 66) outputText("How odd.")
    ...
    static tamaniFirstTimeConsentual() {
        ...
        if (liveData.player.biggestTitSize() > 1) outputText(liveData.player.allBreastsDescript())
        ...
        if (liveData.player.balls > 0) outputText("Your " + liveData.player.ballsDescriptLight() + " swell with seed, spurring your desire to new heights.")
        ...
        if (liveData.player.cockArea(0) <= 90) {
            ...
            if (liveData.player.balls > 0) outputText(" and drips from your " + liveData.player.ballsDescriptLight())
            ...
            if (liveData.player.cockArea(0) > 30) outputText(" and watching her belly bulge from your size")
            outputText(
            ...
            //[CORRUPT]
            if (liveData.player.cor > 66) outputText("  Your " + liveData.player.cockDescript(0) + " twitches at the thought, ready and wanting more.")
        }
        //(TOO BIG)
        else {
            outputText(
            ...
            if (liveData.player.hasSheath()) {
                ...
                if (liveData.player.balls > 0) outputText("licks all over your balls")
                else if (liveData.player.hasVagina()) outputText("sneaks between your folds to tease your now-hardening clit")
                else outputText("licks the sensitive " + liveData.player.skinTone + " " + liveData.player.skinDesc + " of your inner thighs")
            }
            outputText("<br><br>")
            outputText(
            ...
            if (liveData.player.balls > 0 && liveData.player.hoursSinceCum > 200) outputText(", visibly draining your " + liveData.player.ballsDescriptLight() + " down to their normal size")
    ...
    static tamaniPregnantFuck() {
        ...
        if (liveData.player.biggestTitSize() >= 2) outputText(liveData.player.allBreastsDescript())
        else outputText("chest")
        outputText(".\n\n")
        outputText(
        ...
        //(Very large:
        if (liveData.gameFlags[FLAG.TAMANI_TIMES_IMPREGNATED] > 6) outputText("  They're almost too big for you to handle; they probably make it hard for the poor girl to walk.")
        outputText("\n\n")
        ...
        //(Doesn't Fit)
            ...
            if (liveData.player.balls > 0) outputText("massaging your balls with her feet")
            else outputText("jacking the base of your " + liveData.player.cockDescript(0) + " with her feet")
            outputText(
    ...
    static tamaniBirthScene() {
        ...
        if (liveData.gameFlags[FLAG.TAMANI_NUMBER_OF_DAUGHTERS] > 0) outputText("with her daughters, ") //She always has at least two daughters if she has any
        outputText(
