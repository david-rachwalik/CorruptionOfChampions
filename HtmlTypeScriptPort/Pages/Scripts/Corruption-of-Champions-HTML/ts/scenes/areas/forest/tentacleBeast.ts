...
TentacleBeast.prototype.doAI = function () {
    if (rand(2) == 0 || gameFlags[TENTACLE_COOL_DOWN] == 1) TentacleBeast.tentaclePhysicalAttack()
    else TentacleBeast.tentacleEntwine()
    combatRoundOver()
}
...
TentacleBeastScene.tentacleLossRape = function () {
    ...
    if (player.gender == 0) TentacleBeastScene.tentacleLossGenderless()
    // Check for Bad End
    ...
    if (player.hasCock() && player.cumQ() < 1500 && player.ass.analLooseness < 4) {
        outputText(
            "Just as you think it's over, another tentacle rams into your " + player.assholeDescript() + " and begins roughly massaging your prostate as it swells massively, causing another surge of cum to leave you, and another, and another.",
            false
        )
...
TentacleBeastScene.tentacleRapeContinuationForFemales = function () {
    ...
    if (player.vaginas.length == 1) {
        ...
        if ((player.findPerk(PerkLib.BigClit) >= 0 && player.clitLength > 2) || player.clitLength > 3) outputText("almost " + num2Text(Math.floor(player.clitLength * 1.75)) + " inches in length. ")
        //normal girls get big clits
        else outputText("almost four inches in length.  Bruised and sore, you pass into unconsciousness ")
    } else {
        outputText(
    ...
    //Not too corrupt
    if (player.cor < 75) outputText("too intoxicated with lust to fume over your violation. ")
    //Very corrupt
    ...
    if (player.clitLength < 7 && player.clitLength >= 3.5 && player.findPerk(PerkLib.BigClit) >= 0) {
        ...
        if (player.clitLength < 5) outputText("like a tiny cock.")
        if (player.clitLength >= 5 && player.clitLength < 7) outputText("like a slick throbbing cock.")
        if (player.clitLength >= 7) outputText("like a big thick cock.")
    }
...
TentacleBeastScene.encounter = function () {
    ...
    if (player.cor > 50 && player.cor <= 75) outputText("You debate the merits of running from such a creature.<br><br>", false)
    if (player.cor > 75) outputText("You smile and stride forward, welcoming the pleasure you expect from such a monster.<br><br>", false)
    //Worms get nothing!
