...
abstract class GreenSlimeScene {
    static encounterSlime() {
        ...
        //High speed starts on even footing.
            ...
            COMBAT.startCombat(new GreenSlime())
            if (liveData.gameFlags[FLAG.FACTORY_SHUTDOWN] == 1) GUI.outputText("<br><br><b>You are amazed to encounter a slime creature with the factory shut down - most of them have disappeared.</b>")
            return
        }
    ...
    static slimeVictoryRape() {
        ...
        if (liveData.player.cor <= 33) {
            GUI.clearOutput()
            if (liveData.player.lust < 60) GreenSlimeScene.serviceLowCorruption()
        ...
        //higher corruption raaaaeeeep
            ...
            GUI.clearOutput()
            //male or futa
            if (liveData.player.gender == 1 || (liveData.player.gender == 3 && UTIL.rand(2) == 0)) GreenSlimeScene.maleRapesOoze()
            //female or futa
            else if (liveData.player.gender > 0) GreenSlimeScene.femaleRapesOoze()
        }
    }
    ...
    static rapeOozeWithMilk() {
        GUI.clearOutput()
        GUI.outputText(
            "You look over the ooze, wondering what to do about your need to nurse now that it has lost cohesion. After a while of puzzling things out, you decide to wing it, " +
        ...
        //[If Herm-
            ...
            GUI.outputText("caressing your " + liveData.player.multiCockDescriptLight())
            if (liveData.player.balls > 0) GUI.outputText("and slipping slightly further down to engulf your " + liveData.player.ballsDescriptLight() + " as well. ")
            else GUI.outputText("and almost dripping down to coat your " + liveData.player.clitDescript() + " to add to your pleasure. ")
        }
        //[If Genderless-
        else {
        ...
        if (liveData.player.gender < 1) GUI.outputText("gripping the ground as the slime has left you nothing to do; adding your hands to the mix feels like it'd be an insult to the creature's expert work.  ")
        //if duder
        else if (liveData.player.gender == 1) GUI.outputText("Your hands grip the ground as the slime has left you nothing to do; adding your fingers to the mix feels like it'd be an insult to the creature's expert work.  ")
        //[if female or Herm-
        else GUI.outputText("Your hands fly to your " + liveData.player.vaginaDescript(0) + " and frantically plunge your fingers in and out of it.  You wish the slime would engulf it as well.  ")
        ...
        if (liveData.player.gender == 1) {
            ...
            if (liveData.player.balls > 0) GUI.outputText(" and " + liveData.player.ballsDescript())
            GUI.outputText(" and ")
        }
        //[if female-
        else if (liveData.player.gender == 2) GUI.outputText(liveData.player.clitDescript() + " and ")
        //[if Herm-
        else if (liveData.player.gender == 3) {
            GUI.outputText(liveData.player.multiCockDescriptLight())
        ...
        if (liveData.player.gender == 1) GUI.outputText("feverishly jacking yourself off with it.  The cool feel of it contrasts enough to send you over the edge relatively quickly, and you release your cum into it.  You drop most of its bulk ")
        //[if female-
        else if (liveData.player.gender == 2)
            GUI.outputText(
        ...
        //[if genderless-
        else GUI.outputText("rubbing it over your " + liveData.player.assholeDescript() + ".  The cool feel of it contrasts enough to send you over the edge relatively quickly, and you drop it as you feel your muscles spasm with the phantom orgasm ")
    ...
    static maleRapesOoze() {
        ...
        if (UTIL.rand(2) == 0) {
            ...
            //Balls ftw
            if (liveData.player.balls > 1) GUI.outputText("anointing the slime's face with your " + liveData.player.sackDescript() + " as you do.  ")
            //Ball-less bitches
            else GUI.outputText("probing at the 'mouth' of the creature with your free hand.  ")
            //Continue the setup...
            ...
            if (liveData.player.cocks[0].cockLength >= 12) GUI.outputText(", down its throat,")
            if (liveData.player.cocks[0].cockLength >= 24) GUI.outputText(" into its torso,")
            if (liveData.player.cocks[0].cockLength >= 48) GUI.outputText(" and all the way through to its hips,")
            ...
            if (liveData.player.cor >= 60) GUI.outputText("You wonder for a moment if you've broken the creature's membranous skin, then shrug and decide you don't care.")
            //player shows concern for monster
            else GUI.outputText("You wonder for a moment if you've broken the creature's membranous skin, but the continuing presence of the creature's throbbing erection implies that it's in relatively little distress.")
            //New PG
            GUI.outputText("<br><br>")
            ...
            //Balls ftw
            if (liveData.player.balls > 1) GUI.outputText(" as your " + liveData.player.ballsDescriptLight() + " slap against its face")
            ...
            //vaginal orgasm text
            if (liveData.player.vaginas.length > 0) GUI.outputText("  This is quickly followed by a quivering between your nether-lips as your " + liveData.player.vaginaDescript(0) + " quivers in orgasm shortly afterwards.")
            GUI.outputText(
                "  You pull your " +
                    liveData.player.cockDescript(0) +
        ...
        //anal rape text
            ...
            if (liveData.player.cockTotal() > 1) {
                ...
                if (liveData.player.cockTotal() == 2) GUI.outputText("larger")
                else GUI.outputText("largest")
                GUI.outputText(
                    " of your" + liveData.player.multiCockDescriptLight() + " as you grab the slime and roughly try to turn it over.  Surprisingly it maintains its cohesion and flops onto its side, its erection flopping wetly onto the ground.  "
                )
            }
            //Single dicks
            else
            ...
            if (liveData.player.cocks[0].cockLength >= 24) GUI.outputText("By the time you finally bottom out, your " + liveData.player.cockDescript(0) + " is nearly halfway through its torso.")
            else if (liveData.player.cocks[0].cockLength >= 48) GUI.outputText("By the time you finally bottom out, your " + liveData.player.cockDescript(0) + " reaches nearly into the slime's neck.")
            else if (liveData.player.cocks[0].cockLength >= 60)
                GUI.outputText(
                    "Your penis keeps going deeper and deeper, through the chest, neck, and head, before finally stretching the slime as far as it needs to go to accommodate the full length of your " + liveData.player.cockDescript(0) + "."
                )
            GUI.outputText(
            ...
            if (liveData.player.cockTotal() > 1) GUI.outputText("your " + liveData.player.multiCockDescriptLight() + " explodes into orgasm, your load filling the space inside the creature and spraying in thick streams across its back.")
            else GUI.outputText("exploding inside it, your load filling the space around your cock with a white, milky fluid.")
            //vaginal orgasm text
            if (liveData.player.vaginas.length > 0) GUI.outputText("  As you pump your jizz into the creature an almost electrical shock runs through your body, starting from your " + liveData.player.vaginaDescript(0) + " and radiating outwards.")
    ...
    static slimeLoss() {
        if (liveData.gameFlags[FLAG.SFW_MODE] == 1) return //No rape in SFW mode.
        GUI.clearOutput()
    ...
    static oozeRapesYouVaginally() {
        ...
        if (liveData.player.cor <= 20) {
            ...
            if (liveData.player.cockTotal() > 0) {
                ...
                if (liveData.player.cockTotal() > 1) {
                    ...
                    if (liveData.player.cockTotal() > 2) GUI.outputText("switching between cocks as you rapidly stroke yourself in time with the creature's movements.  ")
                    else GUI.outputText("rapidly stroking one cock with each hand, working yourself in time with the creature's movements.  ")
                }
                //Single dick
                else
                    GUI.outputText(
            ...
            GUI.outputText(
                "After a short time the creature penetrates you fully again and stops, and you realize that at some point you had begun moving your hips with it.  You gasp in surprise as the slime's cock begins churning inside of you, massaging you from the inside in a uniquely pleasurable fashion.<br><br>"
            )
            GUI.outputText(
            ...
            if (liveData.player.cockTotal() > 0) {
                //Multi
                if (liveData.player.cockTotal() > 1) GUI.outputText("Your " + liveData.player.multiCockDescript() + " climaxes as well, sending rope after rope of hot, thick cum onto your face and chest.")
                //Single
                else
                    GUI.outputText(
        ...
        //standard result
            ...
            //Loose
                ...
                //SLUT!
                if (liveData.player.vaginalCapacity() >= 30) GUI.outputText(" at least partly")
                GUI.outputText(
                ...
                if (liveData.player.vaginalCapacity() >= 30) GUI.outputText(" almost perfectly.<br>")
                else GUI.outputText(" perfectly.<br>")
            }
            GUI.outputText(
    ...
    static oozeRapesYouOrally() {
        ...
        if (liveData.player.cor < 20) GUI.outputText("you blush as ")
        ...
        if (liveData.player.cor >= 70) {
            ...
            if (liveData.player.cockTotal() > 0) {
                ...
                else GUI.outputText("You feel your " + liveData.player.multiCockDescriptLight() + " begin to harden just slightly as you work the creature, a surge of arousal flowing through your body.")
            }
            if (liveData.player.vaginas.length > 0) {
                GUI.outputText("Your " + liveData.player.vaginaDescript(0) + " ")
                if (liveData.player.cockTotal() > 0) GUI.outputText("also ")
                ...
                if (liveData.player.cockTotal() < 1 || liveData.player.lust <= 40) GUI.outputText("You try to reach for yourself to masturbate, but find yourself unable to reach around the slime's massive bulk.")
            }
            GUI.outputText(
            ...
            if (liveData.player.lust > 40) {
                ...
                liveData.player.orgasm()
            } else GUI.outputText("The creature withdraws while you're distracted, leaving no traces of its presence save for what it left on your body.")
    ...
    static oozeButtRapesYou() {
        ...
        //Girls get fondled.
        if (liveData.player.vaginas.length > 0) GUI.outputText(liveData.player.vaginaDescript(0) + " and " + liveData.player.assholeDescript())
        //No coochie
        else {
            //Ballsy
            if (liveData.player.balls >= 2) GUI.outputText(liveData.player.assholeDescript() + " and " + liveData.player.ballsDescript())
            //No balls, no pussy, just butt
            else GUI.outputText(liveData.player.assholeDescript())
        }
        ...
        if (liveData.player.analCapacity() < 18 || liveData.player.ass.analLooseness == 0) {
            ...
            if (liveData.player.cor <= 50) GUI.outputText("You feel a bit of fear as you tremble under the creature's weight, worried about the damage that its massive size might do to your ass.  ")
            else if (liveData.player.cor < 100) GUI.outputText("In spite of your worries about the creature's size, you can't help but feel a little bit excited at the idea of having this beast take you from behind.  ")
            //and something special for the total whorebags that have somehow not already wrecked their asses.
            else GUI.outputText("You almost lick your lips in anticipation and push your hips back greedily, eager to feel the monster's cock stretch your asshole wide.  ")
        }
        GUI.outputText(
        ...
        //cock arousal
        if (liveData.player.cockTotal() > 0) GUI.outputText("You feel pangs of pleasure run through your body as its member repeatedly strokes your prostate from the inside, stimulating it.  ")
        ...
        if (liveData.player.vaginas.length > 0) {
            ...
            //vaginal-only orgasm
            else GUI.outputText("The creature finally plunges a pair of fingers into your " + liveData.player.vaginaDescript(0) + ", the sudden shock finally pushing you to orgasm.  ")
        }
        //mangasm
    ...
    static serviceLowCorruptionHighLust() {
        liveData.player.slimeFeed()
        GUI.outputText(
            "You find yourself unable to tear your eyes away from the creature as it undulates almost hypnotically in front of you, the heat in your crotch rising.  The rigid protrusion at its middle visibly pulses and throbs, and with a small grin you step closer to the ooze and strip off your clothing.<br><br>"
        )
        ...
        //Tall peeps
        if (liveData.player.tallness >= 82) GUI.outputText("  You grip the head of the member firmly with one of your large hands and tease the tip with your thumb for a moment before squeezing and sliding your hand to its base.")
        //Short peeps
        else GUI.outputText("  You run a finger up and down the creature's member a few times before tightly wrapping your hands around the tip and running them to its base.")
        ...
        if (liveData.player.vaginas.length > 0) {
            ...
            if (!liveData.player.vaginas[0].virgin) GUI.outputText("for that familiar full feeling ")
            GUI.outputText("and paranoid about allowing this creature inside of you.")
            //If you be hella tight
            if (liveData.player.vaginalCapacity() <= 18) GUI.outputText("  On top of that, you are unsure if the thick, foot and a half long appendage would even fit inside of you.")
            GUI.outputText("<br><br>") //this line present to ensure proper paragraph structure
        }
        //Penises foreplay
        if (liveData.player.cockTotal() > 0) {
            //Multicock folk get in their own way
            if (liveData.player.cockTotal() > 1) GUI.outputText("Your own erect " + liveData.player.multiCockDescriptLight() + " rub against the creature's as you work, getting in the way somewhat.  ")
            //Big single cocked folk get in their own way too!
            else if (liveData.player.cocks[0].cockThickness >= 36) GUI.outputText("Your own erect " + liveData.player.cockDescript(0) + " rubs against the creature's as you work, getting in the way somewhat.  ")
        ...
        if (liveData.player.tallness < 82) {
            ...
            if (liveData.player.gender == 1 || (liveData.player.gender == 3 && UTIL.rand(2) == 0)) {
                ...
                if (liveData.player.cocks.length > 1) {
                    ...
                    if (liveData.player.vaginas.length > 0) GUI.outputText("Your " + liveData.player.vaginaDescript(0) + " quivers and shares in the sensation as you grind yourself against the slime.  ")
                    //Create cumz
                    GUI.outputText(
            ...
            //female or herm version (if the herm didn't trigger male)
                ...
                if (liveData.player.breastRows.length > 0 && liveData.player.biggestTitSize() > 0) {
                    ...
                    //Tig ol' bitties
                    if (liveData.player.biggestTitSize() > 8) GUI.outputText("barely contains them.")
                    //Middling Boobiliciousness
                    else if (liveData.player.biggestTitSize() > 3) GUI.outputText("gently covers them.")
                    //Tiny ta-tas.
                    else GUI.outputText("easily smothers them.")
                ...
                if (liveData.player.cockTotal() > 0) {
                    ...
                    //Single prick
                        ...
                        GUI.outputText(
                            "You feel your " + liveData.player.cockDescript(0) + " surge to full size as the slime works you over.  It begins to throb and ache for release, a need made only more pressing by your desire to have it fondled.  "
                        )
                ...
                if (liveData.player.cockTotal() > 0) {
                    ...
                    if (liveData.player.cockTotal() > 1)
                        GUI.outputText(
                            "You let out a strangled gasp as your body, already straining from exertion, shudders again as your " + liveData.player.multiCockDescript() + " explodes into orgasm, showering both you and the creature in jizz.  "
                        )
                    //Single cocked herms
                    else
        ...
        //PHEW! that was a big one.  now...  we do it all again for large players, whee! and this is why this thing is so goddamn long.
            ...
            if (liveData.player.gender == 1 || (liveData.player.gender == 3 && UTIL.rand(2) == 0)) {
                ...
                //SingleCock
                    ...
                    //Compare cocks
                    if (liveData.player.cocks[0].cockType == ENUM.CockType.HUMAN) GUI.outputText("a near exact replica of your own " + liveData.player.cockDescript(0) + "!  ")
                    //Cocks are different so it's a 'human' penis
                    else GUI.outputText("a distinctly human penis.  ")
                }
                ...
                if (liveData.player.vaginas.length > 0) {
                    ...
                    GUI.outputText(
                        "<br><br>It isn't long before you can no longer hold yourself back and with a groan, you explode, showering the creature with your seed as your " +
                            liveData.player.multiCockDescript() +
                            " and " +
                            liveData.player.vaginaDescript(0) +
            ...
            //Female/Herm Masturbation
                ...
                if (liveData.player.breastRows.length > 0 && liveData.player.biggestTitSize() > 0) {
                    ...
                    if (liveData.player.biggestTitSize() > 8) GUI.outputText("Its hand expands, struggling to cover all of your " + liveData.player.allBreastsDescript() + ", ")
                    //Middling ta-tas get covered
                    else if (liveData.player.biggestTitSize() > 3) GUI.outputText("Its hand expands and gently covers your " + liveData.player.allBreastsDescript() + ", ")
                    //Small tits?  Pwned.
                    else GUI.outputText("Its hand easily smothers your " + liveData.player.allBreastsDescript() + ", ")
                    GUI.outputText(
                ...
                if (liveData.player.cockTotal() > 0) {
                    ...
                    if (liveData.player.biggestTitSize() == 0 || liveData.player.breastRows.length == 0) {
                        ...
                        //multi!
                        if (liveData.player.cockTotal() > 1) GUI.outputText("penii ")
                        //Single
                        else GUI.outputText("penis ")
                        //SURPRIZES!
