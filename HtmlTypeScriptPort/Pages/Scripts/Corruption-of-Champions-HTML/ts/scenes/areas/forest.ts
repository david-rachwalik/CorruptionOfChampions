...
abstract class AreasForest {
    static explore() {
        GUI.clearOutput()
        liveData.exploration.exploredForest++ //Increment counter
        var choice = []
        choice[choice.length] = 0 //Goblin/imp encounter
        if ((liveData.player.cor >= 25 || liveData.player.level >= 4 || liveData.gameFlags[FLAG.JOJO_CORRUPTION_STAGE] != 0) && liveData.gameFlags[FLAG.JOJO_CAMP] == 0 /* && flags[JOJO_DEAD_OR_GONE] == 0*/) choice[choice.length] = 1 //Jojo
            ...
            default:
                if (liveData.player.cor < 80) {
                    ...
                    if (liveData.player.gender == 2)
                        GUI.outputText(
                            "idly toying with your " + liveData.player.vaginaDescript(0) + " as you daydream about getting fucked by all kinds of monstrous cocks, from minotaurs' thick, smelly dongs to demons' towering, bumpy pleasure-rods."
                        )
