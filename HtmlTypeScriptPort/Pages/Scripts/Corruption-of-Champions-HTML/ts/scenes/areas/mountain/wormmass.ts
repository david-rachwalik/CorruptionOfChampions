...
abstract class WormsScene {
    ...
    GUI.outputText("You try to summon up an orgasm, but you're too tired and waste your time trying!");
    ...
    player.changeFatigue(40,2);
    GUI.outputText("With a great squeeze, you will your body to push out a load of semen filled with worms. You moan in pleasure and release as your fluids launch at your enemy. Worms and cum fly through the air in an attempt to coat your opponent with your new pets.  ", true);
    //Viable target?
    if (monster.short == "minotaur" || monster.short == "imp") {
    if (monster.lust > 70) {
    GUI.outputText("Your load washes over the " + monster.short + " and " + monster.pronoun1 + " stops dead in " + monster.pronoun3 + " tracks as " + monster.pronoun1 + " chokes and sputters to clear the cum from " + monster.pronoun3 + " face and nose to breathe. The " + monster.short + " stumbles, attempting to stand in your fresh cum puddle and quickly busts its ass on the ground. The worms quickly take over and swarm around the " + monster.short + "' s " + monster.cockDescriptShort(0) + ". With wild hunger, the worms easily push into the " + monster.short + "'s urethra and begin venturing into your victim's body. The " + monster.short + " begins to convulse wildly as " + monster.pronoun3 + " body begins to react to the squirming invaders. The " + monster.short + " quickly peaks and cum flies in all directions, along with some worms. You laugh hysterically as the " + monster.short + " must now endure the endless orgasms your new pets provide. You choose to unload one last batch on your fallen foe to ensure a good infestation and walk away to leave the " + monster.short + " in the hell of endless pleasure.\n", false);
    monster.lust = 100;
    combat.COMBAT.cleanupAfterCombat();
    return;
    }
    else {
    GUI.outputText("The monster watches your display as they step out of the way, a little grossed out by your actions.\n", false);
    monster.lust+=5;
    }
    }
    else if (monster.short == "lizan rogue") {
    GUI.outputText("The lizan makes no attempt to dodge your worm-infused attack. The worms hit him with a fierce clash and slide down his body, disappearing into the ground without even attempting to infect the lizan. Maybe they prefer mammals...");
    }
    else if (monster.short == "anemone") {
    //Infest vs. anemone:
    GUI.outputText("The white mess takes the anemone square in the torso, and the worms nearest her blue shaft waste no time in heading for it.  You both watch curiously as the lead worms ascend her dick, only to run headlong into the tentacles and slip off, stunned.  Eyes twinkling, the girl scoops up a handful of the squirming load still on her stomach and slurps it down greedily.  She smacks her lips and says, \"<i>Chewy!</i>\"\n", false);
    monster.HP += 20;
    //(minus PC lust, of course)
    }
    //nonviable
    else {
    GUI.outputText("While your fluids bathe the " + monster.short + " in your salty lust, the worms take no interest in your foe and scurry off.\n", true);
    }
    ...
    if (player.cockTotal() > 1) GUI.outputText("s are");
    else GUI.outputText(" is");
    GUI.outputText(" open and vulnerable to the piles of spunk-hungry parasites, and as you look on in shock, you see four of them slide into your worm-packed tunnel");
    if (player.cockTotal() > 1) GUI.outputText("s");
    GUI.outputText(", one after another!  No!  You reach for your [cock biggest] to somehow try and stop the infestation, but it's a futile gesture.  The worm pile on your [legs] is as big as the rest of you.  Worse still, you doubt you can pull out the ones that have already crawled inside.");

    GUI.outputText("\n\nGrabbing hold of [oneCock] with both hands, you pinch your fingers around ");
    if (player.cocks[player.biggestCockIndex()].cockThickness >= 6) GUI.outputText("as tight around the girth as you can");
    else GUI.outputText("tightly around your girth");
    GUI.outputText(", hoping to contain the squiggling menace in the upper reaches of your erection.");
    if (player.cockTotal() > 1) {
    GUI.outputText("  It's a vain effort, as your other penis");
    if (player.cockTotal() > 2) GUI.outputText("es are");
    else GUI.outputText(" is");
    GUI.outputText("already turning into ");
    if (player.cockTotal() == 2) GUI.outputText("an ");
    GUI.outputText("infestation highway");
    if (player.cockTotal() > 2) GUI.outputText("s");
    GUI.outputText(".  ");
    if (player.cockTotal() == 2) GUI.outputText("Its");
    else GUI.outputText("Their");
    GUI.outputText("inside");
    if (player.cockTotal() > 2) GUI.outputText("s are");
    else GUI.outputText(" is");
    GUI.outputText(" filled to the brim with moist wigglers.");
    }
    GUI.outputText("  You can feel narrow, tapered heads pushing at where you've pressed your urethra closed, and they try to force their way under again and again while more of their brethren pile in behind.  You stay resolute in your efforts to prevent the coming infection, pinching so hard it's painful and crying out as a result.  The pressure on your [cock biggest] wars with the way your body is reacting to the other sensations assaulting it.  Your [cock biggest] spasms powerfully when a big, fat worm perches on your [cockHead biggest], slithering all around it, leaving behind a trail of slimy gunk and sending accompanying tingles of delight through your rigid pole.");
    ...
    if (player.cockTotal() > 1) GUI.outputText("s");
    GUI.outputText(" to absorb the sensual nutrition.  Some of them even slop off the sides after the dripping whiteness, to get to it before it absorbs into the brittle, wasted earth.  You cum and cum, almost without end, yet each thick jet is slurped into the writhing mass immediately, leaving behind just the parasitic, clear slime.");
