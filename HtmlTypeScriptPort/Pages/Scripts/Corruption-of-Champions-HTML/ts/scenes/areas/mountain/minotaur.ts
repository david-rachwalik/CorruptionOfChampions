...
Minotaur.minoPheromones = function () {
    ...
    if (gameFlags[MINOTAUR_CUM_ADDICT] > 0 || gameFlags[MINOTAUR_CUM_ADDICTION_STATE] == 2) {
        if (rand(2) == 0) outputText("<br><b>You shiver with need, wanting nothing more than to bury your face under that loincloth and slurp out every drop of goopey goodness.</b> ")
        else outputText("<br><b>You groan and lick your lips over and over, craving the taste of him in your mouth.</b> ")
        lustDamage += 5 + rand(5)
    }
...
MinotaurScene.continueMinoVoyeurism = function () {
    ...
    if (player.cor < 33) outputText("in disgust.")
    ...
    if (gameFlags[MINOTAUR_AND_COWGIRL] == 0) outputText(" Apparently this isn't an uncommon show, and the locals enjoy it immensely. ")
    //Lust!
...
MinotaurScene.minoVictoryRapeChoices = function () {
    ...
    if (player.lust >= 33) {
        ...
        if (x >= 0 && player.cockTotal() > 1 && player.countCocksOfType(ENUM.CockType.TENTACLE) > 0) addButton(3, "TentacleDick", MinotaurScene.rapeMinotaurTentacles)
    ...
    //Not an addict
        ...
        if (monster.lust >= monster.maxLust())
            ...
            outputText(
                "You smile in satisfaction as the " + monster.name + " collapses, unable to continue fighting.  Sadly you realize your own needs have not been met.  Of course you could always rape the poor thing...<br><br>What do you do?",
                true
            )
    }
    //Not able to rape but a feeder
        ...
        if (monster.lust >= monster.maxLust())
            outputText(
                "You smile in satisfaction as the " + monster.name + " collapses, unable to continue fighting.  Sadly you realize your own need to breastfeed has not been met.  You could always let the poor thing have a drink...<br><br>Do you?",
                true
            )
        else
            outputText(
...
MinotaurScene.rapeMinotaurTentacles = function () {
    ...
    if (player.cor < 25)
        outputText(
            "You cry, petting his hair, smoothing it across his horns, justifying to yourself that you are raping this beast because this is how this world works as you maneuver your " + player.multiCockDescript() + " towards his tailhole.  ",
            false
        )
    ...
    if (player.cockTotal() != player.countCocksOfType(ENUM.CockType.TENTACLE)) {
        ...
        while (temp > 0 && temp2 == -1) {
            temp--
            if (player.cocks[temp].cockType != ENUM.CockType.TENTACLE) temp2 = temp
        }
        //failsafe
        if (temp2 == -1) temp2 = 0
