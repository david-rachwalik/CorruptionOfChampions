...
class Minotaur extends Creature {
    ...
    minoPheromones() {
        ...
        if (liveData.gameFlags[FLAG.MINOTAUR_CUM_ADDICT] > 0 || liveData.gameFlags[FLAG.MINOTAUR_CUM_ADDICTION_STATE] == 2) {
            if (UTIL.rand(2) == 0) GUI.outputText("<br><b>You shiver with need, wanting nothing more than to bury your face under that loincloth and slurp out every drop of goopey goodness.</b> ")
...
abstract class MinotaurScene {
    ...
    static minoVictoryRapeChoices() {
        ...
        if (liveData.player.lust >= 33) {
            ...
            if (liveData.player.hasCock() && x >= 0) GUI.addButton(0, "Use Cock", this.bumRapeaMinotaur)

            // Checking for vaginal fuck
            if (liveData.player.hasVagina()) GUI.addButton(1, "Use Vagina", this.girlRapeAMinotaur)

            // Checking for herm scene. Centaurs can't do the herm scene
            if (liveData.player.gender == 3 && x >= 0 && !liveData.player.isTaur()) GUI.addButton(2, "Use Both", this.minotaurGetsRapedByHerms)

            // Checking for tentacle fuck
            if (x >= 0 && liveData.player.cockTotal() > 1 && liveData.player.countCocksOfType(ENUM.CockType.TENTACLE) > 0) GUI.addButton(3, "TentacleDick", this.rapeMinotaurTentacles)
            ...
            if (liveData.player.hasCock()) {
                if (liveData.player.cockThatFits(80) >= 0) GUI.addButton(7, "MakeHimSuck", this.minotaurBlowjob)
            }
        }
        ...
        // Are you a feeder?
        if (liveData.player.findPerk(PerkLib.Feeder) >= 0) GUI.addButton(8, "Breastfeed", this.minotaurDrinksMilkNewsAtEleven)
        ...
        //No rape, no feeder
            ...
            if (liveData.monster.lust >= liveData.monster.maxLust()) GUI.outputText("You smile in satisfaction as the " + liveData.monster.name + " drops down on all fours and begins masturbating feverishly.")
        ...
        if (liveData.player.gender == 3 && liveData.player.isTaur()) GUI.outputText("<br>If you had a different body type you might be able to penetrate him while taking him, but as a centaur that's not an option.")
    }
    ...
    static minoRapeIntro() {
        ...
        if (liveData.monster.HP <= 0) {
            ...
            else GUI.outputText("  Glancing back at you with eyes full of despair, the minotaur seems to realize you won't be helping the predicament his " + liveData.monster.ballsDescriptLight() + " are in.")
        }
        //Monster lost to lust
            ...
            else GUI.outputText("  Glancing back at you with eyes full of despair, the minotaur doubts you'll help relieve the predicament his " + liveData.monster.ballsDescriptLight() + " are in.")
        }
    }
    ...
    static girlRapeAMinotaur() {
        ...
        if (liveData.player.vaginalCapacity() * 1.5 >= liveData.monster.cockArea(0))
            GUI.outputText(
                "Every stroke of his " + liveData.monster.cockDescriptShort(0) + " slides easily into your " + liveData.player.vaginaDescript(0) + ", allowing him to slowly build up a puddle of his wondrous druggy pre-seed into your aching womb.  "
            )
        //Resume!
        GUI.outputText(
    ...
    static rapeMinotaurTentacles() {
        ...
        var temp2 = 0
        GUI.clearOutput()
        if (liveData.monster.HP <= 0) GUI.outputText("The minotaur collapses backwards after your last blow. Defeated and tired, he can barely stand up as you approach.  ")
        else {
        ...
        if (liveData.player.cor >= 25 && liveData.player.cor < 50) GUI.outputText("You smoothly guide your " + liveData.player.multiCockDescript() + " toward his tailhole.  ")
        if (liveData.player.cor >= 50 && liveData.player.cor < 80) GUI.outputText("You roughly grab him, slamming him close to your " + liveData.player.multiCockDescript() + " - just how this beast likes it.  ")
        if (liveData.player.cor >= 80) GUI.outputText("You grin and roughly pull his horns towards your " + liveData.player.multiCockDescript() + ", ready to roughly ride this beast.  ")
        //Mixed dix
    ...
    static rapeMinotaurTentacle() {
        ...
        if (liveData.player.vaginas.length > 0) {
            ...
            if (liveData.player.cumQ() < 50) GUI.outputText("Your cock made quite a mess of things, and when you pull him out, your " + liveData.player.vaginaDescript(0) + " makes a wet sucking sound.  ")
        ...
        //Free ass!
            ...
            if (liveData.player.cumQ() < 50) GUI.outputText("Your cock in his ass made a mess of things, and when you pull out, it makes a wet sucking sound, trying to keep you inside.  ")
            if (liveData.player.cumQ() >= 50 && liveData.player.cumQ() < 400) GUI.outputText("Your cock in his ass spills forth, creating a puddle which will mat his fur later.  ")
            if (liveData.player.cumQ() >= 400)
    ...
    static takeMinoCumDirectly() {
        ...
        if (liveData.monster.HP < 1) GUI.outputText("The beast-man has fallen on his back, comatose and insensate, though his shaft has risen to full erectness, displacing his loincloth.")
        else GUI.outputText("The beast-man is on his back with his legs splayed, masturbating hopelessly.  His anguished moans tell you all you need to know - he'll never get off alone.")
    ...
    static minoGetsTitFucked() {
        ...
        if (liveData.player.totalCocks() > 0) GUI.outputText("  Your " + liveData.player.cockDescript(0) + " grows harder and more aroused than ever before while your ")
    ...
    static AddictNagaOnMinotaur() {
        ...
        if (liveData.player.hasCock()) GUI.outputText("furiously stroke your own " + liveData.player.multiCockDescriptLight() + ". Your muffled vocalizations of pleaure weakly join his own.<br><br>")
        else GUI.outputText("part the folds of your " + liveData.player.vaginaDescript(0) + ", sliding it inside and masturbating with a furious vigor. Your muffled vocalizations of pleaure weakly join his own.<br><br>")
    ...
    static layEggsInAMinotaurSpiderLike() {
        ...
        if (liveData.player.gender > 0) {
            GUI.outputText("<br><br>")
            if (liveData.player.hasCock()) GUI.outputText("Forcing [oneCock] between his bound thighs, you frot his flared bull-cock, your mixed pre soaking into the fur of his belly as you thrust against him.")
            if (liveData.player.hasVagina()) GUI.outputText("Grasping his bull-cock, you pull it against your pussy, rubbing your juices up and down his impressive length as your lips and clit grind on him, adding to your pleasure.")
        }
        ...
        else GUI.outputText("<br><br>Chuckling at the thought of the fun the other mountain denizens will have with your well-trussed side of beef, you grab his snout and force him to look you in the eyes.")
    ...
    static layBeeEggsInCowbutt() {
        ...
        if (liveData.player.cor < 50) GUI.outputText("You hesitate, feeling a surge of pity for the once-mighty creature, but your new instincts drive your egg-layer further into his prone body.  ")
        else
            GUI.outputText(
        ...
        //(if female:
        else if (liveData.player.hasVagina()) GUI.outputText("you rub your pussy hard against the minotaur's muscular ass, marking him with the scent of your fem-cum.")

        GUI.outputText(
            "<br><br>Finally, you feel powerful pulses moving through your bee-like abdomen as your eggs move down your ovipositor into their bovine incubator, each one feeling like a mini-climax of its own.  You're beginning to understand why the bee girls are so keen on laying eggs in you when you pass through the forest.  You could definitely get used to this."
        )

        //(If Silly mode:
        if (liveData.silly) GUI.outputText('<br><br>Looking down at the mass of bull-meat filled with eggs, you lean down and whisper into his ear, "<i>And that\'s how you make Scotch Eggs, cowboy.</i>"')
        liveData.player.dumpEggs()
        COMBAT.cleanupAfterCombat()
        liveData.player.orgasm()
    }
    ...
    static getRapedByMinotaur(autoRape = false) {
        ...
        if (liveData.player.vaginas.length > 0) {
            ...
            if (liveData.player.averageVaginalWetness() < 2) GUI.outputText("You aren't very wet, and fear the damage this beast will inflict on your " + liveData.player.vaginaDescript(0) + ".  ")
        ...
        if (liveData.player.biggestTitSize() > 0 && liveData.player.mostBreastsPerRow() > 1 && liveData.player.breastRows.length > 0) {
            ...
            if (liveData.player.biggestLactation() > 1.5) GUI.outputText("He then gives a grunt in what you take to be approval as your milk begins to squirt out. He licks a milk-coated finger in satisfaction.  ")
        }
        if (liveData.player.cockTotal() == 1) GUI.outputText("The bull-man bends forward a little, and grabs your " + liveData.player.cockDescript(0) + " in a crushing grip. He makes short jerking motions as he keeps thrusting into you.")
        if (liveData.player.cockTotal() > 1) GUI.outputText("The bull-man bends forward a little, and grabs one of your " + liveData.player.cockDescript(0) + "s in a crushing grip. He makes short jerking motions as he keeps thrusting into you.")
        if (liveData.player.vaginas.length > 0) {
            if (liveData.player.vaginalCapacity() < liveData.monster.cockArea(0))
                GUI.outputText(
        ...
        if (liveData.player.vaginas.length > 0) {
            if (liveData.player.averageVaginalWetness() >= 2) {
                ...
                if (liveData.player.averageVaginalWetness() == 4) GUI.outputText("You squirm and quiver, orgasming from the beast's rough usage, soaking him with your " + liveData.player.vaginaDescript(0) + ".  ")
        ...
        if (liveData.player.cockTotal() > 0 && liveData.player.sens + UTIL.rand(40) > 50) {
            ...
            if (liveData.player.cumQ() >= 250 && liveData.player.cumQ() < 500) GUI.outputText("erupting potent ropes of seed in thick bursts, splattering the cave walls and floors.  ")
            if (liveData.player.cumQ() >= 500)
                GUI.outputText(
    ...
    static getMinoHawtDawged() {
        ...
        if (liveData.player.lust >= liveData.player.maxLust()) {
            ...
            //(If Male or Herm:
            if (liveData.player.hasCock()) GUI.outputText("[cock biggest], which you frantically try to stroke and rub through your [armor], its throbbing arousal clearly evident.")
            //If Herm:
        ...
        else GUI.outputText("The last great blow from the Minotaur's giant, beefy fist hits you upside your head, the blow strong enough to send you tumbling into the dirt.")
        ...
        if (liveData.player.hasVagina()) GUI.outputText("your [vagina] clenching and unclenching as your entire hips try to push against the Minotaur's searching tongue, dripping with his spit - along with your own juices.")
    ...
    static getOralRapedByMinotaur() {
        ...
        if (liveData.player.hasBreasts()) GUI.outputText("  The bull-man grabs your " + liveData.player.chestDesc() + ", squeezing and mauling them with his calloused hand.")
