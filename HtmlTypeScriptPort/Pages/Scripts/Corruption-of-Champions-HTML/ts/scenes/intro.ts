...
abstract class CharCreation {
    ...
    static setGender(gender: ENUM.GenderType): void {
        switch (gender) {
    ...
    static setBuild(build: string): void {
        switch (build) {
    ...
    static customizeCharacterMenu() {
        ...
        if (liveData.player.hasCock()) GUI.outputText("Cock size: " + liveData.player.cocks[0].cockLength + '" long, ' + liveData.player.cocks[0].cockThickness + '" thick<br>')
    ...
    static confirmComplexion(complexion: string): void {
    ...
    static confirmHairColor(color: string): void {
...
abstract class Intro {
    ...
    static arrivalPartOne(): void {
        ...
        if (liveData.player.gender == ENUM.GenderType.GENDER_FEMALE) GUI.outputText("and your body seems to feel the same way, flushing as you feel a warmth and dampness between your thighs. ")
