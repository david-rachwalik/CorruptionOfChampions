...
abstract class COMBAT {
    ...
    static flee() {
        ...
        if (monster.refName == "Marae") {
            ...
            if (!liveData.player.canFly()) outputText("You may not be able to swim fast enough. ")
            else outputText("You grit your teeth with effort as you try to fly away but the tentacles grab your " + liveData.player.legs() + " and pull you down. ")
            outputText("It looks like you cannot escape. ")
            success = false
        }
        if (monster.refName == "Ember") {
            outputText("You take off")
            if (!liveData.player.canFly()) outputText(" running")
        ...
        if (monster.refName == "lizan rogue") {
            ...
            success = true
        } else if (liveData.player.canFly()) outputText("Gritting your teeth with effort, you beat your wings quickly and lift off! ")
        //Nonflying PCs
        else {
            //In prison!
            /*if (prison.inPrison) {
            ...
            /*else */ if (liveData.player.findStatusEffect(StatusEffects.NoFlee) >= 0) {
                if (monster.refName == "goblin") outputText("You try to flee but get stuck in the sticky white goop surrounding you.<br><br>")
                else outputText("You put all your skills at running to work and make a supreme effort to escape, but are unable to get away!<br><br>")
                success = false
            }
            //Nonstuck!
        ...
        if (liveData.player.tailType == TAIL_TYPE_RACCOON && liveData.player.earType == EARS_RACCOON && liveData.player.findPerk(PerkLib.Runner) >= 0) escapeMod -= 25
        ...
        if (success !== undefined && liveData.player.spe > rand(monster.spe + escapeMod)) {
            ...
            //Non-fliers flee
            else outputText(capitalize(monster.a) + monster.refName + " rapidly disappears into the shifting landscape behind you.")
        ...
        //FAIL FLEE
            ...
            if (liveData.player.canFly()) {
                ...
                if (monster.plural) outputText(capitalize(monster.a) + monster.refName + " manage to grab your " + liveData.player.legs() + " and drag you back to the ground before you can fly away!")
                else outputText(capitalize(monster.a) + monster.refName + " manages to grab your " + liveData.player.legs() + " and drag you back to the ground before you can fly away!")
            } else if (liveData.player.tailType == TAIL_TYPE_RACCOON && liveData.player.earType == EARS_RACCOON && liveData.player.findPerk(PerkLib.Runner) >= 0)
                // >>>>>>[P] FAIL
                ...
                if (liveData.player.balls > 0 && liveData.player.ballSize >= 24) {
                    if (liveData.player.ballSize < 48) outputText("With your " + liveData.player.ballsDescriptLight() + " swinging ponderously beneath you, getting away is far harder than it should be. ")
                    else outputText("With your " + liveData.player.ballsDescriptLight() + " dragging along the ground, getting away is far harder than it should be. ")
                }
                ...
                if (liveData.player.biggestTitSize() >= 35 || liveData.player.buttRating >= 20 || liveData.player.hipRating >= 20) {
                    ...
                    if (liveData.player.biggestTitSize() >= 35 && liveData.player.biggestTitSize() < 66) {
                        if (liveData.player.hipRating >= 20) {
                            ...
                            if (liveData.player.buttRating >= 20) outputText(liveData.player.buttDescript() + " and ")
                    ...
                    //FOR PLAYERS WITH MASSIVE BREASTS
                        ...
                        if (liveData.player.hipRating >= 20) {
                            ...
                            if (liveData.player.buttRating >= 20) outputText("causing the fat of your " + liveData.player.skinTone + liveData.player.buttDescript() + " to wobble heavily, ")
                        ...
                        else outputText("Your " + liveData.player.chestDesc() + " nearly drag along the ground, preventing you from moving quick enough to get escape.")
                    }
                    //FOR PLAYERS WITH EITHER GIANT HIPS OR BUTT BUT NOT THE BREASTS
                        ...
                        if (liveData.player.buttRating >= 20) outputText("causing your " + liveData.player.skinTone + liveData.player.buttDescript() + " to wobble obscenely ")
                        outputText("and forcing your body into an awkward gait that slows you down, preventing you from escaping.")
                    }
                    //JUST DA BOOTAH
                    else if (liveData.player.buttRating >= 20) outputText("Your " + liveData.player.skinTone + liveData.player.buttDescript() + " wobbles so heavily that you're unable to move quick enough to escape.")
                }
    ...
    static fantasize() {
        ...
        if (liveData.player.armor == Items.Armor.GooArmor) {
            ...
            if (liveData.player.gender > 0) outputText(" and genitals")
        ...
        if (lustGain >= 20) outputText("The fantasy is so vivid and pleasurable you wish it was happening now. You wonder if " + monster.a + monster.refName + " can tell what you were thinking.<br><br>")
        else outputText("<br>")
        ...
        switch (liveData.player.statusEffectValue(StatusEffects.Bind, 1)) {
            ...
            case BIND_TYPE_TENTACLE:
                ...
                if (rand(80) < 33 + liveData.player.str / 2) {
                    ...
                    damage += 5
                    liveData.player.changeHP(-damage, true, false)
                    if (liveData.player.cocks.length > 0) outputText("The creature continues spiraling around your cock, sending shivers up and down your body. You must escape or this creature will overwhelm you! ")
    ...
    static fatigueRecovery() {
        ...
        if (liveData.player.findPerk(PerkLib.EnlightenedNinetails) >= 0 || liveData.player.findPerk(PerkLib.CorruptedNinetails) >= 0) liveData.player.changeFatigue(-(1 + rand(3)), false)
    }
    static teaseXP(XP: number) {
        while (XP > 0) {
            XP--
    ...
    static gameOver() {
        ...
        if (silly) gameOverTexts[4] = "You cannot give up just yet. " + liveData.player.name + "! Stay determined..." //You are still filled with DETERMINATION.
