...
class Inventory {
    ...
    inventoryMenu() {
        ...
        if (inCombat()) {
            callNext = this.inventoryCombatHandler
        } else {
            callNext = this.inventoryMenu
        }
        menu()
        for (var i = 0; i < 10; i++) {
            //Supports up to 10 items. You begin with 3 slots.
            if (player.itemSlots[i].quantity > 0 && i < player.getMaxSlots()) {
                addButton(i, player.itemSlots[i].itype.shortName + " x" + player.itemSlots[i].quantity, this.useItemInInventory, i, null, null, player.itemSlots[i].itype.getTooltipDescription(), capitalize(player.itemSlots[i].itype.longName))
            }
        }
        addButton(10, "Unequip", this.unequipMenu)
    ...
    unequipMenu() {
        ...
        if (player.weapon.id != "Nothing") {
            addButton(0, "Weapon", this.unequipWeapon, null, null, null, player.weapon.getTooltipDescription(), capitalizeFirstLetter(player.weapon.equipmentName))
        }
        ...
        if (player.armor.id != "Nothing") {
            addButton(5, "Armour", this.unequipArmor, null, null, null, player.armor.getTooltipDescription(), capitalizeFirstLetter(player.armor.equipmentName))
        }
        ...
        addButton(14, "Back", this.inventoryMenu)
    }
    ...
    useItemInInventory(slotNum) {
        ...
        if (item.canUse()) {
            ...
            this.useItem(item, player.itemSlots[slotNum])
            return
        }
        //}
        //else {
    ...
    deleteItemPrompt(item, slotNum) {
        ...
        menu()
        addButton(0, "Yes", this.deleteItem, item, slotNum)
        addButton(1, "No", this.inventoryMenu)
        //doYesNo(deleteItem, inventoryMenu);
    }

    // Deletes an item
    deleteItem(item, slotNum) {
        clearOutput()
    ...
    takeItemFull(itype, showUseNow, source) {
        ...
        menu()
        for (x = 0; x < 10; x++) {
            if (player.itemSlots[x].itype != Items.NOTHING && x < player.getMaxSlots()) addButton(x, player.itemSlots[x].itype.shortName + " x" + player.itemSlots[x].quantity, this.replaceItem, itype, x)
        }
        if (source != null) {
            var currentItemSlot = source
            addButton(12, "Put Back", createCallBackFunction(this.returnItemToInventory, itype, false))
        }
        if (showUseNow) addButton(13, "Use Now", this.useItemNow, itype, source)
    ...
    returnItemToInventory(item, showNext) {
        //Used only by items that have a sub menu if the player cancels
        //Return item to inventory
        if (currentItemSlot == null) {
            this.takeItem(item, callNext, callNext, null) //Give player another chance to put item in inventory
	...
	*/
    ...
    stashMenu() {
		...
		*/
        ...
        if (gameFlags[HAS_WEAPON_RACK] == 1) {
            ...
            addButton(2, "W.Rack Put", this.pickItemWeaponRack)
            if (this.weaponRackFilled()) addButton(3, "W.Rack Take", this.takeFromWeaponRack)
        }
        //Armor Rack
        ...
        if (gameFlags[HAS_ARMOR_RACK] == 1) {
            ...
            addButton(5, "A.Rack Put", this.pickItemArmorRack)
            if (this.armorRackFilled()) addButton(6, "A.Rack Take", this.takeFromArmorRack)
        }
        ...
        if (gameFlags[HAS_EQUIPMENT_RACK] == 1) {
            ...
            addButton(7, "S.Rack Put", this.pickItemShieldRack)
            if (this.shieldRackFilled()) addButton(8, "S.Rack Take", this.takeFromShieldRack)
        }
    ...
    weaponRackFilled() {
        if (this.itemAnyInStorage(player.itemSlots, 10, 18)) {
            var itemList = []
            for (x = 10; x < 19; x++) {
    ...
    shieldRackFilled() {
        if (this.itemAnyInStorage(player.itemSlots, 28, 36)) {
            var itemList = []
            for (x = 28; x < 37; x++) {
                if (player.itemSlots[x].quantity > 0) itemList[itemList.length] = player.itemSlots[x].itype.longName
                outputText("  It currently holds " + formatStringArray(itemList) + ".")
    ...
    takeFromWeaponRack() {
        callNext = this.takeFromWeaponRack
        this.takeFromStorage(player.itemSlots, 9, 18, "rack")
    }

    takeFromArmorRack() {
        callNext = this.takeFromArmorRack
        this.takeFromStorage(player.itemSlots, 19, 27, "rack")
    }

    takeFromShieldRack() {
        callNext = this.takeFromShieldRack
        this.takeFromStorage(player.itemSlots, 28, 36, "rack")
    }
    ...
    placeInStorage(placeInStorageFunction, typeAcceptableFunction, text, showEmptyWarning) {
        clearOutput()
        hideUpDown()
        ...
        addButton(14, "Back", this.stashMenu)
    }
	...
	*/
    ...
    placeInWeaponRack(slotNum) {
        ...
        doNext(this.pickItemWeaponRack)
    }

    // For armor rack
    placeInArmorRack(slotNum) {
        this.placeIn(19, 27, slotNum)
        doNext(this.pickItemArmorRack)
    }

    // For shield rack
    placeInShieldRack(slotNum) {
        this.placeIn(28, 36, slotNum)
        doNext(this.pickItemShieldRack)
    }
	...
	*/
    ...
    placeIn(startSlot, endSlot, slotNum) {
        ...
        for (x = startSlot; x < endSlot; x++) {
            ...
            if (player.itemSlots[x].itype == player.itemSlots[slotNum].itype && player.itemSlots[x].quantity < 5) {
    ...
    takeFromStorage(Array, startSlot, endSlot, text) {
        clearOutput()
        hideUpDown()
        ...
        for (var x = startSlot; x < endSlot; x++, button++) {
            if (player.itemSlots[x].quantity > 0) addButton(button, player.itemSlots[x].itype.shortName + " x" + player.itemSlots[x].quantity, this.pickFrom, x)
        }
        addButton(14, "Back", this.stashMenu)
    }
    ...
    pickFrom(slotNum) {
        ...
        player.itemSlots[slotNum].removeOneItem()
        this.takeItem(itype, callNext, callNext, player.itemSlots[slotNum])
    }

    itemAnyInStorage(startSlot, endSlot) {
        for (var x = startSlot; x < endSlot; x++) {
            if (player.itemSlots[x] != Item.NOTHING) if (player.itemSlots[x].quantity > 0) return true
        }
        return false
    }
}
