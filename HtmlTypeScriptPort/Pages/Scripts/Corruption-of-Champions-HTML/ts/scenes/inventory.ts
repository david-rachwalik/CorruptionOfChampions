...
abstract class Inventory {
    ...
    static takeItem(itype: IItem, nextAction?: () => void, overrideAbandon?: () => void, source?: IItemSlot) {
        ...
        if (temp >= 0) {
            ...
            GUI.outputText("You place " + itype.longName + " in your " + this.inventorySlotName[temp] + " pouch, giving you " + liveData.player.itemSlots[temp].quantity + " of them.")
            this.itemGoNext()
            return
        }
        ...
        if (temp >= 0) {
            ...
            GUI.outputText("You place " + itype.longName + " in your " + this.inventorySlotName[temp] + " pouch.")
            this.itemGoNext()
            return
        }
    ...
    static useItemInInventory(slotNum: number) {
        GUI.clearOutput()
    ...
    static deleteItemPrompt(item: IItem, slotNum: number) {
        GUI.clearOutput()
        GUI.outputText("Are you sure you want to destroy " + liveData.player.itemSlots[slotNum].quantity + "x " + item.shortName + "?  You won't be able to retrieve " + (liveData.player.itemSlots[slotNum].quantity == 1 ? "it" : "them") + "!")
        GUI.menu()
    ...
    // Deletes an item
    static deleteItem(item: IItem, slotNum: number) {
        GUI.clearOutput()
        GUI.outputText(liveData.player.itemSlots[slotNum].quantity + "x " + item.shortName + " " + (liveData.player.itemSlots[slotNum].quantity == 1 ? "has" : "have") + " been destroyed.")
        liveData.player.destroyItems(item, liveData.player.itemSlots[slotNum].quantity)
        GUI.doNext(this.inventoryMenu)
    }

    // Use an item
    static useItem(item: IItem, fromSlot: IItemSlot) {
        item.useText()
        /*if (item) {
    ...
    static takeItemFull(itype: IItem, showUseNow: boolean, source: IItemSlot) {
        GUI.outputText("There is no room for " + itype.longName + " in your inventory.  You may replace the contents of a pouch with " + itype.longName + " or abandon it.")
        GUI.menu()
        for (let x = 0; x < 10; x++) {
            if (liveData.player.itemSlots[x].itype != Items.NOTHING && x < liveData.player.getMaxSlots()) GUI.addButton(x, liveData.player.itemSlots[x].itype.shortName + " x" + liveData.player.itemSlots[x].quantity, this.replaceItem, itype, x)
        }
        if (source != null) {
        ...
        if (showUseNow) GUI.addButton(13, "Use Now", this.useItemNow, itype, source)
    ...
    static returnItemToInventory(item: IItem, showNext: boolean) {
    ...
    static useItemNow(item: IItem, source: IItemSlot) {
        GUI.clearOutput()
        if (item.canUse()) {
            //If an item cannot be used then canUse should provide a description of why the item cannot be used
    ...
    static replaceItem(itype: IItem, slotNum: number) {
        GUI.clearOutput()
        if (liveData.player.itemSlots[slotNum].itype == itype)
            ...
            if (liveData.player.itemSlots[slotNum].quantity == 1) GUI.outputText("You throw away " + liveData.player.itemSlots[slotNum].itype.longName + " and replace it with " + itype.longName + ".")
            else GUI.outputText("You throw away " + liveData.player.itemSlots[slotNum].itype.longName + "(x" + liveData.player.itemSlots[slotNum].quantity + ") and replace it with " + itype.longName + ".")
	...
	*/
    ...
    static stashMenu() {
		...
		*/
        ...
        if (liveData.gameFlags[FLAG.HAS_WEAPON_RACK] == 1) {
            ...
            if (this.weaponRackFilled()) GUI.addButton(3, "W.Rack Take", this.takeFromWeaponRack)
        }
        //Armor Rack

        if (liveData.gameFlags[FLAG.HAS_ARMOR_RACK] == 1) {
            GUI.outputText("Your camp has an armor rack set up to hold your various sets of gear.  It appears to be able to hold nine different types of armor.<br><br>")
            GUI.addButton(5, "A.Rack Put", this.pickItemArmorRack)
            if (this.armorRackFilled()) GUI.addButton(6, "A.Rack Take", this.takeFromArmorRack)
        }
        ...
        if (liveData.gameFlags[FLAG.HAS_EQUIPMENT_RACK] == 1) {
            ...
            if (this.shieldRackFilled()) GUI.addButton(8, "S.Rack Take", this.takeFromShieldRack)
        }
    ...
    static allAcceptable(itype: string) {
        return true
    }

    static armorAcceptable(type: string) {
        if (type == ITEM_TYPE_ARMOUR) return true
        return false
    }

    static weaponAcceptable(type: string) {
        if (type == ITEM_TYPE_WEAPON) return true
        return false
    }

    static shieldAcceptable(type: string) {
    ...
    static placeInStorage(placeInStorageFunction: (a: number) => void, typeAcceptableFunction: (a: string) => boolean, text: string, showEmptyWarning: boolean) {
        GUI.clearOutput()
        GUI.hideUpDown()
	...
	*/
    ...
    static placeIn(startSlot: number, endSlot: number, slotNum: number) {
    ...
    static takeFromStorage(startSlot: number, endSlot: number, text = "") {
        GUI.clearOutput()
        GUI.hideUpDown()
        ...
        for (let x = startSlot; x < endSlot; x++, button++) {
            if (liveData.player.itemSlots[x].quantity > 0) GUI.addButton(button, liveData.player.itemSlots[x].itype.shortName + " x" + liveData.player.itemSlots[x].quantity, this.pickFrom, x)
        }
        GUI.addButton(14, "Back", this.stashMenu)
    }

    static pickFrom(slotNum: number) {
        GUI.clearOutput()
    ...
    static itemAnyInStorage(startSlot: number, endSlot: number) {
