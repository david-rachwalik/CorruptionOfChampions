...
class Imp extends Creature {
    ...
    lustMagicAttack() {
        ...
        //Lust check
        if (liveData.player.lust < 30) GUI.outputText("You feel strangely warm. ")
        if (liveData.player.lust >= 30 && liveData.player.lust < 60) GUI.outputText("Blood rushes to your groin as a surge of arousal hits you, making your knees weak. ")
        if (liveData.player.lust >= 60) GUI.outputText("Images of yourself fellating and fucking the imp assault your mind, unnaturally arousing you. ")
        //Genitals check
        if (liveData.player.cocks.length > 0) {
            if (liveData.player.lust >= 60) GUI.outputText("You feel your " + liveData.player.multiCockDescriptLight() + " dribble pre-cum.")
            else if (liveData.player.lust >= 30 && liveData.player.cocks.length == 1) GUI.outputText("Your " + liveData.player.cockDescript(0) + " hardens, distracting you further.")
            else if (liveData.player.lust >= 30 && liveData.player.cocks.length > 1) GUI.outputText("Your " + liveData.player.multiCockDescriptLight() + " harden uncomfortably.")
            if (liveData.player.hasVagina()) GUI.outputText(" ")
        }
...
abstract class ImpScene {
    static victoryAgainstImp() {
        ...
        if (canFeed) {
            ...
            else GUI.outputText("  You're not really turned on enough to rape it, but it might be fun to force it to guzzle your breast-milk.<br><br>Do you breastfeed it?")
        ...
        if (liveData.player.lust >= 33) {
            if (liveData.player.hasCock()) {
                if (liveData.player.cockThatFits(liveData.monster.analCapacity()) == -1) GUI.outputText("<br><br><b>You're too big to rape an imp with " + liveData.player.oMultiCockDesc() + ".</b>")
                else {
                    ...
                    if (liveData.player.hasItem(Items.Consumables.Condom) && !liveData.player.isTaur()) GUI.addButton(5, "Use Condom", ImpScene.rapeImpWithDick, true)
                }
            }
            if (liveData.player.hasVagina()) {
                if (liveData.player.isTaur()) GUI.addButton(1, "Group Vaginal", ImpScene.centaurGirlOnImps)
                else GUI.addButton(1, "Female Rape", ImpScene.rapeImpWithPussy)
            }
        ...
        if (canFeed) GUI.addButton(3, "Breastfeed", ImpScene.areImpsLactoseIntolerant)
        ...
        if (liveData.player.canOvipositBee()) GUI.addButton(8, "Oviposit", ImpScene.putBeeEggsInAnImp)
    ...
    static rapeImpWithDick(condomed = false) {
        ...
        if (liveData.player.cocks.length == 1) {
            ...
            else GUI.outputText("stroking it to full hardness languidly.")
            //}
            GUI.outputText(
                "<br><br>With no foreplay, you press your " +
            ...
            if (liveData.player.cocks[0].cockType == ENUM.CockType.CAT) GUI.outputText(" The tiny creature's claws dig into your sides at the feeling of soft, hooked barbs stroking his sensitive insides.")
            if (liveData.player.cocks[0].cockLength >= 7 && liveData.player.cocks[0].cockLength <= 12) GUI.outputText(" Each thrust obviously distorts the imp's abdomen. It amazes you that it doesn't seem to be hurting him.")
            ...
            if (liveData.player.cumQ() >= 14 && liveData.player.cumQ() <= 30) GUI.outputText(" Your orgasm drags on and on, until your slick jism is dripping out around your " + liveData.player.cockDescript(x) + ".")
            ...
            if (liveData.player.cumQ() > 100)
                GUI.outputText(
                    " Your orgasm only seems to grow more and more intense as it goes on, each spurt more powerful and copious than the last. " +
                        (condomed
            ...
            GUI.outputText(
                "<br><br>Satisfied at last, you pull him off just as he reaches his own orgasm, splattering his hot demon-cum all over the ground.  You drop the imp hard and he passes out, dripping " +
        ...
        if (liveData.player.cocks.length >= 2) {
            ...
            else GUI.outputText("stroking one of your members to full hardness languidly.")
            //}
            GUI.outputText(
                "<br><br>With no foreplay, you press a " +
                    liveData.player.cockDescript(x) +
                    " against his tight little pucker and ram it in to the hilt. The imp's eyes bulge in surprise even as a thick stream of pre leaks from his " +
                    liveData.monster.cockDescriptShort(0) +
                    ". You grab him by his distended waist and brutally rape the little demon, whose claws stay busy adding to his pleasure."
            ...
            if (liveData.player.cumQ() > 100)
                GUI.outputText(
                    " Your orgasm only seems to grow more and more intense as it goes on, each spurt more powerful and copious than the last. T" +
                        (condomed
                            ? "Your condom swells nearly to the point of bursting, and tiny jets of cum squirt out around your " + liveData.player.cockDescript(x) + " with each thrust."
                            : "The imp begins to look slightly pregnant as you fill him, and tiny jets of cum squirt out around your " + liveData.player.cockDescript(x) + " with each thrust.") +
                        ""
            ...
            GUI.outputText(
                "<br><br>Satisfied at last, you pull him off just as he reaches his own orgasm, splattering his hot demon-cum all over the ground.  You drop the imp hard and he passes out, dripping mixed fluids that seem to be absorbed by the dry earth as fast as they leak out."
            )
        }
        liveData.player.orgasm()
        if (!condomed) liveData.player.modStats(["cor", 1])
        COMBAT.cleanupAfterCombat()
    }
    ...
    static rapeImpWithPussy() {
        ...
        if (liveData.player.vaginalCapacity() < liveData.monster.cockArea(0)) {
            ...
            GUI.outputText(
                "<br><br>You lose track of time as you languidly pump against the imp's " +
                    liveData.monster.cockDescriptShort(0) +
        ...
        //Big enough!
            ...
            GUI.outputText(
                "<br><br>The two of you lay there a moment while you recover, at last separating as you rise up off his " + liveData.monster.cockDescriptShort(0) + ".  Spunk drips down your legs, quickly wicking into your skin and disappearing."
            )
    ...
    static centaurOnImpStart() {
        ...
        if (liveData.monster.HP == 0) GUI.outputText("panting in exhaustion")
        else GUI.outputText("masturbating furiously")
        GUI.outputText(
        ...
        //<< Cor <50 >>
        if (liveData.player.cor < 50) GUI.outputText("  You lick your lips slightly as you begin to approach the small figure.")
        else GUI.outputText("You lick your lips obscenely as you approach the small figure.<br><br>")
        ...
        var x = liveData.player.cockThatFits(liveData.monster.analCapacity())
        if (x >= 0 && !liveData.player.hasVagina()) ImpScene.centaurOnImpMale()
        else if (liveData.player.hasVagina() && x < 0) ImpScene.centaurOnImpFemale()
        else {
    ...
    static centaurOnImpMale(vape = false) {
        ...
        if (liveData.player.cockTotal() > 1 && liveData.player.cocks[liveData.player.biggestCockIndex()].cockLength >= 24) {
            ...
            //Really big cock, high corruption
                ...
                if (liveData.player.biggestTitSize() >= 0) {
                    ...
                    //<<nips have pussies>>
                    if (liveData.player.hasFuckableNipples()) GUI.outputText(" and fingering your " + liveData.player.nippleDescript(0) + "s")
                    GUI.outputText(" as you drool slightly in absolute pleasure.  ")
                }
                GUI.outputText(
                    "When the creature's noises lessen and all you can hear is the sloppy sounds of its ass being fucked you push yourself in a single mighty heave, grinding the creature into the rock and eliciting one last scream that pushes you over.<br><br>"
                )
                //<<GoTo I1>>
        ...
        // Player has multicock, but not huge ones
            ...
            //<<Cor 80+>>
                ...
                //<<if balls>>
                if (liveData.player.balls > 0) GUI.outputText(liveData.player.ballsDescriptLight() + ".<br><br>")
                else GUI.outputText("groin.<br><br>")
                //<<GoTo I3 then return>>
                ImpScene.centaurOnImpResults(3)
                GUI.outputText("A guttural moan escapes your mouth as you realize the creature has completely passed out underneath you.  ")
                if (liveData.player.hasFuckableNipples()) GUI.outputText("Shoving your fingers deep into your " + liveData.player.nippleDescript(0) + "s")
                else GUI.outputText("With a fierce tug on your " + liveData.player.nippleDescript(0) + "s")
    ...
    static centaurOnImpFemale(vape = false) {
        ...
        if (liveData.player.vaginas[0].vaginalLooseness <= ENUM.VaginalLoosenessType.VAGINA_LOOSENESS_GAPING) {
            ...
            //Not virgin fucking flavors
                ...
                if (liveData.player.vaginalCapacity() < liveData.monster.cockArea(0)) GUI.outputText("It groans in delight at your incredible tightness and shoves itself forward even harder.  ")
        ...
        //<<Gaping>>
            ...
            //<<Cor 50+>>
            if (liveData.player.cor >= 50) GUI.outputText("You take comfort in knowing that at least there is a cock inside of you, and that soon it will be filling you with its seed.  Perhaps it might even impregnate you!  ")
            GUI.outputText(
            ...
            //<<Cor <80>>
            if (liveData.player.cor < 80) GUI.outputText("Tears pour out of your eyes and you are sure you must be bleeding slightly, ")
            //<<Cor <50>>
            if (liveData.player.cor < 50) GUI.outputText("and you hang on to the tree, afraid of the pain from even the slightest movement.  ")
            //<<Cor 50+>>
            else GUI.outputText("and you hang on to the tree, grunting like a rutting animal as you delight in the intense pain.  ")
            //<<Cor 80+>>
            if (liveData.player.cor >= 80) GUI.outputText("You howl out in pain and pleasure, bucking and hoping to intensify the sensation, hurling enticements and insults at the imp like a slut.  ")
            //<<Cor 50+, Breasts>>
            if (liveData.player.cor >= 50 && liveData.player.biggestTitSize() >= 2) {
                ...
                //<<w/ nip-pussies>>
                if (liveData.player.hasFuckableNipples()) GUI.outputText(" and shoving your fingers into your " + liveData.player.nippleDescript(0) + ".  ")
                else GUI.outputText(".  ")
                //<</Breasts>>
            }
            ...
            if (liveData.player.analCapacity() >= 35) {
                ...
                if (liveData.player.ass.analLooseness < 4 && liveData.player.cor < 80) GUI.outputText("Your howl of pain leaves your throat raw.  ")
                else GUI.outputText("Your howl of perverse pleasure leaves your throat raw.  ")
            }
            GUI.outputText(
                "<br><br>It is a relief when you feel the creature's sperm filling your womb and lubricating your raw cervix, your own body is wrecked by an intense orgasm while it breeds you.  You pass out, waking up to find that the imp has slipped out of you and is lying unconscious and coated completely in a mixture of your juices and his own. After looking for anything you might be able to take away from him you limp away, you "
            )
            if (liveData.player.cor < 80) GUI.outputText("promise to yourself that you will not do that again.")
            else GUI.outputText("find your cunt juices already dripping down your legs in anticipation of doing this again.")
            liveData.player.orgasm()
            COMBAT.cleanupAfterCombat()
            return
        }
    ...
    static centaurGirlOnImps() {
        ...
        if (liveData.player.hasVagina()) {
            GUI.outputText(
                "The imps approach you, their various genitalia glistening in the sun and drawing your attention. Their cocks swing lewdly with every flap of their wings, but you turn around, wanting their ministrations to be a surprise.<br><br>"
            )
            ...
            //[Has breasts:
            if (liveData.player.biggestTitSize() > 1) GUI.outputText("  Your hands reflexively grab your " + liveData.player.chestDesc() + " and mash them about.")
            GUI.outputText("<br><br>")
            ...
            if (liveData.player.biggestTitSize() > 1) {
                ...
                //All other nipples:
                else GUI.outputText("  His fingers grope and grab at your nipples, stretching them uncomfortably. Before you can complain he seems to realize his mistake and releases them.")
                //[Is lactating normally:
                if (liveData.player.biggestLactation() >= 1 && liveData.player.lactationQ() < 50) GUI.outputText("  Milk dribbles and squirts from you as his desperate squishing continues, forming small puddles on the ground.")
                else if (liveData.player.biggestLactation() >= 1) GUI.outputText("  Milk sprays from you as his desperate squishing continues, creating massive puddles of milk that you splash through as you continue moving.")
                GUI.outputText("<br><br>")
            }

            GUI.outputText(
                "You stop running, spraying dirt in a massive fan and sending the imp on your back flying into a tree, where he crumples to the ground unceremoniously. The doggy-dicked imp collapses out of you and is sprayed down with your orgasm, coating him in femcum and his own semen.<br><br>"
            )
    ...
    static centaurOnImpResults(iNum: number) {
        var x = liveData.player.cockThatFits(liveData.monster.analCapacity())
        if (x < 0) x = 0
        ...
        if (iNum == 1) {
            ...
            if (liveData.player.cumQ() >= 250) {
                ...
                //<<I1_1>>
                //<<2 cocks>>
                if (liveData.player.cockTotal() == 2) GUI.outputText("Your other cock drenches the imp's back with its own secretions that immediately start dripping down its sides.  ")
                //<<3+ cocks>>
                if (liveData.player.cockTotal() > 2) GUI.outputText("Your other cocks release their cum all over the creature's back and sides, leaving it a glazed mess.  ")
                //<</I1_1>>
        ...
        if (iNum == 2) {
            ...
            if (liveData.player.cumQ() >= 250) {
                ...
                if (liveData.player.cumQ() >= 500) GUI.outputText("No matter how fast it is swallowing it does not seem to be enough, and soon its belly is distended and its skin is covered in a thick coating of cum.  ")
                //<</multiplier>>
            }
        ...
        if (iNum == 3) {
            ...
            if (liveData.player.biggestTitSize() >= 2) {
                ...
                if (liveData.player.hasFuckableNipples()) {
                    ...
                    if (liveData.player.biggestLactation() >= 1) GUI.outputText("Milk streams out from your " + liveData.player.nippleDescript(0) + "s as if they had been recently filled with dripping cum.  ")
                    else GUI.outputText("Your fingers slide faster and faster into your " + liveData.player.nippleDescript(0) + "s even as the imp begins to stroke itself under you.  ")
                }
                //No pussies
                else {
                    ...
                    if (liveData.player.biggestLactation() > 0) {
                        //<<little milk>>
                        if (liveData.player.biggestLactation() <= 1) GUI.outputText("Beads of milk begin to drip down your chest and occasionally spurt outward.  ")
                        //<<else>>
                        else GUI.outputText("Milk pours out of your " + liveData.player.breastDescript(0) + " and streams down your body.  ")
                    } //<</milk>>
                }
            } //<</Breasts>>
            return
    ...
    static impNippleRape() {
        ...
        if (liveData.player.hasVagina()) GUI.outputText(liveData.player.vaginaDescript(0))
        else if (liveData.player.gender == 0) GUI.outputText(liveData.player.assholeDescript())
        GUI.outputText(".<br><br>")

        GUI.outputText("You gently insert a single digit into one of your nipple-cunts, ")
        if (liveData.player.lactationQ() >= 1000) GUI.outputText("unleashing a torrent of thick, creamy milk and ")
        //(if regular milky;
        else if (liveData.player.lactationQ() >= 50 && liveData.player.biggestLactation() >= 1) GUI.outputText("releasing a steady trickle of warm milk and ")
        ...
        if (liveData.player.hasVagina()) GUI.outputText("dripping wet pussy")
        if (liveData.player.gender == 0) GUI.outputText(liveData.player.assholeDescript())
        GUI.outputText(", teasing him with a lewd moan as your head rolls back in sexual ecstasy.")
        if (liveData.silly) GUI.outputText("  The imp is sickened, but curious.")
        GUI.outputText("<br><br>")
        ...
        if (liveData.player.cor >= 66) {
            ...
            //(if normal)
            if (liveData.player.biggestLactation() < 1) GUI.outputText("wet")
            //(if lactating)
            else GUI.outputText("milky")
        ...
        if (liveData.player.biggestTitSize() <= 4) GUI.outputText("bottoming out halfway on his immense dick.")
        else GUI.outputText("only stopping when the flesh of your immense mammary bumps into his quaking ballsack.")
        GUI.outputText("<br><br>")
        ...
        if (liveData.player.biggestTitSize() <= 4) GUI.outputText("you feel his swollen cock bottom out, your petite breast unable to swallow any more of his throbbing maleness")
        //((over D)
        else GUI.outputText("his swollen cock and desperately filled balls are entirely engulfed in tit-flesh")
        GUI.outputText(
        ...
        //[(if male)
        if (liveData.player.hasCock()) GUI.outputText(" pumping your " + liveData.player.cockDescript(0))
        //(if female)
        else if (liveData.player.hasVagina()) GUI.outputText(" fingering your hungry baby tunnel")
        else GUI.outputText(" fingering your tingling anus")
        GUI.outputText(".<br><br>")
        ...
        //(if lactating)
        if (liveData.player.biggestLactation() >= 1 && liveData.player.lactationQ() < 40) GUI.outputText("milk, ")
        ...
        //(if not)
        else GUI.outputText("You quickly get dressed and leave the imp to his slumbering, his hands still tied together by his loincloth.")
    ...
    static areImpsLactoseIntolerant() {
        GUI.clearOutput()
        GUI.outputText(
            "You advance on the masturbating imp, baring your " + liveData.player.allBreastsDescript() + " and swinging them from side to side. The little creature watches them, mesmerized as he masturbates his foot-long erection.<br><br>"
        )
    ...
    static putBeeEggsInAnImp() {
        ...
        GUI.outputText(
            "<br><br>He's too busy humping the air and stroking himself to notice you hooking the tip of one of your [feet] under him.  You kick up one of your [legs], flipping the fapping imp over.  He gasps as he lands face-down on the ground, startled enough to stop jerking his tool."
        )
        ...
        GUI.outputText(
            "<br><br>You deign not to answer him, lost in the unique sensation of your abdomen curling behind you.  You toss your head back, luxuriating in the pleasure of your black ovipositor emerging against smooth, glossy skin of the imp's ass."
        )
        ...
        GUI.outputText(
            "<br><br>You and the imp shudder in tandem as your sweet honey smears between his cheeks, oozing down his crack as you squeeze your throbbing ovipositor further and further into him.  Buried deep in his bowels, you feel the first of your eggs push through your rubbery organ, stretching out your tube along with his asshole."
        )

        GUI.outputText(
            "<br><br>As you lay your first egg inside the imp, he gurgles, face-down against the ground, and you feel him tighten around your ovipositor.  The imp wriggles beneath your body and by the slowly-spreading pool of steaming cum; you guess that he just climaxed."
        )

        GUI.outputText(
            "<br><br>The imp pants, trying to catch his breath as you twitch your abdomen, adjusting your ovipositor inside him.  Before he can recover, you push another egg down your tube, implanting it deep in the imp alongside the first egg."
        )
        ...
        if (liveData.player.isGoo()) GUI.outputText("rippled goo edges")
        else if (liveData.player.isNaga()) GUI.outputText("trembling coils")
        else GUI.outputText("straddling knees")
        GUI.outputText(" as you turn the imp into your own, private incubator.")

        GUI.outputText(
            "<br><br>After a handful of eggs, you grunt, realizing that you've run out of room inside the imp.  Tilting your head to one side, you consider that the imp is face-down, and that his stomach might need more room to stretch.  You rise halfway up and flip him over beneath you, careful to leave your ovipositor still buried inside him."
        )

        GUI.outputText(
            "<br><br>The imp's eyes are almost completely rolled back in his head, his flat chest smothered with his own spunk.  His breathing is ragged, and his hard, massive cock is slathered with thick, white cum.  His belly already bulges slightly with your eggs and his small hands move to clutch at his stomach, giving him the look of a debased, pregnant mother."
        )

        GUI.outputText(
            "<br><br>That realization is enough to stimulate your ovipositor again.  With a groan, you plant your hands on the ground to either side of his head, on your knees as your ovipositor pumps another egg into the imp's bowels.  The imp shudders as his belly swells, filling with your brood."
        )

        GUI.outputText('<br><br>"<i>More... more!</i>" moans the imp beneath you.  You oblige, and ')
        if (liveData.player.biggestTitSize() >= 1) {
            GUI.outputText("his tiny claws grab your ")
            if (liveData.player.bRows() > 1) GUI.outputText("first row of ")
            GUI.outputText(liveData.player.breastDescript(0) + ", squeezing your tits as you fuck him full.")
            if (liveData.player.lactationQ() >= 500) GUI.outputText("  Rivulets of your milk run down his forearms as he inexpertly milks you.")
        }
        //[If cock:
        else if (liveData.player.hasCock()) GUI.outputText("the rise of his swollen belly soon presses against [oneCock] and the rhythm of your thrusts strokes his shiny red stomach against your sensitive organ.")
        ...
        else GUI.outputText("you feel a firm pressure at your [asshole] as the tip of the imp's lashing tail prods frantically against you, manically shoving in and out of your [asshole].")

        GUI.outputText(
            "<br><br>You groan, climaxing against the imp, just as he lets out another gout of hot seed from his cum-smeared dick.  He spatters your front, his spunk mingling with your fluids, shuddering as he takes the last of your eggs inside him, his belly swollen to the size of a beach ball."
        )

        GUI.outputText(
            "<br><br>You pant heavily, and with a messy squelching, you pull yourself out of the imp, pushing yourself up from your crouched position.  A gush of honey pours from the imp's ass, cutting off quickly as an egg rolls into place from the inside, stopping up your imp-cubator."
        )

        GUI.outputText(
            "<br><br>You hear a strange noise from the imp, one that sounds strangely like a giggle.  You glance down at him, instinctively evaluating him as a bearer of your eggs.  The imp is still panting, looking up at you from under his messy, black hair.  With a flushed, submissive expression and swollen, pregnant belly, the imp seems almost... cute?  He cradles his massive, egg-filled belly, caressing it, then looks back to you, blushing."
        )
    ...
    static killImp() {
        ...
        if (liveData.player.cor < 25) liveData.player.modStats(["cor", -0.5])
        GUI.menu()
        ...
        if (liveData.player.lust >= liveData.player.maxLust()) {
            ...
            if (liveData.player.gender == 2 || liveData.player.gender == 3) {
                ...
                if (liveData.player.gender == 3)
                    GUI.outputText(
                        "<br><br>The rough fucking becomes more and more pleasurable as time passes, until you cannot help but stroke your " +
                ...
                if (liveData.player.gender == 2)
                    GUI.outputText(
                        "<br><br>The rough fucking becomes more and more pleasurable as time passes.  You moan loudly and lewdly with each thrust, hips squeezing around the demon-cock, relishing the feeling of fullness.  Before long you cannot help but cum all over him, " +
                            liveData.player.vaginaDescript(0) +
                ...
                if (liveData.player.pregnancyIncubation > 0 && liveData.player.pregnancyIncubation <= 216) {
                    ...
                    if (liveData.player.inHeat) GUI.outputText("You find yourself hoping you're pregnant as you swiftly lose consciousness.")
                    else if (liveData.player.pregnancyIncubation <= 0) {
                        if (liveData.player.cor > 75) GUI.outputText("With an appreciative moan, you bury your fingers in its slimy warmth, hoping you are pregnant with some fiendish offspring, and lose consciousness.")
            ...
            if (liveData.player.gender == 0 || liveData.player.gender == 1) {
                ...
                if (liveData.player.gender == 1 && UTIL.rand(2) == 0) {
                    ...
                    else GUI.outputText("On all fours now, your " + liveData.player.cockDescript(0) + " bounces up and down against you to the rhythm of the imp's thrusts, leaving your belly smeared in your own pre-cum.<br><br>")
                    ...
                    if (!liveData.player.isTaur()) GUI.outputText("as you are disrobed")
                    GUI.outputText(
                        ".  As the imp presses a large bulk against your backside, you realize he has a massive penis!<br><br>The imp pushes his " +
                            liveData.monster.cockDescriptShort(0) +
                    ...
                    if (liveData.player.sens > 40) {
                        ...
                        if (liveData.player.gender == 1) GUI.outputText("  You jizz all over the ground in front of you, spraying cum in huge squirts in time with the demon's thrusts.")
    ...
    static sprocketImp() {
        ...
        if (liveData.player.hairLength > 0) GUI.outputText("His claws dig into your hair ")
        else GUI.outputText("His claws dig into your wrists ")
        GUI.outputText(
        ...
        //Variable cocktext
        if (liveData.player.cocks[0].cockType == ENUM.CockType.HUMAN || liveData.player.cocks[0].cockType == ENUM.CockType.DEMON) GUI.outputText("your " + liveData.player.cockDescript(0) + ", rubbing the tip of his prick against your own, ")
        else if (liveData.player.hasKnot(0)) GUI.outputText("your " + liveData.player.cockDescript(0) + ", rubbing the tip of his prick against your point, ")
        else if (liveData.player.cocks[0].cockType == ENUM.CockType.HORSE) GUI.outputText("your " + liveData.player.cockDescript(0) + ", rubbing the tip of his prick against your flared head, ")
        else if (liveData.player.cocks[0].cockType == ENUM.CockType.TENTACLE) GUI.outputText("your huge green dick, rubbing the tip of his prick against your purplish cock-head, ")
        GUI.outputText(
        ...
        if (liveData.player.cocks[0].cockType == ENUM.CockType.HUMAN || liveData.player.cocks[0].cockType == ENUM.CockType.DEMON) GUI.outputText("shoves his shaft deeply into the slit in the head of your dick.  ")
        else if (liveData.player.hasKnot(0)) GUI.outputText("finds the hole in the pointed head of your cock and plunges his shaft deeply into it, literally fucking your urethra.  ")
        ...
        else if (liveData.player.cocks[0].cockType == ENUM.CockType.TENTACLE) GUI.outputText("shoves his dick deeply into the slit in the head of your vine-like cock.  ")
        //New PG
        GUI.outputText("<br><br>")
        GUI.outputText("He grips your cock tightly as he fucks you, treating you like a ")
        //Differing cocksleeve texts
        if (liveData.player.skinDesc == "fur") GUI.outputText("furry cock-sleeve")
        else {
            if (liveData.player.skinTone == "purple" || liveData.player.skinTone == "blue" || liveData.player.skinTone == "shiny black") GUI.outputText("demonic cock-sleeve")
            else GUI.outputText("human cock-sleeve")
        }
        //Bonus boob shake or period if no boobs.
        if (liveData.player.breastRows.length > 0 && liveData.player.biggestTitSize() > 2) GUI.outputText(", fucking you so hard that your " + liveData.player.allBreastsDescript() + " bounce with each thrust.  ")
        else GUI.outputText(".  ")
        ...
        if (liveData.player.cocks.length == 2) GUI.outputText("your other cock finding pleasure in rubbing against his body ")
        if (liveData.player.cocks.length > 2) GUI.outputText("your other cocks finding pleasure in rubbing against his body ")
        //Cum
        GUI.outputText(
        ...
        //Extra cum-texts
        if (liveData.player.cocks.length == 2) GUI.outputText("Your other cock cums at the same time, liberally splattering your spunk up his back.  ")
        if (liveData.player.cocks.length > 2)
        ...
        //Vagoooz
        if (liveData.player.vaginas.length > 0) GUI.outputText("Your pussy quivers, contracting furiously as your orgasm hits you - like it's trying to milk a phantom dick dry.  ")
        //new PG
        ...
        if (liveData.player.HP < 1) liveData.player.HP = 1
        COMBAT.cleanupAfterCombat()
    }
}
