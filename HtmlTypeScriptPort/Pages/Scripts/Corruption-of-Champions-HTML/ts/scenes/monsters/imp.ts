...
Imp.lustMagicAttack = function () {
    ...
    if (player.cocks.length > 0) {
        if (player.lust >= 60) outputText("You feel your " + player.multiCockDescriptLight() + " dribble pre-cum.")
        else if (player.lust >= 30 && player.cocks.length == 1) outputText("Your " + player.cockDescript(0) + " hardens, distracting you further.")
        else if (player.lust >= 30 && player.cocks.length > 1) outputText("Your " + player.multiCockDescriptLight() + " harden uncomfortably.")
...
ImpScene.victoryAgainstImp = function () {
    ...
    menu()
    if (canFeed) {
        if (player.lust >= 33) outputText("  Sadly you realize your own needs have not been met.  Of course you could always rape the poor thing, but it might be more fun to force it to guzzle your breast-milk.<br><br>What do you do?")
    ...
    if (player.lust >= 33) {
        if (player.hasCock()) {
            if (player.cockThatFits(monster.analCapacity()) == -1) outputText("<br><br><b>You're too big to rape an imp with " + player.oMultiCockDesc() + ".</b>")
            else {
        ...
        if (player.hasVagina()) {
            if (player.isTaur()) addButton(1, "Group Vaginal", ImpScene.centaurGirlOnImps)
            else addButton(1, "Female Rape", ImpScene.rapeImpWithPussy)
        }
...
ImpScene.rapeImpWithDick = function (condomed) {
    ...
    if (player.cocks.length == 1) {
        ...
        if (player.cocks[0].cockType == ENUM.CockType.CAT) outputText(" The tiny creature's claws dig into your sides at the feeling of soft, hooked barbs stroking his sensitive insides.")
        ...
        if (player.balls == 0 && player.vaginas.length > 0)
            outputText(
                " The cum seems to boil out from inside you as your " + player.vaginaDescript(0) + " soaks itself. With delicious slowness you fire rope after rope of cum " + (condomed ? "inside your condom." : "deep into the imp's rectum.") + ""
            )
        if (player.balls == 0 && player.vaginas.length == 0)
...
ImpScene.centaurGirlOnImps = function () {
    ...
    if (player.hasVagina()) {
        ...
        if (player.biggestTitSize() > 1) {
            outputText(
                "He slides up to your torso and grasps your wildly flailing " + player.allBreastsDescript() + ", massaging them harshly. His ministrations are surprisingly crude, and you wonder how many times he's attempted to pleasure a woman."
            )
    ...
    if (iNum == 1) {
        ...
        if (player.cumQ() >= 250) {
            ...
            //<<knot>>
                ...
                outputText(
                    "Soon the abused " + monster.assholeDescript() + " is full to the brim, though your knot keeps any from escaping while more and more pumps in.  Soon the creature's belly is distending and the imp is gasping wordlessly. ",
                    false
                )
...
ImpScene.sprocketImp = function () {
    ...
    //Variable cocktext
    if (player.cocks[0].cockType == ENUM.CockType.HUMAN || player.cocks[0].cockType == ENUM.CockType.DEMON) outputText("your " + player.cockDescript(0) + ", rubbing the tip of his prick against your own, ", false)
    else if (player.hasKnot(0)) outputText("your " + player.cockDescript(0) + ", rubbing the tip of his prick against your point, ", false)
    else if (player.cocks[0].cockType == ENUM.CockType.HORSE) outputText("your " + player.cockDescript(0) + ", rubbing the tip of his prick against your flared head, ", false)
    else if (player.cocks[0].cockType == ENUM.CockType.TENTACLE) outputText("your huge green dick, rubbing the tip of his prick against your purplish cock-head, ", false)
    outputText(
    ...
    if (player.cocks[0].cockType == ENUM.CockType.HUMAN || player.cocks[0].cockType == ENUM.CockType.DEMON) outputText("shoves his shaft deeply into the slit in the head of your dick.  ", false)
    ...
    else if (player.cocks[0].cockType == ENUM.CockType.HORSE) outputText("seats his dick in the flared head of your prick, and then pushes farther. His shaft plunges into yours, filling your cock more than any cum load ever could.  ", false)
    else if (player.cocks[0].cockType == ENUM.CockType.TENTACLE) outputText("shoves his dick deeply into the slit in the head of your vine-like cock.  ", false)
    //New PG
    outputText("<br><br>")
