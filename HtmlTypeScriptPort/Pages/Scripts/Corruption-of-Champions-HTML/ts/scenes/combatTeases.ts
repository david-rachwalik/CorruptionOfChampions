function teaseMain(justText) {
    ...
    if (monster.refName.indexOf("minotaur") != -1) {
        if (player.hasVagina() && player.lactationQ() >= 500 && player.biggestTitSize() >= 6 && player.cowScore() >= 3 && player.tailType == TAIL_TYPE_COW) select = 43
    }
    switch (select) {
        //0 butt shake
        case 0:
        ...
        case 6:
            ...
            if (monster.plural) outputText(capitalizeFirstLetter(monster.a) + monster.refName + "' gazes are riveted on your groin as you run your fingers up and down your folds seductively.")
            else outputText(capitalizeFirstLetter(monster.a) + monster.refName + "'s gaze is riveted on your groin as you run your fingers up and down your folds seductively.")
            if (player.clitLength > 3) outputText(" You smile as your " + player.clitDescript() + " swells out from the folds and stands proudly, begging to be touched.")
    ...
    if (rand(100) <= chance + rand(bonusChance)) {
        ...
        if (gameFlags[PC_FETISH] >= 1 /* && !urtaQuest.isUrta()*/) {
            if (player.lust < 75) outputText("<br>Flaunting your body in such a way gets you a little hot and bothered. ")
            else outputText("<br>If you keep exposing yourself you're going to get too horny to fight back. This exhibitionism fetish makes it hard to resist just stripping naked and giving up. ")
