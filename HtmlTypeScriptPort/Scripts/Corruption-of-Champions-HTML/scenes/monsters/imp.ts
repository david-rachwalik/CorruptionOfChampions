...
export function victoryAgainstImp() {
  ...
  if (canFeed) {
    if (liveData.player.lust >= 33)
      GUI.outputText(
        '  Sadly you realize your own needs have not been met.  Of course you could always rape the poor thing, but it might be more fun to force it to guzzle your breast-milk.<br><br>What do you do?',
      );
    else
      GUI.outputText(
        "  You're not really turned on enough to rape it, but it might be fun to force it to guzzle your breast-milk.<br><br>Do you breastfeed it?",
      );
    ...
    if (liveData.monster.HP <= 0) {
      GUI.addButton(0, 'Kill Him', killImp);
  ...
  if (liveData.player.lust >= 33) {
    ...
    if (liveData.player.hasVagina()) {
      if (liveData.player.isTaur()) GUI.addButton(1, 'Group Vaginal', centaurGirlOnImps);
      else GUI.addButton(1, 'Female Rape', rapeImpWithPussy);
    }
    if (liveData.player.hasFuckableNipples()) {
      GUI.addButton(2, 'NippleFuck', impNippleRape);
    }
  } else {
    COMBAT.cleanupAfterCombat();
    return;
  }
  if (canFeed) GUI.addButton(3, 'Breastfeed', areImpsLactoseIntolerant);
  ...
  if (liveData.player.canOvipositBee()) GUI.addButton(8, 'Oviposit', putBeeEggsInAnImp);
  GUI.addButton(10, 'Kill Him', killImp);
  GUI.addButton(14, 'Leave', COMBAT.cleanupAfterCombat);
}

// MALE RAPE

export function rapeImpWithDick(condomed = false) {
  GUI.clearOutput();
  ...
  if (liveData.player.cocks.length == 1) {
    ...
    if (liveData.player.cumQ() > 30 && liveData.player.cumQ() <= 100)
      GUI.outputText(
        ' Your orgasm seems to last forever, jizz dripping out of ' +
          (condomed ? 'your condom' : "the imp's asshole") +
          ' around your ' +
          liveData.player.cockDescript(x) +
    ...
    if (liveData.player.cumQ() > 100)
      GUI.outputText(
        ' Your orgasm only seems to grow more and more intense as it goes on, each spurt more powerful and copious than the last. ' +
          (condomed
            ? 'Your condom swells nearly to the point of bursting, and tiny jets of cum squirt out around your ' +
              liveData.player.cockDescript(x) +
              ' with each thrust.'
            : 'The imp begins to look slightly pregnant as you fill him, and tiny jets of cum squirt out around your ' +
              liveData.player.cockDescript(x) +
              ' with each thrust.') +
          '',
      );
...
export function rapeImpWithPussy() {
  GUI.clearOutput();
  ...
  //Big enough!
    ...
    GUI.outputText(
      '<br><br>The two of you lay there a moment while you recover, at last separating as you rise up off his ' +
        liveData.monster.cockDescriptShort(0) +
...
export function centaurOnImpStart() {
  ...
  if (x >= 0 && !liveData.player.hasVagina()) centaurOnImpMale();
  else if (liveData.player.hasVagina() && x < 0) centaurOnImpFemale();
  else {
    GUI.outputText('Do you focus on your maleness or girl-parts?');
    GUI.menu();
    GUI.addButton(0, 'Male', centaurOnImpMale, null, null, null, 'TO BE ADDED.');
    GUI.addButton(1, 'Female', centaurOnImpFemale, null, null, null, 'TO BE ADDED.');
  }
}

// MALE RAPE AS TAUR

export function centaurOnImpMale(vape = false) {
  let x = liveData.player.cockThatFits(liveData.monster.analCapacity());
  if (x < 0) x = 0;
    ...
    if (liveData.player.cockArea(x) <= 15) {
      ...
      GUI.outputText(
        'You cannot take the attention for long before your hooves are scraping at the ground and jets of sperm shoot out of your ' +
          liveData.player.cockDescript(x) +
          ' and down its waiting throat.<br><br>',
      );
      //<<GoTo I2>>
      centaurOnImpResults(2);
      //<<End>>
...
export function centaurOnImpFemale(vape = false) {
  if (vape) GUI.clearOutput();
...
export function centaurGirlOnImps() {
  GUI.clearOutput();
  GUI.outputText(
...
export function centaurOnImpResults(iNum: number) {
  let x = liveData.player.cockThatFits(liveData.monster.analCapacity());
  if (x < 0) x = 0;
    ...
    if (liveData.player.biggestTitSize() >= 2) {
      ...
      if (liveData.player.hasFuckableNipples()) {
        ...
        if (liveData.player.biggestLactation() >= 1)
          ...
          GUI.outputText(
            'Your fingers slide faster and faster into your ' + liveData.player.nippleDescript(0) + 's even as the imp begins to stroke itself under you.  ',
          );
      }
      //No pussies
      else {
...
export function impNippleRape() {
  ...
  GUI.outputText(
    'You slowly walk over to the masturbating imp, your ' +
      ...
      ' swaying suggestively with every step.<br><br>',
  );
...
export function areImpsLactoseIntolerant() {
  GUI.clearOutput();
  GUI.outputText(
    'You advance on the masturbating imp, baring your ' +
      liveData.player.allBreastsDescript() +
      ' and swinging them from side to side. The little creature watches them, mesmerized as he masturbates his foot-long erection.<br><br>',
  );
...
export function putBeeEggsInAnImp() {
  GUI.clearOutput();
  //IMP EGGS
  ...
  GUI.outputText(
    "<br><br>You grin, straddling his surprisingly perky ass, resting your [hips] on his small, round cheeks.  With your arms pinning down his shoulders, he can't stroke himself, and he whimpers at the restraint.",
  );
  ...
  GUI.outputText(
    '<br><br>"<i>Suh-stop...</i>" groans the imp even as you push a third egg into his tiny body.  But you\'re beyond stopping.  Egg after egg, you fill his twitching body.  The pool of cum grows, and it oozes around your ',
  );
  ...
  GUI.outputText(
    '<br><br>You blink then stand up.  You shake your head as you walk away, chalking the odd thoughts up to your egg-laying instincts.  Some of these mutations have some weird effects, after all...',
  );
  liveData.player.orgasm();
...
export function killImp() {
  ...
  GUI.menu();
  GUI.addButton(0, 'Take Skull', takeSkull);
...
export function impRapesYou() {
  ...
  if (liveData.player.lust >= liveData.player.maxLust()) {
    ...
    if (liveData.player.gender == 2 || liveData.player.gender == 3) {
      ...
      GUI.outputText(
        '<br><br>He smiles and presses his cock against your ' +
          liveData.player.vaginaDescript(0) +
      ...
      if (liveData.player.gender == 1 && UTIL.rand(2) == 0) {
        ...
        GUI.outputText(
          '. Your eyes glance down to his waist and see a massive bulge form under his loincloth, the sight of which causes your ' +
            liveData.player.cockDescript(0) +
            ' to twitch and begin leaking pre-cum.<br><br>',
        );
        GUI.outputText(
        ...
        if (liveData.player.isTaur())
          GUI.outputText(
            'Dropping down to the ground, your ' +
              liveData.player.cockDescript(0) +
              " trembles against your body to the rhythm of the imp's thrusts, leaving your underbelly smeared with its own pre-cum.<br><br>",
          );
        else
          GUI.outputText(
            'On all fours now, your ' +
              liveData.player.cockDescript(0) +
              " bounces up and down against you to the rhythm of the imp's thrusts, leaving your belly smeared in your own pre-cum.<br><br>",
          );
        ...
        GUI.outputText(
          "<br><br>You drop to the ground when he's done with you, cum spilling from your abused ass all over the ground, too exhausted to move.  Consciousness fades.  ",
        );
      }
    }
    COMBAT.cleanupAfterCombat();
