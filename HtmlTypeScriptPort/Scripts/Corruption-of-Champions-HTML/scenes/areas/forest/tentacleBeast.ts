...
// Merging TentacleBeast.as and as

// Including note from as
...
//Monster definition
//Name and references
export class TentacleBeast extends Creature {
  constructor() {
    super();
    ...
    //Victory/defeat
    this.victory = tentacleBeastWin;
    this.defeat = tentacleBeastLoss;
  }
        ...
        //Genderless
          ...
          GUI.outputText(
            'The creature quickly positions a long tentacle against your ' +
              ...
              '. It circles your pucker with slow, delicate strokes that bring unexpected warmth to your body.<br>',
          );
...
export function tentacleBeastWin() {
  ...
  if (liveData.monster.lust >= 100 && liveData.player.gender > 0) {
    ...
    GUI.doYesNo(tentacleVictoryRape, COMBAT.cleanupAfterCombat);
  } else {
...
// Rape the beast
export function tentacleVictoryRape() {
  GUI.clearOutput();
...
export function tentacleBeastLoss() {
...
export function tentacleLossRape() {
  ...
  if (liveData.player.gender == 1) {
    ...
    GUI.outputText(
      '<br><br>Realizing what is about to happen, you try to struggle. The beast responds by slamming you to the ground a few times, stunning you.  ',
    );
  ...
  if (liveData.player.gender == 3) {
    ...
    GUI.outputText(
      '<br><br>Realizing what is about to happen, you try to struggle. The beast responds by slamming you to the ground a few times, stunning you. In your daze you see a monstrous, hollow tentacle poised over your ',
    );
    ...
    if (liveData.player.cockTotal() > 1)
      GUI.outputText(
        liveData.player.multiCockDescriptLight() + ', now begging for release, and clamps down around your pubic mound, fully encapsulating your members.  ',
      );
  ...
  //Call page 2!
  GUI.doNext(tentacleRapeContinuation);
}

// Continue scene for losing to beast
export function tentacleRapeContinuation() {
  liveData.player.dynStats(['tou', 1], ['int', -0.5], ['lib', 2], ['sen', 1], ['cor', 0.5]);
  GUI.clearOutput();
  // TODO spriteSelect(100);
  // Male
  if (liveData.player.gender == 1) {
    ...
    GUI.outputText(
      '<br><br>Helpless and overwhelmed by the pleasure of such rough and primal stimulation, all you can do is give the creature what it wants; your hot cum. Your body only responds to the sensations from your ',
    );
    if (liveData.player.cockTotal() == 1)
      GUI.outputText(
        liveData.player.multiCockDescriptLight() +
  ...
  // Female (Kicks to special scene)
    ...
    GUI.doNext(tentacleRapeContinuationForFemales);
    return;
  }
  // Herm
...
export function tentacleRapeContinuationForFemales() {
  GUI.clearOutput();
  // TODO spriteSelect(100);
  if (liveData.player.vaginas.length == 1) {
    //single coochie
  ...
  if (liveData.player.clitLength < 7 && liveData.player.clitLength >= 3.5 && liveData.player.findPerk(PerkLib.BigClit) >= 0) {
    ...
    GUI.outputText(
      'Your massive clitty eventually diminishes, retaining a fair portion of its former glory.  It is now ' +
        (liveData.player.clitLength * 10) / 10 +
...
export function centaurLossGendered() {
  GUI.outputText(
...
export function encounter() {
  ...
  if (liveData.player.gender == 0 && UTIL.rand(3) == 0 && !liveData.player.isNaga() && !liveData.player.isTaur() && !liveData.player.isGoo()) {
    ...
    GUI.outputText(
      'Completely confused, all you can do is sit there in shock.<br><br>"<i>Where are your naughty bits, goddammit!</i>" the creature bellows. "<i>Us tentacle creatures need to FEED!</i>"<br><br>',
    );
    GUI.outputText(
  ...
  if (liveData.player.cor > 50) {
    ...
    GUI.menu();
    GUI.addButton(0, 'Fight', startTentacleBeastCombat);
    GUI.addButton(1, 'Submit', tentacleLossRape);
    return;
  }
...
export function tentacleLossGenderless() {
  //Taur madness
  if (liveData.player.isTaur()) {
    ...
    GUI.outputText(
      '<br><br>The beast slaps you squarely on the ass as if to push you along. "<i>Get the fuck out of here!</i>" it screams.  "<i>Get lost so I can hunt me a REAL meal!!!</i>"  ',
    );
    GUI.outputText('You walk away from the creature, which hides back in the brush. After you trek a bit, you wonder if what happened really DID happen...');
...
export function centaurLossGenderless() {
  GUI.clearOutput();
  //TODO spriteSelect(100);
  ...
  if (liveData.gameFlags[FLAG.TENTACLE_GENDERLESS_CENTAUR] == 0 || liveData.player.balls == 0) {
    ...
    GUI.outputText(
      ', but stops when it reaches your haunches.  Another starts testing the same area, briefly touching your ' +
        liveData.player.assholeDescript() +
        " but clearly not finding what it's looking for.<br><br>",
    );
    ...
    GUI.outputText(
      '"<i>Why the FUCK can\'t I find your juicy bits?</i>" the creature shrills.  "<i>I\'m so hungry I could risk stealing spoo from an army of goblins in heat!</i>"<br><br>',
    );
    ...
    GUI.outputText(
      '"<i>I just caught a motherfucking HORSE, just to find out you haven\'t got anything for me to eat!  Do you have any idea how fucking hard it is to catch a horse!?</i>"<br><br>',
    );
  ...
  //(Followup scene, if pc has seen above at least once, is unsexed centaur and has balls: -Z)
    ...
    GUI.outputText(
      ', slipping forward to probe between your haunches.  It arrives at and discovers your ' +
        ...
        ' with some little ceremony, stroking and fondling it.<br><br>',
    );
    ...
    GUI.outputText(
      "You glibly explain that though you do in fact have 'a pair', as requested, you're still very much genderless, without any sexual organs.<br><br>",
    );
...
export function nagaLossGenderless() {
  GUI.clearOutput();
  // TODO spriteSelect(100);
  GUI.outputText(
...
export function gooLossGenderless() {
  GUI.clearOutput();
  // TODO spriteSelect(100);
  GUI.outputText(
    "All of a sudden, tentacles come whipping out of the undergrowth to grab you.  Though, they're moving a little too fast, and manage to compress your body walls so far together that you're almost squeezed in half.<br><br>",
  );
  ...
  GUI.outputText(
    '"<i>Well, fuck.</i>"  A tentacle pokes you and you\'d guess the beast is watching you jiggle as it chuckles.  "<i>Maybe this isn\'t a total waste. I wonder, what do you taste like?</i>"<br><br>',
  );
...
//Start of Bad End
export function futaTentacleBadEnd() {
  GUI.clearOutput();
  // TODO spriteSelect(100);
  GUI.outputText(
  ...
  //Goto rape #2
  GUI.doNext(futaTentacleEpilogue);
}

// End of bad end
export function futaTentacleEpilogue() {
  GUI.clearOutput();
  //spriteSelect(100);
  ...
  if (liveData.gameFlags[FLAG.GIACOMO_MET] > 0) {
    ...
    GUI.outputText(
      "You attempt to say something, only to find yourself incapable of speaking. You feel the man's bony hands pick you up and set you down in what feels like his cart.<br><br>",
    );
    GUI.outputText(
...
export function startTentacleBeastCombat() {
  COMBAT.startCombat(new TentacleBeast());
  liveData.playerMenu();
}
