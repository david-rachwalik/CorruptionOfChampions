...
export function explore() {
  ...
  if (liveData.gameFlags[FLAG.RATHAZUL_CAMP] <= 0) choice[choice.length] = 1; //Rathazul
  ...
  if (liveData.gameFlags[FLAG.AMILY_VILLAGE_ACCESSIBLE] == 0) choice[choice.length] = 9; //Discovering the Town Ruins/Amily Encounter
  ...
  switch (select) {
    ...
    case 3: //Fetish Cultists and Zealots, encounterable if factory is shut down (Not yet implemented)
      break;
    case 4: {
      //Green Slime or Goo Girl, encounterable if you're at least level 2.
      ...
      //OOZE!
      else GreenSlimeScene.encounterSlime();
      break;
    }
    ...
    default:
      if (UTIL.rand(2) == 0) {
        ...
        if (liveData.player.cor > 30 || liveData.player.lust > 50 + UTIL.rand(50) || liveData.player.lib > 40) {
          ...
          if (
            (liveData.player.cor > 30 && liveData.player.cor < 60) ||
            (liveData.player.lust > 50 && liveData.player.lust < 90) ||
            (liveData.player.lib > 40 && liveData.player.lib < 75)
          ) {
