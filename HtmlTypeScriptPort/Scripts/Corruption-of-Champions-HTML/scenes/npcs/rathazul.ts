...
export function encounterRathazul() {
  ...
  if (liveData.gameFlags[FLAG.RATHAZUL_PURCHASE_COUNTER] >= 3 && liveData.gameFlags[FLAG.RATHAZUL_CAMP] == 0 && liveData.player.cor < 75) {
    ...
    GUI.doYesNo(rathazulMoveToCamp, rathazulMoveDecline);
  ...
  const offered = rathazulWorkOffer();
  if (!offered) {
    GUI.outputText(
...
export function rathazulMoveToCamp() {
  GUI.clearOutput();
  GUI.outputText(
...
export function rathazulMoveDecline() {
  GUI.clearOutput();
  GUI.outputText(
...
export function campRathazul() {
  ...
  GUI.outputText(
    'Rathazul looks up from his equipment and gives you an uncertain smile.<br><br>"<i>Oh, don\'t mind me,</i>" he says, "<i>I\'m just running some tests here. Was there something you needed, ' +
...
export function returnToRathazulMenu() {
  if (liveData.gameFlags[FLAG.RATHAZUL_CAMP] > 0) campRathazul();
  else encounterRathazul();
}

export function rathazulWorkOffer() {
  let totalOffers = 0;
  let showArmorMenu = false;
  let purify = false;
  const debimbo = false;
  let reductos = false;
  const lethiciteDefense = false;
  let dyes = false;
  /*if (flags[RATHAZUL_SILK_ARMOR_COUNTDOWN] == 1 && flags[UNKNOWN_FLAG_NUMBER_00275] > 0) {
            collectRathazulArmor();
            return true;
  ...
  if (liveData.player.gems >= 50) {
    GUI.outputText(
      'Rathazul offers, "<i>Since you have enough gems to cover the cost of materials for my dyes as well, you could buy one of my dyes for your hair. ',
    );
  ...
  if (totalOffers > 0) {
    ...
    GUI.menu();
    if (showArmorMenu) GUI.addButton(0, 'Armor', rathazulArmorMenu, null, null, null, 'Ask Rathazul to make an armour for you.');
    if (dyes) {
      GUI.addButton(1, 'Buy Dye', buyDyes, null, null, null, 'Ask him to make a dye for you. <br><br>Cost: 50 Gems.');
      GUI.addButton(2, 'Buy Oil', buyOils, null, null, null, 'Ask him to make a skin oil for you. <br><br>Cost: 50 Gems.');
      GUI.addButton(3, 'Buy Lotion', buyLotions, null, null, null, 'Ask him to make a body lotion for you. <br><br>Cost: 50 Gems.');
    }
...
// ARMOUR
//------------
export function rathazulArmorMenu() {
  GUI.clearOutput();
...
//Gel Armour
export function craftOozeArmor() {
  GUI.clearOutput();
...
export function craftCarapace() {
  ...
  GUI.outputText(
    'He still manages to look somewhat pleased with himself in spite of the blunder, even bragging a little bit, "<i>Let me show you the different lengths of string I used.</i>"<br><br>',
  );
  if (liveData.player.cockTotal() > 0 && liveData.player.biggestCockArea() >= 40)
    GUI.outputText(
      'The silken material does little to hide the bulge of your groin, if anything it looks a little lewd. Rathazul mumbles and looks away, shaking his head.<br><br>',
    );
...
export function craftSilkArmor() {
  ...
  if (!liveData.player.hasItem(Items.Materials.SpiderSilk, 5)) {
    ...
    GUI.doNext(returnToRathazulMenu);
    return;
  }
  GUI.outputText(
...
export function commissionSilkArmorForReal() {
  GUI.clearOutput();
  GUI.outputText(
...
export function collectRathazulArmor() {
  GUI.clearOutput();
...
//Dragonscale Armour
export function craftDragonscaleArmor() {
  GUI.clearOutput();
  GUI.outputText(
...
//Marae's Bark Armour
export function craftMaraeArmor(divine = false) {
  GUI.clearOutput();
  if (!divine) {
    ...
    GUI.outputText(
      'The plates are white like snow. Green tentacles grow from the shoulderpads. The armor includes a breastplate, pauldrons, full arm guards, and knee-high boots. You realize the armor is missing pants. <br><br>',
    );
...
export function buyDyeNevermind() {
  GUI.clearOutput();
...
export function buyOilNevermind() {
  GUI.clearOutput();
...
export function buyLotionNevermind() {
  GUI.clearOutput();
...
// PURIFICATION
//------------
export function purifySomething() {
  GUI.clearOutput();
...
export function purificationByRathazulBegin() {
  GUI.outputText(
    'Hoping the rodent-morph alchemist can assist you, you waste no time in approaching him. Rathazul looks up when he sees you, raising an eye curiously. "<i>Is something the matter, ' +
      liveData.player.name +
      '?</i>"',
  );
  GUI.outputText(
...
//Debimbo Draft
export function rathazulDebimboOffer() {
  GUI.clearOutput();
...
export function makeADeBimboDraft() {
  GUI.clearOutput();
  GUI.outputText(
