...
abstract class PlayerInfo {
    ...
    static levelScreen() {
        ...
        GUI.doNext(this.attributeMenu)
    }
    static attributeMenu() {
        GUI.clearOutput()
        GUI.outputText("You have " + liveData.player.statPoints + " points to spend.<br><br>")
        GUI.outputText("<b></b>Strength:</b> " + liveData.player.str + " + <b>" + liveData.player.tempStr + "</b> → " + (liveData.player.str + liveData.player.tempStr) + "<br>")
        GUI.outputText("<b>Toughness:</b> " + liveData.player.tou + " + <b>" + liveData.player.tempTou + "</b> → " + (liveData.player.tou + liveData.player.tempTou) + "<br>")
        GUI.outputText("<b>Speed:</b> " + liveData.player.spe + " + <b>" + liveData.player.tempSpe + "</b> → " + (liveData.player.spe + liveData.player.tempSpe) + "<br>")
        GUI.outputText("<b>Intelligence:</b> " + liveData.player.inte + " + <b>" + liveData.player.tempInt + "</b> → " + (liveData.player.inte + liveData.player.tempInt) + "<br>")
        GUI.menu()
        //Add attributes
        if (liveData.player.statPoints > 0) {
            if (liveData.player.str < 100) GUI.addButton(0, "STR +", this.addAttribute, "str")
            if (liveData.player.tou < 100) GUI.addButton(1, "TOU +", this.addAttribute, "tou")
            if (liveData.player.spe < 100) GUI.addButton(2, "SPE +", this.addAttribute, "spe")
            if (liveData.player.inte < 100) GUI.addButton(3, "INTE +", this.addAttribute, "int")
        }
        //Subtract attributes
        if (liveData.player.tempStr > 0) GUI.addButton(5, "STR -", this.subtractAttribute, "str")
        if (liveData.player.tempTou > 0) GUI.addButton(6, "TOU +", this.subtractAttribute, "tou")
        if (liveData.player.tempSpe > 0) GUI.addButton(7, "SPE +", this.subtractAttribute, "spe")
        if (liveData.player.tempInt > 0) GUI.addButton(8, "INTE +", this.subtractAttribute, "int")
        //Reset & Done
        GUI.addButton(4, "Reset", this.resetAttributes)
        GUI.addButton(9, "Done", this.finishAttributes)
    }
    static addAttribute(attribute: string) {
        switch (attribute) {
            case "str":
    ...
    static subtractAttribute(attribute: string) {
        switch (attribute) {
            case "str":
                liveData.player.tempStr--
                break
    ...
    static finishAttributes() {
        ...
        liveData.player.tempSpe = 0
        liveData.player.tempInt = 0
        if (liveData.player.perkPoints > 0) GUI.doNext(this.perkBuyMenu)
    ...
    static perkBuyMenu() {
        ...
        if (perksAvailable.length > 0) {
            ...
            GUI.addButton(0, "Confirm", this.perkConfirmation)
    ...
    static statsScreen() {
        ...
        if (liveData.player.teaseLevel < 5)
            combatStats += "<b>Tease Skill Level:</b> " + liveData.player.teaseLevel + " (Exp: " + liveData.player.teaseXP + " / " + (10 + (liveData.player.teaseLevel + 1) * 5 * (liveData.player.teaseLevel + 1)) + ")<br>"
        ...
        // Mino Cum Addiction
        if (liveData.gameFlags[FLAG.EVER_DRANK_MINOCUM] > 0 || liveData.gameFlags[FLAG.MINOTAUR_CUM_ADDICTION_TRACKER] > 0 || liveData.player.findPerk(PerkLib.MinotaurCumAddict) >= 0 || liveData.player.findPerk(PerkLib.MinotaurCumResistance) >= 0) {
            if (liveData.player.findPerk(PerkLib.MinotaurCumAddict) < 0) addictStats += "<b>Minotaur Cum:</b> " + Math.round(liveData.gameFlags[FLAG.MINOTAUR_CUM_ADDICTION_TRACKER] * 10) / 10 + "%<br>"
    ...
    static perksScreen() {
        ...
        if (liveData.player.perkPoints > 0) {
            ...
            GUI.addButton(button++, "Perk Up", this.perkBuyMenu)
        }
    }
    //------------
    // APPEARANCE
    //------------
    static appearanceScreen() {
        GUI.clearOutput()
        let temp = 0
        ...
        GUI.outputText(
            "You are a " + Math.floor(liveData.player.tallness / 12) + " foot " + (liveData.player.tallness % 12) + " inch tall " + liveData.player.maleFemaleHerm() + " " + liveData.player.race() + ", with " + liveData.player.bodyType() + "."
        )
        ...
        GUI.outputText(" <b>You are currently " + (liveData.player.armor.id == "Naked" ? "naked" : "wearing your " + liveData.player.armor.equipmentName) + " and using your " + liveData.player.weapon.equipmentName + " as a weapon.</b>")
        //HEAD DESCRIPTION
        //Face
        switch (liveData.player.faceType) {
            ...
            case ENUM.FaceType.FACE_FERRET_MASK:
                if (liveData.player.skinType == ENUM.SkinType.SKIN_TYPE_PLAIN || liveData.player.skinType == ENUM.SkinType.SKIN_TYPE_GOO) GUI.outputText(" Your face is human in shape and structure, with " + liveData.player.skin() + ".")
                if (liveData.player.skinType == ENUM.SkinType.SKIN_TYPE_FUR) GUI.outputText(" Under your " + liveData.player.skinFurScales() + " you have a human-shaped head with " + liveData.player.skin(true, false) + ".")
                if (liveData.player.skinType == ENUM.SkinType.SKIN_TYPE_SCALES) GUI.outputText(" Your face is fairly human in shape, but is covered in " + liveData.player.skin() + ".")
                if (liveData.player.faceType == ENUM.FaceType.FACE_SHARK_TEETH) GUI.outputText(" A set of razor-sharp, retractable shark-teeth fill your mouth and gives your visage a slightly angular appearance.")
                else if (liveData.player.faceType == ENUM.FaceType.FACE_BUNNY) GUI.outputText(" The constant twitches of your nose and the length of your incisors gives your visage a hint of bunny-like cuteness.")
                else if (liveData.player.faceType == ENUM.FaceType.FACE_SPIDER_FANGS) GUI.outputText(" A set of retractable, needle-like fangs sit in place of your canines and are ready to dispense their venom.")
                else if (liveData.player.faceType == ENUM.FaceType.FACE_FERRET_MASK) GUI.outputText(" The [skinFurScales] around your eyes is significantly darker than the rest of your face, giving you a cute little ferret mask.")
                break
            case ENUM.FaceType.FACE_FERRET:
                if (liveData.player.skinType == ENUM.SkinType.SKIN_TYPE_PLAIN)
                    GUI.outputText(" Your face is an adorable cross between human and ferret features, complete with a wet nose and whiskers. The only oddity is your lack of fur, leaving only [skin] visible on your ferret-like face.")
                else GUI.outputText(" Your face is coated in " + liveData.player.furColor + " fur with [skin] underneath, an adorable cross between human and ferret features. It is complete with a wet nose and whiskers.")
                break
            case ENUM.FaceType.FACE_RACCOON_MASK:
                if (liveData.player.skinType != ENUM.SkinType.SKIN_TYPE_FUR && liveData.player.skinType != ENUM.SkinType.SKIN_TYPE_SCALES) {
                    ...
                    if ((liveData.player.skinTone == "ebony" || liveData.player.skinTone == "black") && (liveData.player.skinType == ENUM.SkinType.SKIN_TYPE_PLAIN || liveData.player.skinType == ENUM.SkinType.SKIN_TYPE_GOO))
                    ...
                    if ((liveData.player.furColor == "black" || liveData.player.furColor == "midnight") && (liveData.player.skinType == ENUM.SkinType.SKIN_TYPE_FUR || liveData.player.skinType == ENUM.SkinType.SKIN_TYPE_SCALES))
            ...
            case ENUM.FaceType.FACE_RACCOON:
                GUI.outputText(" You have a triangular raccoon face, replete with sensitive whiskers and a little black nose; a mask shades the space around your eyes, set apart from your " + liveData.player.skinFurScales() + " by a band of white.")
                ...
                else if (liveData.player.skinType == ENUM.SkinType.SKIN_TYPE_SCALES) GUI.outputText(" The presence of said scales gives your visage an eerie look, more reptile than mammal.")
                break
            case ENUM.FaceType.FACE_FOX:
                ...
                if (liveData.player.skinType == ENUM.SkinType.SKIN_TYPE_PLAIN) GUI.outputText(" Oddly enough, there's no fur on your animalistic muzzle, just " + liveData.player.skinFurScales() + ".")
                else if (liveData.player.skinType == ENUM.SkinType.SKIN_TYPE_FUR) GUI.outputText(" A coat of " + liveData.player.skinFurScales() + " decorates your muzzle.")
                else if (liveData.player.skinType == ENUM.SkinType.SKIN_TYPE_SCALES) GUI.outputText(" Strangely, " + liveData.player.skinFurScales() + " adorn every inch of your animalistic visage.")
                break
            case ENUM.FaceType.FACE_BUCKTEETH:
                ...
                if (liveData.player.skinType == ENUM.SkinType.SKIN_TYPE_FUR || liveData.player.skinType == ENUM.SkinType.SKIN_TYPE_SCALES) GUI.outputText(" under your " + liveData.player.skinFurScales())
            ...
            case ENUM.FaceType.FACE_MOUSE:
                ...
                if (liveData.player.skinType != ENUM.SkinType.SKIN_TYPE_FUR && liveData.player.skinType != ENUM.SkinType.SKIN_TYPE_SCALES) GUI.outputText(liveData.player.skin())
            ...
            case ENUM.FaceType.FACE_SNAKE_FANGS:
                if (liveData.player.skinType == ENUM.SkinType.SKIN_TYPE_PLAIN || liveData.player.skinType == ENUM.SkinType.SKIN_TYPE_GOO)
                    GUI.outputText(" You have a fairly normal face, with " + liveData.player.skin() + ". The only oddity is your pair of dripping fangs which often hang over your lower lip.")
                if (liveData.player.skinType == ENUM.SkinType.SKIN_TYPE_FUR)
                    GUI.outputText(" Under your " + liveData.player.skinFurScales() + " you have a human-shaped head with " + liveData.player.skin(true, false) + ". In addition, a pair of fangs hang over your lower lip, dripping with venom.")
                if (liveData.player.skinType == ENUM.SkinType.SKIN_TYPE_SCALES)
                    GUI.outputText(" Your face is fairly human in shape, but is covered in " + liveData.player.skinFurScales() + ". In addition, a pair of fangs hang over your lower lip, dripping with venom.")
                break
            case ENUM.FaceType.FACE_HORSE:
                ...
                if (liveData.player.skinType == ENUM.SkinType.SKIN_TYPE_FUR)
                    GUI.outputText(" Your face is almost entirely equine in appearance, even having " + liveData.player.skinFurScales() + ". Underneath the fur, you believe you have " + liveData.player.skin(true, false) + ".")
                if (liveData.player.skinType == ENUM.SkinType.SKIN_TYPE_SCALES) GUI.outputText(" You have the face and head structure of a horse, overlaid with glittering " + liveData.player.skinFurScales() + ".")
                break
            case ENUM.FaceType.FACE_DOG:
                if (liveData.player.skinType == ENUM.SkinType.SKIN_TYPE_PLAIN || liveData.player.skinType == ENUM.SkinType.SKIN_TYPE_GOO)
                    GUI.outputText(" You have a dog-like face, complete with a wet nose. The odd visage is hairless and covered with " + liveData.player.skinFurScales() + ".")
                if (liveData.player.skinType == ENUM.SkinType.SKIN_TYPE_FUR)
                    GUI.outputText(" You have a dog's face, complete with wet nose and panting tongue. You've got " + liveData.player.skinFurScales() + ", hiding your " + liveData.player.skin(true, false) + " underneath your furry visage.")
                if (liveData.player.skinType == ENUM.SkinType.SKIN_TYPE_SCALES) GUI.outputText(" You have the facial structure of a dog, wet nose and all, but overlaid with glittering " + liveData.player.skinFurScales() + ".")
                break
            case ENUM.FaceType.FACE_CAT:
                if (liveData.player.skinType == ENUM.SkinType.SKIN_TYPE_PLAIN || liveData.player.skinType == ENUM.SkinType.SKIN_TYPE_GOO)
                    GUI.outputText(" You have a cat-like face, complete with a cute, moist nose and whiskers. The " + liveData.player.skin() + " that is revealed by your lack of fur looks quite unusual on so feline a face.")
                if (liveData.player.skinType == ENUM.SkinType.SKIN_TYPE_FUR)
                    GUI.outputText(" You have a cat-like face, complete with moist nose and whiskers. Your " + liveData.player.skinDesc + " is " + liveData.player.furColor + ", hiding your " + liveData.player.skin(true, false) + " underneath.")
                if (liveData.player.skinType == ENUM.SkinType.SKIN_TYPE_SCALES)
                    GUI.outputText(" Your facial structure blends humanoid features with those of a cat. A moist nose and whiskers are included, but overlaid with glittering " + liveData.player.skinFurScales() + ".")
                if (liveData.player.eyeType != ENUM.EyeType.EYES_BLACK_EYES_SAND_TRAP) GUI.outputText(" Of course, no feline face would be complete without vertically slit eyes.")
                break
            case ENUM.FaceType.FACE_COW_MINOTAUR:
                if (liveData.player.skinType == ENUM.SkinType.SKIN_TYPE_PLAIN || liveData.player.skinType == ENUM.SkinType.SKIN_TYPE_GOO)
                    GUI.outputText(
                        " You have a face resembling that of a minotaur, with cow-like features, particularly a squared off wet nose. Despite your lack of fur elsewhere, your visage does have a short layer of " + liveData.player.furColor + " fuzz."
                    )
                if (liveData.player.skinType == ENUM.SkinType.SKIN_TYPE_FUR)
                    GUI.outputText(
                ...
                if (liveData.player.skinType == ENUM.SkinType.SKIN_TYPE_SCALES)
                    GUI.outputText(" Your face resembles a minotaur's, though strangely it is covered in shimmering scales, right up to the flat cow-like nose that protrudes from your face.")
                break
            case ENUM.FaceType.FACE_LIZARD:
                if (liveData.player.skinType == ENUM.SkinType.SKIN_TYPE_PLAIN || liveData.player.skinType == ENUM.SkinType.SKIN_TYPE_GOO)
                    GUI.outputText(" You have a face resembling that of a lizard, and with your toothy maw, you have quite a fearsome visage. The reptilian visage does look a little odd with just " + liveData.player.skin() + ".")
                if (liveData.player.skinType == ENUM.SkinType.SKIN_TYPE_FUR)
                    GUI.outputText(" You have a face resembling that of a lizard. Between the toothy maw, pointed snout, and the layer of " + liveData.player.skinFurScales() + " covering your face, you have quite the fearsome visage.")
                if (liveData.player.skinType == ENUM.SkinType.SKIN_TYPE_SCALES)
                    GUI.outputText(" Your face is that of a lizard, complete with a toothy maw and pointed snout. Reflective " + liveData.player.skinFurScales() + " complete the look, making you look quite fearsome.")
                break
            ...
            case ENUM.FaceType.FACE_BOAR:
                ...
                if (liveData.player.skinType == ENUM.SkinType.SKIN_TYPE_FUR) GUI.outputText("with " + liveData.player.skinTone + " skin underneath your " + liveData.player.furColor + " fur")
            ...
            case ENUM.FaceType.FACE_RHINO:
                ...
                if (liveData.player.skinType == ENUM.SkinType.SKIN_TYPE_PLAIN) GUI.outputText(", with " + liveData.player.skin() + ", complete with a long muzzle and a horn on your nose.")
            ...
            case ENUM.FaceType.FACE_ECHIDNA:
                ...
                if (liveData.player.skinType == ENUM.SkinType.SKIN_TYPE_PLAIN) GUI.outputText(" The " + liveData.player.skin() + " that is revealed by your lack of fur looks quite unusual.")
                ...
                else if (liveData.player.skinType == ENUM.SkinType.SKIN_TYPE_SCALES) GUI.outputText(" It's covered in " + liveData.player.skinFurScales() + ", making your face even more unusual.")
                break
            case ENUM.FaceType.FACE_DEER:
                GUI.outputText(" Your face is like that of a deer, with a nose at the end of your muzzle.")
                if (liveData.player.skinType == ENUM.SkinType.SKIN_TYPE_PLAIN) GUI.outputText(" The " + liveData.player.skin() + " that is revealed by your lack of fur looks quite unusual.")
                else if (liveData.player.skinType == ENUM.SkinType.SKIN_TYPE_FUR) GUI.outputText(" It's covered in " + liveData.player.skinFurScales() + " that covers your " + liveData.player.skinTone + " skin underneath.")
                else if (liveData.player.skinType == ENUM.SkinType.SKIN_TYPE_SCALES) GUI.outputText(" It's covered in " + liveData.player.skinFurScales() + ", making your face looks more unusual.")
                break
            default:
        ...
        //Eyes
        if (liveData.player.eyeType == ENUM.EyeType.EYES_FOUR_SPIDER_EYES) GUI.outputText(" In addition to your primary two eyes, you have a second, smaller pair on your forehead.")
        ...
        if (liveData.player.hairLength == 0) {
            ...
            else GUI.outputText(" You are totally bald, showing only shiny " + liveData.player.skinTone + " " + liveData.player.skinDesc + " where your hair should be.")
            ...
            else if (liveData.player.earType == ENUM.EarType.EARS_LIZARD) GUI.outputText(" A pair of rounded protrusions with small holes on the sides of your head serve as your ears.")
            else if (liveData.player.earType == ENUM.EarType.EARS_BUNNY) GUI.outputText(" A pair of floppy rabbit ears stick up from the top of your head, flopping around as you walk.")
            ...
            else if (liveData.player.earType == ENUM.EarType.EARS_DRAGON) GUI.outputText(" A pair of rounded protrusions with small holes on the sides of your head serve as your ears. Bony fins sprout behind them.")
            ...
            //</mod>
            if (liveData.player.antennae == ENUM.AntennaeType.ANTENNAE_BEE) GUI.outputText(" Floppy antennae also appear on your skull, bouncing and swaying in the breeze.")
        }
        //not bald
        else {
            if (liveData.player.earType == ENUM.EarType.EARS_HUMAN) GUI.outputText(" Your " + liveData.player.hairDescript() + " looks good on you, accentuating your features well.")
            else if (liveData.player.earType == ENUM.EarType.EARS_FERRET) GUI.outputText(" A pair of small, rounded ferret ears burst through the top of your " + liveData.player.hairDescript() + ".")
            else if (liveData.player.earType == ENUM.EarType.EARS_HORSE) GUI.outputText(" The " + liveData.player.hairDescript() + " on your head parts around a pair of very horse-like ears that grow up from your head.")
            else if (liveData.player.earType == ENUM.EarType.EARS_DOG) GUI.outputText(" The " + liveData.player.hairDescript() + " on your head is overlapped by a pair of pointed dog ears.")
            else if (liveData.player.earType == ENUM.EarType.EARS_COW) GUI.outputText(" The " + liveData.player.hairDescript() + " on your head is parted by a pair of rounded cow ears that stick out sideways.")
            else if (liveData.player.earType == ENUM.EarType.EARS_ELFIN) GUI.outputText(" The " + liveData.player.hairDescript() + " on your head is parted by a pair of cute pointed ears, bigger than your old human ones.")
            else if (liveData.player.earType == ENUM.EarType.EARS_CAT)
                GUI.outputText(" The " + liveData.player.hairDescript() + " on your head is parted by a pair of cute, fuzzy cat ears, sprouting from atop your head and pivoting towards any sudden noises.")
            else if (liveData.player.earType == ENUM.EarType.EARS_LIZARD) GUI.outputText(" The " + liveData.player.hairDescript() + " atop your head makes it nigh-impossible to notice the two small rounded openings that are your ears.")
            else if (liveData.player.earType == ENUM.EarType.EARS_BUNNY) GUI.outputText(" A pair of floppy rabbit ears stick up out of your " + liveData.player.hairDescript() + ", bouncing around as you walk.")
            else if (liveData.player.earType == ENUM.EarType.EARS_KANGAROO) GUI.outputText(" The " + liveData.player.hairDescript() + " atop your head is parted by a pair of long, furred kangaroo ears that stick out at an angle.")
            else if (liveData.player.earType == ENUM.EarType.EARS_FOX) GUI.outputText(" The " + liveData.player.hairDescript() + " atop your head is parted by a pair of large, adept fox ears that always seem to be listening.")
            else if (liveData.player.earType == ENUM.EarType.EARS_DRAGON)
                GUI.outputText(" The " + liveData.player.hairDescript() + " atop your head is parted by a pair of rounded protrusions with small holes on the sides of your head serve as your ears. Bony fins sprout behind them.")
            else if (liveData.player.earType == ENUM.EarType.EARS_RACCOON) GUI.outputText(" The " + liveData.player.hairDescript() + " on your head parts around a pair of egg-shaped, furry raccoon ears.")
            else if (liveData.player.earType == ENUM.EarType.EARS_MOUSE) GUI.outputText(" The " + liveData.player.hairDescript() + " atop your head is funneled between and around a pair of large, dish-shaped mouse ears that stick up prominently.")
            //<mod> Mod-added ears
            else if (liveData.player.earType == ENUM.EarType.EARS_PIG)
                GUI.outputText(" The " + liveData.player.hairDescript() + " on your head is parted by a pair of pointy, floppy pig ears. They often flick about when you're not thinking about it.")
            else if (liveData.player.earType == ENUM.EarType.EARS_RHINO) GUI.outputText(" The " + liveData.player.hairDescript() + " on your head is parted by a pair of tubular rhino ears.")
            else if (liveData.player.earType == ENUM.EarType.EARS_ECHIDNA) GUI.outputText(" Your " + liveData.player.hairDescript() + " makes it near-impossible to see the small, rounded openings that are your ears.")
            else if (liveData.player.earType == ENUM.EarType.EARS_DEER) GUI.outputText(" The " + liveData.player.hairDescript() + " on your head parts around a pair of deer-like ears that grow up from your head.")
            //</mod>
            if (liveData.player.antennae == ENUM.AntennaeType.ANTENNAE_BEE) {
                if (liveData.player.earType == ENUM.EarType.EARS_BUNNY) GUI.outputText(" Limp antennae also grow from just behind your hairline, waving and swaying in the breeze with your ears.")
        ...
        if (liveData.player.tongueType == ENUM.TongueType.TONGUE_SNAKE) GUI.outputText(" A snake-like tongue occasionally flits between your lips, tasting the air.")
            ...
            GUI.outputText(" A slowly undulating tongue occasionally slips from between your lips. It hangs nearly two feet long when you let the whole thing slide out, though you can retract it to appear normal.")
        else if (liveData.player.tongueType == ENUM.TongueType.TONGUE_DRACONIC)
            GUI.outputText(" Your mouth contains a thick, fleshy tongue that, if you so desire, can telescope to a distance of about four feet. It has sufficient manual dexterity that you can use it almost like a third arm.")
        else if (liveData.player.tongueType == ENUM.TongueType.TONGUE_ECHIDNA) GUI.outputText(" A thin echidna tongue, at least a foot long, occasionally flits out from between your lips.")
        //Horns
        switch (liveData.player.hornType) {
            case ENUM.HornType.HORNS_DEMON:
                if (liveData.player.horns == 2) GUI.outputText(" A small pair of pointed horns has broken through the " + liveData.player.skinDesc + " on your forehead, proclaiming some demonic taint to any who see them.")
                if (liveData.player.horns == 4)
                    GUI.outputText(" A quartet of prominent horns has broken through your " + liveData.player.skinDesc + ". The back pair are longer, and curve back along your head. The front pair protrude forward demonically.")
            ...
            case ENUM.HornType.HORNS_COW_MINOTAUR:
                if (liveData.player.horns < 3) GUI.outputText(" Two tiny horn-like nubs protrude from your forehead, resembling the horns of the young livestock kept by your village.")
                if (liveData.player.horns >= 3 && liveData.player.horns < 6) GUI.outputText(" Two moderately sized horns grow from your forehead, similar in size to those on a young bovine.")
                if (liveData.player.horns >= 6 && liveData.player.horns < 12) GUI.outputText(" Two large horns sprout from your forehead, curving forwards like those of a bull.")
                if (liveData.player.horns >= 12 && liveData.player.horns < 20) GUI.outputText(" Two very large and dangerous looking horns sprout from your head, curving forward and over a foot long. They have dangerous looking points.")
                if (liveData.player.horns >= 20) GUI.outputText(" Two huge horns erupt from your forehead, curving outward at first, then forwards. The weight of them is heavy, and they end in dangerous looking points.")
                break
            case ENUM.HornType.HORNS_DRACONIC_X2:
                if (liveData.useMetrics) GUI.outputText(" A pair of " + UTIL.num2Text(Math.floor(liveData.player.horns * 2.54)) + " centimetre horns grow from the sides of your head, sweeping backwards and adding to your imposing visage.")
                else GUI.outputText(" A pair of " + UTIL.num2Text(liveData.player.horns) + " inch horns grow from the sides of your head, sweeping backwards and adding to your imposing visage.")
                break
            case ENUM.HornType.HORNS_DRACONIC_X4_12_INCH_LONG:
                GUI.outputText(" Two pairs of horns, roughly a foot long, sprout from the sides of your head. They sweep back and give you a fearsome look, almost like the dragons from your village's legends.")
                break
            case ENUM.HornType.HORNS_ANTLERS:
                GUI.outputText(" Two antlers, forking into " + UTIL.num2Text(liveData.player.horns) + " points, have sprouted from the top of your head, forming a spiky, regal crown of bone.")
                break
        ...
        switch (liveData.player.wingType) {
            ...
            case ENUM.WingType.WING_TYPE_BEE_LIKE_LARGE:
                GUI.outputText(" A pair of large bee-wings sprout from your back, reflecting the light through their clear membranes beautifully. They flap quickly, allowing you to easily hover in place or fly.")
                break
            ...
            case ENUM.WingType.WING_TYPE_BAT_LIKE_LARGE:
                GUI.outputText(" A pair of large bat-like demon-wings fold behind your shoulders. With a muscle-twitch, you can extend them, and use them to soar gracefully through the air.")
                break
            ...
            case ENUM.WingType.WING_TYPE_FEATHERED_LARGE:
                GUI.outputText(" A pair of large, feathery wings sprout from your back. Though you usually keep the " + liveData.player.furColor + "-colored wings folded close, they can unfurl to allow you to soar as gracefully as a harpy.")
                break
            case ENUM.WingType.WING_TYPE_DRACONIC_SMALL:
                GUI.outputText(" Small, vestigial wings sprout from your shoulders. They might look like bat's wings, but the membranes are covered in fine, delicate scales.")
                break
            ...
            case ENUM.WingType.WING_TYPE_GIANT_DRAGONFLY:
                GUI.outputText(" Giant dragonfly wings hang from your shoulders. At a whim, you could twist them into a whirring rhythm fast enough to lift you off the ground and allow you to fly.")
                break
            default:
            //No wings here!
        }
        //Wing arms
        if (liveData.player.armType == ENUM.ArmType.ARM_TYPE_HARPY) GUI.outputText(" Feathers hang off your arms from shoulder to wrist, giving them a slightly wing-like look.")
        else if (liveData.player.armType == ENUM.ArmType.ARM_TYPE_SPIDER) GUI.outputText(" Shining black exoskeleton covers your arms from the biceps down, resembling a pair of long black gloves from a distance.")
        //Done with head bits. Move on to body stuff
        //Horse lowerbody, other lowerbody texts appear lower
        if (liveData.player.isTaur()) {
            if (liveData.player.lowerBody == ENUM.LowerBodyType.LOWER_BODY_TYPE_HOOFED) GUI.outputText(" From the waist down you have the body of a horse, with all " + UTIL.num2Text(liveData.player.legCount) + " legs capped by hooves.")
            else if (liveData.player.lowerBody == ENUM.LowerBodyType.LOWER_BODY_TYPE_PONY)
                GUI.outputText(" From the waist down you have an incredibly cute and cartoonish parody of a horse's body, with all " + UTIL.num2Text(liveData.player.legCount) + " legs ending in flat, rounded feet.")
            else if (liveData.player.lowerBody == ENUM.LowerBodyType.LOWER_BODY_TYPE_DRIDER_LOWER_BODY)
                GUI.outputText(" Where your legs would normally start you have grown the body of a spider, with " + UTIL.num2Text(liveData.player.legCount) + " spindly legs that sprout from its sides.")
            else GUI.outputText(" Where your legs would normally start you have grown the body of a feral animal, with all " + UTIL.num2Text(liveData.player.legCount) + " legs.")
        }
        ...
        if (!liveData.player.isTaur()) {
            if (liveData.player.thickness > 70) {
                ...
                if (liveData.player.hipRating >= 10 && liveData.player.hipRating < 15) GUI.outputText(" that would be much more noticeable if you weren't so wide-bodied, and")
        ...
        //Non-horse PCs
            ...
            //FITBUTT
                ...
                if (liveData.player.buttRating >= 4 && liveData.player.buttRating < 6) GUI.outputText(" contracts with every motion, displaying the detailed curves of its lean musculature.")
                if (liveData.player.buttRating >= 6 && liveData.player.buttRating < 10) GUI.outputText(" fills out your clothing nicely.")
                ...
                if (liveData.player.buttRating >= 15 && liveData.player.buttRating < 20) GUI.outputText(" threatens to bust out from under your kit each time you clench it.")
        ...
        switch (liveData.player.tailType) {
            ...
            case ENUM.TailType.TAIL_TYPE_DOG:
                GUI.outputText(" A fuzzy " + liveData.player.furColor + " dogtail sprouts just above your " + liveData.player.buttDescript() + ", wagging to and fro whenever you are happy.")
                break
            case ENUM.TailType.TAIL_TYPE_DEMONIC:
                GUI.outputText(" A narrow tail ending in a spaded tip curls down from your " + liveData.player.buttDescript() + ", wrapping around your " + liveData.player.leg() + " sensually at every opportunity.")
                break
            ...
            case ENUM.TailType.TAIL_TYPE_SPIDER_ADBOMEN:
                GUI.outputText(" A large, spherical spider-abdomen has grown out from your backside, covered in shiny black chitin. Though it's heavy and bobs with every motion, it doesn't seem to slow you down.")
            ...
            case ENUM.TailType.TAIL_TYPE_BEE_ABDOMEN:
                GUI.outputText(" A large insectile bee-abdomen dangles from just above your backside, bobbing with its own weight as you shift. It is covered in hard chitin with black and yellow stripes, and tipped with a dagger-like stinger.")
            ...
            case ENUM.TailType.TAIL_TYPE_CAT:
                GUI.outputText(" A soft " + liveData.player.furColor + " cat-tail sprouts just above your " + liveData.player.buttDescript() + ", curling and twisting with every step to maintain perfect balance.")
                break
            case ENUM.TailType.TAIL_TYPE_LIZARD:
                GUI.outputText(" A tapered tail hangs down from just above your " + liveData.player.assDescript() + ". It sways back and forth, assisting you with keeping your balance.")
                break
            case ENUM.TailType.TAIL_TYPE_RABBIT:
                GUI.outputText(" A short, soft bunny tail sprouts just above your " + liveData.player.assDescript() + ", twitching constantly whenever you don't think about it.")
                break
            case ENUM.TailType.TAIL_TYPE_HARPY:
                GUI.outputText(" A tail of feathers fans out from just above your " + liveData.player.assDescript() + ", twitching instinctively to help guide you if you were to take flight.")
                break
            ...
            case ENUM.TailType.TAIL_TYPE_FOX:
                if (liveData.player.tailVenom <= 1) GUI.outputText(" A swishing " + liveData.player.furColor + " fox's brush extends from your " + liveData.player.assDescript() + ", curling around your body - the soft fur feels lovely.")
                else
                    GUI.outputText(
                        " " + UTIL.Num2Text(liveData.player.tailVenom) + " swishing " + liveData.player.furColor + " fox's tails extend from your " + liveData.player.assDescript() + ", curling around your body - the soft fur feels lovely."
                    )
                break
            case ENUM.TailType.TAIL_TYPE_DRACONIC:
                GUI.outputText(" A thin, scaly, prehensile reptilian tail, almost as long as you are tall, swings behind you like a living bullwhip. Its tip menaces with spikes of bone, meant to deliver painful blows.")
                break
        ...
        //LOWERBODY SPECIAL
        if (liveData.player.lowerBody == ENUM.LowerBodyType.LOWER_BODY_TYPE_HUMAN) GUI.outputText(" " + UTIL.Num2Text(liveData.player.legCount) + " normal human legs grow down from your waist, ending in normal human feet.")
        else if (liveData.player.lowerBody == ENUM.LowerBodyType.LOWER_BODY_TYPE_FERRET)
            GUI.outputText(" " + UTIL.Num2Text(liveData.player.legCount) + " furry, digitigrade legs form below your [hips]. The fur is thinner on the feet, and your toes are tipped with claws.")
        else if (liveData.player.lowerBody == ENUM.LowerBodyType.LOWER_BODY_TYPE_HOOFED) GUI.outputText(" Your " + UTIL.num2Text(liveData.player.legCount) + " legs are muscled and jointed oddly, covered in fur, and end in a bestial hooves.")
        else if (liveData.player.lowerBody == ENUM.LowerBodyType.LOWER_BODY_TYPE_DOG) GUI.outputText(" " + UTIL.Num2Text(liveData.player.legCount) + " digitigrade legs grow downwards from your waist, ending in dog-like hind-paws.")
        else if (liveData.player.lowerBody == ENUM.LowerBodyType.LOWER_BODY_TYPE_NAGA) GUI.outputText(" Below your waist your flesh is fused together into a very long snake-like tail.")
        //Horse body is placed higher for readability purposes
            ...
            GUI.outputText(" Your " + UTIL.num2Text(liveData.player.legCount) + " perfect lissome legs end in mostly human feet, apart from the horn protruding straight down from the heel that forces you to walk with a sexy, swaying gait.")
        else if (liveData.player.lowerBody == ENUM.LowerBodyType.LOWER_BODY_TYPE_DEMONIC_CLAWS)
            GUI.outputText(" Your " + UTIL.num2Text(liveData.player.legCount) + " lithe legs are capped with flexible clawed feet. Sharp black nails grow where once you had toe-nails, giving you fantastic grip.")
        ...
        else if (liveData.player.lowerBody == ENUM.LowerBodyType.LOWER_BODY_TYPE_CAT) GUI.outputText(" " + UTIL.Num2Text(liveData.player.legCount) + " digitigrade legs grow downwards from your waist, ending in soft, padded cat-paws.")
        else if (liveData.player.lowerBody == ENUM.LowerBodyType.LOWER_BODY_TYPE_LIZARD)
            GUI.outputText(
                " " + UTIL.Num2Text(liveData.player.legCount) + " digitigrade legs grow down from your " + liveData.player.hipDescript() + ", ending in clawed feet. There are three long toes on the front, and a small hind-claw on the back."
            )
        else if (liveData.player.lowerBody == ENUM.LowerBodyType.LOWER_BODY_TYPE_BUNNY)
            GUI.outputText(" Your " + UTIL.num2Text(liveData.player.legCount) + " legs thicken below the waist as they turn into soft-furred rabbit-like legs. You even have large bunny feet that make hopping around a little easier than walking.")
            ...
            GUI.outputText(" Your " + UTIL.num2Text(liveData.player.legCount) + " furry legs have short thighs and long calves, with even longer feet ending in prominently-nailed toes.")
        else if (liveData.player.lowerBody == ENUM.LowerBodyType.LOWER_BODY_TYPE_CHITINOUS_SPIDER_LEGS)
            GUI.outputText(" Your " + UTIL.num2Text(liveData.player.legCount) + " legs are covered in a reflective black, insectile carapace up to your mid-thigh, looking more like a set of 'fuck-me-boots' than exoskeleton.")
        else if (liveData.player.lowerBody == ENUM.LowerBodyType.LOWER_BODY_TYPE_FOX)
            GUI.outputText(" Your " + UTIL.num2Text(liveData.player.legCount) + " legs are crooked into high knees with hocks and long feet, like those of a fox; cute bulbous toes decorate the ends.")
            ...
            GUI.outputText(" Your " + UTIL.num2Text(liveData.player.legCount) + " legs, though covered in fur, are humanlike. Long feet on the ends bear equally long toes, and the pads on the bottoms are quite sensitive to the touch.")
        else if (liveData.player.lowerBody == ENUM.LowerBodyType.LOWER_BODY_TYPE_CLOVEN_HOOFED) GUI.outputText(" " + UTIL.Num2Text(liveData.player.legCount) + " digitigrade legs form below your [hips], ending in cloven hooves.")
        if (liveData.player.findPerk(PerkLib.Incorporeality) >= 0) GUI.outputText(" Of course, your " + liveData.player.legs() + " are partially transparent due to their ghostly nature.") // isn't goo transparent anyway?

        GUI.outputText("<br>")
        if (liveData.player.findStatusEffect(StatusEffects.GooStuffed) >= 0) {
            GUI.outputText("<br><b>Your gravid-looking belly is absolutely stuffed full of goo. There's no way you can get pregnant like this, but at the same time, you look like some fat-bellied breeder.</b><br>")
        }
        //Pregnancy Shiiiiiitz
        if (liveData.player.buttPregnancyType == ENUM.PregnancyType.PREGNANCY_FROG_GIRL || liveData.player.buttPregnancyType == ENUM.PregnancyType.PREGNANCY_SATYR || liveData.player.isPregnant()) {
            ...
            if (liveData.player.buttPregnancyType == ENUM.PregnancyType.PREGNANCY_SATYR && liveData.player.buttPregnancyIncubation > liveData.player.pregnancyIncubation) {
                if (liveData.player.buttPregnancyIncubation < 125 && liveData.player.buttPregnancyIncubation >= 75) {
                    ...
                    if (
                        ...
                        (liveData.player.pregnancyType == ENUM.PregnancyType.PREGNANCY_JOJO && (liveData.gameFlags[FLAG.JOJO_CORRUPTION_STAGE] <= 0 || liveData.gameFlags[FLAG.JOJO_BIMBO_STATE] >= 3)) ||
                    ...
                    else if (liveData.player.pregnancyType != ENUM.PregnancyType.PREGNANCY_MARBLE) GUI.outputText("<br><b>Your belly protrudes unnaturally far forward, bulging with the unclean spawn of some monster or beast.</b>")
            ...
            //URTA PREG
                ...
                if (liveData.player.pregnancyIncubation <= 216 && liveData.player.pregnancyIncubation > 144) {
                    GUI.outputText("<b>Your belly is large and very obviously pregnant to anyone who looks at you. It's gotten heavy enough to be a pain to carry around all the time.</b>")
                }
                if (liveData.player.pregnancyIncubation <= 144 && liveData.player.pregnancyIncubation > 72) {
                    GUI.outputText("<b>It would be impossible to conceal your growing pregnancy from anyone who glanced your way. It's large and round, frequently moving.</b>")
                }
                ...
                if (liveData.player.buttPregnancyIncubation >= 8)
                    GUI.outputText("<b>Your stomach is so full of frog eggs that you look about to birth at any moment, your belly wobbling and shaking with every step you take, packed with frog ovum.</b>")
                else
                    GUI.outputText(
                ...
                if (liveData.player.pregnancyIncubation <= 100) GUI.outputText("It's full of liquid, though unlike a normal pregnancy the passenger you're carrying is tiny.</b>")
        ...
        GUI.outputText("<br>")
        if (liveData.player.gills) GUI.outputText("A pair of feathery gills are growing out just below your neck, spreading out horizontally and draping down your chest. They allow you to stay in the water for quite a long time. ")
        ...
        if (liveData.player.breastRows.length == 0) {
            ...
            if (liveData.player.breastRows[0].milkFullness > 75) GUI.outputText(" Your " + liveData.player.breastDescript(temp) + " are painful and sensitive from being so stuffed with milk. You should release the pressure soon.")
        ...
        if (liveData.player.lowerBody == ENUM.LowerBodyType.LOWER_BODY_TYPE_NAGA && liveData.player.gender > 0) {
            ...
            GUI.outputText("concealed within a cavity in your tail when not in use, though when the need arises, you can part your concealing slit and reveal your true self.<br>")
        }
        //Cock stuff!
        temp = 0
        if (liveData.player.cocks.length == 1) {
            ...
            if (liveData.useMetrics) GUI.outputText("<br>Your " + liveData.player.cockDescript(temp) + " is " + Math.round(liveData.player.cocks[temp].cockLength * 10 * 2.54) / 10 + " cm long and ")
            else GUI.outputText("<br>Your " + liveData.player.cockDescript(temp) + " is " + Math.round(liveData.player.cocks[temp].cockLength * 10) / 10 + " inches long and ")
            if (Math.round(10 * liveData.player.cocks[temp].cockThickness) / 10 < 10) {
                if (liveData.useMetrics) {
                    if (Math.round(liveData.player.cocks[temp].cockThickness * 10 * 2.54) / 10 == 1) GUI.outputText(Math.round(liveData.player.cocks[temp].cockThickness * 10 * 2.54) / 10 + " centimetre thick.")
                    else GUI.outputText(Math.round(liveData.player.cocks[temp].cockThickness * 10 * 2.54) / 10 + " centimetres thick.")
                } else {
                    if (Math.round(liveData.player.cocks[temp].cockThickness * 10) / 10 == 1) GUI.outputText(Math.round(liveData.player.cocks[temp].cockThickness * 10) / 10 + " inch thick.")
            ...
            if (liveData.player.cocks[temp].cockType == ENUM.CockType.DOG || liveData.player.cocks[temp].cockType == ENUM.CockType.FOX || liveData.player.cocks[temp].cockType == ENUM.CockType.FOX) {
                if (liveData.player.cocks[temp].knotMultiplier >= 1.8) GUI.outputText(" The obscenely swollen lump of flesh near the base of your " + liveData.player.cockDescript(temp) + " looks almost too big for your cock.")
                else if (liveData.player.cocks[temp].knotMultiplier >= 1.4) GUI.outputText(" A large bulge of flesh nestles just above the bottom of your " + liveData.player.cockDescript(temp) + ", to ensure it stays where it belongs during mating.")
                else if (liveData.player.cocks[temp].knotMultiplier > 1) GUI.outputText(" A small knot of thicker flesh is near the base of your " + liveData.player.cockDescript(temp) + ", ready to expand to help you lodge it inside a female.")
                //List thickness
                GUI.outputText(" The knot is " + Math.round(liveData.player.cocks[temp].cockThickness * liveData.player.cocks[temp].knotMultiplier * 10) / 10 + " inches wide when at full size.")
            }
            //Demon cock flavor
            if (liveData.player.cocks[temp].cockType == ENUM.CockType.DEMON) {
                GUI.outputText(" The crown is ringed with a circle of rubbery protrusions that grow larger as you get more aroused. The entire thing is shiny and covered with tiny, sensitive nodules that leave no doubt about its demonic origins.")
            }
            ...
            if (liveData.player.cocks[temp].cockType == ENUM.CockType.LIZARD) {
                GUI.outputText(" It's a deep, iridescent purple in color. Unlike a human penis, the shaft is not smooth, and is instead patterned with multiple bulbous bumps.")
            }
            //Anemone cock flavor
            if (liveData.player.cocks[temp].cockType == ENUM.CockType.ANEMONE) {
                GUI.outputText(" The crown is surrounded by tiny tentacles with a venomous, aphrodisiac payload. At its base a number of similar, longer tentacles have formed, guaranteeing that pleasure will be forced upon your partners.")
            }
            //Kangawang flavor
            ...
            if (liveData.player.cocks[temp].cockType == ENUM.CockType.DRAGON) {
                GUI.outputText(" With its tapered tip, there are few holes you wouldn't be able to get into. It has a strange, knot-like bulb at its base, but doesn't usually flare during arousal as a dog's knot would.")
            }
            //Bee flavor
            ...
            if (liveData.player.findStatusEffect(StatusEffects.Infested) >= 0)
                GUI.outputText(" Every now and again a slimy worm coated in spunk slips partway out of your " + liveData.player.cockDescript(0) + ", tasting the air like a snake's tongue.")
        ...
        if (liveData.player.cocks.length > 1) {
            ...
            while (temp < liveData.player.cocks.length) {
                ...
                if (rando == 0) {
                    ...
                    if (Math.floor(liveData.player.cocks[temp].cockThickness) >= 2) GUI.outputText(UTIL.num2Text(Math.round(liveData.player.cocks[temp].cockThickness * 10) / 10) + " inches wide.")
                    else {
                ...
                if (rando == 1) {
                    ...
                    if (Math.floor(liveData.player.cocks[temp].cockThickness) >= 2) GUI.outputText(UTIL.num2Text(Math.round(liveData.player.cocks[temp].cockThickness * 10) / 10) + " inches thick.")
                    else {
                ...
                if (rando == 3) {
                    ...
                    if (Math.floor(liveData.player.cocks[temp].cockThickness) >= 2) GUI.outputText(UTIL.num2Text(Math.round(liveData.player.cocks[temp].cockThickness * 10) / 10) + " inches in diameter.")
                    else {
                ...
                //horse cock flavor
                if (liveData.player.cocks[temp].cockType == ENUM.CockType.HORSE) {
                    GUI.outputText(" It's mottled black and brown in a very animalistic pattern. The 'head' of your " + liveData.player.cockDescript(temp) + " flares proudly, just like a horse's.")
                }
                //dog cock flavor
                ...
                if (liveData.player.cocks[temp].knotMultiplier > 1) {
                    if (liveData.player.cocks[temp].knotMultiplier >= 1.8)
                        GUI.outputText(" The obscenely swollen lump of flesh near the base of your " + liveData.player.cockDescript(temp) + " looks almost comically mismatched for your " + liveData.player.cockDescript(temp) + ".")
                    else if (liveData.player.cocks[temp].knotMultiplier >= 1.4)
                        GUI.outputText(" A large bulge of flesh nestles just above the bottom of your " + liveData.player.cockDescript(temp) + ", to ensure it stays where it belongs during mating.")
                    else GUI.outputText(" A small knot of thicker flesh is near the base of your " + liveData.player.cockDescript(temp) + ", ready to expand to help you lodge your " + liveData.player.cockDescript(temp) + " inside a female.")
                    //List knot thickness
                    GUI.outputText(" The knot is " + Math.floor(liveData.player.cocks[temp].cockThickness * liveData.player.cocks[temp].knotMultiplier * 10) / 10 + " inches thick when at full size.")
                }
            ...
            //Worm flavor
            if (liveData.player.findStatusEffect(StatusEffects.Infested) >= 0)
                GUI.outputText("Every now and again slimy worms coated in spunk slip partway out of your " + liveData.player.multiCockDescriptLight() + ", tasting the air like tongues of snakes.<br>")
            //DONE WITH COCKS, moving on!
        }
        //Of Balls and Sacks!
        if (liveData.player.balls > 0) {
            if (liveData.player.findStatusEffect(StatusEffects.Uniball) >= 0) {
                if (liveData.player.skinType != ENUM.SkinType.SKIN_TYPE_GOO) GUI.outputText("Your [sack] clings tightly to your groin, holding " + liveData.player.ballsDescript() + " snugly against you.")
                else if (liveData.player.skinType == ENUM.SkinType.SKIN_TYPE_GOO) GUI.outputText("Your [sack] clings tightly to your groin, dripping and holding " + liveData.player.ballsDescript() + " snugly against you.")
            } else if (liveData.player.cockTotal() == 0) {
                if (liveData.player.skinType == ENUM.SkinType.SKIN_TYPE_PLAIN) GUI.outputText("A " + liveData.player.sackDescript() + " with " + liveData.player.ballsDescript() + " swings heavily under where a penis would normally grow.")
                if (liveData.player.skinType == ENUM.SkinType.SKIN_TYPE_FUR) GUI.outputText("A fuzzy " + liveData.player.sackDescript() + " filled with " + liveData.player.ballsDescript() + " swings low under where a penis would normally grow.")
                if (liveData.player.skinType == ENUM.SkinType.SKIN_TYPE_SCALES) GUI.outputText("A scaley " + liveData.player.sackDescript() + " hugs your " + liveData.player.ballsDescript() + " tightly against your body.")
                if (liveData.player.skinType == ENUM.SkinType.SKIN_TYPE_GOO) GUI.outputText("An oozing, semi-solid sack with " + liveData.player.ballsDescript() + " swings heavily under where a penis would normally grow.")
            } else {
                if (liveData.player.skinType == ENUM.SkinType.SKIN_TYPE_PLAIN)
                    GUI.outputText("A " + liveData.player.sackDescript() + " with " + liveData.player.ballsDescript() + " swings heavily beneath your " + liveData.player.multiCockDescriptLight() + ".")
                if (liveData.player.skinType == ENUM.SkinType.SKIN_TYPE_FUR)
                    GUI.outputText("A fuzzy " + liveData.player.sackDescript() + " filled with " + liveData.player.ballsDescript() + " swings low under your " + liveData.player.multiCockDescriptLight() + ".")
                if (liveData.player.skinType == ENUM.SkinType.SKIN_TYPE_SCALES) GUI.outputText("A scaley " + liveData.player.sackDescript() + " hugs your " + liveData.player.ballsDescript() + " tightly against your body.")
                if (liveData.player.skinType == ENUM.SkinType.SKIN_TYPE_GOO) GUI.outputText("An oozing, semi-solid sack with " + liveData.player.ballsDescript() + " swings heavily beneath your " + liveData.player.multiCockDescriptLight() + ".")
            }
        ...
        if (liveData.player.vaginas.length > 0) {
            if (liveData.player.gender == 2 && liveData.player.isTaur()) GUI.outputText("<br>Your womanly parts have shifted to lie between your hind legs, in a rather feral fashion.")
            GUI.outputText("<br>")
            if (liveData.player.vaginas.length == 1) {
                if (liveData.useMetrics) GUI.outputText("You have a " + liveData.player.vaginaDescript(0) + ", with a " + Math.floor(liveData.player.vaginas[0].clitLength * 10 * 2.54) / 10 + "-centimetre clit")
                else GUI.outputText("You have a " + liveData.player.vaginaDescript(0) + ", with a " + Math.floor(liveData.player.vaginas[0].clitLength * 10) / 10 + "-inch clit")
                if (liveData.player.vaginas[0].virgin) GUI.outputText(" and an intact hymen")
                GUI.outputText(". ")
            }
            if (liveData.player.vaginas.length > 1) {
                if (liveData.useMetrics)
                    GUI.outputText("You have " + liveData.player.vaginas.length + " " + liveData.player.vaginaDescript(0) + "s, with " + Math.floor(liveData.player.vaginas[0].clitLength * 10 * 2.54) / 10 + "-centimetre clits each. ")
                else GUI.outputText("You have " + liveData.player.vaginas.length + " " + liveData.player.vaginaDescript(0) + "s, with " + Math.floor(liveData.player.vaginas[0].clitLength * 10) / 10 + "-inch clits each. ")
            }
            if (liveData.player.lib < 50 && liveData.player.lust < 50) {
                //not particularly horny
                //Wetness
                if (liveData.player.vaginas[0].vaginalWetness >= ENUM.VaginalWetnessType.VAGINA_WETNESS_WET && liveData.player.vaginas[0].vaginalWetness < ENUM.VaginalWetnessType.VAGINA_WETNESS_DROOLING) GUI.outputText("Moisture gleams in ")
                ...
                if (liveData.player.vaginas[0].vaginalWetness >= ENUM.VaginalWetnessType.VAGINA_WETNESS_WET) {
                    if (liveData.player.vaginas[0].vaginalLooseness < ENUM.VaginalLoosenessType.VAGINA_LOOSENESS_LOOSE) GUI.outputText("your " + liveData.player.vaginaDescript(0) + ". ")
                    if (liveData.player.vaginas[0].vaginalLooseness >= ENUM.VaginalLoosenessType.VAGINA_LOOSENESS_LOOSE && liveData.player.vaginas[0].vaginalLooseness < ENUM.VaginalLoosenessType.VAGINA_LOOSENESS_GAPING_WIDE)
                        GUI.outputText("your " + liveData.player.vaginaDescript(0) + ", its lips slightly parted. ")
                    if (liveData.player.vaginas[0].vaginalLooseness >= ENUM.VaginalLoosenessType.VAGINA_LOOSENESS_GAPING_WIDE) GUI.outputText("the massive hole that is your " + liveData.player.vaginaDescript(0) + ". ")
                }
            }
            ...
            if (liveData.player.lib > 80 || liveData.player.lust > 80) {
                ...
                if (liveData.player.vaginas[0].vaginalLooseness >= ENUM.VaginalLoosenessType.VAGINA_LOOSENESS_LOOSE && liveData.player.vaginas[0].vaginalLooseness < ENUM.VaginalLoosenessType.VAGINA_LOOSENESS_GAPING_WIDE)
                    GUI.outputText("your " + liveData.player.vaginaDescript(0) + ", its lips slightly parted. ")
                if (liveData.player.vaginas[0].vaginalLooseness >= ENUM.VaginalLoosenessType.VAGINA_LOOSENESS_GAPING_WIDE) GUI.outputText("the massive hole that is your cunt. ")
            }
        ...
        if (liveData.player.gems > 1) GUI.outputText("<br><br><b>You have " + UTIL.formatNumber(Math.floor(liveData.player.gems)) + " shining gems, collected in your travels.")
        if (liveData.player.gems == 1) GUI.outputText("<br><br><b>You have " + UTIL.formatNumber(Math.floor(liveData.player.gems)) + " shining gem, collected in your travels.")
        GUI.doNext(liveData.playerMenu)
    }
}
