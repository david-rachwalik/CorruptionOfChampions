...
export function reductoClit() {
  ...
  GUI.outputText(
    'Your ' + liveData.player.clitDescript() + ' shrinks rapidly, dwindling down to almost half its old size before it finishes absorbing the paste.',
  );
  liveData.player.dynStats(['sen', 2]);
  liveData.player.changeLust(10);
  Inventory.itemGoNext();
    ...
    if (liveData.player.cocks.length == 1) {
      GUI.outputText(
        'Your ' +
          ...
          (liveData.player.hasSheath() ? 'sheath' : 'crotch') +
