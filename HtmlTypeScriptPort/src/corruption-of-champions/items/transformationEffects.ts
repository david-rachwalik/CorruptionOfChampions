...
export function beeTFs(type: number): void {
  ...
  if ((pure || special) && liveData.player.pregnancyType == ENUM.PregnancyType.FAERIE) {
    ...
    if (liveData.gameFlags[FLAG.PREGNANCY_CORRUPTION] > 1) {
      ...
      GUI.outputText(
        "<br><br>You feel queasy and want to throw up.  There's a pain in your belly and you realize the baby you're carrying didn't like that at all.  Then again, maybe pure honey is good for it.",
      );
    } else if (liveData.gameFlags[FLAG.PREGNANCY_CORRUPTION] < 1) {
      //Child is faerie, loves pure honey
      GUI.outputText(
        "<br><br>A warm sensation starts in your belly and runs all through your body.  It's almost as if you're feeling music and you guess your passenger enjoyed the meal.",
      );
    } else {
  ...
  if (changes < changeLimit && liveData.player.tailType == ENUM.TailType.BEE_ABDOMEN && liveData.player.tailRecharge < 15 && UTIL.rand(2)) {
    ...
    GUI.outputText(
      '<br><br>Your abdomen swells with vitality and a drop of venom escapes your stinger as it begins producing it in slightly larger quantities.',
    );
    changes++;
  }
  //Wings
  ...
  if (special) {
    ...
    if (!liveData.player.hasCock()) {
      ...
      const biggest = liveData.player.biggestCockIndex();
      GUI.outputText(
      ...
      if (mult > 0) {
        GUI.outputText(
          "<br><br>The effects of the honey don't seem to focus on your groin this time, but you still feel your " +
            liveData.player.cockDescript(0) +
    ...
    if (liveData.player.cor >= 5) {
      GUI.outputText(
        '<br><br>Your mind feels surprisingly clear of the twisted thoughts that have plagued it as of late, but you find yourself feeling more and more aroused than usual.',
      );
      const corLoss = Math.min(0.1 * liveData.player.cor + 5, liveData.player.cor);
      liveData.player.modStats(['cor', -corLoss], ['lib', corLoss]); //Lose corruption and gains that much libido
    } else {
      GUI.outputText(
        '<br><br>You find your mind is drifting to the thought of using your member to fertilize hundreds and hundreds of eggs every day. You shake your head, the bizarre fantasy catching you completely off guard.',
      );
...
export function canineTFs(type: number): void {
  ...
  let temp2 = 0;
  let temp3 = 0;
  const crit = UTIL.rand(10) / 10 + 0.5;
  ...
  if (type == 3) {
    ...
    GUI.outputText(
      "<br><br>You feel yourself relaxing as gentle warmth spreads through your body. Honestly you don't think you'd mind running into a demon or monster right now, they'd make for good entertainment.",
    );
      ...
      if (liveData.player.countCocksOfType(ENUM.CockType.DOG) > 0) {
        ...
        while (temp > 0) {
          ...
          if (
            ...
            liveData.player.cocks[temp].knotMultiplier < liveData.player.cocks[temp2].knotMultiplier
          )
            temp2 = temp;
        }
  ...
  if (liveData.player.cocks.length > 0) {
    ...
    if (liveData.player.cumMultiplier < 2 && UTIL.rand(2) == 0 && changes < changeLimit) {
      ...
      if (temp < liveData.player.cumMultiplier + 0.05 * crit) {
        ...
        if (liveData.player.balls > 0)
          GUI.outputText(
            '<br><br>You feel a churning in your ' + liveData.player.ballsDescriptLight() + '. It quickly settles, leaving them feeling somewhat more dense.',
          );
    ...
    if (type == 1) {
      ...
      if (temp2 <= 1)
        GUI.outputText(
          '<br><br>A slight pressure builds and releases as your ' + liveData.player.cockDescript(temp) + ' pushes a bit further out of your crotch.',
        );
    }
  }
  //Female Stuff
      ...
      if (liveData.player.breastRows.length < 3 && UTIL.rand(2) == 0 && changes < changeLimit) {
        ...
        if (liveData.player.breastRows.length == 2) {
          ...
          if (liveData.player.breastRows[0].breastRating - 1 == 0)
            ...
            GUI.outputText(
              '<br><br>A second set of breasts bulges forth under your current pair, stopping as they reach ' + liveData.player.breastCup(temp) + 's.',
            );
        ...
        if (liveData.player.breastRows.length > 2 && liveData.player.breastRows[0].breastRating > liveData.player.breastRows.length) {
          ...
          if (liveData.player.breastRows[temp].breastRating == 0)
            GUI.outputText(
              '<br><br>Your abdomen tingles and twitches as a new row of breasts sprouts below the others. Your new breasts stay flat and masculine, not growing any larger.',
            );
          else
            GUI.outputText(
              '<br><br>Your abdomen tingles and twitches as a new row of ' +
                liveData.player.breastCup(temp) +
                ' ' +
                liveData.player.breastDescript(temp) +
  ...
  if (changes < changeLimit && liveData.player.dogScore() >= 3 && UTIL.rand(4) == 0) {
    changes++;
    GUI.outputText(
      '<br><br>Images and thoughts come unbidden to your mind, overwhelming your control as you rapidly lose yourself in them, daydreaming of... ',
    );
    //cawk fantasies
        ...
        if (liveData.player.cor < 66)
          ...
          if (liveData.player.cor < 80) {
            if (liveData.player.vaginas.length > 0)
              GUI.outputText(
                '<br>You reluctantly pry your hand from your aching ' + liveData.player.vaginaDescript(0) + ' as you drag yourself out of your fantasy.',
              );
            else
              GUI.outputText(
                '<br>You reluctantly pry your hand from your aching ' + liveData.player.cockDescript(0) + ' as you drag yourself out of your fantasy.',
              );
          } else {
            GUI.outputText(
...
export function cowTFs(tainted: boolean, enhanced: boolean): void {
  liveData.player.slimeFeed();
  //Changes done
  let changes = 0;
  //Change limit
  ...
  if (changes < changeLimit && UTIL.rand(3) == 0) {
    ...
    if (temp <= 0) temp = 0;
      ...
      if (UTIL.rand(2) == 0)
        GUI.outputText(
          '<br><br>There is a slight pain as you feel your muscles shift somewhat.  Their appearance does not change much, but you feel much stronger.',
        );
  ...
  if (liveData.player.cocks.length > 0 && UTIL.rand(2) == 0 && !liveData.gameFlags[FLAG.HYPER_HAPPY]) {
    ...
    if (liveData.player.cocks[temp].cockLength < 2) {
      ...
      if (liveData.player.cockTotal() == 1 && !liveData.player.hasVagina()) {
        ...
        if (liveData.player.balls > 0)
          GUI.outputText(
            '  At the same time, your ' + liveData.player.ballsDescriptLight() + ' fall victim to the same sensation; eagerly swallowed whole by your crotch.',
          );
        GUI.outputText(
    ...
    if (liveData.player.cocks.length == 0 && !liveData.player.hasVagina()) {
      ...
      liveData.player.clitLength = 0.25;
      GUI.outputText(
        '<br><br>An itching starts in your crotch and spreads vertically.  You reach down and discover an opening.  You have grown a <b>new ' +
          liveData.player.vaginaDescript(0) +
          '</b>!',
      );
  ...
  if (
    ((tainted && liveData.player.biggestTitSize() <= 11) || (!tainted && liveData.player.biggestTitSize() <= 5)) &&
    changes < changeLimit &&
    (UTIL.rand(3) == 0 || enhanced)
  ) {
    if (UTIL.rand(2) == 0) GUI.outputText('<br><br>Your ' + liveData.player.breastDescript(0) + ' tingle for a moment before becoming larger.');
    else
      GUI.outputText(
        '<br><br>You feel a little weight added to your chest as your ' + liveData.player.breastDescript(0) + ' seem to inflate and settle in a larger size.',
      );
  ...
  if (
    ...
    liveData.player.breastRows[0].lactationMultiplier < 1 &&
    changes < changeLimit &&
    (UTIL.rand(3) == 0 || boobsGrew || enhanced)
  ) {
    GUI.outputText(
  ...
  if (enhanced) {
    ...
    //QUAD DAMAGE IF WEIRD SHIT BROKE BEFORE
      ...
      if (UTIL.rand(2) == 0)
        GUI.outputText(
          '<br><br>A wave of pleasure passes through your chest as your ' +
            liveData.player.breastDescript(0) +
            ' start leaking milk from a massive jump in production.',
        );
      else
        GUI.outputText(
          '<br><br>Something shifts inside your ' +
            liveData.player.breastDescript(0) +
  ...
  //If breasts are already lactating and the player is not lactating beyond a reasonable level, they start lactating more:
    ...
    if (!tainted) {
      if (
        liveData.player.breastRows[0].lactationMultiplier > 1 &&
        liveData.player.breastRows[0].lactationMultiplier < 3.2 &&
        changes < changeLimit &&
        UTIL.rand(3) == 0
      ) {
        if (UTIL.rand(2) == 0)
      ...
      if (
        ...
        if (UTIL.rand(2) == 0) GUI.outputText("<br><br>Your breasts suddenly feel less full, it seems you aren't lactating at quite the level you were.");
          ...
          GUI.outputText(
            '<br><br>The insides of your breasts suddenly feel bloated.  There is a spray of milk from them, and they settle closer to a more natural level of lactation.',
          );
        changes++;
        liveData.player.dynStats(['sen', 0.5]);
  ...
  if (((enhanced && liveData.player.tallness < 96) || liveData.player.tallness < 84) && changes < changeLimit && UTIL.rand(2) == 0) {
    ...
    if (temp == 7)
      GUI.outputText(
        '<br><br>Staggering forwards, you clutch at your head dizzily.  You spend a moment getting your balance, and stand up, feeling noticeably taller.',
      );
  ...
  if (UTIL.rand(2) == 0 && liveData.player.buttRating < 13 && changes < changeLimit) {
    if ((!tainted && liveData.player.buttRating < 8) || tainted) {
      GUI.outputText(
        '<br><br>A sensation of being unbalanced makes it difficult to walk.  You pause, paying careful attention to your new center of gravity before understanding dawns on you - your ass has grown!',
      );
  ...
  if (changes < changeLimit && UTIL.rand(3) == 0 && liveData.player.vaginaType() == 5 && liveData.player.hasVagina()) {
    GUI.outputText(
      '<br><br>Something invisible brushes against your sex, making you twinge.  Undoing your clothes, you take a look at your vagina and find that it has turned back to its natural flesh colour.',
    );
...
export function demonTFs(type: number, purified: boolean): void {
  if (type == 1) {
    //Incubi Draft
  }
  if (type == 2) {
...
export function succubiDelight(tainted: boolean): void {
  liveData.player.slimeFeed();
  let changes = 0;
  let crit = 1;
...
export function equineTFs(): void {
      ...
      if (liveData.player.cockTotal() > 0) {
        ...
        if (liveData.player.countCocksOfType(ENUM.CockType.HORSE) > 0) {
          ...
          if (liveData.player.gender == 2)
            GUI.outputText(
              'She is clearly a female, as you can see her six breasts jiggle as she walks towards you, small stains appearing on her shirt where her nipples are.<br><br>',
            );
          if (liveData.player.gender == 3)
            GUI.outputText(
              "You are somewhat confused as you can see a bulge near her thighs but also huge boobs jiggling as she walks, and you can't say if she's a male or female.<br><br>",
            );
          GUI.outputText(
  ...
  if ((liveData.player.gender == 1 || liveData.player.gender == 3) && UTIL.rand(1.5) == 0 && changes < changeLimit) {
    ...
    if (liveData.player.countCocksOfType(ENUM.CockType.HORSE) + liveData.player.countCocksOfType(ENUM.CockType.DEMON) < liveData.player.cocks.length) {
      ...
      //MULTICOCK
        ...
        GUI.outputText(
          '<br><br>One of your penises begins to feel strange. You pull down your clothes to take a look and see the skin of your ' +
            liveData.player.cockDescript(temp) +
      ...
      if (UTIL.rand(2) == 0 && changes < changeLimit) {
        ...
        if (liveData.player.breastRows.length == 1) {
          ...
          if (liveData.player.breastRows[0].breastRating > 3) {
            ...
            if (temp == 1)
              GUI.outputText(
                '<br><br>You feel a weight lifted from you, and realize your ' +
                  liveData.player.breastDescript(0) +
                  ' have shrunk to a ' +
            ...
            if (temp == 2)
              GUI.outputText(
                '<br><br>You feel significantly lighter. Looking down, you realize your breasts are MUCH smaller, down to ' +
                  liveData.player.breastCup(0) +
                  's.',
              );
            changes++;
  ...
  if (liveData.player.tailType != ENUM.TailType.HORSE && UTIL.rand(2) == 0 && changes < changeLimit) {
    ...
    if (liveData.player.tailType == 0) {
      GUI.outputText(
        '<br><br>There is a sudden tickling on your ass, and you notice you have sprouted a long shiny horsetail of the same ' +
          liveData.player.hairColor +
          ' color as your hair.',
      );
    }
...
export function felineTFs(): void {
  ...
  if (liveData.player.str < 40 && UTIL.rand(3) == 0 && changes < changeLimit) {
    ...
    if (UTIL.rand(2) == 0) GUI.outputText('<br><br>Your muscles feel taut, like a coiled spring, and a bit more on edge.');
      ...
      GUI.outputText(
        "<br><br>You arch your back as your muscles clench painfully. The cramp passes swiftly, leaving you feeling like you've gotten a bit stronger.",
      );
    liveData.player.modStats(['str', 1]);
    changes++;
    ...
    GUI.outputText(
      '<br><br>Shivers run from your head to your toes, leaving you feeling weak. Looking yourself over, your muscles seemed to have lost some bulk.',
    );
  ...
  if (liveData.player.spe < 75 && UTIL.rand(3) == 0 && changes < changeLimit) {
    ...
    //medium speed
      ...
      GUI.outputText(
        "<br><br>You stumble as you shift position, surprised by how quickly you move. After a moment or two of disorientation, you adjust. You're certain that you can run faster now.",
      );
    ...
    //high speed
      ...
      GUI.outputText(
        '<br><br>You pause mid-step and crouch. Your leg muscles have cramped up like crazy. After a few moments, the pain passes and you feel like you could chase anything down.',
      );
  ...
  if (UTIL.rand(4) == 0 && changes < changeLimit) {
    //Heat
    const intensified = liveData.player.inHeat;
    if (liveData.player.goIntoHeat(false)) {
      if (intensified) {
        if (UTIL.rand(2) == 0)
          GUI.outputText(
            '<br><br>The itch inside your ' +
              liveData.player.vaginaDescript(0) +
      ...
      for (let k = 0; k < liveData.player.breastRows.length; k++) {
        ...
        if (liveData.player.breastRows[k].breastRating > temp2) {
          ...
          //Small change
            ...
            if (temp3 == 0)
              GUI.outputText(
                '<br><br>All at once, your sense of gravity shifts. Your back feels a sense of relief, and it takes you a moment to realize your ' +
                  liveData.player.breastDescript(k) +
                  ' have shrunk!',
              );
            else
              GUI.outputText(
  ...
  if (liveData.player.countCocksOfType(ENUM.CockType.CAT) > 0 && UTIL.rand(3) == 0 && changes < changeLimit && !liveData.hyperHappy) {
    ...
    if (temp == 1) {
      ...
      if (liveData.player.cocks[j].cockLength > 16) {
        GUI.outputText(
          '<br><br>Your ' +
            liveData.player.cockDescript(j) +
  ...
  if (liveData.player.earType != ENUM.EarType.Cat && UTIL.rand(5) == 0 && changes < changeLimit) {
    ...
    //non human to cat:
      ...
      if (UTIL.rand(2) == 0)
        GUI.outputText(
          '<br><br>Your ears change shape, morphing into pointed, feline ears! They swivel about reflexively as you adjust to them. <b>You now have cat ears.</b>',
        );
      else
        GUI.outputText(
  ...
  if (liveData.player.tailType != ENUM.TailType.CAT && liveData.player.earType == ENUM.EarType.Cat && UTIL.rand(5) == 0 && changes < changeLimit) {
    if (liveData.player.tailType == ENUM.TailType.NONE) {
      ...
      if (temp == 1)
        GUI.outputText(
          '<br><br>You feel your backside shift and change, flesh molding and displacing into a long, flexible tail! <b>You now have a cat tail.</b>',
        );
      if (temp == 2)
        GUI.outputText(
  ...
  if (
    ...
    //non hoof to cat:
      ...
      GUI.outputText(
        '<br><br>You scream in agony as you feel the bones in your ' +
          liveData.player.feet() +
...
export function humanTFs(): void {
  ...
  GUI.clearOutput();
  GUI.outputText(
    'You crack open the small clay jar to reveal a lightly colored paste that smells surprisingly delicious. You begin eating it with your fingers, wishing all the while for some crackers... ',
  );
  liveData.player.slimeFeed();
  liveData.player.refillHunger(20);
  if (liveData.player.humanScore() > 4) {
    GUI.outputText(
      "<br><br>You blink and the world twists around you. You feel more like yourself than you have in a while, but exactly how isn't immediately apparent. Maybe you should take a look at yourself?",
    );
  } else {
    GUI.outputText(
  ...
  if (
    ...
    GUI.outputText(
      '<br><br>A slowly-building itch spreads over your whole body, and as you idly scratch yourself, you find that your ' +
  ...
  if (UTIL.rand(2) == 0 && changes < changeLimit && liveData.player.tallness > 84) {
    GUI.outputText(
      "<br><br>Your skin crawls, making you close your eyes and shiver. When you open them again the world seems... different. After a bit of investigation, you realize you've become shorter!<br>",
    );
  ...
  if (liveData.player.balls > 2 && UTIL.rand(3) == 0 && changes < changeLimit) {
    if (liveData.player.ballSize > 2) {
      ...
      liveData.player.ballSize -= 1;
      GUI.outputText(
        '<br><br>Your scrotum slowly shrinks, settling down at a smaller size. <b>Your ' +
          liveData.player.ballsDescriptLight() +
          ' are smaller now.</b><br><br>',
      );
    } else {
      liveData.player.balls = 2;
      GUI.outputText(
        '<br><br>Your scrotum slowly shrinks until they seem to have reached a normal size. <b>You can feel as if your extra balls fused together, leaving you with a pair of balls.</b><br><br>',
      );
    }
    changes++;
  }
  ...
  if (liveData.player.hasCock() && liveData.player.biggestCockLength() > 12 && UTIL.rand(3) == 0 && changes < changeLimit) {
    const idx = liveData.player.biggestCockIndex();
  ...
  if (liveData.player.averageNipplesPerBreast() > 1 && UTIL.rand(4) == 0 && changes < changeLimit) {
    GUI.outputText(
      '<br><br>A tightness arises in your nipples as three out of four on each breast recede completely, the leftover nipples migrating to the middle of your breasts. <b>You are left with only one nipple on each breast.</b>',
    );
  ...
  if (liveData.player.hasVagina() && UTIL.rand(3) == 0 && changes < changeLimit) {
    GUI.outputText(
      '<br><br>The vague numbness in your skin sinks slowly downwards, and you put a hand on your lower stomach as the sensation centers itself there. ',
    );
...
export function impTFs(): void {
  ...
  if (changes < changeLimit && liveData.player.hasCock() && liveData.player.cocks[0].cockLength < 12) {
    const temp = liveData.player.increaseCock(0, UTIL.rand(2) + 2);
    GUI.outputText('<br><br>');
...
export function pigTFs(boar: boolean): void {
  let changes = 0;
  let changeLimit = 1;
  const temp = 0;
  const x = 0;
  ...
  if (UTIL.rand(3) == 0 && changes < changeLimit && liveData.player.gender >= 2 && liveData.player.hipRating < 15) {
    GUI.outputText(
      '<br><br>Your gait shifts slightly to accommodate your widening ' +
  ...
  if (UTIL.rand(3) == 0 && changes < changeLimit && liveData.player.balls > 0 && liveData.player.ballSize < 4) {
    if (liveData.player.ballSize < 3)
      ...
      GUI.outputText(
        '<br><br>A sudden onset of heat envelops your groin, focusing on your ballsack. Walking becomes difficult as you discover your ' +
          (liveData.player.balls == 4 ? 'quartette' : 'duo') +
          ' of testicles have enlarged again.',
      );
    liveData.player.ballSize++;
    changes++;
  }
  ...
  if (UTIL.rand(boar ? 2 : 3) == 0 && changes < changeLimit && liveData.player.earType == ENUM.EarType.Pig && liveData.player.tailType != ENUM.TailType.PIG) {
    if (liveData.player.tailType > 0)
      ...
      GUI.outputText(
        '<br><br>You feel a pinching sensation in your [tail] as it begins to warp in change. When the sensation dissipates, <b>you are left with a small, curly pig tail.</b>',
      );
  ...
  if (UTIL.rand(boar ? 3 : 4) == 0 && changes < changeLimit) {
    ...
    GUI.outputText(
      "<br><br>Your skin tingles ever so slightly as you skin's color changes before your eyes. As the tingling diminishes, you find that your skin has turned " +
        skinToBeChosen +
        '.',
    );
...
export function lizardTFs(): void {
  ...
  if (liveData.player.lib < 100 && changes < changeLimit && UTIL.rand(3) == 0) {
    GUI.outputText(
      '<br><br>A knot of fire in your gut doubles you over but passes after a few moments.  As you straighten you can feel the heat seeping into you, ',
    );
    //(DICK)
  ...
  if (liveData.player.tou < 70 && changes < changeLimit && UTIL.rand(3) == 0) {
    ...
    if (liveData.player.tou < 40) {
      GUI.outputText(
        '<br><br>Your body and skin both thicken noticeably.  You pinch your ' +
          liveData.player.skinDesc +
    ...
    //(+2)
      ...
      GUI.outputText(
        '<br><br>You grin as you feel your form getting a little more solid.  It seems like your whole body is toughening up quite nicely, and by the time the sensation goes away, you feel ready to take a hit.',
      );
    ...
    //(+1)
      ...
      GUI.outputText(
        "<br><br>You snarl happily as you feel yourself getting even tougher.  It's a barely discernible difference, but you can feel your " +
          liveData.player.skinDesc +
  ...
  if (
    ...
    liveData.player.countCocksOfType(ENUM.CockType.LIZARD) > 0 &&
    liveData.player.cockTotal() > liveData.player.countCocksOfType(ENUM.CockType.LIZARD) &&
    UTIL.rand(4) == 0 &&
    changes < changeLimit
  ) {
    GUI.outputText(
      '<br><br>A familiar tingle starts in your crotch, and before you can miss the show, you pull open your ' +
        liveData.player.armor.equipmentName +
  ...
  if (
    ...
    if (liveData.player.balls > 1)
      GUI.outputText(
        '  The remaining ' +
          UTIL.num2Text(liveData.player.balls - 1) +
  ...
  if (liveData.player.biggestTitSize() >= 1 && liveData.player.gender == 1 && changes < changeLimit && UTIL.rand(3) == 0) {
    ...
    //(NOT HUEG < 4)
      ...
      GUI.outputText(
        "<br><br>In an instant, your chest compacts in on itself, consuming every ounce of breast-flesh.  You're left with a  smooth, masculine torso, though your nipples remain.",
      );
    //(BOTH – no new PG)
      ...
      if (liveData.player.hornType == ENUM.HornType.DRACONIC_X2) {
        if (liveData.player.horns < 12) {
          if (UTIL.rand(2) == 0) {
            ...
            GUI.outputText(
              '<br><br>Your head aches as your horns grow a few inches longer.  They get even thicker about the base, giving you a menacing appearance.',
            );
  ...
  if (liveData.gameFlags[FLAG.HAIR_GROWTH_STOPPED_BECAUSE_LIZARD] == 0 && changes < changeLimit && UTIL.rand(4) == 0) {
    GUI.outputText(
      '<br><br>Your scalp tingles oddly.  In a panic, you reach up to your ' +
        liveData.player.hairDescript() +
        ', but thankfully it appears unchanged.<br><br>',
    );
  ...
  if (liveData.player.lowerBody != ENUM.LowerBodyType.LIZARD && changes < changeLimit && UTIL.rand(5) == 0) {
    ...
    //TAURS -
      ...
      GUI.outputText(
        "<br><br>Your lower body is wracked by pain!  Once it passes, you discover that you're standing on digitigrade legs with lizard-like claws.",
      );
    //feet types -
    else if (
      liveData.player.lowerBody == ENUM.LowerBodyType.HUMAN ||
...
export function harpyTFs(type: number): void {
  ...
  if (liveData.player.spe < 100 && UTIL.rand(3) == 0) {
    ...
    if (liveData.player.spe >= 75)
      GUI.outputText(
        '<br><br>A familiar chill runs down your spine. Your muscles feel like well oiled machinery, ready to snap into action with lightning speed.',
      );
  ...
  if (liveData.player.tou > 50 && UTIL.rand(3) == 0 && changes < changeLimit) {
    ...
    if (UTIL.rand(2) == 0)
      GUI.outputText(
        '<br><br>A nice, slow warmth rolls from your gut out to your limbs, flowing through them before dissipating entirely. As it leaves, you note that your body feels softer and less resilient.',
      );
    else
  ...
  if (liveData.player.str < 70 && UTIL.rand(3) == 0 && changes < changeLimit) {
    ...
    if (liveData.player.str < 40)
      GUI.outputText(
        '<br><br>Shivering, you feel a feverish sensation that reminds you of the last time you got sick. Thankfully, it passes swiftly, leaving slightly enhanced strength in its wake.',
      );
    //(hi str – 50+)
  ...
  if ((liveData.player.lib < 90 || UTIL.rand(3) == 0) && UTIL.rand(3) == 0 && changes < changeLimit) {
    ...
    if (liveData.player.lib < 40) {
      GUI.outputText(
        '<br><br>A passing flush colors your ' +
          liveData.player.face() +
    ...
    //(sub 75 lib)
      ...
      GUI.outputText(
        '<br><br>Heat, blessed heat, works through you from head to groin, leaving you to shudder and fantasize about the sex you could be having right now.<br><br>',
      );
    //(hi lib)
    else if (liveData.player.lib < 90)
      GUI.outputText(
        '<br><br>Sexual need courses through you, flushing your skin with a reddish hue while you pant and daydream of the wondrous sex you should be having right now.<br><br>',
      );
    //(90+)
    else
      GUI.outputText(
        '<br><br>You groan, something about the seed rubbing your libido in just the right way to make you horny. Panting heavily, you sigh and fantasize about the sex you could be having.<br><br>',
      );
    //(fork to fantasy)
    if (liveData.player.lib >= 40) {
  ...
  //Cannot happen at same time as row removal
    ...
    changes < changeLimit &&
    liveData.player.breastRows.length == 1 &&
    UTIL.rand(3) == 0 &&
    liveData.player.breastRows[0].breastRating >= 7 &&
    !liveData.gameFlags[FLAG.HYPER_HAPPY]
  ) {
    changes++;
  ...
  if (
    ...
    liveData.player.countCocksOfType(ENUM.CockType.AVIAN) < liveData.player.cockTotal() &&
    UTIL.rand(type == 1 ? 4 : 10) == 0
  ) {
  ...
  if (
    ...
    GUI.outputText(liveData.player.skinTone + ' colored.</b>');
    updateClaws(liveData.player.clawType);
  }
  ...
  if (liveData.player.hipRating >= 15 && changes < changeLimit && UTIL.rand(3) == 0) {
    GUI.outputText(
      '<br><br>Your gait shifts inward, your ' +
        liveData.player.hipDescript() +
...
export function minotaurTFs(): void {
  ...
  if (UTIL.rand(3) == 0 && changes < changeLimit) {
    ...
    if (liveData.player.str <= 50) {
      GUI.outputText(
        '<br><br>Painful aches ripple through your body, flooding you with pain as your muscles flex and bulge, growing much stronger and more well-defined.',
      );
  ...
  if (!liveData.gameFlags[FLAG.HYPER_HAPPY]) {
    ...
    if (liveData.player.vaginas.length > 0) {
      if (liveData.player.vaginas[0].vaginalLooseness > ENUM.VaginalLoosenessType.TIGHT) {
        ...
        GUI.outputText(
          '<br><br>A tightness in your groin is the only warning you get before your <b>' + liveData.player.vaginaDescript(0) + ' disappears forever</b>!',
        );
        //Goodbye womanhood!
  ...
  if (changes < changeLimit && UTIL.rand(2) == 0 && liveData.player.cocks.length > 0) {
    ...
    for (let i = 0; i < liveData.player.cocks.length; i++) {
      if (
        liveData.player.cocks[i].cockType == ENUM.CockType.HORSE &&
        (liveData.player.cocks[i].cockLength < 36 || liveData.player.cocks[i].cockThickness < 5)
      ) {
        selectedCock = i;
        break;
      }
    ...
    if (selectedCock != -1) {
      ...
      if (liveData.player.cocks[selectedCock].cockThickness < 5) {
        ...
        if (temp > 6)
          GUI.outputText(
            '<br><br>Gasping in sudden pleasure, your ' +
              liveData.player.cockDescript(selectedCock) +
              ' surges free of its sheath, emerging with over half a foot of new dick-flesh.',
          );
        if (temp <= 6 && temp >= 3)
          GUI.outputText(
            '<br><br>You pant in delight as a few inches of ' +
              liveData.player.cockDescript(selectedCock) +
    ...
    if (liveData.player.hornType == ENUM.HornType.COW_MINOTAUR || liveData.player.hornType == ENUM.HornType.NONE) {
      ...
      if (liveData.player.hornType == ENUM.HornType.COW_MINOTAUR) {
        ...
        //Males horns get 'uge.
          ...
          if (temp == 2)
            GUI.outputText(
              '<br><br>A powerful headache momentarily doubles you over.  With painful slowness, you feel your horns push another two inches of length out from your brow, gradually thickening as they grow.  ',
            );
          if (temp == 3)
            GUI.outputText(
...
export function snakeTFs(): void {
  ...
  if (
    ...
    GUI.outputText(
      '<br><br>You find it increasingly harder to keep standing as your legs start feeling weak.  You swiftly collapse, unable to maintain your own weight.',
    );
    ...
    //FUCK NO
    GUI.outputText(
      '<br><br>Trying to get back up, you realize that the skin on the inner sides of your thighs is merging together like it was being sewn by an invisible needle.',
    );
    GUI.outputText(
    ...
    GUI.outputText(
      '<br><br>Wondering what happened to your sex, you pass your hand down the front of your body until you find a large, horizontal slit around your pelvic area, which contains all of your sexual organs.',
    );
...
export function slimeTFs(): void {
  ...
  if (liveData.player.lib < 80) {
    ...
    GUI.outputText(
      '<br><br>Blushing and feeling horny, you make sure to rub it over your chest and erect nipples, letting the strange slimy fluid soak into you.',
    );
  }
  ...
  if (liveData.player.vaginalCapacity() < 9000) {
    ...
    GUI.outputText(
      "<br><br>Your gel-like body swells up from the intake of additional slime.  If you had to guess, you'd bet you were about two inches taller.",
    );
...
export function trapOil(): void {
  ...
  if (liveData.player.ass.analWetness < 5 && UTIL.rand(4) == 0 && changes < changeLimit) {
    if (liveData.player.ass.analWetness < 4)
      GUI.outputText(
        '<br><br>Your eyes widen in shock as you feel oily moisture bead out of your [asshole].  Your asshole has become wetter and more pliable.',
      );
  ...
  if (liveData.gameFlags[FLAG.HAS_BLACK_NIPPLES] == 0 && UTIL.rand(6) == 0 && changes < changeLimit) {
    GUI.outputText(
      '<br><br>A tickling sensation plucks at your nipples and you cringe, trying not to giggle.  Looking down you are in time to see the last spot of flesh tone disappear from your [nipples].  They have turned an onyx black!',
    );
...
export function restoreArms(tfSource = '') {
  //Set up changes and changeLimit
  let changes = 0;
  const changeLimit = 1;
