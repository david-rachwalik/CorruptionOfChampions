...
export function mainMenu(): void {
  ...
  GUI.hideUpDown();
  GUI.hideMenus();
  GUI.setMenuButton('buttonMain', 'New Game', Intro.initializeNewGame);
  ...
  GUI.addButton(1, 'Settings', settingsScreenMain);
  GUI.addButton(2, 'Credits', creditsScreen);
}
//---------
...
export function errorOldBrowser() {
  GUI.clearOutput();
...
export function errorOldInternetExplorerEwwww() {
  GUI.clearOutput();
  GUI.outputText(
    '<font size="64"><b>:(</b></font><br>Please don\'t use Internet Explorer, especially the old version. It\'s a shitty browser. Do me a favour and upgrade your browser or switch to Firefox or Chrome.',
  );
  GUI.menu();
  GUI.hideMenus();
}
export function errorEdgeNeverWorks() {
  GUI.clearOutput();
  GUI.outputText(
...
export function settingsScreenMain() {
  ...
  //Silly Mode
  if (liveData.silly)
    GUI.outputText('Silly Mode: <b><font color="#008000">ON</font></b><br>&nbsp; Crazy, nonsensical, and possibly hilarious things may occur.<br><br>');
  else
    GUI.outputText('Silly Mode: <b><font color="#800000">OFF</font></b><br>&nbsp; You\'re an incorrigable stick-in-the-mud with no sense of humour.<br><br>');
  //Time Format
  ...
  //Set buttons
  GUI.menu();
  GUI.addButton(0, 'Silly Mode', toggleSilly);
  GUI.addButton(1, 'Time Format', toggleTimeFormat);
  GUI.addButton(4, 'Font', fontSettings);
  GUI.addButton(14, 'Back', mainMenu);
}
//Game settings
export function toggleSilly() {
...
export function fontSettings() {
  ...
  GUI.menu();
  GUI.addButton(0, 'Main Font', changeMainFont);
  if (liveData.mainFontSizeIndex < 8) GUI.addButton(5, 'Main Size+', changeMainFontSize, 'bigger');
  if (liveData.mainFontSizeIndex > 0) GUI.addButton(10, 'Main Size-', changeMainFontSize, 'smaller');

  GUI.addButton(14, 'Back', settingsScreenMain);
}
export function changeMainFont() {
...
export function changeMainFontSize(biggerSmaller: string): void {
...
export function creditsScreen(): void {
  ...
  creditsContents +=
    '<li> Abraxas (Alternate Naga Scenes w/Various Monsters, Tamani Anal, Female Shouldra Tongue Licking, Chameleon Girl, Christmas Harpy)</li>';
  creditsContents += '<li> Astronomy (Fetish Cultist Centaur Footjob Scene)</li>';
  creditsContents +=
    '<li> Adjatha (Scylla the Cum Addicted Nun, Vala, Goo-girls, Bimbo Sophie Eggs, Ceraph Urta Roleplay, Gnoll with Balls Scene, Kiha futa scene, Goblin Web Fuck Scene, and 69 Bunny Scene)</li>';
  creditsContents += '<li> ComfyCushion (Muff Wrangler)</li>';
  creditsContents += '<li> B (Brooke)</li>';
  creditsContents +=
    "<li> Quiet Browser (Half of Niamh, Ember, Amily The Mouse-girl Breeder, Katherine, Part of Katherine Employment Expansion, Urta's in-bar Dialogue Trees, some of Izma, Loppe)</li>";
  creditsContents +=
    '<li> Indirect (Alternate Non-Scylla Katherine Recruitment, Part of Katherine Employment Expansion, Phouka, Coding of Bee Girl Expansion)</li>';
  ...
  creditsContents +=
    '<li> Xodin (Nipplefucking paragraph of Imp GangBang, Encumbered by Big Genitals Exploration Scene, Big Bits Run Encumbrance, Player Getting Beer Tits, Sand Witch Dungeon Misc Scenes)</li>';
  ...
  creditsContents +=
    "<li> The Dark Master (Marble, Fetish Cultist, Fetish Zealot, Hellhound, Lumi, Some Cat Transformations, LaBova, Ceraph's Cat-Slaves, a Cum Witch Scene, Mouse Dreams, Forced Nursing:Imps&Goblins, Bee Girl Expansion)</li>";
