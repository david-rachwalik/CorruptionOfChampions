...
// Victory Menu
export function tamaniVictoryMenu() {
  ...
  if (liveData.player.lust >= 33 && liveData.player.totalCocks() > 0 && liveData.gameFlags[FLAG.SFW_MODE] <= 0) {
    ...
    GUI.menu();
    GUI.addButton(0, 'Fuck', tamaniSexWon);
    if (liveData.player.cockThatFits(liveData.monster.analCapacity()) >= 0) GUI.addButton(1, 'Buttfuck', tamaniAnalSex);
    else
      GUI.addButtonDisabled(
        1,
        'Buttfuck',
        liveData.player.cockTotal() == 1 ? "Your cock is too big to fit in Tamani's ass." : "None of your cocks will fit in Tamani's ass.",
      );
    if (!liveData.tamanipreg.isPregnant() && liveData.player.canOvipositSpider()) GUI.addButton(2, 'Lay Eggs', tamaniBeaten);
    if (liveData.gameFlags[FLAG.TAMANI_DEFEAT_COUNTER] >= 4 && liveData.monster.HP <= 0) GUI.addButton(3, 'NO MORE!', killTamaniChoice);
    ...
    if (liveData.gameFlags[FLAG.TAMANI_DEFEAT_COUNTER] >= 4 && liveData.monster.HP <= 0) {
      ...
      GUI.addButton(3, 'NO MORE!', killTamaniChoice);
      GUI.addButton(4, 'Leave', COMBAT.cleanupAfterCombat);
    } else {
...
//Fuck Tamani
export function tamaniSexWon() {
  //spriteSelect(56); TODO Sprite
  tamaniKnockUp();
  let x = liveData.player.cockThatFits(90);
  ...
  if (liveData.player.cockArea(x) <= 90) {
    ...
    GUI.outputText(
      '<br><br>Tamani winks, sucking on her finger as she staggers up.  She coos, "<i>Mmmm, Tamani loves it when her ' +
        liveData.player.mf('sexy lover', 'stud') +
    ...
    GUI.outputText(
      'She doesn\'t give you a chance to answer as she wobbles off, jiggling pleasantly in all the right places, "<i>Of course you do.  I\'ll be back for the rest later!</i>"<br><br>',
    );
    if (COMBAT.inCombat()) {
      COMBAT.cleanupAfterCombat();
  ...
  //Too big? Jerk off with feet and bukkake
    ...
    GUI.outputText(
      'She doesn\'t give you a chance to answer as she staggers up and strikes a sexy pose, "<i>Of course you do.  I\'ll be back for the rest later!</i>"<br><br>',
    );
...
export function killTamaniChoice() {
  GUI.clearOutput();
  GUI.outputText(
    'Tamani has really gotten to your nerves and you tire of her forcing herself upon you. You kick Tamani once and she writhes in pain. "<i>Guess you don\'t want me anymore?</i>" Tamani sniffles.',
  );
  GUI.outputText('');
  GUI.menu();
  if (liveData.silly) {
    GUI.addButton(0, 'FIGHT', killTamani);
    GUI.addButton(1, 'MERCY', spareTamani);
  } else {
    GUI.addButton(0, 'Kill Her', killTamani);
    GUI.addButton(1, 'Spare Her', spareTamani);
  }
}

//Choose to kill Tamani - no difference in Silly or Normal mode
export function killTamani() {
  GUI.clearOutput();
  GUI.outputText(
    'You step over and grab her head. "<i>NO!</i>" Tamani cries out. You tell her that she shouldn\'t have forced herself upon you. You snap her neck and set her lifeless body on the ground.',
  );
...
//LOSE
export function tamaniLoss() {
  if (liveData.player.HP <= 0) {
    if (liveData.player.totalCocks() > 0) {
      if (UTIL.rand(2) == 0) tamaniSexLost();
    ...
    if (liveData.player.totalCocks() > 0) {
      ...
      if (liveData.gameFlags[FLAG.TAMANI_TIMES_HYPNOTIZED] > 19 && UTIL.rand(2) == 0) {
        tamaniHypnosis();
      } else if (UTIL.rand(2) == 0) tamaniSexLost();
      else tamaniSexLetHer();
    } else {
      GUI.outputText(
...
export function tamaniSexLost() {
  ...
  GUI.outputText(
    "\"<i>You were such a naughty boy, teasing me with that juicy dick and then pretending like you didn't want   I'll have to punish you for being bad, that's all there is to it,</i>\" she explains, pushing your hands away and fitting the ring around your flare.  ",
  );
  if (liveData.player.cocks[0].cockThickness >= 5)
    ...
    GUI.outputText(
      'Somehow the ring fits onto you perfectly, as if it was made exactly for your ' +
        liveData.player.cockDescript(0) +
        ', though it remains painfully tight.<br><br>',
    );
  GUI.outputText(
    '"<i>Don\'t fret, ' +
...
export function tamaniSexLetHer() {
  ...
  if (COMBAT.inCombat()) {
    ...
    //Not taur
      ...
      GUI.outputText('  She wiggles her ' + tamaniChest() + ', and despite your efforts to control your body, ');
  ...
  //Not-taurs
    ...
    if (liveData.player.cockArea(0) <= 90) {
      ...
      if (liveData.player.balls > 0)
        GUI.outputText(
          'Your ' + liveData.player.ballsDescriptLight() + ' twitch powerfully, venting their contents, releasing a jet of hot cum into the goblin.',
        );
    ...
    //(Doesnt fit)
      ...
      GUI.outputText(
        '  A burning heat radiates through your body, making your ' + liveData.player.nippleDescript(0) + ' tingle and grow ever more sensitive.<br><br>',
      );
      GUI.outputText(
      ...
      GUI.outputText(
        'The dildo slips out of your abused ' + liveData.player.assholeOrPussy() + ' forgotten in the heat of your lovemaking, leaking pink goo.<br><br>',
      );
      GUI.outputText(
...
export function tamaniHypnosis() {
  //TODO TAMANI SPRITE spriteSelect(56);
  GUI.clearOutput();
  //Find a dick that fits
  const primary = liveData.player.cockThatFits(65);
  let secondary = 0;
  const cocks = liveData.player.totalCocks();
  ...
  if (primary >= 0) {
    ...
    if (cocks > 1) {
      ...
      GUI.outputText(
        'She moans and pulls your ' +
          liveData.player.cockDescript(secondary) +
    ...
    GUI.outputText(
      '  She visibly orgasms, shaking and trembling from head to toe as her pussy milks your ' +
        ...
        '.  It only lasts a few seconds, though they feel like heaven, before her body goes nerveless.<br><br>',
    );
...
export function encounterTamani() {
  if (liveData.player.totalCocks() <= 0) {
...
export function tamaniFemaleEncounter() {
  ...
  //[Umm OK?] [No]
  GUI.menu();
  GUI.addButton(0, 'Yes', tamaniFemaleYes);
  GUI.addButton(1, 'No', tamaniFemaleNo);
  GUI.addButton(2, 'PreferGirls', preferTamaniFemdom);
}

// Female Agree with Tamani, Obtain Deluxe Dildo
export function tamaniFemaleYes() {
  //spriteSelect(56); TODO Tamani Sprite
  GUI.clearOutput();
  GUI.outputText(
    '"<i>That\'s what I thought,</i>" says the goblin as she fishes around in her pouches, "<i>but I\'m not cruel, I\'ll give you my best dildo so you can keep your hot little box stuffed all the time.</i>"<br><br>',
  );
...
export function tamaniFemaleNo() {
  //spriteSelect(56); TODO Tamani Sprite
  GUI.clearOutput();
  GUI.outputText(
    "The goblin harrumphs, \"<i>We'll see about that slut.  I'll be knocked up from every monster and stud this side of the mountain.  Hell, just grow one dick, and see how fast Tamani's legs get wrapped around you!</i>\"<br><br>",
  );
...
export function preferTamaniFemdom() {
  GUI.clearOutput();
  //Tamani Facesit
  //===========Tamani============
  ...
  GUI.outputText(
    '<br><br>Tamani advances closer, staring you straight in the eye with an impish smirk.  "<i>I\'ll let you get me off if you swear to stay away from </i>my<i> cocks. Deal?</i>"',
  );
  GUI.menu();
  GUI.addButton(0, 'Accept', acceptTamaniFacesits);
  GUI.addButton(1, 'Refuse', declineTamaniFacesits);
}

// Decline Femdom
export function declineTamaniFacesits() {
  GUI.clearOutput();
...
// Accept Femdom
export function acceptTamaniFacesits() {
  GUI.clearOutput();
...
export function tamaniMaleFirstEncounter() {
  //spriteSelect(56); TODO Tamani Sprite
  ...
  //[Fuck Her] [Refuse]
  GUI.menu();
  GUI.addButton(0, 'Fuck Her', tamaniFirstTimeConsentual);
  GUI.addButton(1, 'Refuse', tamaniFirstTimeRefusal);
}

//Male - Accept Sex First Time
export function tamaniFirstTimeConsentual() {
  //spriteSelect(56); TODO Tamani Sprite
  tamaniKnockUp();
  GUI.clearOutput();
  GUI.outputText(
  ...
  GUI.outputText(
    '.  She wraps her tiny arms around your neck and kisses you passionately, letting her tongue slither through your lips.   The two of you french kiss hard, virtually tongue-fucking each other.<br><br>',
  );
  GUI.outputText(
  ...
  if (liveData.player.cockArea(0) <= 90) {
    ...
    GUI.outputText(
      '.  She stops and teases, "<i>Ready to stuff me full of your cream?  I just KNOW I\'ll get pregnant from such a purrfect mate.</i>"<br><br>',
    );
    GUI.outputText(
...
export function tamaniFirstTimeRefusal() {
  //TODO Tamani Sprite spriteSelect(56);
  GUI.clearOutput();
  GUI.outputText(
  ...
  GUI.addButton(0, 'Fuck Her', tamaniFirstTimeConsentual);
  GUI.addButton(1, 'Refuse', tamaniSecondRefusal);
}

//Male - Refuse Sex First Time - Second Refusal
export function tamaniSecondRefusal() {
  //spriteSelect(56); TODO Tamani Sprite
  GUI.clearOutput();
  GUI.outputText(
    'The goblin pouts, anger clouding her cute little features.  She turns and storms off, clearly pissed at you, "<i>Think about it.  Next time that dick better ache for me, or I\'ll MAKE you want it.</i>"<br><br>',
  );
  GUI.outputText('...What?');
  GUI.doNext(Camp.returnToCampUseOneHour);
}
...
export function tamaniMaleRepeatEncounter() {
  //spriteSelect(56); TODO SPRITE
  GUI.clearOutput();
  ...
  //[No – starts fight]
  GUI.menu();
  GUI.addButton(0, 'Take Her', tamaniSexWon);
  GUI.addButton(1, 'Let Her', tamaniSexLetHer);
  GUI.addButton(2, 'No', tamaniStartFight);
}

// Starts Tamani Fight
export function tamaniStartFight() {
  GUI.clearOutput();
...
export function tamaniPregnantEncounter() {
  //TODO SPRITE spriteSelect(56);
  GUI.clearOutput();
  GUI.outputText(
  ...
  //[Fuck Her] [Refuse]
  GUI.addButton(0, 'Fuck Her', tamaniPregnantFuck);
  GUI.addButton(1, 'Refuse', tamaniPregnantRefusal);
}

export function tamaniPregnantFuck() {
  //TODO Sprite spriteSelect(56);
  GUI.clearOutput();
  GUI.outputText(
...
export function tamaniPregnantRefusal() {
  //TODO SPrite spriteSelect(56);
  GUI.clearOutput();
...
export function tamaniBirthScene() {
