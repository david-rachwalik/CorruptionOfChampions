...
export function ottahGirl() {
  ...
  if (liveData.gameFlags[FLAG.MET_OTTERGIRL] == 1) {
    ...
    GUI.outputText(
      '<br><br>"<i>Fish\'n\'fuck,</i>" she replies, simply. "<i>I fish, you fuck. Ya ain\'t dense, are you ' + liveData.player.mf('boy', 'girl') + '?</i>"',
    );
    ...
    GUI.outputText(
      "<br><br>\"<i>Name's Callu. Don't worry darlin', I don't plan on stickin' nothin' where it don't belong,</i>\" her soft voice chimes, \"<i>Unlike damn near everything else around here.</i>\"",
    );
  ...
  if (liveData.player.lust < 33) GUI.outputText("<br><br>You aren't aroused enough to fuck her.");
    ...
    if (liveData.player.hasCock()) {
      ...
      else GUI.addButton(0, 'Fuck Her', ottergirlLikesDongs);
    }
    if (liveData.player.hasVagina() || !liveData.player.hasCock()) GUI.addButton(1, 'Facesitting', ottersForGals);
  }
  if (liveData.gameFlags[FLAG.MET_OTTERGIRL] > 1) GUI.addButton(2, 'Get Fish', getSomeFishYaFatty);
  GUI.addButton(4, 'Leave', avoidZeOtterPussy);
}

//For Dicks
export function ottergirlLikesDongs() {
  GUI.clearOutput();
  ...
  //(10"-24")
    ...
    GUI.outputText(
      '<br><br>"<i>Oh my, now that\'s a manly piece of meat right there,</i>" she remarks, inspecting your oversized cock. "<i>I could enjoy that bad boy all day.</i>"',
    );
  //(24"-48")
  else
    GUI.outputText(
  ...
  GUI.outputText(
    '<br><br>Likewise, her tongue and lips dance over your ' +
  ...
  GUI.outputText(
    "<br><br>Your body arches, thrusting your cock fully inside Callu, your hips meeting with a lewd, wet smack. Your cock jerks, spurting jet after jet of seed into the otter-girl's greedy cunt.",
  );
...
export function ottersForGals() {
  ...
  GUI.outputText(
    '<br><br>Likewise, her tongue and lips dance across your flesh like a trio of dancers. They twirl, spin, hop and tease. Not one inch is left untouched. From your ',
  );
  ...
  GUI.outputText(
    '<br><br>Grabbing hold of your [nipples], you start playing with them while Callu does her thing. Your fingers deftly tweak and tease them, knowing all the right techniques to really get you going. ',
  );
...
//For Pansies
export function avoidZeOtterPussy() {
  GUI.clearOutput();
...
//For Fatties
export function getSomeFishYaFatty() {
  GUI.clearOutput();
  GUI.outputText(
