...
export class GooGirl extends Creature {
  constructor() {
    super();
    ...
    this.battleDesc =
      ...
      (liveData.player.biggestTitSize() >= 12
        ? '  She has apparently made herself a bit more like you, as her chest appears to be a perfect copy of your ' + liveData.player.chestDesc() + '.'
        : '');
  ...
  gooGalAttack() {
    ...
    if (damage > 0) {
      if (
        liveData.monster.lustVuln > 0 &&
        (liveData.player.armor.equipmentName == 'barely-decent bondage straps' || liveData.player.armor.equipmentName == 'naked')
      ) {
  ...
  override outputDefaultTeaseReaction(lustDelta: number): void {
    if (liveData.monster.lust <= 99) {
      if (lustDelta <= 0) GUI.outputText("<br><br>The goo-girl looks confused by your actions, as if she's trying to understand what you're doing.");
        ...
        GUI.outputText(
          '<br>The girl begins to understand your intent. She opens and closes her mouth, as if panting, while she works slimy fingers between her thighs and across her jiggling nipples.',
        );
    } else
      GUI.outputText(
        '<br>It appears the goo-girl has gotten lost in her mimicry, squeezing her breasts and jilling her shiny ' +
          liveData.monster.skinTone +
...
export function encounterGooGirl() {
  GUI.clearOutput();
  GUI.displaySprite('googirl');
  GUI.outputText(
    'As you walk around the lake, you notice a pale red light pulsing in the ' + (liveData.gameFlags[FLAG.FACTORY_SHUTDOWN] == 2 ? 'murky' : 'sapphire'),
  );
  GUI.outputText(
...
export function spyOnGooAndOozeSex() {
...
export function gooGirlWin() {
  beatUpGoo();
}

export function beatUpGoo() {
  ...
  // Else choose from these options
    ...
    if (liveData.player.hasCock()) {
      if (liveData.player.cocks[liveData.player.smallestCockIndex()].cockLength < 24) {
        GUI.addButton(0, 'DickSex', gooMaleRape, 2);
      }
      if (liveData.player.longestCockLength() >= 24) {
        GUI.addButton(1, 'BigDickSex', gooMaleRape, 1);
      }
      if (liveData.player.hasVagina()) {
        GUI.addButton(2, 'Herm Sex', victoryHermSex);
      }
    } else if (liveData.player.hasVagina()) {
      GUI.addButton(0, 'Fem.Fuck', normalFemaleRapesAGooGirl);
...
//Victory – Male
export function gooMaleRape(type = 1) {
...
//Victory – Herm
export function victoryHermSex() {
...
//FEMALE VICTORIES
//[Feeder perk]
export function victoryRapeAGooGalAsFeeder() {
  liveData.gameFlags[FLAG.TIMES_FUCKED_NORMAL_GOOS]++;
  GUI.clearOutput();
  GUI.outputText(
  ...
  if (liveData.player.hasVagina())
    GUI.outputText(
      'Even without stimulation, your clit practically vibrates under your climax, squeezing your body to its limits until everything fades into a pearl haze and you lose yourself to the euphoria.<br><br>',
    );
  else if (liveData.player.hasCock())
    if (liveData.player.hasVagina())
      GUI.outputText(
        'Even without stimulation, your cock practically vibrates under your climax, squeezing your body to its limits until everything fades into a pearl haze and you lose yourself to the euphoria.<br><br>',
      );
  // else if (liveData.player.hasVagina())
  //   GUI.outputText(
  //     'Even without stimulation, your tingling anus practically vibrates under your climax, squeezing to its limits until everything fades into a pearl haze and you lose yourself to the euphoria.<br><br>',
  //   );
...
//[Exhibitionist Perk]
export function exhibitionismGooGirlVictoryRape() {
  liveData.gameFlags[FLAG.TIMES_FUCKED_NORMAL_GOOS]++;
  GUI.clearOutput();
  GUI.outputText(
...
//[Normal Female]
export function normalFemaleRapesAGooGirl() {
  liveData.gameFlags[FLAG.TIMES_FUCKED_NORMAL_GOOS]++;
  GUI.clearOutput();
  GUI.outputText(
...
// CORE DROP
export function coreDropChance() {
  if (
    UTIL.rand(4) == 0 &&
...
export function layBeeEggsInGoo() {
  GUI.clearOutput();
  GUI.outputText(
...
// Loss Selector
export function getBeatByGooGirl() {
  //if (doSFWloss()) return; //No rape in SFW mode.
  liveData.gameFlags[FLAG.TIMES_FUCKED_NORMAL_GOOS]++;
  liveData.gameFlags[FLAG.GOOGIRL_CONSECUTIVE_LOSSES]++;

  if (liveData.gameFlags[FLAG.GOOGIRL_CONSECUTIVE_LOSSES] >= 5 && liveData.player.gooScore() >= 4) gooGirlBadEnd();
  else if (liveData.player.gender == 0) genderlessLoseToGooGal();
  else if (liveData.player.gender == 1) dudeLoseToGooGal();
  else if (liveData.player.gender == 2) femaleLoseToGooGal();
  else hermLoseToGooGal();
}

//Defeat – Neuter
export function genderlessLoseToGooGal() {
  GUI.clearOutput();
  GUI.outputText(
...
//Defeat – Male
export function dudeLoseToGooGal() {
  GUI.clearOutput();
  const x = liveData.player.biggestCockIndex();
  GUI.outputText(
...
//Defeat – Herm
export function hermLoseToGooGal() {
  const x = liveData.player.biggestCockIndex();
  GUI.clearOutput();
  GUI.outputText(
...
//Defeat – Female
export function femaleLoseToGooGal() {
  GUI.clearOutput();
  GUI.outputText(
...
export function gooGirlBadEnd() {
  ...
  //[Next]
  GUI.doNext(gooGirlBadEnd2);
}

export function gooGirlBadEnd2() {
  GUI.clearOutput();
