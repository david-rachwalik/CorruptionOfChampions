...
export function encounterSlime() {
  GUI.clearOutput();
...
export function greenSlimeWin() {
  ...
  if (liveData.player.findStatusEffect(StatusEffects.Feeder) >= 0 && liveData.gameFlags[FLAG.SFW_MODE] <= 0) {
    ...
    if (liveData.player.lust >= 33 && liveData.player.gender > 0) {
      ...
      GUI.addButton(0, 'B.Feed', rapeOozeWithMilk);
      GUI.addButton(1, 'Rape', slimeVictoryRape);
    ...
    //Rapes not on the table.
      ...
      GUI.doYesNo(rapeOozeWithMilk, COMBAT.cleanupAfterCombat);
    }
  }
  //Not a breastfeeder
    ...
    GUI.doYesNo(slimeVictoryRape, COMBAT.cleanupAfterCombat);
  }
  COMBAT.cleanupAfterCombat();
}

export function slimeVictoryRape() {
  //Service for lower corruption
  if (liveData.player.cor <= 33) {
    GUI.clearOutput();
    if (liveData.player.lust < 60) serviceLowCorruption();
  ...
  //higher corruption raaaaeeeep
    ...
    GUI.clearOutput();
    //male or futa
    if (liveData.player.gender == 1 || (liveData.player.gender == 3 && UTIL.rand(2) == 0)) maleRapesOoze();
    //female or futa
    else if (liveData.player.gender > 0) femaleRapesOoze();
  }
}

export function rapeOozeWithMilk() {
  GUI.clearOutput();
  GUI.outputText(
  ...
  //[If Genderless-
    ...
    GUI.outputText(
      'caressing the blank spot where your genitalia should be' +
        liveData.player.clothedOrNaked(' despite your ' + liveData.player.armorName + ' still being in place') +
        '. ',
    );
    GUI.outputText('<br><br>');
    GUI.outputText(
      'The barrage of pleasurable feelings causes you to fall over onto your ' + liveData.player.buttDescript() + ' and just soak in them, your hands ',
    );
  }
  ...
  GUI.outputText(
    "Now empty, you leave the ooze in the pile it's in and walk away, feeling somewhat disappointed by the lackluster ending of it all, but overall satisfied with the fact that you've nursed.<br><br>",
  );
...
export function femaleRapesOoze() {
...
export function maleRapesOoze() {
  GUI.outputText(
    'Noting your own erectness, you decide that the unusual nature of the thing in front of you is of, at best, minor concern, and resolve to fuck it to satisfy your own desires.<br><br>',
  );
...
export function greenSlimeLose() {
  //WORMS
  /*
            if (pcCameWorms) {
...
export function slimeLoss() {
  ...
  //Everybody else
    ...
    const temp = UTIL.rand(2);
    if (temp == 0) oozeRapesYouOrally();
    if (temp == 1) oozeButtRapesYou();
  }
}

export function oozeRapesYouVaginally() {
  liveData.player.slimeFeed();
  GUI.outputText(
  ...
  if (liveData.player.cor <= 20) {
    ...
    if (liveData.player.cockTotal() > 0) {
      ...
      if (liveData.player.cockTotal() > 1)
        GUI.outputText(
          'Your ' + liveData.player.multiCockDescript() + ' climaxes as well, sending rope after rope of hot, thick cum onto your face and chest.',
        );
      //Single
      else
        GUI.outputText(
...
export function oozeRapesYouOrally() {
  liveData.player.slimeFeed();
  GUI.outputText(
  ...
  GUI.outputText(
    "a large erection shaped like a human's dick forms at the bottom of its torso.  The creature thrusts its hips forward, pushing this new appendage against your face and making its intent very clear.<br>",
  );
  //slutty
...
export function oozeButtRapesYou() {
  liveData.player.slimeFeed();
  GUI.outputText(
  ...
  if (liveData.player.vaginas.length > 0) {
    ...
    //vaginal-only orgasm
      ...
      GUI.outputText(
        'The creature finally plunges a pair of fingers into your ' + liveData.player.vaginaDescript(0) + ', the sudden shock finally pushing you to orgasm.  ',
      );
  }
  //mangasm
...
export function serviceLowCorruptionHighLust() {
  liveData.player.slimeFeed();
  GUI.outputText(
  ...
  if (liveData.player.tallness < 82) {
    ...
    GUI.outputText(
      "A light green fluid begins to coat your hand as you stroke the slime's cock, making your fingers feel warm and sending shivers of arousal through your entire body.<br><br>",
    );
    ...
    GUI.outputText(
      'After a few moments the creature slides out from under you, slinking back into the waters of the lake as you recover from the ordeal.  When your wits return, there is nothing to do but stop back by camp.<br>',
    );
  }
...
export function serviceLowCorruption() {
  GUI.outputText(
