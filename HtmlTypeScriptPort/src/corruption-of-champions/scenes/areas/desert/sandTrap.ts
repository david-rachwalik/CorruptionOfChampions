...
export class SandTrap extends Creature {
  ...
  sandTrapWait() {
    ...
    GUI.outputText(
      'Current Trap Level is ' +
        liveData.gameFlags[FLAG.TRAP_LEVEL] +
        '<br>Current Climb Status is ' +
        liveData.gameFlags[FLAG.CLIMBED_TRAP_THIS_ROUND] +
        '<br>',
    );
    GUI.outputText(
  ...
  sandTrapPheremones() {
    ...
    // Got caught
      ...
      let damage = 10 + liveData.player.lib / 10;
      GUI.outputText(
  ...
  sandTrapLoss() {
    ...
    if (liveData.player.hasCock()) GUI.doNext(cockLoseToSandTrap);
    else if (liveData.player.hasVagina()) GUI.doNext(chicksLoseToSandTrap);
    else GUI.doNext(genderlessLoseToSandTrap);
  }
}
...
export function cockWin() {
  GUI.clearOutput();
  //TODO spriteSelect(97);
  //Requires: Penis and str requirement
  let x = liveData.player.cockThatFits(80);
  ...
  if (liveData.player.cocks[x].cockLength < 8) {
    GUI.outputText(
      "<br><br>The creature's moist cunt is tight and is a perfect fit for your modest cock.  Working yourself slowly backwards and forwards, you sigh as you soon hilt yourself in the warm wet, nowhere near its full depth.",
    );
  }
  //8 inches to 35:
  ...
  GUI.outputText(
    "<br><br>The creature's tube-like opening sucks and sticks to your cock ever so slightly as you slide outwards, like an anemone grabbing at your finger",
  );
...
export function vaginaWin() {
  ...
  GUI.outputText(
    '<br><br>It takes the creature several moments to gather its wits.  "<i>Th-thank you, lion.</i>"  Glowing with satisfaction, you begin the climb out of your conquest\'s hole and back towards camp.',
  );
...
export function handjobWin() {
  ...
  if (liveData.player.gender == 1) {
    ...
    //Cock more than 16 inches:
      ...
      GUI.outputText(
        '<br><br>The sandtrap quickly comes to the realisation it needs both hands for   Still cupping your behind, it moves one oily hand up the top end of your massive shaft, while the other works the other half.  You sigh with deep satisfaction, enjoying the all-too-rare sensation of having every inch of your ' +
          liveData.player.cockDescript(0) +
  ...
  //Herm:
    ...
    //Cock 6-16 inches:
      ...
      GUI.outputText(
        '<br><br>The sandtrap quickly comes to the realisation it needs both hands for   Still cupping your behind, it moves one oily hand up the top end of your massive shaft, whilst the other works the other half.  You sigh with deep satisfaction, enjoying the all-too-rare sensation of having every inch of your ' +
          liveData.player.cockDescript(0) +
          " immersed in pleasure.  The creature screws its face up and looks away in distaste at the size of your straining dick, which is in danger of poking it in the nose.  As you enjoy the warm, slippery sensation inundating you, you find you couldn't care less what it thinks.",
      );
    }
...
//Defeat Scenes
export function cockLoseToSandTrap() {
  GUI.clearOutput();
  //TODO Sprite spriteSelect(97);
  GUI.outputText(
  ...
  GUI.outputText(
    '<br><br>"<i>You\'hhve done well for me so farhh,</i>" chuckles the sandtrap lowly, bending over you and threading one set of arms under your ',
  );
...
export function chicksLoseToSandTrap() {
  GUI.clearOutput();
  // TODO Sprite spriteSelect(97);
  GUI.outputText(
...
export function genderlessLoseToSandTrap() {
  GUI.clearOutput();
  //TODO Sprite spriteSelect(97);
  GUI.outputText(
...
export function birfSandTarps() {
  ...
  if (liveData.player.lib < 33)
    GUI.outputText(
      "<br><br>You pick yourself up wearily, flap the flytraps you have birthed away and make your way back to camp.  This whole experience has been deeply unnerving, and you vow to make sure you don't have to repeat it.",
    );
  //Libido 30-65:
...
export function encounterASandTrap() {
  GUI.clearOutput();
  //TODO Sprite spriteSelect(97);
  ...
  if (liveData.gameFlags[FLAG.TIMES_ENCOUNTERED_SAND_TRAPS] == 1) {
    ...
    GUI.menu();
    GUI.addButton(0, 'Save', saveTheSandTrap);
    // GUI.addButton(1, "Don't Save", dontSaveTheTrap)
  }
  // Later meetings
  else {
    //Standard encounter:
    ...
    if (liveData.player.spe / 10 + UTIL.rand(20) >= 16) {
      ...
      //Fight]/[Leave]
      GUI.addButton(0, 'Fight', startSandTrapFight);
    ...
    //Speed check fail:
      ...
      GUI.outputText(
        '<br><br>"<i>Only a matter of time now, little ant,</i>" it says huskily, in its fluttering, buzzing voice.  You will have to defeat it in order to escape, and before it pulls you to the bottom!',
      );
...
export function saveTheSandTrap() {
  GUI.clearOutput();
  //TODO Sprite spriteSelect(97);
  //(femininity <= 65):
  if (liveData.player.femininity <= 65) {
...
export function dontSaveTheSandTrap() {
  GUI.clearOutput();
  //TODO SPRITE spriteSelect(97);
  ...
  GUI.outputText(' cries, neck sinking into the drift.  "<i>Don\'t let me die like .. oh, very well.  Have it YOUR way!</i>"  The ');
  if (liveData.player.femininity <= 65) GUI.outputText('woman');
  else GUI.outputText('man');
...
export function startSandTrapFight() {
  liveData.gameFlags[FLAG.CLIMBED_TRAP_THIS_ROUND] = 0;
  liveData.gameFlags[FLAG.TRAP_LEVEL] = 2;
  COMBAT.startCombat(new SandTrap());
...
export function sandTrapBadEnd() {
  GUI.clearOutput();
  //TODO Sprite spriteSelect(97);
  GUI.outputText(
  ...
  GUI.menu();
  GUI.addButton(0, 'Fight', sandTrapBadEndFight);
  GUI.addButton(1, 'Desztiny', destiny);
  GUI.addButton(4, 'Leave', leaveSandTrapBadEnd);
}

//Bad End Fight:
export function sandTrapBadEndFight() {
  GUI.clearOutput();
  //TODO SPrite spriteSelect(97);
  GUI.outputText(
...
//Bad End Lose Fight;
export function loseLastFightWithSandTrap() {
  GUI.clearOutput();
  //TODO Sprite spriteSelect(97);
  GUI.outputText(
...
//Bad End Leave:
export function leaveSandTrapBadEnd() {
  GUI.clearOutput();
  //TODO Sprite spriteSelect(97);
  GUI.outputText(
  ...
  GUI.outputText(
    '<br><br>"<i>I understand, Flytrap,</i>" a calm voice reaches you from behind.  "<i>You need time to think things over and truly recognise what you are.  I know you will come back.  You always do.</i>"',
  );
...
//End the game
export function sandTrapBadEndFinale() {
  //TODO Sprite spriteSelect(97);
  GUI.outputText(
