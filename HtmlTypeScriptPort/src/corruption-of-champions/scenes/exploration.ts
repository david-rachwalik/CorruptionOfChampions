...
export function exploreMenu() {
  ...
  GUI.menu();
  GUI.addButton(0, 'Explore', tryDiscover);
  if (liveData.exploration.exploredForest > 0)
    GUI.addButton(
      ...
      null,
      null,
      'Visit the lush forest. <br><br>Recommended level: 1' + (liveData.player.level < 5 ? '<br><br>Beware of Tentacle Beasts!' : ''),
    );
...
export function genericGobImpEncounters() {
  ...
  if (UTIL.rand(10) < impGob) {
    ...
    COMBAT.startCombat(new ImpScene.Imp());
  } else {
    //A wild goblin appears!
    GUI.displaySprite('goblin');
    if (liveData.player.gender > 0)
      GUI.outputText(
        'A goblin saunters out of the bushes with a dangerous glint in her eyes.<br><br>She says, "<i>Time to get fucked, ' +
          liveData.player.mf('stud', 'slut') +
          '.</i>"',
      );
    else
      GUI.outputText(
        'A goblin saunters out of the bushes with a dangerous glint in her eyes.<br><br>She says, "<i>Time to get fuc-oh shit, you don\'t even have anything to play with! This is for wasting my time!</i>"',
      );
    ...
    COMBAT.startCombat(new GoblinScene.Goblin());
  }
}
