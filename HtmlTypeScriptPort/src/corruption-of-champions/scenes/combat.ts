...
// ACTIONS
//------------
export function battleMenu(): void {
  GUI.clearOutput();
  ...
  GUI.outputText(
    '<br>HP: ' +
      liveData.monster.HP +
      ' / ' +
      liveData.monster.maxHP() +
      ' (' +
      Math.floor((liveData.monster.HP / liveData.monster.maxHP()) * 1000) / 10 +
      '%)',
  );
...
export function tease(justText = false): void {
  //Go on!
  ...
  if (liveData.monster.findStatusEffect(liveData.StatusEffects.Blind) >= 0) {
    GUI.outputText(
      'You do your best to tease ' +
        ...
        " with your body. It doesn't work - you blinded " +
  ...
  if (liveData.player.findStatusEffect(liveData.StatusEffects.Sealed) >= 0 && liveData.player.statusEffectValue(liveData.StatusEffects.Sealed, 2) == 1) {
    GUI.outputText(
      'You do your best to tease ' +
        liveData.monster.a +
        liveData.monster.refName +
        ' with your body. Your artless twirls have no effect, as <b>your ability to tease is sealed.</b><br><br>',
    );
    return;
  }
...
export function flee(): void {
  ...
  if (liveData.monster.refName == 'lizan rogue') {
    GUI.outputText(
      "As you retreat the lizan doesn't even attempt to stop you. When you look back to see if he's still there you find nothing but the empty bog around you.",
    );
    success = true;
  ...
  //FAIL FLEE
    ...
    if (liveData.player.canFly()) {
      ...
      // >>>>>>[P] FAIL
      GUI.outputText(
        'Using your running skill, you build up a head of steam and jump, but before you can clear the ground more than a foot, your opponent latches onto you and drags you back down with a thud!',
      );
    else {
      //Nonflyer messages
      //Huge balls messages
      if (liveData.player.balls > 0 && liveData.player.ballSize >= 24) {
        if (liveData.player.ballSize < 48)
          GUI.outputText(
            'With your ' + liveData.player.ballsDescriptLight() + ' swinging ponderously beneath you, getting away is far harder than it should be. ',
          );
      ...
      if (liveData.player.biggestTitSize() >= 35 || liveData.player.buttRating >= 20 || liveData.player.hipRating >= 20) {
          ...
          if (liveData.player.hipRating >= 20) {
            GUI.outputText(
              'Your ' +
                ...
                ' forces your gait to lurch slightly side to side, which causes the fat of your ' +
                liveData.player.skinTone +
                ' ',
            );
            if (liveData.player.buttRating >= 20) GUI.outputText(liveData.player.buttDescript() + ' and ');
            GUI.outputText(
              liveData.player.chestDesc() + ' to wobble immensely, throwing you off balance and preventing you from moving quick enough to escape.',
            );
        ...
        //FOR PLAYERS WITH MASSIVE BREASTS
          ...
          if (liveData.player.hipRating >= 20) {
            GUI.outputText(
              'Your ' + liveData.player.chestDesc() + ' nearly drag along the ground while your ' + liveData.player.hipDescript() + ' swing side to side ',
            );
        ...
        //JUST DA BOOTAH
          ...
          GUI.outputText(
            'Your ' + liveData.player.skinTone + liveData.player.buttDescript() + " wobbles so heavily that you're unable to move quick enough to escape.",
          );
      }
...
export function fantasize(): void {
  ...
  if (liveData.player.armor == liveData.Items.Armor.GooArmor) {
    ...
    GUI.outputText(
      'You daydream about fucking ' +
        liveData.monster.a +
        liveData.monster.refName +
        ', feeling your balls swell with seed as you prepare to fuck ' +
        liveData.monster.himHer +
        ' full of cum.<br>',
    );
    ...
    GUI.outputText(
      'You fill your mind with perverted thoughts about ' +
        liveData.monster.a +
        liveData.monster.refName +
        ', picturing ' +
        liveData.monster.himHer +
        ' in all kinds of perverse situations with you.<br>',
    );
    lustGain = 10 + UTIL.rand(liveData.player.lib / 5 + liveData.player.cor / 8);
  }
  if (lustGain >= 20)
    GUI.outputText(
      'The fantasy is so vivid and pleasurable you wish it was happening now. You wonder if ' +
        liveData.monster.a +
        liveData.monster.refName +
        ' can tell what you were thinking.<br><br>',
    );
...
export function struggle(): void {
  ...
  switch (liveData.player.statusEffectValue(liveData.StatusEffects.Bind, 1)) {
    ...
    case BindType.BIND_TYPE_TENTACLE:
      GUI.outputText(
        'You struggle with all of your might to free yourself from the tentacles before the creature can fulfill whatever unholy desire it has for you.<br>',
      );
      if (UTIL.rand(80) < 33 + liveData.player.str / 2) {
        ...
        if (liveData.player.cocks.length > 0)
          GUI.outputText(
            'The creature continues spiraling around your cock, sending shivers up and down your body. You must escape or this creature will overwhelm you! ',
          );
        else if (liveData.player.hasVagina())
          GUI.outputText(
            'The creature continues sucking your clit and now has latched two more suckers on your nipples, amplifying your growing lust. You must escape or you will become a mere toy to this thing! ',
          );
        else
          GUI.outputText(
...
export function combatRoundOver(skipEnemy = false): boolean {
...
export function awardPlayer(nextFunc: () => void): void {
  ...
  const gemGain = Math.floor(liveData.monster.gems);
  ...
  const item = liveData.monster.dropItem();
  if (item) {
...
export function gameOver(): void {
  ...
  GUI.menu();
  const gameOverTexts = ['GAME OVER', 'Game over, man! Game over!', 'You just got Bad-Ended!', 'Your adventures have came to an end...'];
...
export function magicMenu(): void {
  ...
  GUI.menu();
  //White Spells
  if (liveData.player.spells.blind) GUI.addButton(0, 'Blind', spellBlind);
  if (liveData.player.spells.chargeWeapon) GUI.addButton(1, 'Charge Weapon', spellChargeWeapon);
  if (liveData.player.spells.whitefire) GUI.addButton(2, 'Whitefire', spellWhitefire);
  //Black Spells
  if (liveData.player.spells.arouse) GUI.addButton(5, 'Arouse', spellArouse);
...
export function spellChargeWeapon(silent: boolean): void {
  ...
  liveData.player.changeFatigue(spellCost(15), false);
  GUI.outputText(
    'You utter words of power, summoning an electrical charge around your ' +
...
export function spellMight(silent: any = undefined): void {
  // blank
}
//Special Spells
...
// M. SPECIAL
//------------
export function mentalSpecials(): void {
  GUI.menu();
  GUI.addButton(14, 'Back', battleMenu);
}
...
// SPEC UTIL
//------------
export function spellCost(cost: number): number {
  const temp = cost;
  return temp;
}
export function physicalCost(cost: number): number {
  const temp = cost;
  return temp;
}
