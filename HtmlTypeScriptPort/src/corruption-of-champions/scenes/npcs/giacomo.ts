...
export function giacomoEncounter() {
  ...
  if (liveData.gameFlags[FLAG.GIACOMO_MET] == 0) {
    ...
    GUI.outputText(
      '"<i>Been playing with creatures best left alone, I see</i>," he chuckles. "<i>Infestations of any kind are annoying, yet your plight is quite challenging given the magnitude of corrupt creatures around here. It is not the first time I have seen one infested with THOSE worms.</i>"<br><br>',
    );
    ...
    GUI.outputText(
      '"<i>Fear not!</i>" the purveyor jingles. "<i>I have something that will cure you of those little bastards. Of course, there is also a chance that it will purge your system in general. This potion is not cheap. I will trade it for 175 gems.</i>"<br><br>',
    );
    ...
    if (liveData.player.gems < 175) {
      ...
      //Remove/No
      GUI.doYesNo(wormRemoval, giacomoEncounter);
      return;
    }
  } else {
    //Normal greeting
  ...
  GUI.menu();
  GUI.addButton(0, 'Potions', potionMenu);
  GUI.addButton(1, 'Books', bookMenu);
  GUI.addButton(2, 'Erotica', eroticaMenu);
  GUI.addButton(3, 'Worm Cure', wormRemovalOffer);
...
export function firstEncounter() {
  ...
  GUI.outputText(
    '"<i>Greetings, traveler! My name is Giacomo. I am, as you can see, a humble purveyor of items, curios and other accoutrements. While I am not in a position to show you my full wares as my shop is packed on this push-cart, I do offer some small trinkets for travelers I meet.</i>"<br><br>',
  );
  GUI.outputText(
    'The merchant looks at you sharply and cracks a wide, toothy smile you find... unnerving. The merchant twists his way around to access a sack he has around his back. After a moment, he swings the sack from his back to have better access to its contents. Inquisitively, the merchant turns back to you.<br>',
  );
  GUI.outputText(
    '"<i>So stranger, be you interested in some drafts to aid you in your travels, some quick pamphlets to warn you of dangers on journeys or...</i>"<br><br>',
  );
  GUI.outputText(
    'Giacomo pauses and turns his head in both directions in a mocking gesture of paranoid observation. His little bit of theatrics does make you wonder what he is about to offer.<br>',
  );
...
export function potionMenu() {
  ...
  GUI.menu();
  GUI.addButton(0, 'Vitality T.', pitchVitailtyTincture);
  GUI.addButton(1, 'Scholars T.', pitchScholarsTea);
  //GUI.addButton(2, "Cerulean P.", pitchCeruleanPotion);
  GUI.addButton(4, 'Back', giacomoEncounter);
}
...
export function bookMenu() {
  ...
  GUI.menu();
  GUI.addButton(0, 'Dangerous Plants', pitchDangerousPlantsBook);
  GUI.addButton(1, "Traveler's Guide", pitchTravellersGuide);
  GUI.addButton(2, 'Hentai Comic', pitchHentaiComic);
  GUI.addButton(3, 'Yoga Guide', pitchYogaGuide);
  GUI.addButton(4, 'Back', giacomoEncounter);
}

export function eroticaMenu() {
  GUI.clearOutput();
  GUI.outputText("Giacomo's grin is nothing short of creepy as he offers his wares to you. What are you interested in?");
  GUI.menu();
  GUI.addButton(0, 'Dildo', pitchDildo);
  if (liveData.player.hasVagina()) GUI.addButton(1, 'Stim-Belt', pitchSelfStimulationBelt);
  if (liveData.player.hasVagina()) GUI.addButton(2, 'AN Stim-Belt', pitchAllNaturalSelfStimulationBelt);
  if (liveData.player.hasCock()) GUI.addButton(3, 'Onahole', pitchOnahole);
  if (liveData.player.hasCock()) GUI.addButton(4, 'D Onahole', pitchDeluxeOnahole);
  if (liveData.player.hasCock() && liveData.player.hasVagina()) GUI.addButton(5, 'Dual Belt', pitchDualStimulationBelt);
  if (liveData.player.hasCock() && liveData.player.hasVagina()) GUI.addButton(6, 'AN Onahole', pitchAllNaturalOnahole);
  GUI.addButton(7, 'Condom', pitchCondom);
  GUI.addButton(14, 'Back', giacomoEncounter);
}

//------------
// CONSUMABLES
//------------
export function pitchVitailtyTincture() {
  GUI.clearOutput();
  GUI.outputText(
  ...
  GUI.doYesNo(buyVitailtyTincture, potionMenu);
}
export function buyVitailtyTincture() {
...
export function pitchScholarsTea() {
  GUI.clearOutput();
  GUI.outputText(
  ...
  GUI.doYesNo(buyScholarsTea, potionMenu);
}
export function buyScholarsTea() {
  if (liveData.player.gems < 15) {
    GUI.clearOutput();
    GUI.outputText('<br><br>Giacomo sighs, indicating you need ' + UTIL.num2Text(15 - liveData.player.gems) + ' more gems to purchase this item. ');
    GUI.doNext(potionMenu);
...
// export function pitchCeruleanPotion() {
//     GUI.clearOutput()
//     GUI.outputText(
...
//     )
//     GUI.doYesNo(buyCeruleanPotion, potionMenu)
// }
// export function buyCeruleanPotion() {
...
export function pitchCondom() {
  GUI.outputText(
    'Giacomo holds up the packet and says, "<i>Ah, yes! This is a condom. Just slip it on any cocks and have it penetrate any holes. It\'s guaranteed to prevent the spread of STDs and it will help to prevent pregnancy. I must warn you that it does not completely prevent pregnancy. Rarely, it will fail. However, it will work most of the time. So, <b>two gems</b>. What do you say?</i>"',
  );
  GUI.doYesNo(buyCondom, eroticaMenu);
}
export function buyCondom() {
...
export function pitchDangerousPlantsBook() {
  ...
  GUI.doYesNo(buyDangerousPlantsBook, bookMenu);
}
export function buyDangerousPlantsBook() {
  GUI.clearOutput();
...
export function pitchTravellersGuide() {
  ...
  GUI.doYesNo(buyTravellersGuide, bookMenu);
}
export function buyTravellersGuide() {
  GUI.clearOutput();
...
export function pitchHentaiComic() {
  ...
  GUI.doYesNo(buyHentaiComic, bookMenu);
}
export function buyHentaiComic() {
  GUI.clearOutput();
  if (liveData.player.gems < 10) {
    GUI.outputText('<br><br>Giacomo sighs, indicating you need ' + UTIL.num2Text(10 - liveData.player.gems) + ' more gems to purchase this item.');
  } else {
...
export function pitchYogaGuide() {
  ...
  GUI.doYesNo(buyYogaGuide, bookMenu);
}
export function buyYogaGuide() {
  GUI.clearOutput();
...
export function pitchDildo() {
  ...
  GUI.doYesNo(buyDildo, eroticaMenu);
}
export function buyDildo() {
  GUI.clearOutput();
...
export function pitchSelfStimulationBelt() {
  ...
  GUI.doYesNo(buySelfStimulationBelt, eroticaMenu);
}
export function buySelfStimulationBelt() {
  GUI.clearOutput();
...
export function pitchAllNaturalSelfStimulationBelt() {
  ...
  if (liveData.player.hasKeyItem(liveData.KeyItems.BookDangerousPlants) >= 0 && liveData.player.inte > 39) {
    ...
    if (liveData.player.inte > 29) {
      GUI.outputText(
        "<br><br>You notice a change in Giacomo's attitude. He REALLY wants to sell you this sex toy. Something this exotic should cost much more than what he is offering.",
      );
    }
  }
  GUI.outputText('<br><br>Do you buy the All-Natural Self-Stimulation Belt?');
  GUI.doYesNo(buyAllNaturalSelfStimulationBelt, eroticaMenu);
}
export function buyAllNaturalSelfStimulationBelt() {
  GUI.clearOutput();
...
export function pitchOnahole() {
  ...
  GUI.doYesNo(buyOnahole, eroticaMenu);
}
export function buyOnahole() {
  GUI.clearOutput();
  if (liveData.player.gems < 20) {
    GUI.outputText('<br><br>Giacomo sighs, indicating you need ' + UTIL.num2Text(20 - liveData.player.gems) + ' more gems to purchase this item.');
  } else {
...
export function pitchDeluxeOnahole() {
  ...
  GUI.doYesNo(buyDeluxeOnahole, eroticaMenu);
}
export function buyDeluxeOnahole() {
  GUI.clearOutput();
...
export function pitchAllNaturalOnahole() {
  ...
  GUI.doYesNo(buyAllNaturalOnahole, eroticaMenu);
}
export function buyAllNaturalOnahole() {
  GUI.clearOutput();
...
export function pitchDualStimulationBelt() {
  ...
  GUI.doYesNo(buyDualStimulationBelt, eroticaMenu);
}
export function buyDualStimulationBelt() {
  GUI.clearOutput();
  if (liveData.player.gems < 50) {
    GUI.outputText('<br><br>Giacomo sighs, indicating you need ' + UTIL.num2Text(50 - liveData.player.gems) + ' more gems to purchase this item.');
  } else {
...
export function wormRemovalOffer() {
  GUI.clearOutput();
  GUI.outputText(
    '<br><br>"<i>Been playing with creatures best left alone, I see</i>", he chuckles, "<i>Infestations of any kind are annoying, yet your plight is quite challenging given the magnitude of corrupt creatures around here. It is not the first time I have seen one infested with THOSE worms.</i>"<br><br>',
  );
  GUI.outputText('You ask how he knows of your change and the merchant giggles heartily.<br><br>');
  GUI.outputText('"<i>Do not look at me as if I am a mystic.</i>", Giacomo heckles lightly. "<i>Your crotch is squirming.</i>"<br><br>');
  GUI.outputText('Looking down, you realize how right he is and attempt to cover yourself in embarrassment.<br><br>');
...
export function wormRemoval() {
  ...
  GUI.outputText(
    'Upon seeing the fat worm, Giacomo displays a freakish celerity by jumping off his cart, grabbing an empty container and collecting the fat worm. Regaining your senses, you look at him with inquisitive shock at what he just did.<br><br>',
  );
  GUI.outputText(
    '"<i>You have to realize that I AM a merchant, after all.</i>", he calmly replies. "<i>Hell for you is heaven for someone else. This bugger will easily fetch 10,000 gems to some noble looking for a quick buzz. Don\'t WE know better!</i>"<br><br>',
  );
  GUI.outputText(
    'The merchant puts away his prize and comes back to help you up. "<i>Here.</i>", he says as he shoves a couple of bottles into your hand. "<i>This is on the house. You probably need it after the shock of getting those things out.</i>"<br><br>',
  );
...
/*export function nightSuccubiFirstTime() {
        spriteSelect(8);
    GUI.outputText("<br>As you sleep, your rest becomes increasingly disturbed. You feel a great weight on top of you and you find it difficult to breathe. Stirred to consciousness, your eyes are greeted by an enormous pair of blue tinged breasts. The nipples are quite long and thick and are surrounded by large, round areola. A deep, feminine voice breaks the silence. \"<i>I was wondering if you would wake up.</i>\" You turn your head to the voice to see the visage of a sharp-featured, attractive woman. The woman grins mischievously and speaks again. \"<i>I was hoping that idiot, Giacomo, did not dilute the 'potion' again.</i>\" Your campfire reflects off the woman's face and her beauty contains some sharply contrasting features. The pupils of her eyes are slit like a cat's. As she grins, she bares her teeth, which contain two pairs of long and short fangs. This woman is clearly NOT human! In shock, you attempt to get up, only prompting the woman to prove her inhuman nature by grabbing your shoulders and pinning you to the ground. You see that each finger on her hand also contains a fourth joint, further proving her status. Before you can speak a word, the woman begins mocking your fear and places her face in front of yours. Her face is almost certainly demonic in nature.<br><br>");
    if (player.gender == 0) {
        GUI.outputText("She quickly moves down to your crotch... only to discover no organs down there.<br><br>");
        GUI.outputText("*record scratch*<br><br>");
        GUI.outputText("\"<i>Wait a fucking minute,</i>\" the Succubus says, \"<i>Where's your dick?!</i>\"<br><br>");
        GUI.outputText("As you state your genderless nature, the succubus hops off and from nowhere pulls out a large folder marked \"<i>Corruption of Champions-Script</i>\" and begins thumbing through the pages. After finding the page she is looking for, she reads it and looks off into the distance in disgust.<br><br>");
        GUI.outputText("\"<i>Hey Fenoxo and Dxasmodeus!!!!!!</i>\" the Succubus crows, \"<i>The goddamn script says that I should be milking someone's DICK!!! Man, futa, herm, I don't give a shit. YOUR OWN FUCKING SCRIPT SAYS I SHOULD BE MOUNTING AND MILKING A COCK!!!! THIS IS A SEX GAME!!!!!! THAT MEANS FUCKING! WHAT THE HELL AM I SUPPOSED TO FUCK???!!!</i>\"<br><br>");
        GUI.outputText("The Succubus looks at you with utter contempt, \"<i>THIS motherfucker doesn't have a DAMN thing! What am I supposed to do?! I can't exactly order a fucking Happy Meal!!!!!</i>\"<br><br>");
        GUI.outputText("Throwing the script down in an utter rage, the tantrum continues, \"<i>Goddammit! I can't believe this shit! HEY!!!!! INTERN!!!! Bring me my robe, aspirins and cancer sticks!!!!</i>\"<br><br>");
        GUI.outputText("The Succubus walks a few paces away where a plain-dressed woman with a clipboard hands the Succubus a pack of cigarettes and a small bottle of aspirin. She takes a fistful of the painkillers and immediately lights up a smoke. The Succubus takes a couple of drags off the cig and rubs her temples.<br><br>");
        GUI.outputText("\"<i>You two are killing me!</i>\" she groans in clear frustration, \"<i>I come to work for you perverts based off the promise of MORE perverts to feed from and you do THIS to me! I can't work like this!</i>\"<br><br>");
        GUI.outputText("The plain woman hands the Succubus a robe, which she crudely puts on as she storms off into the night.<br><br>");
        GUI.outputText("\"<i>I will discuss this horseshit with my agent,</i>\" the Succubus continues bitching, \"<i>THIS was NOT in my contract.</i>\"<br><br>");
        GUI.outputText("The Succubus stops, turns and points to you in derision. \"<i>And YOU! You no-cock, no-cunt having pissant! Take your ass back to the lab before they find out you escaped!!!!!</i>\"<br><br>");
        GUI.outputText("The Succubus resumes her stormy exit. You look at the bottle of Cerulean Potion and wonder if it REALLY had some psychotropics in it. What the hell just happened?!");
        flags[kFLAGS.CERULEAN_POTION_NEUTER_ATTEMPTED] = 1;
        GUI.doNext(playerMenu);
        return;
    }
    if (player.gender == 1)
        GUI.outputText("\"<i>Awwww! Did my blue skin and pointy teeth scare you?</i>\" she says in a childish voice. \"<i>Believe me stud, if I wanted to harm you, I would not have let you wake up at all. I am here because you have 'called' me.</i>\" She teases you with the empty blue bottle you bought from the merchant. \"<i>My essence is in this bottle. Any man who drinks this, I am compelled to return the pleasure by drinking his.</i>\" The demon woman reaches her skinny hand down to your crotch where you see you have become fiercely erect. The demon gently strokes your cock until you begin oozing generous amounts of your own natural lubricants. The demon takes one of her massive breasts and teases you with her fat nipples. \"<i>Open your mouth,</i>\" she demands. \"<i>Take me into your mouth as I will soon take you into mine.</i>\"<br><br>");
    else if (player.gender == 3) {
        flags[kFLAGS.UNKNOWN_FLAG_NUMBER_00111]++;
        GUI.outputText("<br>It is obvious that you have been confronted by a succubus. As the fire illuminates your captor, her grin widens broadly.<br><br>");
        GUI.outputText("\"<i>Well, well, well!</i>\" the Succubus jingles. \"<i>What have we here?! A little girl with a big cock!</i>\"<br><br>");
        ...
        GUI.outputText("\"<i>Quit whimpering,</i>\" the Succubus orders. \"<i>I hope the rumors about you futas are true. I need a good, fiery load of cum to get me going. I haven't had one in a while and as much as I LOVE men, they can only feed me so much.</i>\"<br><br>");
        GUI.outputText("You quickly try to struggle, but find the Succubus to be utterly dominating. She wraps her arms around your back and entwines her lean legs around your hips. The Succubus playfully licks your lips and grins.<br><br>");
        GUI.outputText("\"<i>You are getting your dick milked,</i>\" the Succubus says flatly, \"<i>Accept it. Trust me, when I am done, you will want more of me, anyway.</i>\"<br><br>");
        GUI.outputText("As the Succubus finishes her ultimatum, you feel churning vaginal contractions stroking your massive cock. Heavy, powerful, coordinated undulations work your dick as surely as the best handjob. You quickly moan in shock and pleasure at such rough treatment.");
    }
    dynStats("lus", 35);
    GUI.doNext(ceruleanSuccubusEncounterPart2);
    }

    export function ceruleanSuccubusEncounterPart2() {
        GUI.clearOutput();
    spriteSelect(8);
    if (player.gender == 1) {
        GUI.outputText("Your natural instincts immediately take over and you open your mouth and allow her nipple inside. Immediately, your mouth has a mind of its own as you press your head firmly into her breast and begin suckling the unnaturally long teat like a starving baby. The demon-woman laughs in satisfaction. \"<i>To think, that you believed me to do you harm!</i>\" she taunts. \"<i>Drink, little man. Feed your lust as you will soon feed mine.</i>\" Immediately, you feel her milk flood your mouth. Its taste immediately reminds you of the potion you got from Giacomo. You realize the potion was not a potion at all, but this demon's breast milk! Concerned only for your blind libido, the suction of your mouth coaxes torrents of the devil's fluid into your mouth and down your throat. She continues teasing your cock only enough to maintain your erection. In time, your stomach signals that you are full and you break the seal from her tit, making a loud 'pop'. She briefly hoses you down with milk, soaking you.<br><br>");
        GUI.outputText("The demon has a satisfied look on her face. \"<i>Did I taste good? Was I wholesome and fulfilling?</i>\" she asks. \"<i>Since you have fed from my life-milk, it is only fair that I do the same. To be fair, 'yes', I am as fierce as I look and I will leave you sore and insensible. However, I do so to pleasure you and feed myself. Accept it and be happy.</i>\" She gives you another inhumanly toothy grin and kisses you deeply. A small pang of fear still shoots through you as you feel the sharpness of her teeth. She breaks away from your lips and sighs in excitement. \"<i>Now, I FEED!</i>\" she utters jubilantly.");
    }
    else {
        GUI.outputText("\"<i>See,</i>\" the Succubus says triumphantly, \"<i>You are already enjoying it. You don't even have to hump. My cunt does all the work. Try THAT with a human woman. Good fucking luck. Half of them don't even know how to keep a prick hard once it's inside them and then they bitch because THEY didn't cum.</i>\"<br><br>");
        GUI.outputText("The fierce milking continues for several minutes until you reflexively buck your hips as your inner organs fill with the white, milky life-water the demoness demands in order to slake her thirst. Quick to take notice of your muscular reaction, the succubus snickers.<br><br>");
        GUI.outputText("\"<i>Ready to burst, are you?</i>\" the Succubus playfully challenges. \"<i>Well, then. No reason for you to hold back.</i>\"");
    }
    dynStats("tou", .3, "lib", .5, "sen", .5, "lus", 5, "cor", 1);
    GUI.doNext(ceruleanSuccubusEncounterPart3);
    }

    export function ceruleanSuccubusEncounterPart3() {
        GUI.clearOutput();
    spriteSelect(8);
    if (player.gender == 1) {
        GUI.outputText("Rotating herself into a 69 position, she seizes your throbbing member and effortlessly begins deep throating. Her thighs wrap around your head and confront you with her surprisingly hairy pussy. Her clitoris is long and erect, begging for attention and the smell of her pheromones enslaves you. You bury your face into her furry mound, ignoring your normal revulsion to such an unshaved state and begin eating her as well as any woman you have ever pleased. The demon takes your cock out of her mouth to cry in delight. \"<i>YES, LITTLE MAN!</i>\" she screams. \"<i>LICK ME! TEASE ME! LOVE MY WOMB WITH YOUR TONGUE!</i>\" She responds by clamping her mouth around the head of your penis and sucking smartly. A sharp pain in your ass signals the entry of her bony fingers working their way to your inner manhood. Finding the root of your sex easily, she mashes down to force you to cum.<br><br>");
        GUI.outputText("Finding it impossible to resist such pleasure, you immediately begin cumming. Glob after glob, stream after stream of your semen shoots into the woman's mouth. Her timed sucking ensures that she swallows each drop as you launch it into her. While you have been proud of the ability to cum in a woman for over a minute, you are wracked with both pain and pleasure as your ejaculations continue for almost ten. Once you have spent your last, the demon releases your penis to bear down on your face with her thighs and unloads a massive squirting orgasm. Your face is soaked with pussy juice as you see her cunt spasm from the force of her pleasure. The sight of her rhythmic muscles is hypnotic. She then promptly removes her finger from your ass.");
    }
    else {
        GUI.outputText("The Succubus rears up and pins your shoulders firmly to the ground. The speed of her vaginal contractions becomes impossibly fast. As you look down at the Succubus' crotch, you can clearly see her contractions working your penis despite the massive growth of fur between your legs. The pressure deep behind your cock is crushing. With one last spasmodic push, you release a thick, superhuman gout of semen deep in the Succubus' cunt. She lets out an inhuman howl of pleasure as her womb clamps down upon you like a vise. She literally squeezes each stream and drop of cum out of you.<br><br>");
        GUI.outputText("After a couple of minutes, you feel your dick weaken and begin going limp. Satisfied, you feel the Succubus' grip on your prick release and you quickly fall out of her womb. Much to your surprise, despite the massive load you released, not a single drop of cum falls out or is left on your prick. The Succubus begins rubbing herself all over, clearly in post-orgasmic ecstasy from absorbing your fluids. After a couple of chain-orgasms, the Succubus sits down next to the empty jar.<br><br>");
        GUI.outputText("She places the mouth of the jar next to one of her fat nipples and begins milking her tit into the jar, filling it back up with her fluids. She places the jar next to you and stands up.<br><br>");
        GUI.outputText("\"<i>It has been a long time since I soaked up a load that good,</i>\" she says, \"<i>Anytime you want more of that, just drink my milk again. It's only fair. I take your milk, you take mine.</i>\"<br><br>");
        GUI.outputText("She smiles and flies off, leaving you with a fresh bottle of \"Cerulean Potion\". As pleasing as the experience was, it has left you thoroughly exhausted.");
    ...
    if (player.gender == 0) {
        ...
        //REPEAT
            ...
            GUI.outputText("<br>As you begin to relax, you hear footsteps behind you, expecting the unholy interloper and pray for a better... and more understanding... encounter.<br><br>");

            GUI.outputText("You turn around, hoping for an exciting encounter only to find a rather short, plain-faced woman with horned-rim glasses and a purple dress on. She appears to be holding a stack of papers in her hand.<br><br>");

            GUI.outputText("\"<i>Ahem.</i>\", the woman says meekly, \"<i>I hate to bother you, but I was sent by the CoC writers and staff to hand you </i>\"<br><br>");

            GUI.outputText("Scratching your head, you inquire what the document is. The woman smiles shyly and hands it to you.<br><br>");

            GUI.outputText("\"<i>This is the script and production notes for Corruption of Champions,</i>\" she says with a small bit of pride, \"<i>Apparently, you need to read the highlighted sections. They are important.</i>\"<br><br>");

            GUI.outputText("You take the script, scratching your head at the surreal nature of the moment. You thumb through the pages, finding virtually every aspect of your life and encounters written as if foreseen by great mystics. The accuracy is nothing short of horrifying. You find a highlighted section that appears to be what the woman is referring to. The note is terse and outright blunt.<br><br>");

            GUI.outputText("\"<i>GENDER NEUTRAL CHARACTERS ARE BUTT-MONKEYS. IF THE ENCOUNTER INVOLVES SEX, EXPECT SOMETHING FUCKED UP TO HAPPEN INSTEAD. ACTORS WHO PLAY NEUTER CHARACTERS SHOULD EXPECT TO PLAY ONLY FOR LULZ</i>.\"<br><br>");

            GUI.outputText("The shock is overwhelming. The script basically says that you will never catch a break. As this reality drapes about you, the script disappears and you hear a cacophony of mocking laughter in all directions. The woman is nowhere to be found.<br><br>");

            GUI.outputText("As the cacophony fades, you only hear one facetiously toned word,<br><br>");

            GUI.outputText("\"<i><b>Problem?</b></i>\"");
        }
        GUI.doNext(playerMenu);
        return;
    }
    ...
    if (player.gender == 1) {
        if (player.cor < 66) {
            GUI.outputText("<br>Against your better judgment, you've again partaken of the cerulean elixir and fallen asleep. You are quickly awakened by a thick nipple being thrust into your mouth and torrents of breast milk gushing down your throat as the succubus returns to have her way with you. Looking up, your eyes meet hers as a hungry manipulative grin stretches across her blue face. Unable to control your lust, your prick jumps to attention, which prompts the demoness to ");
            if (player.isTaur()) GUI.outputText(" crouch between your legs and impale herself on your " + player.cockDescript(0) + " with a wet sound caused by her well-lubricated vulva. Y");
            else GUI.outputText(" open her womb and quickly consume your " + player.cockDescript(0) + ". She embraces you, entrapping your head in her cleavage as y");
            GUI.outputText("ou quickly feel her superhuman vaginal muscles work and stroke your " + player.cockDescript(0) + " better than any human woman or pair of hands could ever hope to accomplish. You are helpless as your unholy embrace milks the both of you in an infernal symphony of debauchery. The familiar cramp of an impending ejaculation grips you and your twitching signals the succubus of your approaching climax.<br><br>");
            if (player.isTaur()) GUI.outputText("Pushing on your forelegs, she engulfs even more of your " + player.cockDescript(0), false);
            else GUI.outputText("Almost crushing your pelvis, she wraps her legs around your body");
            GUI.outputText(" and her muscles churn mercilessly demanding that you release your 'milk' as freely as she has released hers into you. Stimulated beyond any human ability to maintain control, you bear down and release a milky flood of your own inside the succubus. Moaning in ecstasy, she ");
            if (player.isTaur()) GUI.outputText("arches under your belly as you feel your " + player.cockDescript(0) + " bending pleasurably inside her, and");
            else GUI.outputText("releases you from her grip, allowing you to finally breathe deeply, and leans back, arching high to reveal your joined genitals in the moonlight. You visibly see");
            GUI.outputText(" her contractions milking your " + player.cockDescript(0) + " as fiercely as a maid milks a cow! Another torrent of cum pushes its way out of your body and you let out a moan of pleasure and exhaustion.<br><br>");
            GUI.outputText("As you are passing out, you feel a deep kiss upon your lips from the succubus. \"You taste better each time we join. Call upon me soon, lest I take what I want from you anyway,\", says the lustful creature.<br><br>");
            GUI.outputText("Fatigue takes you and you collapse into a deep sleep. ");
        }
        else {
            GUI.outputText("<br>Knowing the succubus will come, you do not even bother trying to sleep. Instead, you prepare a little surprise for her. You briefly jerk off and start edging yourself, preparing a massive batch to unload inside her. Hopefully, she will be the one to get more than she bargained for.<br><br>", true);
            GUI.outputText("The succubus comes, as you predicted. Despite her obvious strength and size difference to you, you grab her and push her down to the ground and immediately push your angry cock into her hairy hole. The succubus, surprised and enthralled, laughs at your aggression.<br><br>");
            GUI.outputText("\"<i>I thought I was the hungry one.</i>\", she chuckles. \"I am all yours, little man. FEED ME!\"<br><br>");
            GUI.outputText("You begin bucking your ");
            if (player.isTaur()) GUI.outputText("flanks");
            else GUI.outputText("hips");
            GUI.outputText(" in the all-too-familiar rhythm, hammering away at the succubus' cunt. Impressed with your initiative, she chooses to remain submissive as you work up an impressive load of spunk. Trying with all of your might, you continue to hold off your orgasm, painfully, as you continue stimulating your inhuman lover.<br><br>");
            GUI.outputText("However, she senses your control and immediately brings her own muscles into the little love game. With one good squeeze, she breaks down any control and resistance you have. Sensing you are about to explode, she ");
            if (player.isTaur()) GUI.outputText("pushes on your forelegs, impaling herself even deeper on your " + player.cockDescript(0), false);
            else GUI.outputText("wraps her legs around your hips and bears down");
            GUI.outputText(". You feel the head of your prick push past the dilated opening in her cervix, which immediately contracts around your head. Your penis is literally trapped and caught in her womb!<br><br>");
            GUI.outputText("Groaning loudly, long muscle spasms release the painfully stored semen into the deepest parts of the succubus. The sensation of your hot cum so deep inside her body triggers her peak. ");

            if (player.isTaur()) GUI.outputText("She moans inhumanly, and reflexively digs her claws into your forelegs. Searing with lust, the pain means little to you as you only feel the sensation of your body forcing your fluids out of your body and into hers. You press your " + player.cockDescript(0) + " into her");
            else GUI.outputText("She embraces you, moaning inhumanly, and reflexively digs her claws into your back. Searing with lust, the pain means little to you as you only feel the sensation of your body forcing your fluids out of your body and into hers. You slam your pelvis into hers");
            GUI.outputText(", as if to force yourself to cum harder than you already are capable of, prompting an equally pleasurable reaction from her.<br><br>");
            GUI.outputText("For the first time since you have had your 'visits', the succubus appears winded. Without another word, her muscles release your manhood, which she quickly licks clean of your intermingled juices. She tongues your face in lustful approval and flies away. You quickly fall asleep, utterly spent. ");
            dynStats("lib", -1);
        }
    }
    else if (player.gender == 3) {
        ...
        if (flags[kFLAGS.CERULEAN_POTION_BAD_END_FUTA_COUNTER] > 10 && player.cor > 50) {
            ...
            GUI.outputText("After a short time, you wake up sore from head to toe. The Succubus is sitting next to you with an utterly satisfied look on her face.<br><br>");

            GUI.outputText("\"<i>Well, this was unexpected.</i>\", she says, \"<i>I did not expect you to change. Normally, men are susceptible to my milk, but apparently it works on herms, too.</i>\"<br><br>");
            ...
            if (player.horseScore() + player.dogScore() + player.nagaScore() + player.goblinScore() + player.sharkScore() + player.minoScore() + player.cowScore() > 5) GUI.outputText(" All of the other corruptions and changes to your body have faded away as your new form has taken shape.");
            GUI.outputText(" As the reality soaks in, you feel a sharp pain in your stomach and your cock. You NEED to feed. Cum, milk, it doesn't matter. Likewise, your dick is hard and you need to cum. Despite your need, you cannot bring yourself to masturbate. You want ANOTHER'S attention.<br><br>");

            GUI.outputText("Without further acknowledgement, you take up your on your demonic wings to find your first \"meal\". The Succubus left behind simply giggles as she sees another of her kind take up the night in search for more meals and pleasure.");
            getGame().gameOver();
            return;
        }
        else {
            ...
            GUI.outputText("<br>As you begin to relax from a long day of adventuring, the succubus returns and lands squarely in your lap, just missing your throbbing erection. The succubus growls in arousal as she thrusts one of her fat nipples into your mouth. Reflexively, you begin suckling the teat with neither shame nor restraint. Milk floods into your mouth as you sense the weight of the succubus descend upon your cock. The familiar warmth and snugness of her cunt greet your hungry prick as her muscles begin the savory churning to coax your body into producing the 'milk' she needs to sate her own hunger. Your eyes roll back into your head as the torrent of milk pouring down your throat increases the sensitivity in all of your organs, compelling your hips to reflexively buck to press your dick deeper.<br><br>");

            GUI.outputText("The Succubus restrains you without missing a stroke or disrupting your breastfeeding as the pangs of orgasmic pleasure swell up at the base of your cock. You wrap your arms forcefully around the succubus as you bear down upon your crotch, releasing the painfully stockpiled load of lust into the demoness' cunt for her own sustenance. The succubus lets out an inhuman howl of pleasure as her own orgasm begins to crush your cock, draining every last drop out of you.<br><br>");

            GUI.outputText("Your consciousness begins to fade as the orgasm subsides. The succubus pops her tit out of your mouth and squeezes more of her essence into the empty bottle. She licks your lips and flies away just in time for you to pass out. ");
    ...
    GUI.outputText("<br>");
    player.orgasm();
    dynStats("str", UTIL.rand(2),"tou", UTIL.rand(2), "spe", UTIL.rand(2), "int", UTIL.rand(2), "cor", 1);
    inventory.takeItem(consumables.CERUL_P, playerMenu);
    }
    }*/
