...
export function lumiEncounter() {
  ...
  if (liveData.gameFlags[FLAG.LUMI_MET] == 0) {
    ...
    GUI.doYesNo(lumiLabChoices, Camp.returnToCampUseOneHour);
  } else {
    //placeholder text for outside the cathedral
    GUI.outputText(
...
export function lumiLabChoices() {
  ...
  GUI.menu();
  GUI.addButton(0, 'Shop', lumiShop);
  //if (lumiEnhance(true)) GUI.addButton(1, "Enhance", lumiEnhance); //NOT YET IMPLEMENTED
...
export function lumiShop() {
  ...
  GUI.clearOutput();
  GUI.outputText(
    'You ask Lumi if you can see her potions. She smiles at you and pulls out several bottles from her desk and shows them to you.<br><br>"<i>Gawantied qwality, made by Lumi herself,</i>" she says proudly.<br><br>',
  );
...
//Lust Draft
export function lumiLustDraftPitch() {
  GUI.clearOutput();
  GUI.outputText(
...
export function lumiPurchase(itype: Item) {
  ...
  if (liveData.player.gems >= cost) {
    ...
    GUI.outputText(
      'You go to pay Lumi the gems, but then you realize that you don\'t have enough. Lumi seems to know what happened and tells you "<i>Ok, is dere somefing you want to buy that you can affowd?</i>"<br><br>',
    );
...
/*lumiEnhance = function (justCheck) {
