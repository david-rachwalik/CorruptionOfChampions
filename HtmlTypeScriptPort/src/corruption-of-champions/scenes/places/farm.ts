...
export function farmExploreEncounter() {
  ...
  if (liveData.gameFlags[FLAG.MET_WHITNEY] < 2) {
    if (liveData.gameFlags[FLAG.MET_WHITNEY] == 0) {
      ...
      GUI.outputText(
        "You stumble across Whitney's farm again. The anthropomorphic canine woman gives you a friendly wave and tosses you another Canine pepper.<br><br>",
      );
      if (liveData.gameFlags[FLAG.MET_WHITNEY] == 2)
        GUI.outputText(
          "<b>You've been to the farm enough to easily find it. You can return by selecting it from the places menu (and will no longer encounter it during random lake exploration)</b>.<br><br>",
        );
    }
  ...
  //Repeat Offender
    ...
    if (liveData.gameFlags[FLAG.KELT_BREAK_LEVEL] >= 4 && liveData.gameFlags[FLAG.WHITNEY_FLIPPED_OUT_OVER_KELLY] == 0) {
      ...
      GUI.outputText(
        '<br><br>You hold your hands up, knowing full-well what the angry bitch is on about. "<i>I didn\'t do anything he wouldn\'t have done to me.</i>"',
      );
      GUI.outputText(
      ...
      GUI.outputText(
        "<br><br>She spins about and trots back to her farm, picking up a pitchfork as she goes. It looks like you won't have access to the farm any more, at least until you come up with a way to deal with Whitney.",
      );
    ...
    if (liveData.gameFlags[FLAG.WHITNEY_FLIPPED_OUT_OVER_KELLY] == 0) {
      GUI.addButton(0, 'Explore', exploreFarm);
      GUI.addButton(1, 'Talk', WhitneyScene.talkWhitney);
      GUI.addButton(2, 'Work', workFarm);
