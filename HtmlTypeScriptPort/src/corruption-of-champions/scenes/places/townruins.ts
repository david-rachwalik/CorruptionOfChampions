...
export function firstExploration() {
  ...
  GUI.menu();
  GUI.addButton(0, 'Yes', exploreAmilyVillage);
  GUI.addButton(1, 'No', dontExploreAmilyVillage);
}

export function exploreAmilyVillage() {
  GUI.clearOutput();
...
export function dontExploreAmilyVillage() {
  GUI.clearOutput();
  GUI.outputText(
...
export function exploreVillageRuin() {
  GUI.clearOutput();
  ...
  if (
    UTIL.rand(5) == 0 &&
    (liveData.gameFlags[FLAG.HAS_ARMOR_RACK] == 0 || liveData.gameFlags[FLAG.HAS_WEAPON_RACK] == 0 || liveData.gameFlags[FLAG.HAS_EQUIPMENT_RACK] == 0)
  ) {
    let rack = 0;
    const rackArray: number[] = [];
    if (liveData.gameFlags[FLAG.HAS_ARMOR_RACK] == 0) rackArray[rackArray.length] = 0;
    if (liveData.gameFlags[FLAG.HAS_WEAPON_RACK] == 0) rackArray[rackArray.length] = 1;
    if (liveData.gameFlags[FLAG.HAS_EQUIPMENT_RACK] == 0) rackArray[rackArray.length] = 2;
